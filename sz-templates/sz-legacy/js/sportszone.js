...
jq(document).ready( function() {
	...
	sz_init_objects( objects );

	/* @mention Compose Scrolling */
	if ( $whats_new.length && sz_get_querystring('r') ) {
	...
	jq('#aw-whats-new-submit').on( 'click', function() {
		...
		firstrow = jq( '#sportszone ul.activity-list li' ).first();
		...
		post_data = jq.extend( {
			...
			'_sz_as_nonce': jq('#_sz_as_nonce').val() || ''
		}, inputs );
		...
		jq.post( ajaxurl, post_data, function( response ) {
			...
			if ( response[0] + response[1] === '-1' ) {
				...
				if ( 0 !== jq('#latest-update').length ) {
					...
					u += '<a href="' + v + '" rel="nofollow">' + SZ_DTheme.view + '</a>';
	...
	jq('div.activity-type-tabs').on( 'click', function(event) {
		...
		/* Reset the page */
		jq.cookie( 'sz-activity-oldestpage', 1, {
			path: '/',
		...
		sz_activity_request(scope, filter);
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.hasClass('fav') || target.hasClass('unfav') ) {
			...
			nonce     = sz_get_query_var( '_wpnonce', target.attr( 'href' ) );
			...
			function(response) {
				...
				target.fadeOut( 200, function() {
					...
					jq(this).attr('title', 'fav' === type ? SZ_DTheme.remove_fav : SZ_DTheme.mark_as_fav);
					jq(this).fadeIn(200);
				});
				...
				if ( 'fav' === type ) {
					if ( !jq('.item-list-tabs #activity-favs-personal-li').length ) {
						if ( !jq('.item-list-tabs #activity-favorites').length ) {
							jq('.item-list-tabs ul #activity-mentions').before( '<li id="activity-favorites"><a href="#">' + SZ_DTheme.my_favs + ' <span>0</span></a></li>');
						}
		...
		if ( target.parent().hasClass('load-more') ) {
			...
			jq('#sportszone li.load-more').addClass('loading');

			if ( ! jq.cookie('sz-activity-oldestpage') ) {
				jq.cookie('sz-activity-oldestpage', 1, {
					path: '/',
					secure: ( 'https:' === window.location.protocol )
				} );
			...
			oldest_page = ( jq.cookie('sz-activity-oldestpage') * 1 ) + 1;
			just_posted = [];
			...
			load_more_search = sz_get_querystring('s');

			if ( load_more_search ) {
				load_more_args.search_terms = load_more_search;
			}

			sz_ajax_request = jq.post( ajaxurl, load_more_args,
			function(response)
			{
				jq('#sportszone li.load-more').removeClass('loading');
				jq.cookie( 'sz-activity-oldestpage', oldest_page, {
					path: '/',
					secure: ( 'https:' === window.location.protocol )
				} );
				jq('#sportszone ul.activity-list').append(response.contents);
		...
		if ( target.parent().hasClass('load-newest') ) {
			...
			jq( '#sportszone ul.activity-list' ).prepend( newest_activities );
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.attr('name') === 'ac_form_submit' ) {
			...
			// Akismet
			ak_nonce = jq('#_sz_as_nonce_' + comment_id).val();
			if ( ak_nonce ) {
				ajaxdata['_sz_as_nonce_' + comment_id] = ak_nonce;
			}
			...
			jq.post( ajaxurl, ajaxdata, function(response) {
				...
				if ( response[0] + response[1] === '-1' ) {
					...
					if ( show_all_a ) {
						show_all_a.html( SZ_DTheme.show_x_comments.replace( '%d', new_count ) );
					}
				}
		...
		if ( target.hasClass( 'spam-activity-comment' ) ) {
			...
			jq.post( ajaxurl, {
				action: 'sz_spam_activity_comment',
				'cookie': encodeURIComponent( document.cookie ),
	...
	jq( '.dir-search, .groups-members-search' ).on( 'click', function(event) {
		...
		if ( target.attr('type') === 'submit' ) {
			...
			sz_filter_request( object, jq.cookie('sz-' + object + '-filter'), jq.cookie('sz-' + object + '-scope') , 'div.' + object, search_terms, 1, jq.cookie('sz-' + object + '-extras'), null, template );
	...
	jq('div.item-list-tabs').on( 'click', function(event) {
		...
		if ( 'LI' === target[0].nodeName && !target.hasClass( 'last' ) ) {
			...
			sz_filter_request( object, filter, scope, 'div.' + object, search_terms, 1, jq.cookie('sz-' + object + '-extras') );
	...
	jq('li.filter select').change( function() {
		...
		sz_filter_request( object, filter, scope, 'div.' + object, search_terms, 1, jq.cookie('sz-' + object + '-extras'), null, template );

		return false;
	});

	/* All pagination links run through this function */
	jq('#sportszone').on( 'click', function(event) {
		var target = jq(event.target),
			el,
		...
		if ( target.parent().parent().hasClass('pagination') && !target.parent().parent().hasClass('no-ajax') ) {
			...
			if ( jq('div.dir-search input').length ) {
				...
				if ( ! search_terms.val() && sz_get_querystring( search_terms.attr( 'name' ) ) ) {
			...
			sz_filter_request( object, jq.cookie('sz-' + object + '-filter'), jq.cookie('sz-' + object + '-scope'), 'div.' + object, search_terms, page_number, jq.cookie('sz-' + object + '-extras'), caller, template );
	...
	jq('#send-invite-form').on( 'click', '#invite-list input', function() {
		...
		function(response)
		{
			...
			if ( invites_new_template ) {
				...
				// entire list
				sz_filter_request( 'invite', 'sz-invite-filter', 'sz-invite-scope', 'div.invite', false, 1, '', '', '' );
			} else {
	...
	jq('#profile-edit-form input:not(:submit), #profile-edit-form textarea, #profile-edit-form select, #signup_form input:not(:submit), #signup_form textarea, #signup_form select').change( function() {
		...
		window.onbeforeunload = function(e) {
			if ( shouldconfirm ) {
				return SZ_DTheme.unsaved_changes;
			}
		};
	});
			...
			if ( response[0] + response[1] === '-1' ) {
				...
				button.fadeOut( 100, function() {
					if ( jq(this).hasClass('accept') ) {
						...
						jq(this).html( SZ_DTheme.accepted ).contents().unwrap();
					} else {
						action_div.children('a.accept').hide();
						jq(this).html( SZ_DTheme.rejected ).contents().unwrap();
					}
				});
			}
		});
	...
	jq('#sportszone').on('click', '.group-button .leave-group', function() {
		if ( false === confirm( SZ_DTheme.leave_group_confirm ) ) {
			return false;
		}
	});
	...
	jq('#groups-dir-list').on('click', '.group-button a', function() {
		...
		// AJAX request
		if ( thelink.hasClass( 'leave-group' ) && false === confirm( SZ_DTheme.leave_group_confirm ) ) {
			return false;
		}
	...
	jq('#sportszone').on( 'click', '.pending', function() {
		return false;
	});
		...
		function(response) {
			if ( 1 === parseInt( response, 10 ) ) {
				if ( 'unstar' === link.data('star-status') ) {
					...
					link.find('.sz-screen-reader-text').text( SZ_PM_Star.strings.text_star );

					if ( 1 === SZ_PM_Star.is_single_thread ) {
						link.attr('data-sz-tooltip', SZ_PM_Star.strings.title_star );
					} else {
						link.attr('data-sz-tooltip', SZ_PM_Star.strings.title_star_thread );
					}

				} else {
					link.data('star-status', 'unstar');
					link.removeClass('message-action-star').addClass('message-action-unstar');
					link.find('.sz-screen-reader-text').text(SZ_PM_Star.strings.text_unstar);

					if ( 1 === SZ_PM_Star.is_single_thread ) {
						link.attr('data-sz-tooltip', SZ_PM_Star.strings.title_unstar );
					} else {
						link.attr('data-sz-tooltip', SZ_PM_Star.strings.title_unstar_thread );
					}
				}
			}
		});
	...
	jq('#message-threads td.bulk-select-check :checkbox').on('change', function() {
		...
		if ( box.prop('checked') ) {
			if( 'unstar' === star.data('star-status') ) {
				SZ_PM_Star.star_counter++;
			} else {
				SZ_PM_Star.unstar_counter++;
			}
		} else {
			if( 'unstar' === star.data('star-status') ) {
				SZ_PM_Star.star_counter--;
		...
		if ( SZ_PM_Star.star_counter > 0 && parseInt( SZ_PM_Star.unstar_counter, 10 ) === 0 ) {
			jq('option[value="star"]').hide();
		} else {
			jq('option[value="star"]').show();
		}

		if ( SZ_PM_Star.unstar_counter > 0 && parseInt( SZ_PM_Star.star_counter, 10 ) === 0 ) {
	...
	jq('#wp-admin-bar-logout, a.logout').on( 'click', function() {
		jq.removeCookie('sz-activity-scope', {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		});
		jq.removeCookie('sz-activity-filter', {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		});
		jq.removeCookie('sz-activity-oldestpage', {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		});
		...
		jq(objects).each( function(i) {
			jq.removeCookie('sz-' + objects[i] + '-scope', {
				path: '/',
				secure: ( 'https:' === window.location.protocol )
			} );
			jq.removeCookie('sz-' + objects[i] + '-filter', {
				path: '/',
				secure: ( 'https:' === window.location.protocol )
			} );
			jq.removeCookie('sz-' + objects[i] + '-extras', {
				path: '/',
				secure: ( 'https:' === window.location.protocol )
			} );
		});
	...
	if ( typeof wp !== 'undefined' && typeof wp.heartbeat !== 'undefined' && typeof SZ_DTheme.pulse !== 'undefined' ) {

		wp.heartbeat.interval( Number( SZ_DTheme.pulse ) );
	...
	jq( document ).on( 'heartbeat-send.sportszone', function( e, data ) {

		first_item_recorded = 0;

		// First row is default latest activity id
		if ( jq( '#sportszone ul.activity-list li' ).first().prop( 'id' ) ) {
			// getting the timestamp
			timestamp = jq( '#sportszone ul.activity-list li' ).first().prop( 'class' ).match( /date-recorded-([0-9]+)/ );
		...
		data.sz_activity_last_recorded = activity_last_recorded;

		last_recorded_search = sz_get_querystring('s');

		if ( last_recorded_search ) {
			data.sz_activity_last_recorded_search_terms = last_recorded_search;
		}
	});
	...
	jq( document ).on( 'heartbeat-tick', function( e, data ) {
		...
		if ( ! data.sz_activity_newest_activities ) {
			return;
		}

		newest_activities = data.sz_activity_newest_activities.activities + newest_activities;
		activity_last_recorded  = Number( data.sz_activity_newest_activities.last_recorded );

		if ( jq( '#sportszone ul.activity-list li' ).first().hasClass( 'load-newest' ) ) {
			return;
		}

		jq( '#sportszone ul.activity-list' ).prepend( '<li class="load-newest"><a href="#newest">' + SZ_DTheme.newest + '</a></li>' );
	});
});

/* Setup activity scope and filter based on the current cookie settings. */
function sz_init_activity() {
	if ( undefined !== jq.cookie('sz-activity-filter') && jq('#activity-filter-select').length ) {
		jq('#activity-filter-select select option[value="' + jq.cookie('sz-activity-filter') + '"]').prop( 'selected', true );
	}

	/* Activity Tab Set */
	if ( undefined !== jq.cookie('sz-activity-scope') && jq('.activity-type-tabs').length ) {
		jq('.activity-type-tabs li').each( function() {
			jq(this).removeClass('selected');
		});
		jq('#activity-' + jq.cookie('sz-activity-scope') + ', .item-list-tabs li.current').addClass('selected');
	}
}

/* Setup object scope and filter based on the current cookie settings for the object. */
function sz_init_objects(objects) {
	jq(objects).each( function(i) {
		if ( undefined !== jq.cookie('sz-' + objects[i] + '-filter') && jq('#' + objects[i] + '-order-select select').length ) {
			jq('#' + objects[i] + '-order-select select option[value="' + jq.cookie('sz-' + objects[i] + '-filter') + '"]').prop( 'selected', true );
		}

		if ( undefined !== jq.cookie('sz-' + objects[i] + '-scope') && jq('div.' + objects[i]).length ) {
			jq('.item-list-tabs li').each( function() {
				jq(this).removeClass('selected');
			});
			jq('#' + objects[i] + '-' + jq.cookie('sz-' + objects[i] + '-scope') + ', #object-nav li.current').addClass('selected');
		}
	});
}

/* Filter the current content list (groups/members/blogs/topics) */
function sz_filter_request( object, filter, scope, target, search_terms, page, extras, caller, template ) {
	if ( 'activity' === object ) {
		return false;
	}
	...
	jq.cookie( 'sz-' + object + '-scope', scope, {
		path: '/',
		secure: ( 'https:' === window.location.protocol )
	} );
	jq.cookie( 'sz-' + object + '-filter', filter, {
		path: '/',
		secure: ( 'https:' === window.location.protocol )
	} );
	jq.cookie( 'sz-' + object + '-extras', extras, {
		path: '/',
		secure: ( 'https:' === window.location.protocol )
	} );
	...
	sz_ajax_request = jq.post( ajaxurl, {
...
function sz_activity_request(scope, filter) {
	/* Save the type and filter to a session cookie */
	if ( null !== scope ) {
		jq.cookie( 'sz-activity-scope', scope, {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		} );
	}
	if ( null !== filter ) {
		jq.cookie( 'sz-activity-filter', filter, {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		} );
	}
	...
	jq('.widget_sz_activity_widget h2 span.ajax-loader').show();
	...
	function(response)
	{
		jq('.widget_sz_activity_widget h2 span.ajax-loader').hide();
...
function sz_legacy_theme_hide_comments() {
	...
	comments_divs.each( function() {
		...
		comment_lis.each( function(i) {
			...
			if ( i < comment_lis.length - 5 ) {
				...
				if ( !i ) {
					jq(this).before( '<li class="show-all"><a href="#' + parent_li.attr('id') + '/show-all/">' + SZ_DTheme.show_x_comments.replace( '%d', comment_count ) + '</a></li>' );
				}
			}
		});
...
/* Returns a querystring of BP cookies (cookies beginning with 'sz-') */
function sz_get_cookies() {
 ...
 */
function sz_get_query_var( param, url ) {
	var qs = {};
