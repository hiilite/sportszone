...
/**
 * This control let users change the order of the SportsZone
 ...
 * @since 3.0.0
 */
class SZ_Nouveau_Nav_Customize_Control extends WP_Customize_Control {
	/**
	 * @var string
	 */
	public $type = '';
	...
	public function render_content() {
		...
		$setting  = "sz_nouveau_appearance[{$this->type}_nav_order]";
		$item_nav = array();

		// It's a group
		if ( 'group' === $this->type ) {
			$guide = __( 'Customizing the Groups navigation order needs you create at least one group first.', 'sportszone' );
		...
		// It's a user!
			...
			$item_nav = sz_nouveau_member_customizer_nav();

			$guide = __( 'Drag each possible member navigation items that are listed below into the order you prefer.', 'sportszone' );
		}
		?>
		...
		<?php if ( ! empty( $item_nav ) ) : ?>
			<ul id="<?php echo esc_attr( $id ); ?>" class="ui-sortable" style="margin-top: 0px; height: 500px;" data-sz-type="<?php echo esc_attr( $this->type ); ?>">
				...
				foreach ( $item_nav as $item ) :
					$i += 1;
				?>
					<li data-sz-nav="<?php echo esc_attr( $item->slug ); ?>">
						<div class="menu-item-bar">
							<div class="menu-item-handle ui-sortable-handle">
								<span class="item-title" aria-hidden="true">
									<span class="menu-item-title"><?php echo esc_html( _sz_strip_spans_from_title( $item->name ) ); ?></span>
								</span>
							</div>
						</div>
					</li>
		...
		<?php endif; ?>
			...
			<input id="<?php echo esc_attr( 'sz_item_' . $this->type ); ?>" type="hidden" value="" data-customize-setting-link="<?php echo esc_attr( $setting ); ?>" />
