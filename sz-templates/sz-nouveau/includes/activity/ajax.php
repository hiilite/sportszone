	...
	$ajax_actions = array(
		...
		array(
			'get_single_activity_content' => array(
				'function' => 'sz_nouveau_ajax_get_single_activity_content',
				'nopriv'   => true,
			),
		),
		array(
			'activity_mark_fav' => array(
				'function' => 'sz_nouveau_ajax_mark_activity_favorite',
				'nopriv'   => false,
			),
		),
		array(
			'activity_mark_unfav' => array(
				'function' => 'sz_nouveau_ajax_unmark_activity_favorite',
				'nopriv'   => false,
			),
		),
		array(
			'activity_clear_new_mentions' => array(
				'function' => 'sz_nouveau_ajax_clear_new_mentions',
				'nopriv'   => false,
			),
		),
		array(
			'delete_activity' => array(
				'function' => 'sz_nouveau_ajax_delete_activity',
				'nopriv'   => false,
			),
		),
		array(
			'new_activity_comment' => array(
				'function' => 'sz_nouveau_ajax_new_activity_comment',
				'nopriv'   => false,
			),
		),
		array(
			'sz_nouveau_get_activity_objects' => array(
				'function' => 'sz_nouveau_ajax_get_activity_objects',
				'nopriv'   => false,
			),
		),
		array(
			'post_update' => array(
				'function' => 'sz_nouveau_ajax_post_update',
				'nopriv'   => false,
			),
		),
		array(
			'sz_spam_activity' => array(
				'function' => 'sz_nouveau_ajax_spam_activity',
				'nopriv'   => false,
			),
		),
	);
...
function sz_nouveau_ajax_mark_activity_favorite() {
	...
	if ( sz_activity_add_user_favorite( $_POST['id'] ) ) {
		$response = array( 'content' => __( 'Remove Favorite', 'sportszone' ) );

		if ( ! sz_is_user() ) {
			$fav_count = (int) sz_get_total_favorite_count_for_user( sz_loggedin_user_id() );

			if ( 1 === $fav_count ) {
				$response['directory_tab'] = '<li id="activity-favorites" data-sz-scope="favorites" data-sz-object="activity">
					<a href="' . sz_loggedin_user_domain() . sz_get_activity_slug() . '/favorites/">
						' . esc_html__( 'My Favorites', 'sportszone' ) . '
					</a>
				</li>';
			} else {
...
function sz_nouveau_ajax_unmark_activity_favorite() {
	...
	if ( sz_activity_remove_user_favorite( $_POST['id'] ) ) {
		$response = array( 'content' => __( 'Favorite', 'sportszone' ) );

		$fav_count = (int) sz_get_total_favorite_count_for_user( sz_loggedin_user_id() );

		if ( 0 === $fav_count && ! sz_is_single_activity() ) {
			$response['no_favorite'] = '<li><div class="sz-feedback sz-messages info">
				' . __( 'Sorry, there was no activity found. Please try a different filter.', 'sportszone' ) . '
			</div></li>';
		} else {
			$response['fav_count'] = $fav_count;
		}
...
function sz_nouveau_ajax_delete_activity() {
	$response = array(
		'feedback' => sprintf(
			'<div class="sz-feedback sz-messages error">%s</div>',
			esc_html__( 'There was a problem when deleting. Please try again.', 'sportszone' )
		),
	);
	...
	// Nonce check!
	if ( empty( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'sz_activity_delete_link' ) ) {
		wp_send_json_error( $response );
	}
	...
	if ( ! empty( $_POST['is_comment'] ) ) {
		if ( ! sz_activity_delete_comment( $activity->item_id, $activity->id ) ) {
			wp_send_json_error( $response );
		}
	...
	if ( ! empty( $_POST['is_single'] ) ) {
		$response['redirect'] = sz_core_get_user_domain( $activity->user_id );
...
function sz_nouveau_ajax_get_single_activity_content() {
	$response = array(
		'feedback' => sprintf(
			'<div class="sz-feedback sz-messages error">%s</div>',
			esc_html__( 'There was a problem displaying the content. Please try again.', 'sportszone' )
		),
	);
	...
	$activity_array = sz_activity_get_specific(
		array(
	 ...
	 */
	do_action_ref_array( 'sz_nouveau_get_single_activity_content', array( &$activity ) );
...
function sz_nouveau_ajax_new_activity_comment() {
	...
	$response = array(
		'feedback' => sprintf(
			'<div class="sz-feedback sz-messages error">%s</div>',
			esc_html__( 'There was an error posting your reply. Please try again.', 'sportszone' )
		),
	);
	...
	if ( empty( $_POST['content'] ) ) {
		wp_send_json_error( array( 'feedback' => sprintf(
			'<div class="sz-feedback sz-messages error">%s</div>',
			esc_html__( 'Please do not leave the comment area blank.', 'sportszone' )
		) ) );
	}
	...
	if ( ! $comment_id ) {
		if ( ! empty( $sz->activity->errors['new_comment'] ) && is_wp_error( $sz->activity->errors['new_comment'] ) ) {
			$response = array( 'feedback' => sprintf(
				'<div class="sz-feedback sz-messages error">%s</div>',
				esc_html( $sz->activity->errors['new_comment']->get_error_message() )
			) );
			unset( $sz->activity->errors['new_comment'] );
		}
...
function sz_nouveau_ajax_get_activity_objects() {
	...
	if ( 'group' === $_POST['type'] ) {
		 ...
		 */
		$response = apply_filters( 'sz_nouveau_get_activity_custom_objects', $response, $_POST['type'] );
	}

	if ( empty( $response ) ) {
		wp_send_json_error( array( 'error' => __( 'No activites were found.', 'sportszone' ) ) );
	} else {
		wp_send_json_success( $response );
	}
}
...
function sz_nouveau_ajax_post_update() {
	...
	if ( empty( $_POST['content'] ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'Please enter some content to post.', 'sportszone' ),
			)
		);
	}
	...
	if ( 'user' === $object && sz_is_active( 'activity' ) ) {
		$activity_id = sz_activity_post_update( array( 'content' => $_POST['content'] ) );
		...
		if ( $item_id && sz_is_active( 'groups' ) ) {
			...
			if ( empty( $status ) ) {
				if ( ! empty( $sz->groups->current_group->status ) ) {
					$status = $sz->groups->current_group->status;
				} else {
	...
	if ( empty( $activity_id ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'There was a problem posting your update. Please try again.', 'sportszone' ),
			)
		);
	}

	ob_start();
	if ( sz_has_activities( array( 'include' => $activity_id, 'show_hidden' => $is_private ) ) ) {
		while ( sz_activities() ) {
			sz_the_activity();
	...
	wp_send_json_success( array(
		'id'           => $activity_id,
		'message'      => esc_html__( 'Update posted.', 'sportszone' ) . ' ' . sprintf( '<a href="%s" class="just-posted">%s</a>', esc_url( sz_activity_get_permalink( $activity_id ) ), esc_html__( 'View activity.', 'sportszone' ) ),
		'activity'     => $acivity,
		 ...
		 */
		'is_private'   => apply_filters( 'sz_nouveau_ajax_post_update_is_private', $is_private ),
		'is_directory' => sz_is_activity_directory(),
	) );
}
...
function sz_nouveau_ajax_spam_activity() {
	...
	$response = array(
		'feedback' => sprintf(
			'<div class="sz-feedback sz-messages error">%s</div>',
			esc_html__( 'There was a problem marking this activity as spam. Please try again.', 'sportszone' )
		),
	);
	...
	// Nonce check!
	if ( empty( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'sz_activity_akismet_spam_' . $activity->id ) ) {
		wp_send_json_error( $response );
	}
	...
	if ( ! empty( $_POST['is_single'] ) ) {
		$response['redirect'] = sz_core_get_user_domain( $activity->user_id );
		sz_core_add_message( __( 'This activity has been marked as spam and is no longer visible.', 'sportszone' ) );
	}
