...
class SZ_Nouveau_Members {
	...
	protected function setup_actions() {
		...
		add_action( 'sz_nouveau_enqueue_scripts', 'sz_nouveau_members_enqueue_scripts' );
	...
	protected function setup_filters() {
		...
		add_filter( 'sz_displayed_user_get_front_template', 'sz_nouveau_member_reset_front_template', 10, 1 );
	}
	...
	public function user_home_sidebar_set( $sidebar_index = '' ) {
		if ( 'sidebar-sportszone-members' !== $sidebar_index ) {
			return;
		}
		...
		// Add needed filters.
		sz_nouveau_members_add_home_widget_filters();
	}
	...
	public function user_home_sidebar_unset( $sidebar_index = '' ) {
		...
		sz_nouveau_members_remove_home_widget_filters();
	}
}
 ...
 * @since 3.0.0
 */
function sz_nouveau_members( $sz_nouveau = null ) {
	if ( is_null( $sz_nouveau ) ) {
		return;
	}

	$sz_nouveau->members = new SZ_Nouveau_Members();
}
add_action( 'sz_nouveau_includes', 'sz_nouveau_members', 5, 1 );
