 ...
 * @since 3.0.0
 */
function sz_nouveau_members_enqueue_scripts() {
	// Neutralize Ajax when using SportsZone Groups & member widgets on default front page
	if ( ! sz_is_user_front() || ! sz_nouveau_get_appearance_settings( 'user_front_page' ) ) {
		return;
	}
 ...
 */
function sz_nouveau_get_members_directory_nav_items() {
	$nav_items = array();
	...
	$nav_items['all'] = array(
		...
		'li_class'  => array(),
		'link'      => sz_get_members_directory_permalink(),
		'text'      => __( 'All Members', 'sportszone' ),
		'count'     => sz_get_total_member_count(),
		'position'  => 5,
	);
	...
	if ( is_user_logged_in() ) {
		...
		if ( sz_is_active( 'friends' ) && sz_get_total_friend_count( sz_loggedin_user_id() ) ) {
			$nav_items['personal'] = array(
				...
				'li_class'  => array(),
				'link'      => sz_loggedin_user_domain() . sz_get_friends_slug() . '/my-friends/',
				'text'      => __( 'My Friends', 'sportszone' ),
				'count'     => sz_get_total_friend_count( sz_loggedin_user_id() ),
				'position'  => 15,
			);
		}
	}
	...
	$extra_nav_items = sz_nouveau_parse_hooked_dir_nav( 'sz_members_directory_member_types', 'members', 20 );
	 ...
	 */
	return apply_filters( 'sz_nouveau_get_members_directory_nav_items', $nav_items );
}
 ...
 */
function sz_nouveau_get_members_filters( $context = '' ) {
	if ( 'group' !== $context ) {
		$filters = array(
			'active' => __( 'Last Active', 'sportszone' ),
			'newest' => __( 'Newest Registered', 'sportszone' ),
		);

		if ( sz_is_active( 'xprofile' ) ) {
			$filters['alphabetical'] = __( 'Alphabetical', 'sportszone' );
		}

		$action = 'sz_members_directory_order_options';

		if ( 'friends' === $context ) {
			$action = 'sz_member_friends_order_options';
		}
	} else {
		$filters = array(
			'last_joined'  => __( 'Newest', 'sportszone' ),
			'first_joined' => __( 'Oldest', 'sportszone' ),
		);

		if ( sz_is_active( 'activity' ) ) {
			$filters['group_activity'] = __( 'Group Activity', 'sportszone' );
		}

		$filters['alphabetical'] = __( 'Alphabetical', 'sportszone' );
		$action                  = 'sz_groups_members_order_options';
	}

	/**
	 * Recommended, filter here instead of adding an action to 'sz_members_directory_order_options'
	 *
	 * @since 3.0.0
	 *
	 ...
	 */
	$filters = apply_filters( 'sz_nouveau_get_members_filters', $filters, $context );
 ...
 */
function sz_nouveau_members_catch_button_args( $button = array() ) {
	/*
	 * Globalize the arguments so that we can use it
	 * in sz_nouveau_get_member_header_buttons().
	 */
	sz_nouveau()->members->button_args = $button;
 ...
 */
function sz_nouveau_get_hooked_member_meta() {
	ob_start();
 ...
 */
function sz_nouveau_member_reset_front_template( $templates = array() ) {
	$use_default_front = sz_nouveau_get_appearance_settings( 'user_front_page' );

	// Setting the front template happens too early, so we need this!
	if ( is_customize_preview() ) {
		$use_default_front = sz_nouveau_get_temporary_setting( 'user_front_page', $use_default_front );
	}
	...
	/**
	 * Filters the SportsZone Nouveau template hierarchy after resetting front template for members.
	 *
	 * @since 3.0.0
	 *
	 * @param array $templates Array of templates.
	 */
	return apply_filters( '_sz_nouveau_member_reset_front_template', $templates );
}
 ...
 */
function sz_nouveau_member_restrict_user_front_templates( $templates = array() ) {
 ...
 */
function sz_nouveau_member_locate_template_part( $template = '' ) {
	...
	if ( ! isset( $sz_nouveau->members->displayed_user_hierarchy ) ) {
		$sz_nouveau->members->displayed_user_hierarchy = array(
		...
		if ( sz_get_member_types() ) {
			...
			$sz_nouveau->members->displayed_user_hierarchy[] = 'members/single/%s-member-type-' . sanitize_file_name( $displayed_user_member_type ) . '.php';
		}

		// And the regular one
		$sz_nouveau->members->displayed_user_hierarchy[] = 'members/single/%s.php';
	}

	$templates = array();

	// Loop in the hierarchy to fill it for the requested template part
	foreach ( $sz_nouveau->members->displayed_user_hierarchy as $part ) {
	 ...
	 */
	return sz_locate_template( apply_filters( 'sz_nouveau_member_locate_template_part', $templates ), false, true );
}
 ...
 */
function sz_nouveau_member_get_template_part( $template = '' ) {
	$located = sz_nouveau_member_locate_template_part( $template );
 ...
 */
function sz_nouveau_members_wp_bio_info() {
	$user_settings = sz_nouveau_get_appearance_settings();
 ...
 */
function sz_nouveau_member_is_home_widgets() {
	return ( true === sz_nouveau()->members->is_user_home_sidebar );
}
 ...
 */
function sz_nouveau_member_activity_widget_overrides( $args = array() ) {
 ...
 */
function sz_nouveau_member_groups_widget_overrides( $args = array() ) {
	return array_merge( $args, array(
		'user_id' => sz_displayed_user_id(),
	) );
}
 ...
 */
function sz_nouveau_member_members_widget_overrides( $args = array() ) {
	// Do nothing for the friends widget
	if ( ! empty( $args['user_id'] ) && (int) $args['user_id'] === (int) sz_displayed_user_id() ) {
		return $args;
	}

	return array_merge( $args, array(
		'include' => sz_displayed_user_id(),
	) );
}
 ...
 * @since 3.0.0
 */
function sz_nouveau_members_add_home_widget_filters() {
	add_filter( 'sz_nouveau_activity_widget_query', 'sz_nouveau_member_activity_widget_overrides', 10, 1 );
	add_filter( 'sz_before_has_groups_parse_args', 'sz_nouveau_member_groups_widget_overrides', 10, 1 );
	add_filter( 'sz_before_has_members_parse_args', 'sz_nouveau_member_members_widget_overrides', 10, 1 );
	 ...
	 * @since 3.0.0
	 */
	do_action( 'sz_nouveau_members_add_home_widget_filters' );
}
 ...
 * @since 3.0.0
 */
function sz_nouveau_members_remove_home_widget_filters() {
	remove_filter( 'sz_nouveau_activity_widget_query', 'sz_nouveau_member_activity_widget_overrides', 10, 1 );
	remove_filter( 'sz_before_has_groups_parse_args', 'sz_nouveau_member_groups_widget_overrides', 10, 1 );
	remove_filter( 'sz_before_has_members_parse_args', 'sz_nouveau_member_members_widget_overrides', 10, 1 );
	 ...
	 * @since 3.0.0
	 */
	do_action( 'sz_nouveau_members_remove_home_widget_filters' );
}
 ...
 */
function sz_nouveau_get_wp_profile_fields( $user = null ) {
	/**
	 ...
	 */
	$contact_methods = (array) apply_filters( 'sz_nouveau_get_wp_profile_field', wp_get_user_contact_methods( $user ), $user );

	$wp_fields = array(
		'display_name'     => __( 'Name', 'sportszone' ),
		'user_description' => __( 'About Me', 'sportszone' ),
		'user_url'         => __( 'Website', 'sportszone' ),
	);
 ...
 */
function sz_nouveau_member_customizer_nav() {
	add_filter( '_sz_nouveau_member_reset_front_template', 'sz_nouveau_member_restrict_user_front_templates', 10, 1 );

	if ( sz_displayed_user_get_front_template( sportszone()->loggedin_user ) ) {
		sportszone()->members->nav->add_nav(
			array(
	...
	remove_filter( '_sz_nouveau_member_reset_front_template', 'sz_nouveau_member_restrict_user_front_templates', 10, 1 );

	$nav = sportszone()->members->nav;

	// Eventually reset the order.
	sz_nouveau_set_nav_item_order( $nav, sz_nouveau_get_appearance_settings( 'user_nav_order' ) );
