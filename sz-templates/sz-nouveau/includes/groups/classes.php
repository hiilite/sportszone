 ...
 * @since 3.0.0
 */
class SZ_Nouveau_Group_Invite_Query extends SZ_User_Query {
	/**
	...
	public static function get_inviter_ids( $user_id = 0, $group_id = 0 ) {
		...
		return $wpdb->get_col( $wpdb->prepare( "SELECT inviter_id FROM {$sz->groups->table_name_members} WHERE user_id = %d AND group_id = %d", $user_id, $group_id ) );
	}
}

/**
 * A specific Group Nav class to make it possible to set new positions for
 * sportszone()->groups->nav.
 *
 * @since 3.0.0
 */
class SZ_Nouveau_Customizer_Group_Nav extends SZ_Core_Nav {
	/**
	 * Constructor
	 *
	...
	protected function setup_nav() {
		$nav_items = array(
			'root'    => array(
				...
				'position'            => -1,
				/** This filter is documented in sz-groups/classes/class-sz-groups-component.php. */
				'default_subnav_slug' => apply_filters( 'sz_groups_default_extension', defined( 'SZ_GROUPS_DEFAULT_EXTENSION' ) ? SZ_GROUPS_DEFAULT_EXTENSION : 'home' ),
			),
			'home'    => array(
			...
			'invites' => array(
				'name'        => _x( 'Invite', 'My Group screen nav', 'sportszone' ),
				'slug'        => 'send-invites',
				'parent_slug' => $this->group->slug,
				'position'    => 70,
			),
		...
		add_filter( '_sz_nouveau_group_reset_front_template', array( $this, 'all_groups_fronts' ), 10, 1 );

		$front_template = sz_groups_get_front_template( $this->group );

		remove_filter( '_sz_nouveau_group_reset_front_template', array( $this, 'all_groups_fronts' ), 10, 1 );

		if ( ! $front_template ) {
			if ( sz_is_active( 'activity' ) ) {
				$nav_items['home']['name'] = _x( 'Home (Activity)', 'Group screen navigation title', 'sportszone' );
			} else {
				$nav_items['home']['name'] = _x( 'Home (Members)', 'Group screen navigation title', 'sportszone' );
			}
		} else {
			if ( sz_is_active( 'activity' ) ) {
			...
			$nav_items['members'] = array(
				'name'        => _x( 'Members', 'My Group screen nav', 'sportszone' ),
				'slug'        => 'members',
				'parent_slug' => $this->group->slug,
				'position'    => 60,
			);
		...
		foreach ( get_declared_classes() as $class ) {
			if ( is_subclass_of( $class, 'SZ_Group_Extension' ) ) {
				$extension = new $class;
	...
	public function get_group_nav() {
		...
		sz_nouveau_set_nav_item_order( $this, sz_nouveau_get_appearance_settings( 'group_nav_order' ), $this->group->slug );
