 ...
 * @since 3.0.0
 */
function sz_nouveau_before_groups_directory_content() {
	/**
 ...
 * @since 3.0.0
 */
function sz_nouveau_after_groups_directory_content() {
	/**
 ...
 */
function sz_nouveau_groups_create_hook( $when = '', $suffix = '' ) {
	$hook = array( 'sz' );
 ...
 * @param string $suffix Optional. Use it to add terms at the end of the hook name.
 */
function sz_nouveau_group_hook( $when = '', $suffix = '' ) {
	$hook = array( 'sz' );
 ...
 * @since 3.0.0
 */
function sz_nouveau_groups_loop_item() {
	/**
 ...
 * @since 3.0.0
 */
function sz_nouveau_groups_activity_post_form() {
	/**
	...
	if ( is_user_logged_in() && sz_group_is_member() ) {
 ...
 */
function sz_nouveau_group_invites_interface() {
	/**
 ...
 */
function sz_nouveau_groups_get_group_invites_setting() {
 ...
 */
function sz_nouveau_group_creation_tabs() {
	$sz = sportszone();
	...
	foreach ( (array) $sz->groups->group_creation_steps as $slug => $step ) {
		...
		<li<?php if ( sz_get_groups_current_create_step() === $slug ) : ?> class="current"<?php endif; ?>>
			<?php if ( $is_enabled ) : ?>
				<a href="<?php echo esc_url( sz_groups_directory_permalink() . 'create/step/' . $slug . '/' ); ?>">
 ...
 * @since 3.0.0
 */
function sz_nouveau_group_creation_screen() {
	return sz_nouveau_group_manage_screen();
}
...
function sz_nouveau_group_manage_screen() {
	...
	if ( ! sz_is_group_admin_screen( $screen_id ) && ! sz_is_group_creation_step( $screen_id ) ) {
		return;
	}
	...
	if ( ! $is_group_create ) {
		 ...
		 * @since 1.1.0
		 */
		do_action( 'sz_before_group_admin_content' );

		$core_screen = sz_nouveau_group_get_core_manage_screens( $screen_id );

	// It's a group step, get the creation screens.
	} else {
		$core_screen = sz_nouveau_group_get_core_create_screens( $screen_id );
	}
			...
			if ( ! $is_group_create ) {
				...
				if ( 'delete-group' === $screen_id ) {
					$output = sprintf(
						...
						esc_attr__( 'Delete Group', 'sportszone' )
					);
				}
			}
		}
		...
		if ( 'crop-image' !== sz_get_avatar_admin_step() ) {
			...
			if ( ! sz_is_first_group_creation_step() ) {
				$creation_step_buttons .= sprintf(
					...
					"location.href='" . esc_js( esc_url_raw( sz_get_group_creation_previous_link() ) ) . "'"
				);
			}

			if ( ! sz_is_last_group_creation_step() && ! sz_is_first_group_creation_step() ) {
				$creation_step_buttons .= sprintf(
			...
			if ( sz_is_first_group_creation_step() ) {
				$creation_step_buttons .= sprintf(
					...
					esc_attr__( 'Create Group and Continue', 'sportszone' )
				);
			}

			if ( sz_is_last_group_creation_step() ) {
				$creation_step_buttons .= sprintf(
	...
	if ( 'group-invites' === sz_get_groups_current_create_step() ) {
		printf(
			...
			sz_get_group_creation_form_action()
		);
	}
	...
	printf(
		...
		$is_group_create ? esc_attr( sz_get_new_group_id() ) : esc_attr( sz_get_group_id() )
	);
	...
	if ( ! $is_group_create ) {
		 ...
		 * @since 1.1.0
		 */
		do_action( 'sz_after_group_admin_content' );
 ...
 */
function sz_nouveau_group_header_buttons( $args = array() ) {
	$sz_nouveau = sz_nouveau();

	$output = join( ' ', sz_nouveau_get_groups_buttons( $args ) );

	// On the group's header we need to reset the group button's global.
	if ( ! empty( $sz_nouveau->groups->group_buttons ) ) {
		unset( $sz_nouveau->groups->group_buttons );
	}
	 ...
	 * @since 1.2.6
	 */
	do_action( 'sz_group_header_actions' );
	$output .= ob_get_clean();
 ...
 * @param array $args Optional. See sz_nouveau_wrapper() for the description of parameters.
 */
function sz_nouveau_groups_loop_buttons( $args = array() ) {
 ...
 * @param array $args Optional. See sz_nouveau_wrapper() for the description of parameters.
 */
function sz_nouveau_groups_invite_buttons( $args = array() ) {
	if ( empty( $GLOBALS['groups_template'] ) ) {
		return;
	}
 ...
 * @param array $args Optional. See sz_nouveau_wrapper() for the description of parameters.
 */
function sz_nouveau_groups_request_buttons( $args = array() ) {
 ...
 * @param array $args Optional. See sz_nouveau_wrapper() for the description of parameters.
 */
function sz_nouveau_groups_manage_members_buttons( $args = array() ) {
	 ...
	 * @since 1.1.0
	 */
	do_action( 'sz_group_manage_members_admin_item' );
	$output .= ob_get_clean();
	 ...
	 * @param array $args Optional. See sz_nouveau_wrapper() for the description of parameters.
	 */
	function sz_nouveau_get_groups_buttons( $args = array() ) {
		...
		if ( 'invite' === $type ) {
			...
			if ( ! is_user_logged_in() || sz_group_is_user_banned( $group ) || empty( $group->status ) ) {
				return $buttons;
			}
			...
			if ( 'button' === $button_element ) {
				$buttons['accept_invite']['button_attr']['data-sz-nonce'] = esc_url( sz_get_group_accept_invite_link() );
			} else {
				$buttons['accept_invite']['button_attr']['href'] = esc_url( sz_get_group_accept_invite_link() );
			}
			...
			if ( 'button' === $button_element ) {
				$buttons['reject_invite']['button_attr']['data-sz-nonce'] = esc_url( sz_get_group_reject_invite_link() );
			} else {
				$buttons['reject_invite']['button_attr']['href'] = esc_url( sz_get_group_reject_invite_link() );
			}
		...
		// Request button for the group's manage screen
			...
			// If button element set add nonce link to data-attr attr
			if ( 'button' === $button_element ) {
				$buttons['group_membership_accept']['button_attr']['data-sz-nonce'] = esc_url( sz_get_group_request_accept_link() );
			} else {
				$buttons['group_membership_accept']['button_attr']['href'] = esc_url( sz_get_group_request_accept_link() );
			}
			...
			// If button element set add nonce link to data-attr attr
			if ( 'button' === $button_element ) {
				$buttons['group_membership_reject']['button_attr']['data-sz-nonce'] = esc_url( sz_get_group_request_reject_link() );
			} else {
				$buttons['group_membership_reject']['button_attr']['href'] = esc_url( sz_get_group_request_reject_link() );
			}
		 ...
		 */
			...
			$buttons = array(
				...
				'promote_admin' => array(
					...
					'button_attr'       => array(
						'href'  => esc_url( sz_get_group_member_promote_admin_link() ),
						'id'    => '',
				...
				'remove_member' => array(
					...
					'parent_element'    => $parent_element,
					'button_element'    => $button_element,
					'link_text'         => __( 'Remove from group', 'sportszone' ),
					'parent_attr'       => array(
						'id'    => '',
						'class' => $parent_class,
					),
			...
			if ( 'button' === $button_element ) {
				$buttons['unban_member']['button_attr']['data-sz-nonce'] = sz_get_group_member_unban_link( $user_id );
				$buttons['ban_member']['button_attr']['data-sz-nonce'] = sz_get_group_member_ban_link( $user_id );
				$buttons['promote_mod']['button_attr']['data-sz-nonce'] = sz_get_group_member_promote_mod_link();
				$buttons['promote_admin']['button_attr']['data-sz-nonce'] = sz_get_group_member_promote_admin_link();
				$buttons['remove_member']['button_attr']['data-sz-nonce'] = sz_get_group_member_remove_link( $user_id );
			} else {
				$buttons['unban_member']['button_attr']['href'] = sz_get_group_member_unban_link( $user_id );
				$buttons['ban_member']['button_attr']['href'] = sz_get_group_member_ban_link( $user_id );
				$buttons['promote_mod']['button_attr']['href'] = sz_get_group_member_promote_mod_link();
				$buttons['promote_admin']['button_attr']['href'] = sz_get_group_member_promote_admin_link();
				$buttons['remove_member']['button_attr']['href'] = sz_get_group_member_remove_link( $user_id );
			}
		...
		// Membership button on groups loop or single group's header
			 ...
			 */
			add_filter( 'sz_get_group_join_button', 'sz_nouveau_groups_catch_button_args', 100, 1 );

			sz_get_group_join_button( $group );

			remove_filter( 'sz_get_group_join_button', 'sz_nouveau_groups_catch_button_args', 100, 1 );

			if ( ! empty( sz_nouveau()->groups->button_args ) ) {
				$button_args = sz_nouveau()->groups->button_args;
			...
			if ( 'button' === $button_element ) {
				$buttons['group_membership']['button_attr']['data-sz-nonce'] = $button_args['link_href'];
			} else {
			// Else this is an anchor so use an 'href' attr.
				$buttons['group_membership']['button_attr']['href'] = $button_args['link_href'];
			}

				unset( sz_nouveau()->groups->button_args );
			}
		}
		 ...
		 */
		$buttons_group = apply_filters( 'sz_nouveau_get_groups_buttons', $buttons, $group, $type );
		...
		if ( ! isset( sz_nouveau()->groups->group_buttons ) || ! is_a( sz_nouveau()->groups->group_buttons, 'SZ_Buttons_Group' ) ) {
			$sort = true;
			sz_nouveau()->groups->group_buttons = new SZ_Buttons_Group( $buttons_group );

		// It's not the first entry, the order is set, we simply need to update the Buttons Group
		} else {
			$sort = false;
			sz_nouveau()->groups->group_buttons->update( $buttons_group );
		}

		$return = sz_nouveau()->groups->group_buttons->get( $sort );
		...
		if ( 'manage_members' === $type && isset( $GLOBALS['members_template'] ) ) {
			if ( sz_get_group_member_is_banned() ) {
		 ...
		 */
		do_action_ref_array( 'sz_nouveau_return_groups_buttons', array( &$return, $group, $type ) );
 ...
 */
function sz_nouveau_group_has_meta() {
	return (bool) sz_nouveau_get_group_meta();
}
 ...
 * @return bool True if the group has meta. False otherwise.
 */
function sz_nouveau_group_has_meta_extra() {
	return (bool) sz_nouveau_get_hooked_group_meta();
}
 ...
 * @return string HTML Output.
 */
function sz_nouveau_group_meta() {
	$meta = sz_nouveau_get_group_meta();
	...
	if ( ! sz_is_group() ) {
		 ...
		 */
		return (object) sz_nouveau_get_group_meta();
	}
}
	 ...
	 */
	function sz_nouveau_get_group_meta() {
		/*
		...
		if ( empty( $group->template_meta ) ) {
			...
			if ( $is_group ) {
					$meta = array(
						'status'          =>  sz_get_group_type(),
						'group_type_list' =>  sz_get_group_type_list(),
						'description'     =>  sz_get_group_description(),
					);

				// Make sure to include hooked meta.
				$extra_meta = sz_nouveau_get_hooked_group_meta();
			...
			// We're in the groups loop
				...
				$meta = array(
					'status' => sz_get_group_type(),
					'count'  => sz_get_group_member_count(),
				);
			}
			 ...
			 */
			$group->template_meta = apply_filters( 'sz_nouveau_get_group_meta', $meta, $group, $is_group );
		}
 ...
 * @since 3.0.0
 */
function sz_nouveau_group_template_part() {
	/**
	...
	$sz_is_group_home = sz_is_group_home();

	if ( $sz_is_group_home && ! sz_current_user_can( 'groups_access_group' ) ) {
		/**
	...
	} elseif ( $sz_is_group_home && false !== sz_groups_get_front_template() ) {
	...
	// Otherwise use SZ_Nouveau template hierarchy
		...
		// Not the home page
		} elseif ( sz_is_group_admin_page() ) {
			$template = 'admin';
		} elseif ( sz_is_group_activity() ) {
			$template = 'activity';
		} elseif ( sz_is_group_members() ) {
			$template = 'members';
		} elseif ( sz_is_group_invites() ) {
			$template = 'send-invites';
		} elseif ( sz_is_group_membership_request() ) {
			$template = 'request-membership';
		}

		sz_nouveau_group_get_template_part( $template );
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_group_header_template_part() {
 ...
 * @return string HTML Output
 */
function sz_nouveau_groups_get_customizer_option_link() {
	return sz_nouveau_get_customizer_link(
		array(
			'object'    => 'group',
			'autofocus' => 'sz_nouveau_group_front_page',
			'text'      => __( 'Groups default front page', 'sportszone' ),
		)
	);
}
 ...
 * @return string HTML Output
 */
function sz_nouveau_groups_get_customizer_widgets_link() {
	return sz_nouveau_get_customizer_link(
		array(
			'object'    => 'group',
			'autofocus' => 'sidebar-widgets-sidebar-sportszone-groups',
 ...
 */
function sz_nouveau_group_description_excerpt( $group = null, $length = null ) {
	echo sz_nouveau_get_group_description_excerpt( $group, $length );
}
 ...
 * @return string Excerpt.
 */
function sz_nouveau_get_group_description_excerpt( $group = null, $length = null ) {
	 ...
	 */
	if ( sz_nouveau_loop_is_grid() && 'groups' === sz_current_component() ) {
		if ( ! $length ) {
			$length = 100;
		} else {
			$length = $length;
	 ...
	 */
	return apply_filters( 'sz_nouveau_get_group_description_excerpt', sz_create_excerpt( $group->description, $length ), $group );
}
