...
/**
 * Builds a group of SZ_Button
 *
 * @since 3.0.0
 */
class SZ_Buttons_Group {
	...
	public function __construct( $args = array() ) {
		foreach ( $args as $arg ) {
			...
			/*
			 * If, in sz_nouveau_get_*_buttons(), we pass through a false value for 'parent_element'
	...
	/**
	 * Get the SportsZone buttons for the group
	 *
	 * @since 3.0.0
	 *
	...
	public function get( $sort = true ) {
		...
		foreach ( $this->group as $key_button => $button ) {
			...
			$buttons[ $button['id'] ] = sz_get_button( $button );
		}
 ...
 * @uses WP_Widget
 */
class SZ_Nouveau_Object_Nav_Widget extends WP_Widget {
	/**
	 * Constructor
	 *
	 * @since 3.0.0
	...
	public function __construct() {
		$widget_ops = array(
			'description' => __( 'Displays SportsZone primary nav in the sidebar of your site. Make sure to use it as the first widget of the sidebar and only once.', 'sportszone' ),
			'classname'   => 'widget_nav_menu sportszone_object_nav',
		);

		parent::__construct(
			'sz_nouveau_sidebar_object_nav_widget',
			__( '(SportsZone) Primary navigation', 'sportszone' ),
			$widget_ops
		);
	}
	...
	public static function register_widget() {
		register_widget( 'SZ_Nouveau_Object_Nav_Widget' );
	}
	...
	public function widget( $args, $instance ) {
		if ( ! is_sportszone() || sz_is_group_create() ) {
			return;
		}

		/**
		 * Filters the nav widget args for the SZ_Nouveau_Object_Nav_Widget widget.
		 *
		 * @since 3.0.0
		 *
		 * @param array $value Array of arguments {
		 *     @param bool $sz_nouveau_widget_title Whether or not to assign a title for the widget.
		 * }
		 */
		$item_nav_args = wp_parse_args( $instance, apply_filters( 'sz_nouveau_object_nav_widget_args', array(
			'sz_nouveau_widget_title' => true,
		) ) );

		$title = '';

		if ( ! empty( $item_nav_args['sz_nouveau_widget_title'] ) ) {
			if ( sz_is_group() ) {
				$title = sz_get_current_group_name();
			} elseif ( sz_is_user() ) {
				$title = sz_get_displayed_user_fullname();
			} elseif ( sz_get_directory_title( sz_current_component() ) ) {
				$title = sz_get_directory_title( sz_current_component() );
			}
		}

		/**
		 * Filters the SZ_Nouveau_Object_Nav_Widget widget title.
		 *
		 * @since 3.0.0
		 *
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['sz_nouveau_widget_title'] = (bool) $new_instance['sz_nouveau_widget_title'];
	...
	public function form( $instance ) {
		...
		$sz_nouveau_widget_title = (bool) $instance['sz_nouveau_widget_title'];
		?>

		<p>
			<input class="checkbox" type="checkbox" <?php checked( $sz_nouveau_widget_title, true ); ?> id="<?php echo $this->get_field_id( 'sz_nouveau_widget_title' ); ?>" name="<?php echo $this->get_field_name( 'sz_nouveau_widget_title' ); ?>" />
			<label for="<?php echo $this->get_field_id( 'sz_nouveau_widget_title' ); ?>"><?php esc_html_e( 'Include navigation title', 'sportszone' ); ?></label>
		</p>
