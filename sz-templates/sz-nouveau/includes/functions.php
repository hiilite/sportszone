 ...
 */
function sz_nouveau_ajax_querystring( $query_string, $object ) {
	if ( empty( $object ) ) {
		return '';
	}
	...
	if ( ! empty( $_POST ) ) {
		...
		if ( ! empty( $post_query['data']['sz_heartbeat'] ) ) {
			$sz_heartbeat = $post_query['data']['sz_heartbeat'];

			// Remove heartbeat specific vars
			$post_query = array_diff_key(
				wp_parse_args( $sz_heartbeat, $post_query ),
				array(
					'data'      => false,
					'interval'  => false,
					'_nonce'    => false,
	...
	if ( 'all' !== $post_query['scope'] && ! sz_displayed_user_id() && ! sz_is_single_item() ) {
	 ...
	 */
	return apply_filters( 'sz_nouveau_ajax_querystring', $query_string, $object, $filter, $scope, $page, $search_terms, $extras );
}
 ...
 * @return string
 */
function sz_nouveau_ajax_button( $output = '', $button = null, $before = '', $after = '', $r = array() ) {
	if ( empty( $button->component ) ) {
		return $output;
	}

	// Custom data attribute.
	$r['button_attr']['data-sz-btn-action'] = $button->id;
	...
	$output = new SZ_Core_HTML_Element( array(
	...
	// Add span sz-screen-reader-text class
 ...
 * }
 */
function sz_nouveau_wrapper( $args = array() ) {
 /**
 ...
 * @since 3.0.0
 */
function sz_nouveau_register_sidebars() {
	...
	if ( is_customize_preview() ) {
		$default_user_front = sz_nouveau_get_temporary_setting( 'user_front_page', $default_user_front );
	...
	if ( $default_user_front ) {
		$sidebars[] = array(
			'name'          => __( 'SportsZone Member\'s Home', 'sportszone' ),
			'id'            => 'sidebar-sportszone-members',
			'description'   => __( 'Add widgets here to appear in the front page of each member of your community.', 'sportszone' ),
 ...
 * @return bool
 */
function sz_nouveau_is_object_nav_in_sidebar() {
	return is_active_widget( false, false, 'sz_nouveau_sidebar_object_nav_widget', true );
}
 ...
 * @return bool
 */
function sz_nouveau_current_user_can( $capability = '' ) {
	/**
	 * Filters whether or not the current user can perform an action for SportsZone Nouveau.
	 *
	 * @since 3.0.0
	 *
	 ...
	 */
	return apply_filters( 'sz_nouveau_current_user_can', is_user_logged_in(), $capability, sz_loggedin_user_id() );
}
 ...
 */
function sz_nouveau_parse_hooked_dir_nav( $hook = '', $component = '', $position = 99 ) {
	...
	/**
	 * Fires at the start of the output for `sz_nouveau_parse_hooked_dir_nav()`.
	 *
 ...
 * @return array
 */
function sz_nouveau_parse_hooked_options( $hook = '', $filters = array() ) {
	if ( empty( $hook ) ) {
		return $filters;
	}
	...
	/**
	 * Fires at the start of the output for `sz_nouveau_parse_hooked_options()`.
	 *
	 * This hook is variable and depends on the hook parameter passed in.
	 *
	 * @since 3.0.0
 ...
 * @param string $component The SportsZone component ID
 *
 * @return array the dropdown filters
 */
function sz_nouveau_get_component_filters( $context = '', $component = '' ) {
	$filters = array();
	...
	if ( empty( $component ) ) {
		...
		} elseif ( 'group' === $context && sz_is_group_activity() ) {
			$component = 'activity';
	...
	if ( ! sz_is_active( $component ) ) {
		return $filters;
	}

	if ( 'members' === $component ) {
		$filters = sz_nouveau_get_members_filters( $context );
	} elseif ( 'activity' === $component ) {
		$filters = sz_nouveau_get_activity_filters();
 ...
 */
function sz_nouveau_get_temporary_setting( $option = '', $retval = false ) {
	...
	if ( 'any' === $option ) {
		...
		foreach ( $temporary_setting as $key => $setting ) {
			if ( 0 !== strpos( $key, 'sz_nouveau_appearance' ) ) {
				continue;
			}
	...
	} elseif ( isset( $temporary_setting[ 'sz_nouveau_appearance[' . $option . ']' ] ) ) {
		$retval = $temporary_setting[ 'sz_nouveau_appearance[' . $option . ']' ];

	// Used when it's an ajax request
	} elseif ( isset( $_POST['customized'][ 'sz_nouveau_appearance_' . $option ] ) ) {
		$retval = $_POST['customized'][ 'sz_nouveau_appearance_' . $option ];
	}
 ...
 */
function sz_nouveau_get_appearance_settings( $option = '' ) {
	$default_args = array(
		'avatar_style'       => 0,
		'user_front_page'    => 1,
		'user_front_bio'     => 0,
	...
	$settings = sz_parse_args(
		sz_get_option( 'sz_nouveau_appearance', array() ),
		$default_args,
 ...
 */
function sz_nouveau_customizer_grid_choices( $type = 'option' ) {
	$columns = array(
		array( 'key' => '1', 'label' => __( 'One column', 'sportszone'    ), 'class' => ''      ),
		array( 'key' => '2', 'label' => __( 'Two columns', 'sportszone'   ), 'class' => 'two'   ),
		array( 'key' => '3', 'label' => __( 'Three columns', 'sportszone' ), 'class' => 'three' ),
		array( 'key' => '4', 'label' => __( 'Four columns', 'sportszone'  ), 'class' => 'four'  ),
	);
 ...
 */
function sz_nouveau_sanitize_nav_order( $option = '' ) {
 ...
 * @return string
 */
function sz_nouveau_theme_cover_image( $params = array() ) {
	if ( empty( $params ) ) {
		return '';
	}
	...
	return '
		...
		#sportszone #item-header-cover-image:after {
			clear: both;
			content: "";
			display: table;
		}
 ...
 */
function sz_nouveau_get_user_feedback( $feedback_id = '' ) {
	/**
	 * Filters the SportsZone Nouveau feedback messages.
	 *
	 ...
	 */
	$feedback_messages = apply_filters( 'sz_nouveau_feedback_messages', array(
		'registration-disabled' => array(
			'type'    => 'info',
			'message' => __( 'Member registration is currently not allowed.', 'sportszone' ),
			'before'  => 'sz_before_registration_disabled',
			'after'   => 'sz_after_registration_disabled'
		),
		'request-details' => array(
			'type'    => 'info',
			'message' => __( 'Registering for this site is easy. Just fill in the fields below, and we\'ll get a new account set up for you in no time.', 'sportszone' ),
			'before'  => false,
			'after'   => false,
		),
		'completed-confirmation' => array(
			'type'    => 'info',
			'message' => __( 'You have successfully created your account! Please log in using the username and password you have just created.', 'sportszone' ),
			'before'  => 'sz_before_registration_confirmed',
			'after'   => 'sz_after_registration_confirmed',
		),
		'directory-activity-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the community updates. Please wait.', 'sportszone' ),
		),
		'single-activity-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the update. Please wait.', 'sportszone' ),
		),
		'activity-loop-none' => array(
			'type'    => 'info',
			'message' => __( 'Sorry, there was no activity found. Please try a different filter.', 'sportszone' ),
		),
		'blogs-loop-none' => array(
			'type'    => 'info',
			'message' => __( 'Sorry, there were no sites found.', 'sportszone' ),
		),
		'blogs-no-signup' => array(
			'type'    => 'info',
			'message' => __( 'Site registration is currently disabled.', 'sportszone' ),
		),
		'directory-blogs-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the sites of the network. Please wait.', 'sportszone' ),
		),
		...
		'group-activity-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the group updates. Please wait.', 'sportszone' ),
		),
		'group-members-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Requesting the group members. Please wait.', 'sportszone' ),
		),
		'group-members-none' => array(
			'type'    => 'info',
			'message' => __( 'Sorry, there were no group members found.', 'sportszone' ),
		),
		'group-members-search-none' => array(
			'type'    => 'info',
			'message' => __( 'Sorry, there was no member of that name found in this group.', 'sportszone' ),
		),
		'group-manage-members-none' => array(
			'type'    => 'info',
			'message' => __( 'This group has no members.', 'sportszone' ),
		),
		...
		'group-requests-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the members who requested to join the group. Please wait.', 'sportszone' ),
		),
		'group-delete-warning' => array(
			'type'    => 'warning',
			'message' => __( 'WARNING: Deleting this group will completely remove ALL content associated with it. There is no way back. Please be careful with this option.', 'sportszone' ),
		),
		...
		'directory-members-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the members of your community. Please wait.', 'sportszone' ),
		),
		'members-loop-none' => array(
			'type'    => 'info',
			'message' => __( 'Sorry, no members were found.', 'sportszone' ),
		),
		'member-requests-none' => array(
			'type'    => 'info',
			'message' => __( 'You have no pending friendship requests.', 'sportszone' ),
		),
		'member-invites-none' => array(
			'type'    => 'info',
			'message' => __( 'You have no outstanding group invites.', 'sportszone' ),
		),
		'member-notifications-none' => array(
			'type'    => 'info',
			'message' => __( 'This member has no notifications.', 'sportszone' ),
		),
		'member-wp-profile-none' => array(
			'type'    => 'info',
			'message' => __( '%s did not save any profile information yet.', 'sportszone' ),
		),
		'member-delete-account' => array(
			'type'    => 'warning',
			'message' => __( 'Deleting this account will delete all of the content it has created. It will be completely unrecoverable.', 'sportszone' ),
		),
		'member-activity-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the member\'s updates. Please wait.', 'sportszone' ),
		),
		'member-blogs-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the member\'s blogs. Please wait.', 'sportszone' ),
		),
		'member-friends-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the member\'s friends. Please wait.', 'sportszone' ),
		),
		'member-groups-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading the member\'s groups. Please wait.', 'sportszone' ),
		),
		'member-notifications-loading' => array(
			'type'    => 'loading',
			'message' => __( 'Loading notifications. Please wait.', 'sportszone' ),
		),
		'member-group-invites-all' => array(
			'type'    => 'info',
			'message' => __( 'Currently every member of the community can invite you to join their groups. If you are not comfortable with it, you can always restrict group invites to your friends only.', 'sportszone' ),
		),
		'member-group-invites-friends-only' => array(
			'type'    => 'info',
			'message' => __( 'Currently only your friends can invite you to groups. Uncheck the box to allow any member to send invites.', 'sportszone' ),
		),
	) );
	 ...
	 */
	if ( 'completed-confirmation' === $feedback_id && sz_registration_needs_activation() ) {
		$feedback_messages['completed-confirmation']['message'] = __( 'You have successfully created your account! To begin using this site you will need to activate your account via the email we have just sent to your address.', 'sportszone' );
	} elseif ( 'member-notifications-none' === $feedback_id ) {
		$is_myprofile = sz_is_my_profile();

		if ( sz_is_current_action( 'unread' ) ) {
			$feedback_messages['member-notifications-none']['message'] = __( 'This member has no unread notifications.', 'sportszone' );

			if ( $is_myprofile ) {
				$feedback_messages['member-notifications-none']['message'] = __( 'You have no unread notifications.', 'sportszone' );
			}
		} elseif ( $is_myprofile ) {
			$feedback_messages['member-notifications-none']['message'] = __( 'You have no notifications.', 'sportszone' );
		}
	} elseif ( 'member-wp-profile-none' === $feedback_id && sz_is_user_profile() ) {
		$feedback_messages['member-wp-profile-none']['message'] = sprintf( $feedback_messages['member-wp-profile-none']['message'], sz_get_displayed_user_fullname() );
	} elseif ( 'member-delete-account' === $feedback_id && sz_is_my_profile() ) {
		$feedback_messages['member-delete-account']['message'] = __( 'Deleting your account will delete all of the content you have created. It will be completely irrecoverable.', 'sportszone' );
	} elseif ( 'member-activity-loading' === $feedback_id && sz_is_my_profile() ) {
		$feedback_messages['member-activity-loading']['message'] = __( 'Loading your updates. Please wait.', 'sportszone' );
	} elseif ( 'member-blogs-loading' === $feedback_id && sz_is_my_profile() ) {
		$feedback_messages['member-blogs-loading']['message'] = __( 'Loading your blogs. Please wait.', 'sportszone' );
	} elseif ( 'member-friends-loading' === $feedback_id && sz_is_my_profile() ) {
		$feedback_messages['member-friends-loading']['message'] = __( 'Loading your friends. Please wait.', 'sportszone' );
	} elseif ( 'member-groups-loading' === $feedback_id && sz_is_my_profile() ) {
		$feedback_messages['member-groups-loading']['message'] = __( 'Loading your groups. Please wait.', 'sportszone' );
	}
	 ...
	 */
	return apply_filters( 'sz_nouveau_get_user_feedback', $feedback_messages[ $feedback_id ] );
}
 ...
 */
function sz_nouveau_get_signup_fields( $section = '' ) {
	if ( empty( $section ) ) {
		return false;
	}
	 ...
	 * do_action( 'sz_account_details_fields' ) or do_action( 'blog_details' ) hooks instead.
	 *
	 * @since 3.0.0
	 *
	 * @param array $value The list of fields organized into sections.
	 */
	$fields = apply_filters( 'sz_nouveau_get_signup_fields', array(
		'account_details' => array(
			'signup_username' => array(
				'label'          => __( 'Username', 'sportszone' ),
				'required'       => true,
				'value'          => 'sz_get_signup_username_value',
				'attribute_type' => 'username',
				'type'           => 'text',
				'class'          => '',
			),
			'signup_email' => array(
				'label'          => __( 'Email Address', 'sportszone' ),
				'required'       => true,
				'value'          => 'sz_get_signup_email_value',
				'attribute_type' => 'email',
				'type'           => 'email',
				'class'          => '',
			),
			'signup_password' => array(
				'label'          => __( 'Choose a Password', 'sportszone' ),
				'required'       => true,
				'value'          => '',
			...
			'signup_password_confirm' => array(
				'label'          => __( 'Confirm Password', 'sportszone' ),
				'required'       => true,
				'value'          => '',
				'attribute_type' => 'password',
				'type'           => 'password',
		...
		'blog_details' => array(
			'signup_blog_url' => array(
				'label'          => __( 'Site URL', 'sportszone' ),
				'required'       => true,
				'value'          => 'sz_get_signup_blog_url_value',
				'attribute_type' => 'slug',
				'type'           => 'text',
				'class'          => '',
			),
			'signup_blog_title' => array(
				'label'          => __( 'Site Title', 'sportszone' ),
				'required'       => true,
				'value'          => 'sz_get_signup_blog_title_value',
				'attribute_type' => 'title',
				'type'           => 'text',
				'class'          => '',
			),
			'signup_blog_privacy_public' => array(
				'label'          => __( 'Yes', 'sportszone' ),
				'required'       => false,
				'value'          => 'public',
				'attribute_type' => '',
				'type'           => 'radio',
 ...
 */
function sz_nouveau_get_submit_button( $action = '' ) {
	if ( empty( $action ) ) {
		return false;
	}
	 ...
	 * @return array|false
	 */
	$actions = apply_filters( 'sz_nouveau_get_submit_button', array(
		'register' => array(
			'before'     => 'sz_before_registration_submit_buttons',
			'after'      => 'sz_after_registration_submit_buttons',
			'nonce'      => 'sz_new_signup',
			'attributes' => array(
				'name'  => 'signup_submit',
				'id'    => 'signup_submit',
				'value' => __( 'Complete Sign Up', 'sportszone' ),
			),
		),
		'member-profile-edit' => array(
			'before' => '',
			'after'  => '',
			'nonce'  => 'sz_xprofile_edit',
			'attributes' => array(
				'name'  => 'profile-group-edit-submit',
				'id'    => 'profile-group-edit-submit',
				'value' => __( 'Save Changes', 'sportszone' ),
			),
		),
		'member-capabilities' => array(
			'before' => 'sz_members_capabilities_account_before_submit',
			'after'  => 'sz_members_capabilities_account_after_submit',
			'nonce'  => 'capabilities',
			'attributes' => array(
				'name'  => 'capabilities-submit',
				'id'    => 'capabilities-submit',
				'value' => __( 'Save', 'sportszone' ),
			),
		),
		'member-delete-account' => array(
			'before' => 'sz_members_delete_account_before_submit',
			'after'  => 'sz_members_delete_account_after_submit',
			'nonce'  => 'delete-account',
			'attributes' => array(
				'disabled' => 'disabled',
				'name'     => 'delete-account-button',
				'id'       => 'delete-account-button',
				'value'    => __( 'Delete Account', 'sportszone' ),
			),
		),
		'members-general-settings' => array(
			'before' => 'sz_core_general_settings_before_submit',
			'after'  => 'sz_core_general_settings_after_submit',
			'nonce'  => 'sz_settings_general',
			'attributes' => array(
				'name'  => 'submit',
				'id'    => 'submit',
				'value' => __( 'Save Changes', 'sportszone' ),
		...
		'member-notifications-settings' => array(
			'before' => 'sz_members_notification_settings_before_submit',
			'after'  => 'sz_members_notification_settings_after_submit',
			'nonce'  => 'sz_settings_notifications',
			'attributes' => array(
				'name'  => 'submit',
				'id'    => 'submit',
				'value' => __( 'Save Changes', 'sportszone' ),
		...
		'members-profile-settings' => array(
			'before' => 'sz_core_xprofile_settings_before_submit',
			'after'  => 'sz_core_xprofile_settings_after_submit',
			'nonce'  => 'sz_xprofile_settings',
			'attributes' => array(
		...
		'member-group-invites' => array(
			'nonce'  => 'sz_nouveau_group_invites_settings',
			'attributes' => array(
...
/**
 * Reorder a SportsZone item nav according to a given list of nav item slugs
 *
 * @since 3.0.0
 *
 * @param object $nav         The SportsZone Item Nav object to reorder
 ...
 */
function sz_nouveau_set_nav_item_order( $nav = null, $order = array(), $parent_slug = '' ) {
