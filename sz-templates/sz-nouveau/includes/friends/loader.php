...
class SZ_Nouveau_Friends {
	...
	protected function setup_actions() {
		// Remove SportsZone action for the members loop
		remove_action( 'sz_directory_members_actions', 'sz_member_add_friend_button' );

		// Register the friends Notifications filters
		add_action( 'sz_nouveau_notifications_init_filters', array( $this, 'notification_filters' ) );
	}
	...
	public function notification_filters() {
		$notifications = array(
			array(
				...
				'label'    => __( 'Accepted friendship requests', 'sportszone' ),
				'position' => 35,
			),
			array(
				'id'       => 'friendship_request',
				'label'    => __( 'Pending friendship requests', 'sportszone' ),
				'position' => 45,
			),
		);
 ...
 * @since 3.0.0
 */
function sz_nouveau_friends( $sz_nouveau = null ) {
	if ( is_null( $sz_nouveau ) ) {
		return;
	}

	$sz_nouveau->friends = new SZ_Nouveau_Friends();
}
add_action( 'sz_nouveau_includes', 'sz_nouveau_friends', 10, 1 );
