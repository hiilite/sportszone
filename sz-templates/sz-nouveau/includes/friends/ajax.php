	...
	$ajax_actions = array(
		array(
			'friends_remove_friend' => array(
				'function' => 'sz_nouveau_ajax_addremove_friend',
				'nopriv'   => false,
			),
		),
		array(
...
function sz_nouveau_ajax_addremove_friend() {
	$response = array(
		'feedback' => sprintf(
			'<div class="sz-feedback error sz-ajax-message"><p>%s</p></div>',
	...
	if ( empty( $_POST['nonce'] ) || empty( $_POST['item_id'] ) || ! sz_is_active( 'friends' ) ) {
		wp_send_json_error( $response );
	}

	// Use default nonce
	$nonce = $_POST['nonce'];
	$check = 'sz_nouveau_friends';
	...
	if ( isset( $_POST['action'] ) && $_POST['action'] !== 'friends_accept_friendship' && $_POST['action'] !== 'friends_reject_friendship' ) {
		...
		if ( ! $user ) {
			wp_send_json_error(
				array(
					'feedback' => sprintf(
						'<div class="sz-feedback error">%s</div>',
						esc_html__( 'No member found by that ID.', 'sportszone' )
					),
				)
			);
		}
		...
		if ( ! friends_accept_friendship( $friend_id ) ) {
			wp_send_json_error(
				array(
					'feedback' => sprintf(
						'<div class="sz-feedback error">%s</div>',
						esc_html__( 'There was a problem accepting that request. Please try again.', 'sportszone' )
					),
				)
			);
		} else {
			wp_send_json_success(
				array(
					'feedback' => sprintf(
						'<div class="sz-feedback success">%s</div>',
						esc_html__( 'Friendship accepted.', 'sportszone' )
					),
					'type'     => 'success',
					'is_user'  => true,
				)
		...
		if ( ! friends_reject_friendship( $friend_id ) ) {
			wp_send_json_error(
				array(
					'feedback' => sprintf(
						'<div class="sz-feedback error">%s</div>',
						esc_html__( 'There was a problem rejecting that request. Please try again.', 'sportszone' )
					),
				)
			);
		} else {
			wp_send_json_success(
				array(
					'feedback' => sprintf(
						'<div class="sz-feedback success">%s</div>',
						esc_html__( 'Friendship rejected.', 'sportszone' )
					),
					'type'     => 'success',
					'is_user'  => true,
				)
	...
	} elseif ( 'is_friend' === SZ_Friends_Friendship::check_is_friend( sz_loggedin_user_id(), $friend_id ) ) {
		if ( ! friends_remove_friend( sz_loggedin_user_id(), $friend_id ) ) {
			$response['feedback'] = sprintf(
				'<div class="sz-feedback error">%s</div>',
				esc_html__( 'Friendship could not be cancelled.', 'sportszone' )
			);

			wp_send_json_error( $response );
		} else {
			$is_user = sz_is_my_profile();

			if ( ! $is_user ) {
				$response = array( 'contents' => sz_get_add_friend_button( $friend_id ) );
			} else {
				$response = array(
					'feedback' => sprintf(
						'<div class="sz-feedback success">%s</div>',
						esc_html__( 'Friendship cancelled.', 'sportszone' )
					),
					'type'     => 'success',
					'is_user'  => $is_user,
				);
	...
	// Trying to request friendship.
		...
		if ( ! friends_add_friend( sz_loggedin_user_id(), $friend_id ) ) {
			$response['feedback'] = sprintf(
				'<div class="sz-feedback error">%s</div>',
				esc_html__( 'Friendship could not be requested.', 'sportszone' )
			);

			wp_send_json_error( $response );
		} else {
			wp_send_json_success( array( 'contents' => sz_get_add_friend_button( $friend_id ) ) );
		}
	...
	// Trying to cancel pending request.
		...
		if ( friends_withdraw_friendship( sz_loggedin_user_id(), $friend_id ) ) {
			...
			$response['feedback'] = sprintf(
				'<div class="sz-feedback error">%s</div>',
				esc_html__( 'Friendship request could not be cancelled.', 'sportszone' )
			);
	...
	// Request already pending.
		...
		$response['feedback'] = sprintf(
			'<div class="sz-feedback error">%s</div>',
			esc_html__( 'Request Pending', 'sportszone' )
		);
