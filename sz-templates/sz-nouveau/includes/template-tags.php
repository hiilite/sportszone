 ...
 */
function sz_nouveau_hook( $pieces = array() ) {
	if ( ! $pieces ) {
		return;
	}

	$sz_prefix = reset( $pieces );
	if ( 'sz' !== $sz_prefix ) {
		array_unshift( $pieces, 'sz' );
	}

	$hook = join( '_', $pieces );

	/**
	 * Fires inside the `sz_nouveau_hook()` function.
	 *
	 * @since 3.0.0
	 */
	do_action( $hook );
 ...
 */
function sz_nouveau_plugin_hook( $suffix = '' ) {
	if ( ! $suffix ) {
		return;
	}
 ...
 * Fire friend hooks
 *
 * @todo Move this into sz-nouveau/includes/friends/template-tags.php
 ...
 * @param string The suffix of the hook.
 */
function sz_nouveau_friend_hook( $suffix = '' ) {
	if ( ! $suffix ) {
		return;
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_template_message_classes() {
	$classes = array( 'sz-feedback', 'sz-messages' );

	if ( ! empty( sz_nouveau()->template_message['message'] ) ) {
		$classes[] = 'sz-template-notice';
	}

	$classes[] = sz_nouveau_get_template_message_type();
	 ...
	 */
	function sz_nouveau_get_template_message_type() {
		$sz_nouveau = sz_nouveau();
		$type       = 'error';

		if ( ! empty( $sz_nouveau->template_message['type'] ) ) {
			$type = $sz_nouveau->template_message['type'];
		} elseif ( ! empty( $sz_nouveau->user_feedback['type'] ) ) {
			$type = $sz_nouveau->user_feedback['type'];
		}
 ...
 */
function sz_nouveau_has_template_message() {
	$sz_nouveau = sz_nouveau();

	if ( empty( $sz_nouveau->template_message['message'] ) && empty( $sz_nouveau->user_feedback ) ) {
		return false;
	}
 ...
 */
function sz_nouveau_has_dismiss_button() {
	$sz_nouveau = sz_nouveau();

	// BP template notices - set 'dismiss' true for a type in `sz_nouveau_template_notices()`
	if ( ! empty( $sz_nouveau->template_message['message'] ) && true === $sz_nouveau->template_message['dismiss'] ) {
		return true;
	}

	// Test for isset as value can be falsey.
	if ( isset( $sz_nouveau->user_feedback['dismiss'] ) ) {
		return true;
	}
 ...
 * 'clear' is tested for & used to remove cookies, if set, in sportszone-nouveau.js.
 * Currently template_notices(BP) will take $type = 'clear' if button set to true.
 *
 * @since 3.0.0
 */
function sz_nouveau_dismiss_button_type() {
	$sz_nouveau = sz_nouveau();
	$type       = 'clear';

	if ( ! empty( $sz_nouveau->user_feedback['dismiss'] ) ) {
		$type = $sz_nouveau->user_feedback['dismiss'];
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_template_message() {
	echo sz_nouveau_get_template_message();
}
	 ...
	 */
	function sz_nouveau_get_template_message() {
		$sz_nouveau = sz_nouveau();
		...
		if ( ! empty( $sz_nouveau->user_feedback['message'] ) ) {
		...
		} elseif ( ! empty( $sz_nouveau->template_message['message'] ) ) {
			/**
			 ...
			 */
			return apply_filters( 'sz_core_render_message_content', $sz_nouveau->template_message['message'], sz_nouveau_get_template_message_type() );
		}
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_template_notices() {
	$sz         = sportszone();
	$sz_nouveau = sz_nouveau();

	if ( ! empty( $sz->template_message ) ) {
		// Clone SportsZone template message to avoid altering it.
		$template_message = array( 'message' => $sz->template_message );

		if ( ! empty( $sz->template_message_type ) ) {
			$template_message['type'] = $sz->template_message_type;
		}
		...
		if ( 'sz-sitewide-notice' == $template_message['type'] ) {
			$template_message['dismiss'] = true;
		}

		$sz_nouveau->template_message = $template_message;
		sz_get_template_part( 'common/notices/template-notices' );

		// Reset just after rendering it.
		$sz_nouveau->template_message = array();
 ...
 */
function sz_nouveau_user_feedback( $feedback_id = '' ) {
	if ( ! isset( $feedback_id ) ) {
		return;
	}

	$sz_nouveau = sz_nouveau();
	$feedback   = sz_nouveau_get_user_feedback( $feedback_id );
	...
	if ( ! empty( $feedback['before'] ) ) {
		 ...
		 *
		 * This is a dynamic filter that is dependent on the "before" value provided by sz_nouveau_get_user_feedback().
		 *
		 * @since 3.0.0
		 */
	...
	$sz_nouveau->user_feedback = $feedback;

	sz_get_template_part(
		 ...
		 */
		apply_filters( 'sz_nouveau_user_feedback_template', 'common/notices/template-notices' )
	);
	...
	if ( ! empty( $feedback['after'] ) ) {
		 ...
		 * Fires before display of a feedback message to the user.
		 *
		 * This is a dynamic filter that is dependent on the "after" value provided by sz_nouveau_get_user_feedback().
		 *
		 * @since 3.0.0
		 */
	...
	$sz_nouveau->user_feedback = array();
}
...
function sz_nouveau_before_loop() {
	 ...
	 * @since 1.2.0
	 */
	do_action( "sz_before_{$component}_loop" );
}
...
function sz_nouveau_after_loop() {
	 ...
	 * @since 1.2.0
	 */
	do_action( "sz_after_{$component}_loop" );
}
...
function sz_nouveau_pagination( $position ) {
	...
	switch ( $pagination_type ) {
		case 'blogs':
			$pag_count   = sz_get_blogs_pagination_count();
			$pag_links   = sz_get_blogs_pagination_links();
			$top_hook    = 'sz_before_directory_blogs_list';
			$bottom_hook = 'sz_after_directory_blogs_list';
		...
		case 'friends':
		case 'manage-members':
			$pag_count = sz_get_members_pagination_count();
		...
		case 'groups':
			$pag_count   = sz_get_groups_pagination_count();
			$pag_links   = sz_get_groups_pagination_links();
		...
		case 'notifications':
			$pag_count   = sz_get_notifications_pagination_count();
			$pag_links   = sz_get_notifications_pagination_links();
			$top_hook    = '';
			$bottom_hook = '';
			$page_arg    = sportszone()->notifications->query_loop->pag_arg;
			break;

		case 'membership-requests':
			$pag_count   = sz_get_group_requests_pagination_count();
			$pag_links   = sz_get_group_requests_pagination_links();
			$top_hook    = '';
			$bottom_hook = '';
 ...
 */
function sz_nouveau_loop_classes() {
	echo esc_attr( sz_nouveau_get_loop_classes() );
}
	 ...
	 */
	function sz_nouveau_get_loop_classes() {
		$sz_nouveau = sz_nouveau();
		...
		if ( ! sz_is_directory() && ( sz_is_group() && 'members' === sz_current_action() ) ) {
			...
			$component = sanitize_key( sz_current_component() );
		}
		...
		if ( sz_is_user() && 'my-friends' === sz_current_action() ) {
			$classes[] = 'members-list';
		}

		if ( sz_is_user() && 'requests' === sz_current_action() ) {
		...
		if ( ! empty( $available_components[ $component ] ) && ( sz_is_directory() || sz_is_group() || sz_is_event() || sz_is_user() ) ) {
			...
			$layout_prefs      = sz_nouveau_get_temporary_setting(
				$customizer_option,
				sz_nouveau_get_appearance_settings( $customizer_option )
			);

			if ( $layout_prefs && (int) $layout_prefs > 1 ) {
				$grid_classes = sz_nouveau_customizer_grid_choices( 'classes' );
				...
				$sz_nouveau->{$component}->loop_layout = $layout_prefs;
			}
		}
		 ...
		 */
		$class_list = (array) apply_filters( 'sz_nouveau_get_loop_classes', $classes, $component );
 ...
 */
function sz_nouveau_loop_is_grid() {
	$sz_nouveau = sz_nouveau();
	$component  = sanitize_key( sz_current_component() );

	return ! empty( $sz_nouveau->{$component}->loop_layout ) && $sz_nouveau->{$component}->loop_layout > 1;
}
 ...
 */
function sz_nouveau_loop_get_grid_columns() {
	$sz_nouveau = sz_nouveau();
	$component  = sanitize_key( sz_current_component() );

	$columns = 1;

	if ( ! empty( $sz_nouveau->{$component}->loop_layout ) ) {
		$columns = (int) $sz_nouveau->{$component}->loop_layout;
	}
	 ...
	 */
	return (int) apply_filters( 'sz_nouveau_loop_get_grid_columns', $columns );
}
 ...
 * @return bool
 */
function sz_dir_is_vert_layout() {
	$sz_nouveau = sz_nouveau();
	$component  = sanitize_key( sz_current_component() );

	return (bool) $sz_nouveau->{$component}->directory_vertical_layout;
}
 ...
 */
function sz_nouveau_avatar_args() {
	/**
	 ...
	 * }
	 */
	return apply_filters( 'sz_nouveau_avatar_args', array(
		'type'   => 'full',
		'width'  => sz_core_avatar_full_width(),
		'height' => sz_core_avatar_full_height(),
	) );
}


/** Template Tags for SportsZone navigations **********************************/

/*
 * This is the BP Nouveau Navigation Loop.
 *
 * It can be used by any object using the
 * SZ_Core_Nav API introduced in SportsZone 2.6.0.
 */
...
function sz_nouveau_has_nav( $args = array() ) {
	...
	// So far it's only possible to build a Group nav when displaying it.
		...
		$group_nav                 = sportszone()->groups->nav;

		if ( 'group_manage' === $sz_nouveau->object_nav && sz_is_group_admin_page() ) {
			$parent_slug .= '_manage';
		 ...
		 */
			...
			sz_nouveau_set_nav_item_order( $group_nav, sz_nouveau_get_appearance_settings( 'group_nav_order' ), $parent_slug );
		}
	...
	// Build the nav for the displayed user
		...
		$sz_nouveau->displayed_nav = 'personal';
		$user_nav                  = sportszone()->members->nav;

		if ( 'secondary' === $n['type'] ) {
			$nav = $user_nav->get_secondary(
				array(
					'parent_slug'     => sz_current_component(),
					'user_has_access' => (bool) $n['user_has_access'],
				)
			);
			...
			if ( true === (bool) $n['show_for_displayed_user'] && ! sz_is_my_profile() ) {
				$args = array( 'show_for_displayed_user' => true );
			}

			// Reorder the user's primary nav according to the customizer setting.
			sz_nouveau_set_nav_item_order( $user_nav, sz_nouveau_get_appearance_settings( 'user_nav_order' ) );

			$nav = $user_nav->get_primary( $args );
		}

	} elseif ( ! empty( $sz_nouveau->object_nav ) ) {
		$sz_nouveau->displayed_nav = $sz_nouveau->object_nav;
		 ...
		 * @since 3.0.0
		 *
		 * @param array $nav The list of item navigations generated by the SZ_Core_Nav API.
		 * @param array $n   The arguments of the Navigation loop.
		 */
		$nav = apply_filters( 'sz_nouveau_get_nav', $nav, $n );
	...
	$sz_nouveau->sorted_nav = array_values( $nav );

	if ( 0 === count( $sz_nouveau->sorted_nav ) || ! $sz_nouveau->displayed_nav ) {
		unset( $sz_nouveau->sorted_nav, $sz_nouveau->displayed_nav, $sz_nouveau->object_nav );

		return false;
	}

	$sz_nouveau->current_nav_index = 0;
	return true;
}
 ...
 */
function sz_nouveau_nav_items() {
	$sz_nouveau = sz_nouveau();

	if ( isset( $sz_nouveau->sorted_nav[ $sz_nouveau->current_nav_index ] ) ) {
		return true;
	}

	$sz_nouveau->current_nav_index = 0;
	unset( $sz_nouveau->current_nav_item );
 ...
 * @since 3.0.0
 */
function sz_nouveau_nav_item() {
	$sz_nouveau = sz_nouveau();

	$sz_nouveau->current_nav_item   = $sz_nouveau->sorted_nav[ $sz_nouveau->current_nav_index ];
	$sz_nouveau->current_nav_index += 1;
}
 ...
 * @since 3.0.0
 */
function sz_nouveau_nav_id() {
	echo esc_attr( sz_nouveau_get_nav_id() );
}
	 ...
	 */
	function sz_nouveau_get_nav_id() {
		$sz_nouveau = sz_nouveau();
		$nav_item   = $sz_nouveau->current_nav_item;

		if ( 'directory' === $sz_nouveau->displayed_nav ) {
		 ...
		 */
		return apply_filters( 'sz_nouveau_get_nav_id', $id, $nav_item, $sz_nouveau->displayed_nav );
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_nav_classes() {
	echo esc_attr( sz_nouveau_get_nav_classes() );
}
	 ...
	 */
	function sz_nouveau_get_nav_classes() {
		$sz_nouveau = sz_nouveau();
		$nav_item   = $sz_nouveau->current_nav_item;
		$classes    = array();

		if ( 'directory' === $sz_nouveau->displayed_nav && ! empty( $nav_item->li_class ) ) {
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		$classes_list = apply_filters( 'sz_nouveau_get_classes', join( ' ', $classes ), $classes, $nav_item, $sz_nouveau->displayed_nav );
		if ( ! $classes_list ) {
			$classes_list = '';
		}
 ...
 * @since 3.0.0
 */
function sz_nouveau_nav_scope() {
	echo sz_nouveau_get_nav_scope();  // Escaped by sz_get_form_field_attributes().
}
	 ...
	 */
	function sz_nouveau_get_nav_scope() {
		$sz_nouveau = sz_nouveau();
		$nav_item   = $sz_nouveau->current_nav_item;
		$scope      = array();

		if ( 'directory' === $sz_nouveau->displayed_nav ) {
			$scope = array( 'data-sz-scope' => $nav_item->slug );

		} elseif ( 'personal' === $sz_nouveau->displayed_nav && ! empty( $nav_item->secondary ) ) {
			$scope = array( 'data-sz-user-scope' => $nav_item->slug );
			 ...
			 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
			 */
			$scope = apply_filters( 'sz_nouveau_set_nav_scope', $scope, $nav_item, $sz_nouveau->displayed_nav );
		}
		...
		return sz_get_form_field_attributes( 'scope', $scope );
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_nav_link() {
	echo esc_url( sz_nouveau_get_nav_link() );
}
	 ...
	 */
	function sz_nouveau_get_nav_link() {
		$sz_nouveau = sz_nouveau();
		$nav_item   = $sz_nouveau->current_nav_item;
		...
		if ( 'personal' === $sz_nouveau->displayed_nav && ! empty( $nav_item->primary ) ) {
			if ( sz_loggedin_user_domain() ) {
				$link = str_replace( sz_loggedin_user_domain(), sz_displayed_user_domain(), $link );
			} else {
				$link = trailingslashit( sz_displayed_user_domain() . $link );
			}
		}
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return apply_filters( 'sz_nouveau_get_nav_link', $link, $nav_item, $sz_nouveau->displayed_nav );
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_nav_link_id() {
	echo esc_attr( sz_nouveau_get_nav_link_id() );
}
	 ...
	 */
	function sz_nouveau_get_nav_link_id() {
		$sz_nouveau = sz_nouveau();
		$nav_item   = $sz_nouveau->current_nav_item;
		$link_id   = '';

		if ( ( 'groups' === $sz_nouveau->displayed_nav || 'personal' === $sz_nouveau->displayed_nav ) && ! empty( $nav_item->css_id ) ) {
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return apply_filters( 'sz_nouveau_get_nav_link_id', $link_id, $nav_item, $sz_nouveau->displayed_nav );
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_nav_link_title() {
	echo esc_attr( sz_nouveau_get_nav_link_title() );
}
	 ...
	 */
	function sz_nouveau_get_nav_link_title() {
		$sz_nouveau = sz_nouveau();
		$nav_item   = $sz_nouveau->current_nav_item;
		$title      = '';

		if ( 'directory' === $sz_nouveau->displayed_nav && ! empty( $nav_item->title ) ) {
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return apply_filters( 'sz_nouveau_get_nav_link_title', $title, $nav_item, $sz_nouveau->displayed_nav );
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_nav_link_text() {
	echo esc_html( sz_nouveau_get_nav_link_text() );
}
	 ...
	 */
	function sz_nouveau_get_nav_link_text() {
		$sz_nouveau = sz_nouveau();
		$nav_item   = $sz_nouveau->current_nav_item;
		$link_text  = '';

		if ( 'directory' === $sz_nouveau->displayed_nav && ! empty( $nav_item->text ) ) {
			$link_text = _sz_strip_spans_from_title( $nav_item->text );
		 ...
		 * @param string $value     The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return apply_filters( 'sz_nouveau_get_nav_link_text', $link_text, $nav_item, $sz_nouveau->displayed_nav );
	}
 ...
 * @return bool
 */
function sz_nouveau_nav_has_count() {
	$sz_nouveau = sz_nouveau();
	$nav_item   = $sz_nouveau->current_nav_item;
	$count      = false;
	 ...
	 * @param string $value     The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
	 */
	return (bool) apply_filters( 'sz_nouveau_nav_has_count', false !== $count, $nav_item, $sz_nouveau->displayed_nav );
}
 ...
 * @since 3.0.0
 */
function sz_nouveau_nav_count() {
	echo esc_html( number_format_i18n( sz_nouveau_get_nav_count() ) );
}
	 ...
	 */
	function sz_nouveau_get_nav_count() {
		$sz_nouveau = sz_nouveau();
		$nav_item   = $sz_nouveau->current_nav_item;
		$count      = 0;
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return (int) apply_filters( 'sz_nouveau_get_nav_count', $count, $nav_item, $sz_nouveau->displayed_nav );
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_directory_type_navs_class() {
	echo esc_attr( sz_nouveau_get_directory_type_navs_class() );
}
	 ...
	 * @return string
	 */
	function sz_nouveau_get_directory_type_navs_class() {
		$component  = sanitize_key( sz_current_component() );
		...
		$nav_style  = sz_nouveau_get_temporary_setting( $customizer_option, sz_nouveau_get_appearance_settings( $customizer_option ) );
		$tab_style = '';
		 ...
		 */
		$nav_wrapper_classes = (array) apply_filters( 'sz_nouveau_get_directory_type_navs_class', $nav_wrapper_classes );
 ...
 * @since 3.0.0
 */
function sz_nouveau_directory_list_class() {
	echo esc_attr( sz_nouveau_get_directory_list_class() );
}
	 ...
	 * @return string
	 */
	function sz_nouveau_get_directory_list_class() {
		return sanitize_html_class( sprintf( '%s-nav', sz_current_component() ) );
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_directory_nav_object() {
	$obj = sz_nouveau_get_directory_nav_object();
	 ...
	 *
	 * @see SZ_Component::setup_nav().
	 *
	 * @since 3.0.0
	 *
	 * @return array
	 */
	function sz_nouveau_get_directory_nav_object() {
		$nav_item = sz_nouveau()->current_nav_item;
...
/**
 * Output main SportsZone container classes.
 *
 * @since 3.0.0
 *
 * @return string CSS classes
 */
function sz_nouveau_container_classes() {
	echo esc_attr( sz_nouveau_get_container_classes() );
}

	/**
	 * Returns the main SportsZone container classes.
	 *
	 * @since 3.0.0
	 *
	 * @return string CSS classes
	 */
	function sz_nouveau_get_container_classes() {
		$classes           = array( 'sportszone-wrap' );
		$component         = sz_current_component();
		$sz_nouveau        = sz_nouveau();
		$member_type_class = '';
		...
		if ( sz_is_user() ) {
			...
			$component         = 'members';
			$user_type         = sz_get_member_type( sz_displayed_user_id() );
			...
			/**
			 * Filters the SportsZone Nouveau single item setting ID.
			 *
			 * @since 3.0.0
			 *
			 * @param string $value Setting ID.
			 */
			$customizer_option = apply_filters( 'sz_nouveau_single_item_display_settings_id', '' );
		}
		...
		if ( 'register' === sz_current_component() && sz_is_active( 'xprofile' ) && sz_nouveau_base_account_has_xprofile()) {
		...
		$general_settings = sz_nouveau_get_temporary_setting( 'avatar_style', sz_nouveau_get_appearance_settings( 'avatar_style' ) );
		if ( $general_settings ) {
		...
		if ( $customizer_option ) {
			...
			if ( $layout_prefs && sz_is_directory() ) {
				$classes[] = 'sz-dir-vert-nav';
				$classes[] = 'sz-vertical-navs';
				$sz_nouveau->{$component}->directory_vertical_layout = $layout_prefs;
			} else {
				$classes[] = 'sz-dir-hori-nav';
			}
		}
		...
		/**
		 * Filters the final results for SportsZone Nouveau container classes.
		 *
		 ...
		 */
		return apply_filters( 'sz_nouveau_get_container_classes', join( ' ', $class ), $classes );
	}
 ...
 * @return string CSS classes
 */
function sz_nouveau_single_item_nav_classes() {
	echo esc_attr( sz_nouveau_get_single_item_nav_classes() );
}
	...
	function sz_nouveau_get_single_item_nav_classes() {
		...
		// Set the global for a later use - this is moved from the `sz_nouveau_get_container_classes()
		// But was set as a check for this array class addition.
		$sz_nouveau->{$component}->single_primary_nav_layout = $layout_prefs;
		...
		/**
		 * Filters the final results for SportsZone Nouveau single item nav classes.
		 *
		 * This filter will return a single string of concatenated classes to be used.
		 *
		 * @since 3.0.0
		 ...
		 * @param array  $classes Array of classes that were concatenated.
		 */
		return apply_filters( 'sz_nouveau_get_single_item_nav_classes', join( ' ', $class ), $classes );
	}
 ...
 * @return string CSS classes
 */
function sz_nouveau_single_item_subnav_classes() {
	echo esc_attr( sz_nouveau_get_single_item_subnav_classes() );
}
	 ...
	 * @return string CSS classes
	 */
	function sz_nouveau_get_single_item_subnav_classes() {
		$classes = array( 'sz-navs', 'sz-subnavs', 'no-ajax' );
		...
		/**
		 * Filters the final results for SportsZone Nouveau single item subnav classes.
		 *
		 * This filter will return a single string of concatenated classes to be used.
		 *
		 * @since 3.0.0
		 ...
		 * @param array  $classes Array of classes that were concatenated.
		 */
		return apply_filters( 'sz_nouveau_get_single_item_subnav_classes', join( ' ', $class ), $classes );
	}
 ...
 * @return string CSS classes
 */
function sz_nouveau_groups_create_steps_classes() {
	echo esc_attr( sz_nouveau_get_group_create_steps_classes() );
}
	 ...
	 * @return string CSS classes
	 */
	function sz_nouveau_get_group_create_steps_classes() {
		$classes  = array( 'sz-navs', 'group-create-links', 'no-ajax' );
		$nav_tabs = (int) sz_nouveau_get_temporary_setting( 'groups_create_tabs', sz_nouveau_get_appearance_settings( 'groups_create_tabs' ) );
		...
		/**
		 * Filters the final results for SportsZone Nouveau group creation step classes.
		 *
		 * This filter will return a single string of concatenated classes to be used.
		 *
		 * @since 3.0.0
 ...
 */
function sz_nouveau_get_search_primary_object( $object = '' ) {
	if ( sz_is_user() ) {
		$object = 'member';
	} elseif ( sz_is_group() ) {
		$object = 'group';
		 ...
		 */
		$object = apply_filters( 'sz_nouveau_get_search_primary_object', $object );
	}
 ...
 */
function sz_nouveau_get_search_objects( $objects = array() ) {
	$primary = sz_nouveau_get_search_primary_object();
	if ( ! $primary ) {
		return $objects;
	}
	...
	if ( 'member' === $primary || 'dir' === $primary ) {
		 ...
		 */
		$objects = apply_filters( 'sz_nouveau_get_search_objects', $objects );
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_search_container_class() {
 ...
 */
function sz_nouveau_search_object_data_attr( $attr = '' ) {
	$objects = sz_nouveau_get_search_objects();
 ...
 */
function sz_nouveau_search_selector_id( $suffix = '', $sep = '-' ) {
	$id = join( $sep, array_merge( sz_nouveau_get_search_objects(), (array) $suffix ) );
	echo esc_attr( $id );
}
 ...
 */
function sz_nouveau_search_selector_name( $suffix = '', $sep = '_' ) {
	$objects = sz_nouveau_get_search_objects();

	if ( isset( $objects['secondary'] ) && ! $suffix ) {
		$name = sz_core_get_component_search_query_arg( $objects['secondary'] );
	} else {
 ...
 */
function sz_nouveau_search_default_text( $text = '', $is_attr = true ) {
	$objects = sz_nouveau_get_search_objects();

	if ( ! empty( $objects['secondary'] ) && empty( $text ) ) {
		$text = sz_get_search_default_text( $objects['secondary'] );
	}
...
function sz_nouveau_search_form() {
	...
	if ( 'dir' === $objects['primary'] ) {
		if ( 'activity' === $objects['secondary'] ) {
			 ...
			 * @since 1.5.0
			 */
			do_action( 'sz_blogs_directory_blog_sub_types' );
 ...
 *
 * If on single group screens we need to switch from component to sz_current_action() to add the correct
 * IDs/labels for group/activity & similar screens.
 *
 * @since 3.0.0
 */
function sz_nouveau_current_object() {
	/*
	...
	if ( sz_is_group() ) {
		...
		$component['object']           = sz_current_component();
		$component['data_filter']      = sz_current_component();
	}
 ...
 * @since 3.0.0
 */
function sz_nouveau_filter_container_id() {
	echo esc_attr( sz_nouveau_get_filter_container_id() );
}
	 ...
	 * @param string
	 */
	function sz_nouveau_get_filter_container_id() {
		$component = sz_nouveau_current_object();
		...
		if ( isset( $ids[ $component['object'] ] ) ) {
			...
			/**
			 * Filters the container ID for SportsZone Nouveau filters.
			 *
			 * @since 3.0.0
			 *
			 * @param string $value ID based on current component object.
			 */
			return apply_filters( 'sz_nouveau_get_filter_container_id', $ids[ $component['object'] ] );
		}
 ...
 * @since 3.0.0
 */
function sz_nouveau_filter_id() {
	echo esc_attr( sz_nouveau_get_filter_id() );
}
	 ...
	 * @param string
	 */
	function sz_nouveau_get_filter_id() {
		$component = sz_nouveau_current_object();
		...
		if ( isset( $ids[ $component['object'] ] ) ) {
			...
			/**
			 * Filters the filter ID for SportsZone Nouveau filters.
			 *
			 * @since 3.0.0
			 *
			 * @param string $value ID based on current component object.
			 */
			return apply_filters( 'sz_nouveau_get_filter_id', $ids[ $component['object'] ] );
		}
 ...
 * @since 3.0.0
 */
function sz_nouveau_filter_label() {
	echo esc_html( sz_nouveau_get_filter_label() );
}
	 ...
	 * @param string
	 */
	function sz_nouveau_get_filter_label() {
		$component = sz_nouveau_current_object();
		$label     = __( 'Order By:', 'sportszone' );

		if ( 'activity' === $component['object'] || 'friends' === $component['object'] ) {
			$label = __( 'Show:', 'sportszone' );
		}

		/**
		 * Filters the label for SportsZone Nouveau filters.
		 *
		 * @since 3.0.0
		 *
		 * @param string $label     Label for SportsZone Nouveau filter.
		 * @param array  $component The data filter's data-sz-filter attribute value.
		 */
		return apply_filters( 'sz_nouveau_get_filter_label', $label, $component );
	}

/**
 * Output data filter's data-sz-filter attribute value.
 *
 * @since 3.0.0
 */
function sz_nouveau_filter_component() {
	$component = sz_nouveau_current_object();
 ...
 * @since 3.0.0
 */
function sz_nouveau_filter_options() {
	echo sz_nouveau_get_filter_options();  // Escaped in inner functions.
}
	 ...
	 * @return string
	 */
	function sz_nouveau_get_filter_options() {
		$output = '';

		if ( 'notifications' === sz_current_component() ) {
			$output = sz_nouveau_get_notifications_filters();

		} else {
			$filters = sz_nouveau_get_component_filters();
 ...
 * @return string HTML.
 */
function sz_nouveau_get_customizer_link( $args = array() ) {
	...
	if ( ! sz_current_user_can( $r['capability'] ) ) {
		return '';
	}
	...
	if ( sz_is_user() ) {
		$url = rawurlencode( sz_displayed_user_domain() );
 ...
 */
function sz_nouveau_signup_hook( $when = '', $prefix = '' ) {
	$hook = array( 'sz' );
 ...
 * @param string $prefix Use it to add terms before the hook name
 */
function sz_nouveau_activation_hook( $when = '', $suffix = '' ) {
	$hook = array( 'sz' );
 ...
 */
function sz_nouveau_signup_form( $section = 'account_details' ) {
	$fields = sz_nouveau_get_signup_fields( $section );
	if ( ! $fields ) {
		return;
	}
	...
	foreach ( $fields as $name => $attributes ) {
		...
		if ( $required ) {
			...
			$label_output = __( '<label for="%1$s">%2$s %3$s</label>', 'sportszone' );
		}

		// Output the label for regular fields
		if ( 'radio' !== $type ) {
			if ( $required ) {
				printf( $label_output, esc_attr( $name ), esc_html( $label ), __( '(required)', 'sportszone' ) );
			} else {
		...
		// Handle the specific case of Site's privacy differently
			...
			?>
				<span class="label">
					<?php esc_html_e( 'I would like my site to appear in search engines, and in public listings around this network.', 'sportszone' ); ?>
				</span>
			<?php
		}
		...
		if ( $attribute_type ) {
			...
			$attribute_type = ' ' . sz_get_form_field_attributes( $attribute_type, $existing_attributes );
		}
		...
		if ( 'signup_blog_privacy_public' === $name || 'signup_blog_privacy_private' === $name ) {
			...
			$submitted = sz_get_signup_blog_privacy_value();
		...
		if ( 'private' !== $value ) {
			 ...
			 */
			if ( isset( sportszone()->signup->errors[ $name ] ) ) {
				nouveau_error_template( sportszone()->signup->errors[ $name ] );
				$invalid = 'invalid';
			}
		}

		if ( isset( $invalid ) && isset( sportszone()->signup->errors[ $name ] ) ) {
			if ( ! empty( $class ) ) {
				$class = $class . ' ' . $invalid;
			} else {
				$class = $invalid;
 ...
 */
function sz_nouveau_submit_button( $action ) {
	$submit_data = sz_nouveau_get_submit_button( $action );
	...
	if ( ! empty( $submit_data['before'] ) ) {
		 ...
		 *
		 * This is a dynamic filter that is dependent on the "before" value provided by sz_nouveau_get_submit_button().
		 *
		 * @since 3.0.0
		 */
	...
	$submit_input = sprintf( '<input type="submit" %s/>',
		sz_get_form_field_attributes( 'submit', $submit_data['attributes'] )  // Safe.
	);
	...
	if ( ! empty( $submit_data['after'] ) ) {
		 ...
		 * Fires before display of the submit button.
		 *
		 * This is a dynamic filter that is dependent on the "after" value provided by sz_nouveau_get_submit_button().
		 *
		 * @since 3.0.0
		 */
...
function nouveau_error_template( $message = '', $type = '' ) {
	...
	<div class="<?php echo esc_attr( 'sz-messages sz-feedback ' . $type ); ?>">
