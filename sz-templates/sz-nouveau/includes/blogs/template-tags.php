 ...
 * @since 3.0.0
 */
function sz_nouveau_before_blogs_directory_content() {
	/**
	 * Fires at the begining of the templates BP injected content.
	 *
	 * @since 2.3.0
	 */
	do_action( 'sz_before_directory_blogs_page' );
	 ...
	 * @since 1.5.0
	 */
	do_action( 'sz_before_directory_blogs' );
	 ...
	 * @since 3.0.0
	 */
	do_action( 'sz_before_directory_blogs_content' );
	 ...
	 * @since 2.3.0
	 */
	do_action( 'sz_before_directory_blogs_tabs' );
}
 ...
 * @since 3.0.0
 */
function sz_nouveau_after_blogs_directory_content() {
	/**
	 * Fires inside and displays the blogs content.
	 *
	 * @since 3.0.0
	 */
	do_action( 'sz_directory_blogs_content' );
	 ...
	 * @since 3.0.0
	 */
	do_action( 'sz_after_directory_blogs_content' );
	 ...
	 * @since 1.5.0
	 */
	do_action( 'sz_after_directory_blogs' );
	 ...
	 * @since 2.3.0
	 */
	do_action( 'sz_after_directory_blogs_page' );
}
 ...
 */
function sz_nouveau_blogs_create_hook( $when = '', $suffix = '' ) {
	$hook = array( 'sz' );
 ...
 * @since 3.0.0
 */
function sz_nouveau_blogs_loop_item() {
	/**
	 * Fires after the listing of a blog item in the blogs loop.
	 *
	 * @since 1.2.0
	 */
	do_action( 'sz_directory_blogs_item' );
}
 ...
 */
function sz_nouveau_blogs_loop_buttons( $args = array() ) {
	...
	$output = join( ' ', sz_nouveau_get_blogs_buttons( $args ) );
	 ...
	 * @since 3.0.0
	 */
	do_action( 'sz_directory_blogs_actions' );
	$output .= ob_get_clean();
	 ...
	 * @return array
	 */
	function sz_nouveau_get_blogs_buttons( $args ) {
		 ...
		 */
		add_filter( 'sz_get_blogs_visit_blog_button', 'sz_nouveau_blogs_catch_button_args', 100, 1 );

		sz_get_blogs_visit_blog_button();

		remove_filter( 'sz_get_blogs_visit_blog_button', 'sz_nouveau_blogs_catch_button_args', 100, 1 );

		if ( ! empty( sz_nouveau()->blogs->button_args ) ) {
			$button_args = sz_nouveau()->blogs->button_args ;
			...
			unset( sz_nouveau()->blogs->button_args );
		}
		 ...
		 */
		$buttons_group = apply_filters( 'sz_nouveau_get_blogs_buttons', $buttons, $blog, $type );
		...
		if ( ! isset( sz_nouveau()->blogs->group_buttons ) || ! is_a( sz_nouveau()->blogs->group_buttons, 'SZ_Buttons_Group' ) ) {
			$sort = true;
			sz_nouveau()->blogs->group_buttons = new SZ_Buttons_Group( $buttons_group );

		// It's not the first entry, the order is set, we simply need to update the Buttons Group
		} else {
			$sort = false;
			sz_nouveau()->blogs->group_buttons->update( $buttons_group );
		}

		$return = sz_nouveau()->blogs->group_buttons->get( $sort );
		 ...
		 * @param string $type    Whether we're displaying a blogs loop or a the blogs single item (in the future!).
		 */
		do_action_ref_array( 'sz_nouveau_return_blogs_buttons', array( &$return, $blog, $type ) );
 ...
 */
function sz_nouveau_blog_has_latest_post() {
	return (bool) sz_get_blog_latest_post_title();
}
