 ...
 */
function sz_nouveau_notifications_register_scripts( $scripts = array() ) {
	...
	return array_merge( $scripts, array(
		'sz-nouveau-notifications' => array(
			'file'         => 'js/sportszone-notifications%s.js',
 ...
 * @since 3.0.0
 */
function sz_nouveau_notifications_enqueue_scripts() {

	if ( ! sz_is_user_notifications() ) {
		return;
	}

	wp_enqueue_script( 'sz-nouveau-notifications' );
}
 ...
 * @since 3.0.0
 */
function sz_nouveau_notifications_init_filters() {
	if ( ! sz_is_user_notifications() ) {
		return;
	}

	sz_nouveau()->notifications->filters = array();
	 ...
	 * @since 3.0.0
	 */
	do_action( 'sz_nouveau_notifications_init_filters' );
}
 ...
 */
function sz_nouveau_notifications_register_filter( $args = array() ) {
	...
	if ( isset( $sz_nouveau->notifications->filters[ $r['id'] ] ) ) {
		return false;
	}

	$sz_nouveau->notifications->filters[ $r['id'] ] = $r;
	return true;
}
 ...
 */
function sz_nouveau_notifications_get_filters( $id = '' ) {
	$sz_nouveau = sz_nouveau();

	// Get all filters
	if ( empty( $id ) ) {
		return $sz_nouveau->notifications->filters;

	// Get a specific filter
	} elseif ( ! empty( $id ) && isset( $sz_nouveau->notifications->filters[ $id ] ) ) {
		return $sz_nouveau->notifications->filters[ $id ];
 ...
 */
function sz_nouveau_notifications_sort( $filters = array() ) {
	$sorted = array();
 ...
 * @param  string $sz_tooltip  The data-sz-attribute of the link.
 ...
 */
function sz_nouveau_notifications_dashiconified_link( $link = '', $sz_tooltip = '', $dashicon = '' ) {
	preg_match( '/<a\s[^>]*>(.*)<\/a>/siU', $link, $match );

	if ( ! empty( $match[0] ) && ! empty( $match[1] ) && ! empty( $dashicon ) && ! empty( $sz_tooltip ) ) {
		$link = str_replace(
			'>' . $match[1] . '<',
			sprintf(
				' class="sz-tooltip" data-sz-tooltip="%1$s"><span class="dashicons %2$s" aria-hidden="true"></span><span class="sz-screen-reader-text">%3$s</span><',
				esc_attr( $sz_tooltip ),
 ...
 * @return string Link Output.
 */
function sz_nouveau_notifications_mark_unread_link( $link = '' ) {
	return sz_nouveau_notifications_dashiconified_link(
		$link,
		_x( 'Mark Unread', 'link', 'sportszone' ),
		'dashicons-hidden'
	);
}
 ...
 * @return string Link Output.
 */
function sz_nouveau_notifications_mark_read_link( $link = '' ) {
	return sz_nouveau_notifications_dashiconified_link(
		$link,
		_x( 'Mark Read', 'link', 'sportszone' ),
 ...
 * @return string Link Output.
 */
function sz_nouveau_notifications_delete_link( $link = '' ) {
	return sz_nouveau_notifications_dashiconified_link(
		$link,
		_x( 'Delete', 'link', 'sportszone' ),
		'dashicons-dismiss'
	);
}
