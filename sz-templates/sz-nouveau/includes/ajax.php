...
function sz_nouveau_ajax_object_template_loader() {
	...
	// Nonce check!
	if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'sz_nouveau_' . $object ) ) {
		wp_send_json_error();
	}
	...
	if ( 'activity' === $object ) {
		...
		switch ( $scope ) {
			...
			case 'mentions':
				...
				$new_mentions = sz_get_user_meta( sz_loggedin_user_id(), 'sz_new_mentions', true );
				...
				break;
			default:
				$feed_url = sz_get_sitewide_activity_feed_link();
				break;
		}
		 ...
		 */
		$result['feed_url'] = apply_filters( 'sz_nouveau_ajax_object_template_loader', $feed_url, $scope );
	}
