...
( function( exports, $ ) {
	...
	bp.Nouveau.Activity = {
		...
		setupGlobals: function() {
			...
			// Init mentions count
			this.mentions_count = Number( $( bp.Nouveau.objectNavParent + ' [data-sz-scope="mentions"]' ).find( 'a span' ).html() ) || 0;
		...
		addListeners: function() {
			// HeartBeat listeners
			$( '#sportszone' ).on( 'sz_heartbeat_send', this.heartbeatSend.bind( this ) );
			$( '#sportszone' ).on( 'sz_heartbeat_tick', this.heartbeatTick.bind( this ) );

			// Inject Activities
			$( '#sportszone [data-sz-list="activity"]' ).on( 'click', 'li.load-newest, li.load-more', this.injectActivities.bind( this ) );

			// Hightlight new activities & clean up the stream
			$( '#sportszone' ).on( 'sz_ajax_request', '[data-sz-list="activity"]', this.scopeLoaded.bind( this ) );

			// Activity comments effect
			$( '#sportszone [data-sz-list="activity"]' ).on( 'sz_ajax_append', this.hideComments );
			$( '#sportszone [data-sz-list="activity"]' ).on( 'click', '.show-all', this.showComments );

			// Activity actions
			$( '#sportszone [data-sz-list="activity"]' ).on( 'click', '.activity-item', bp.Nouveau, this.activityActions );
		...
		heartbeatSend: function( event, data ) {
			this.heartbeat_data.first_recorded = $( '#sportszone [data-sz-list] [data-sz-activity-id]' ).first().data( 'sz-timestamp' ) || 0;
			...
			data.sz_activity_last_recorded = this.heartbeat_data.last_recorded;

			if ( $( '#sportszone .dir-search input[type=search]' ).length ) {
				data.sz_activity_last_recorded_search_terms = $( '#sportszone .dir-search input[type=search]' ).val();
			}

			$.extend( data, { sz_heartbeat: bp.Nouveau.getStorage( 'sz-activity' ) } );
		},
		...
		heartbeatTick: function( event, data ) {
			var newest_activities_count, newest_activities, objects = bp.Nouveau.objects,
				scope = bp.Nouveau.getStorage( 'sz-activity', 'scope' ), self = this;

			// Only proceed if we have newest activities
			if ( undefined === data || ! data.sz_activity_newest_activities ) {
				return;
			}

			this.heartbeat_data.newest = $.trim( data.sz_activity_newest_activities.activities ) + this.heartbeat_data.newest;
			this.heartbeat_data.last_recorded  = Number( data.sz_activity_newest_activities.last_recorded );
			...
			if ( 'all' === scope ) {
				...
				$.each( newest_activities, function( a, activity ) {
					...
					$.each( objects, function( o, object ) {
						if ( -1 !== $.inArray( 'sz-my-' + object, activity.get( 0 ).classList ) ) {
							if ( undefined === self.heartbeat_data.highlights[ object ] ) {
								self.heartbeat_data.highlights[ object ] = [ activity.data( 'sz-activity-id' ) ];
							} else if ( -1 === $.inArray( activity.data( 'sz-activity-id' ), self.heartbeat_data.highlights[ object ] ) ) {
								self.heartbeat_data.highlights[ object ].push( activity.data( 'sz-activity-id' ) );
							}
						}
					} );
				} );
				...
				var regexp = new RegExp( 'sz-my-(' + objects.join( '|' ) + ')', 'g' );
				 ...
				 */
				 $( bp.Nouveau.objectNavParent + ' [data-sz-scope="all"]' ).find( 'a span' ).html( newest_activities_count );
			...
			// Set all activities to be highlighted for the current scope
				...
				$.each( newest_activities, function( a, activity ) {
					self.heartbeat_data.highlights[ scope ].push( $( activity ).data( 'sz-activity-id' ) );
				} );
			}
			...
			$.each( objects, function( o, object ) {
				if ( undefined !== self.heartbeat_data.highlights[ object ] && self.heartbeat_data.highlights[ object ].length ) {
					...
					$( bp.Nouveau.objectNavParent + ' [data-sz-scope="' + object + '"]' ).find( 'a span' ).html( Number( self.heartbeat_data.highlights[ object ].length ) + count );
				}
			} );
			...
			if ( $( '#sportszone [data-sz-list="activity"] li' ).first().hasClass( 'load-newest' ) ) {
				var newest_link = $( '#sportszone [data-sz-list="activity"] .load-newest a' ).html();
				$( '#sportszone [data-sz-list="activity"] .load-newest a' ).html( newest_link.replace( /([0-9]+)/, newest_activities_count ) );

			// Otherwise add it
			} else {
				$( '#sportszone [data-sz-list="activity"] ul.activity-list' ).prepend( '<li class="load-newest"><a href="#newest">' + SZ_Nouveau.newest + ' (' + newest_activities_count + ')</a></li>' );
			}

			/**
			 * Finally trigger a pending event containing the activity heartbeat data
			 */
			$( '#sportszone [data-sz-list="activity"]' ).trigger( 'sz_heartbeat_pending', this.heartbeat_data );
		},
		...
		injectActivities: function( event ) {
			var store = bp.Nouveau.getStorage( 'sz-activity' ),
			...
			if ( $( event.currentTarget ).hasClass( 'load-newest' ) ) {
				...
				$( event.delegateTarget ).find( '.activity-list' ).prepend( this.heartbeat_data.newest ).trigger( 'sz_heartbeat_prepend', this.heartbeat_data );
				...
				if ( 'all' === scope ) {
					$( bp.Nouveau.objectNavParent + ' [data-sz-scope="all"]' ).find( 'a span' ).html( '' );
				}
				...
				$( bp.Nouveau.objectNavParent + ' [data-sz-scope="' + scope + '"]' ).find( 'a span' ).html( '' );
				...
				setTimeout( function () {
					$( event.delegateTarget ).find( '[data-sz-activity-id]' ).removeClass( 'newest_' + scope + '_activity' );
				}, 3000 );
			...
			// Load more activities
				...
				$( event.delegateTarget ).children( '.just-posted' ).each( function() {
					self.just_posted.push( $( this ).data( 'sz-activity-id' ) );
				} );

				if ( $( '#sportszone .dir-search input[type=search]' ).length ) {
					search_terms = $( '#sportszone .dir-search input[type=search]' ).val();
				}
				...
				bp.Nouveau.objectRequest( {
					...
					target              : '#sportszone [data-sz-list] ul.sz-list'
		...
		hideComments: function( event ) {
			...
			comments.each( function( c, comment ) {
				...
				comment_count = $( '#acomment-comment-' + activity_item.data( 'sz-activity-id' ) + ' span.comment-count' ).html() || ' ';

				// Keep latest 5 comments
				comment_items.each( function( i, item ) {
					if ( i < comment_items.length - 5 ) {
						$( item ).addClass('sz-hidden').hide();

						// Prepend a link to display all
						if ( ! i ) {
							$( item ).before( '<li class="show-all"><button class="text-button" type="button" data-sz-show-comments-id="#' + activity_item.prop( 'id' ) + '/show-all/"><span class="icon dashicons dashicons-visibility" aria-hidden="true"></span> ' + SZ_Nouveau.show_x_comments.replace( '%d', comment_count ) + '</button></li>' );
						}
					}
				} );

				// If all parents are hidden, reveal at least one. It seems very risky to manipulate the DOM to keep exactly 5 comments!
				if ( $( comment_parents ).children( '.sz-hidden' ).length === $( comment_parents ).children( 'li' ).length - 1 && $( comment_parents ).find( 'li.show-all' ).length ) {
					$( comment_parents ).children( 'li' ).removeClass( 'sz-hidden' ).toggle();
				}
			} );
		},
		...
		showComments: function( event ) {
			...
			setTimeout( function() {
				$( event.target ).closest( 'ul' ).find( 'li' ).removeClass('sz-hidden').fadeIn( 300, function() {
		...
		scopeLoaded: function ( event, data ) {
			...
			if ( 'mentions' === data.scope && undefined !== data.response.new_mentions ) {
				$.each( data.response.new_mentions, function( i, id ) {
					$( '#sportszone #activity-stream' ).find( '[data-sz-activity-id="' + id + '"]' ).addClass( 'newest_mentions_activity' );
				} );
				...
				$.each( this.heartbeat_data.highlights[data.scope], function( i, id ) {
					if ( $( '#sportszone #activity-stream' ).find( '[data-sz-activity-id="' + id + '"]' ).length ) {
						$( '#sportszone #activity-stream' ).find( '[data-sz-activity-id="' + id + '"]' ).addClass( 'newest_' + data.scope + '_activity' );
					}
				} );
			}
			...
			// Reset the newest activities now they're displayed
			this.heartbeat_data.newest = '';
			$.each( $( bp.Nouveau.objectNavParent + ' [data-sz-scope]' ).find( 'a span' ), function( s, count ) {
			...
			setTimeout( function () {
				$( '#sportszone #activity-stream .activity-item' ).removeClass( 'newest_' + data.scope +'_activity' );
			}, 3000 );
		},
		...
		activityActions: function( event ) {
			var parent = event.data, target = $( event.target ), activity_item = $( event.currentTarget ),
				activity_id = activity_item.data( 'sz-activity-id' ), stream = $( event.delegateTarget ),
				item_id, form;
				...
				parent.ajax( { action: 'activity_mark_' + type, 'id': activity_id }, 'activity' ).done( function( response ) {
					...
					if ( 'fav' === type ) {
						if ( undefined !== response.data.directory_tab ) {
							if ( ! $( parent.objectNavParent + ' [data-sz-scope="favorites"]' ).length ) {
								$( parent.objectNavParent + ' [data-sz-scope="all"]' ).after( response.data.directory_tab );
							}
						}
						...
						var favoriteScope = $( '[data-sz-user-scope="favorites"]' ).hasClass( 'selected' ) || $( parent.objectNavParent + ' [data-sz-scope="favorites"]' ).hasClass( 'selected' );
						...
						if ( undefined !== response.data.no_favorite ) {
							...
							if ( $( parent.objectNavParent + ' [data-sz-scope="all"]' ).length && $( parent.objectNavParent + ' [data-sz-scope="all"]' ).hasClass( 'selected' ) ) {
								$( parent.objectNavParent + ' [data-sz-scope="favorites"]' ).remove();
			...
			if ( target.hasClass( 'delete-activity' ) || target.hasClass( 'acomment-delete' ) || target.hasClass( 'spam-activity' ) || target.hasClass( 'spam-activity-comment' ) ) {
				var activity_comment_li = target.closest( '[data-sz-activity-comment-id]' ),
				    activity_comment_id = activity_comment_li.data( 'sz-activity-comment-id' ),
				    li_parent, comment_count_span, comment_count, show_all_a, deleted_comments_count = 0;

				// Stop event propagation
				event.preventDefault();

				if ( undefined !== SZ_Nouveau.confirm && false === window.confirm( SZ_Nouveau.confirm ) ) {
					return false;
				}
				...
				var ajaxData = {
					...
					'is_single' : target.closest( '[data-sz-single]' ).length
				};
				...
				if ( target.hasClass( 'spam-activity' ) || target.hasClass( 'spam-activity-comment' ) ) {
					ajaxData.action = 'sz_spam_activity';
				}
				...
				parent.ajax( ajaxData, 'activity' ).done( function( response ) {
					...
					if ( false === response.success ) {
						...
						li_parent.find( '.sz-feedback' ).hide().fadeIn( 300 );
					} else {
						...
						if ( activity_comment_id ) {
							...
							if ( show_all_a.length ) {
								show_all_a.html( SZ_Nouveau.show_x_comments.replace( '%d', comment_count ) );
							}
						...
						if ( ! activity_comment_id && activity_item.data( 'sz-timestamp' ) === parent.Activity.heartbeat_data.last_recorded ) {
			...
			if ( target.closest( 'span' ).hasClass( 'activity-read-more' ) ) {
				...
				if ( $( content ).hasClass( 'activity-inner' ) ) {
					...
					item_id = target.closest( 'li' ).data( 'sz-activity-comment-id' );
				}
				...
				parent.ajax( {
					...
					if ( content.parent().find( '.sz-feedback' ).length ) {
						content.parent().find( '.sz-feedback' ).remove();
					}

					if ( false === response.success ) {
						content.after( response.data.feedback );
						content.parent().find( '.sz-feedback' ).hide().fadeIn( 300 );
					} else {
			...
			if ( target.hasClass( 'acomment-reply' ) || target.parent().hasClass( 'acomment-reply' ) ) {
				...
				if ( target.closest( 'li' ).data( 'sz-activity-comment-id' ) ) {
					item_id = target.closest( 'li' ).data( 'sz-activity-comment-id' );
				}
				...
				if ( item_id === activity_id ) {
					$( '[data-sz-activity-id="' + item_id + '"] .activity-comments' ).append( form );
					form.addClass( 'root' );

				// It's a comment we're replying to
				} else {
					$( '[data-sz-activity-comment-id="' + item_id + '"]' ).append( form );
				}
			...
			if ( 'ac_form_submit' === target.prop( 'name' ) ) {
				...
				if ( $( '#_sz_as_nonce_' + activity_id ).val() ) {
					comment_data['_sz_as_nonce_' + activity_id] = $( '#_sz_as_nonce_' + activity_id ).val();
				}
