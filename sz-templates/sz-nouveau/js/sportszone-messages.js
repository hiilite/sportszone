/* global wp, bp, SZ_Nouveau, _, Backbone, tinymce, tinyMCE */
/* jshint devel: true */
/* @version 3.1.0 */
window.wp = window.wp || {};
window.bp = window.bp || {};
...
( function( exports, $ ) {
	...
	bp.Nouveau.Messages = {
		...
		start: function() {
			...
			Backbone.history.start( {
				pushState: true,
				root: SZ_Nouveau.messages.rootUrl
			} );
		},
		...
		displayFeedback: function( message, type ) {
			...
			feedback.inject( '.sz-messages-feedback' );
		},
		...
		composeView: function() {
			...
			form.inject( '.sz-messages-content' );
		},
		...
		threadsView: function() {
			...
			threads_list.inject( '.sz-messages-content' );
		...
		displayFilters: function( collection ) {
			...
			filters_view.inject( '.sz-messages-filters' );
		},
		...
		singleView: function( thread ) {
			...
			single_thread.inject( '.sz-messages-content' );
		}
	};
	...
	bp.Models.Message = Backbone.Model.extend( {
		...
		sendMessage: function() {
			...
			var sent = bp.ajax.post( 'messages_send_message', _.extend(
				{
					nonce: SZ_Nouveau.messages.nonces.send
				},
				this.attributes
			) );
		...
		updateReadState: function( options ) {
			...
			options.data = _.extend(
				_.pick( this.attributes, ['id', 'message_id'] ),
				{
					...
					nonce  : SZ_Nouveau.nonces.messages
				}
			);
	...
	bp.Collections.Threads = Backbone.Collection.extend( {
		...
		sync: function( method, model, options ) {
			...
			// Add generic nonce
			options.data.nonce = SZ_Nouveau.nonces.messages;
	...
	bp.Views.Feedback = bp.Nouveau.Messages.View.extend( {
		tagName: 'div',
		className: 'sz-messages sz-user-messages-feedback',
		template  : bp.template( 'sz-messages-feedback' ),
	...
	bp.Views.Hook = bp.Nouveau.Messages.View.extend( {
		tagName: 'div',
		template  : bp.template( 'sz-messages-hook' ),
		...
		initialize: function() {
			...
			this.el.className = 'sz-messages-hook';
	...
	bp.Views.messageEditor = bp.Nouveau.Messages.View.extend( {
		template  : bp.template( 'sz-messages-editor' ),
	...
	bp.Views.messageForm = bp.Nouveau.Messages.View.extend( {
		...
		template  : bp.template( 'sz-messages-form' ),

		events: {
			'click #sz-messages-send'  : 'sendMessage',
			'click #sz-messages-reset' : 'resetForm'
		},
		...
		sendMessage: function( event ) {
			...
			if ( errors.length ) {
				...
				_.each( errors, function( e ) {
					feedback += '<div class="sz-feedback error"><span class="sz-icon" aria-hidden="true"></span><p>'+SZ_Nouveau.messages.errors[ e ] + '</p></div>';
				} );
	...
	bp.Views.userThreads = bp.Nouveau.Messages.View.extend( {
		...
		requestThreads: function() {
			...
			bp.Nouveau.Messages.displayFeedback( SZ_Nouveau.messages.loading, 'loading' );
		...
		threadsFetched: function() {
			...
			// Display the sz_after_member_messages_loop hook.
			...
			// Display the sz_before_member_messages_loop hook.
			...
			bp.Nouveau.Messages.displayFeedback( SZ_Nouveau.messages.howto, 'info' );
		},
	...
	bp.Views.userThread = bp.Nouveau.Messages.View.extend( {
		tagName   : 'li',
		template  : bp.template( 'sz-messages-thread' ),
		...
		initialize: function() {
			...
			if ( 'sentbox' === bp.Nouveau.Messages.box ) {
				...
				if ( 2 === recipientsCount ) {
					toOthers = SZ_Nouveau.messages.toOthers.one;
				} else if ( 2 < recipientsCount ) {
					toOthers = SZ_Nouveau.messages.toOthers.more.replace( '%d', Number( recipientsCount - 1 ) );
				}
		...
		bulkSelect: function( model ) {
			if ( $( '#sz-message-thread-' + model.get( 'id' ) ).length ) {
				$( '#sz-message-thread-' + model.get( 'id' ) ).prop( 'checked',model.get( 'checked' ) );
			}
		},
		...
		singleSelect: function( event ) {
			...
			if ( hasChecked ) {
				$( '#user-messages-bulk-actions' ).closest( '.bulk-actions-wrap' ).removeClass( 'sz-hide' );

				// Inform the user about how to use the bulk actions.
				bp.Nouveau.Messages.displayFeedback( SZ_Nouveau.messages.howtoBulk, 'info' );
			} else {
				$( '#user-messages-bulk-actions' ).closest( '.bulk-actions-wrap' ).addClass( 'sz-hide' );
	...
	bp.Views.previewThread = bp.Nouveau.Messages.View.extend( {
		tagName: 'div',
		id: 'thread-preview',
		template  : bp.template( 'sz-messages-preview' ),
		...
		doAction: function( event ) {
			var action = $( event.currentTarget ).data( 'sz-action' ), self = this, options = {}, mid,
			    feedback = SZ_Nouveau.messages.doingAction;
	...
	bp.Views.Pagination = bp.Nouveau.Messages.View.extend( {
		...
		template  :  bp.template( 'sz-messages-paginate' )
	} );

	bp.Views.BulkActions = bp.Nouveau.Messages.View.extend( {
		tagName   : 'div',
		template  :  bp.template( 'sz-bulk-actions' ),
		...
		bulkSelect: function( event ) {
			...
			if ( isChecked ) {
				$( this.el ).find( '.bulk-actions-wrap' ).removeClass( 'sz-hide' ).addClass( 'sz-show' );

				// Inform the user about how to use the bulk actions.
				bp.Nouveau.Messages.displayFeedback( SZ_Nouveau.messages.howtoBulk, 'info' );
			} else {
				$( this.el ).find( '.bulk-actions-wrap' ).addClass( 'sz-hide' );
	...
	bp.Views.messageFilters = bp.Nouveau.Messages.View.extend( {
		tagName: 'ul',
		template:  bp.template( 'sz-messages-filters' ),

		events : {
			'search #user_messages_search'      : 'resetSearchTerms',
			'submit #user_messages_search_form' : 'setSearchTerms',
			'click #sz-messages-next-page'      : 'nextPage',
			'click #sz-messages-prev-page'      : 'prevPage'
		},
		...
		addPaginatation: function( collection ) {
			...
			this.views.add( '.user-messages-bulk-actions', new bp.Views.BulkActions( {
				model: new Backbone.Model( SZ_Nouveau.messages.bulk_actions ),
		...
		resetSearchTerms: function( event ) {
			...
			if ( ! $( event.target ).val() ) {
				...
				$( event.target ).closest( 'form' ).find( '[type=submit]' ).addClass('sz-show').removeClass('sz-hide');
			}
		},
	...
	bp.Views.userMessagesHeader = bp.Nouveau.Messages.View.extend( {
		tagName  : 'div',
		template : bp.template( 'sz-messages-single-header' ),
		...
		doAction: function( event ) {
			var action = $( event.currentTarget ).data( 'sz-action' ), self = this, options = {},
			    feedback = SZ_Nouveau.messages.doingAction;
			...
			if ( 'star' === action || 'unstar' === action ) {
				...
				$( event.currentTarget ).addClass( 'sz-hide' );
				$( event.currentTarget ).parent().find( '[data-sz-action="' + opposite[ action ] + '"]' ).removeClass( 'sz-hide' );
	...
	bp.Views.userMessagesEntry = bp.Views.userMessagesHeader.extend( {
		tagName  : 'li',
		template : bp.template( 'sz-messages-single-list' ),

		events: {
			'click [data-sz-action]' : 'doAction'
		},
	...
	bp.Views.userMessages = bp.Nouveau.Messages.View.extend( {
		tagName  : 'div',
		template : bp.template( 'sz-messages-single' ),
		...
		messagesFetched: function( collection, response ) {
			...
			this.views.add( '#sz-message-thread-header', new bp.Views.userMessagesHeader( { model: this.options.thread } ) );
		},
		...
		addMessage: function( message ) {
			this.views.add( '#sz-message-thread-list', new bp.Views.userMessagesEntry( { model: message } ) );
		},
