/* global wp, bp, SZ_Nouveau, _, Backbone */
/* @version 3.0.0 */
window.wp = window.wp || {};
window.bp = window.bp || {};
...
( function( exports, $ ) {
	...
	bp.Nouveau.GroupInvites = {
		...
		start: function() {
			...
			this.setupNav();
			this.setupLoops();
			this.displayFeedback( SZ_Nouveau.group_invites.loading, 'loading' );
		...
		setupNav: function() {
			...
			_.each( SZ_Nouveau.group_invites.nav, function( item, index ) {
				if ( ! _.isObject( item ) ) {
					return;
				}
			...
			this.nav.inject( '.sz-invites-nav' );

			// Listen to the confirm event
			this.nav.on( 'sz-invites:confirm', this.loadConfirmView, this );
			this.nav.on( 'sz-invites:loops', this.setupLoops, this );
		},
		...
		setupLoops: function( scope ) {
			...
			users.inject( '.sz-invites-content' );
		...
		displayFilters: function( collection ) {
			...
			filters_view.inject( '.sz-invites-filters' );
		},
		...
		displayFeedback: function( message, type ) {
			...
			feedback.inject( '.sz-invites-feedback' );
		},
		...
		loadConfirmView: function() {
			...
			this.displayFeedback( SZ_Nouveau.group_invites.invites_form, 'help' );
			...
			invites.inject( '.sz-invites-content' );
		}
	};
	...
	bp.Collections.Users = Backbone.Collection.extend( {
		...
		initialize : function() {
			this.options = { page: 1, total_page: 0, group_id: SZ_Nouveau.group_invites.group_id };
		},
		...
		sync: function( method, model, options ) {
			...
			// Add generic nonce
			options.data.nonce = SZ_Nouveau.nonces.groups;
			...
			if ( 'create' === method ) {
				options.data = _.extend( options.data, {
					...
					_wpnonce : SZ_Nouveau.group_invites.nonces.send_invites
				} );
			...
			if ( 'delete' === method ) {
				options.data = _.extend( options.data, {
					...
					_wpnonce : SZ_Nouveau.group_invites.nonces.uninvite
				} );
	...
	bp.Views.Feedback = bp.Nouveau.GroupInvites.View.extend( {
		tagName   : 'div',
		className : 'sz-invites-feedback',
		template  : bp.template( 'sz-group-invites-feedback' ),
	...
	bp.Views.invitesNav = bp.Nouveau.GroupInvites.View.extend( {
		...
		events: {
			'click .sz-invites-nav-item' : 'toggleView'
		},
		...
		toggleView: function( event ) {
			...
			_.each( this.collection.models, function( nav ) {
				if ( nav.id === current_nav_id ) {
					...
					if ( 'invites' === nav.id ) {
						this.trigger( 'sz-invites:confirm' );
					} else {
						this.trigger( 'sz-invites:loops', nav.id );
					}
	...
	bp.Views.invitesNavItem = bp.Nouveau.GroupInvites.View.extend( {
		tagName  : 'li',
		template : bp.template( 'sz-invites-nav' ),
	...
	bp.Views.Pagination = bp.Nouveau.GroupInvites.View.extend( {
		tagName   : 'div',
		className : 'last',
		template  :  bp.template( 'sz-invites-paginate' )
	} );

	bp.Views.inviteFilters = bp.Nouveau.GroupInvites.View.extend( {
		tagName: 'div',
		template:  bp.template( 'sz-invites-filters' ),

		events : {
			'search #group_invites_search'      : 'resetSearchTerms',
			'submit #group_invites_search_form' : 'setSearchTerms',
			'click #sz-invites-next-page'       : 'nextPage',
			'click #sz-invites-prev-page'       : 'prevPage'
		},
		...
		filterUsers: function() {
			bp.Nouveau.GroupInvites.displayFeedback( SZ_Nouveau.group_invites.loading, 'loading' );
		...
		resetSearchTerms: function( event ) {
			...
			if ( ! $( event.target ).val() ) {
				...
				$( event.target ).closest( 'form' ).find( '[type=submit]' ).addClass('sz-show');
			}
		},
	...
	bp.Views.inviteUser = bp.Nouveau.GroupInvites.View.extend( {
		tagName  : 'li',
		template : bp.template( 'sz-invites-users' ),
	...
	bp.Views.invitesEditor = bp.Nouveau.GroupInvites.View.extend( {
		...
		events: {
			'click #sz-invites-send'  : 'sendInvites',
			'click #sz-invites-reset' : 'clearForm'
		},
		...
		sendInvites: function( event ) {
			...
			$( this.el ).addClass( 'sz-hide' );

			bp.Nouveau.GroupInvites.displayFeedback( SZ_Nouveau.group_invites.invites_sending, 'info' );
		...
		invitesSent: function( response ) {
			...
			if ( 1 === bp.Nouveau.GroupInvites.navItems.get( 'invited' ).get( 'hide' ) && ! SZ_Nouveau.group_invites.is_group_create ) {
		...
		cleanViews: function() {
			...
			bp.Nouveau.GroupInvites.displayFeedback( SZ_Nouveau.group_invites.invites_form_reset, 'success' );
		}
	} );

	bp.Views.invitesForm = bp.Nouveau.GroupInvites.View.extend( {
		tagName  : 'div',
		id       : 'sz-send-invites-form',
		template :  bp.template( 'sz-invites-form' )
	} );
	...
	bp.Views.selectedUser = bp.Nouveau.GroupInvites.View.extend( {
		tagName  : 'li',
		template : bp.template( 'sz-invites-selection' ),
		...
		initialize: function() {
			...
			if ( ! this.model.get( 'uninviteTooltip' ) ) {
				this.model.set( 'uninviteTooltip',
					SZ_Nouveau.group_invites.removeUserInvite.replace( '%s', this.model.get( 'name' ) ),
					{ silent: true }
				);
			}
