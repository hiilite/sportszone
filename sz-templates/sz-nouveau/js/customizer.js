...
( function( wp, $ ) {
	...
	$( document ).ready( function() {
		...
		$( wp.customize.control( 'group_front_page' ).selector ).on( 'click', 'input[type=checkbox]', function( event ) {
			...
			_.each( wp.customize.section( 'sz_nouveau_group_front_page' ).controls(), function( control ) {
		...
		$( wp.customize.control( 'user_front_page' ).selector ).on( 'click', 'input[type=checkbox]', function( event ) {
			...
			_.each( wp.customize.section( 'sz_nouveau_user_front_page' ).controls(), function( control ) {
				if ( control.selector !== '#' + controller ) {
					if ( true === checked ) {
						$( control.selector ).show();
					} else {
		...
		$( 'ul#customize-control-group_nav_order, ul#customize-control-user_nav_order' ).sortable( {
			...
			update: function() {
				...
				$( this ).find( '[data-sz-nav]' ).each( function( s, slug ) {
					order.push( $( slug ).data( 'sz-nav' ) );
				} );

				if ( order.length ) {
					$( '#sz_item_' + $( this ).data( 'sz-type' ) ).val( order.join() ).trigger( 'change' );
				}
			}
		} ).disableSelection();
