/* global bp, SZ_Nouveau, _, Backbone */
/* @version 3.1.0 */
window.wp = window.wp || {};
window.bp = window.bp || {};
...
( function( exports, $ ) {
	...
	// Bail if not set
	if ( typeof bp.Nouveau.Activity === 'undefined' || typeof SZ_Nouveau === 'undefined' ) {
		return;
	}
	...
	bp.Nouveau.Activity.postForm = {
		...
		postFormView: function() {
			...
			if ( ! $( '#sz-nouveau-activity-form' ).length ) {
				return;
			}
			...
			// Display it
			postForm.inject( '#sz-nouveau-activity-form' );
		}
	};
		...
		sync: function( method, model, options ) {
			...
			if ( 'read' === method ) {
				...
				options.data = _.extend( options.data || {}, {
					action: 'sz_nouveau_get_activity_objects'
				} );
	...
	bp.Views.activityFeedback = bp.View.extend( {
		...
		initialize: function() {
			...
			this.el.className = 'sz-messages sz-feedback ' + this.type ;
		}
	} );
	...
	bp.Views.WhatsNew = bp.View.extend( {
		...
		attributes: {
			...
			cols         : '50',
			rows         : '4',
			placeholder  : SZ_Nouveau.activity.strings.whatsnewPlaceholder,
			'aria-label' : SZ_Nouveau.activity.strings.whatsnewLabel
		},
	...
	bp.Views.WhatsNewPostIn = bp.View.extend( {
		...
		attributes: {
			...
			'aria-label' : SZ_Nouveau.activity.strings.whatsnewpostinLabel
		},
		...
		autoComplete: function() {
			...
			this.collection.fetch( {
				data: {
					...
					search : search,
					nonce  : SZ_Nouveau.nonces.activity
				},
	...
	bp.Views.FormAvatar = bp.View.extend( {
		...
		initialize: function() {
			this.model = new Backbone.Model( _.pick( SZ_Nouveau.activity.params, [
				'user_id',
				'avatar_url',
				'avatar_width',
				'avatar_height',
	...
	bp.Views.FormTarget = bp.View.extend( {
		...
		initialize: function() {
			var select = new bp.Views.WhatsNewPostIn( { filters: SZ_Nouveau.activity.params.objects } );
	...
	bp.Views.FormSubmit = bp.View.extend( {
		...
		initialize: function() {
			var reset = new bp.Views.ActivityInput( {
				...
				value : SZ_Nouveau.activity.strings.cancelButton
			} );
			...
			var submit = new bp.Views.ActivityInput( {
				...
				value : SZ_Nouveau.activity.strings.postUpdateButton
			} );
	...
	bp.Views.PostForm = bp.View.extend( {
		...
		displayFull: function( event ) {
			...
			if ( true === SZ_Nouveau.activity.params.backcompat ) {
				this.views.add( new bp.Views.FormOptions( { model: this.model } ) );
			}

			// Attach buttons
			if ( ! _.isUndefined( SZ_Nouveau.activity.params.buttons ) ) {
				// Global
				bp.Nouveau.Activity.postForm.buttons.set( SZ_Nouveau.activity.params.buttons );
			...
			if ( ! _.isUndefined( SZ_Nouveau.activity.params.objects ) && 1 < _.keys( SZ_Nouveau.activity.params.objects ).length ) {
		...
		postUpdate: function( event ) {
			...
			var data = {
				'_wpnonce_post_update': SZ_Nouveau.activity.params.post_nonce
			};

			// Add the Akismet nonce if it exists.
			if ( $('#_sz_as_nonce').val() ) {
				data._sz_as_nonce = $('#_sz_as_nonce').val();
			}
			...
			bp.ajax.post( 'post_update', _.extend( data, this.model.attributes ) ).done( function( response ) {
				var store       = bp.Nouveau.getStorage( 'sz-activity' ),
					searchTerms = $( '[data-sz-search="activity"] input[type="search"]' ).val(), matches = {},
					toPrepend = false;
				...
				// Inject the activity into the stream only if it hasn't been done already (HeartBeat).
					...
					if ( ! $( '#activity-stream ul.activity-list').length ) {
						$( '#activity-stream' ).html( $( '<ul></ul>').addClass( 'activity-list item-list sz-list' ) );
					}
