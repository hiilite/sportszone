...
$defaults = array(
	...
	'title' => false,
	'show_title' => get_option( 'sportszone_table_show_title', 'yes' ) == 'yes' ? true : false,
	'show_team_logo' => get_option( 'sportszone_table_show_logos', 'yes' ) == 'yes' ? true : false,
	'link_posts' => null,
	...
	'paginated' => get_option( 'sportszone_table_paginated', 'yes' ) == 'yes' ? true : false,
	'rows' => get_option( 'sportszone_table_rows', 10 ),
);

extract( $defaults, EXTR_SKIP );

if ( ! isset( $link_posts ) ) {
	if ( 'player' === sz_get_post_mode( $id ) ) {
		$link_posts = get_option( 'sportszone_link_players', 'yes' ) == 'yes' ? true : false;
	} else {
		$link_posts = get_option( 'sportszone_link_teams', 'no' ) == 'yes' ? true : false;
	}
}

if ( ! isset( $highlight ) ) $highlight = get_post_meta( $id, 'sz_highlight', true );
...
if ( $columns === null )
	$columns = get_post_meta( $id, 'sz_columns', true );

if ( null !== $columns && ! is_array( $columns ) )
	$columns = explode( ',', $columns );

$output .= '<th class="data-rank">' . __( 'Pos', 'sportszone' ) . '</th>';
...
foreach ( $data as $team_id => $row ):
	...
	// Rank
	$output .= '<td class="data-rank' . $td_class . '" data-label="'.$labels['pos'].'">' . sz_array_value( $row, 'pos' ) . '</td>';

	$name_class = '';

	if ( $show_team_logo ):
		if ( has_post_thumbnail( $team_id ) ):
			$logo = get_the_post_thumbnail( $team_id, 'sportszone-fit-icon' );
	...
	foreach( $labels as $key => $value ):
		...
		if ( ! is_array( $columns ) || in_array( $key, $columns ) )
			$output .= '<td class="data-' . $key . $td_class . '" data-label="'.$labels[$key].'">' . sz_array_value( $row, $key, '&mdash;' ) . '</td>';
	endforeach;
...
if ( $show_full_table_link )
	$output .= '<div class="sp-league-table-link sp-view-all-link"><a href="' . get_permalink( $id ) . '">' . __( 'View full table', 'sportszone' ) . '</a></div>';
?>
