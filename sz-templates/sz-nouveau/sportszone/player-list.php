...
$defaults = array(
	...
	'show_title' => get_option( 'sportszone_list_show_title', 'yes' ) == 'yes' ? true : false,
	'show_player_photo' => get_option( 'sportszone_list_show_photos', 'no' ) == 'yes' ? true : false,
	'show_player_flag' => get_option( 'sportszone_list_show_flags', 'no' ) == 'yes' ? true : false,
	...
	'paginated' => get_option( 'sportszone_list_paginated', 'yes' ) == 'yes' ? true : false,
	'rows' => get_option( 'sportszone_list_rows', 10 ),
);
...
if ( $grouping === 'position' ):
	$groups = get_terms( 'sz_position', array(
...
foreach ( $groups as $group ):
	...
	foreach( $data as $player_id => $row ): if ( empty( $group->term_id ) || has_term( $group->term_id, 'sz_position', $player_id ) ):

		if ( isset( $limit ) && $i >= $limit ) continue;

		$name = sz_array_value( $row, 'name', null );
		if ( ! $name ) continue;
		...
		if ( ! is_array( $labels ) || array_key_exists( 'number', $labels ) ):
			if ( isset( $orderby ) && $orderby != 'number' ):
				...
				$tbody .= '<td class="data-number" data-label="'.$labels['number'].'">' . sz_array_value( $row, 'number', '&nbsp;' ) . '</td>';
			endif;
		endif;
		...
		if ( $show_player_photo ):
			if ( has_post_thumbnail( $player_id ) ):
				$logo = get_the_post_thumbnail( $player_id, 'sportszone-fit-icon' );
		...
		if ( array_key_exists( 'team', $labels ) ):
			$team = sz_array_value( $row, 'team', get_post_meta( $id, 'sz_team', true ) );
		...
		if ( array_key_exists( 'position', $labels ) ):
			$position = sz_array_value( $row, 'position', null );
			if ( null === $position || ! $position ):
				$positions = wp_strip_all_tags( get_the_term_list( $player_id, 'sz_position', '', ', ' ) );
			else:
				$position_term = get_term_by( 'id', $position, 'sz_position', ARRAY_A );
				$positions = sz_array_value( $position_term, 'name', '&mdash;' );
			endif;
		...
		foreach( $labels as $key => $value ):
			...
			$tbody .= '<td class="data-' . $key . '" data-label="'.$labels[$key].'">' . sz_array_value( $row, $key, '&mdash;' ) . '</td>';
		endforeach;
	...
	if ( ! is_array( $labels ) || array_key_exists( 'number', $labels ) ):
		if ( in_array( $orderby, array( 'number', 'name' ) ) ):
			...
			$output .= '<th class="data-rank">' . __( 'Rank', 'sportszone' ) . '</th>';
		endif;
	endif;
	...
	if ( $show_all_players_link ):
		$output .= '<div class="sp-player-list-link sp-view-all-link"><a href="' . get_permalink( $id ) . '">' . __( 'View all players', 'sportszone' ) . '</a></div>';
	endif;
