...
if ( get_option( 'sportszone_event_show_results', 'yes' ) === 'no' ) return;
...
if ( ! isset( $caption ) ) $caption = __( 'Results', 'sportszone' );
...
foreach( $data as $team_id => $result ):
	if ( $show_outcomes ):
		$outcomes = array();
		$result_outcome = sz_array_value( $result, 'outcome' );
		if ( ! is_array( $result_outcome ) ):
			$outcomes = array( '&mdash;' );
		else:
			foreach( $result_outcome as $outcome ):
				$the_outcome = get_page_by_path( $outcome, OBJECT, 'sz_outcome' );
	...
	if ( $link_teams && sz_post_exists( $team_id ) ):
	...
	foreach( $labels as $key => $label ):
		...
		if ( array_key_exists( $key, $result ) && $result[ $key ] != '' ):
			...
			$value = apply_filters( 'sportszone_event_empty_result_string', '&mdash;' );
		endif;
...
if ( empty( $table_rows ) ):
	...
	$output .= '<div class="sp-table-wrapper">' .
		...
		'<th class="data-name">' . __( 'Team', 'sportszone' ) . '</th>';
	foreach( $labels as $key => $label ):
...
echo '<!--sz-templates/sz-nouveau/sportszone/match-results-->';
?>
