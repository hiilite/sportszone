 ...
 * would not be robust enough to predict where SportsZone templates might exist.
 *
 * @package SportsZone
...
/**
 * Get a SportsZone template part for display in a theme.
 *
 * @since 1.7.0
 *
 ...
 * @return false|string Path to located template. See {@link sz_locate_template()}.
 */
function sz_get_template_part( $slug, $name = null ) {

	/**
	 * Fires at the start of sz_get_template_part().
	 *
	 ...
	 * @param string $name      Template part name requested.
	 */
	$templates = apply_filters( 'sz_get_template_part', $templates, $slug, $name );

	// Return the part that is found.
	return sz_locate_template( $templates, true, false );
}

/**
 * Get an asset template part.
 *
 * Basically the same as {@link sz_get_template_part()}, but with 'assets/'
 * prepended to the slug.
 *
 * @since 2.6.0
 *
 * @see sz_get_template_part() for full documentation.
 *
 * @param string      $slug Template slug.
 * @param string|null $name Template name.
 * @return false|string
 */
function sz_get_asset_template_part( $slug, $name = null ) {
	return sz_get_template_part( "assets/{$slug}", $name );
}
 ...
 */
function sz_locate_template( $template_names, $load = false, $require_once = true ) {
	...
	// No file found yet.
	$located            = false;
	$template_locations = sz_get_template_stack();
	...
	/**
	 * This action exists only to follow the standard SportsZone coding convention,
	 ...
	 * 'sz_get_template_part' or add a new location to the template stack.
	 */
	do_action( 'sz_locate_template', $located, $template_name, $template_names, $template_locations, $load, $require_once );
	 ...
	 */
	$load_template = (bool) apply_filters( 'sz_locate_template_and_load', true );
 ...
 *
 * Similar to {@link sz_locate_template()}, but for files like CSS and JS.
 *
 * @since 2.6.0
 *
 ...
 */
function sz_locate_template_asset( $filename ) {
	// Ensure assets can be located when running from /src/.
	if ( defined( 'SZ_SOURCE_SUBDIRECTORY' ) && 'src' === SZ_SOURCE_SUBDIRECTORY ) {
		$filename = str_replace( '.min', '', $filename );
	}

	// Use sz_locate_template() to find our asset.
	$located = sz_locate_template( $filename, false );
	if ( false === $located ) {
		return false;
	}
	...
	$find = array(
		get_theme_root(),
		sz_get_theme_compat_dir()
	);

	$replace = array(
		get_theme_root_uri(),
		sz_get_theme_compat_url()
	);
 ...
 * with sz_locate_template(), this allows for easy template overrides.
 *
 * @since 1.7.0
 *
 ...
 */
function sz_register_template_stack( $location_callback = '', $priority = 10 ) {
	...
	return add_filter( 'sz_template_stack', $location_callback, (int) $priority );
}
 ...
 * @since 1.7.0
 *
 * @see sz_register_template_stack()
 *
 * @param string $location_callback Callback function that returns the stack location.
 * @param int    $priority          Optional. The priority parameter passed to
 *                                  {@link sz_register_template_stack()}. Default: 10.
 * @return bool See {@link remove_filter()}.
 */
function sz_deregister_template_stack( $location_callback = '', $priority = 10 ) {
	...
	// Add location callback to template stack.
	return remove_filter( 'sz_template_stack', $location_callback, (int) $priority );
}

/**
 * Get the "template stack", a list of registered directories where templates can be found.
 *
 * Calls the functions added to the 'sz_template_stack' filter hook, and return
 ...
 */
function sz_get_template_stack() {
	global $wp_filter, $merged_filters, $wp_current_filter;

	// Setup some default variables.
	$tag  = 'sz_template_stack';
	$args = $stack = array();

	// Add 'sz_template_stack' to the current filter array.
	...
	// Loop through 'sz_template_stack' filters, and call callback functions.
	do {
	...
	// Remove 'sz_template_stack' from the current filter array.
	 ...
	 */
	return (array) apply_filters( 'sz_get_template_stack', $stack ) ;
}
 ...
 * @since 1.7.0
 *
 * @see sz_get_template_part() for a description of $slug and $name params.
 *
 * @param string      $slug See {@link sz_get_template_part()}.
 * @param string|null $name See {@link sz_get_template_part()}.
 ...
 */
function sz_buffer_template_part( $slug, $name = null, $echo = true ) {
	ob_start();

	// Remove 'sz_replace_the_content' filter to prevent infinite loops.
	remove_filter( 'the_content', 'sz_replace_the_content' );

	sz_get_template_part( $slug, $name );
 ...
 * the use of {@link sz_locate_template()}. Allows for more generic template
 ...
 */
function sz_get_query_template( $type, $templates = array() ) {
	 ...
	 * sz_get_query_template.
	 *
	 * @since 1.7.0
	 *
	 * @param array $templates Array of template files already prepared.
	 */
	$templates = apply_filters( "sz_get_{$type}_template", $templates );

	// Filter possible templates, try to match one, and set any SportsZone theme
	// compat properties so they can be cross-checked later.
	$templates = sz_set_theme_compat_templates( $templates );
	$template  = sz_locate_template( $templates );
	$template  = sz_set_theme_compat_template( $template );
	 ...
	 */
	return apply_filters( "sz_{$type}_template", $template );
}
 ...
 */
function sz_get_template_locations( $templates = array() ) {
	$locations = array(
		'sportszone',
		'community',
		''
	 ...
	 */
	return apply_filters( 'sz_get_template_locations', $locations, $templates );
}
 ...
 */
function sz_add_template_stack_locations( $stacks = array() ) {
	$retval = array();

	// Get alternate locations.
	$locations = sz_get_template_locations();
	 ...
	 * @param array $stacks Array of template locations.
	 */
	return apply_filters( 'sz_add_template_stack_locations', array_unique( $retval ), $stacks );
}

/**
 * Add checks for SportsZone conditions to 'parse_query' action.
 *
 * @since 1.7.0
 *
 * @param WP_Query $posts_query WP_Query object.
 */
function sz_parse_query( $posts_query ) {
	...
	/**
	 * Fires at the end of the sz_parse_query function.
	 *
	 * Allow SportsZone components to parse the main query.
	 *
	 * @since 1.7.0
	 *
	 * @param WP_Query $posts_query WP_Query instance. Passed by reference.
	 */
	do_action_ref_array( 'sz_parse_query', array( &$posts_query ) );
}

/**
 * Possibly intercept the template being loaded.
 *
 * Listens to the 'template_include' filter and waits for any SportsZone specific
 ...
 */
function sz_template_include_theme_supports( $template = '' ) {
	 ...
	 * @param string $template The path to the template file that is being used.
	 */
	$new_template = apply_filters( 'sz_get_root_template', false, $template );

	// A SportsZone template file was located, so override the WordPress
	// template and use it to switch off SportsZone's theme compatibility.
	if ( ! empty( $new_template ) ) {
		$template = sz_set_template_included( $new_template );
	}
	 ...
	 * @param string $template The path to the template file that is being used.
	 */
	return apply_filters( 'sz_template_include_theme_supports', $template );
}
 ...
 */
function sz_set_template_included( $template = false ) {
	sportszone()->theme_compat->found_template = $template;

	return sportszone()->theme_compat->found_template;
}

/**
 * Is a SportsZone template being included?
 *
 * @since 1.8.0
 *
 * @return bool True if yes, false if no.
 */
function sz_is_template_included() {
	return ! empty( sportszone()->theme_compat->found_template );
}
 ...
 */
function sz_load_theme_functions() {
	...
	// Do not include on SportsZone deactivation.
	...
	if ( ! defined( 'WP_INSTALLING' ) || 'wp-activate.php' === $pagenow ) {
		sz_locate_template( 'sportszone-functions.php', true );
	}
}

/**
 * Get the templates to use as the endpoint for SportsZone template parts.
 *
 * @since 1.7.0
 ...
 */
function sz_get_theme_compat_templates() {
	return sz_get_query_template( 'sportszone', array(
		'plugin-sportszone.php',
		'sportszone.php',
		'community.php',
		'generic.php',
		'page.php',
