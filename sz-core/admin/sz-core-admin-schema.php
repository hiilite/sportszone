 ...
 */
function sz_core_install( $active_components = false ) {

	sz_pre_schema_upgrade();
	...
	if ( !empty( $active_components['xprofile'] ) ) {
		sz_core_install_extended_profiles();
	}

	// Blog tracking.
	if ( !empty( $active_components['blogs'] ) ) {
		sz_core_install_blog_tracking();
	}
}
 ...
 *
 */
function sz_core_install_notifications() {
	$sql             = array();
	$charset_collate = $GLOBALS['wpdb']->get_charset_collate();
	$sz_prefix       = sz_core_get_table_prefix();

	$sql[] = "CREATE TABLE {$sz_prefix}sz_notifications (
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_notifications_meta (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
 ...
 *
 */
function sz_core_install_activity_streams() {
	$sql             = array();
	$charset_collate = $GLOBALS['wpdb']->get_charset_collate();
	$sz_prefix       = sz_core_get_table_prefix();

	$sql[] = "CREATE TABLE {$sz_prefix}sz_activity (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
				user_id bigint(20) NOT NULL,
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_activity_meta (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
 ...
 *
 */
function sz_core_install_friends() {
	$sql             = array();
	$charset_collate = $GLOBALS['wpdb']->get_charset_collate();
	$sz_prefix       = sz_core_get_table_prefix();

	$sql[] = "CREATE TABLE {$sz_prefix}sz_friends (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
 ...
 *
 */
function sz_core_install_groups() {
	$sql             = array();
	$charset_collate = $GLOBALS['wpdb']->get_charset_collate();
	$sz_prefix       = sz_core_get_table_prefix();

	$sql[] = "CREATE TABLE {$sz_prefix}sz_groups (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_groups_members (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_groups_groupmeta (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
				group_id bigint(20) NOT NULL,
				meta_key varchar(255) DEFAULT NULL,
				meta_value longtext DEFAULT NULL,
 ...
 *
 */
function sz_core_install_private_messaging() {
	$sql             = array();
	$charset_collate = $GLOBALS['wpdb']->get_charset_collate();
	$sz_prefix       = sz_core_get_table_prefix();

	$sql[] = "CREATE TABLE {$sz_prefix}sz_messages_messages (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_messages_recipients (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
				user_id bigint(20) NOT NULL,
				thread_id bigint(20) NOT NULL,
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_messages_notices (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
				subject varchar(200) NOT NULL,
				message longtext NOT NULL,
				date_sent datetime NOT NULL,
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_messages_meta (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
 ...
 *
 */
function sz_core_install_extended_profiles() {
	global $wpdb;
	...
	if ( ! sz_get_option( 'sz-xprofile-base-group-name' ) ) {
		sz_update_option( 'sz-xprofile-base-group-name', _x( 'General', 'First field-group name', 'sportszone' ) );
	}

	if ( ! sz_get_option( 'sz-xprofile-fullname-field-name' ) ) {
		sz_update_option( 'sz-xprofile-fullname-field-name', _x( 'Display Name', 'Display name field', 'sportszone' ) );
	}

	$sql[] = "CREATE TABLE {$sz_prefix}sz_xprofile_groups (
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_xprofile_fields (
				id bigint(20) unsigned NOT NULL AUTO_INCREMENT PRIMARY KEY,
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_xprofile_data (
				id bigint(20) unsigned NOT NULL AUTO_INCREMENT PRIMARY KEY,
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_xprofile_meta (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
	...
	if ( ! $wpdb->get_var( "SELECT id FROM {$sz_prefix}sz_xprofile_groups WHERE id = 1" ) ) {
		$insert_sql[] = "INSERT INTO {$sz_prefix}sz_xprofile_groups ( name, description, can_delete ) VALUES ( " . $wpdb->prepare( '%s', stripslashes( sz_get_option( 'sz-xprofile-base-group-name' ) ) ) . ", '', 0 );";
	}

	if ( ! $wpdb->get_var( "SELECT id FROM {$sz_prefix}sz_xprofile_fields WHERE id = 1" ) ) {
		$insert_sql[] = "INSERT INTO {$sz_prefix}sz_xprofile_fields ( group_id, parent_id, type, name, description, is_required, can_delete ) VALUES ( 1, 0, 'textbox', " . $wpdb->prepare( '%s', stripslashes( sz_get_option( 'sz-xprofile-fullname-field-name' ) ) ) . ", '', 1, 0 );";
	}
 ...
 *
 */
function sz_core_install_blog_tracking() {
	$sql             = array();
	$charset_collate = $GLOBALS['wpdb']->get_charset_collate();
	$sz_prefix       = sz_core_get_table_prefix();

	$sql[] = "CREATE TABLE {$sz_prefix}sz_user_blogs (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
				user_id bigint(20) NOT NULL,
	...
	$sql[] = "CREATE TABLE {$sz_prefix}sz_user_blogs_blogmeta (
				id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
				blog_id bigint(20) NOT NULL,
				meta_key varchar(255) DEFAULT NULL,
				meta_value longtext DEFAULT NULL,
 ...
 * @global $wpdb
 */
function sz_core_install_signups() {
	global $wpdb;
 ...
 * @link https://sportszone.trac.wordpress.org/ticket/5563 SportsZone Trac Ticket
 *
 * @global WPDB $wpdb
 */
function sz_core_upgrade_signups() {
	global $wpdb;
 ...
 * @since 2.5.0
 */
function sz_core_install_emails() {
	$defaults = array(
		'post_status' => 'publish',
		'post_type'   => sz_get_email_post_type(),
	);

	$emails       = sz_email_get_schema();
	$descriptions = sz_email_get_type_schema( 'description' );
	...
	foreach ( $emails as $id => $email ) {
		...
		$post_id = wp_insert_post( sz_parse_args( $email, $defaults, 'install_email_' . $id ) );
		if ( ! $post_id ) {
			continue;
		}

		$tt_ids = wp_set_object_terms( $post_id, $id, sz_get_email_tax_type() );
		foreach ( $tt_ids as $tt_id ) {
			$term = get_term_by( 'term_taxonomy_id', (int) $tt_id, sz_get_email_tax_type() );
			wp_update_term( (int) $term->term_id, sz_get_email_tax_type(), array(
				'description' => $descriptions[ $id ],
			) );
		}
	}

	sz_update_option( 'sz-emails-unsubscribe-salt', base64_encode( wp_generate_password( 64, true, true ) ) );

	/**
	 * Fires after SportsZone adds the posts for its emails.
	 *
	 * @since 2.5.0
	 */
	do_action( 'sz_core_install_emails' );
}
