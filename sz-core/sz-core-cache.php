<?php
/**
 * SportsZone Core Caching Functions.
 *
 ...
 */
function sz_core_clear_user_object_cache( $user_id ) {
	wp_cache_delete( 'sz_user_' . $user_id, 'sz' );
}
 ...
 * @since 1.6.0
 */
function sz_core_clear_member_count_caches() {
	wp_cache_delete( 'sz_total_member_count', 'sz' );
	delete_transient( 'sz_active_member_count' );
}
add_action( 'sz_core_activated_user',         'sz_core_clear_member_count_caches' );
add_action( 'sz_core_process_spammer_status', 'sz_core_clear_member_count_caches' );
add_action( 'sz_core_deleted_account',        'sz_core_clear_member_count_caches' );
add_action( 'sz_first_activity_for_member',   'sz_core_clear_member_count_caches' );
add_action( 'deleted_user',                   'sz_core_clear_member_count_caches' );
 ...
 */
function sz_core_clear_directory_pages_cache_page_edit( $post_id = 0 ) {
	...
	wp_cache_delete( 'directory_pages', 'sz_pages' );
}
add_action( 'save_post', 'sz_core_clear_directory_pages_cache_page_edit' );

/**
 * Clear the directory_pages cache when the sz-pages option is updated.
 *
 * @since 2.0.0
 *
 * @param string $option Option name.
 */
function sz_core_clear_directory_pages_cache_settings_edit( $option ) {
	if ( 'sz-pages' === $option ) {
		wp_cache_delete( 'directory_pages', 'sz_pages' );
	}
}
add_action( 'update_option', 'sz_core_clear_directory_pages_cache_settings_edit' );
 ...
 * @param string $option Option name.
 */
function sz_core_clear_root_options_cache( $option ) {
	foreach ( array( 'add_option', 'add_site_option', 'update_option', 'update_site_option' ) as $action ) {
		remove_action( $action, 'sz_core_clear_root_options_cache' );
	}

	// Surrounding code prevents infinite loops on WP < 4.4.
	$keys = array_keys( sz_get_default_options() );

	foreach ( array( 'add_option', 'add_site_option', 'update_option', 'update_site_option' ) as $action ) {
		add_action( $action, 'sz_core_clear_root_options_cache' );
	}
	...
	if ( in_array( $option, $keys ) ) {
		wp_cache_delete( 'root_blog_options', 'sz' );
	}
}
add_action( 'update_option', 'sz_core_clear_root_options_cache' );
add_action( 'update_site_option', 'sz_core_clear_root_options_cache' );
add_action( 'add_option', 'sz_core_clear_root_options_cache' );
add_action( 'add_site_option', 'sz_core_clear_root_options_cache' );
 ...
 * @return array
 */
function sz_get_non_cached_ids( $item_ids, $cache_group ) {
	$uncached = array();
 ...
 */
function sz_update_meta_cache( $args = array() ) {
	global $wpdb;
	...
	$defaults = array(
		...
		'cache_key_prefix' => ''       // Prefix to use when creating cache key names. Eg 'sz_groups_groupmeta'.
	);
	...
	$uncached_ids = sz_get_non_cached_ids( $object_ids, $cache_group );
 ...
 *
 * A utility function for use by query methods like SZ_Activity_Activity::get().
 *
 * @since 2.7.0
 * @see sz_core_set_incremented_cache()
 *
 * @param string $key   Unique key for the query. Usually a SQL string.
 * @param string $group Cache group. Eg 'sz_activity'.
 * @return array|bool False if no cached values are found, otherwise an array of IDs.
 */
function sz_core_get_incremented_cache( $key, $group ) {
	$cache_key = sz_core_get_incremented_cache_key( $key, $group );
 ...
 * underlying set of objects has been modified. See, eg, SZ_Activity_Activity::get().
 *
 * @since 2.7.0
 *
 * @param string $key   Unique key for the query. Usually a SQL string.
 ...
 * @return bool
 */
function sz_core_set_incremented_cache( $key, $group, $ids ) {
	$cache_key = sz_core_get_incremented_cache_key( $key, $group );
 ...
 */
function sz_core_delete_incremented_cache( $key, $group ) {
	$cache_key = sz_core_get_incremented_cache_key( $key, $group );
 ...
 * @return string
 */
function sz_core_get_incremented_cache_key( $key, $group ) {
	$incrementor = sz_core_get_incrementor( $group );
 ...
 * @return string
 */
function sz_core_get_incrementor( $group ) {
 ...
 */
function sz_core_reset_incrementor( $group ) {
