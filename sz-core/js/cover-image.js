...
( function( exports, $ ) {
	...
	bp.CoverImage = {
		...
		deleteView: function() {
			...
			// Display it
			deleteView.inject( '.sz-cover-image-manage' );
		},
		...
		uploadResult: function( model ) {
			...
			if ( ! _.isUndefined( model.get( 'url' ) ) ) {
				...
				if ( 0 === model.get( 'feedback_code' ) ) {
					message = SZ_Uploader.strings.cover_image_warnings.dimensions;
					type    = 'warning';

				// Success, Rock n roll!
				} else {
					message = SZ_Uploader.strings.feedback_messages[ model.get( 'feedback_code' ) ];
					type = 'success';
				}

				this.views.set( '.sz-uploader-progress', new bp.Views.CoverImageStatus( {
					value : message,
					type  : type
				} ) );
				...
				bp.CoverImage.Attachment.set( _.extend(
					_.pick( SZ_Uploader.settings.defaults.multipart_params.sz_params, ['object', 'item_id'] ),
