/* global bp, plupload, SZ_Uploader, _, JSON, Backbone */
...
( function( exports, $ ) {
	 ...
	 * and make sure previously defined SportsZone attributes
	 * are not removed (eg: bp.mentions)
	 */
	_.extend( bp, _.pick( wp, 'Backbone', 'ajax', 'template' ) );

	// Init Models, Collections, Views and the SportsZone Uploader
	bp.Models      = bp.Models || {};
	...
	bp.Uploader.uploader = function() {
		...
		this.params  = SZ_Uploader.settings;
		this.strings = SZ_Uploader.strings;
		...
		this.uploader.bind( 'Init', function( uploader ) {
			 ...
			 * difficulties, we're adding a ui_available_width argument to the sz_params
		...
		this.uploader.bind( 'FilesAdded', function( uploader, files ) {
			...
			if ( ! uploader.settings.multi_selection && files.length > 1 ) {
				...
				$( self ).trigger( 'sz-uploader-warning', self.strings.unique_file_warning );
				return;
			}
		...
		this.uploader.bind( 'BeforeUpload', function( uploader, files ) {
			$( self ).trigger( 'sz-uploader-new-upload', uploader, files );
		} );
		...
		this.uploader.bind( 'Error', function( uploader, pluploadError ) {
			...
			$( self ).trigger( 'sz-uploader-warning', message );
			uploader.refresh();
		} );
	};
	...
	// SportsZone Uploader main view
	bp.Views.Uploader = bp.View.extend( {
		className: 'sz-uploader-window',
		template: bp.template( 'upload-window' ),

		defaults: _.pick( SZ_Uploader.settings.defaults, 'container', 'drop_element', 'browse_button' ),
		...
		initUploader: function() {
			...
			$( this.uploader ).on( 'sz-uploader-warning', _.bind( this.setWarning, this ) );
			$( this.uploader ).on( 'sz-uploader-new-upload', _.bind( this.resetWarning, this ) );
		},
	...
	// SportsZone Uploader warning view
	...
	// SportsZone Uploader Files view
	bp.Views.uploaderStatus = bp.View.extend( {
		...
		progress:function( model ) {
			if ( ! _.isUndefined( model.get( 'percent' ) ) ) {
				$( '#' + model.get('id') + ' .sz-progress .sz-bar' ).css( 'width', model.get('percent') + '%' );
			}
		},
	...
	// SportsZone Uploader File progress view
	bp.Views.uploaderProgress = bp.View.extend( {
		className: 'sz-uploader-progress',
