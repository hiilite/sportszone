...
( function( exports, $ ) {
	...
	bp.Avatar = {
		start: function() {
			...
			$( 'body.wp-admin' ).on( 'click', '.sz-xprofile-avatar-user-edit', function() {
				self.resetViews();
			} );
		},
		...
		removeLegacyUI: function() {
			...
			} else if ( $( '#sz_xprofile_user_admin_avatar a.sz-xprofile-avatar-user-admin' ).length ) {
				$( '#sz_xprofile_user_admin_avatar a.sz-xprofile-avatar-user-admin' ).remove();
			}
		},
		...
		resetViews: function() {
			...
			this.nav.trigger( 'sz-avatar-view:changed', 'upload' );
		...
		setupNav: function() {
			...
			this.nav.on( 'sz-avatar-view:changed', _.bind( this.setView, this ) );
		},
		...
		uploadProgress: function() {
			...
			// Display it
			avatarStatus.inject( '.sz-avatar-status' );
		},
		...
		displayWarning: function( message ) {
			...
			this.warning.inject( '.sz-avatar-status' );
		}
	};
	...
	bp.Views.Nav = bp.View.extend( {
		...
		events: {
			'click .sz-avatar-nav-item' : 'toggleView'
		},
		...
		toggleView: function( event ) {
			...
			_.each( this.collection.models, function( model ) {
				if ( model.id === active ) {
					model.set( { active: 1 } );
					this.trigger( 'sz-avatar-view:changed', model.id );
				} else {
					model.set( { active: 0 } );
				}
			}, this );
	...
	bp.Views.NavItem = bp.View.extend( {
		...
		template: bp.template( 'sz-avatar-nav' ),

		initialize: function() {
			if ( 1 === this.model.get( 'active' ) ) {
				this.el.className += ' current';
			}
			this.el.id += 'sz-avatar-' + this.model.get( 'id' );
	...
	bp.Views.Avatars = bp.View.extend( {
		...
		addItemView: function( item ) {
			...
			// Make sure to take in account sz_core_avatar_full_height or sz_core_avatar_full_width php filters
	...
	bp.Views.Avatar = bp.View.extend( {
		className: 'item',
		template: bp.template( 'sz-avatar-item' ),
	...
	// SportsZone Avatar Feedback view
	bp.Views.AvatarStatus = bp.View.extend( {
		tagName: 'p',
		className: 'updated',
		id: 'sz-avatar-feedback',
	...
	// SportsZone Avatar Delete view
	bp.Views.DeleteAvatar = bp.View.extend( {
		tagName: 'div',
		id: 'sz-delete-avatar-container',
		template: bp.template( 'sz-avatar-delete' ),

		events: {
			'click #sz-delete-avatar': 'deleteAvatar'
		},
