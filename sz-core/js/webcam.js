...
( function() {
	...
	bp.WebCam = {
		start: function() {
			...
			bp.Avatar.nav.on( 'sz-avatar-view:changed', _.bind( this.setView, this ) );
		},
		...
		displayWarning: function( code ) {
			...
			this.params.warning = new bp.Views.uploaderWarning( {
				value: SZ_Uploader.strings.camera_warnings[code]
			} );

			this.params.warning.inject( '.sz-avatar-status' );
		}
	};

	// SportsZone WebCam view
	bp.Views.WebCamAvatar = bp.View.extend( {
		tagName: 'div',
		id: 'sz-webcam-avatar',
		template: bp.template( 'sz-avatar-webcam' ),
		...
		initialize: function() {
			...
			if ( typeof navigator.getUserMedia !== 'undefined' ) {
				...
				params = _.extend( _.pick( SZ_Uploader.settings.defaults.multipart_params.sz_params,
					'object',
					'item_id',
					'nonces'
					), {
						user_media:  true,
						w: SZ_Uploader.settings.crop.full_w,
						h: SZ_Uploader.settings.crop.full_h,
						x: 0,
						y: 0,
						type: 'camera'
					}
	...
	// SportsZone Canvas (capture) view
	bp.Views.WebCamCanvas = bp.View.extend( {
		...
		initialize: function() {
			...
			if ( ! _.isUndefined( SZ_Uploader.settings.crop.full_h ) && ! _.isUndefined( SZ_Uploader.settings.crop.full_w ) ) {
				this.el.attributes.width.value  = SZ_Uploader.settings.crop.full_w;
				this.el.attributes.height.value = SZ_Uploader.settings.crop.full_h;
			}
		}
	} );
