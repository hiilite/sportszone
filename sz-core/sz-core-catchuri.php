<?php
/**
 * SportsZone URI catcher.
 *
 * Functions for parsing the URI and determining which SportsZone template file
 * to use on-screen.
 *
 * @package SportsZone
 * @subpackage Core
...
/**
 * Analyze the URI and break it down into SportsZone-usable chunks.
 *
 * SportsZone can use complete custom friendly URIs without the user having to
 ...
 *   - OUTSIDE ROOT: http:// example.com / sites / sportszone / members / andy / [current_component] / [current_action] / [action_variables] / [action_variables] / ...
 *
 * Example:
 *    - http://example.com/members/andy/profile/edit/group/5/
 *    - $sz->current_component: string 'xprofile'
 *    - $sz->current_action: string 'edit'
 *    - $sz->action_variables: array ['group', 5]
 *
 * @since 1.0.0
 */
function sz_core_set_uri_globals() {
	global $current_blog, $wp_rewrite;

	// Don't catch URIs on non-root blogs unless multiblog mode is on.
	if ( !sz_is_root_blog() && !sz_is_multiblog_mode() )
		return false;
	...
	if ( defined( 'DOING_AJAX' ) && DOING_AJAX || strpos( $_SERVER['REQUEST_URI'], 'wp-load.php' ) )
		$path = sz_get_referer_path();
	else
		$path = esc_url( $_SERVER['REQUEST_URI'] );

	/**
	 * Filters the SportsZone global URI path.
	 *
	 * @since 1.0.0
	 *
	 * @param string $path Path to set.
	 */
	$path = apply_filters( 'sz_uri', $path );
	...
	foreach ( (array) $sz_uri as $key => $uri_chunk ) {
		if ( empty( $sz_uri[$key] ) ) {
			unset( $sz_uri[$key] );
		}
	}
	 ...
	 * removed from $sz_uri. This includes two cases:
	 *
	 ...
	 */
	if ( is_multisite() && !is_subdomain_install() && ( sz_is_multiblog_mode() || 1 != sz_get_root_blog_id() ) ) {
		...
		if ( !empty( $chunks ) ) {
			...
			foreach( $chunks as $key => $chunk ) {
				$bkey = array_search( $chunk, $sz_uri );
				...
				$sz_uri = array_values( $sz_uri );
			}
		}
	}

	// Get site path items.
	$paths = explode( '/', sz_core_get_site_path() );
	...
	$sz_uri = array_merge( array(), $sz_uri );

	/*
	 * If a component is set to the front page, force its name into $sz_uri
	 ...
	 */
	if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_on_front' ) && empty( $sz_uri ) && empty( $_GET['p'] ) && empty( $_GET['page_id'] ) ) {
		$post = get_post( get_option( 'page_on_front' ) );
		if ( !empty( $post ) ) {
			$sz_uri[0] = $post->post_name;
		}
	}

	// Keep the unfiltered URI safe.
	$sz->unfiltered_uri = $sz_uri;

	// Don't use $sz_unfiltered_uri, this is only for backpat with old plugins. Use $sz->unfiltered_uri.
	$GLOBALS['sz_unfiltered_uri'] = &$sz->unfiltered_uri;

	// Get slugs of pages into array.
	foreach ( (array) $sz->pages as $page_key => $sz_page )
		$key_slugs[$page_key] = trailingslashit( '/' . $sz_page->slug );
	...
	// URLs with SZ_ENABLE_ROOT_PROFILES enabled won't be caught above.
	if ( empty( $matches ) && sz_core_enable_root_profiles() && ! empty( $sz_uri[0] ) ) {

		// Switch field based on compat.
		$field = sz_is_username_compatibility_mode() ? 'login' : 'slug';
		 ...
		 */
		$member_slug = apply_filters( 'sz_core_set_uri_globals_member_slug', $sz_uri[0] );

		// Make sure there's a user corresponding to $sz_uri[0].
		if ( ! empty( $sz->pages->members ) && $root_profile = get_user_by( $field, $member_slug ) ) {

			// Force BP to recognize that this is a members page.
			$matches[]  = 1;
			$match      = $sz->pages->members;
			$match->key = 'members';
		}
	}

	// Search doesn't have an associated page, so we check for it separately.
	if ( !empty( $sz_uri[0] ) && ( sz_get_search_slug() == $sz_uri[0] ) ) {
		$matches[]   = 1;
		$match       = new stdClass;
		$match->key  = 'search';
		$match->slug = sz_get_search_slug();
	}

	// This is not a SportsZone page, so just return.
	if ( empty( $matches ) )
		return false;
	...
	// Global the unfiltered offset to use in sz_core_load_template().
	// To avoid PHP warnings in sz_core_load_template(), it must always be >= 0.
	$sz->unfiltered_uri_offset = $uri_offset >= 0 ? $uri_offset : 0;

	// We have an exact match.
	if ( isset( $match->key ) ) {

		// Set current component to matched key.
		$sz->current_component = $match->key;

		// If members component, do more work to find the actual component.
		if ( 'members' == $match->key ) {

			$after_member_slug = false;
			if ( ! empty( $sz_uri[ $uri_offset + 1 ] ) ) {
				$after_member_slug = $sz_uri[ $uri_offset + 1 ];
			}

			// Are we viewing a specific user?
			if ( $after_member_slug ) {

				/** This filter is documented in sz-core/sz-core-catchuri.php */
				$after_member_slug = apply_filters( 'sz_core_set_uri_globals_member_slug', $after_member_slug );

				// If root profile, we've already queried for the user.
				if ( $root_profile instanceof WP_User ) {
					$sz->displayed_user->id = $root_profile->ID;

				// Switch the displayed_user based on compatibility mode.
				} elseif ( sz_is_username_compatibility_mode() ) {
					$sz->displayed_user->id = (int) sz_core_get_userid( urldecode( $after_member_slug ) );

				} else {
					$sz->displayed_user->id = (int) sz_core_get_userid_from_nicename( $after_member_slug );
				}
			}

			// Is this a member type directory?
			if ( ! sz_displayed_user_id() && $after_member_slug === sz_get_members_member_type_base() && ! empty( $sz_uri[ $uri_offset + 2 ] ) ) {
				$matched_types = sz_get_member_types( array(
					'has_directory'  => true,
					'directory_slug' => $sz_uri[ $uri_offset + 2 ],
				) );

				if ( ! empty( $matched_types ) ) {
					$sz->current_member_type = reset( $matched_types );
					unset( $sz_uri[ $uri_offset + 1 ] );
				}
			}

			// If the slug matches neither a member type nor a specific member, 404.
			if ( ! sz_displayed_user_id() && ! sz_get_current_member_type() && $after_member_slug ) {
			...
			if ( sz_displayed_user_id() && sz_is_user_spammer( sz_displayed_user_id() ) ) {
				if ( sz_current_user_can( 'sz_moderate' ) ) {
					sz_core_add_message( __( 'This user has been marked as a spammer. Only site admins can view this profile.', 'sportszone' ), 'warning' );
				} else {
					sz_do_404();
					return;
				}
			...
			if ( sz_displayed_user_id() ) {
				if ( isset( $sz_uri[$uri_offset + 2] ) ) {
					$sz_uri                = array_merge( array(), array_slice( $sz_uri, $uri_offset + 2 ) );
					$sz->current_component = $sz_uri[0];
	...
	$current_action = isset( $sz_uri[ $uri_offset + 1 ] ) ? $sz_uri[ $uri_offset + 1 ] : '';

	/*
	 * If a SportsZone directory is set to the WP front page, URLs like example.com/members/?s=foo
	...
	// Slice the rest of the $sz_uri array and reset offset.
	$sz_uri      = array_slice( $sz_uri, $uri_offset + 2 );
	$uri_offset  = 0;

	// Set the entire URI as the action variables, we will unset the current_component and action in a second.
	$sz->action_variables = $sz_uri;

	// Reset the keys by merging with an empty array.
	$sz->action_variables = array_merge( array(), $sz->action_variables );
}
 ...
 */
function sz_core_enable_root_profiles() {

	$retval = false;

	if ( defined( 'SZ_ENABLE_ROOT_PROFILES' ) && ( true == SZ_ENABLE_ROOT_PROFILES ) )
		$retval = true;
	 ...
	 */
	return apply_filters( 'sz_core_enable_root_profiles', $retval );
}
 ...
 *
 * Example:
 *   sz_core_load_template( 'members/index' );
 * Loads:
 ...
 */
function sz_core_load_template( $templates ) {
	global $wp_query;
	...
	sz_set_theme_compat_active( false );
	...
	// Only perform template lookup for sz-default themes.
	if ( ! sz_use_theme_compat_with_current_theme() ) {
	 ...
	 */
	$located_template = apply_filters( 'sz_located_template', $template, $filtered_templates );

	/*
	 * If current page is an embed, wipe out sz-default template.
	 *
	 * Wiping out the sz-default template allows WordPress to use their special
	...
	if ( !empty( $located_template ) ) {
		 ...
		 */
		do_action( 'sz_core_pre_load_template', $located_template );
		 ...
		 * @param string $located_template Template found to be loaded.
		 */
		load_template( apply_filters( 'sz_load_template', $located_template ) );
		 ...
		 */
		do_action( 'sz_core_post_load_template', $located_template );
	...
	// @todo Some other 404 handling if theme compat doesn't kick in.
		...
		// The rest will be done by sz_theme_compat_reset_post() later.
		if ( is_sportszone() ) {
			status_header( 200 );
			$wp_query->is_page     = true;
			$wp_query->is_singular = true;
		 ...
		 * @since 1.7.0
		 */
		do_action( 'sz_setup_theme_compat' );
	}
}
 ...
 * @since 1.0.0
 */
function sz_core_catch_profile_uri() {
	if ( !sz_is_active( 'xprofile' ) ) {
		 ...
		 */
		sz_core_load_template( apply_filters( 'sz_core_template_display_profile', 'members/single/home' ) );
	}
}
 ...
 */
function sz_core_members_shortlink_redirector( $member_slug ) {
	 ...
	 */
	$me_slug = apply_filters( 'sz_core_members_shortlink_slug', 'me' );
	...
	if ( false === is_user_logged_in() ) {
		...
		add_action( 'template_redirect', 'sz_core_no_access', 0 );
	...
	return sz_core_get_username( $user->ID, $user->user_nicename, $user->user_login );
}
add_filter( 'sz_core_set_uri_globals_member_slug', 'sz_core_members_shortlink_redirector' );

/**
 * Catch unauthorized access to certain SportsZone pages and redirect accordingly.
 *
 * @since 1.5.0
 */
function sz_core_catch_no_access() {
	global $wp_query;

	$sz = sportszone();

	// If coming from sz_core_redirect() and $sz_no_status_set is true,
	// we are redirecting to an accessible page so skip this check.
	if ( !empty( $sz->no_status_set ) )
		return false;

	if ( !isset( $wp_query->queried_object ) && !sz_is_blog_page() ) {
		sz_do_404();
	}
}
add_action( 'sz_template_redirect', 'sz_core_catch_no_access', 1 );
 ...
 *                            Default: the value of {@link sz_get_root_domain()}.
 ...
 * }
 */
function sz_core_no_access( $args = '' ) {
	...
	$defaults = array(
		...
		'root'     => sz_get_root_domain(), // the landing page you get redirected to when a user doesn't have access.
		'message'  => __( 'You must log in to access the page you requested.', 'sportszone' )
	);
	 ...
	 */
	$r = apply_filters( 'sz_core_no_access', $r );
	extract( $r, EXTR_SKIP );

	/*
	 * @ignore Ignore these filters and use 'sz_core_no_access' above.
	 */
	$mode     = apply_filters( 'sz_no_access_mode',     $mode,     $root,     $redirect, $message );
	$redirect = apply_filters( 'sz_no_access_redirect', $redirect, $root,     $message,  $mode    );
	$root     = apply_filters( 'sz_no_access_root',     $root,     $redirect, $message,  $mode    );
	$message  = apply_filters( 'sz_no_access_message',  $message,  $root,     $redirect, $mode    );
	$root     = trailingslashit( $root );

	switch ( $mode ) {

		// Option to redirect to wp-login.php.
		// Error message is displayed with sz_core_no_access_wp_login_error().
		case 2 :
			if ( !empty( $redirect ) ) {
				sz_core_redirect( add_query_arg( array(
					'sz-auth' => 1,
					'action'  => 'bpnoaccess'
				), wp_login_url( $redirect ) ) );
			} else {
				sz_core_redirect( $root );
			}

			break;

		// Redirect to root with "redirect_to" parameter.
		// Error message is displayed with sz_core_add_message().
		case 1 :
		default :
 ...
 * @since 2.9.0
 */
function sz_login_redirector() {
	// Redirect links must include the `redirect_to` and `sz-auth` parameters.
	if ( empty( $_GET['redirect_to'] ) || empty( $_GET['sz-auth'] ) ) {
		return;
	}
...
add_action( 'login_init', 'sz_login_redirector', 1 );

/**
 * Add a custom SportsZone no access error message to wp-login.php.
 *
 * @since 1.5.0
 ...
 * @return WP_Error
 */
function sz_core_no_access_wp_login_error( $errors ) {
	 ...
	 */
	$message = apply_filters( 'sz_wp_login_error', __( 'You must log in to access the page you requested.', 'sportszone' ), $_REQUEST['redirect_to'] );

	$errors->add( 'sz_no_access', $message );

	return $errors;
}
add_filter( 'wp_login_errors', 'sz_core_no_access_wp_login_error' );
 ...
 * @return array
 */
function sz_core_login_filter_shake_codes( $codes ) {
	$codes[] = 'sz_no_access';
	return $codes;
}
add_filter( 'shake_error_codes', 'sz_core_login_filter_shake_codes' );

/**
 * Canonicalize SportsZone URLs.
 *
 * This function ensures that requests for SportsZone content are always
 ...
 * @since 1.6.0
 *
 * @see SZ_Members_Component::setup_globals() where
 *      $sz->canonical_stack['base_url'] and ['component'] may be set.
 * @see sz_core_new_nav_item() where $sz->canonical_stack['action'] may be set.
 */
function sz_redirect_canonical() {

	/**
	 * Filters whether or not to do canonical redirects on SportsZone URLs.
	 *
	 * @since 1.6.0
	 *
	 * @param bool $value Whether or not to do canonical redirects. Default true.
	 */
	if ( !sz_is_blog_page() && apply_filters( 'sz_do_redirect_canonical', true ) ) {
		...
		$requested_url  = sz_get_requested_url();
		...
		$canonical_url  = sz_get_canonical_url();
		...
		if ( $canonical_url !== $req_url_clean ) {
			...
			if ( isset( $sz->template_message ) ) {
				$message      = stripslashes( $sz->template_message );
				$message_type = isset( $sz->template_message_type ) ? $sz->template_message_type : 'success';

				sz_core_add_message( $message, $message_type );
			}
			...
			sz_core_redirect( $canonical_url, 301 );
		}
	}
}

/**
 * Output rel=canonical header tag for SportsZone content.
 *
 * @since 1.6.0
 */
function sz_rel_canonical() {
	$canonical_url = sz_get_canonical_url();
 ...
 */
function sz_get_canonical_url( $args = array() ) {

	// For non-BP content, return the requested url, and let WP do the work.
	if ( sz_is_blog_page() ) {
		return sz_get_requested_url();
	}
	...
	// Special case: when a SportsZone directory (eg example.com/members)
	...
	if ( 'page' == get_option( 'show_on_front' ) && $page_on_front = (int) get_option( 'page_on_front' ) ) {
		$front_page_component = array_search( $page_on_front, sz_core_get_directory_page_ids() );

		/*
		 * If requesting the front page component directory, canonical
		 * URL is the front page. We detect whether we're detecting a
		 * component *directory* by checking that sz_current_action()
		 * is empty - ie, this not a single item, a feed, or an item
		 * type directory.
		 */
		if ( false !== $front_page_component && sz_is_current_component( $front_page_component ) && ! sz_current_action() && ! sz_get_current_member_type() ) {
			$sz->canonical_stack['canonical_url'] = trailingslashit( sz_get_root_domain() );
		...
		// the members directory to avoid redirect loops.
		} elseif ( sz_is_register_page() && 'register' == $front_page_component && is_user_logged_in() ) {
			 ...
			 */
			$sz->canonical_stack['canonical_url'] = apply_filters( 'sz_loggedin_register_page_redirect_to', sz_get_members_directory_permalink() );
		}
	}

	if ( empty( $sz->canonical_stack['canonical_url'] ) ) {
		// Build the URL in the address bar.
		$requested_url  = sz_get_requested_url();
		...
		if ( isset( $sz->canonical_stack['base_url'] ) )
			$url_stack[0] = $sz->canonical_stack['base_url'];

		if ( isset( $sz->canonical_stack['component'] ) )
			$url_stack[0] = trailingslashit( $url_stack[0] . $sz->canonical_stack['component'] );

		if ( isset( $sz->canonical_stack['action'] ) )
			$url_stack[0] = trailingslashit( $url_stack[0] . $sz->canonical_stack['action'] );

		if ( !empty( $sz->canonical_stack['action_variables'] ) ) {
			foreach( (array) $sz->canonical_stack['action_variables'] as $av ) {
		...
		// Stash in the $sz global.
		$sz->canonical_stack['canonical_url'] = implode( '?', $url_stack );
	}

	$canonical_url = $sz->canonical_stack['canonical_url'];
	 ...
	 */
	return apply_filters( 'sz_get_canonical_url', $canonical_url, $args );
}
 ...
 */
function sz_get_requested_url() {
	$sz = sportszone();

	if ( empty( $sz->canonical_stack['requested_url'] ) ) {
		$sz->canonical_stack['requested_url']  = is_ssl() ? 'https://' : 'http://';
		$sz->canonical_stack['requested_url'] .= $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
	}
	 ...
	 */
	return apply_filters( 'sz_get_requested_url', $sz->canonical_stack['requested_url'] );
}

/**
 * Remove WP's canonical redirect when we are trying to load BP-specific content.
 *
 * Avoids issues with WordPress thinking that a SportsZone URL might actually
 * be a blog post or page.
 *
 * This function should be considered temporary, and may be removed without
 * notice in future versions of SportsZone.
 *
 * @since 1.6.0
 */
function _sz_maybe_remove_redirect_canonical() {
	if ( ! sz_is_blog_page() )
		remove_action( 'template_redirect', 'redirect_canonical' );
}
add_action( 'sz_init', '_sz_maybe_remove_redirect_canonical' );
 ...
 * creates a race condition with sz_template_redirect(), our piggyback hook.
 * Due to a legacy bug in SportsZone, internal BP content (such as members and
 * groups) is marked 404 in $wp_query until sz_core_load_template(), when BP
 ...
 * the race condition. If sz_core_load_template() runs, it dies before reaching
 * maybe_redirect_404(). If sz_core_load_template() does not run, it means that
 * the 404 is legitimate, and maybe_redirect_404() can proceed as expected.
 *
 * This function will be removed in a later version of SportsZone. Plugins
 * (and plugin authors!) should ignore it.
 *
 * @since 1.6.1
 *
 * @link https://sportszone.trac.wordpress.org/ticket/4329
 * @link https://sportszone.trac.wordpress.org/ticket/4415
 */
function _sz_rehook_maybe_redirect_404() {
	if ( defined( 'NOBLOGREDIRECT' ) && is_multisite() ) {
		remove_action( 'template_redirect', 'maybe_redirect_404' );
		add_action( 'template_redirect', 'maybe_redirect_404', 100 );
	}
}
add_action( 'template_redirect', '_sz_rehook_maybe_redirect_404', 1 );
 ...
 * @since 1.6.0
 */
function _sz_maybe_remove_rel_canonical() {
	if ( ! sz_is_blog_page() && ! is_404() ) {
		remove_action( 'wp_head', 'rel_canonical' );
		add_action( 'sz_head', 'sz_rel_canonical' );
	}
}
add_action( 'wp_head', '_sz_maybe_remove_rel_canonical', 8 );
 ...
 * @return null|array
 */
function sz_core_filter_wp_query( $retval, $query ) {
	 ...
	 * Too early to use sz_is_single_item(), so use BP conditionals.
	 */
	if ( false === ( sz_is_group() || sz_is_user() || sz_is_single_activity() ) ) {
		return $retval;
	}
