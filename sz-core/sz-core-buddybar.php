<?php
/**
 * Core SportsZone Navigational Functions.
 *
 * @package SportsZone
 * @subpackage Core
 * @since 1.5.0
 ...
 *                                                (those with the 'sz_moderate' cap). Default: false.
 ...
 */
function sz_core_new_nav_item( $args, $component = 'members' ) {
	...
	$nav_item = sz_core_create_nav_link( $r, $component );

	/*
	 * To mimic legacy behavior, if sz_core_create_nav_link() returns false, we make
	...
	$hooked = sz_core_register_nav_screen_function( $nav_item );
	if ( false === $hooked ){
		return false;
	}

	/**
	 * Fires after adding an item to the main SportsZone navigation array.
	 * Note that, when possible, the more specific action hooks
	 * `sz_core_create_nav_link` or `sz_core_register_nav_screen_function`
	 * should be used.
	 *
	 * @since 1.5.0
	 *
	 ...
	 */
	do_action( 'sz_core_new_nav_item', $r, $args, $defaults );
}

/**
 * Add a link to the main SportsZone navigation.
 *
 * @since 2.4.0
 * @since 2.6.0 Introduced the `$component` parameter. Began returning a SZ_Core_Nav_Item object on success.
 *
 * @param array|string $args {
 *     Array describing the new nav item.
 *     @type string      $name                    Display name for the nav item.
 ...
 */
function sz_core_create_nav_link( $args = '', $component = 'members' ) {
	$sz = sportszone();
	...
	if ( ! empty( $r['site_admin_only'] ) && ! sz_current_user_can( 'sz_moderate' ) ) {
		return false;
	}
	...
	$nav_item = array(
		'name'                    => $r['name'],
		'slug'                    => $r['slug'],
		'link'                    => trailingslashit( sz_loggedin_user_domain() . $r['slug'] ),
	...
	sportszone()->{$component}->nav->add_nav( $nav_item );

	/**
	 * Fires after a link is added to the main SportsZone nav.
	 *
	 * @since 2.4.0
	 ...
	 */
	do_action( 'sz_core_create_nav_link', $r, $args, $defaults, $component );
 ...
 */
function sz_core_register_nav_screen_function( $args = '' ) {
	$sz = sportszone();
	 ...
	 */
	if ( empty( $r['show_for_displayed_user'] ) && ! sz_user_has_access() ) {
		return false;
	}
	 ...
	 */
	if ( ( -1 != $r['position'] ) && sz_is_root_component( $r['slug'] ) && ! sz_displayed_user_id() ) {
		return;
	}

	// Look for current component.
	if ( sz_is_current_component( $r['slug'] ) || sz_is_current_item( $r['slug'] ) ) {
		...
		if ( ! empty( $r['default_subnav_slug'] ) && sz_is_current_action( $r['default_subnav_slug'] ) && ! sz_action_variable( 0 ) ) {
			unset( $sz->canonical_stack['action'] );
		} elseif ( ! sz_current_action() ) {

			// Add our screen hook if screen function is callable.
			if ( is_callable( $r['screen_function'] ) ) {
				add_action( 'sz_screens', $r['screen_function'], 3 );
			}
			...
			if ( ! empty( $r['default_subnav_slug'] ) ) {
				 ...
				 * @param array  $r     Parsed arguments for the nav item.
				 */
				$sz->current_action = apply_filters( 'sz_default_component_subnav', $r['default_subnav_slug'], $r );
			}
		}
	}

	/**
	 * Fires after the screen function for an item in the SportsZone main
	 ...
	 * @param array $defaults Default arguments for a nav item.
	 */
	do_action( 'sz_core_register_nav_screen_function', $r, $args, $defaults );
}
 ...
 * }
 */
function sz_core_new_nav_default( $args = '' ) {
	$sz = sportszone();
	...
	$parent_nav = $sz->members->nav->get_primary( array( 'slug' => $r['parent_slug'] ), false );
	...
	if ( ! empty( $parent_nav->screen_function ) ) {
		...
		if ( is_callable( $parent_nav->screen_function ) ) {
			remove_action( 'sz_screens', $parent_nav->screen_function, 3 );
		}
	}
	...
	if ( sz_is_current_component( $parent_nav->slug ) ) {

		// The only way to tell whether to set the subnav is to peek at the unfiltered_uri
		// Find the component.
		$component_uri_key = array_search( $parent_nav->slug, $sz->unfiltered_uri );

		if ( false !== $component_uri_key ) {
			if ( ! empty( $sz->unfiltered_uri[$component_uri_key + 1] ) ) {
				$unfiltered_action = $sz->unfiltered_uri[$component_uri_key + 1];
			}
		}

		// No subnav item has been requested in the URL, so set a new nav default.
		if ( empty( $unfiltered_action ) ) {
			if ( ! sz_is_current_action( $r['subnav_slug'] ) ) {
				if ( is_callable( $r['screen_function'] ) ) {
					add_action( 'sz_screens', $r['screen_function'], 3 );
				}

				$sz->current_action = $r['subnav_slug'];
				unset( $sz->canonical_stack['action'] );
			}
 ...
 *                                          eg, use `sz_is_my_profile()` to restrict access to profile owners only.
 *                                          Default: true.
 *     @type bool        $site_admin_only   Optional. Whether the nav item should be visible only to site admins
 *                                          (those with the 'sz_moderate' cap). Default: false.
 ...
 * @return null|false Returns false on failure.
 */
function sz_core_new_subnav_item( $args, $component = null ) {
	// Backward compatibility for plugins using `sz_core_new_subnav_item()` without `$component`
	// to add group subnav items.
	if ( null === $component && sz_is_active( 'groups' ) && sz_is_group() && isset( $args['parent_slug'] ) ) {
		/*
		 ...
		 */
		$group_slug = sz_get_current_group_slug();
		if (
			$group_slug === $args['parent_slug'] ||
			( 0 === strpos( $args['parent_slug'], $group_slug ) && ! sportszone()->members->nav->get_primary( array( 'slug' => $args['parent_slug'] ), false ) )
		) {
			$component = 'groups';
		}
	}
	...
	$subnav_item = sz_core_create_subnav_link( $args, $component );

	/*
	 * To mimic legacy behavior, if sz_core_create_subnav_link() returns false, we make an
	...
	$hooked = sz_core_register_subnav_screen_function( $subnav_item, $component );
	if ( false === $hooked ) {
		return false;
	}
}
...
/**
 * Add a subnav link to the SportsZone navigation.
 *
 * @since 2.4.0
 * @since 2.6.0 Introduced the `$component` parameter. Began returning a SZ_Core_Nav_Item object on success.
 *
 ...
 *                                          when registering the subnav; eg, use sz_is_my_profile()
 ...
 *                                          to site admins (those with the 'sz_moderate' cap).
 *                                          Default: false.
 ...
 * }
 * @param string       $component The component the navigation is attached to. Defaults to 'members'.
 * @return false|array Returns false on failure, new SZ_Core_Nav_Item instance on success.
 */
function sz_core_create_subnav_link( $args = '', $component = 'members' ) {
	$sz = sportszone();
	...
	if ( empty( $r['link'] ) ) {
		...
		$parent_nav = $sz->{$component}->nav->get_primary( array( 'slug' => $r['parent_slug'] ), false );
	...
	sportszone()->{$component}->nav->add_nav( $subnav_item );
 ...
 *                                       only to site admins (those with the 'sz_moderate' cap).
 *                                       Default: false.
 *     @type int      $position          Optional. Numerical index specifying where the item
 *                                       should appear in the subnav array. Default: 90.
 *     @type callable $screen_function   The callback function that will run
 ...
 * @return null|false Returns false on failure.
 */
function sz_core_register_subnav_screen_function( $args = '', $component = 'members' ) {
	$sz = sportszone();
	...
	if ( ! sz_is_current_component( $r['parent_slug'] ) && ! sz_is_current_item( $r['parent_slug'] ) ) {
		return;
	}
	...
	if ( ( sz_current_action() && sz_is_current_action( $r['slug'] ) ) || ( sz_is_user() && ! sz_current_action() && ! empty( $parent_nav->screen_function ) && $r['screen_function'] == $parent_nav->screen_function ) ) {
		...
		$hooked = sz_core_maybe_hook_new_subnav_screen_function( $r, $component );
		...
		if ( ! empty( $hooked['status'] ) && 'failure' === $hooked['status'] && isset( $hooked['redirect_args'] ) ) {
			sz_core_no_access( $hooked['redirect_args'] );
		}
	}
}
 ...
 *                            the component in sz_core_new_subnav_item().
 * @param string $component   The component the navigation is attached to. Defaults to 'members'.
 * @return array
 */
function sz_core_maybe_hook_new_subnav_screen_function( $subnav_item, $component = 'members' ) {
	$retval = array(
		'status' => '',
	);
	...
	if ( ! empty( $subnav_item['site_admin_only'] ) && ! sz_current_user_can( 'sz_moderate' ) ) {
	...
	if ( ! empty( $subnav_item['user_has_access'] ) && ! $site_admin_restricted ) {
		...
		// Success - hook to sz_screens.
		} else {
			add_action( 'sz_screens', $subnav_item['screen_function'], 3 );
	...
	// user status.
		...
		if ( is_user_logged_in() ) {
			...
			if ( ! empty( $subnav_item['no_access_url'] ) ) {
				$message     = __( 'You do not have access to that page.', 'sportszone' );
			...
			// redirect URL.
				...
				$parent_nav_default = $sz->{$component}->nav->get_primary( array( 'slug' => $sz->default_component ), false );
				...
				// publicly accessible.
				if ( sz_is_my_profile() || ( isset( $parent_nav_default_item ) && $parent_nav_default_item->show_for_displayed_user ) ) {
					$message     = __( 'You do not have access to that page.', 'sportszone' );
					$redirect_to = sz_displayed_user_domain();
				...
				// know will be accessible.
					...
					// Try 'activity' first.
					if ( sz_is_active( 'activity' ) && isset( $sz->pages->activity ) ) {
						$redirect_to = trailingslashit( sz_displayed_user_domain() . sz_get_activity_slug() );
					// Then try 'profile'.
					} else {
						$redirect_to = trailingslashit( sz_displayed_user_domain() . ( 'xprofile' == $sz->profile->id ? 'profile' : $sz->profile->id ) );
					}
			...
			// Fall back to the home page.
				...
				$message     = __( 'You do not have access to this page.', 'sportszone' );
				$redirect_to = sz_get_root_domain();
			}
 ...
 */
function sz_nav_item_has_subnav( $nav_item = '', $component = 'members' ) {
	$sz = sportszone();
	...
	if ( ! $nav_item ) {
		$nav_item = sz_current_component();

		if ( sz_is_group() ) {
			$nav_item = sz_current_item();
		}
	}

	$has_subnav = (bool) $sz->{$component}->nav->get_secondary( array( 'parent_slug' => $nav_item ), false );
	 ...
	 * @param string $nav_item   The slug of the top-level nav item whose subnav items you're checking.
	 */
	return apply_filters( 'sz_nav_item_has_subnav', $has_subnav, $nav_item );
}
 ...
 */
function sz_core_remove_nav_item( $slug, $component = null ) {
	$sz = sportszone();

	// Backward compatibility for removing group nav items using the group slug as `$parent_slug`.
	if ( ! $component && sz_is_active( 'groups' ) && isset( $sz->groups->nav ) ) {
		if ( $sz->groups->nav->get_primary( array( 'slug' => $slug ) ) ) {
			$component = 'groups';
		}
	}
	...
	$screen_functions = $sz->{$component}->nav->delete_nav( $slug );

	// Reset backcompat nav items so that subsequent references will be correct.
	$sz->sz_nav->reset();
	$sz->sz_options_nav->reset();
	...
	foreach ( $screen_functions as $screen_function ) {
		...
		if ( is_callable( $screen_function ) ) {
			remove_action( 'sz_screens', $screen_function, 3 );
		}
	}
 ...
 * @return bool Returns false on failure, True on success.
 */
function sz_core_remove_subnav_item( $parent_slug, $slug, $component = null ) {
	$sz = sportszone();

	// Backward compatibility for removing group nav items using the group slug as `$parent_slug`.
	if ( ! $component && sz_is_active( 'groups' ) && isset( $sz->groups->nav ) ) {
		if ( $sz->groups->nav->get_primary( array( 'slug' => $parent_slug ) ) ) {
			$component = 'groups';
		}
	}
	...
	$screen_functions = $sz->{$component}->nav->delete_nav( $slug, $parent_slug );
 ...
 * @param string $component   The component the navigation is attached to. Defaults to 'members'.
 */
function sz_core_reset_subnav_items( $parent_slug, $component = 'members' ) {
	$sz = sportszone();
	...
	$subnav_items = $sz->{$component}->nav->get_secondary( array( 'parent_slug' => $parent_slug ), false );
	...
	foreach( $subnav_items as $subnav_item ) {
		$sz->{$component}->nav->delete_nav( $subnav_item->slug, $parent_slug );
	}
}
 ...
 */
function sz_get_admin_bar_pref( $context, $user = 0 ) {
