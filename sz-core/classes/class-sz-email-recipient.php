...
class SZ_Email_Recipient {
	...
	public function __construct( $email_or_user, $name = '' ) {
		...
		if ( $this->user_object ) {
			// This is escaped with esc_html in sz_core_get_user_displayname()
			$wp_name = wp_specialchars_decode( sz_core_get_user_displayname( $this->user_object->ID ), ENT_QUOTES );
		...
		/**
		 * Fires inside __construct() method for SZ_Email_Recipient class.
		 *
		 * @since 2.5.0
		 *
		 * @param string|array|int|WP_User $email_or_user Either a email address, user ID, WP_User object,
		 ...
		 * @param SZ_Email_Recipient $this Current instance of the email type class.
		 */
		do_action( 'sz_email_recipient', $email_or_user, $name, $this );
	}
	...
	public function get_address() {
		 ...
		 * @param SZ_Email $recipient $this Current instance of the email recipient class.
		 */
		return apply_filters( 'sz_email_recipient_get_address', $this->address, $this );
	}
	...
	public function get_name() {
		 ...
		 * @param SZ_Email $recipient $this Current instance of the email recipient class.
		 */
		return apply_filters( 'sz_email_recipient_get_name', $this->name, $this );
	}
	...
	public function get_user( $transform = 'raw' ) {
		 ...
		 * @param SZ_Email $recipient $this Current instance of the email recipient class.
		 */
		return apply_filters( 'sz_email_recipient_get_user', $this->user_object, $transform, $this );
	}
}
