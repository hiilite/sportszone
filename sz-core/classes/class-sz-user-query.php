...
/**
 * SportsZone User Query class.
 *
 * Used for querying users in a SportsZone context, in situations where WP_User_Query won't do the trick:
 ...
 *                                                  See {@see SZ_XProfile_Query} for details.
 ...
 *     @type string            $count_total         Determines how SZ_User_Query will do a count of total users matching
...
class SZ_User_Query {
	 ...
	 * @since 1.7.0
	 *
	 * @param string|array|null $query See {@link SZ_User_Query}.
	 */
	public function __construct( $query = null ) {
		...
		if ( ! empty( $this->query_vars_raw ) ) {
			...
			/**
			 * Fires before the construction of the SZ_User_Query query.
			 *
			 * @since 1.7.0
			 *
			 * @param SZ_User_Query $this Current instance of the SZ_User_Query. Passed by reference.
			 */
			do_action_ref_array( 'sz_pre_user_query_construct', array( &$this ) );
		...
		// Get SportsZone specific user data.
	 ...
	 *
	 * When extending SZ_User_Query, you may need to use some of its
	 ...
	 * SZ_User_Query::__construct() contains a fair amount of logic that
	 ...
	 * {@link SZ_Group_Member_Query::setup_hooks()} for an example.
	 *
	 * @since 1.8.0
	 */
	...
	public function prepare_user_ids_query() {
		...
		switch ( $type ) {
			...
			// Filter 'sz_user_query_online_interval' to modify the
			// number of minutes used as an interval.
			case 'online' :
				$this->uid_name = 'user_id';
				$this->uid_table = $sz->members->table_name_last_activity;
				$sql['select']  = "SELECT u.{$this->uid_name} as id FROM {$this->uid_table} u";
				$sql['where'][] = $wpdb->prepare( "u.component = %s AND u.type = 'last_activity'", sportszone()->members->id );
				 ...
				 */
				$sql['where'][] = $wpdb->prepare( "u.date_recorded >= DATE_SUB( UTC_TIMESTAMP(), INTERVAL %d MINUTE )", apply_filters( 'sz_user_query_online_interval', 15 ) );
			...
			case 'popular' :
				...
				$sql['select']  = "SELECT u.{$this->uid_name} as id FROM {$this->uid_table} u";
				$sql['where'][] = $wpdb->prepare( "u.meta_key = %s", sz_get_user_meta_key( 'total_friend_count' ) );
			...
			case 'alphabetical' :
				...
				//
				// @todo remove need for sz_is_active() check.
				if ( ! sz_disable_profile_sync() || ! sz_is_active( 'xprofile' ) ) {
					$this->uid_name = 'ID';
				...
				// the xprofile table.
					...
					$this->uid_name = 'user_id';
					$this->uid_table = $sz->profile->table_name_data;
					$sql['select']  = "SELECT u.{$this->uid_name} as id FROM {$this->uid_table} u";
					$sql['where'][] = $wpdb->prepare( "u.field_id = %d", sz_xprofile_fullname_field_id() );
				...
				$sql['where'][] = "u.{$this->uid_name} IN ( SELECT ID FROM {$wpdb->users} WHERE " . sz_core_get_status_sql( '' ) . " )";
		...
		// @todo remove need for sz_is_active() check.
		if ( ! empty( $user_id ) && sz_is_active( 'friends' ) ) {
		...
		// xprofile field matches happen in sz_xprofile_sz_user_query_search().
		if ( false !== $search_terms ) {
			$search_terms = sz_esc_like( wp_kses_normalize_entities( $search_terms ) );
		 ...
		 * @param SZ_User_Query $this Current SZ_User_Query instance.
		 */
		$sql = apply_filters_ref_array( 'sz_user_query_uid_clauses', array( $sql, &$this ) );
		...
		/**
		 * Fires before the SZ_User_Query query is made.
		 *
		 * @since 1.7.0
		 *
		 * @param SZ_User_Query $this Current SZ_User_Query instance. Passed by reference.
		 */
		do_action_ref_array( 'sz_pre_user_query', array( &$this ) );
	}
	...
	public function do_user_ids_query() {
		...
		if ( 'sql_calc_found_rows' == $this->query_vars['count_total'] ) {
			 ...
			 * @param SZ_User_Query $this  Current SZ_User_Query instance.
			 */
			$this->total_users = $wpdb->get_var( apply_filters( 'sz_found_user_query', "SELECT FOUND_ROWS()", $this ) );
			...
			/** This filter is documented in sz-core/classes/class-sz-user-query.php */
			$this->total_users = $wpdb->get_var( apply_filters( 'sz_found_user_query', "{$count_select} {$this->uid_clauses['where']}", $this ) );
		}
	}
	...
	public function do_wp_user_query() {
		 ...
		 * @param SZ_User_Query $this  Current SZ_User_Query instance.
		 */
		$wp_user_query = new WP_User_Query( apply_filters( 'sz_wp_user_query_args', array(
	 ...
	 * ids passed to the class constructor. See {@link SZ_Group_Member_Query}
	 * for an example.
	 *
	 * @since 1.8.0
	 *
	 ...
	 *
	 * Different components will hook into the 'sz_user_query_populate_extras'
	...
	public function populate_extras() {
		 ...
		 */
		do_action_ref_array( 'sz_user_query_populate_extras', array( $this, $user_ids_sql ) );

		// Fetch last_active data from the activity table.
		$last_activities = SZ_Core_User::get_last_activity( $this->user_ids );
		...
		// - friend count
		// - latest update.
		$total_friend_count_key = sz_get_user_meta_key( 'total_friend_count' );
		$sz_latest_update_key   = sz_get_user_meta_key( 'sz_latest_update'   );
		...
		$user_metas = $wpdb->get_results( $wpdb->prepare( "SELECT user_id, meta_key, meta_value FROM {$wpdb->usermeta} WHERE meta_key IN (%s,%s) AND user_id IN ({$user_ids_sql})", $total_friend_count_key, $sz_latest_update_key ) );
	...
	protected function get_sql_clause_for_member_types( $member_types, $operator ) {
		...
		foreach ( $member_types as $mt ) {
			if ( sz_get_member_type_object( $mt ) ) {
				$types[] = $mt;
			}
		}
		...
		$tax_query = new WP_Tax_Query( array(
			array(
				'taxonomy' => sz_get_member_type_tax_name(),
				'field'    => 'name',
				'operator' => $operator,
				'terms'    => $types,
			),
		...
		$site_id  = sz_get_taxonomy_term_site_id( sz_get_member_type_tax_name() );
		$switched = false;
