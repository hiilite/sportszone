...
class SZ_Core_Nav {
	...
	public function __construct( $object_id = 0 ) {
		if ( empty( $object_id ) ) {
			$this->object_id = (int) sz_displayed_user_id();
		} else {
	...
	public function __set( $key, $value ) {
		...
		$this->nav[ $this->object_id ][ $key ] = new SZ_Core_Nav_Item( $value );
	}
	...
	public function add_nav( $args ) {
		...
		// Add to the nav.
		$this->nav[ $this->object_id ][ $slug ] = new SZ_Core_Nav_Item( $args );
	...
	public function edit_nav( $args = array(), $slug = '', $parent_slug = '' ) {
		...
		if ( empty( $parent_slug ) ) {
			...
			$this->nav[ $this->object_id ][ $slug ] = new SZ_Core_Nav_Item( wp_parse_args( $args, (array) $nav_item ) );
		...
		// We're editing a child.
			...
			$this->nav[ $this->object_id ][ $parent_slug . '/' . $slug ] = new SZ_Core_Nav_Item( $params );
