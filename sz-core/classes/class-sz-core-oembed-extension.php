 ...
 * @since 2.6.0
 */
abstract class SZ_Core_oEmbed_Extension {
	 ...
	 *
	 * You'll want to return a boolean here. eg. sz_is_single_activity().
	 *
	 * @since 2.6.0
	 *
	 * @return bool
	...
	protected function set_permalink() {
		...
		// Remove querystring from sz_get_requested_url().
		if ( false !== strpos( sz_get_requested_url(), '?' ) ) {
			$url = substr( sz_get_requested_url(), 0, strpos( sz_get_requested_url(), '?' ) );
		}
	...
	protected function get_item_id() {
		return $this->is_page() ? $this->validate_url_to_item_id( $this->set_permalink() ) : sportszone()->{$this->slug_endpoint}->embedid_in_progress;
	}
	...
	protected function setup_hooks() {
		...
		add_action( 'sz_embed_content', array( $this, 'inject_content' ) );
	...
	/**
	 * Set up custom embed template parts for SportsZone use.
	 *
	 * @since 2.6.0
	 *
	 ...
	 * and inject our own template for SportsZone use.
	 *
	 * @since 2.6.0
	 *
	...
	public function content_buffer_end( $name ) {
		...
		// Start our custom SportsZone embed template!
		echo '<div ';
		post_class( 'wp-embed' );
		echo '>';

		// Template part for our embed header.
		sz_get_asset_template_part( 'embeds/header', sz_current_component() );

		/**
		 * Inject SportsZone embed content on this hook.
		 *
		 * You shouldn't really need to use this if you extend the
		 * {@link SZ_oEmbed_Component} class.
		 *
		 * @since 2.6.0
		 */
		do_action( 'sz_embed_content' );

		// Template part for our embed footer.
		sz_get_asset_template_part( 'embeds/footer', sz_current_component() );
	...
	public function get_item( $request ) {
		...
		if ( ! empty( $item_id ) ) {
			...
			if ( empty( sportszone()->{$this->slug_endpoint} ) ) {
				sportszone()->{$this->slug_endpoint} = new stdClass;
			}
			sportszone()->{$this->slug_endpoint}->embedurl_in_progress = $url;
			sportszone()->{$this->slug_endpoint}->embedid_in_progress  = $item_id;

			// Save custom route args as well.
			$custom_args = array_keys( (array) $this->set_route_args() );
			if ( ! empty( $custom_args ) ) {
				sportszone()->{$this->slug_endpoint}->embedargs_in_progress = array();

				foreach( $custom_args as $arg ) {
					if ( isset( $request[ $arg ] ) ) {
						sportszone()->{$this->slug_endpoint}->embedargs_in_progress[ $arg ] = $request[ $arg ];
					}
				}
			}
	...
	/**
	 * Pass our SportsZone activity permalink for embedding.
	 *
	 * @since 2.6.0
	 *
	 * @see sz_activity_embed_rest_route_callback()
	 *
	 * @param string $retval Current embed URL.
	 * @return string
	 */
	public function filter_embed_url( $retval ) {
		if ( false === isset( sportszone()->{$this->slug_endpoint}->embedurl_in_progress ) && ! $this->is_page() ) {
			return $retval;
		}

		$url = $this->is_page() ? $this->set_permalink() : sportszone()->{$this->slug_endpoint}->embedurl_in_progress;
		$url = trailingslashit( $url );

		// This is for the 'WordPress Embed' block
		// @see sz_activity_embed_comments_button().
		if ( 'the_permalink' !== current_filter() ) {
			$url = add_query_arg( 'embed', 'true', trailingslashit( $url ) );

			// Add custom route args to iframe.
			if ( ! empty( sportszone()->{$this->slug_endpoint}->embedargs_in_progress ) ) {
				foreach( sportszone()->{$this->slug_endpoint}->embedargs_in_progress as $key => $value ) {
	...
	public function filter_embed_html( $retval ) {
		...
		$item_id = $this->is_page() ? $this->validate_url_to_item_id( $url ) : sportszone()->{$this->slug_endpoint}->embedid_in_progress;
		 ...
		 * @var string $retval
		 */
		return apply_filters( "sz_{$this->slug_endpoint}_embed_html", $fallback_html . $retval );
	}
