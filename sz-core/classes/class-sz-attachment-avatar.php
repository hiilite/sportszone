 ...
 * @since 2.3.0
 */
class SZ_Attachment_Avatar extends SZ_Attachment {
	 ...
	 * @since 2.3.0
	 *
	 * @see  SZ_Attachment::__construct() for list of parameters
	 */
	public function __construct() {
		...
		parent::__construct( array(
			'action'                => 'sz_avatar_upload',
			'file_input'            => 'file',
			'original_max_filesize' => sz_core_avatar_original_max_filesize(),

			// Specific errors for avatars.
			'upload_error_strings'  => array(
				9  => sprintf( __( 'That photo is too big. Please upload one smaller than %s', 'sportszone' ), size_format( sz_core_avatar_original_max_filesize() ) ),
				10 => sprintf( _n( 'Please upload only this file type: %s.', 'Please upload only these file types: %s.', count( $allowed_types ), 'sportszone' ), self::get_avatar_types( $allowed_types ) ),
			),
		) );
	}
	...
	public static function get_avatar_types( $allowed_types = array() ) {
		...
		$comma = _x( ',', 'avatar types separator', 'sportszone' );
	...
	public function set_upload_dir() {
		if ( sz_core_avatar_upload_path() && sz_core_avatar_url() ) {
			$this->upload_path = sz_core_avatar_upload_path();
			$this->url         = sz_core_avatar_url();
	 ...
	 *
	 * Adds an error if the avatar size or type don't match SportsZone needs.
	...
	public function validate_upload( $file = array() ) {
		...
		if ( ! sz_core_check_avatar_size( array( 'file' => $file ) ) ) {
			$file['error'] = 9;

		// File is of invalid type.
		} elseif ( ! sz_core_check_avatar_type( array( 'file' => $file ) ) ) {
			$file['error'] = 10;
		}
	...
	public static function shrink( $file = '', $ui_available_width = 0 ) {
		...
		$original_max_width = sz_core_avatar_original_max_width();
		...
		if ( ! empty( $ui_available_width ) && $ui_available_width < $original_max_width ) {
			...
			if ( $original_max_width < sz_core_avatar_full_width() ) {
				$original_max_width = sz_core_avatar_full_width();
			}
		}
	...
	public function crop( $args = array() ) {
		...
		if ( ! sz_attachments_current_user_can( 'edit_avatar', $args ) ) {
			return false;
		}
		...
		if ( empty( $args['item_id'] ) ) {
			...
			$avatar_folder_dir = apply_filters( 'sz_core_avatar_folder_dir', dirname( $absolute_path ), $args['item_id'], $args['object'], $args['avatar_dir'] );
		} else {

			/** This filter is documented in sz-core/sz-core-avatars.php */
			$avatar_folder_dir = apply_filters( 'sz_core_avatar_folder_dir', $this->upload_path . '/' . $args['avatar_dir'] . '/' . $args['item_id'], $args['item_id'], $args['object'], $args['avatar_dir'] );
		}
		...
		$existing_avatar = sz_core_fetch_avatar( array(
			'object'  => $args['object'],
		...
		if ( ! empty( $existing_avatar ) && $existing_avatar !== $this->url . $relative_path ) {
			sz_core_delete_existing_avatar( array( 'object' => $args['object'], 'item_id' => $args['item_id'], 'avatar_path' => $avatar_folder_dir ) );
		}

		// Make sure we at least have minimal data for cropping.
		if ( empty( $args['crop_w'] ) ) {
			$args['crop_w'] = sz_core_avatar_full_width();
		}

		if ( empty( $args['crop_h'] ) ) {
			$args['crop_h'] = sz_core_avatar_full_height();
		}
		...
		foreach ( $avatar_types as $key_type => $type ) {
			if ( 'thumb' === $key_type ) {
				$args['dst_w'] = sz_core_avatar_thumb_width();
				$args['dst_h'] = sz_core_avatar_thumb_height();
			} else {
				$args['dst_w'] = sz_core_avatar_full_width();
				$args['dst_h'] = sz_core_avatar_full_height();
			}
	...
	private function get_user_id() {
		...
		if ( ! empty( $sz->members->admin->user_id ) ) {
			$user_id = $sz->members->admin->user_id;
		}
	...
	public function script_data() {
		...
		if ( ! empty( $user_id ) ) {
			...
			if ( sz_avatar_use_webcam() ) {
				$js_scripts = array( 'sz-webcam' );
			}

			$script_data['sz_params'] = array(
				'object'     => 'user',
				'item_id'    => $user_id,
				'has_avatar' => sz_get_user_has_avatar( $user_id ),
				'nonces'  => array(
					'set'    => wp_create_nonce( 'sz_avatar_cropstore' ),
					'remove' => wp_create_nonce( 'sz_delete_avatar_link' ),
				),
			);

			// Set feedback messages.
			$script_data['feedback_messages'] = array(
				1 => __( 'There was a problem cropping your profile photo.', 'sportszone' ),
				2 => __( 'Your new profile photo was uploaded successfully.', 'sportszone' ),
				3 => __( 'There was a problem deleting your profile photo. Please try again.', 'sportszone' ),
				4 => __( 'Your profile photo was deleted successfully!', 'sportszone' ),
			);
		} elseif ( ! empty( $group_id ) ) {
			$script_data['sz_params'] = array(
				'object'     => 'group',
				'item_id'    => $group_id,
				'has_avatar' => sz_get_group_has_avatar( $group_id ),
				'nonces'     => array(
					'set'    => wp_create_nonce( 'sz_avatar_cropstore' ),
					'remove' => wp_create_nonce( 'sz_group_avatar_delete' ),
				),
			);
			...
			/**
			 * Use this filter to include specific SportsZone params for your object.
			 * e.g. Blavatar.
			 *
			 * @since 2.3.0
			 *
			 * @param array $value The avatar specific SportsZone parameters.
			 */
			$script_data['sz_params'] = apply_filters( 'sz_attachment_avatar_params', array() );
		}
		...
		if ( isset( $script_data['sz_params']['object'] ) ) {
			$object = $script_data['sz_params']['object'];
		}
		 ...
		 */
		return apply_filters( 'sz_attachment_avatar_script_data', $script_data, $object );
	}
}
