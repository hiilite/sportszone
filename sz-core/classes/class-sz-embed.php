...
/**
 * Enable oEmbeds in SportsZone contexts.
 *
 * Extends WP_Embed class for use with SportsZone.
 *
 * @since 1.5.0
 *
 * @see WP_Embed
 */
class SZ_Embed extends WP_Embed {
	...
	public function __construct() {
		...
		if ( sz_use_embed_in_activity() ) {
			add_filter( 'sz_get_activity_content_body', array( &$this, 'autoembed' ), 8 );
			add_filter( 'sz_get_activity_content_body', array( &$this, 'run_shortcode' ), 7 );
		}

		if ( sz_use_embed_in_activity_replies() ) {
			add_filter( 'sz_get_activity_content', array( &$this, 'autoembed' ), 8 );
			add_filter( 'sz_get_activity_content', array( &$this, 'run_shortcode' ), 7 );
		}

		if ( sz_use_embed_in_private_messages() ) {
			add_filter( 'sz_get_the_thread_message_content', array( &$this, 'autoembed' ), 8 );
			add_filter( 'sz_get_the_thread_message_content', array( &$this, 'run_shortcode' ), 7 );
		}

		/**
		 * Filters the SportsZone Core oEmbed setup.
		 *
		 * @since 1.5.0
		 *
		 * @param SZ_Embed $this Current instance of the SZ_Embed. Passed by reference.
		 */
		do_action_ref_array( 'sz_core_setup_oembed', array( &$this ) );
	}
	 ...
	 * enabled, then the URL will be passed to {@link SZ_Embed::parse_oembed()}
	 * for oEmbed parsing.
	 *
	 *
	...
	public function shortcode( $attr, $url = '' ) {
		 ...
		 */
		$attr['discover'] = ( apply_filters( 'sz_embed_oembed_discover', $default_discovery ) && $unfiltered_html );
	 ...
	 *
	 * View an example to add support in {@link sz_activity_embed()}.
	 *
	 *       on success.
	 *       oEmbed failure.
	 *
	...
	public function parse_oembed( $id, $url, $attr, $rawattr ) {
		...
		if ( $id ) {
			 ...
			 */
			$cache = apply_filters( 'sz_embed_get_cache', $cache, $id, $cachekey, $url, $attr, $rawattr );
			...
			if ( !empty( $cache ) ) {
				 ...
				 */
				return apply_filters( 'sz_embed_oembed_html', $cache, $url, $attr, $rawattr );
			...
			// If no cache, ping the oEmbed provider and cache the result.
				 ...
				 * @param int    $id       ID to do the caching for.
				 */
				do_action( 'sz_embed_update_cache', $cache, $cachekey, $id );

				// If there was a result, return it.
				if ( $html ) {

					/** This filter is documented in sz-core/classes/class-sz-embed.php */
					return apply_filters( 'sz_embed_oembed_html', $html, $url, $attr, $rawattr );
				}
			}
		}
