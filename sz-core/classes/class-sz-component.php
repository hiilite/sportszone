...
/**
 * SportsZone Component Class.
 *
 * The SportsZone component class is responsible for simplifying the creation
 * of components that share similar behaviors and routines. It is used
 * internally by SportsZone to create the bundled components, but can be
...
class SZ_Component {
	 ...
	 *                     __( 'Groups', 'sportszone' ).
	 * @param string $path The file path for the component's files. Used by {@link SZ_Component::includes()}.
	 * @param array  $params {
	 ...
	 *                                            would look for a file called "sz-{$this->id}-awesome.php" and you could use
	 *                                            sz_is_active( $this->id, 'awesome' ) to determine if the feature is active.
	...
	public function setup_globals( $args = array() ) {
		...
		$default_root_slug = isset( sportszone()->pages->{$this->id}->slug ) ? sportszone()->pages->{$this->id}->slug : '';
		 ...
		 */
		$this->slug                  = apply_filters( 'sz_' . $this->id . '_slug',                  $r['slug']                  );
		 ...
		 */
		$this->root_slug             = apply_filters( 'sz_' . $this->id . '_root_slug',             $r['root_slug']             );
		 ...
		 */
		$this->has_directory         = apply_filters( 'sz_' . $this->id . '_has_directory',         $r['has_directory']         );
		 ...
		 */
		$this->directory_title       = apply_filters( 'sz_' . $this->id . '_directory_title',       $r['directory_title']         );
		 ...
		 */
		$this->search_string         = apply_filters( 'sz_' . $this->id . '_search_string',         $r['search_string']         );
		 ...
		 */
		$this->notification_callback = apply_filters( 'sz_' . $this->id . '_notification_callback', $r['notification_callback'] );
		...
		sportszone()->loaded_components[$this->slug] = $this->id;

		/**
		 * Fires at the end of the setup_globals method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'sz_' . $this->id . '_setup_globals' );
	}

	/**
	 * Include required files.
	 *
	 * Please note that, by default, this method is fired on the sz_include
	 ...
	 * loaded at sz_include with priority 10 (as recommended), will not be
	 ...
	 * called 'my_component'):
	 *   - ./actions
	 *   - ./sz-my_component/actions
	 *   - ./sz-my_component/sz-my_component-actions.php
	 *
	 * @since 1.5.0
	 *
	 *
	...
	public function includes( $includes = array() ) {
		...
		if ( ! empty( $includes ) ) {
			...
			foreach ( (array) $includes as $file ) {
				...
				$paths = array(
					...
					'sz-' . $this->id . '/sz-' . $this->id . '-' . $file  . '.php',
					'sz-' . $this->id . '-' . $file . '.php',
					'sz-' . $this->id . '/' . $file . '.php',
		...
		/**
		 * Fires at the end of the includes method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'sz_' . $this->id . '_includes' );
	}
	 ...
	 * conditionals such as {@link sz_is_current_component()}. Intentionally left
	 * empty.
	 *
	 * @since 3.0.0
	 */
	...
	public function setup_actions() {
		...
		// Setup globals.
		add_action( 'sz_setup_globals',          array( $this, 'setup_globals'          ), 10 );

		// Set up canonical stack.
		add_action( 'sz_setup_canonical_stack',  array( $this, 'setup_canonical_stack'  ), 10 );
		...
		// to sz_include with the default priority of 10. This is for backwards
		// compatibility; henceforth, plugins should register themselves by
		// extending this base class.
		add_action( 'sz_include',                array( $this, 'includes'               ), 8 );

		// Load files conditionally, based on certain pages.
		add_action( 'sz_late_include',           array( $this, 'late_includes'          ) );

		// Setup navigation.
		add_action( 'sz_setup_nav',              array( $this, 'setup_nav'              ), 10 );

		// Setup WP Toolbar menus.
		add_action( 'sz_setup_admin_bar',        array( $this, 'setup_admin_bar'        ), $this->adminbar_myaccount_order );

		// Setup component title.
		add_action( 'sz_setup_title',            array( $this, 'setup_title'            ), 10 );
		...
		// Register taxonomies.
		add_action( 'sz_register_taxonomies',    array( $this, 'register_taxonomies'    ), 10 );

		// Add the rewrite tags.
		add_action( 'sz_add_rewrite_tags',       array( $this, 'add_rewrite_tags'       ), 10 );

		// Add the rewrite rules.
		add_action( 'sz_add_rewrite_rules',      array( $this, 'add_rewrite_rules'      ), 10 );

		// Add the permalink structure.
		add_action( 'sz_add_permastructs',       array( $this, 'add_permastructs'       ), 10 );

		// Allow components to parse the main query.
		add_action( 'sz_parse_query',            array( $this, 'parse_query'            ), 10 );

		// Generate rewrite rules.
		add_action( 'sz_generate_rewrite_rules', array( $this, 'generate_rewrite_rules' ), 10 );

		/**
		 * Fires at the end of the setup_actions method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'sz_' . $this->id . '_setup_actions' );
	}
	 ...
	 * @since 1.5.0
	 *
	 * @see sz_core_new_nav_item() For a description of the $main_nav
	 *      parameter formatting.
	 * @see sz_core_new_subnav_item() For a description of how each item
	 *      in the $sub_nav parameter array should be formatted.
	 *
	 * @param array $main_nav Optional. Passed directly to sz_core_new_nav_item().
	 *                        See that function for a description.
	 * @param array $sub_nav  Optional. Multidimensional array, each item in
	 *                        which is passed to sz_core_new_subnav_item(). See that
	...
	public function setup_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		if ( !empty( $main_nav ) ) {
			sz_core_new_nav_item( $main_nav, 'members' );

			// Sub nav items are not required.
			if ( !empty( $sub_nav ) ) {
				foreach( (array) $sub_nav as $nav ) {
					sz_core_new_subnav_item( $nav, 'members' );
				}
			}
		}

		/**
		 * Fires at the end of the setup_nav method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'sz_' . $this->id . '_setup_nav' );
	}
	...
	public function setup_admin_bar( $wp_admin_nav = array() ) {
		...
		// Do not proceed if SZ_USE_WP_ADMIN_BAR constant is not set or is false.
		 ...
		 */
		$wp_admin_nav = apply_filters( 'sz_' . $this->id . '_admin_nav', $wp_admin_nav );
		...
		if ( !empty( $wp_admin_nav ) ) {
			...
			$wp_admin_nav = sz_sort_by_key( $wp_admin_nav, 'position', 'num' );
		...
		/**
		 * Fires at the end of the setup_admin_bar method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'sz_' . $this->id . '_setup_admin_bar' );
	}
	...
	public function setup_title() {
		...
		/**
		 * Fires in the setup_title method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action(  'sz_' . $this->id . '_setup_title' );
	}
	...
	public function setup_cache_groups() {
		...
		/**
		 * Fires in the setup_cache_groups method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 2.2.0
	...
	public function register_global_tables( $tables = array() ) {
		 ...
		 * *all* tables, use the 'sz_core_get_table_prefix' filter instead.
		 *
		 * @since 1.6.0
		 */
		$tables = apply_filters( 'sz_' . $this->id . '_global_tables', $tables );

		// Add to the SportsZone global object.
		...
		/**
		 * Fires at the end of the register_global_tables method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 2.0.0
		 */
		do_action( 'sz_' . $this->id . '_register_global_tables' );
	}
	...
	public function register_meta_tables( $tables = array() ) {
		 ...
		 * @since 2.0.0
		 */
		$tables = apply_filters( 'sz_' . $this->id . '_meta_tables', $tables );

		/**
		 * Add the name of each metadata table to WPDB to allow SportsZone
		...
		/**
		 * Fires at the end of the register_meta_tables method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 2.0.0
		 */
		do_action( 'sz_' . $this->id . '_register_meta_tables' );
	}
	...
	public function register_post_types() {
		...
		/**
		 * Fires in the register_post_types method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'sz_' . $this->id . '_register_post_types' );
	}
	...
	public function register_taxonomies() {
		...
		/**
		 * Fires in the register_taxonomies method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'sz_' . $this->id . '_register_taxonomies' );
	}
	...
	public function add_rewrite_tags() {
		...
		/**
		 * Fires in the add_rewrite_tags method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'sz_' . $this->id . '_add_rewrite_tags' );
	}
	...
	public function add_rewrite_rules() {
		...
		/**
		 * Fires in the add_rewrite_rules method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.9.0
		 */
		do_action( 'sz_' . $this->id . '_add_rewrite_rules' );
	}
	...
	public function add_permastructs() {
		...
		/**
		 * Fires in the add_permastructs method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.9.0
		 */
		do_action( 'sz_' . $this->id . '_add_permastructs' );
	}
	...
	public function parse_query( $query ) {
		...
		/**
		 * Fires in the parse_query method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.9.0
		 ...
		 */
		do_action_ref_array( 'sz_' . $this->id . '_parse_query', array( &$query ) );
	}
	...
	public function generate_rewrite_rules() {
		...
		/**
		 * Fires in the generate_rewrite_rules method inside SZ_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'sz_' . $this->id . '_generate_rewrite_rules' );
	}
}
endif; // SZ_Component.
