...
/**
 * SZ_Core_Notification is deprecated.
 *
 * Use SZ_Notifications_Notification instead.
 *
 * @deprecated since 1.9.0
 */
class SZ_Core_Notification {
	...
	public function save() {
		...
		// Update.
		if ( !empty( $this->id ) ) {
			$sql = $wpdb->prepare( "UPDATE {$sz->core->table_name_notifications} SET item_id = %d, secondary_item_id = %d, user_id = %d, component_name = %s, component_action = %d, date_notified = %s, is_new = %d ) WHERE id = %d", $this->item_id, $this->secondary_item_id, $this->user_id, $this->component_name, $this->component_action, $this->date_notified, $this->is_new, $this->id );

		// Save.
		} else {
			$sql = $wpdb->prepare( "INSERT INTO {$sz->core->table_name_notifications} ( item_id, secondary_item_id, user_id, component_name, component_action, date_notified, is_new ) VALUES ( %d, %d, %d, %s, %s, %s, %d )", $this->item_id, $this->secondary_item_id, $this->user_id, $this->component_name, $this->component_action, $this->date_notified, $this->is_new );
		}
	...
	public function populate() {
		...
		if ( $notification = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$sz->core->table_name_notifications} WHERE id = %d", $this->id ) ) ) {
	...
	public static function get_all_for_user( $user_id, $status = 'is_new' ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$sz->core->table_name_notifications} WHERE user_id = %d {$is_new}", $user_id ) );
	}
	...
	public static function delete_for_user_by_type( $user_id, $component_name, $component_action ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->core->table_name_notifications} WHERE user_id = %d AND component_name = %s AND component_action = %s", $user_id, $component_name, $component_action ) );
	}
	...
	public static function delete_for_user_by_item_id( $user_id, $item_id, $component_name, $component_action, $secondary_item_id = false ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->core->table_name_notifications} WHERE user_id = %d AND item_id = %d AND component_name = %s AND component_action = %s{$secondary_item_sql}", $user_id, $item_id, $component_name, $component_action ) );
	}
	...
	public static function delete_from_user_by_type( $user_id, $component_name, $component_action ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->core->table_name_notifications} WHERE item_id = %d AND component_name = %s AND component_action = %s", $user_id, $component_name, $component_action ) );
	}
	...
	public static function delete_all_by_type( $item_id, $component_name, $component_action, $secondary_item_id ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->core->table_name_notifications} WHERE item_id = %d AND component_name = %s {$component_action_sql} {$secondary_item_sql}", $item_id, $component_name ) );
	}
}
