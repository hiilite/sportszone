...
abstract class SZ_Attachment {
	...
	public function __construct( $args = '' ) {
		...
		$params = sz_parse_args( $args, $this->default_args, $this->action . '_upload_params' );
	 ...
	 *
	 * Used into the $overrides argument of SZ_Attachment->upload()
	 *
	 * @since 2.3.0
	 *
	 * @param array $param A list of error messages to add to SportsZone core ones.
	...
	public function set_upload_error_strings( $param = array() ) {
		 ...
		 */
		$upload_errors = array(
			0 => __( 'The file was uploaded successfully', 'sportszone' ),
			1 => __( 'The uploaded file exceeds the maximum allowed file size for this site', 'sportszone' ),
			2 => sprintf( __( 'The uploaded file exceeds the maximum allowed file size of: %s', 'sportszone' ), size_format( $this->original_max_filesize ) ),
			3 => __( 'The uploaded file was only partially uploaded.', 'sportszone' ),
			4 => __( 'No file was uploaded.', 'sportszone' ),
			5 => '',
			6 => __( 'Missing a temporary folder.', 'sportszone' ),
			7 => __( 'Failed to write file to disk.', 'sportszone' ),
			8 => __( 'File upload stopped by extension.', 'sportszone' ),
		);
	...
	public function upload( $file, $upload_dir_filter = '', $time = null ) {
		 ...
		 */
		$overrides = apply_filters( 'sz_attachment_upload_overrides', $overrides );
		 ...
		 * @see  SZ_Attachment->upload_dir_filter()
		 */
	 ...
	 * the 'upload_filetypes' setting. SportsZone will respect this setting.
	 *
	 ...
	 * on the file size.
	 *
	 * @see SZ_Attachment_Avatar->validate_upload() for an example of use
	 *
	 * @since 2.3.0
	 *
	...
	public function upload_dir_filter( $upload_dir = array() ) {
		 ...
		 * @param array $upload_dir     The original Uploads dir.
		 */
		return apply_filters( 'sz_attachment_upload_dir', array(
	...
	public function crop( $args = array() ) {
		...
		$r = sz_parse_args( $args, array(
			...
			'src_abs'       => false,
			'dst_file'      => false,
		), 'sz_attachment_crop_args' );

		if ( empty( $r['original_file'] ) || ! file_exists( $r['original_file'] ) ) {
			$wp_error->add( 'crop_error', __( 'Cropping the file failed: missing source file.', 'sportszone' ) );
			return $wp_error;
		}

		// Check image file pathes.
		$path_error = __( 'Cropping the file failed: the file path is not allowed.', 'sportszone' );
		...
		$check_types = array( 'src_file' => array( 'file' => $r['original_file'], 'error' => _x( 'source file', 'Attachment source file', 'sportszone' ) ) );
		if ( ! empty( $r['dst_file'] ) ) {
			$check_types['dst_file'] = array( 'file' => $r['dst_file'], 'error' => _x( 'destination file', 'Attachment destination file', 'sportszone' ) );
		}
		...
		foreach ( $check_types as $file ) {
			...
			if ( empty( $ext ) || empty( $supported_image_types[ $ext ] ) ) {
				$wp_error->add( 'crop_error', sprintf( __( 'Cropping the file failed: %s is not a supported image file.', 'sportszone' ), $file['error'] ) );
				return $wp_error;
			}
		}
	...
	public function script_data() {
		$script_data = array(
			...
			'feedback_messages' => array(
				1 => __( 'Sorry, uploading the file failed.', 'sportszone' ),
				2 => __( 'File successfully uploaded.', 'sportszone' ),
			),
		);
	...
	public static function get_image_data( $file ) {
		 ...
		 */
		return apply_filters( 'sz_attachments_get_image_data', $image_data );
	}
