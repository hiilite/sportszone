...
/**
 * API to create SportsZone buttons.
 *
 * @since 1.2.6
...
class SZ_Button {
	 ...
	 * @since 1.2.6
	 *
	 * @param array|string $args See {@SZ_Button}.
	 */
	public function __construct( $args = '' ) {
		...
		if ( empty( $r['id'] ) || empty( $r['component'] ) || ! sz_is_active( $this->component ) ) {
			return false;
		}
		...
		if ( true == $this->block_self ) {
			 ...
			 * profiles.
			 */
			if ( sz_get_member_user_id() ) {
				if ( is_user_logged_in() && sz_loggedin_user_id() == sz_get_member_user_id() ) {
					return false;
				}

			// No button if viewing your own profile (and not in a members loop).
			} elseif ( sz_is_my_profile() ) {
				return false;
			}
		}
		...
		if ( ! empty( $r['button_element'] ) ) {
			$button = new SZ_Core_HTML_Element( array(
		 ...
		 * @param SZ_Button $this     Current SZ_Button instance.
		 ...
		 */
		$this->contents = apply_filters( 'sz_button_' . $this->component . '_' . $this->id, $this->contents, $this, $before, $after, $r );
	}
	 ...
	 * @since 2.7.0.
	 *
	 * @param  array $r See {@link SZ_Button} class for full documentation.
	 * @return array
	 */
