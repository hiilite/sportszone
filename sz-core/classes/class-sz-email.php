...
class SZ_Email {
	 ...
	 * @since 2.5.0
	 *
	 * @var SZ_Email_Recipient[] BCC recipients.
	 */
	protected $bcc = array();
	 ...
	 * @since 2.5.0
	 *
	 * @var SZ_Email_Recipient[] CC recipients.
	 */
	protected $cc = array();
	 ...
	 * @since 2.5.0
	 *
	 * @var SZ_Email_Recipient Sender details.
	 */
	protected $from = null;
	 ...
	 * @since 2.5.0
	 *
	 * @var SZ_Email_Recipient "Reply to" details.
	 */
	 ...
	 * @since 2.5.0
	 *
	 * @var SZ_Email_Recipient[] Email recipients.
	 * }
	 */
	protected $to = array();
	...
	public function __construct( $email_type ) {
		...
		$from_name    = wp_specialchars_decode( sz_get_option( 'blogname' ), ENT_QUOTES );
		...
		$this->set_reply_to( sz_get_option( 'admin_email' ), $from_name );

		/**
		 * Fires inside __construct() method for SZ_Email class.
		 *
		 * @since 2.5.0
		 *
		 * @param string $email_type Unique identifier for this type of email.
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		do_action( 'sz_email', $email_type, $this );
	}
	...
	public function get( $property_name, $transform = 'raw' ) {
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$retval = apply_filters( "sz_email_get_{$property_name}", $this->$property_name, $property_name, $transform, $this );
		...
		switch ( $transform ) {
			...
			case 'replace-tokens':
				$retval = sz_core_replace_tokens_in_text( $retval, $this->get_tokens( 'raw' ) );
				// Fall through.
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		return apply_filters( 'sz_email_get_property', $retval, $property_name, $transform, $this );
	}
	 ...
	 * @param string $transform Optional. How to transform the return value.
	 *                          Accepts 'raw' (default) or 'replace-tokens'.
	 * @return SZ_Email_Recipient[] BCC recipients.
	 */
	 ...
	 * @param string $transform Optional. How to transform the return value.
	 *                          Accepts 'raw' (default) or 'replace-tokens'.
	 * @return SZ_Email_Recipient[] CC recipients.
	 */
	 ...
	 * @param string $transform Optional. How to transform the return value.
	 *                          Accepts 'raw' (default) or 'replace-tokens'.
	 * @return SZ_Email_Recipient "From" recipient.
	 */
	 ...
	 * @param string $transform Optional. How to transform the return value.
	 *                          Accepts 'raw' (default) or 'replace-tokens'.
	 * @return SZ_Email_Recipient "Reply to" recipient.
	 */
	 ...
	 * @param string $transform Optional. How to transform the return value.
	 *                          Accepts 'raw' (default) or 'replace-tokens'.
	 * @return SZ_Email_Recipient[] "To" recipients.
	 */
	...
	public function set_headers( array $headers ) {
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->headers = apply_filters( 'sz_email_set_headers', $new_headers, $this );
	...
	public function set_bcc( $bcc_address, $name = '', $operation = 'replace' ) {
		...
		if ( is_array( $bcc_address ) ) {
			foreach ( $bcc_address as $address ) {
				$bcc[] = new SZ_Email_Recipient( $address );
			}

		} else {
			$bcc[] = new SZ_Email_Recipient( $bcc_address, $name );
		}
		 ...
		 * @since 2.5.0
		 *
		 * @param SZ_Email_Recipient[] $bcc BCC recipients.
		 * @param string|array|int|WP_User $bcc_address Either a email address, user ID, WP_User object,
		 *                                              or an array containing any combination of the above.
		 * @param string $name Optional. If $bcc_address is a string, this is the recipient's name.
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->bcc = apply_filters( 'sz_email_set_bcc', $bcc, $bcc_address, $name, $operation, $this );
	...
	public function set_cc( $cc_address, $name = '', $operation = 'replace' ) {
		...
		if ( is_array( $cc_address ) ) {
			foreach ( $cc_address as $address ) {
				$cc[] = new SZ_Email_Recipient( $address );
			}

		} else {
			$cc[] = new SZ_Email_Recipient( $cc_address, $name );
		}
		 ...
		 * @since 2.5.0
		 *
		 * @param SZ_Email_Recipient[] $cc CC recipients.
		 * @param string|array|int|WP_User $cc_address Either a email address, user ID, WP_User object,
		 *                                             or an array containing any combination of the above.
		 * @param string $name Optional. If $cc_address is a string, this is the recipient's name.
		 * @param string $operation If "replace", $to_address replaced previous recipients. If "add",
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->cc = apply_filters( 'sz_email_set_cc', $cc, $cc_address, $name, $operation, $this );
	...
	public function set_content_html( $content ) {
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->content_html = apply_filters( 'sz_email_set_content_html', $content, $this );
	...
	public function set_content_plaintext( $content ) {
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->content_plaintext = apply_filters( 'sz_email_set_content_plaintext', $content, $this );
	...
	public function set_content_type( $content_type ) {
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->content_type = apply_filters( 'sz_email_set_content_type', $content_type, $this );
	...
	public function set_from( $email_address, $name = '' ) {
		$from = new SZ_Email_Recipient( $email_address, $name );
		 ...
		 * @since 2.5.0
		 *
		 * @param SZ_Email_Recipient $from Sender details.
		 * @param string|array|int|WP_User $email_address Either a email address, user ID, or WP_User object.
		 * @param string $name Optional. If $email_address is a string, this is the recipient's name.
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->from = apply_filters( 'sz_email_set_from', $from, $email_address, $name, $this );
	...
	public function set_post_object( WP_Post $post ) {
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->post_object = apply_filters( 'sz_email_set_post_object', $post, $this );
		...
		if ( is_a( $this->post_object, 'WP_Post' ) ) {
			...
			// Load the template.
			add_filter( 'sz_locate_template_and_load', '__return_true' );

			sz_locate_template( sz_email_get_template( $this->post_object ), true, false );

			remove_filter( 'sz_locate_template_and_load', '__return_true' );
	...
	public function set_reply_to( $email_address, $name = '' ) {
		$reply_to = new SZ_Email_Recipient( $email_address, $name );
		 ...
		 * @since 2.5.0
		 *
		 * @param SZ_Email_Recipient $reply_to "Reply to" recipient.
		 * @param string|array|int|WP_User $email_address Either a email address, user ID, WP_User object,
		 *                                                or an array containing any combination of the above.
		 * @param string $name Optional. If $email_address is a string, this is the recipient's name.
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->reply_to = apply_filters( 'sz_email_set_reply_to', $reply_to, $email_address, $name, $this );
	...
	public function set_subject( $subject ) {
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->subject = apply_filters( 'sz_email_set_subject', $subject, $this );
	...
	public function set_template( $template ) {
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->template = apply_filters( 'sz_email_set_template', $template, $this );
	 ...
	 *
	 * IMPORTANT NOTE: the assumption with all emails sent by (and belonging to) SportsZone itself
	...
	public function set_to( $to_address, $name = '', $operation = 'replace' ) {
		...
		if ( is_array( $to_address ) ) {
			foreach ( $to_address as $address ) {
				$to[] = new SZ_Email_Recipient( $address );
			}

		} else {
			$to[] = new SZ_Email_Recipient( $to_address, $name );
		}
		 ...
		 * @since 2.5.0
		 *
		 * @param SZ_Email_Recipient[] "To" recipients.
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		$this->to = apply_filters( 'sz_email_set_to', $to, $to_address, $name, $operation, $this );
	...
	public function set_tokens( array $tokens ) {
		 ...
		 * @param SZ_Email $this             Current instance of the email type class.
		 */
		$this->tokens = apply_filters( 'sz_email_set_tokens', $formatted_tokens, $tokens, $this );
	...
	public function validate() {
		 ...
		 * @param SZ_Email $this Current instance of the email type class.
		 */
		return apply_filters( 'sz_email_validate', $retval, $this );
	}
}
