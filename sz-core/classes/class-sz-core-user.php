...
/**
 * Fetch data about a SportsZone user.
 *
 * SZ_Core_User class can be used by any component. It will fetch useful
 * details for any user when provided with a user_id.
 *
 * Example:
 *    $user = new SZ_Core_User( $user_id );
...
class SZ_Core_User {
	...
	public function populate() {
		...
		if ( !empty( $this->profile_data ) ) {
			$full_name_field_name = sz_xprofile_fullname_field_name();

			$this->user_url  = sz_core_get_user_domain( $this->id, $this->profile_data['user_nicename'], $this->profile_data['user_login'] );
			...
			$this->user_url  = sz_core_get_user_domain( $this->id );
			$this->user_link = sz_core_get_userlink( $this->id );
			$this->fullname  = esc_attr( sz_core_get_user_displayname( $this->id ) );
			$this->email     = esc_attr( sz_core_get_user_email( $this->id ) );
		}

		$this->avatar       = sz_core_fetch_avatar( array( 'item_id' => $this->id, 'type' => 'full', 'alt' => sprintf( __( 'Profile photo of %s', 'sportszone' ), $this->fullname ) ) );
		$this->avatar_thumb = sz_core_fetch_avatar( array( 'item_id' => $this->id, 'type' => 'thumb', 'alt' => sprintf( __( 'Profile photo of %s', 'sportszone' ), $this->fullname ) ) );
		$this->avatar_mini  = sz_core_fetch_avatar( array( 'item_id' => $this->id, 'type' => 'thumb', 'alt' => sprintf( __( 'Profile photo of %s', 'sportszone' ), $this->fullname ), 'width' => 30, 'height' => 30 ) );
		$this->last_active  = sz_core_get_last_activity( sz_get_user_last_activity( $this->id ), __( 'active %s', 'sportszone' ) );
	}
	...
	public function populate_extras() {
		...
		if ( sz_is_active( 'friends' ) ) {
			$this->total_friends = SZ_Friends_Friendship::total_friend_count( $this->id );
		}

		if ( sz_is_active( 'groups' ) ) {
			$this->total_groups = SZ_Groups_Member::total_group_count( $this->id );
			$this->total_groups = sprintf( _n( '%d group', '%d groups', $this->total_groups, 'sportszone' ), $this->total_groups );
		}
	}
	 ...
	 *
	 * @see SZ_XProfile_ProfileData::get_all_for_user() for description of
	 *      return value.
	 *
	 * @return array See {@link SZ_XProfile_Profile_Data::get_all_for_user()}.
	 */
	public function get_profile_data() {
		return SZ_XProfile_ProfileData::get_all_for_user( $this->id );
	}
	 ...
	 *
	 * Since SportsZone 1.7, use {@link SZ_User_Query} instead.
	 *
	 * @deprecated 1.7.0 Use {@link SZ_User_Query}.
	 *
	 * @see SZ_User_Query for a description of parameters, most of which
	 *      are used there in the same way.
	 *
	 * @param string      $type            See {@link SZ_User_Query}.
	 * @param int         $limit           See {@link SZ_User_Query}. Default: 0.
	 * @param int         $page            See {@link SZ_User_Query}. Default: 1.
	 * @param int         $user_id         See {@link SZ_User_Query}. Default: 0.
	 * @param mixed       $include         See {@link SZ_User_Query}. Default: false.
	 * @param string|bool $search_terms    See {@link SZ_User_Query}.
	 *                                     Default: false.
	 * @param bool        $populate_extras See {@link SZ_User_Query}.
	 *                                     Default: true.
	 * @param mixed       $exclude         See {@link SZ_User_Query}. Default: false.
	 * @param string|bool $meta_key        See {@link SZ_User_Query}.
	 *                                     Default: false.
	 * @param string|bool $meta_value      See {@link SZ_User_Query}.
	 *                                     Default: false.
	 * @return false|array {
	...
	public static function get_users( $type, $limit = 0, $page = 1, $user_id = 0, $include = false, $search_terms = false, $populate_extras = true, $exclude = false, $meta_key = false, $meta_value = false ) {
		...
		_deprecated_function( __METHOD__, '1.7', 'SZ_User_Query' );
		...
		if ( $search_terms && sz_is_active( 'xprofile' ) ) {
			$sql['join_profiledata_search'] = "LEFT JOIN {$sz->profile->table_name_data} spd ON u.ID = spd.user_id";
		}

		// Alphabetical sorting is done by the xprofile Full Name field.
		if ( 'alphabetical' == $type ) {
			$sql['join_profiledata_alpha'] = "LEFT JOIN {$sz->profile->table_name_data} pd ON u.ID = pd.user_id";
		}
		...
		$sql['where'] = 'WHERE ' . sz_core_get_status_sql( 'u.' );

		if ( 'active' == $type || 'online' == $type || 'newest' == $type ) {
			$sql['where_active'] = $wpdb->prepare( "AND um.meta_key = %s", sz_get_user_meta_key( 'last_activity' ) );
		}

		if ( 'popular' == $type ) {
			$sql['where_popular'] = $wpdb->prepare( "AND um.meta_key = %s", sz_get_user_meta_key( 'total_friend_count' ) );
		}
		...
		if ( 0 === $include || '0' === $include ) {
			...
			} elseif ( !empty( $user_id ) && sz_is_active( 'friends' ) ) {
		...
		if ( !empty( $search_terms ) && sz_is_active( 'xprofile' ) ) {
			$search_terms_like        = '%' . sz_esc_like( $search_terms ) . '%';
		 ...
		 */
		$paged_users_sql = apply_filters( 'sz_core_get_paged_users_sql', join( ' ', (array) $sql ), $sql );
		 ...
		 * @param array  $sql   Array of SQL statement parts for the query.
		 */
		$total_users_sql = apply_filters( 'sz_core_get_total_users_sql', join( ' ', (array) $sql ), $sql );
		...
		if ( !empty( $populate_extras ) ) {
			...
			$paged_users = SZ_Core_User::get_user_extras( $paged_users, $user_ids, $type );
		}
	...
	public static function get_users_by_letter( $letter, $limit = null, $page = 1, $populate_extras = true, $exclude = '' ) {
		...
		$letter_like = sz_esc_like( $letter ) . '%';
		$status_sql  = sz_core_get_status_sql( 'u.' );
		 ...
		 */
		$total_users_sql = apply_filters( 'sz_core_users_by_letter_count_sql', $wpdb->prepare( "SELECT COUNT(DISTINCT u.ID) FROM {$wpdb->users} u LEFT JOIN {$sz->profile->table_name_data} pd ON u.ID = pd.user_id LEFT JOIN {$sz->profile->table_name_fields} pf ON pd.field_id = pf.id WHERE {$status_sql} AND pf.name = %s {$exclude_sql} AND pd.value LIKE %s ORDER BY pd.value ASC", sz_xprofile_fullname_field_name(), $letter_like ) );
		 ...
		 */
		$paged_users_sql = apply_filters( 'sz_core_users_by_letter_sql',       $wpdb->prepare( "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.user_email FROM {$wpdb->users} u LEFT JOIN {$sz->profile->table_name_data} pd ON u.ID = pd.user_id LEFT JOIN {$sz->profile->table_name_fields} pf ON pd.field_id = pf.id WHERE {$status_sql} AND pf.name = %s {$exclude_sql} AND pd.value LIKE %s ORDER BY pd.value ASC{$pag_sql}", sz_xprofile_fullname_field_name(), $letter_like ) );
		...
		// Add additional data to the returned results.
		if ( $populate_extras ) {
			$paged_users = SZ_Core_User::get_user_extras( $paged_users, $user_ids );
		}
	 ...
	 *
	 * Use {@link SZ_User_Query} with the 'user_ids' param instead.
	 *
	 * @global wpdb $wpdb WordPress database object.
	 *
	...
	public static function get_specific_users( $user_ids, $limit = null, $page = 1, $populate_extras = true ) {
		 ...
		 * Use 'sz_core_user_get_specific_users_count_sql' instead.
		 *
		 * @deprecated 2.3.0
		 *
		 * @param string $sql SQL string.
		 */
		$total_users_sql = apply_filters( 'sz_core_get_specific_users_count_sql', "SELECT COUNT(ID) FROM {$wpdb->users} WHERE {$status_sql} AND ID IN ({$user_ids})" );

		/**
		 * Filter the SQL string used for querying specific user count results.
		 *
		 * Use this instead of the deprecated 'sz_core_get_specific_users_count_sql'.
		 *
		 * @since 2.3.0
		 *
		 * @param string   $sql             SQL string.
		 ...
		 */
		$total_users_sql = apply_filters( 'sz_core_user_get_specific_users_count_sql', $total_users_sql, $user_ids, $limit, $page, $populate_extras );
		 ...
		 * Use 'sz_core_user_get_specific_users_paged_sql' instead.
		 *
		 * @deprecated 2.3.0
		 *
		 * @param string $sql SQL string.
		 */
		$paged_users_sql = apply_filters( 'sz_core_get_specific_users_count_sql', "SELECT ID as id, user_registered, user_nicename, user_login, user_email FROM {$wpdb->users} WHERE {$status_sql} AND ID IN ({$user_ids}) {$pag_sql}" );
		 ...
		 * @param bool     $populate_extras Whether to populate extra user fields.
		 */
		$paged_users_sql = apply_filters( 'sz_core_user_get_specific_users_paged_sql', $paged_users_sql, $user_ids, $limit, $page, $populate_extras );
	...
	public static function search_users( $search_terms, $limit = null, $page = 1, $populate_extras = true ) {
		 ...
		 */
		$total_users_sql = apply_filters( 'sz_core_search_users_count_sql', $wpdb->prepare( "SELECT COUNT(DISTINCT u.ID) as id FROM {$wpdb->users} u LEFT JOIN {$sz->profile->table_name_data} pd ON u.ID = pd.user_id WHERE {$status_sql} AND pd.value LIKE %s ORDER BY pd.value ASC", $search_terms_like ), $search_terms );
		 ...
		 */
		$paged_users_sql = apply_filters( 'sz_core_search_users_sql',       $wpdb->prepare( "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.user_email FROM {$wpdb->users} u LEFT JOIN {$sz->profile->table_name_data} pd ON u.ID = pd.user_id WHERE {$status_sql} AND pd.value LIKE %s ORDER BY pd.value ASC{$pag_sql}", $search_terms_like ), $search_terms, $pag_sql );
	...
	public static function get_user_extras( &$paged_users, &$user_ids, $type = false ) {
		...
		if ( sz_is_active( 'xprofile' ) && 'alphabetical' != $type ) {
			$names = $wpdb->get_results( $wpdb->prepare( "SELECT pd.user_id as id, pd.value as fullname FROM {$sz->profile->table_name_fields} pf, {$sz->profile->table_name_data} pd WHERE pf.id = pd.field_id AND pf.name = %s AND pd.user_id IN ( {$user_ids} )", sz_xprofile_fullname_field_name() ) );
		...
		if ( 'popular' != $type ) {
			$friend_count = $wpdb->get_results( $wpdb->prepare( "SELECT user_id as id, meta_value as total_friend_count FROM {$wpdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )", sz_get_user_meta_key( 'total_friend_count' ) ) );
			for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
		...
		if ( 'active' != $type ) {
			$user_activity = $wpdb->get_results( $wpdb->prepare( "SELECT user_id as id, meta_value as last_activity FROM {$wpdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )", sz_get_user_meta_key( 'last_activity' ) ) );
			for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
		...
		$user_update = $wpdb->get_results( $wpdb->prepare( "SELECT user_id as id, meta_value as latest_update FROM {$wpdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )", sz_get_user_meta_key( 'sz_latest_update' ) ) );
		for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
	...
	public static function get_last_activity( $user_id ) {
		...
		$uncached_user_ids = sz_get_non_cached_ids( $user_ids, 'sz_last_activity' );
		if ( ! empty( $uncached_user_ids ) ) {
			...
			$last_activities = $wpdb->get_results( $wpdb->prepare( "SELECT id, user_id, date_recorded FROM {$sz->members->table_name_last_activity} WHERE component = %s AND type = 'last_activity' AND user_id IN ({$user_ids_sql}) LIMIT {$user_count}", $sz->members->id ) );
		...
		foreach ( $user_ids as $user_id ) {
			$retval[ $user_id ] = wp_cache_get( $user_id, 'sz_last_activity' );
	...
	public static function update_last_activity( $user_id, $time ) {
		...
		$table_name = sportszone()->members->table_name_last_activity;
		...
		// Set cache.
		wp_cache_set( $user_id, $activity[ $user_id ], 'sz_last_activity' );
		 ...
		 */
		do_action( 'sz_core_user_updated_last_activity', $user_id, $time );
	...
	public static function delete_last_activity( $user_id ) {
		...
		$deleted = $wpdb->delete(
			sportszone()->members->table_name_last_activity,
		...
		wp_cache_delete( $user_id, 'sz_last_activity' );
