...
/**
 * Base class for the SportsZone Suggestions API.
 *
 * Originally built to power SportsZone' at-mentions suggestions, it's flexible enough to be used
 ...
 * the list of default services in {@link sz_core_get_suggestions()}. If you're building a plugin,
 * it's recommend that you use the `sz_suggestions_services` filter to do this. :)
 *
...
abstract class SZ_Suggestions {
	...
	public function validate() {
		...
		/**
		 * Filters the arguments to be validated for the SZ_Suggestions query.
		 *
		 * @since 2.1.0
		 *
		 * @param SZ_Suggestions $value Arguments to be validated.
		 * @param SZ_Suggestions $this  Current SZ_Suggestions instance.
		 */
		$this->args          = apply_filters( 'sz_suggestions_args', $this->args, $this );
		...
		if ( is_user_logged_in() && ! sz_is_user_active( get_current_user_id() ) ) {
			return new WP_Error( 'invalid_user' );
		}

		/**
		 * Filters the status of validation for the SZ_Suggestions query.
		 *
		 * @since 2.1.0
		 *
		 ...
		 * @param SZ_Suggestions $this  Current SZ_Suggestions instance.
		 */
		return apply_filters( 'sz_suggestions_validate_args', true, $this );
	}
