 ...
 *
 * @see SZ_Media_Extractor::extract() Use this to extract media.
 */
class SZ_Media_Extractor {
	/**
	 * Media type.
	 *
	 * @since 2.3.0
	...
	public function extract( $richtext, $what_to_extract = self::ALL, $extra_args = array() ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $media           Extracted media. See {@link SZ_Media_Extractor::extract()} for format.
		 ...
		 */
		return apply_filters( 'sz_media_extractor_extract', $media, $richtext, $what_to_extract, $extra_args, $plaintext );
	}
	 ...
	 *
	 * You shouldn't need to use these directly; just use {@link SZ_Media_Extractor::extract()}.
	 */
	...
	protected function extract_links( $richtext, $plaintext, $extra_args = array() ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $data       Extracted links. See {@link SZ_Media_Extractor::extract_links()} for format.
		 * @param string $richtext   Content to parse.
		 * @param string $plaintext  Copy of $richtext without any markup.
		 * @param array  $extra_args Bespoke data for a particular extractor.
		 */
		return apply_filters( 'sz_media_extractor_links', $data, $richtext, $plaintext, $extra_args );
	}
	...
	protected function extract_mentions( $richtext, $plaintext, $extra_args = array() ) {
		...
		if ( sz_is_active( 'activity' ) ) {
			$mentions = sz_activity_find_mentions( $plaintext );
			if ( ! $mentions ) {
				$mentions = array();
			}
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $data       Extracted @mentions. See {@link SZ_Media_Extractor::extract_mentions()} for format.
		 * @param string $richtext   Content to parse.
		 * @param string $plaintext  Copy of $richtext without any markup.
		 * @param array  $extra_args Bespoke data for a particular extractor (optional).
		 */
		return apply_filters( 'sz_media_extractor_mentions', $data, $richtext, $plaintext, $extra_args );
	}
	...
	protected function extract_images( $richtext, $plaintext, $extra_args = array() ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $media      Extracted images. See {@link SZ_Media_Extractor::extract_images()} for format.
		 * @param string $richtext   Content to parse.
		 * @param string $plaintext  Copy of $richtext without any markup.
		 * @param array  $extra_args Bespoke data for a particular extractor.
		 */
		return apply_filters( 'sz_media_extractor_images', $media, $richtext, $plaintext, $extra_args );
	}
	...
	protected function extract_shortcodes( $richtext, $plaintext, $extra_args = array() ) {
		 ...
		 *                           See {@link SZ_Media_Extractor::extract_shortcodes()} for format.
		 * @param string $richtext   Content to parse.
		 * @param string $plaintext  Copy of $richtext without any markup.
		 * @param array  $extra_args Bespoke data for a particular extractor.
		 */
		return apply_filters( 'sz_media_extractor_shortcodes', $data, $richtext, $plaintext, $extra_args );
	}
	...
	protected function extract_embeds( $richtext, $plaintext, $extra_args = array() ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $data       Extracted embeds. See {@link SZ_Media_Extractor::extract_embeds()} for format.
		 * @param string $richtext   Content to parse.
		 * @param string $plaintext  Copy of $richtext without any markup.
		 * @param array  $extra_args Bespoke data for a particular extractor.
		 */
		return apply_filters( 'sz_media_extractor_embeds', $data, $richtext, $plaintext, $extra_args );
	}
	...
	protected function extract_audio( $richtext, $plaintext, $extra_args = array() ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $data       Extracted audio. See {@link SZ_Media_Extractor::extract_audio()} for format.
		 * @param string $richtext   Content to parse.
		 * @param string $plaintext  Copy of $richtext without any markup.
		 * @param array  $extra_args Bespoke data for a particular extractor.
		 */
		return apply_filters( 'sz_media_extractor_audio', $data, $richtext, $plaintext, $extra_args );
	}
	...
	protected function extract_video( $richtext, $plaintext, $extra_args = array() ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $data       Extracted videos. See {@link SZ_Media_Extractor::extract_videos()} for format.
		 * @param string $richtext   Content to parse.
		 * @param string $plaintext  Copy of $richtext without any markup.
		 * @param array  $extra_args Bespoke data for a particular extractor.
		 */
		return apply_filters( 'sz_media_extractor_videos', $data, $richtext, $plaintext, $extra_args );
	}
	...
	protected function extract_images_from_galleries( $richtext, $plaintext, $extra_args = array() ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $galleries_data Galleries. See {@link SZ_Media_Extractor::extract_images_from_galleries()}.
		 * @param string $richtext       Content to parse.
		 * @param string $plaintext      Copy of $richtext without any markup.
		 * @param array  $extra_args     Bespoke data for a particular extractor.
		 */
		return apply_filters( 'sz_media_extractor_galleries', $galleries_data, $richtext, $plaintext, $extra_args );
	}
	...
	protected function extract_images_from_featured_images( $richtext, $plaintext, $extra_args ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $image      Extracted images. See {@link SZ_Media_Extractor_Post::extract_images()} for format.
		 * @param string $richtext   Content to parse.
		 * @param string $plaintext  Copy of $richtext without any markup.
		 * @param array  $extra_args Bespoke data for a particular extractor.
		 */
		return apply_filters( 'sz_media_extractor_featured_images', $image, $richtext, $plaintext, $extra_args );
	}
	...
	protected function strip_markup( $richtext ) {
		 ...
		 */
		return apply_filters( 'sz_media_extractor_strip_markup', $plaintext, $richtext );
	}
}
