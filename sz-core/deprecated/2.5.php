 ...
 *                   Use the "sz_email_get_from" action instead.
 *
 * @return string The blog name for the root blog.
 */
function sz_core_email_from_name_filter() {
	 ...
	 */
	return apply_filters( 'sz_core_email_from_name_filter', sz_get_option( 'blogname', 'WordPress' ) );
}
 ...
 * @return mixed
 */
function sz_core_deprecated_email_filters( $value, $property, $transform, $email ) {
	...
	remove_filter( 'sz_email_get_property', 'sz_core_deprecated_email_filters', 20 );
	$email_type = $email->get( 'type' );
	$tokens     = $email->get( 'tokens' );
	add_filter( 'sz_email_get_property', 'sz_core_deprecated_email_filters', 20, 4 );
	...
	if ( $email_type === 'activity-comment' ) {
		if ( $property === 'to' ) {
			 ...
			 *
			 * @since 1.2.0
			 * @deprecated 2.5.0 Use the filters in SZ_Email.
			 *
			 * @param string $value
			 */
			$value = apply_filters( 'sz_activity_new_comment_notification_to', $value );
			 ...
			 */
			$value = apply_filters( 'sz_activity_new_comment_notification_subject', $value, $tokens['poster.name'] );
			 ...
			 *
			 * @since 1.2.0
			 * @deprecated 2.5.0 Use the filters in SZ_Email. $settings_link argument unset and deprecated.
			 *
			 ...
			 */
			$value = apply_filters( 'sz_activity_new_comment_notification_message', $value, $tokens['poster.name'], $tokens['content'], $tokens['thread.url'], '' );
		}
		...
		if ( $property === 'to' ) {
			 ...
			 * @param string $value
			 */
			$value = apply_filters( 'sz_activity_new_comment_notification_comment_author_to', $value );
			 ...
			 * @param string $poster_name Name of the person who made the comment.
			 */
			$value = apply_filters( 'sz_activity_new_comment_notification_comment_author_subject', $value, $tokens['poster.name'] );
			 ...
			 * @param string $thread_link   URL permalink for the activity thread.
			 */
			$value = apply_filters( 'sz_activity_new_comment_notification_comment_author_message', $value, $tokens['poster.name'], $tokens['content'], '', $tokens['thread.url'] );
		}
		...
		if ( $property === 'to' ) {
			 ...
			 * @param string $value
			 */
			$value = apply_filters( 'sz_activity_at_message_notification_to', $value );
			 ...
			 */
			$value = apply_filters( 'sz_activity_at_message_notification_subject', $value, $tokens['poster.name'] );
			 ...
			 * @param string $deprecated    Removed in 2.5; now an empty string.
			 */
			$value = apply_filters( 'sz_activity_at_message_notification_message', $value, $tokens['poster.name'], $tokens['content'], $tokens['mentioned.url'], '' );
		}
		...
		if ( is_multisite() ) {
			$registration_filters = array(
				'to'      => 'sz_core_activation_signup_user_notification_to',
				'subject' => 'sz_core_activation_signup_user_notification_subject',
				'content' => 'sz_core_activation_signup_user_notification_message',
			);
		} else {
			$registration_filters = array(
				'to'      => 'sz_core_signup_send_validation_email_to',
				'subject' => 'sz_core_signup_send_validation_email_subject',
				'content' => 'sz_core_signup_send_validation_email_message',
			);
		}
		...
		if ( $property === 'to' ) {
			 ...
			 *
			 * @since 1.2.0
			 * @deprecated 2.5.0 Use the filters in SZ_Email. $meta argument unset and deprecated.
			 *
		...
		if ( $property === 'to' ) {
			 ...
			 * @param array  $meta               Removed in 2.5; now an empty array.
			 */
			$value = apply_filters( 'sz_core_activation_signup_blog_notification_to', $value, $tokens['domain'], $tokens['path'], $tokens['title'], $tokens['recipient.username'], $tokens['recipient.email'], $tokens['key_blog'], array() );
			 ...
			 * @param array  $meta               Removed in 2.5; now an empty array.
			 */
			$value = apply_filters( 'sz_core_activation_signup_blog_notification_subject', $value, $tokens['domain'], $tokens['path'], $tokens['title'], $tokens['recipient.username'], $tokens['recipient.email'], $tokens['key_blog'], array() );
			 ...
			 * @param array  $meta               Removed in 2.5; now an empty array.
			 */
			$value = apply_filters( 'sz_core_activation_signup_blog_notification_message', $value, $tokens['domain'], $tokens['path'], $tokens['title'], $tokens['recipient.username'], $tokens['recipient.email'], $tokens['key_blog'], array() );
		}
		...
		if ( $property === 'to' ) {
			 ...
			 *
			 * @param string          $value Email notification subject text.
			 * @param SZ_Groups_Group $group Object holding the current group instance. Passed by reference.
			 */
		...
		if ( $property === 'to' ) {
			 ...
			 *
			 * @since 1.2.0
			 * @deprecated 2.5.0 Use the filters in SZ_Email. $ud argument unset and deprecated.
			 *
		...
		if ( $property === 'content' ) {
			 ...
			 *
			 * @since 2.1.0
			 * @deprecated 2.5.0 Use the filters in SZ_Email. $update_user argument unset and deprecated.
			 *
			 ...
			 * @param bool    $deprecated      Removed in 2.5; now a bool (false).
			 */
			$value = apply_filters( 'sz_new_user_email_content', $value, $tokens['recipient.email'], $tokens['old-user.email'], false );
		}
	}

	if ( $property === 'to' && $to_changed ) {
		// We normally apart $to, so we often need to rebuild it.
		array_unshift( $original_value, new SZ_Email_Recipient( $value, $recipient_name ) );
 ...
 * @return mixed
 */
function sz_core_deprecated_email_actions( $delivery_status, $email ) {
	$pre_2_5_emails = array(
		'activity-comment',
		'activity-comment-author',
		'core-user-registration',
	...
	remove_action( 'sz_send_email_success', 'sz_core_deprecated_email_actions', 20 );
	$email_content = $email->get( 'content' );
	$email_subject = $email->get( 'subject' );
	$email_type    = $email->get( 'type' );
	$tokens        = $email->get( 'tokens' );
	add_action( 'sz_send_email_success', 'sz_core_deprecated_email_actions', 20, 2 );
	...
	if ( $email_type === 'activity-comment' ) {
		 ...
		 *
		 * @since 1.5.0
		 * @deprecated 2.5.0 Use the filters in SZ_Email. $params argument unset and deprecated.
		 *
		 ...
		 */
		do_action( 'sz_activity_sent_reply_to_update_email', $tokens['original_activity.user_id'], $email_subject, $email_content, $tokens['comment.id'], $tokens['commenter.id'], array() );
		 ...
		 * @param array  $deprecated    Removed in 2.5; now an empty array.
		 */
		do_action( 'sz_activity_sent_reply_to_reply_email', $tokens['parent-comment-user.id'], $email_subject, $email_content, $tokens['comment.id'], $tokens['commenter.id'], array() );
		...
		if ( ! empty( $tokens['user.id'] ) ) {
			 ...
			 */
			do_action( 'sz_core_sent_user_validation_email', $email_subject, $email_content, $tokens['user.id'], $tokens['recipient.email'], $tokens['key'] );
			 ...
			 * @param array  $meta               Removed in 2.5; now an empty array.
			 */
			do_action( 'sz_core_sent_user_signup_email', sz_get_option( 'admin_email' ), $email_subject, $email_content, $tokens['recipient.username'], $tokens['recipient.email'], $tokens['key'], array() );
		}
		 ...
		 * @param array  $meta               Removed in 2.5; now an empty array.
		 */
		do_action( 'sz_core_sent_blog_signup_email', sz_get_option( 'admin_email' ), $email_subject, $email_content, $tokens['domain'], $tokens['path'], $tokens['title'], $tokens['recipient.username'], $tokens['recipient.email'], $tokens['key_blog'], array() );
		 ...
		 */
		do_action( 'sz_friends_sent_request_email', $tokens['friend.id'], $email_subject, $email_content, $tokens['friendship.id'], $tokens['initiator.id'] );
		 ...
		 * @param int    $friend_id     ID of the request recipient.
		 */
		do_action( 'sz_friends_sent_accepted_email', $tokens['initiator.id'], $email_subject, $email_content, $tokens['friendship.id'], $tokens['friend.id'] );
		 ...
		 */
		do_action( 'sz_groups_sent_invited_email', $tokens['inviter.id'], $email_subject, $email_content, $tokens['group'] );
		 ...
		 */
		do_action( 'sz_groups_sent_promoted_email', $tokens['user.id'], $email_subject, $email_content, $tokens['group.id'] );
		 ...
		 */
		do_action( 'sz_groups_sent_membership_request_email', $tokens['admin.id'], $email_subject, $email_content, $tokens['requesting-user.id'], $tokens['group.id'], $tokens['membership.id'] );
		 ...
		 */
		do_action( 'sz_groups_sent_membership_approved_email', $tokens['requesting-user.id'], $email_subject, $email_content, $tokens['group.id'] );
	}
}
add_action( 'sz_send_email_success', 'sz_core_deprecated_email_actions', 20, 2 );
 ...
 */
function sz_blogs_transition_activity_status( $new_status, $old_status, $comment ) {
	_deprecated_function( __FUNCTION__, '2.5.0', 'sz_activity_transition_post_type_comment_status()' );
	sz_activity_transition_post_type_comment_status( $new_status, $old_status, $comment );
}

/**
 * Record a new blog comment in the SportsZone activity stream.
 *
 ...
 */
function sz_blogs_record_comment( $comment_id, $is_approved = true ) {
	_deprecated_function( __FUNCTION__, '2.5.0', 'sz_activity_post_type_comment()' );
	sz_activity_post_type_comment( $comment_id, $is_approved );
}
 ...
 */
function sz_blogs_remove_comment( $comment_id ) {
	_deprecated_function( __FUNCTION__, '2.5.0', 'sz_activity_post_type_remove_comment()' );
	sz_activity_post_type_remove_comment( $comment_id );
}
