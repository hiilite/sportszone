 ...
 * @since 1.2.0
 */
function sz_core_set_avatar_constants() {

	$sz = sportszone();

	if ( !defined( 'SZ_AVATAR_THUMB_WIDTH' ) )
		define( 'SZ_AVATAR_THUMB_WIDTH', 50 );

	if ( !defined( 'SZ_AVATAR_THUMB_HEIGHT' ) )
		define( 'SZ_AVATAR_THUMB_HEIGHT', 50 );

	if ( !defined( 'SZ_AVATAR_FULL_WIDTH' ) )
		define( 'SZ_AVATAR_FULL_WIDTH', 150 );

	if ( !defined( 'SZ_AVATAR_FULL_HEIGHT' ) )
		define( 'SZ_AVATAR_FULL_HEIGHT', 150 );

	if ( !defined( 'SZ_AVATAR_ORIGINAL_MAX_WIDTH' ) )
		define( 'SZ_AVATAR_ORIGINAL_MAX_WIDTH', 450 );

	if ( !defined( 'SZ_AVATAR_ORIGINAL_MAX_FILESIZE' ) ) {
		define( 'SZ_AVATAR_ORIGINAL_MAX_FILESIZE', sz_attachments_get_max_upload_file_size( 'avatar' ) );
	}

	if ( ! defined( 'SZ_SHOW_AVATARS' ) ) {
		define( 'SZ_SHOW_AVATARS', sz_get_option( 'show_avatars' ) );
	}
}
add_action( 'sz_init', 'sz_core_set_avatar_constants', 3 );
 ...
 * @since 1.5.0
 */
function sz_core_set_avatar_globals() {
	$sz = sportszone();

	$sz->avatar        = new stdClass;
	$sz->avatar->thumb = new stdClass;
	$sz->avatar->full  = new stdClass;

	// Dimensions.
	$sz->avatar->thumb->width  = SZ_AVATAR_THUMB_WIDTH;
	$sz->avatar->thumb->height = SZ_AVATAR_THUMB_HEIGHT;
	$sz->avatar->full->width   = SZ_AVATAR_FULL_WIDTH;
	$sz->avatar->full->height  = SZ_AVATAR_FULL_HEIGHT;

	// Upload maximums.
	$sz->avatar->original_max_width    = SZ_AVATAR_ORIGINAL_MAX_WIDTH;
	$sz->avatar->original_max_filesize = SZ_AVATAR_ORIGINAL_MAX_FILESIZE;

	// Defaults.
	$sz->avatar->thumb->default = sz_core_avatar_default_thumb();
	$sz->avatar->full->default  = sz_core_avatar_default();

	// These have to be set on page load in order to avoid infinite filter loops at runtime.
	$sz->avatar->upload_path = sz_core_avatar_upload_path();
	$sz->avatar->url = sz_core_avatar_url();

	// Cache the root blog's show_avatars setting, to avoid unnecessary
	// calls to switch_to_blog().
	$sz->avatar->show_avatars = (bool) SZ_SHOW_AVATARS;

	// Backpat for pre-1.5.
	if ( ! defined( 'SZ_AVATAR_UPLOAD_PATH' ) )
		define( 'SZ_AVATAR_UPLOAD_PATH', $sz->avatar->upload_path );

	// Backpat for pre-1.5.
	if ( ! defined( 'SZ_AVATAR_URL' ) )
		define( 'SZ_AVATAR_URL', $sz->avatar->url );
	 ...
	 * @since 1.5.0
	 */
	do_action( 'sz_core_set_avatar_globals' );
}
add_action( 'sz_setup_globals', 'sz_core_set_avatar_globals' );

/**
 * Get an avatar for a SportsZone object.
 *
 ...
 * locally:
 *    add_filter( 'sz_core_fetch_avatar_no_grav', '__return_true' );
 *
 * @since 1.1.0
 ...
 *     will, if not provided, be auto-detected by sz_core_fetch_avatar(). This
 ...
 *                                   avatar. SportsZone natively supports three options: 'user',
 ...
 *                                   {@link sz_get_activity_avatar()} for an example. Default: ''.
 ...
 *                                   fallbacks globally, see the 'sz_core_fetch_avatar_no_grav' filter.
...
function sz_core_fetch_avatar( $args = '' ) {
	...
	if ( empty( $params['item_id'] ) ) {
		 ...
		 */
		$params['item_id'] = apply_filters( 'sz_core_avatar_item_id', $params['item_id'], $params['object'], $params );
	...
	if ( empty( $params['avatar_dir'] ) ) {
		 ...
		 * @param array  $params Array of parameters for the request.
		 */
		$params['avatar_dir'] = apply_filters( 'sz_core_avatar_dir', $params['avatar_dir'], $params['object'], $params );
	...
	if ( false !== strpos( $params['alt'], '%s' ) || false !== strpos( $params['alt'], '%1$s' ) ) {
		 ...
		 * @param array  $params Array of parameters for the request.
		 */
		$item_name = apply_filters( 'sz_core_avatar_alt', $item_name, $params['item_id'], $params['object'], $params );
	 ...
	 * @param array  $params Array of parameters for the request.
	 */
	$params['title'] = apply_filters( 'sz_core_avatar_title', $params['title'], $params['item_id'], $params['object'], $params );
	...
	if ( false !== $params['width'] ) {
		...
		$params['width'] = sz_core_avatar_thumb_width();
	} else {
		$params['width'] = sz_core_avatar_full_width();
	}
	...
	if ( false !== $params['height'] ) {
		...
		$params['height'] = sz_core_avatar_thumb_height();
	} else {
		$params['height'] = sz_core_avatar_full_height();
	}
	 ...
	 * @param array        $params Array of parameters for the request.
	 */
	$params['class'] = apply_filters( 'sz_core_avatar_class', $params['class'], $params['item_id'], $params['object'], $params );
	...
	$avatar_loc->path  = trailingslashit( sz_core_avatar_upload_path() );
	$avatar_loc->url   = trailingslashit( sz_core_avatar_url() );
	 ...
	 */
	$avatar_folder_url = apply_filters( 'sz_core_avatar_folder_url', ( $avatar_loc->url  . $avatar_loc->dir . $params['item_id'] ), $params['item_id'], $params['object'], $params['avatar_dir'] );
	 ...
	 * @param string $value Subdirectory where the requested avatar should be found.
	 */
	$avatar_folder_dir = apply_filters( 'sz_core_avatar_folder_dir', ( $avatar_loc->path . $avatar_loc->dir . $params['item_id'] ), $params['item_id'], $params['object'], $params['avatar_dir'] );
	...
	if ( file_exists( $avatar_folder_dir ) ) {
		...
		if ( isset( $avatar_url ) ) {
			...
			if ( true === $params['html'] ) {
				 ...
				 */
				return apply_filters( 'sz_core_fetch_avatar', '<img src="' . $avatar_url . '"' . $html_class . $html_css_id  . $html_width . $html_height . $html_alt . $html_title . $extra_attr . ' />', $params, $params['item_id'], $params['avatar_dir'], $html_css_id, $html_width, $html_height, $avatar_folder_url, $avatar_folder_dir );
			...
			// ...or only the URL
				 ...
				 * @param array  $params     Array of parameters for the request.
				 */
				return apply_filters( 'sz_core_fetch_avatar_url', $avatar_url, $params );
			}
		}
	}
	 ...
	 */
	if ( ! apply_filters( 'sz_core_fetch_avatar_no_grav', $params['no_grav'], $params ) ) {

		// Set gravatar type.
		if ( empty( $sz->grav_default->{$params['object']} ) ) {
			$default_grav = 'wavatar';
		} elseif ( 'mystery' == $sz->grav_default->{$params['object']} ) {
			 ...
			 */
			$default_grav = apply_filters( 'sz_core_mysteryman_src', 'mm', $params['width'] );
		} else {
			$default_grav = $sz->grav_default->{$params['object']};
		}

		// Set gravatar object.
		if ( empty( $params['email'] ) ) {
			if ( 'user' == $params['object'] ) {
				$params['email'] = sz_core_get_user_email( $params['item_id'] );
			} elseif ( 'group' == $params['object'] || 'blog' == $params['object'] ) {
				$params['email'] = $params['item_id'] . '-' . $params['object'] . '@' . sz_get_root_domain();
			}
		}
		 ...
		 */
		$params['email'] = apply_filters( 'sz_core_gravatar_email', $params['email'], $params['item_id'], $params['object'] );
		 ...
		 */
		$gravatar = apply_filters( 'sz_gravatar_url', '//www.gravatar.com/avatar/' );
		 ...
		 */
		$default_grav = apply_filters( 'sz_core_avatar_default', $default_grav, $params );
	...
	// No avatar was found, and we've been told not to use a gravatar.
		 ...
		 * @param array  $params Array of parameters for the avatar request.
		 */
		$gravatar = apply_filters( 'sz_core_default_avatar_' . $params['object'], sz_core_avatar_default( 'local', $params ), $params );
	}

	if ( true === $params['html'] ) {

		/** This filter is documented in sz-core/sz-core-avatars.php */
		return apply_filters( 'sz_core_fetch_avatar', '<img src="' . $gravatar . '"' . $html_css_id . $html_class . $html_width . $html_height . $html_alt . $html_title . $extra_attr . ' />', $params, $params['item_id'], $params['avatar_dir'], $html_css_id, $html_width, $html_height, $avatar_folder_url, $avatar_folder_dir );
	} else {

		/** This filter is documented in sz-core/sz-core-avatars.php */
		return apply_filters( 'sz_core_fetch_avatar_url', $gravatar, $params );
	}
}
 ...
 */
function sz_core_delete_existing_avatar( $args = '' ) {
	 ...
	 * }
	 */
	if ( ! apply_filters( 'sz_core_pre_delete_existing_avatar', true, $args ) ) {
		return true;
	}
	...
	if ( empty( $item_id ) ) {
		...
		/** This filter is documented in sz-core/sz-core-avatars.php */
		$item_id = apply_filters( 'sz_core_avatar_item_id', $item_id, $object );
	...
	if ( empty( $avatar_dir ) ) {
		...
		/** This filter is documented in sz-core/sz-core-avatars.php */
		$avatar_dir = apply_filters( 'sz_core_avatar_dir', $avatar_dir, $object );

		if ( !$avatar_dir ) return false;
	}

	/** This filter is documented in sz-core/sz-core-avatars.php */
	$avatar_folder_dir = apply_filters( 'sz_core_avatar_folder_dir', sz_core_avatar_upload_path() . '/' . $avatar_dir . '/' . $item_id, $item_id, $object, $avatar_dir );
	 ...
	 */
	do_action( 'sz_core_delete_existing_avatar', $args );
 ...
 */
function sz_avatar_ajax_delete() {
	...
	$nonce = 'sz_delete_avatar_link';
	if ( 'group' === $avatar_data['object'] ) {
		$nonce = 'sz_group_avatar_delete';
	}
	...
	// Capability check.
	if ( ! sz_attachments_current_user_can( 'edit_avatar', $avatar_data ) ) {
		wp_send_json_error();
	}

	// Handle delete.
	if ( sz_core_delete_existing_avatar( array( 'item_id' => $avatar_data['item_id'], 'object' => $avatar_data['object'] ) ) ) {
		$return = array(
			'avatar' => html_entity_decode( sz_core_fetch_avatar( array(
...
add_action( 'wp_ajax_sz_avatar_delete', 'sz_avatar_ajax_delete' );

/**
 * Handle avatar uploading.
 *
 * The functions starts off by checking that the file has been uploaded
 * properly using sz_core_check_avatar_upload(). It then checks that the file
 ...
 * @since 1.1.0
 *
 * @see sz_core_check_avatar_upload()
 * @see sz_core_check_avatar_type()
 *
 * @param array  $file              The appropriate entry the from $_FILES superglobal.
 * @param string $upload_dir_filter A filter to be applied to 'upload_dir'.
 * @return bool True on success, false on failure.
 */
function sz_core_avatar_handle_upload( $file, $upload_dir_filter ) {
	 ...
	 */
	if ( ! apply_filters( 'sz_core_pre_avatar_handle_upload', true, $file, $upload_dir_filter ) ) {
		return true;
	}

	// Setup some variables.
	$sz          = sportszone();
	$upload_path = sz_core_avatar_upload_path();
	...
	if ( isset( $sz->avatar_admin->ui_available_width ) ) {
		$ui_available_width = $sz->avatar_admin->ui_available_width;
	}

	// Maybe resize.
	$sz->avatar_admin->resized = $avatar_attachment->shrink( $sz->avatar_admin->original['file'], $ui_available_width );
	$sz->avatar_admin->image   = new stdClass();

	// We only want to handle one image after resize.
	if ( empty( $sz->avatar_admin->resized ) ) {
		$sz->avatar_admin->image->file = $sz->avatar_admin->original['file'];
		$sz->avatar_admin->image->dir  = str_replace( $upload_path, '', $sz->avatar_admin->original['file'] );
	} else {
		$sz->avatar_admin->image->file = $sz->avatar_admin->resized['path'];
		$sz->avatar_admin->image->dir  = str_replace( $upload_path, '', $sz->avatar_admin->resized['path'] );
		@unlink( $sz->avatar_admin->original['file'] );
	}

	// Check for WP_Error on what should be an image.
	if ( is_wp_error( $sz->avatar_admin->image->dir ) ) {
		sz_core_add_message( sprintf( __( 'Upload failed! Error was: %s', 'sportszone' ), $sz->avatar_admin->image->dir->get_error_message() ), 'error' );
		return false;
	}

	// If the uploaded image is smaller than the "full" dimensions, throw a warning.
	if ( $avatar_attachment->is_too_small( $sz->avatar_admin->image->file ) ) {
		sz_core_add_message( sprintf( __( 'You have selected an image that is smaller than recommended. For best results, upload a picture larger than %d x %d pixels.', 'sportszone' ), sz_core_avatar_full_width(), sz_core_avatar_full_height() ), 'error' );
	}

	// Set the url value for the image.
	$sz->avatar_admin->image->url = sz_core_avatar_url() . $sz->avatar_admin->image->dir;
 ...
 *                     error message otherwise.
 */
function sz_avatar_ajax_upload() {
	if ( ! sz_is_post_request() ) {
		wp_die();
	}
	...
	// Capability check.
	if ( ! sz_attachments_current_user_can( 'edit_avatar', $sz_params ) ) {
		sz_attachments_json_response( false, $is_html4 );
	}
	...
	if ( 'user' === $sz_params['object'] && sz_is_active( 'xprofile' ) ) {
		$sz_params['upload_dir_filter'] = 'xprofile_avatar_upload_dir';
		...
		$sz_params['upload_dir_filter'] = 'groups_avatar_upload_dir';
		 ...
		 */
		$sz_params = apply_filters( 'sz_core_avatar_ajax_upload_params', $sz_params );
	}
	...
	if ( isset( $sz_params['ui_available_width'] ) ) {
		$sz->avatar_admin->ui_available_width =  (int) $sz_params['ui_available_width'];
	}

	// Upload the avatar.
	$avatar = sz_core_avatar_handle_upload( $_FILES, $sz_params['upload_dir_filter'] );
	...
	if ( empty( $sz->avatar_admin->image->file ) ) {
		sz_attachments_json_response( false, $is_html4 );
	}

	$uploaded_image = @getimagesize( $sz->avatar_admin->image->file );
	...
	sz_attachments_json_response( true, $is_html4, array(
		'name'      => $name,
		'url'       => $sz->avatar_admin->image->url,
		'width'     => $uploaded_image[0],
		'height'    => $uploaded_image[1],
		'feedback'  => $feedback_message,
	) );
}
add_action( 'wp_ajax_sz_avatar_upload', 'sz_avatar_ajax_upload' );
 ...
 * @return bool True on success, false on failure.
 */
function sz_avatar_handle_capture( $data = '', $item_id = 0 ) {
	 ...
	 * @param int    $item_id Item to associate.
	 */
	if ( ! apply_filters( 'sz_avatar_pre_handle_capture', true, $data, $item_id ) ) {
		return true;
	}

	$avatar_dir = sz_core_avatar_upload_path() . '/avatars';
	 ...
	 */
	$avatar_folder_dir = apply_filters( 'sz_core_avatar_folder_dir', $avatar_dir . '/' . $item_id, $item_id, 'user', 'avatars' );
	...
	if ( file_put_contents( $original_file, $data ) ) {
		$avatar_to_crop = str_replace( sz_core_avatar_upload_path(), '', $original_file );

		// Crop to default values.
		$crop_args = array( 'item_id' => $item_id, 'original_file' => $avatar_to_crop, 'crop_x' => 0, 'crop_y' => 0 );

		return sz_core_avatar_handle_crop( $crop_args );
	} else {
		return false;
	}
}
 ...
 *                                      as retrieved by sz_core_avatar_full_width().
 *     @type int         $crop_h        Crop height. Default: the global 'full' avatar height,
 *                                      as retrieved by sz_core_avatar_full_height().
 ...
 * @return bool True on success, false on failure.
 */
function sz_core_avatar_handle_crop( $args = '' ) {
	...
	$r = wp_parse_args( $args, array(
		...
		'item_id'       => false,
		'original_file' => false,
		'crop_w'        => sz_core_avatar_full_width(),
		'crop_h'        => sz_core_avatar_full_height(),
		'crop_x'        => 0,
		'crop_y'        => 0
	) );
	 ...
	 */
	if ( ! apply_filters( 'sz_core_pre_avatar_handle_crop', true, $r ) ) {
		return true;
	}
 ...
 *                     error message otherwise.
 */
function sz_avatar_ajax_set() {
	if ( ! sz_is_post_request() ) {
		wp_send_json_error();
	}

	// Check the nonce.
	check_admin_referer( 'sz_avatar_cropstore', 'nonce' );
	...
	if ( ! empty( $avatar_data['type'] ) && 'camera' === $avatar_data['type'] && 'user' === $avatar_data['object'] ) {
		...
		if ( ! sz_avatar_handle_capture( $webcam_avatar, $avatar_data['item_id'] ) ) {
			wp_send_json_error( array(
				'feedback_code' => 1
			) );
	...
	$original_file = str_replace( sz_core_avatar_url(), '', $avatar_data['original_file'] );
	...
	// Handle crop.
	if ( sz_core_avatar_handle_crop( $r ) ) {
		$return = array(
			'avatar' => html_entity_decode( sz_core_fetch_avatar( array(
				'object'  => $avatar_data['object'],
				'item_id' => $avatar_data['item_id'],
		...
		if ( 'user' === $avatar_data['object'] ) {
			/** This action is documented in sz-core/sz-core-avatars.php */
...
add_action( 'wp_ajax_sz_avatar_set', 'sz_avatar_ajax_set' );

/**
 * Filter {@link get_avatar_url()} to use the SportsZone user avatar URL.
 *
 * @since 2.9.0
 *
 ...
 * @return string
 */
function sz_core_get_avatar_data_url_filter( $retval, $id_or_email, $args ) {
	$user = null;
	...
	// Set SportsZone-specific avatar args.
	$args['item_id'] = $user->ID;
	$args['html']    = false;

	// Use the 'full' type if size is larger than BP's thumb width.
	if ( (int) $args['size'] > sz_core_avatar_thumb_width() ) {
		$args['type'] = 'full';
	}

	// Get the SportsZone avatar URL.
	if ( $sz_avatar = sz_core_fetch_avatar( $args ) ) {
		return $sz_avatar;
	}

	return $retval;
}
add_filter( 'get_avatar_url', 'sz_core_get_avatar_data_url_filter', 10, 3 );
 ...
 */
function sz_core_check_avatar_upload( $file ) {
 ...
 */
function sz_core_check_avatar_size( $file ) {
	if ( $file['file']['size'] > sz_core_avatar_original_max_filesize() )
		return false;
 ...
 * @return array
 */
function sz_core_get_allowed_avatar_types() {
	$allowed_types = sz_attachments_get_allowed_types( 'avatar' );
	 ...
	 */
	$avatar_types = (array) apply_filters( 'sz_core_get_allowed_avatar_types', $allowed_types );
 ...
 * @return array
 */
function sz_core_get_allowed_avatar_mimes() {
	$allowed_types  = sz_core_get_allowed_avatar_types();

	return sz_attachments_get_allowed_mimes( 'avatar', $allowed_types );
}
 ...
 */
function sz_core_check_avatar_type( $file ) {
	return sz_attachments_check_filetype( $file['file']['tmp_name'], $file['file']['name'], sz_core_get_allowed_avatar_mimes() );
}
 ...
 */
function sz_core_get_upload_dir( $type = 'upload_path' ) {
	$sz = sportszone();

	switch ( $type ) {
		case 'upload_path' :
			$constant = 'SZ_AVATAR_UPLOAD_PATH';
			$key      = 'basedir';

			break;

		case 'url' :
			$constant = 'SZ_AVATAR_URL';
			$key      = 'baseurl';
	...
	if ( isset( $sz->avatar->$type ) ) {
		$retval = $sz->avatar->$type;
	} else {
		...
		if ( defined( $constant ) ) {
			...
			if ( ! empty( $sz->avatar->upload_dir ) ) {
				$upload_dir = $sz->avatar->upload_dir;
			...
			// No cache, so query for it.
				...
				$sz->avatar->upload_dir = $upload_dir;
			}
 ...
 */
function sz_core_avatar_upload_path() {
	 ...
	 */
	return apply_filters( 'sz_core_avatar_upload_path', sz_core_get_upload_dir() );
}
...
function sz_core_avatar_url() {
	 ...
	 */
	return apply_filters( 'sz_core_avatar_url', sz_core_get_upload_dir( 'url' ) );
}
 ...
 */
function sz_get_user_has_avatar( $user_id = 0 ) {

	if ( empty( $user_id ) )
		$user_id = sz_displayed_user_id();

	$retval = false;
	if ( sz_core_fetch_avatar( array( 'item_id' => $user_id, 'no_grav' => true, 'html' => false, 'type' => 'full' ) ) != sz_core_avatar_default( 'local' ) )
		$retval = true;
	 ...
	 */
	return (bool) apply_filters( 'sz_get_user_has_avatar', $retval, $user_id );
}
 ...
 */
function sz_core_avatar_dimension( $type = 'thumb', $h_or_w = 'height' ) {
	$sz  = sportszone();
	$dim = isset( $sz->avatar->{$type}->{$h_or_w} ) ? (int) $sz->avatar->{$type}->{$h_or_w} : false;
	 ...
	 */
	return apply_filters( 'sz_core_avatar_dimension', $dim, $type, $h_or_w );
}
 ...
 */
function sz_core_avatar_thumb_width() {
	 ...
	 */
	return apply_filters( 'sz_core_avatar_thumb_width', sz_core_avatar_dimension( 'thumb', 'width' ) );
}
 ...
 */
function sz_core_avatar_thumb_height() {
	 ...
	 */
	return apply_filters( 'sz_core_avatar_thumb_height', sz_core_avatar_dimension( 'thumb', 'height' ) );
}
 ...
 */
function sz_core_avatar_full_width() {
	 ...
	 */
	return apply_filters( 'sz_core_avatar_full_width', sz_core_avatar_dimension( 'full', 'width' ) );
}
 ...
 */
function sz_core_avatar_full_height() {
	 ...
	 */
	return apply_filters( 'sz_core_avatar_full_height', sz_core_avatar_dimension( 'full', 'height' ) );
}
 ...
 */
function sz_core_avatar_original_max_width() {
	 ...
	 */
	return apply_filters( 'sz_core_avatar_original_max_width', (int) sportszone()->avatar->original_max_width );
}
 ...
 */
function sz_core_avatar_original_max_filesize() {
	 ...
	 */
	return apply_filters( 'sz_core_avatar_original_max_filesize', (int) sportszone()->avatar->original_max_filesize );
}
 ...
 * @param array  $params Parameters passed to sz_core_fetch_avatar().
 * @return string The URL of the default avatar.
 */
function sz_core_avatar_default( $type = 'gravatar', $params = array() ) {
	// Local override.
	if ( defined( 'SZ_AVATAR_DEFAULT' ) ) {
		$avatar = SZ_AVATAR_DEFAULT;

	// Use the local default image.
	} elseif ( 'local' === $type ) {
		$size = '';
		if (
			( isset( $params['type'] ) && 'thumb' === $params['type'] && sz_core_avatar_thumb_width() <= 50 ) ||
		...
		$avatar = sportszone()->plugin_url . "sz-core/images/mystery-man{$size}.jpg";
	 ...
	 * @param array  $params Params provided to sz_core_fetch_avatar().
	 */
	return apply_filters( 'sz_core_avatar_default', $avatar, $params );
}

/**
 * Get the URL of the 'thumb' default avatar.
 *
 * Uses Gravatar's mystery-person avatar, unless SZ_AVATAR_DEFAULT_THUMB has been
 * defined.
 *
 * @since 1.5.0
 ...
 */
function sz_core_avatar_default_thumb( $type = 'gravatar', $params = array() ) {
	// Local override.
	if ( defined( 'SZ_AVATAR_DEFAULT_THUMB' ) ) {
		$avatar = SZ_AVATAR_DEFAULT_THUMB;

	// Use the local default image.
	} elseif ( 'local' === $type ) {
		$avatar = sportszone()->plugin_url . 'sz-core/images/mystery-man-50.jpg';

	// Use Gravatar's mystery person as fallback.
	} else {
		$avatar = '//www.gravatar.com/avatar/00000000000000000000000000000000?d=mm&amp;s=' . sz_core_avatar_thumb_width();
	}
	 ...
	 *
	 * @param string $avatar URL of the default avatar.
	 * @param string $params Params provided to sz_core_fetch_avatar().
	 */
	return apply_filters( 'sz_core_avatar_thumb', $avatar, $params );
}
 ...
 */
function sz_core_avatar_reset_query( $posts_query = null ) {
	$reset_w = false;

	// Group's avatar edit screen.
	if ( sz_is_group_admin_page() ) {
		$reset_w = sz_is_group_admin_screen( 'group-avatar' );
	...
	// Group's avatar create screen.
		 ...
		 * as it's not set yet
		 */
		$reset_w = 'group-avatar' === sz_action_variable( 1 );

	// User's change avatar screen.
	} else {
		$reset_w = sz_is_user_change_avatar();
	}
...
add_action( 'sz_parse_query', 'sz_core_avatar_reset_query', 10, 1 );
 ...
 */
function sz_avatar_is_front_edit() {
	$retval = false;
	...
	if ( sz_is_user_change_avatar() && 'crop-image' !== sz_get_avatar_admin_step() ) {
		$retval = ! sz_core_get_root_option( 'sz-disable-avatar-uploads' );
	}

	if ( sz_is_active( 'groups' ) ) {
		// Group creation.
		if ( sz_is_group_create() && sz_is_group_creation_step( 'group-avatar' ) && 'crop-image' !== sz_get_avatar_admin_step() ) {
			$retval = ! sz_disable_group_avatar_uploads();

		// Group Manage.
		} elseif ( sz_is_group_admin_page() && sz_is_group_admin_screen( 'group-avatar' ) && 'crop-image' !== sz_get_avatar_admin_step() ) {
			$retval = ! sz_disable_group_avatar_uploads();
		}
	}
	 ...
	 */
	return apply_filters( 'sz_avatar_is_front_edit', $retval );
}
...
function sz_avatar_use_webcam() {
	 ...
	 */
	return apply_filters( 'sz_avatar_use_webcam', true );
}
 ...
 * @since 2.3.0
 */
function sz_avatar_get_templates() {
	if ( ! sz_avatar_is_front_edit() ) {
		return;
	}

	sz_attachments_get_template_part( 'avatars/index' );
}
 ...
 * @since 2.3.0
 */
function sz_avatar_template_check() {
	if ( ! sz_avatar_is_front_edit() ) {
		return;
	}

	if ( ! did_action( 'sz_attachments_avatar_check_template' ) ) {
		sz_attachments_get_template_part( 'avatars/index' );
	}
}
