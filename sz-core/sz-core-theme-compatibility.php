<?php
/**
 * SportsZone Core Theme Compatibility.
 *
 * @package SportsZone
 ...
 * to replace the_content() with the appropriate SportsZone content.
 *
 * To do this, SportsZone does several direct manipulations of global variables
 ...
 */
function sz_setup_theme_compat( $theme = '' ) {
	$sz = sportszone();

	// Make sure theme package is available, set to default if not.
	if ( ! isset( $sz->theme_compat->packages[$theme] ) || ! is_a( $sz->theme_compat->packages[$theme], 'SZ_Theme_Compat' ) ) {
		$theme = 'legacy';
	}

	// Set the active theme compat theme.
	$sz->theme_compat->theme = $sz->theme_compat->packages[$theme];
}
 ...
 * default and include their own SportsZone compatibility layers for their themes.
 *
 * @since 1.7.0
 *
 * @return string ID of the theme package in use.
 */
function sz_get_theme_compat_id() {
	 ...
	 */
	return apply_filters( 'sz_get_theme_compat_id', sportszone()->theme_compat->theme->id );
}
 ...
 */
function sz_get_theme_compat_name() {
	 ...
	 */
	return apply_filters( 'sz_get_theme_compat_name', sportszone()->theme_compat->theme->name );
}
 ...
 */
function sz_get_theme_compat_version() {
	 ...
	 */
	return apply_filters( 'sz_get_theme_compat_version', sportszone()->theme_compat->theme->version );
}
 ...
 * their own SportsZone compatibility layers for their themes.
 *
 * @since 1.7.0
 *
 * @return string The absolute path of the theme package currently in use.
 */
function sz_get_theme_compat_dir() {
	 ...
	 */
	return apply_filters( 'sz_get_theme_compat_dir', sportszone()->theme_compat->theme->dir );
}
 ...
 * the default and include their own SportsZone compatibility layers for their
 * themes.
 *
 * @since 1.7.0
 *
 ...
 */
function sz_get_theme_compat_url() {
	 ...
	 */
	return apply_filters( 'sz_get_theme_compat_url', sportszone()->theme_compat->theme->url );
}
 ...
 * do so using sz_detect_theme_compat_with_current_theme().
 *
 * @since 1.9.0
 *
 * @return bool True if the current theme needs theme compatibility.
 */
function sz_use_theme_compat_with_current_theme() {
	if ( ! isset( sportszone()->theme_compat->use_with_current_theme ) ) {
		sz_detect_theme_compat_with_current_theme();
	}
	 ...
	 */
	return apply_filters( 'sz_use_theme_compat_with_current_theme', sportszone()->theme_compat->use_with_current_theme );
}
 ...
 * 1) It declares BP support with add_theme_support( 'sportszone' )
 * 2) It is sz-default, or a child theme of sz-default
 * 3) A legacy template is found at members/members-loop.php. This is a
 *    fallback check for themes that were derived from sz-default, and have
 ...
 * @return bool True if the current theme needs theme compatibility.
 */
function sz_detect_theme_compat_with_current_theme() {
	if ( isset( sportszone()->theme_compat->use_with_current_theme ) ) {
		return sportszone()->theme_compat->use_with_current_theme;
	}

	// Theme compat enabled by default.
	$theme_compat = true;

	// If the theme supports 'sportszone', bail.
	if ( current_theme_supports( 'sportszone' ) ) {
		$theme_compat = false;

	// If the theme doesn't support BP, do some additional checks.
	} else {
		// Bail if theme is a derivative of sz-default.
		if ( in_array( 'sz-default', array( get_template(), get_stylesheet() ) ) ) {
			$theme_compat = false;

		// Brute-force check for a BP template.
		// Examples are clones of sz-default.
		} elseif ( locate_template( 'members/members-loop.php', false, false ) ) {
			$theme_compat = false;
		}
	}

	// Set a flag in the sportszone() singleton so we don't have to run this again.
	sportszone()->theme_compat->use_with_current_theme = $theme_compat;
 ...
 */
function sz_is_theme_compat_active() {
	$sz = sportszone();

	if ( empty( $sz->theme_compat->active ) ) {
		return false;
	}

	return $sz->theme_compat->active;
}
 ...
 */
function sz_set_theme_compat_active( $set = true ) {
	sportszone()->theme_compat->active = $set;

	return (bool) sportszone()->theme_compat->active;
}
 ...
 */
function sz_set_theme_compat_templates( $templates = array() ) {
	sportszone()->theme_compat->templates = $templates;

	return sportszone()->theme_compat->templates;
}
 ...
 */
function sz_set_theme_compat_template( $template = '' ) {
	sportszone()->theme_compat->template = $template;

	return sportszone()->theme_compat->template;
}
 ...
 * if SportsZone was able to find a more appropriate template.
 *
 * @since 1.7.0
 *
 ...
 */
function sz_set_theme_compat_original_template( $template = '' ) {
	sportszone()->theme_compat->original_template = $template;

	return sportszone()->theme_compat->original_template;
}
 ...
 */
function sz_set_theme_compat_feature( $theme_id, $feature = array() ) {
	...
	$theme_compat_theme = $sz->theme_compat->theme;

	// Bail if the Theme Compat theme is not in use.
	if ( $theme_id !== sz_get_theme_compat_id() ) {
		return;
	}
	...
	if ( isset( $features[ $feature['name'] ]->components ) ) {
		...
		foreach ( (array) $features[ $feature['name'] ]->components as $component ) {
			...
			if ( isset( $sz->{$component} ) ) {
				if ( isset( $sz->{$component}->features ) ) {
					$sz->{$component}->features[] = $feature['name'];
				} else {
					$sz->{$component}->features = array( $feature['name'] );
				}
			}
		}
	}
 ...
 */
function sz_get_theme_compat_feature( $feature = '' ) {
	// Get current theme compat theme.
	$theme_compat_theme = sportszone()->theme_compat->theme;
 ...
 *
 * Note: BP Legacy's sportszone-functions.php is not loaded in WP Administration
 * as it's loaded using sz_locate_template(). That's why this function is here.
 *
 * @since 2.4.0
 *
 * @global string $content_width the content width of the theme
 */
function sz_register_theme_compat_default_features() {
	global $content_width;

	// Do not set up default features on deactivation.
	if ( sz_is_deactivation() ) {
		return;
	}
	...
	if ( 'legacy' !== sz_get_theme_compat_id() ) {
		return;
	}
	 ...
	 * than the width used by SportsZone, so we need to manually set the
	 * content width for the concerned themes.
	 *
	 * Example: array( stylesheet => content width used by SportsZone )
	 */
	$sz_content_widths = array(
		'twentyfifteen'  => 1300,
		'twentyfourteen' => 955,
		'twentythirteen' => 890,
	);
	...
	// Default values.
	$sz_content_width = (int) $content_width;
	$sz_handle        = 'sz-legacy-css';

	// Specific to themes having companion stylesheets.
	if ( isset( $sz_content_widths[ $theme_handle ] ) ) {
		$sz_content_width = $sz_content_widths[ $theme_handle ];
		$sz_handle        = 'sz-' . $theme_handle;
	}
	...
	sz_set_theme_compat_feature( 'legacy', array(
		'name'     => 'cover_image',
		'settings' => array(
 ...
 */
function sz_is_theme_compat_original_template( $template = '' ) {
	$sz = sportszone();

	if ( empty( $sz->theme_compat->original_template ) ) {
		return false;
	}

	return (bool) ( $sz->theme_compat->original_template == $template );
}

/**
 * Register a new SportsZone theme package in the active theme packages array.
 *
 * For an example of how this function is used, see:
 * {@link SportsZone::register_theme_packages()}.
 *
 * @since 1.7.0
 *
 * @see SZ_Theme_Compat for a description of the $theme parameter arguments.
 *
 * @param array $theme    See {@link SZ_Theme_Compat}.
 * @param bool  $override If true, overrides whatever package is currently set.
 *                        Default: true.
 */
function sz_register_theme_package( $theme = array(), $override = true ) {

	// Create new SZ_Theme_Compat object from the $theme array.
	if ( is_array( $theme ) ) {
		$theme = new SZ_Theme_Compat( $theme );
	}

	// Bail if $theme isn't a proper object.
	if ( ! is_a( $theme, 'SZ_Theme_Compat' ) ) {
		return;
	}
	...
	if ( empty( $sz->theme_compat->packages[$theme->id] ) || ( true === $override ) ) {
		$sz->theme_compat->packages[$theme->id] = $theme;
	}
}
 ...
 */
function sz_theme_compat_reset_post( $args = array() ) {
	...
	sz_set_theme_compat_active( true );

	// If we are in theme compat, we don't need the 'Edit' post link.
	add_filter( 'get_edit_post_link', 'sz_core_filter_edit_post_link', 10, 2 );
}

/**
 * Reset main query vars and filter 'the_content' to output a SportsZone template part as needed.
 *
 * @since 1.7.0
 *
 ...
 */
function sz_template_include_theme_compat( $template = '' ) {
	...
	/**
	 * Fires when resetting main query vars and filtering 'the_content' to output SportsZone template parts.
	 *
	 * Use this action to execute code that will communicate to SportsZone's
	 * theme compatibility layer whether or not we're replacing the_content()
	 * with some other template part.
	 *
	 * @since 1.7.0
	 */
	do_action( 'sz_template_include_reset_dummy_post_data' );

	// Bail if the template already matches a SportsZone template.
	if ( ! empty( sportszone()->theme_compat->found_template ) ) {
		return $template;
	}

	/**
	 * If we are relying on SportsZone's built in theme compatibility to load
	 ...
	 *
	 * Uses sz_get_theme_compat_templates() to provide fall-backs that
	 * should be coded without superfluous mark-up and logic (prev/next
	 * navigation, comments, date/time, etc...)
	 *
	 * Hook into 'sz_get_sportszone_template' to override the array of
	 * possible templates, or 'sz_sportszone_template' to override the result.
	 */
	if ( sz_is_theme_compat_active() ) {
		$template = sz_get_theme_compat_templates();

		add_filter( 'the_content', 'sz_replace_the_content' );

		// Add SportsZone's head action to wp_head.
		if ( ! has_action( 'wp_head', 'sz_head' ) ) {
			add_action( 'wp_head', 'sz_head' );
		}
	}
	 ...
	 * @param string $template Template name.
	 */
	return apply_filters( 'sz_template_include_theme_compat', $template );
}
 ...
 * normally be handled by SportsZone, but proper single page templates do not
 ...
 */
function sz_replace_the_content( $content = '' ) {

	// Bail if not the main loop where theme compat is happening.
	if ( ! sz_do_theme_compat() ) {
		return $content;
	}
	 ...
	 * @param string $content Original post content.
	 */
	$new_content = apply_filters( 'sz_replace_the_content', $content );
 ...
 */
function sz_do_theme_compat() {
	return (bool) ( ! sz_is_template_included() && in_the_loop() && sz_is_theme_compat_active() );
}
 ...
 *
 * Removed filters are stashed in the $sz global, in case they need to be
 * restored later.
 *
 * @since 1.7.0
 *
 ...
 */
function sz_remove_all_filters( $tag, $priority = false ) {
	...
	if ( isset( $wp_filter[$tag] ) ) {
		...
		if ( ! empty( $priority ) && isset( $wp_filter[$tag][$priority] ) ) {
			...
			$sz->filters->wp_filter[$tag][$priority] = $wp_filter[$tag][$priority];
		...
		// Priority is empty.
			...
			// Store filters in a backup.
			$sz->filters->wp_filter[$tag] = $wp_filter[$tag];
	...
	if ( isset( $merged_filters[$tag] ) ) {
		...
		// Store filters in a backup.
		$sz->filters->merged_filters[$tag] = $merged_filters[$tag];
...
/**
 * Restore filters that were removed using sz_remove_all_filters().
 *
 * @since 1.7.0
 *
 * @global WP_filter $wp_filter
 ...
 * @return bool True on success.
 */
function sz_restore_all_filters( $tag, $priority = false ) {
	global $wp_filter, $merged_filters;

	$sz = sportszone();

	// Filters exist.
	if ( isset( $sz->filters->wp_filter[$tag] ) ) {

		// Filters exist in this priority.
		if ( ! empty( $priority ) && isset( $sz->filters->wp_filter[$tag][$priority] ) ) {

			// Store filters in a backup.
			$wp_filter[$tag][$priority] = $sz->filters->wp_filter[$tag][$priority];

			// Unset the filters.
			unset( $sz->filters->wp_filter[$tag][$priority] );

		// Priority is empty.
		} else {

			// Store filters in a backup.
			$wp_filter[$tag] = $sz->filters->wp_filter[$tag];

			// Unset the filters.
			unset( $sz->filters->wp_filter[$tag] );
		}
	}

	// Check merged filters.
	if ( isset( $sz->filters->merged_filters[$tag] ) ) {

		// Store filters in a backup.
		$merged_filters[$tag] = $sz->filters->merged_filters[$tag];

		// Unset the filters.
		unset( $sz->filters->merged_filters[$tag] );
	}
...
/**
 * Force comments_status to 'closed' for SportsZone post types.
 *
 * @since 1.7.0
 *
 ...
 */
function sz_comments_open( $open, $post_id = 0 ) {

	$retval = is_sportszone() ? false : $open;

	/**
	 * Filters whether or not to force comments_status to closed for SportsZone post types.
	 *
	 * @since 1.7.0
	 *
	 * @param bool $retval  Whether or not we are on a SportsZone post type.
	 * @param bool $open    True if comments open, false if closed.
	 * @param int  $post_id Post ID for the checked post.
	 */
	return apply_filters( 'sz_force_comment_status', $retval, $open, $post_id );
}
 ...
 * {@link sz_theme_compat_reset_post()}, themes that use comments_template()
 ...
 */
function sz_theme_compat_toggle_is_page( $retval = '' ) {
	global $wp_query;
	...
	if ( $wp_query->is_page ) {
		...
		sportszone()->theme_compat->is_page_toggled = true;
	}

	return $retval;
}
add_filter( 'sz_replace_the_content', 'sz_theme_compat_toggle_is_page', 9999 );

/**
 * Restores the 'is_single' and 'is_page' flags if toggled by SportsZone.
 *
 * @since 1.9.2
 *
 * @see sz_theme_compat_toggle_is_page()
 * @param object $query The WP_Query object.
 */
function sz_theme_compat_loop_end( $query ) {

	// Get SportsZone.
	$sz = sportszone();

	// Bail if page is not toggled.
	if ( ! isset( $sz->theme_compat->is_page_toggled ) ) {
		return;
	}
	...
	// Unset our switch.
	unset( $sz->theme_compat->is_page_toggled );
}
add_action( 'loop_end', 'sz_theme_compat_loop_end' );
 ...
 * @since 3.0.0
 */
function sz_check_theme_template_pack_dependency() {
	if ( sz_is_deactivation() ) {
		return;
	}

	$all_packages = array_keys( sportszone()->theme_compat->packages );

	foreach ( $all_packages as $package ) {
		// e.g. "sportszone-use-nouveau", "sportszone-use-legacy".
		if ( ! current_theme_supports( "sportszone-use-{$package}" ) ) {
			continue;
		}

		sz_setup_theme_compat( $package );
		return;
	}
}
