<?php
/**
 * SportsZone Moderation Functions.
 *
 * @package SportsZone
 * @subpackage Core
 * @since 1.6.0
 ...
 */
function sz_core_check_for_flood( $user_id = 0 ) {

	// Option disabled. No flood checks.
	if ( !$throttle_time = sz_get_option( '_sz_throttle_time' ) ) {
		return true;
	}
	...
	$last_posted = get_user_meta( $user_id, '_sz_last_posted', true );
 ...
 */
function sz_core_check_for_moderation( $user_id = 0, $title = '', $content = '', $error_type = 'bool' ) {
	 ...
	 * @param string $content $the content being posted.
	 */
	if ( apply_filters( 'sz_bypass_check_for_moderation', false, $user_id, $title, $content ) ) {
		return true;
	}
	...
	$_post['user_ip'] = sz_core_current_user_ip();
	$_post['user_ua'] = sz_core_current_user_ua();
	...
	if ( ! empty( $max_links ) ) {
		...
		if ( $num_links >= $max_links ) {
			if ( 'bool' === $error_type ) {
				...
				return new WP_Error( 'sz_moderation_too_many_links', __( 'You have posted too many links', 'sportszone' ) );
			}
		}
	}
			...
			foreach ( $_post as $post_data ) {
				...
				if ( preg_match( $pattern, $post_data ) ) {
					if ( 'bool' === $error_type ) {
						...
						return new WP_Error( 'sz_moderation_word_match', _x( 'You have posted an inappropriate word.', 'Comment moderation', 'sportszone' ) );
					}
				}
			}
		}
 ...
 * @return bool|WP_Error True if test is passed, false if fail.
 */
function sz_core_check_for_blacklist( $user_id = 0, $title = '', $content = '', $error_type = 'bool' ) {
	 ...
	 * @param string $content $the content being posted.
	 */
	if ( apply_filters( 'sz_bypass_check_for_blacklist', false, $user_id, $title, $content ) ) {
		return true;
	}
		...
		foreach( $_post as $post_data ) {
			...
			if ( preg_match( $pattern, $post_data ) ) {
				if ( 'bool' === $error_type ) {
					...
					return new WP_Error( 'sz_moderation_blacklist_match', _x( 'You have posted an inappropriate word.', 'Comment blacklist', 'sportszone' ) );
				}
			}
		}
	}
 ...
 */
function sz_core_current_user_ip() {
	 ...
	 */
	return apply_filters( 'sz_core_current_user_ip', $retval );
}
 ...
 */
function sz_core_current_user_ua() {
	 ...
	 */
	return apply_filters( 'sz_core_current_user_ua', $retval );
}
