 ...
 * }
 */
function sz_group_cover_image( $args = '' ) {
	echo sz_get_group_cover_image( $args );
}
	/**
	 ...
	 * @return string Group avatar string.
	 */
	function sz_get_group_cover_image( $args = '' ) {
		global $groups_template;
		// Bail if avatars are turned off.
		...
		$r = sz_parse_args( $args, array(
			...
			'class'  => 'cover-image',
			'id'     => false,
			'alt'    => sprintf( __( 'Group cover of %s', 'sportszone' ), $groups_template->group->name )
		) );
		...
		if ( empty( $cover_image ) ) {
			$cover_image = '<img src="' . esc_url( $groups_template->group->cover_image_thumb ) . '" class="cover-image" alt="' . esc_attr( $groups_template->group->name ) . '" />';
		}
		 ...
		 */
		return apply_filters( 'sz_get_group_cover_image', $cover_image, $r );
	}
 ...
 *                           Default: current group in loop.
 */
function sz_group_cover_image_thumb( $group = false ) {
	echo sz_get_group_cover_image_thumb( $group );
}
	/**
	 ...
	 * @return string
	 */
	function sz_get_group_cover_image_thumb( $group = false ) {
		return sz_get_group_cover_image( array(
			'type' => 'thumb',
			'id'   => ! empty( $group->id ) ? $group->id : false
		) );
	}
 ...
 * @return boolean
 */
function sz_get_group_has_cover_image( $group_id = false ) {
	...
	$group_cover_image = sz_core_fetch_cover_image( $cover_image_args ); 
	if ( sz_core_cover_image_default( 'local', $cover_image_args ) === $group_cover_image ) {
		return false;
	}
 ...
 * @since 3.1.0
 */
function sz_group_cover_image_delete_link() {
	echo sz_get_group_cover_image_delete_link();
}
	 ...
	 * @return mixed|void
	 */
	function sz_get_group_cover_image_delete_link() {
		$sz = sportszone();
		 ...
		 * @param string $value URL to delete the group avatar.
		 */
		return apply_filters( 'sz_get_group_cover_image_delete_link', wp_nonce_url( trailingslashit( sz_get_group_permalink( $sz->groups->current_group ) . 'admin/group-cover-image/delete' ), 'sz_group_cover_image_delete' ) );
	}
