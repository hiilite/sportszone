...
if( ! class_exists( 'SZ_Groups_Group' ) ) {
	// Groups component is not enabled; don't initialize this class
	sz_group_hierarchy_debug( 'Groups class was not loaded before Groups Hierarchy' );
	return;
}
 ...
 */
class SZ_Groups_Hierarchy extends SZ_Groups_Group {
	...
	function __construct( $id, $parent_id = 0, $args = array() ) {
		...
		if( ! function_exists( 'sportszone' ) ) {
			sz_group_hierarchy_debug('BP not loaded');
			return;
		}
		
		if( ! sz_is_active( 'groups' ) ) {
			sz_group_hierarchy_debug('BP Groups Component not loaded');
			return;
		}
	...
	function populate() {
		...
		parent::populate();
 		...
 		$parent_id = $wpdb->get_var( $wpdb->prepare( "SELECT g.parent_id FROM {$sz->groups->table_name} g WHERE g.id = %d", $this->id ) );
		if ( is_null( $parent_id ) ) {
			sz_group_hierarchy_debug( 'Could not load parent_id column from database.  Hierarchical processing is disabled.' );
			$this->parent_id = 0;
		} else {
	...
	function buildPath() {
		...
		do {
			$parent = new SZ_Groups_Hierarchy($parent->parent_id);
	...
	function save() {
		...
		if($this->id) {
			...
			do_action_ref_array( 'sz_group_hierarchy_before_save', array( &$this ) );
			...
			do_action_ref_array( 'sz_group_hierarchy_after_save', array( &$this ) ); 
	 ...
	 */
	public static function has_children( $id = null) {
		global $sz, $wpdb;
		if(is_null($id)) {
			if(!isset(self::$id) || self::$id == 0)	return false;
			$id = self::$id;
		}
		return $wpdb->get_col($wpdb->prepare("SELECT DISTINCT g.id FROM {$sz->groups->table_name} g WHERE g.parent_id=%d",$id));
	}
	 ...
	 */
	public static function is_child( $group_id, $parent_id = null ) {
		if(is_null($parent_id)) {
			if(!isset($this->id) || $this->id == 0)	return false;
			$parent_id = $this->id;
		}
		return $wpdb->get_var($wpdb->prepare("SELECT COUNT(g.id) FROM {$sz->groups->table_name} g WHERE g.parent_id=%d AND g.id = %d",$parent_id, $group_id));
	}
	...
	public static function get_total_subgroup_count( $group_id = null ) {
		...
		$group_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(id) FROM {$sz->groups->table_name} WHERE parent_id = %d AND status != 'hidden'", $group_id ) );
	...
	function __isset($varName) {
		...
		sz_group_hierarchy_debug( 'Magic method: __isset called for "' . $varName . '", but class is not ready.' );
		return false;
	}
	...
	function __get($varName) {
		...
		sz_group_hierarchy_debug( 'Magic method: __get called for "' . $varName . '", but class is not ready.' );
		return false;
	}
	 ...
	 */
	public static function check_slug( $slug, $parent_id = SZ_GROUPS_HIERARCHY_ANY_PARENT ) {
		global $wpdb, $sz;

		if ( !$slug )
			return false;

		if($parent_id == SZ_GROUPS_HIERARCHY_ANY_PARENT) {
			return $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$sz->groups->table_name} WHERE slug = %s", $slug ) );
		}
		return $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$sz->groups->table_name} WHERE slug = %s AND parent_id = %d", $slug, $parent_id ) );
	}
	...
	function check_slug_stem( $path ) {
		...
		return $wpdb->get_col( "SELECT slug FROM {$sz->groups->table_name} WHERE slug LIKE '$slug%'" );
	}
	...
	public static function get_by_parent( $parent_id, $type='active', $limit = null, $page = null, $user_id = false, $search_terms = false, $populate_extras = true ) {
		...
		if ( $limit && $page ) {
			...
			$total_groups = $wpdb->get_var( "SELECT COUNT(DISTINCT g.id) FROM {$sz->groups->table_name_groupmeta} gm1, {$sz->groups->table_name_groupmeta} gm2, {$sz->groups->table_name} g WHERE g.id = gm1.group_id AND g.id = gm2.group_id AND gm2.meta_key = 'last_activity' AND gm1.meta_key = 'total_member_count' AND g.parent_id = $parent_id {$hidden_sql} {$search_sql}" );
		} else {
			$pag_sql = '';
		}
		...
		switch($type) {
			...
			case 'prolific':
				...
				break;
			default:
				$order_sql = apply_filters('sz_group_hierarchy_directory_order_sort','',$type);
		}
		
		if($type == 'prolific') {
			$paged_groups = $wpdb->get_results( "SELECT g.*, (SELECT COUNT(id)FROM wp_sz_groups g2 WHERE g2.parent_id = g.id) AS child_groups, gm1.meta_value as total_member_count, gm2.meta_value as last_activity FROM {$sz->groups->table_name_groupmeta} gm1, {$sz->groups->table_name_groupmeta} gm2, {$sz->groups->table_name} g WHERE g.id = gm1.group_id AND g.id = gm2.group_id AND gm2.meta_key = 'last_activity' AND gm1.meta_key = 'total_member_count' AND g.parent_id = $parent_id {$hidden_sql} {$search_sql} {$order_sql} {$pag_sql}" );
		} else {
			$paged_groups = $wpdb->get_results( "SELECT g.*, gm1.meta_value as total_member_count, gm2.meta_value as last_activity FROM {$sz->groups->table_name_groupmeta} gm1, {$sz->groups->table_name_groupmeta} gm2, {$sz->groups->table_name} g WHERE g.id = gm1.group_id AND g.id = gm2.group_id AND gm2.meta_key = 'last_activity' AND gm1.meta_key = 'total_member_count' AND g.parent_id = $parent_id {$hidden_sql} {$search_sql} {$order_sql} {$pag_sql}" );
		}

		foreach ( (array)$paged_groups as $key => $group ) {
			$paged_groups[$key] = new SZ_Groups_Hierarchy( $group->id );
	...
	function get_tree() {
		global $wpdb, $sz;
		$groups = $wpdb->get_results( "SELECT g.* FROM {$sz->groups->table_name} g ORDER BY g.parent_id" );
		return $groups;
