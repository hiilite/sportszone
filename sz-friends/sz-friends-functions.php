...
function friends_add_friend( $initiator_userid, $friend_userid, $force_accept = false ) {
	 ...
	 * @param object $friendship        SportsZone Friendship Object.
	 */
...
function friends_remove_friend( $initiator_userid, $friend_userid ) {
	...
	$friendship_id = SZ_Friends_Friendship::get_friendship_id( $initiator_userid, $friend_userid );
	$friendship    = new SZ_Friends_Friendship( $friendship_id );
...
function friends_accept_friendship( $friendship_id ) {
	...
	$friendship = new SZ_Friends_Friendship( $friendship_id, true, false );

	// Accepting friendship.
	if ( empty( $friendship->is_confirmed ) && SZ_Friends_Friendship::accept( $friendship_id ) ) {
...
function friends_reject_friendship( $friendship_id ) {
	...
	if ( empty( $friendship->is_confirmed ) && SZ_Friends_Friendship::reject( $friendship_id ) ) {
		 ...
		 * @param SZ_Friends_Friendship $friendships Friendship object. Passed by reference.
		 */
...
function friends_withdraw_friendship( $initiator_userid, $friend_userid ) {
	...
	if ( empty( $friendship->is_confirmed ) && SZ_Friends_Friendship::withdraw( $friendship_id ) ) {
		 ...
		 * @param SZ_Friends_Friendship $friendship    Friendship object. Passed by reference.
		 */
...
function friends_check_friendship( $user_id, $possible_friend_id ) {
	...
	if ( 'is_friend' == SZ_Friends_Friendship::check_is_friend( $user_id, $possible_friend_id ) )
		return true;
...
function friends_check_friendship_status( $user_id, $possible_friend_id ) {
	...
	// Check the SZ_User_Query first
	// @see sz_friends_filter_user_query_populate_extras().
	...
	return SZ_Friends_Friendship::check_is_friend( $user_id, $possible_friend_id );
}
...
function friends_get_total_friend_count( $user_id = 0 ) {
	...
	$count = sz_get_user_meta( $user_id, 'total_friend_count', true );
	if ( empty( $count ) )
		$count = 0;
...
function friends_get_friendship_id( $initiator_user_id, $friend_user_id ) {
	return SZ_Friends_Friendship::get_friendship_id( $initiator_user_id, $friend_user_id );
}
...
function friends_get_friend_user_ids( $user_id, $friend_requests_only = false, $assoc_arr = false ) {
	return SZ_Friends_Friendship::get_friend_user_ids( $user_id, $friend_requests_only, $assoc_arr );
}
...
function friends_search_friends( $search_terms, $user_id, $pag_num = 10, $pag_page = 1 ) {
	return SZ_Friends_Friendship::search_friends( $search_terms, $user_id, $pag_num, $pag_page );
}
...
function friends_get_friendship_request_user_ids( $user_id ) {
	return SZ_Friends_Friendship::get_friendship_request_user_ids( $user_id );
}
 ...
 * @since 1.0.0
 *
 * @see SZ_Core_User::get_users() for a description of return value.
 *
 * @param int    $user_id  ID of the user whose friends are being retrieved.
 ...
 *                         string.
 * @return array See {@link SZ_Core_User::get_users()}.
 */
function friends_get_recently_active( $user_id, $per_page = 0, $page = 0, $filter = '' ) {
	$friends = sz_core_get_users( array(
		'type'         => 'active',
		'per_page'     => $per_page,
		'page'         => $page,
		'user_id'      => $user_id,
 ...
 * @since 1.0.0
 *
 * @see SZ_Friends_Friendship::get_bulk_last_active() for a description of
 *      arguments and return value.
 *
 * @param array $friend_ids See SZ_Friends_Friendship::get_bulk_last_active().
 * @return array $user_ids See SZ_Friends_Friendship::get_bulk_last_active().
 */
function friends_get_bulk_last_active( $friend_ids ) {
	return SZ_Friends_Friendship::get_bulk_last_active( $friend_ids );
}
...
function friends_get_friends_invite_list( $user_id = 0, $group_id = 0 ) {
	 ...
	 */
	$args = apply_filters( 'sz_friends_pre_get_invite_list', array(
		'user_id'  => $user_id,
		'type'     => 'alphabetical',
		'per_page' => 0
	) );
	...
	if ( sz_has_members( $args ) ) {
		...
		while ( sz_members() ) :
			...
			$friend_user_id = sz_get_member_user_id();
	 ...
	 */
	return apply_filters( 'sz_friends_get_invite_list', $friends, $user_id, $group_id );
}
...
function friends_count_invitable_friends( $user_id, $group_id ) {
	return SZ_Friends_Friendship::get_invitable_friend_count( $user_id, $group_id );
}
...
function friends_get_friend_count_for_user( $user_id ) {
	return SZ_Friends_Friendship::total_friend_count( $user_id );
}
 ...
 *                             pagination - show all results).
 * @return array Array of SZ_Core_User objects corresponding to friends.
 */
function friends_search_users( $search_terms, $user_id, $pag_num = 0, $pag_page = 0 ) {

	$user_ids = SZ_Friends_Friendship::search_users( $search_terms, $user_id, $pag_num, $pag_page );
	...
	for ( $i = 0, $count = count( $user_ids ); $i < $count; ++$i )
		$users[] = new SZ_Core_User( $user_ids[$i] );

	return array( 'users' => $users, 'count' => SZ_Friends_Friendship::search_users_count( $search_terms ) );
}
...
function friends_update_friend_totals( $initiator_user_id, $friend_user_id, $status = 'add' ) {
	...
	if ( 'add' == $status ) {
		sz_update_user_meta( $initiator_user_id, 'total_friend_count', (int)sz_get_user_meta( $initiator_user_id, 'total_friend_count', true ) + 1 );
		sz_update_user_meta( $friend_user_id, 'total_friend_count', (int)sz_get_user_meta( $friend_user_id, 'total_friend_count', true ) + 1 );
	} else {
		sz_update_user_meta( $initiator_user_id, 'total_friend_count', (int)sz_get_user_meta( $initiator_user_id, 'total_friend_count', true ) - 1 );
		sz_update_user_meta( $friend_user_id, 'total_friend_count', (int)sz_get_user_meta( $friend_user_id, 'total_friend_count', true ) - 1 );
	}
...
function friends_remove_data( $user_id ) {
	...
	SZ_Friends_Friendship::delete_all_for_user( $user_id );

	// Remove usermeta.
	sz_delete_user_meta( $user_id, 'total_friend_count' );
...
add_action( 'sz_make_spam_user', 'friends_remove_data' );
 ...
 * @since 2.1.0
 *
 * @see sz_activity_mentions_script()
 */
function sz_friends_prime_mentions_results() {
	...
	if ( ! sz_activity_maybe_load_mentions_scripts() ) {
		return;
	}
	...
	$friends_query = new SZ_User_Query( $friends_query );
	$results       = array();

	foreach ( $friends_query->results as $user ) {
		$result        = new stdClass();
		$result->ID    = $user->user_nicename;
		$result->image = sz_core_fetch_avatar( array( 'html' => false, 'item_id' => $user->ID ) );

		if ( ! empty( $user->display_name ) && ! sz_disable_profile_sync() ) {
			$result->name = $user->display_name;
		} else {
			$result->name = sz_core_get_user_displayname( $user->ID );
		}

		$results[] = $result;
	}

	wp_localize_script( 'sz-mentions', 'SZ_Suggestions', array(
		'friends' => $results,
	) );
}
add_action( 'sz_activity_mentions_prime_results', 'sz_friends_prime_mentions_results' );
...
function friends_notification_new_request( $friendship_id, $initiator_id, $friend_id ) {
	if ( 'no' == sz_get_user_meta( (int) $friend_id, 'notification_friends_friendship_request', true ) ) {
		return;
	}
	...
	$args = array(
		'tokens' => array(
			'friend-requests.url' => esc_url( sz_core_get_user_domain( $friend_id ) . sz_get_friends_slug() . '/requests/' ),
			'friend.id'           => $friend_id,
			'friendship.id'       => $friendship_id,
			'initiator.id'        => $initiator_id,
			'initiator.url'       => esc_url( sz_core_get_user_domain( $initiator_id ) ),
			'initiator.name'      => sz_core_get_user_displayname( $initiator_id ),
			'unsubscribe'         => esc_url( sz_email_get_unsubscribe_link( $unsubscribe_args ) ),
		),
	);
	sz_send_email( 'friends-request', $friend_id, $args );
}
...
function friends_notification_accepted_request( $friendship_id, $initiator_id, $friend_id ) {
	if ( 'no' == sz_get_user_meta( (int) $initiator_id, 'notification_friends_friendship_accepted', true ) ) {
		return;
	}
	...
	$args = array(
		'tokens' => array(
			'friend.id'      => $friend_id,
			'friendship.url' => esc_url( sz_core_get_user_domain( $friend_id ) ),
			'friend.name'    => sz_core_get_user_displayname( $friend_id ),
			'friendship.id'  => $friendship_id,
			'initiator.id'   => $initiator_id,
			'unsubscribe'	   => esc_url( sz_email_get_unsubscribe_link( $unsubscribe_args ) ),
		),
	);
	sz_send_email( 'friends-request-accepted', $initiator_id, $args );
}
