 ...
 *
 * A wrapper for {@link sz_activity_add()} that provides some Friends-specific
 * defaults.
 *
 * @since 1.0.0
 *
 ...
 *     of {@link sz_activity_add()}. The one difference is the following
...
/**
 * Register the activity actions for sz-friends.
 *
 * @since 1.1.0
 */
function friends_register_activity_actions() {
	...
	sz_activity_set_action(
		$sz->friends->id,
		'friendship_accepted',
		__( 'Friendships accepted', 'sportszone' ),
		'sz_friends_format_activity_action_friendship_accepted',
		__( 'Friendships', 'sportszone' ),
	...
	sz_activity_set_action(
		$sz->friends->id,
		'friendship_created',
		__( 'New friendships', 'sportszone' ),
		'sz_friends_format_activity_action_friendship_created',
		__( 'Friendships', 'sportszone' ),
		array( 'activity', 'member' )
	);

	// < BP 1.6 backpat.
	sz_activity_set_action( $sz->friends->id, 'friends_register_activity_action', __( 'New friendship created', 'sportszone' ) );

	/**
	 * Fires after all default sz-friends activity actions have been registered.
	 *
	 * @since 1.1.0
	 */
	do_action( 'friends_register_activity_actions' );
}
add_action( 'sz_register_activity_actions', 'friends_register_activity_actions' );
 ...
 */
function sz_friends_format_activity_action_friendship_accepted( $action, $activity ) {
	$initiator_link = sz_core_get_userlink( $activity->user_id );
	$friend_link    = sz_core_get_userlink( $activity->secondary_item_id );

	$action = sprintf( __( '%1$s and %2$s are now friends', 'sportszone' ), $initiator_link, $friend_link );
	...
	if ( has_filter( 'friends_activity_friendship_accepted_action' ) ) {
		$friendship = new SZ_Friends_Friendship( $activity->item_id );
	 ...
	 * @param object $activity Activity data.
	 */
	return apply_filters( 'sz_friends_format_activity_action_friendship_accepted', $action, $activity );
}
 ...
 * @return string $action Formatted activity action.
 */
function sz_friends_format_activity_action_friendship_created( $action, $activity ) {
	$initiator_link = sz_core_get_userlink( $activity->user_id );
	$friend_link    = sz_core_get_userlink( $activity->secondary_item_id );
	 ...
	 * @param object $activity Activity data.
	 */
	return apply_filters( 'sz_friends_format_activity_action_friendship_created', $action, $activity );
}
 ...
 * @return array
 */
function sz_friends_prefetch_activity_object_data( $activities ) {
	if ( empty( $activities ) ) {
		return $activities;
	}
	...
	foreach ( $activities as $activity ) {
		if ( sportszone()->friends->id !== $activity->component ) {
			continue;
		}
...
add_filter( 'sz_activity_prefetch_object_data', 'sz_friends_prefetch_activity_object_data' );

/**
 * Set up activity arguments for use with the 'friends' scope.
 *
 * For details on the syntax, see {@link SZ_Activity_Query}.
 *
 * @since 2.2.0
 *
 ...
 * @return array
 */
function sz_friends_filter_activity_scope( $retval = array(), $filter = array() ) {
...
add_filter( 'sz_activity_set_friends_scope_args', 'sz_friends_filter_activity_scope', 10, 2 );
 ...
 * @return array
 */
function sz_friends_filter_activity_just_me_scope( $retval = array(), $filter = array() ) {
...
add_filter( 'sz_activity_set_just-me_scope_args', 'sz_friends_filter_activity_just_me_scope', 20, 2 );
 ...
 */
function sz_friends_friendship_accepted_activity( $friendship_id, $initiator_user_id, $friend_user_id, $friendship = false ) {
	if ( ! sz_is_active( 'activity' ) ) {
		return;
	}
...
add_action( 'friends_friendship_accepted', 'sz_friends_friendship_accepted_activity', 10, 4 );
 ...
 */
function sz_friends_delete_activity_on_user_delete( $user_id = 0 ) {
	if ( ! sz_is_active( 'activity' ) ) {
		return;
	}
...
add_action( 'friends_remove_data', 'sz_friends_delete_activity_on_user_delete' );
