<?php
/**
 * SportsZone Activity component admin list table.
 *
 ...
 * @since 1.6.0
 */
class SZ_Activity_List_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		$this->disable_blogforum_comments = sz_disable_blogforum_comments();
	...
	function prepare_items() {
		...
		if ( ! empty( $_REQUEST['activity_type'] ) ) {
			 ...
			 * @param array $filter
			 */
			$has_filter_query = apply_filters( 'sz_activity_list_table_filter_activity_type_items', $filter );
		...
		if ( $include_id ) {
			$activities['activities'] = SZ_Activity_List_Table::flatten_activity_array( $activities['activities'] );
		...
		// The sz_activity_get function returns an array of objects; cast these to arrays for WP_List_Table.
		...
		// Don't truncate activity items; sz_activity_truncate_entry() needs to be used inside a SZ_Activity_Template loop.
	 ...
	 */
	function no_items() {
		_e( 'No activities found.', 'sportszone' );
	}
	 ...
	 */
	function get_views() {
		$url_base = add_query_arg( array( 'page' => 'sz-activity' ), sz_get_admin_url( 'admin.php' ) ); ?>

		<h2 class="screen-reader-text"><?php
			/* translators: accessibility text */
			_e( 'Filter activities list', 'sportszone' );
		?></h2>

		<ul class="subsubsub">
			<li class="all"><a href="<?php echo esc_url( $url_base ); ?>" class="<?php if ( 'spam' != $this->view ) echo 'current'; ?>"><?php _e( 'All', 'sportszone' ); ?></a> |</li>
			<li class="spam"><a href="<?php echo esc_url( add_query_arg( array( 'activity_status' => 'spam' ), $url_base ) ); ?>" class="<?php if ( 'spam' == $this->view ) echo 'current'; ?>"><?php printf( __( 'Spam <span class="count">(%s)</span>', 'sportszone' ), number_format_i18n( $this->spam_count ) ); ?></a></li>
			 ...
			 */
			do_action( 'sz_activity_list_table_get_views', $url_base, $this->view ); ?>
		</ul>
	<?php
	}
	...
	function get_bulk_actions() {
		$actions = array();
		$actions['bulk_spam']   = __( 'Mark as Spam', 'sportszone' );
		$actions['bulk_ham']    = __( 'Not Spam', 'sportszone' );
		$actions['bulk_delete'] = __( 'Delete Permanently', 'sportszone' );
		 ...
		 */
		return apply_filters( 'sz_activity_list_table_get_bulk_actions', $actions );
	}
	...
	function get_columns() {
		 ...
		 */
		return apply_filters( 'sz_activity_list_table_get_columns', array(
			'cb'       => '<input name type="checkbox" />',
			'author'   => _x('Author', 'Admin SWA column header', 'sportszone' ),
			'comment'  => _x( 'Activity', 'Admin SWA column header', 'sportszone' ),
			'action'   => _x( 'Action', 'Admin SWA column header', 'sportszone' ),
			'response' => _x( 'In Response To', 'Admin SWA column header', 'sportszone' ),
		) );
	}
	 ...
	 *
	 * @since 1.6.0
	 * @todo For this to work, SZ_Activity_Activity::get() needs updating
	...
	function extra_tablenav( $which ) {
		...
		// Get the actions.
		$activity_actions = sz_activity_get_actions(); ?>

		<div class="alignleft actions">
			<label for="activity-type" class="screen-reader-text"><?php
				/* translators: accessibility text */
				_e( 'Filter by activity type', 'sportszone' );
			?></label>
			<select name="activity_type" id="activity-type">
				<option value="" <?php selected( ! $selected ); ?>><?php _e( 'View all actions', 'sportszone' ); ?></option>
				...
				<?php foreach ( $activity_actions as $component => $actions ) : ?>
					...
					if ( sz_is_active( $component ) ) {
						if ( $component === 'xprofile' ) {
							$component_name = sportszone()->profile->name;
						} else {
							$component_name = sportszone()->$component->name;
						}
			...
			<?php submit_button( __( 'Filter', 'sportszone' ), 'secondary', false, false, array( 'id' => 'post-query-submit' ) ); ?>
		</div>
	...
	function column_cb( $item ) {
		/* translators: accessibility text */
		printf( '<label class="screen-reader-text" for="aid-%1$d">' . __( 'Select activity item %1$d', 'sportszone' ) . '</label><input type="checkbox" name="aid[]" value="%1$d" id="aid-%1$d" />', $item['id'] );
	}
	...
	function column_author( $item ) {
		echo '<strong>' . get_avatar( $item['user_id'], '32' ) . ' ' . sz_core_get_userlink( $item['user_id'] ) . '</strong>';
	}
	...
	function column_action( $item ) {
		...
		if ( isset( $actions[ $item['type'] ] ) ) {
			...
			printf( __( 'Unregistered action - %s', 'sportszone' ), $item['type'] );
		}
	}
	...
	function column_comment( $item ) {
		...
		// Build actions URLs.
		$base_url   = sz_get_admin_url( 'admin.php?page=sz-activity&amp;aid=' . $item['id'] );
		...
		if ( 'spam' != $item_status ) {
			if ( $this->can_comment( $item ) ) {
				$actions['reply'] = sprintf( '<a href="#" class="reply hide-if-no-js">%s</a>', __( 'Reply', 'sportszone' ) );
			} else {
				$actions['reply'] = sprintf( '<span class="form-input-tip">%s</span>', __( 'Replies disabled', 'sportszone' ) );
			}

			// Edit.
			$actions['edit'] = sprintf( '<a href="%s">%s</a>', $edit_url, __( 'Edit', 'sportszone' ) );
		}

		// Spam/unspam.
		if ( 'spam' == $item_status )
			$actions['unspam'] = sprintf( '<a href="%s">%s</a>', $ham_url, __( 'Not Spam', 'sportszone' ) );
		else
			$actions['spam'] = sprintf( '<a href="%s">%s</a>', $spam_url, __( 'Spam', 'sportszone' ) );

		// Delete.
		$actions['delete'] = sprintf( '<a href="%s" onclick="%s">%s</a>', $delete_url, "javascript:return confirm('" . esc_js( __( 'Are you sure?', 'sportszone' ) ) . "'); ", __( 'Delete Permanently', 'sportszone' ) );
		 ...
		 */
		$actions = apply_filters( 'sz_activity_admin_comment_row_actions', array_filter( $actions ), $item );

		printf(
			/* translators: %s: activity date and time */
			__( 'Submitted on %s', 'sportszone' ),
			sprintf(
				'<a href="%1$s">%2$s</a>',
				sz_activity_get_permalink( $item['id'] ),
				sprintf(
					/* translators: 1: activity date, 2: activity time */
					__( '%1$s at %2$s', 'sportszone' ),
					date_i18n( sz_get_option( 'date_format' ), strtotime( $item['date_recorded'] ) ),
					get_date_from_gmt( $item['date_recorded'], sz_get_option( 'time_format' ) )
				)
			)
		);
		...
		if ( ! empty( $item['content'] ) ) {
			$activity = new SZ_Activity_Activity( $item['id'] );

			/** This filter is documented in sz-activity/sz-activity-template.php */
			$content = apply_filters_ref_array( 'sz_get_activity_content_body', array( $item['content'], &$activity ) );
		} else {
			/**
			 ...
			 */
			$content = apply_filters_ref_array( 'sz_get_activity_action', array( $item['action'] ) );
		}
		 ...
		 */
		echo apply_filters( 'sz_activity_admin_comment_content', $content, $item ) . ' ' . $this->row_actions( $actions );
	}
	...
	function column_response( $item ) {
		...
		$activity_permalink = '';
		if ( ! $item['is_spam'] ) {
			$activity_permalink = '<a href="' . sz_activity_get_permalink( $item['id'], (object) $item ) . '" class="comments-view-item-link">' . __( 'View Activity', 'sportszone' ) . '</a>';
		}
		 ...
		 */
		if ( empty( $item['item_id'] ) || ! in_array( $item['type'], apply_filters( 'sz_activity_admin_root_activity_types', array( 'activity_comment' ), $item ) ) ) {
			echo $activity_permalink;

			$comment_count     = !empty( $item['children'] ) ? sz_activity_recurse_comment_count( (object) $item ) : 0;
			$root_activity_url = sz_get_admin_url( 'admin.php?page=sz-activity&amp;aid=' . $item['id'] );
		...
		// For non-root activities, display a link to the replied-to activity's author's profile.
			...
			echo '<strong>' . get_avatar( $this->get_activity_user_id( $item['item_id'] ), '32' ) . ' ' . sz_core_get_userlink( $this->get_activity_user_id( $item['item_id'] ) ) . '</strong><br />';
			echo $activity_permalink;
		}
		?>
	...
	public function column_default( $item = array(), $column_name = '' ) {
		 ...
		 */
		return apply_filters( 'sz_activity_admin_get_custom_column', '', $column_name, $item );
	}

	/**
	 * Get the user id associated with a given activity item.
	 *
	 * Wraps sz_activity_get_specific(), with some additional logic for
	...
	protected function get_activity_user_id( $activity_id ) {
		 ...
		 */
			...
			$activity = sz_activity_get_specific( array( 'activity_ids' => $activity_id, 'show_hidden' => true, 'spam' => 'all', ) );
	 ...
	 *
	 * This method merges functionality from {@link sz_activity_can_comment()} and
	 * {@link sz_blogs_disable_activity_commenting()}. This is done because the activity
	 * list table doesn't use a SportsZone activity loop, which prevents those
	 * functions from working as intended.
	 *
	 * @since 2.0.0
	 * @since 2.5.0 Include Post type activities types
	 *
	 * @param array $item An array version of the SZ_Activity_Activity object.
	 * @return bool $can_comment
	 */
	protected function can_comment( $item  ) {
		$can_comment = sz_activity_type_supports( $item['type'], 'comment-reply' );

		if ( ! $this->disable_blogforum_comments && sz_is_active( 'blogs' ) ) {
			$parent_activity = false;

			if ( sz_activity_type_supports( $item['type'], 'post-type-comment-tracking' ) ) {
				$parent_activity = (object) $item;
			} elseif ( 'activity_comment' === $item['type'] ) {
				$parent_activity = new SZ_Activity_Activity( $item['item_id'] );
				$can_comment     = sz_activity_can_comment_reply( (object) $item );
			}
			...
			if ( isset( $parent_activity->type ) && sz_activity_post_type_get_tracking_arg( $parent_activity->type, 'post_type' ) ) {
				...
				$can_comment = sz_blogs_can_comment_reply( true, $item );
			}
		}
		 ...
		 * @param array $item        An array version of the SZ_Activity_Activity object.
		 */
		return apply_filters( 'sz_activity_list_table_can_comment', $can_comment, $item );
	}

	/**
	 * Flatten the activity array.
	 *
	 * In some cases, SportsZone gives us a structured tree of activity
	...
	public static function flatten_activity_array( $tree ){
		foreach ( (array) $tree as $node ) {
			if ( isset( $node->children ) ) {
				...
				foreach ( SZ_Activity_List_Table::flatten_activity_array( $node->children ) as $child ) {
					$tree[] = $child;
				}
