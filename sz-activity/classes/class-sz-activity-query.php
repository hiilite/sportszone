 ...
 *
 * This is notably used in {@link SZ_Activity_Activity::get()} with the
 * 'filter_query' parameter.
 *
 * @since 2.2.0
 */
class SZ_Activity_Query extends SZ_Recursive_Query {
	/**
	 * Array of activity queries.
	 *
	 * See {@see SZ_Activity_Query::__construct()} for information on query arguments.
	 *
	 * @since 2.2.0
	 * @var array
	 */
	 ...
	 *
	 * See the 'wp_sz_activity' DB table schema.
	 *
	 * @since 2.2.0
	 * @var array
	 */
	 ...
	 *                      Default: 'a'. SZ_Activity_Activity::get() uses 'a', so we default to that.
	...
	public function get_sql( $alias = 'a' ) {
		...
		//
		// Also trim trailing "AND" clause from parent SZ_Recursive_Query class
		...
		if ( isset( $clause['value'] ) ) {
			...
			if ( ! empty( $column ) && true === in_array( $column, array( 'hide_sitewide', 'is_spam' ) ) ) {
				...
				switch ( $compare ) {
					...
					case 'IN' :
					case 'NOT IN' :
						$in_sql = SZ_Activity_Activity::get_in_operator_sql( "{$alias}{$column}", $value );
	 ...
	 * See {@link SZ_Activity_Query::db_tables}.
	 *
	 * @since 2.2.0
	 *
