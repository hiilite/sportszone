...
class SZ_Activity_Template {
	 ...
	 * format as {@link SZ_Activity_Activity::get()}.
	 *
	 * @since 1.5.0
	 *
	 * @see SZ_Activity_Activity::get() for a description of the argument
	 *      structure, as well as default values.
	 *
	 * @param array $args {
	 *     Array of arguments. Supports all arguments from
	 *     SZ_Activity_Activity::get(), as well as 'page_arg' and
	...
	public function __construct( $args ) {
		...
		$this->disable_blogforum_replies = (bool) sz_core_get_root_option( 'sz-disable-blogforum-comments' );

		// Get an array of the logged in user's favorite activities.
		$this->my_favs = sz_get_user_meta( sz_loggedin_user_id(), 'sz_favorite_activities', true );

		// Fetch specific activity items based on ID's.
		if ( !empty( $include ) ) {
			$this->activities = sz_activity_get_specific( array(
		...
		// Fetch all activity items.
			...
			$this->activities = sz_activity_get( array(
				'display_comments'  => $display_comments,
				'max'               => $max,
				'count_total'       => $count_total,
				'per_page'          => $this->pag_num,
		...
		$this->full_name = sz_get_displayed_user_fullname();
		...
		if ( !empty( $parent_ids ) ) {
			$activity_parents = sz_activity_get_specific( array( 'activity_ids' => $parent_ids ) );
		}
		...
		if ( (int) $this->total_activity_count && (int) $this->pag_num ) {
			$this->pag_links = paginate_links( array(
				...
				'prev_text' => _x( '&larr;', 'Activity pagination previous text', 'sportszone' ),
				'next_text' => _x( '&rarr;', 'Activity pagination next text', 'sportszone' ),
				'mid_size'  => 1,
				'add_args'  => array(),
			) );
		}
	 ...
	 *
	 * This method is used by {@link sz_activities()} as part of the while loop
	 ...
	 *
	 * Used by {@link sz_the_activity()} to set up the current activity item
