 ...
 * @since 1.8.0
 *
 * See {@link sz_activity_action_sitewide_feed()} as an example.
 *
 * @param array $args {
 ...
 *   @type array  $activity_args    Optional. Arguments passed to {@link sz_has_activities()}
 * }
 */
class SZ_Activity_Feed {
	 ...
	 *
	 * @see SZ_Feed::__construct() This is where $data is added.
	 *
	 * @since 1.8.0
	 * @var array
	 */
	...
	public function __construct( $args = array() ) {
		...
		/**
		 * Filters if SportsZone should consider feeds enabled. If disabled, it will return early.
		 *
		 * @since 1.8.0
		 *
		 * @param bool true Default true aka feeds are enabled.
		 */
		if ( false === (bool) apply_filters( 'sz_activity_enable_feeds', true ) ) {
			global $wp_query;
		...
		$this->data = wp_parse_args( $args, array(
			...
			// Activity arguments passed to sz_has_activities().
			'activity_args'    => array()
		) );
		 ...
		 * @since 1.8.0
		 *
		 * @param SZ_Activity_Feed $this Current instance of activity feed. Passed by reference.
		 */
		do_action_ref_array( 'sz_activity_feed_prefetch', array( &$this ) );
		...
		if ( empty( $this->id ) ) {
			_doing_it_wrong( 'SZ_Activity_Feed', __( "RSS feed 'id' must be defined", 'sportszone' ), 'BP 1.8' );
			return false;
		}
		 ...
		 * @param SZ_Activity_Feed $this Current instance of activity feed. Passed by reference.
		 */
		do_action_ref_array( 'sz_activity_feed_postfetch', array( &$this ) );
	...
	protected function setup_hooks() {
		add_action( 'sz_activity_feed_rss_attributes',   array( $this, 'backpat_rss_attributes' ) );
		add_action( 'sz_activity_feed_channel_elements', array( $this, 'backpat_channel_elements' ) );
		add_action( 'sz_activity_feed_item_elements',    array( $this, 'backpat_item_elements' ) );
	}
	...
	public function backpat_rss_attributes() {
		 ...
		 *
		 * This hook was originally separated out for individual components but has since been abstracted into the SZ_Activity_Feed class.
		 *
		 * @since 1.0.0
		 */
		do_action( 'sz_activity_' . $this->id . '_feed' );
	}
	...
	public function backpat_channel_elements() {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'sz_activity_' . $this->id . '_feed_head' );
	}
	...
	public function backpat_item_elements() {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'sz_activity_' . $id . '_feed_item' );
	}
	...
	protected function feed_content() {
		...
		switch ( $this->id ) {
			...
			case 'personal' :
				...
				if ( 'activity_comment' == sz_get_activity_action_name() ) :
			?>
				<strong><?php _e( 'In reply to', 'sportszone' ) ?></strong> -
				<?php sz_activity_parent_content() ?>
			<?php
				endif;
	...
	protected function http_headers() {
		...
		$last_modified      = mysql2date( 'D, d M Y H:i:s O', sz_activity_get_last_updated(), false );
		...
		// First commit of SportsZone! (Easter egg).
...
<rss version="2.0"
	 ...
	 * @since 1.8.0
	 */
	do_action( 'sz_activity_feed_rss_attributes' ); ?>
>
...
<channel>
	...
	<lastBuildDate><?php echo mysql2date( 'D, d M Y H:i:s O', sz_activity_get_last_updated(), false ); ?></lastBuildDate>
	<generator>https://sportszone.org/?v=<?php sz_version(); ?></generator>
	 ...
	 * @since 1.8.0
	 */
	do_action( 'sz_activity_feed_channel_elements' ); ?>

	<?php if ( sz_has_activities( $this->activity_args ) ) : ?>
		<?php while ( sz_activities() ) : sz_the_activity(); ?>
			<item>
				<guid isPermaLink="false"><?php sz_activity_feed_item_guid(); ?></guid>
				<title><?php echo stripslashes( sz_get_activity_feed_item_title() ); ?></title>
				<link><?php sz_activity_thread_permalink() ?></link>
				<pubDate><?php echo mysql2date( 'D, d M Y H:i:s O', sz_get_activity_feed_item_date(), false ); ?></pubDate>

				<?php if ( sz_get_activity_feed_item_description() ) : ?>
					<content:encoded><![CDATA[<?php $this->feed_content(); ?>]]></content:encoded>
				<?php endif; ?>

				<?php if ( sz_activity_can_comment() ) : ?>
					<slash:comments><?php sz_activity_comment_count(); ?></slash:comments>
				<?php endif; ?>
				 ...
				 * @since 1.8.0
				 */
				do_action( 'sz_activity_feed_item_elements' ); ?>
			</item>
		<?php endwhile; ?>
