...
class SZ_Akismet {
	...
	protected function setup_actions() {
		...
		add_action( 'sz_after_activity_post_form', array( $this, 'add_activity_stream_nonce' ) );
		add_action( 'sz_activity_entry_comments',  array( $this, 'add_activity_stream_nonce' ) );

		// Add a "mark as spam" button to individual activity items.
		add_action( 'sz_activity_entry_meta',      array( $this, 'add_activity_spam_button' ) );
		add_action( 'sz_activity_comment_options', array( $this, 'add_activity_comment_spam_button' ) );

		// Check activity for spam.
		add_action( 'sz_activity_before_save',     array( $this, 'check_activity' ), 4, 1 );

		// Tidy up member's latest (activity) update.
		add_action( 'sz_activity_posted_update',   array( $this, 'check_member_activity_update' ), 1, 3 );

		// Hooks to extend Activity core spam/ham functions for Akismet.
		add_action( 'sz_activity_mark_as_spam',    array( $this, 'mark_as_spam' ), 10, 2 );
		add_action( 'sz_activity_mark_as_ham',     array( $this, 'mark_as_ham' ),  10, 2 );

		// Hook into the Activity wp-admin screen.
		add_action( 'sz_activity_admin_comment_row_actions', array( $this, 'comment_row_action' ), 10, 2 );
		add_action( 'sz_activity_admin_load',                array( $this, 'add_history_metabox' ) );
	}
	...
	function comment_row_action( $actions, $activity ) {
		$akismet_result = sz_activity_get_meta( $activity['id'], '_sz_akismet_result' );
		$user_result    = sz_activity_get_meta( $activity['id'], '_sz_akismet_user_result' );
		$desc           = '';

		if ( !$user_result || $user_result == $akismet_result ) {
			// Show the original Akismet result if the user hasn't overridden it, or if their decision was the same.
			if ( 'true' == $akismet_result && $activity['is_spam'] )
				$desc = __( 'Flagged as spam by Akismet', 'sportszone' );

			elseif ( 'false' == $akismet_result && !$activity['is_spam'] )
				$desc = __( 'Cleared by Akismet', 'sportszone' );

		} else {
			$who = sz_activity_get_meta( $activity['id'], '_sz_akismet_user' );

			if ( 'true' == $user_result )
				$desc = sprintf( __( 'Flagged as spam by %s', 'sportszone' ), $who );
			else
				$desc = sprintf( __( 'Un-spammed by %s', 'sportszone' ), $who );
		}
		...
		if ( $akismet_result ) {
			...
			foreach ( $actions as $k => $item ) {
				$b[ $k ] = $item;
				if ( $k == 'edit' )
					$b['history'] = '<a href="' . esc_url( sz_get_admin_url( 'admin.php?page=sz-activity&amp;action=edit&aid=' . $activity['id'] ) ) . '#sz_activity_history"> '. __( 'History', 'sportszone' ) . '</a>';
			}
		...
		if ( $desc )
			echo '<span class="akismet-status"><a href="' . esc_url( sz_get_admin_url( 'admin.php?page=sz-activity&amp;action=edit&aid=' . $activity['id'] ) ) . '#sz_activity_history">' . htmlspecialchars( $desc ) . '</a></span>';
		 ...
		 */
		return apply_filters( 'sz_akismet_comment_row_action', $actions );
	}
	...
	public function add_activity_stream_nonce() {
		$form_id = '_sz_as_nonce';
		$value   = '_sz_as_nonce_' . sz_loggedin_user_id();

		// If we're in the activity stream loop, we can use the current item's ID to make the nonce unique.
		if ( 'sz_activity_entry_comments' == current_filter() ) {
			$form_id .= '_' . sz_get_activity_id();
			$value   .= '_' . sz_get_activity_id();
		}
	...
	/**
	 * Clean up the sz_latest_update usermeta in case of spamming.
	 *
	 ...
	 * the cached update is cleared from the user's 'sz_latest_update'
	 * usermeta, ensuring that it won't appear in the member header and
	 * elsewhere in the theme.
	 *
	 * This can't be done in SZ_Akismet::check_activity() due to the
	 * default AJAX implementation; see sz_dtheme_post_update().
	 *
	 * @since 1.6.0
	 *
	 * @see sz_dtheme_post_update()
	 *
	...
	public function check_member_activity_update( $content, $user_id, $activity_id ) {
		...
		if ( empty( $this->last_activity ) || !in_array( $this->last_activity->type, SZ_Akismet::get_activity_types() ) )
			return;

		// Was this $activity_id just marked as spam? If not, bail out.
		if ( !$this->last_activity->id || $activity_id != $this->last_activity->id || 'false' == $this->last_activity->akismet_submission['sz_as_result'] )
			return;
	...
	public function add_activity_spam_button() {
		...
		// By default, only handle activity updates and activity comments.
		if ( !in_array( sz_get_activity_type(), SZ_Akismet::get_activity_types() ) )
			return;
		...
		sz_button(
			array(
				'block_self' => false,
				'component'  => 'activity',
				'id'         => 'activity_make_spam_' . sz_get_activity_id(),
				'link_class' => 'sz-secondary-action spam-activity confirm button item-button',
				'link_href'  => wp_nonce_url( sz_get_root_domain() . '/' . sz_get_activity_slug() . '/spam/' . sz_get_activity_id() . '/', 'sz_activity_akismet_spam_' . sz_get_activity_id() ),
				'link_text'  => __( 'Spam', 'sportszone' ),
				'wrapper'    => false,
			)
		);
	}
	...
	public function add_activity_comment_spam_button() {
		...
		// By default, only handle activity updates and activity comments.
		$current_comment = sz_activity_current_comment();
		if ( empty( $current_comment ) || !in_array( $current_comment->type, SZ_Akismet::get_activity_types() ) )
			return;
		...
		sz_button(
			array(
				'block_self' => false,
				'component'  => 'activity',
				'id'         => 'activity_make_spam_' . sz_get_activity_comment_id(),
				'link_class' => 'sz-secondary-action spam-activity-comment confirm',
				'link_href'  => wp_nonce_url( sz_get_root_domain() . '/' . sz_get_activity_slug() . '/spam/' . sz_get_activity_comment_id() . '/?cid=' . sz_get_activity_comment_id(), 'sz_activity_akismet_spam_' . sz_get_activity_comment_id() ),
				'link_text'  => __( 'Spam', 'sportszone' ),
				'wrapper'    => false,
			)
		);
	...
	public static function get_activity_types() {
		 ...
		 */
		return apply_filters( 'sz_akismet_get_activity_types', array( 'activity_comment', 'activity_update' ) );
	}
	 ...
	 * @since 1.6.0
	 *
	 * @param SZ_Activity_Activity $activity Activity item being spammed.
	...
	public function mark_as_spam( $activity, $source ) {
		// Record this item so we can do some tidyup in SZ_Akismet::check_member_activity_update().
		 ...
		 * @since 1.6.0
		 *
		 * @param SZ_Activity_Activity $activity Activity object being marked as spam.
		 ...
		 *                                       or "by_akismet".
		 */
		do_action( 'sz_activity_akismet_mark_as_spam', $activity, $source );
	}
	 ...
	 * @since 1.6.0
	 *
	 * @param SZ_Activity_Activity $activity Activity item being hammed.
	 * @param string               $source   Either "by_a_person" (e.g. a person has
	...
	public function mark_as_ham( $activity, $source ) {
		...
		if ( 'true' == sz_activity_get_meta( $activity->id, '_sz_akismet_result' ) && !sz_activity_get_meta( $activity->id, '_sz_akismet_user_result' ) )
			$activity->content = sz_activity_at_name_filter( $activity->content, $activity->id );
		 ...
		 * @since 1.6.0
		 *
		 * @param SZ_Activity_Activity $activity Activity object being marked as ham.
		 * @param string               $source   Source of the whom marked as ham.
		 *                                       Either "by_a_person" (e.g. a person has
		 *                                       manually marked the activity as ham) or
		 *                                       "by_akismet" (automatically hammed).
		 */
		do_action( 'sz_activity_akismet_mark_as_ham', $activity, $source );
	}
	 ...
	 * @static
	 *
	 * @param SZ_Activity_Activity $activity Activity item data.
	...
	public static function build_akismet_data_package( $activity ) {
		...
		$activity_data['comment_author_url']    = sz_core_get_userlink( $userdata->ID, false, true);
		$activity_data['comment_content']       = $activity->content;
		$activity_data['comment_type']          = $activity->type;
		$activity_data['permalink']             = sz_activity_get_permalink( $activity->id, $activity );
		 ...
		 */
		if ( !empty( $_POST['_sz_as_nonce'] ) )
			$activity_data['akismet_comment_nonce'] = wp_verify_nonce( $_POST['_sz_as_nonce'], "_sz_as_nonce_{$userdata->ID}" ) ? 'passed' : 'failed';

		/**
		 * If the new activity was a reply to an existing item, check the nonce with the activity parent ID.
		 * This helps Akismet ensure that the update was a valid form submission.
		 */
		elseif ( !empty( $activity->secondary_item_id ) && !empty( $_POST['_sz_as_nonce_' . $activity->secondary_item_id] ) )
			$activity_data['akismet_comment_nonce'] = wp_verify_nonce( $_POST["_sz_as_nonce_{$activity->secondary_item_id}"], "_sz_as_nonce_{$userdata->ID}_{$activity->secondary_item_id}" ) ? 'passed' : 'failed';
		 ...
		 * @param SZ_Activity_Activity $activity      Activity item data.
		 */
		return apply_filters( 'sz_akismet_build_akismet_data_package', $activity_data, $activity );
	}
	 ...
	 *
	 * @param SZ_Activity_Activity $activity The activity item to check.
	 */
	public function check_activity( $activity ) {
		// By default, only handle activity updates and activity comments.
		if ( !in_array( $activity->type, SZ_Akismet::get_activity_types() ) )
			return;
		...
		// Spam.
		if ( 'true' == $activity_data['sz_as_result'] ) {
			/**
			 * Fires after an activity item has been proven to be spam, but before officially being marked as spam.
			 *
			 * @since 1.6.0
			 *
			 * @param SZ_Activity_Activity $activity      The activity item proven to be spam.
			 * @param array                $activity_data Array of activity data for item including
			 *                                            Akismet check results data.
			 */
			do_action_ref_array( 'sz_activity_akismet_spam_caught', array( &$activity, $activity_data ) );

			// Mark as spam.
			sz_activity_mark_as_spam( $activity, 'by_akismet' );
		...
		add_action( 'sz_activity_after_save', array( $this, 'update_activity_akismet_meta' ), 1, 1 );
	}
	 ...
	 * @since 1.6.0
	 *
	 * @param SZ_Activity_Activity $activity The activity to check.
	 */
	public function update_activity_spam_meta( $activity ) {
		...
		$this->update_activity_history( $activity->id, sprintf( __( '%s reported this activity as spam', 'sportszone' ), sz_get_loggedin_user_username() ), 'report-spam' );
		sz_activity_update_meta( $activity->id, '_sz_akismet_user_result', 'true' );
		sz_activity_update_meta( $activity->id, '_sz_akismet_user', sz_get_loggedin_user_username() );
	}
	...
	public function update_activity_ham_meta( $activity ) {
		...
		$this->update_activity_history( $activity->id, sprintf( __( '%s reported this activity as not spam', 'sportszone' ), sz_get_loggedin_user_username() ), 'report-ham' );
		sz_activity_update_meta( $activity->id, '_sz_akismet_user_result', 'false' );
		sz_activity_update_meta( $activity->id, '_sz_akismet_user', sz_get_loggedin_user_username() );
	}
	...
	public function update_activity_akismet_meta( $activity ) {
		...
		// By default, only handle activity updates and activity comments.
		if ( !in_array( $this->last_activity->type, SZ_Akismet::get_activity_types() ) )
			return;

		// Spam.
		if ( 'true' == $this->last_activity->akismet_submission['sz_as_result'] ) {
			sz_activity_update_meta( $activity->id, '_sz_akismet_result', 'true' );
			$this->update_activity_history( $activity->id, __( 'Akismet caught this item as spam', 'sportszone' ), 'check-spam' );

		// Not spam.
		} elseif ( 'false' == $this->last_activity->akismet_submission['sz_as_result'] ) {
			sz_activity_update_meta( $activity->id, '_sz_akismet_result', 'false' );
			$this->update_activity_history( $activity->id, __( 'Akismet cleared this item', 'sportszone' ), 'check-ham' );

		// Uh oh, something's gone horribly wrong. Unexpected result.
		} else {
			sz_activity_update_meta( $activity->id, '_sz_akismet_error', sz_core_current_time() );
			$this->update_activity_history( $activity->id, sprintf( __( 'Akismet was unable to check this item (response: %s), will automatically retry again later.', 'sportszone' ), $this->last_activity->akismet_submission['sz_as_result'] ), 'check-error' );
		}

		// Record the original data which was submitted to Akismet for checking.
		sz_activity_update_meta( $activity->id, '_sz_akismet_submission', $this->last_activity->akismet_submission );
	}
	...
	public function send_akismet_request( $activity_data, $check = 'check', $spam = 'spam' ) {
		...
		$activity_data['blog']         = sz_get_option( 'home' );
		$activity_data['blog_charset'] = sz_get_option( 'blog_charset' );
		$activity_data['blog_lang']    = get_locale();
		$activity_data['referrer']     = $_SERVER['HTTP_REFERER'];
		$activity_data['user_agent']   = sz_core_current_user_ua();
		$activity_data['user_ip']      = sz_core_current_user_ip();
		...
		// Send to Akismet.
		add_filter( 'akismet_ua', array( $this, 'sportszone_ua' ) );
		$response = Akismet::http_post( $query_string, $path );
		remove_filter( 'akismet_ua', array( $this, 'sportszone_ua' ) );

		// Save response data.
		$activity_data['sz_as_result'] = $response[1];
		if ( isset( $response[0]['x-akismet-pro-tip'] ) ) {
			$activity_data['akismet_pro_tip'] = $response[0]['x-akismet-pro-tip'];
		}

		// Perform a daily tidy up.
		if ( ! wp_next_scheduled( 'sz_activity_akismet_delete_old_metadata' ) )
			wp_schedule_event( time(), 'daily', 'sz_activity_akismet_delete_old_metadata' );

		return $activity_data;
	}

	/**
	 * Filters user agent when sending to Akismet to add SportsZone info.
	 *
	 * @since 1.6.0
	 *
	 ...
	 */
	public function sportszone_ua( $user_agent ) {
		$user_agent = 'SportsZone/' . sz_get_version() . ' | Akismet/'. constant( 'AKISMET_VERSION' );
		return $user_agent;
	}
	...
	function add_history_metabox( $screen_action ) {
		...
		add_meta_box( 'sz_activity_history',  __( 'Activity History', 'sportszone' ), array( $this, 'history_metabox' ), get_current_screen()->id, 'normal', 'low' );
	}
	 ...
	 * @since 1.6.0
	 *
	 * @see https://sportszone.trac.wordpress.org/ticket/3907
	 * @todo Update activity meta to allow >1 record with the same key (iterate through $history).
	 *
	 * @param object $item Activity item.
	 */
	function history_metabox( $item ) {
		$history = SZ_Akismet::get_activity_history( $item->id );

		if ( empty( $history ) )
			return;

		echo '<div class="akismet-history"><div>';
		printf( _x( '%1$s &mdash; %2$s', 'x hours ago - akismet cleared this item', 'sportszone' ), '<span>' . sz_core_time_since( $history[2] ) . '</span>', esc_html( $history[1] ) );
		echo '</div></div>';
	}
	...
	public function update_activity_history( $activity_id = 0, $message = '', $event = '' ) {
		$event = array(
			...
			'user'    => sz_loggedin_user_id(),
		);

		// Save the history data.
		sz_activity_update_meta( $activity_id, '_sz_akismet_history', $event );
	}
	...
	public function get_activity_history( $activity_id = 0 ) {
		$history = sz_activity_get_meta( $activity_id, '_sz_akismet_history' );
		if ( $history === false )
			$history = array();
