 ...
 * @since 2.6.0
 */
class SZ_Activity_oEmbed_Extension extends SZ_Core_oEmbed_Extension {
	/**
	...
	protected function custom_hooks() {
		...
		add_filter( 'sz_activity_embed_html', array( $this, 'modify_iframe' ) );
	}
	...
	protected function content() {
		sz_get_asset_template_part( 'embeds/activity' );
	}
	...
	protected function validate_url_to_item_id( $url ) {
		if ( sz_core_enable_root_profiles() ) {
			$domain = sz_get_root_domain();
		} else {
			$domain = sz_get_members_directory_permalink();
		}
		...
		if ( false === strpos( $url, '/' . sz_get_activity_slug() . '/' ) ) {
			return false;
		}
	...
	protected function set_oembed_response_data( $item_id ) {
		$activity = new SZ_Activity_Activity( $item_id );

		return array(
			'content'      => $activity->content,
			'title'        => __( 'Activity', 'sportszone' ),
			'author_name'  => sz_core_get_user_displayname( $activity->user_id ),
			'author_url'   => sz_core_get_user_domain( $activity->user_id ),

			// Custom identifier.
			'x_sportszone' => 'activity'
		);
	}
	...
	protected function set_fallback_html( $item_id ) {
		$activity    = new SZ_Activity_Activity( $item_id );
		$mentionname = sz_activity_do_mentions() ? ' (@' . sz_activity_get_user_mentionname( $activity->user_id ) . ')' : '';
		...
		$blockquote = sprintf( '<blockquote class="wp-embedded-content sz-activity-item">%1$s%2$s %3$s</blockquote>',
			sz_activity_get_embed_excerpt( $activity->content ),
			'- ' . sz_core_get_user_displayname( $activity->user_id ) . $mentionname,
			'<a href="' . esc_url( sz_activity_get_permalink( $item_id ) ) . '">' . $date . '</a>'
		);
		 ...
		 */
		return apply_filters( 'sz_activity_embed_fallback_html', $blockquote, $activity );
	}
	...
	protected function set_iframe_title( $item_id ) {
		return __( 'Embedded Activity Item', 'sportszone' );
	}
	 ...
	 * @since 2.6.0
	 *
	 * @see SZ_Activity_Component::modify_iframe() where our custom sandbox value is set.
	 *
	...
	public function use_custom_iframe_sandbox_attribute( $result, $data, $url ) {
		// Make sure we are on a SportsZone activity oEmbed request.
		if ( false === isset( $data->x_sportszone ) || 'activity' !== $data->x_sportszone ) {
			return $result;
		}
	 ...
	 *
	 * Currently, removes wpautop() from the sz_activity_action() function.
	 *
	 * @since 2.6.0
	 *
	...
	public function on_activity_header( $slug, $name ) {
		...
		remove_filter( 'sz_get_activity_action', 'wpautop' );
	}
	...
	public function embed_comments_button() {
		if ( ! did_action( 'sz_embed_content' ) || ! sz_is_single_activity() ) {
			return;
		}
		...
		$count = sz_activity_get_comment_count();
		if ( empty( $count ) ) {
			return;
		}
	?>
		...
		<div class="wp-embed-comments">
			<a href="<?php sz_activity_thread_permalink(); ?>">
