...
class SZ_Activity_Activity {
	...
	/**
	 * SportsZone component the activity item relates to.
	 *
	 * @since 1.2.0
	 * @var string
	 */
	...
	public function populate() {
		...
		$row = wp_cache_get( $this->id, 'sz_activity' );

		if ( false === $row ) {
			$sz  = sportszone();
			$row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$sz->activity->table_name} WHERE id = %d", $this->id ) );

			wp_cache_set( $this->id, $row, 'sz_activity' );
		}
		...
		$action = sz_activity_generate_action_string( $this );
		if ( false !== $action ) {
			$this->action = $action;
	...
	public function save() {
		...
		$this->id                = apply_filters_ref_array( 'sz_activity_id_before_save',                array( $this->id,                &$this ) );
		$this->item_id           = apply_filters_ref_array( 'sz_activity_item_id_before_save',           array( $this->item_id,           &$this ) );
		$this->secondary_item_id = apply_filters_ref_array( 'sz_activity_secondary_item_id_before_save', array( $this->secondary_item_id, &$this ) );
		$this->user_id           = apply_filters_ref_array( 'sz_activity_user_id_before_save',           array( $this->user_id,           &$this ) );
		$this->primary_link      = apply_filters_ref_array( 'sz_activity_primary_link_before_save',      array( $this->primary_link,      &$this ) );
		$this->component         = apply_filters_ref_array( 'sz_activity_component_before_save',         array( $this->component,         &$this ) );
		$this->type              = apply_filters_ref_array( 'sz_activity_type_before_save',              array( $this->type,              &$this ) );
		$this->action            = apply_filters_ref_array( 'sz_activity_action_before_save',            array( $this->action,            &$this ) );
		$this->content           = apply_filters_ref_array( 'sz_activity_content_before_save',           array( $this->content,           &$this ) );
		$this->date_recorded     = apply_filters_ref_array( 'sz_activity_date_recorded_before_save',     array( $this->date_recorded,     &$this ) );
		$this->hide_sitewide     = apply_filters_ref_array( 'sz_activity_hide_sitewide_before_save',     array( $this->hide_sitewide,     &$this ) );
		$this->mptt_left         = apply_filters_ref_array( 'sz_activity_mptt_left_before_save',         array( $this->mptt_left,         &$this ) );
		$this->mptt_right        = apply_filters_ref_array( 'sz_activity_mptt_right_before_save',        array( $this->mptt_right,        &$this ) );
		$this->is_spam           = apply_filters_ref_array( 'sz_activity_is_spam_before_save',           array( $this->is_spam,           &$this ) );
		 ...
		 * @since 1.0.0
		 *
		 * @param SZ_Activity_Activity $this Current instance of the activity item being saved. Passed by reference.
		 */
		do_action_ref_array( 'sz_activity_before_save', array( &$this ) );
		...
		if ( empty( $this->component ) || empty( $this->type ) ) {
			if ( 'bool' === $this->error_type ) {
				...
				if ( empty( $this->component ) ) {
					$this->errors->add( 'sz_activity_missing_component' );
				} else {
					$this->errors->add( 'sz_activity_missing_type' );
				}
		...
		if ( empty( $this->primary_link ) ) {
			$this->primary_link = sz_loggedin_user_domain();
		}

		// If we have an existing ID, update the activity item, otherwise insert it.
		if ( ! empty( $this->id ) ) {
			$q = $wpdb->prepare( "UPDATE {$sz->activity->table_name} SET user_id = %d, component = %s, type = %s, action = %s, content = %s, primary_link = %s, date_recorded = %s, item_id = %d, secondary_item_id = %d, hide_sitewide = %d, is_spam = %d WHERE id = %d", $this->user_id, $this->component, $this->type, $this->action, $this->content, $this->primary_link, $this->date_recorded, $this->item_id, $this->secondary_item_id, $this->hide_sitewide, $this->is_spam, $this->id );
		} else {
			$q = $wpdb->prepare( "INSERT INTO {$sz->activity->table_name} ( user_id, component, type, action, content, primary_link, date_recorded, item_id, secondary_item_id, hide_sitewide, is_spam ) VALUES ( %d, %s, %s, %s, %s, %s, %s, %d, %d, %d, %d )", $this->user_id, $this->component, $this->type, $this->action, $this->content, $this->primary_link, $this->date_recorded, $this->item_id, $this->secondary_item_id, $this->hide_sitewide, $this->is_spam );
		}
		...
		if ( empty( $this->id ) ) {
			...
			add_filter( 'sz_activity_at_name_do_notifications', '__return_false' );
		}
		 ...
		 * @since 1.0.0
		 *
		 * @param SZ_Activity_Activity $this Current instance of activity item being saved. Passed by reference.
		 */
		do_action_ref_array( 'sz_activity_after_save', array( &$this ) );
	 ...
	 *
	 * @see SZ_Activity_Activity::get_filter_sql() for a description of the
	 *      'filter' parameter.
	 ...
	 *     @type array        $filter_query      Array of advanced query conditions. See SZ_Activity_Query::__construct().
	 *     @type string|array $scope             Pre-determined set of activity arguments.
	 *     @type array        $filter            See SZ_Activity_Activity::get_filter_sql().
	...
	public static function get( $args = array() ) {
		...
		if ( !is_array( $args ) || func_num_args() > 1 ) {
			_deprecated_argument( __METHOD__, '1.6', sprintf( __( 'Arguments passed to %1$s should be in an associative array. See the inline documentation at %2$s for more details.', 'sportszone' ), __METHOD__, __FILE__ ) );
		...
		$r  = wp_parse_args( $args, array(
			...
			'filter_query'      => false,           // Advanced filtering - see SZ_Activity_Query.
		...
		$from_sql   = " FROM {$sz->activity->table_name} a";
		...
		if ( ! empty( $r['scope'] ) ) {
			...
			$filter_query = new SZ_Activity_Query( $r['filter_query'] );
		...
		// Regular filtering.
		if ( $r['filter'] && $filter_sql = SZ_Activity_Activity::get_filter_sql( $r['filter'] ) ) {
		...
		// Searching.
		if ( $r['search_terms'] ) {
			$search_terms_like = '%' . sz_esc_like( $r['search_terms'] ) . '%';
			 ...
			 */
			if ( apply_filters( 'sz_activity_get_include_user_search', false ) ) {
		 ...
		 */
		$where_conditions = apply_filters( 'sz_activity_get_where_conditions', $where_conditions, $r, $select_sql, $from_sql, $join_sql );
		 ...
		 */
		$join_sql = apply_filters( 'sz_activity_get_join_sql', $join_sql, $r, $select_sql, $from_sql, $where_sql );
		...
		/**
		 * Filters if SportsZone should use legacy query structure over current structure for version 2.0+.
		 *
		 ...
		 * @param SZ_Activity_Activity $value Current method being called.
		 * @param array                $r     Parsed arguments passed into method.
		 */
		if ( apply_filters( 'sz_use_legacy_activity_query', false, __METHOD__, $r ) ) {
			...
			$from_sql   = " FROM {$sz->activity->table_name} a LEFT JOIN {$wpdb->users} u ON a.user_id = u.ID";

			if ( ! empty( $page ) && ! empty( $per_page ) ) {
				$pag_sql = $wpdb->prepare( "LIMIT %d, %d", absint( ( $page - 1 ) * $per_page ), $per_page );

				/** This filter is documented in sz-activity/sz-activity-classes.php */
				$activity_sql = apply_filters( 'sz_activity_get_user_join_filter', "{$select_sql} {$from_sql} {$join_sql} {$where_sql} ORDER BY a.date_recorded {$sort}, a.id {$sort} {$pag_sql}", $select_sql, $from_sql, $where_sql, $sort, $pag_sql );
			} else {
				$pag_sql = '';
				 ...
				 */
				$activity_sql = apply_filters( 'sz_activity_get_user_join_filter', "{$select_sql} {$from_sql} {$join_sql} {$where_sql} ORDER BY a.date_recorded {$sort}, a.id {$sort}", $select_sql, $from_sql, $where_sql, $sort, $pag_sql );
			}
			...
			if ( preg_match( '/a\.type NOT IN \([^\)]*\'last_activity\'[^\)]*\)/', $activity_sql ) ) {
				$cache_group = 'sz_activity';
			} else {
				$cache_group = 'sz_activity_with_last_activity';
			}
			 ...
			 */
			$activity_ids_sql = apply_filters( 'sz_activity_paged_activities_sql', $activity_ids_sql, $r );
			...
			$cached = sz_core_get_incremented_cache( $activity_ids_sql, $cache_group );
			if ( false === $cached ) {
				$activity_ids = $wpdb->get_col( $activity_ids_sql );
				sz_core_set_incremented_cache( $activity_ids_sql, $cache_group, $activity_ids );
			} else {
				$activity_ids = $cached;
			}
		...
		if ( 'ids' !== $r['fields'] ) {
			...
			if ( ! empty( $activity_ids ) && $r['update_meta_cache'] ) {
				sz_activity_update_meta_cache( $activity_ids );
			}

			if ( $activities && $r['display_comments'] ) {
				$activities = SZ_Activity_Activity::append_comments( $activities, $r['spam'] );
			}

			// Pre-fetch data associated with activity users and other objects.
			SZ_Activity_Activity::prefetch_object_data( $activities );

			// Generate action strings.
			$activities = SZ_Activity_Activity::generate_action_strings( $activities );
		}
		...
		if ( ! empty( $r['count_total'] ) ) {
			 ...
			 */
			$total_activities_sql = apply_filters( 'sz_activity_total_activities_sql', "SELECT count(DISTINCT a.id) FROM {$sz->activity->table_name} a {$join_sql} {$where_sql}", $where_sql, $sort );
			$cached = sz_core_get_incremented_cache( $total_activities_sql, $cache_group );
			if ( false === $cached ) {
				$total_activities = $wpdb->get_var( $total_activities_sql );
				sz_core_set_incremented_cache( $total_activities_sql, $cache_group, $total_activities );
			} else {
	...
	protected static function get_activity_data( $activity_ids = array() ) {
		...
		$activities   = array();
		$uncached_ids = sz_get_non_cached_ids( $activity_ids, 'sz_activity' );
		...
		if ( ! empty( $uncached_ids ) ) {
			...
			$queried_adata = $wpdb->get_results( "SELECT * FROM {$sz->activity->table_name} WHERE id IN ({$uncached_ids_sql})");

			// Put that data into the placeholders created earlier,
			// and add it to the cache.
			foreach ( (array) $queried_adata as $adata ) {
				wp_cache_set( $adata->id, $adata, 'sz_activity' );
			}
		}
		...
		foreach ( $activity_ids as $activity_id ) {
			// Integer casting.
			$activity = wp_cache_get( $activity_id, 'sz_activity' );
			if ( ! empty( $activity ) ) {
		...
		$user_query = new SZ_User_Query( array(
	...
	protected static function append_user_fullnames( $activities ) {
		...
		if ( sz_is_active( 'xprofile' ) && ! empty( $activities ) ) {
			$activity_user_ids = wp_list_pluck( $activities, 'user_id' );

			if ( ! empty( $activity_user_ids ) ) {
				$fullnames = sz_core_get_user_displaynames( $activity_user_ids );
				if ( ! empty( $fullnames ) ) {
	 ...
	 * SportsZone data objects. Here, we pre-fetch data about these
	 ...
	 * info is fetched separately in SZ_Activity_Activity::get_activity_data().
	 * So this method contains nothing but a filter that allows other
	 * components, such as sz-friends and sz-groups, to hook in and prime
	...
	protected static function prefetch_object_data( $activities ) {
		 ...
		 * @param array $activities Array of activities.
		 */
		return apply_filters( 'sz_activity_prefetch_object_data', $activities );
	}

	/**
	 * Generate action strings for the activities located in SZ_Activity_Activity::get().
	 *
	...
	protected static function generate_action_strings( $activities ) {
		foreach ( $activities as $key => $activity ) {
			$generated_action = sz_activity_generate_action_string( $activity );
	...
	/**
	 * Get the SQL for the 'meta_query' param in SZ_Activity_Activity::get().
	 *
	 * We use WP_Meta_Query to do the heavy lifting of parsing the
	 * meta_query array and creating the necessary SQL clauses. However,
	 * since SZ_Activity_Activity::get() builds its SQL differently than
	...
	public static function get_meta_query_sql( $meta_query = array() ) {
		...
		if ( ! empty( $meta_query ) ) {
			...
			// $wpdb->activitymeta.
			$wpdb->activitymeta = sportszone()->activity->table_name_meta;
	...
	/**
	 * Get the SQL for the 'date_query' param in SZ_Activity_Activity::get().
	 *
	 * We use SZ_Date_Query, which extends WP_Date_Query, to do the heavy lifting
	 * of parsing the date_query array and creating the necessary SQL clauses.
	 * However, since SZ_Activity_Activity::get() builds its SQL differently than
	...
	public static function get_date_query_sql( $date_query = array() ) {
		...
		if ( ! empty( $date_query ) && is_array( $date_query ) ) {
			$date_query = new SZ_Date_Query( $date_query, 'date_recorded' );
	...
	/**
	 * Get the SQL for the 'scope' param in SZ_Activity_Activity::get().
	 *
	 ...
	 * @param  array $r      Current activity arguments. Same as those of SZ_Activity_Activity::get(),
	...
	public static function get_scope_query_sql( $scope = false, $r = array() ) {
		...
		foreach ( $scopes as $scope ) {
			 ...
			 *   - 'sz_activity_set_groups_scope_args'
			 *   - 'sz_activity_set_friends_scope_args'
			 *
			 * To see how this filter is used, plugin devs should check out:
			 *   - sz_groups_filter_activity_scope() - used for 'groups' scope
			 *   - sz_friends_filter_activity_scope() - used for 'friends' scope
			 *
			 * @since 2.2.0
			 *
			 * @param array {
			 ...
			 *         See {@link SZ_Activity_Query::_construct()} for more details.
			 *     }
			 *     @type array  $override Optional. Override existing activity arguments passed by $r.
			 *     }
			 * }
			 * @param array $r Current activity arguments passed in SZ_Activity_Activity::get().
			 */
			$scope_args = apply_filters( "sz_activity_set_{$scope}_scope_args", array(), $r );
		...
		if ( ! empty( $query_args ) ) {
			...
			$query = new SZ_Activity_Query( $query_args );
			$sql   = $query->get_sql();
			if ( ! empty( $sql ) ) {
				$retval['sql'] = $sql;
			}
	...
	/**
	 * In SportsZone 1.2.x, this was used to retrieve specific activity stream items (for example, on an activity's permalink page).
	 *
	 * As of 1.5.x, use SZ_Activity_Activity::get() with an 'in' parameter instead.
	 *
	 * @since 1.2.0
	 *
	 * @deprecated 1.5
	 * @deprecated Use SZ_Activity_Activity::get() with an 'in' parameter instead.
	 *
	...
	public static function get_specific( $activity_ids, $max = false, $page = 1, $per_page = 25, $sort = 'DESC', $display_comments = false ) {
		_deprecated_function( __FUNCTION__, '1.5', 'Use SZ_Activity_Activity::get() with the "in" parameter instead.' );
		return SZ_Activity_Activity::get( $max, $page, $per_page, $sort, false, false, $display_comments, false, false, $activity_ids );
	}
	...
	public static function get_id( $user_id, $component, $type, $item_id, $secondary_item_id, $action, $content, $date_recorded ) {
		...
		if ( ! empty( $where_args ) ) {
			...
			$result = $wpdb->get_var( "SELECT id FROM {$sz->activity->table_name} {$where_sql}" );
	...
	public static function delete( $args = array() ) {
		...
		$activities = $wpdb->get_results( "SELECT * FROM {$sz->activity->table_name} {$where_sql}" );
		 ...
		 */
		do_action_ref_array( 'sz_activity_before_delete', array( $activities, $r ) );

		// Attempt to delete activities from the database.
		$deleted = $wpdb->query( "DELETE FROM {$sz->activity->table_name} {$where_sql}" );
		 ...
		 * @param array $r          Array of parsed arguments.
		 */
		do_action_ref_array( 'sz_activity_after_delete', array( $activities, $r ) );
		...
		if ( ! empty( $activity_ids ) ) {
			...
			SZ_Activity_Activity::delete_activity_meta_entries( $activity_ids );
			...
			foreach ( $activity_ids as $activity_id ) {
				...
				$comments = SZ_Activity_Activity::delete( array(
	 ...
	 *
	 * This method is no longer used by SportsZone, and it is recommended not to
	 * use it going forward, and use SZ_Activity_Activity::delete() instead.
	 *
	 * @since 1.2.0
	 *
	 * @deprecated 2.3.0
	...
	public static function delete_activity_item_comments( $activity_ids = array(), $delete_meta = true ) {
		...
		if ( $delete_meta ) {
			...
			$activity_comment_ids = $wpdb->get_col( "SELECT id FROM {$sz->activity->table_name} WHERE type = 'activity_comment' AND item_id IN ({$activity_ids})" );
		...
		return $wpdb->query( "DELETE FROM {$sz->activity->table_name} WHERE type = 'activity_comment' AND item_id IN ({$activity_ids})" );
	}
	...
	public static function delete_activity_meta_entries( $activity_ids = array() ) {
		...
		foreach ( $activity_ids as $activity_id ) {
			sz_activity_delete_meta( $activity_id );
		}
	...
	public static function append_comments( $activities, $spam = 'ham_only' ) {
		...
		foreach ( (array) $activities as $activity ) {
			...
			$activity_comments[$activity->id] = SZ_Activity_Activity::get_activity_comments( $activity->id, $activity->mptt_left, $activity->mptt_right, $spam, $top_level_parent_id );
		}
	...
	public static function get_activity_comments( $activity_id, $left, $right, $spam = 'ham_only', $top_level_parent_id = 0 ) {
		...
		$comments = wp_cache_get( $activity_id, 'sz_activity_comments' );
		...
		if ( 'none' === $comments ) {
			...
			if ( sz_is_active( 'xprofile' ) ) {
				...
				$fullname_from = ", {$sz->profile->table_name_data} pd ";
			...
			/**
			 * Filters if SportsZone should use the legacy activity query.
			 *
			 * @since 2.0.0
			 *
			 * @param bool                 $value     Whether or not to use the legacy query.
			 * @param SZ_Activity_Activity $value     Magic method referring to currently called method.
			 * @param array                $func_args Array of the method's argument list.
			 */
			if ( apply_filters( 'sz_use_legacy_activity_query', false, __METHOD__, func_get_args() ) ) {
				 ...
				 */
				$sql = apply_filters( 'sz_activity_comments_user_join_filter', $wpdb->prepare( "SELECT a.*, u.user_email, u.user_nicename, u.user_login, u.display_name{$fullname_select} FROM {$sz->activity->table_name} a, {$wpdb->users} u{$fullname_from} WHERE u.ID = a.user_id {$fullname_where} AND a.type = 'activity_comment' {$spam_sql} AND a.item_id = %d AND a.mptt_left > %d AND a.mptt_left < %d ORDER BY a.date_recorded ASC", $top_level_parent_id, $left, $right ), $activity_id, $left, $right, $spam_sql );
				...
				$sql = $wpdb->prepare( "SELECT id FROM {$sz->activity->table_name} a WHERE a.type = 'activity_comment' {$spam_sql} AND a.item_id = %d and a.mptt_left > %d AND a.mptt_left < %d ORDER BY a.date_recorded ASC", $top_level_parent_id, $left, $right );
			...
			foreach ( $ref as &$r ) {
				...
				while ( $parent_id !== $r->item_id ) {
					...
					if ( empty( $ref[ $parent_id ] ) ) {
						$direct_parent = new SZ_Activity_Activity( $parent_id );
			...
			wp_cache_set( $activity_id, $cache_value, 'sz_activity_comments' );
		}
	...
	public static function rebuild_activity_comment_tree( $parent_id, $left = 1 ) {
		...
		$comments    = SZ_Activity_Activity::get_child_comments( $parent_id );
		...
		foreach ( (array) $descendants as $descendant_id ) {
			$right = SZ_Activity_Activity::rebuild_activity_comment_tree( $descendant_id, $right );
		}

		// We've got the left value, and now that we've processed the children
		// of this node we also know the right value.
		if ( 1 === $left ) {
			$wpdb->query( $wpdb->prepare( "UPDATE {$sz->activity->table_name} SET mptt_left = %d, mptt_right = %d WHERE id = %d", $left, $right, $parent_id ) );
		} else {
			$wpdb->query( $wpdb->prepare( "UPDATE {$sz->activity->table_name} SET mptt_left = %d, mptt_right = %d WHERE type = 'activity_comment' AND id = %d", $left, $right, $parent_id ) );
		}
	...
	public static function get_child_comments( $parent_id ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT id FROM {$sz->activity->table_name} WHERE type = 'activity_comment' AND secondary_item_id = %d", $parent_id ) );
	}
	...
	public static function get_recorded_components( $skip_last_activity = true ) {
		...
		if ( true === $skip_last_activity ) {
			$components = $wpdb->get_col( "SELECT DISTINCT component FROM {$sz->activity->table_name} WHERE action != '' AND action != 'last_activity' ORDER BY component ASC" );
		} else {
			$components = $wpdb->get_col( "SELECT DISTINCT component FROM {$sz->activity->table_name} ORDER BY component ASC" );
		}
	...
	public static function get_sitewide_items_for_feed( $limit = 35 ) {
		$activities    = sz_activity_get_sitewide( array( 'max' => $limit ) );
		$activity_feed = array();
	 ...
	 * @since 1.5.0
	 *
	 * @see SZ_Activity_Activity::get_filter_sql()
	 *
	...
	public static function get_filter_sql( $filter_array ) {
		...
		if ( !empty( $filter_array['user_id'] ) ) {
			$user_sql = SZ_Activity_Activity::get_in_operator_sql( 'a.user_id', $filter_array['user_id'] );
			if ( !empty( $user_sql ) )
				$filter_sql[] = $user_sql;
		}

		if ( !empty( $filter_array['object'] ) ) {
			$object_sql = SZ_Activity_Activity::get_in_operator_sql( 'a.component', $filter_array['object'] );
			if ( !empty( $object_sql ) )
				$filter_sql[] = $object_sql;
		}

		if ( !empty( $filter_array['action'] ) ) {
			$action_sql = SZ_Activity_Activity::get_in_operator_sql( 'a.type', $filter_array['action'] );
			if ( ! empty( $action_sql ) )
				$filter_sql[] = $action_sql;
		}

		if ( !empty( $filter_array['primary_id'] ) ) {
			$pid_sql = SZ_Activity_Activity::get_in_operator_sql( 'a.item_id', $filter_array['primary_id'] );
			if ( !empty( $pid_sql ) )
				$filter_sql[] = $pid_sql;
		}

		if ( !empty( $filter_array['secondary_id'] ) ) {
			$sid_sql = SZ_Activity_Activity::get_in_operator_sql( 'a.secondary_item_id', $filter_array['secondary_id'] );
			if ( !empty( $sid_sql ) )
				$filter_sql[] = $sid_sql;
		}
	...
	public static function get_last_updated() {
		...
		return $wpdb->get_var( "SELECT date_recorded FROM {$sz->activity->table_name} ORDER BY date_recorded DESC LIMIT 1" );
	}
	...
	public static function total_favorite_count( $user_id ) {
		...
		$favorite_activity_entries = sz_get_user_meta( $user_id, 'sz_favorite_activities', true );
	...
	public static function check_exists_by_content( $content ) {
		...
		$result = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$sz->activity->table_name} WHERE content = %s", $content ) );
	...
	public static function hide_all_for_user( $user_id ) {
		...
		return $wpdb->get_var( $wpdb->prepare( "UPDATE {$sz->activity->table_name} SET hide_sitewide = 1 WHERE user_id = %d", $user_id ) );
	}
}
