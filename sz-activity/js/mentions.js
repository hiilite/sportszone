...
( function( bp, $, undefined ) {
	...
	if ( typeof window.SZ_Suggestions === 'object' ) {
		bp.mentions.users = window.SZ_Suggestions.friends || bp.mentions.users;
	}

	/**
	 * Adds SportsZone @mentions to form inputs.
	 *
	 * @param {array|object} options If array, becomes the suggestions' data source. If object, passed as config to $.atwho().
	 * @since 2.1.0
	 */
	$.fn.sz_mentions = function( options ) {
		if ( $.isArray( options ) ) {
		...
		mentionsDefaults = {
			callbacks: { 
				...
				remoteFilter: function( query, render_view ) {
					...
					params = { 'action': 'sz_get_suggestions', 'term': query, 'type': 'members' };
	...
	$( document ).ready(function() {
		...
		$( '.sz-suggestions, #comments form textarea, .wp-editor-area' ).sz_mentions( bp.mentions.users );
	});
