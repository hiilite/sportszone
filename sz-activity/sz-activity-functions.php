...
/**
 * Check whether the $sz global lists an activity directory page.
 *
 * @since 1.5.0
 *
 * @return bool True if activity directory page is found, otherwise false.
 */
function sz_activity_has_directory() {
	return (bool) !empty( sportszone()->pages->activity->id );
}
 ...
 * sz-custom.php or your theme's functions.php file:
 *
 *   add_filter( 'sz_activity_do_mentions', '__return_false' );
 *
 * @since 1.8.0
 *
 * @return bool $retval True to enable mentions, false to disable.
 */
function sz_activity_do_mentions() {
	 ...
	 */
	return (bool) apply_filters( 'sz_activity_do_mentions', true );
}
 ...
 */
function sz_activity_maybe_load_mentions_scripts() {
	$mentions_enabled = sz_activity_do_mentions() && sz_is_user_active();
	$load_mentions    = $mentions_enabled && ( sz_is_activity_component() || is_admin() );

	/**
	 * Filters whether or not SportsZone should load mentions scripts and assets.
	 *
	 * @since 2.1.0
	 *
	 ...
	 */
	return (bool) apply_filters( 'sz_activity_maybe_load_mentions_scripts', $load_mentions, $mentions_enabled );
}
 ...
 */
function sz_activity_find_mentions( $content ) {
	...
	foreach( (array) array_values( $usernames ) as $username ) {
		$user_id = sz_activity_get_userid_from_mentionname( $username );
	 ...
	 */
	return apply_filters( 'sz_activity_mentioned_users', $mentioned_users );
}
 ...
 */
function sz_activity_clear_new_mentions( $user_id ) {
	sz_delete_user_meta( $user_id, 'sz_new_mention_count' );
	sz_delete_user_meta( $user_id, 'sz_new_mentions'      );
	 ...
	 * @param  int $user_id The id of the user whose unread mentions are being reset.
	 */
	do_action( 'sz_activity_clear_new_mentions', $user_id );
}
 ...
 *
 * Currently, only used in {@link sz_activity_delete()}.
 *
 * @since 1.5.0
 *
 ...
 * @return bool
 */
function sz_activity_adjust_mention_count( $activity_id = 0, $action = 'add' ) {
	...
	$usernames = sz_activity_find_mentions( strip_tags( $activity->content ) );
	...
	foreach( (array) array_keys( $usernames ) as $user_id ) {
		sz_activity_update_mention_count_for_user( $user_id, $activity_id, $action );
	}
}
 ...
 * @return bool
 */
function sz_activity_update_mention_count_for_user( $user_id, $activity_id, $action = 'add' ) {
	...
	$new_mention_count = (int) sz_get_user_meta( $user_id, 'sz_new_mention_count', true );
	...
	sz_update_user_meta( $user_id, 'sz_new_mention_count', $new_mention_count );
	sz_update_user_meta( $user_id, 'sz_new_mentions',      $new_mentions );
 ...
 */
function sz_activity_get_user_mentionname( $user_id ) {
	$mentionname = '';

	$userdata = sz_core_get_core_userdata( $user_id );
 ...
 */
function sz_activity_get_userid_from_mentionname( $mentionname ) {
	$user_id = false;
	...
	// the same as the nicename.
		...
		$user_id = sz_core_get_userid_from_nicename( $mentionname );
	}
 ...
 * not registered using sz_activity_set_action(), it is not recommended;
 ...
 */
function sz_activity_set_action( $component_id, $type, $description, $format_callback = false, $label = false, $context = array(), $position = 0 ) {
	$sz = sportszone();
	...
	// Set activity action.
	if ( ! isset( $sz->activity->actions ) || ! is_object( $sz->activity->actions ) ) {
		$sz->activity->actions = new stdClass;
	}
	...
	if ( ! isset( $sz->activity->actions->{$component_id} ) || ! is_object( $sz->activity->actions->{$component_id} ) ) {
		$sz->activity->actions->{$component_id} = new stdClass;
	}
	 ...
	 *                                  'member_groups', 'group'.
	 */
	$sz->activity->actions->{$component_id}->{$type} = apply_filters( 'sz_activity_set_action', array(
		'key'             => $type,
		'value'           => $description,
	...
	$action_array = (array) $sz->activity->actions->{$component_id};
	$action_array = sz_sort_by_key( $action_array, 'position', 'num' );

	// Restore keys.
	$sz->activity->actions->{$component_id} = new stdClass;
	foreach ( $action_array as $key_ordered ) {
		$sz->activity->actions->{$component_id}->{$key_ordered['key']} = $key_ordered;
	}
 ...
 *     @type string   $sz_activity_admin_filter String to use in the Dashboard > Activity dropdown.
 *     @type string   $sz_activity_front_filter String to use in the front-end dropdown.
 *     @type string   $sz_activity_new_post     String format to use for generating the activity action. Should be a
 *                                              translatable string where %1$s is replaced by a user link and %2$s is
 *                                              the URL of the newly created post.
 *     @type string   $sz_activity_new_post_ms  String format to use for generating the activity action on Multisite.
 ...
 *                                              the site.
 *     @type string   $component_id             ID of the SportsZone component to associate the activity item.
 ...
 *                                              Default: 'sz_activity_format_activity_action_custom_post_type_post'.
 ...
 * @return bool
 */
function sz_activity_set_post_type_tracking_args( $post_type = '', $args = array() ) {
	global $wp_post_types;

	if ( empty( $wp_post_types[ $post_type ] ) || ! post_type_supports( $post_type, 'sportszone-activity' ) || ! is_array( $args ) ) {
		return false;
	}

	$activity_labels = array(
		/* Post labels */
		'sz_activity_admin_filter',
		'sz_activity_front_filter',
		'sz_activity_new_post',
		'sz_activity_new_post_ms',
		/* Comment labels */
		'sz_activity_comments_admin_filter',
		'sz_activity_comments_front_filter',
		'sz_activity_new_comment',
		'sz_activity_new_comment_ms'
	);
	...
	// If there are any additional args, put them in the sz_activity attribute of the post type.
	if ( ! empty( $args ) ) {
		$wp_post_types[ $post_type ]->sz_activity = $args;
	}
}
 ...
 */
function sz_activity_get_post_type_tracking_args( $post_type ) {
	if ( ! post_type_supports( $post_type, 'sportszone-activity' ) ) {
		return false;
	}
	...
	$post_type_activity = array(
		'component_id'            => sportszone()->activity->id,
		'action_id'               => 'new_' . $post_type,
		'format_callback'         => 'sz_activity_format_activity_action_custom_post_type_post',
		...
		'comment_format_callback' => 'sz_activity_format_activity_action_custom_post_type_comment',
	);

	if ( ! empty( $post_type_object->sz_activity ) ) {
		$post_type_activity = sz_parse_args( (array) $post_type_object->sz_activity, $post_type_activity, $post_type . '_tracking_args' );
	}

	$post_type_activity = (object) $post_type_activity;

	// Try to get the admin filter from the post type labels.
	if ( ! empty( $post_type_object->labels->sz_activity_admin_filter ) ) {
		$post_type_activity->admin_filter = $post_type_object->labels->sz_activity_admin_filter;

	// Fall back to a generic name.
	} else {
		$post_type_activity->admin_filter = _x( 'New item published', 'Post Type generic activity post admin filter', 'sportszone' );
	}

	// Check for the front filter in the post type labels.
	if ( ! empty( $post_type_object->labels->sz_activity_front_filter ) ) {
		$post_type_activity->front_filter = $post_type_object->labels->sz_activity_front_filter;
	}

	// Try to get the action for new post type action on non-multisite installations.
	if ( ! empty( $post_type_object->labels->sz_activity_new_post ) ) {
		$post_type_activity->new_post_type_action = $post_type_object->labels->sz_activity_new_post;
	}

	// Try to get the action for new post type action on multisite installations.
	if ( ! empty( $post_type_object->labels->sz_activity_new_post_ms ) ) {
		$post_type_activity->new_post_type_action_ms = $post_type_object->labels->sz_activity_new_post_ms;
	}
	...
	if ( $post_type_support_comments && ! empty( $post_type_activity->comment_action_id ) ) {
		...
		if ( ! empty( $post_type_object->labels->sz_activity_comments_admin_filter ) ) {
			$post_type_activity->comments_tracking->admin_filter = $post_type_object->labels->sz_activity_comments_admin_filter;

		// Fall back to a generic name.
		} else {
			$post_type_activity->comments_tracking->admin_filter = _x( 'New item comment posted', 'Post Type generic comments activity admin filter', 'sportszone' );
		}
		...
		if ( ! empty( $post_type_object->labels->sz_activity_comments_front_filter ) ) {
			$post_type_activity->comments_tracking->front_filter = $post_type_object->labels->sz_activity_comments_front_filter;

		// Fall back to a generic name.
		} else {
			$post_type_activity->comments_tracking->front_filter = _x( 'Item comments', 'Post Type generic comments activity front filter', 'sportszone' );
		}
		...
		if ( ! empty( $post_type_object->labels->sz_activity_new_comment ) ) {
			$post_type_activity->comments_tracking->new_post_type_comment_action = $post_type_object->labels->sz_activity_new_comment;
		}

		// Try to get the action for new post type comment action on multisite installations.
		if ( ! empty( $post_type_object->labels->sz_activity_new_comment_ms ) ) {
			$post_type_activity->comments_tracking->new_post_type_comment_action_ms = $post_type_object->labels->sz_activity_new_comment_ms;
		}
	}
	 ...
	 * @param string $post_type          Name of the post type.
	 */
	return apply_filters( 'sz_activity_get_post_type_tracking_args', $post_type_activity, $post_type );
}
 ...
 */
function sz_activity_get_post_types_tracking_args() {
	...
	foreach ( $post_types as $post_type ) {
		$track_post_type = sz_activity_get_post_type_tracking_args( $post_type );
	 ...
	 */
	return apply_filters( 'sz_activity_get_post_types_tracking_args', $post_types_tracking_args );
}
 ...
 * @return bool
 */
function sz_activity_type_supports( $activity_type = '', $feature = '' ) {
	$retval = false;
	...
	switch ( $feature ) {
		...
		case 'post-type-comment-tracking' :
			...
			if ( empty( $sz->activity->track ) ) {
				$sz->activity->track = sz_activity_get_post_types_tracking_args();
			}

			if ( ! empty( $sz->activity->track[ $activity_type ]->comments_tracking ) ) {
				$retval = true;
			}
			break;
		...
		case 'post-type-comment-reply' :
			...
			if ( ! empty( $sz->activity->track[ $activity_type ]->comments_tracking ) && ! empty( $sz->activity->track[ $activity_type ]->comment_action_id ) ) {
				$retval = true;
			}
			break;
		...
		case 'comment-reply' :
			...
			if ( ! empty( $sz->activity->track[ $activity_type ] ) ) {
				if ( isset( $sz->activity->track[ $activity_type ]->activity_comment ) ) {
					$retval = $sz->activity->track[ $activity_type ]->activity_comment;
				}

				// Eventually override with comment synchronization feature.
				if ( isset( $sz->activity->track[ $activity_type ]->comments_tracking ) ) {
					$retval = $sz->activity->track[ $activity_type ]->comments_tracking && ! sz_disable_blogforum_comments();
				}

			// Retired Forums component
			} elseif ( 'new_forum_topic' === $activity_type || 'new_forum_post' === $activity_type ) {
				$retval = ! sz_disable_blogforum_comments();
 ...
 */
function sz_activity_post_type_get_tracking_arg( $activity_type, $arg = '' ) {
	...
	if ( isset( $sz->activity->track[ $activity_type ]->{$arg} ) ) {
		return $sz->activity->track[ $activity_type ]->{$arg};
	} else {
		return false;
	}
}
 ...
 */
function sz_activity_get_actions() {
	$sz = sportszone();
	...
	if ( ! empty( $sz->activity->track ) ) {
		foreach ( $sz->activity->track as $post_type ) {
			if ( isset( $sz->activity->actions->{$post_type->component_id}->{$post_type->action_id} ) ) {
				continue;
			}
 ...
 */
function sz_activity_get_action( $component_id, $key ) {
	...
	$actions = sz_activity_get_actions();
	$retval  = false;
	 ...
	 * @param string      $key          The action key.
	 */
	return apply_filters( 'sz_activity_get_action', $retval, $component_id, $key );
}
 ...
 */
function sz_activity_get_types() {
	$actions  = array();

	// Walk through the registered actions, and build an array of actions/values.
	foreach ( sz_activity_get_actions() as $action ) {
	 ...
	 */
	return apply_filters( 'sz_activity_get_types', $actions );
}
 ...
 */
function sz_activity_get_current_context() {
 ...
 * @return array
 */
function sz_activity_get_actions_for_context( $context = '' ) {
	if ( ! $context ) {
		$context = sz_activity_get_current_context();
	}

	$actions = array();
	foreach ( sz_activity_get_actions() as $component_actions ) {
 ...
 */
function sz_activity_get_user_favorites( $user_id = 0 ) {
	...
	$favs = sz_get_user_meta( $user_id, 'sz_favorite_activities', true );
	 ...
	 */
	return apply_filters( 'sz_activity_get_user_favorites', $favs );
}
 ...
 */
function sz_activity_add_user_favorite( $activity_id, $user_id = 0 ) {
	...
	$my_favs = sz_get_user_meta( $user_id, 'sz_favorite_activities', true );
	...
	$fav_count = sz_activity_get_meta( $activity_id, 'favorite_count' );
	$fav_count = !empty( $fav_count ) ? (int) $fav_count + 1 : 1;

	// Update user meta.
	sz_update_user_meta( $user_id, 'sz_favorite_activities', $my_favs );

	// Update activity meta counts.
	if ( sz_activity_update_meta( $activity_id, 'favorite_count', $fav_count ) ) {

		/**
		 * Fires if sz_activity_update_meta() for favorite_count is successful and before returning a true value for success.
		 *
		 * @since 1.2.1
		 *
		 * @param int $activity_id ID of the activity item being favorited.
		 ...
		 */
		do_action( 'sz_activity_add_user_favorite', $activity_id, $user_id );
	...
	// Saving meta was unsuccessful for an unknown reason.
		...
		/**
		 * Fires if sz_activity_update_meta() for favorite_count is unsuccessful and before returning a false value for failure.
		 *
		 * @since 1.5.0
		 *
		 * @param int $activity_id ID of the activity item being favorited.
		 * @param int $user_id     ID of the user doing the favoriting.
		 */
		do_action( 'sz_activity_add_user_favorite_fail', $activity_id, $user_id );
 ...
 * @return bool True on success, false on failure.
 */
function sz_activity_remove_user_favorite( $activity_id, $user_id = 0 ) {
	...
	if ( ! empty( $fav_count ) ) {
		...
		if ( sz_activity_update_meta( $activity_id, 'favorite_count', (int) $fav_count - 1 ) ) {

			// Update users favorites.
			if ( sz_update_user_meta( $user_id, 'sz_favorite_activities', $my_favs ) ) {

				/**
				 * Fires if sz_update_user_meta() is successful and before returning a true value for success.
				 *
				 * @since 1.2.1
				 *
				 * @param int $activity_id ID of the activity item being unfavorited.
				 ...
				 */
				do_action( 'sz_activity_remove_user_favorite', $activity_id, $user_id );
 ...
 */
function sz_activity_check_exists_by_content( $content ) {
	 ...
	 * @since 1.1.0
	 *
	 * @param SZ_Activity_Activity $value ID of the activity if found, else null.
	 */
	return apply_filters( 'sz_activity_check_exists_by_content', SZ_Activity_Activity::check_exists_by_content( $content ) );
}
 ...
 */
function sz_activity_get_last_updated() {
	 ...
	 * @since 1.1.0
	 *
	 * @param SZ_Activity_Activity $last_updated Date last updated.
	 */
	return apply_filters( 'sz_activity_get_last_updated', SZ_Activity_Activity::get_last_updated() );
}
 ...
 */
function sz_activity_total_favorites_for_user( $user_id = 0 ) {
	...
	return SZ_Activity_Activity::total_favorite_count( $user_id );
}
 ...
 * @return bool True on success, false on failure.
 */
function sz_activity_delete_meta( $activity_id, $meta_key = '', $meta_value = '', $delete_all = false ) {

	// Legacy - if no meta_key is passed, delete all for the item.
	if ( empty( $meta_key ) ) {
		$all_meta = sz_activity_get_meta( $activity_id );
 ...
 */
function sz_activity_get_meta( $activity_id = 0, $meta_key = '', $single = true ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
	 ...
	 */
	return apply_filters( 'sz_activity_get_meta', $retval, $activity_id, $meta_key, $single );
}
 ...
 */
function sz_activity_update_meta( $activity_id, $meta_key, $meta_value, $prev_value = '' ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
 ...
 */
function sz_activity_add_meta( $activity_id, $meta_key, $meta_value, $unique = false ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
 ...
 * @return bool
 */
function sz_activity_remove_all_user_data( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
		return false;
	}

	// Clear the user's activity from the sitewide stream and clear their activity tables.
	sz_activity_delete( array( 'user_id' => $user_id ) );

	// Remove any usermeta.
	sz_delete_user_meta( $user_id, 'sz_latest_update'       );
	sz_delete_user_meta( $user_id, 'sz_favorite_activities' );

	// Execute additional code
	do_action( 'sz_activity_remove_data', $user_id ); // Deprecated! Do not use!
	 ...
	 */
	do_action( 'sz_activity_remove_all_user_data', $user_id );
}
add_action( 'wpmu_delete_user',  'sz_activity_remove_all_user_data' );
add_action( 'delete_user',       'sz_activity_remove_all_user_data' );
 ...
 * @return bool
 */
function sz_activity_spam_all_user_data( $user_id = 0 ) {
	global $wpdb;
	...
	$activities = sz_activity_get( array(
	...
	foreach ( (array) $activities['activities'] as $activity ) {
		...
		$activity_obj = new SZ_Activity_Activity;
		foreach ( $activity as $k => $v ) {
			$activity_obj->$k = $v;
		}

		// Mark as spam.
		sz_activity_mark_as_spam( $activity_obj );

		/*
		 * If Akismet is present, update the activity history meta.
		 *
		 * This is usually taken care of when SZ_Activity_Activity::save() happens, but
		 * as we're going to be updating all the activity statuses directly, for efficiency,
		 * we need to update manually.
		 */
		if ( ! empty( $sz->activity->akismet ) ) {
			$sz->activity->akismet->update_activity_spam_meta( $activity_obj );
		}
	...
	$wpdb->query( $wpdb->prepare( "UPDATE {$sz->activity->table_name} SET is_spam = 1 WHERE user_id = %d", $user_id ) );
	 ...
	 */
	do_action( 'sz_activity_spam_all_user_data', $user_id, $activities['activities'] );
}
add_action( 'sz_make_spam_user', 'sz_activity_spam_all_user_data' );
 ...
 * @return bool
 */
function sz_activity_ham_all_user_data( $user_id = 0 ) {
	global $wpdb;
	...
	foreach ( (array) $activities['activities'] as $activity ) {
		...
		// Mark as not spam.
		sz_activity_mark_as_ham( $activity_obj );
		 ...
		 */
		if ( ! empty( $sz->activity->akismet ) ) {
			$sz->activity->akismet->update_activity_ham_meta( $activity_obj );
		}
	...
	$wpdb->query( $wpdb->prepare( "UPDATE {$sz->activity->table_name} SET is_spam = 0 WHERE user_id = %d", $user_id ) );
	 ...
	 */
	do_action( 'sz_activity_ham_all_user_data', $user_id, $activities['activities'] );
}
add_action( 'sz_make_ham_user', 'sz_activity_ham_all_user_data' );
 ...
 * @since 1.2.0
 */
function sz_register_activity_actions() {
	/**
	 * Fires on sz_init to allow core components and dependent plugins to register activity actions.
	 *
	 * @since 1.2.0
	 */
	do_action( 'sz_register_activity_actions' );
}
add_action( 'sz_init', 'sz_register_activity_actions', 8 );
 ...
 * @since 1.6.0
 */
function sz_activity_register_activity_actions() {
	$sz = sportszone();
	...
	sz_activity_set_action(
		$sz->activity->id,
		'activity_comment',
		__( 'Replied to a status update', 'sportszone' ),
		'sz_activity_format_activity_action_activity_comment',
		__( 'Activity Comments', 'sportszone' )
	);
	 ...
	 * @since 1.6.0
	 */
	do_action( 'sz_activity_register_activity_actions' );

	// Backpat. Don't use this.
	do_action( 'updates_register_activity_actions' );
}
add_action( 'sz_register_activity_actions', 'sz_activity_register_activity_actions' );
 ...
 */
function sz_activity_generate_action_string( $activity ) {
	...
	add_filter( 'sz_activity_generate_action_string', $actions->{$activity->component}->{$activity->type}['format_callback'], 10, 2 );
	 ...
	 * @since 2.0.0
	 *
	 * @param SZ_Activity_Activity $action   Action string being requested.
	 * @param SZ_Activity_Activity $activity Activity item object.
	 */
	$action = apply_filters( 'sz_activity_generate_action_string', $activity->action, $activity );

	// Remove the filter for future activity items.
	remove_filter( 'sz_activity_generate_action_string', $actions->{$activity->component}->{$activity->type}['format_callback'], 10 );
 ...
 */
function sz_activity_format_activity_action_activity_update( $action, $activity ) {
	$action = sprintf( __( '%s posted an update', 'sportszone' ), sz_core_get_userlink( $activity->user_id ) );
	 ...
	 * @param SZ_Activity_Activity $activity Activity item object.
	 */
	return apply_filters( 'sz_activity_new_update_action', $action, $activity );
}
 ...
 * @return string $action
 */
function sz_activity_format_activity_action_activity_comment( $action, $activity ) {
	$action = sprintf( __( '%s posted a new activity comment', 'sportszone' ), sz_core_get_userlink( $activity->user_id ) );
	 ...
	 * @param SZ_Activity_Activity $activity Activity item object.
	 */
	return apply_filters( 'sz_activity_comment_action', $action, $activity );
}
 ...
 * @return string $action
 */
function sz_activity_format_activity_action_custom_post_type_post( $action, $activity ) {
	$sz = sportszone();
	...
	if ( empty( $activity->type ) || empty( $sz->activity->track[ $activity->type ] ) ) {
		return $action;
	}
	...
	if ( is_multisite() ) {
		...
		if ( ! empty( $sz->activity->track[ $activity->type ]->new_post_type_action_ms ) ) {
			$action = sprintf( $sz->activity->track[ $activity->type ]->new_post_type_action_ms, $user_link, $post_url, $blog_link );
		} else {
			$action = sprintf( _x( '%1$s wrote a new <a href="%2$s">item</a>, on the site %3$s', 'Activity Custom Post Type post action', 'sportszone' ), $user_link, esc_url( $post_url ), $blog_link );
		}
	} else {
		if ( ! empty( $sz->activity->track[ $activity->type ]->new_post_type_action ) ) {
			$action = sprintf( $sz->activity->track[ $activity->type ]->new_post_type_action, $user_link, $post_url );
		} else {
			$action = sprintf( _x( '%1$s wrote a new <a href="%2$s">item</a>', 'Activity Custom Post Type post action', 'sportszone' ), $user_link, esc_url( $post_url ) );
		}
	}
	 ...
	 * @param SZ_Activity_Activity $activity Activity item object.
	 */
	return apply_filters( 'sz_activity_custom_post_type_post_action', $action, $activity );
}
 ...
 * @return string
 */
function sz_activity_format_activity_action_custom_post_type_comment( $action, $activity ) {
	$sz = sportszone();
	...
	if ( is_multisite() ) {
		...
		if ( ! empty( $sz->activity->track[ $activity->type ]->new_post_type_comment_action_ms ) ) {
			$action = sprintf( $sz->activity->track[ $activity->type ]->new_post_type_comment_action_ms, $user_link, $activity->primary_link, $blog_link );
		} else {
			$action = sprintf( _x( '%1$s commented on the <a href="%2$s">item</a>, on the site %3$s', 'Activity Custom Post Type comment action', 'sportszone' ), $user_link, $activity->primary_link, $blog_link );
		}
	} else {
		if ( ! empty( $sz->activity->track[ $activity->type ]->new_post_type_comment_action ) ) {
			$action = sprintf( $sz->activity->track[ $activity->type ]->new_post_type_comment_action, $user_link, $activity->primary_link );
		} else {
			$action = sprintf( _x( '%1$s commented on the <a href="%2$s">item</a>', 'Activity Custom Post Type post comment action', 'sportszone' ), $user_link, $activity->primary_link );
		}
	}
	 ...
	 * @param SZ_Activity_Activity $activity Activity item object.
	 */
	return apply_filters( 'sz_activity_custom_post_type_comment_action', $action, $activity );
}

/*
 * Business functions are where all the magic happens in SportsZone. They will
 ...
 *
 * The sz_activity_get() function shares all arguments with SZ_Activity_Activity::get().
 * The following is a list of sz_activity_get() parameters that have different
 * default values from SZ_Activity_Activity::get() (value in parentheses is
 * the default for the sz_activity_get()).
 *   - 'per_page' (false)
 *
 * @since 1.2.0
 * @since 2.4.0 Introduced the `$fields` parameter.
 *
 * @see SZ_Activity_Activity::get() For more information on accepted arguments
 *      and the format of the returned value.
 *
 * @param array|string $args See SZ_Activity_Activity::get() for description.
 * @return array $activity See SZ_Activity_Activity::get() for description.
 */
function sz_activity_get( $args = '' ) {
	...
	$activity = SZ_Activity_Activity::get( array(
		'page'              => $r['page'],
		'per_page'          => $r['per_page'],
		'max'               => $r['max'],
		'sort'              => $r['sort'],
	 ...
	 * @since 1.2.0
	 *
	 * @param SZ_Activity_Activity $activity Requested activity object.
	 * @param array                $r        Arguments used for the activity query.
	 */
	return apply_filters_ref_array( 'sz_activity_get', array( &$activity, &$r ) );
}
 ...
 *     All arguments and defaults are shared with SZ_Activity_Activity::get(),
 ...
 * @return array $activity See SZ_Activity_Activity::get() for description.
 */
function sz_activity_get_specific( $args = '' ) {
	 ...
	 */
	return apply_filters( 'sz_activity_get_specific', SZ_Activity_Activity::get( $get_args ), $args, $get_args );
}
 ...
 *                                       See {@link sz_activity_set_action()}.
 ...
 */
function sz_activity_add( $args = '' ) {

	$r = sz_parse_args( $args, array(
		'id'                => false,                  // Pass an existing activity ID to update an existing entry.
		'action'            => '',                     // The activity action - e.g. "Jon Doe posted an update"
		'content'           => '',                     // Optional: The content of the activity item e.g. "SportsZone is awesome guys!"
		'component'         => false,                  // The name/ID of the component e.g. groups, profile, mycomponent.
		'type'              => false,                  // The activity type e.g. activity_update, profile_updated.
		'primary_link'      => '',                     // Optional: The primary URL for this item in RSS feeds (defaults to activity permalink).
		'user_id'           => sz_loggedin_user_id(),  // Optional: The user to record the activity for, can be false if this activity is not for a user.
		'item_id'           => false,                  // Optional: The ID of the specific item being recorded, e.g. a blog_id.
		'secondary_item_id' => false,                  // Optional: A second ID used to further filter e.g. a comment_id.
		'recorded_time'     => sz_core_current_time(), // The GMT time that this activity was recorded.
	...
	$activity                    = new SZ_Activity_Activity( $r['id'] );
	...
	$activity->action            = ! empty( $r['action'] )
						? $r['action']
						: sz_activity_generate_action_string( $activity );
	...
	if ( 'activity_comment' === $activity->type ) {
		...
		wp_cache_delete( $activity->item_id, 'sz_activity_comments' );

		SZ_Activity_Activity::rebuild_activity_comment_tree( $activity->item_id );
	}
 ...
 */
function sz_activity_post_update( $args = '' ) {
	...
	if ( sz_is_user_inactive( $r['user_id'] ) ) {
		return false;
	}

	// Record this on the user's profile.
	$activity_content = $r['content'];
	$primary_link     = sz_core_get_userlink( $r['user_id'], false, true );
	 ...
	 */
	$add_content = apply_filters( 'sz_activity_new_update_content', $activity_content );
	 ...
	 */
	$add_primary_link = apply_filters( 'sz_activity_new_update_primary_link', $primary_link );

	// Now write the values.
	$activity_id = sz_activity_add( array(
		'user_id'      => $r['user_id'],
		'content'      => $add_content,
		'primary_link' => $add_primary_link,
		'component'    => sportszone()->activity->id,
	 ...
	 */
	$activity_content = apply_filters( 'sz_activity_latest_update_content', $r['content'], $activity_content );

	// Add this update to the "latest update" usermeta so it can be fetched anywhere.
	sz_update_user_meta( sz_loggedin_user_id(), 'sz_latest_update', array(
		'id'      => $activity_id,
	 ...
	 */
	do_action( 'sz_activity_posted_update', $r['content'], $r['user_id'], $activity_id );
 ...
 */
function sz_activity_post_type_publish( $post_id = 0, $post = null, $user_id = 0 ) {
	...
	$activity_post_object = sz_activity_get_post_type_tracking_args( $post->post_type );
	...
	$existing = sz_activity_get( array(
		'filter' => array(
	 ...
	 */
	if ( false === apply_filters( "sz_activity_{$post->post_type}_pre_publish", true, $blog_id, $post_id, $user_id ) ) {
		return;
	}
	...
	if ( 'blogs' == $activity_post_object->component_id )  {
		$activity_content      = apply_filters( 'sz_blogs_activity_new_post_content', $post->post_content, $post, $post_url, $post->post_type );
		$activity_primary_link = apply_filters( 'sz_blogs_activity_new_post_primary_link', $post_url, $post_id, $post->post_type );
	} else {
	...
	if ( ! empty( $activity_args['content'] ) ) {
		// Create the excerpt.
		$activity_summary = sz_activity_create_summary( $activity_args['content'], $activity_args );

		// Backward compatibility filter for blog posts.
		if ( 'blogs' == $activity_post_object->component_id )  {
			$activity_args['content'] = apply_filters( 'sz_blogs_record_activity_content', $activity_summary, $activity_args['content'], $activity_args, $post->post_type );
		} else {
	...
	if ( empty( $activity_args['action'] ) ) {
		...
		if ( 'blogs' == $activity_post_object->component_id )  {
			$activity_args['action'] = apply_filters( 'sz_blogs_record_activity_action', $activity_args['action'] );
		}
	}

	$activity_id = sz_activity_add( $activity_args );
	 ...
	 */
	do_action( 'sz_activity_post_type_published', $activity_id, $post, $activity_args );
 ...
 */
function sz_activity_post_type_update( $post = null ) {
	...
	$activity_id = sz_activity_get_activity_id( array(
		'component'         => $activity_post_object->component_id,
	...
	if ( ! empty( $post->post_password ) ) {
		sz_activity_delete( array( 'id' => $activity_id ) );
	}
	...
	if ( ! empty( $post->post_content ) ) {
		$activity_summary = sz_activity_create_summary( $post->post_content, (array) $activity );

		// Backward compatibility filter for the blogs component.
		if ( 'blogs' == $activity_post_object->component_id ) {
			$activity->content = apply_filters( 'sz_blogs_record_activity_content', $activity_summary, $post->post_content, (array) $activity, $post->post_type );
		} else {
	 ...
	 */
	do_action( 'sz_activity_post_type_updated', $post, $activity, $activity_post_object );
 ...
 * @return bool True on success, false on failure.
 */
function sz_activity_post_type_unpublish( $post_id = 0, $post = null ) {
	...
	$deleted = sz_activity_delete_by_item_id( $delete_activity_args );
	 ...
	 */
	do_action( 'sz_activity_post_type_unpublished', $delete_activity_args, $post, $deleted );
 ...
 * @return null|WP_Error|bool|int The ID of the activity on success. False on error.
 */
function sz_activity_post_type_comment( $comment_id = 0, $is_approved = true, $activity_post_object = null ) {
	 ...
	 */
	$is_post_status_not_allowed = (bool) apply_filters( 'sz_activity_post_type_is_post_status_allowed', 'publish' !== $post_type_comment->post->post_status || ! empty( $post_type_comment->post->post_password ) );
	...
	if ( empty( $activity_post_object ) ) {
		// Get the post type tracking args.
		$activity_post_object = sz_activity_get_post_type_tracking_args( $post_type );
	 ...
	 */
	if ( false === apply_filters( "sz_activity_{$post_type}_pre_comment", true, $blog_id, $post_type_comment->post->ID, $user_id, $comment_id ) ) {
		return false;
	}
	...
	if ( 'blogs' == $activity_comment_object->component_id )  {
		$activity_content      = apply_filters_ref_array( 'sz_blogs_activity_new_comment_content',      array( $post_type_comment->comment_content, &$post_type_comment, $comment_link ) );
		$activity_primary_link = apply_filters_ref_array( 'sz_blogs_activity_new_comment_primary_link', array( $comment_link, &$post_type_comment ) );
	} else {
	...
	if ( sz_disable_blogforum_comments() ) {
		$blog_url = get_home_url( $blog_id );
		$post_url = add_query_arg(
			'p',
		...
		if ( ! empty( $activity_args['content'] ) ) {
			...
			if ( 'blogs' == $activity_post_object->component_id )  {
				$activity_args['content'] = apply_filters( 'sz_blogs_record_activity_content', $activity_summary, $activity_args['content'], $activity_args, $post_type );
			} else {
				$activity_args['content'] = $activity_summary;
			}
		}
	 ...
	 * @param object     $activity_post_object the post type tracking args object.
	 */
	do_action_ref_array( 'sz_activity_post_type_comment', array( &$activity_id, $post_type_comment, $activity_args, $activity_post_object ) );

	return $activity_id;
}
add_action( 'comment_post', 'sz_activity_post_type_comment', 10, 2 );
add_action( 'edit_comment', 'sz_activity_post_type_comment', 10    );
 ...
 */
function sz_activity_post_type_remove_comment( $comment_id = 0, $activity_post_object = null ) {
	if ( empty( $activity_post_object ) ) {
	...
	if ( sz_disable_blogforum_comments() ) {
		$deleted = sz_activity_delete_by_item_id( array(
			'item_id'           => get_current_blog_id(),
			'secondary_item_id' => $comment_id,
			'component'         => $activity_comment_object->component_id,
			'type'              => $activity_comment_object->action_id,
	 ...
	 */
	do_action( 'sz_activity_post_type_remove_comment', $deleted, $comment_id, $activity_post_object, $activity_comment_object->action_id );

	return $deleted;
}
add_action( 'delete_comment', 'sz_activity_post_type_remove_comment', 10, 1 );
 ...
 */
function sz_activity_new_comment( $args = '' ) {
	$sz = sportszone();
	...
	if ( 'bool' === $r['error_type'] ) {
		if ( empty( $sz->activity->errors ) ) {
			$sz->activity->errors = array();
		}
	}
	...
	if ( empty( $r['content'] ) || empty( $r['user_id'] ) || empty( $r['activity_id'] ) ) {
		...
		// Backpat.
			...
			$sz->activity->errors['new_comment'] = $error;
			return false;
		}
	}
	...
	if ( empty( $activity->date_recorded ) ) {
		$error = new WP_Error( 'missing_activity', __( 'The item you were replying to no longer exists.', 'sportszone' ) );
	 ...
	 *
	 * @since 1.2.0
	 * @since 3.0.0 Added $context parameter to disambiguate from sz_get_activity_comment_content().
	 *
	 * @param string $r       Content for the newly posted comment.
	 * @param string $context This filter's context ("new").
	 */
	$comment_content = apply_filters( 'sz_activity_comment_content', $r['content'], 'new' );

	// Insert the activity comment.
	$comment_id = sz_activity_add( array(
		'id'                => $r['id'],
	...
	wp_cache_delete( $activity_id, 'sz_activity_comments' );

	// Walk the tree to clear caches for all parent items.
	$clear_id = $r['parent_id'];
	while ( $clear_id != $activity_id ) {
		$clear_object = new SZ_Activity_Activity( $clear_id );
		wp_cache_delete( $clear_id, 'sz_activity' );
		$clear_id = intval( $clear_object->secondary_item_id );
	}
	wp_cache_delete( $activity_id, 'sz_activity' );

	if ( empty( $r[ 'skip_notification' ] ) ) {
		/**
		 * Fires near the end of an activity comment posting, before the returning of the comment ID.
		 * Sends a notification to the user @see sz_activity_new_comment_notification_helper().
		 *
		 * @since 1.2.0
		 *
		 * @param int                  $comment_id ID of the newly posted activity comment.
		 * @param array                $r          Array of parsed comment arguments.
		 * @param SZ_Activity_Activity $activity   Activity item being commented on.
		 */
		do_action( 'sz_activity_comment_posted', $comment_id, $r, $activity );
	} else {
		/**
		 * Fires near the end of an activity comment posting, before the returning of the comment ID.
		 ...
		 * @param SZ_Activity_Activity $activity   Activity item being commented on.
		 */
		do_action( 'sz_activity_comment_posted_notification_skipped', $comment_id, $r, $activity );
	}
 ...
 */
function sz_activity_get_activity_id( $args = '' ) {
	 ...
	 *
	 * @param SZ_Activity_Activity $value ID returned by SZ_Activity_Activity get_id() method with provided arguments.
	 * @param array                $r     Parsed function arguments.
	 * @param array                $args  Arguments passed to the function.
	 */
	return apply_filters( 'sz_activity_get_activity_id', SZ_Activity_Activity::get_id(
		$r['user_id'],
		$r['component'],
		$r['type'],
		$r['item_id'],
 ...
 *
 * add_action( 'sz_activity_deleted_activities', 'my_function' );
 *
 * The action passes one parameter that is a single activity ID or an
 * array of activity IDs depending on the number deleted.
 *
 * If you are deleting an activity comment please use sz_activity_delete_comment();
 *
 * @since 1.0.0
 *
 * @see SZ_Activity_Activity::get() For more information on accepted arguments.
 ...
 *                           the same as SZ_Activity_Activity::get().
 *                           See that method for a description.
 * @return bool True on success, false on failure.
 */
function sz_activity_delete( $args = '' ) {
	 ...
	 */
	do_action( 'sz_before_activity_delete', $args );

	// Adjust the new mention count of any mentioned member.
	sz_activity_adjust_mention_count( $args['id'], 'delete' );

	$activity_ids_deleted = SZ_Activity_Activity::delete( $args );
	...
	$latest_update = sz_get_user_meta( $user_id, 'sz_latest_update', true );
	 ...
	 */
	do_action( 'sz_activity_delete', $args );
	 ...
	 */
	do_action( 'sz_activity_deleted_activities', $activity_ids_deleted );
	 ...
	 *
	 * You should use sz_activity_delete() instead.
	 *
	 * @since 1.1.0
	 * @deprecated 1.2.0
	 *
	 *
	 * @param array|string $args See SZ_Activity_Activity::get for a
	 *                           description of accepted arguments.
	 * @return bool True on success, false on failure.
	 */
	function sz_activity_delete_by_item_id( $args = '' ) {
	 ...
	 * @return bool True on success, false on failure.
	 */
	function sz_activity_delete_by_activity_id( $activity_id ) {
		return sz_activity_delete( array( 'id' => $activity_id ) );
	}
	 ...
	 * @return bool True on success, false on failure.
	 */
	function sz_activity_delete_by_content( $user_id, $content, $component, $type ) {
		return sz_activity_delete( array(
			'user_id'   => $user_id,
			'content'   => $content,
			'component' => $component,
			'type'      => $type
	 ...
	 * @return bool True on success, false on failure.
	 */
	function sz_activity_delete_for_user_by_component( $user_id, $component ) {
		return sz_activity_delete( array(
			'user_id'   => $user_id,
			'component' => $component
		) );
 ...
 *       as easily examine the activity type in sz_activity_delete() and then
 ...
 * @return bool True on success, false on failure.
 */
function sz_activity_delete_comment( $activity_id, $comment_id ) {
	$deleted = false;

	/**
	 * Filters whether SportsZone should delete an activity comment or not.
	 *
	 ...
	 *
	 * @param bool $value       Whether SportsZone should continue or not.
	 ...
	 */
	if ( ! apply_filters_ref_array( 'sz_activity_delete_comment_pre', array( true, $activity_id, $comment_id, &$deleted ) ) ) {
		return $deleted;
	}

	// Check if comment still exists.
	$comment = new SZ_Activity_Activity( $comment_id );
	if ( empty( $comment->id ) ) {
		return false;
	}

	// Delete any children of this comment.
	sz_activity_delete_children( $activity_id, $comment_id );

	// Delete the actual comment.
	if ( ! sz_activity_delete( array( 'id' => $comment_id, 'type' => 'activity_comment' ) ) ) {
		return false;
	} else {
		$deleted = true;
	}
	...
	SZ_Activity_Activity::rebuild_activity_comment_tree( $activity_id );
	 ...
	 */
	do_action( 'sz_activity_delete_comment', $activity_id, $comment_id );
	 ...
	 * @param int $comment_id  The ID of the comment to be deleted.
	 */
	function sz_activity_delete_children( $activity_id, $comment_id ) {
		// Check if comment still exists.
		$comment = new SZ_Activity_Activity( $comment_id );
		if ( empty( $comment->id ) ) {
			return;
		...
		$children = SZ_Activity_Activity::get_child_comments( $comment_id );

		// Recursively delete all children of this comment.
		if ( ! empty( $children ) ) {
			foreach( (array) $children as $child ) {
				sz_activity_delete_children( $activity_id, $child->id );
			}
		}
 ...
 * SZ_Activity_Activity object. To save yourself some processing overhead,
 ...
 */
function sz_activity_get_permalink( $activity_id, $activity_obj = false ) {
	$sz = sportszone();

	if ( empty( $activity_obj ) ) {
		$activity_obj = new SZ_Activity_Activity( $activity_id );
	}
	...
	if ( ! empty( $sz->activity->track ) ) {
		$use_primary_links = array_merge( $use_primary_links, array_keys( $sz->activity->track ) );
	}
	...
	if ( false !== array_search( $activity_obj->type, $use_primary_links ) ) {
		...
		if ( 'activity_comment' == $activity_obj->type ) {
			$link = sz_get_root_domain() . '/' . sz_get_activity_root_slug() . '/p/' . $activity_obj->item_id . '/#acomment-' . $activity_obj->id;
		} else {
			$link = sz_get_root_domain() . '/' . sz_get_activity_root_slug() . '/p/' . $activity_obj->id . '/';
		}
	}
	 ...
	 */
	return apply_filters_ref_array( 'sz_activity_get_permalink', array( $link, &$activity_obj ) );
}
 ...
 */
function sz_activity_user_can_read( $activity, $user_id = 0 ) {
	$retval = true;
	...
	if ( sz_is_active( 'groups' ) && sportszone()->groups->id === $activity->component ) {
	 ...
	 * @param SZ_Activity_Activity $activity Activity object.
	 */
	return apply_filters( 'sz_activity_user_can_read', $retval, $user_id, $activity );
}
 ...
 * @return bool True on success, false on failure.
 */
function sz_activity_hide_user_activity( $user_id ) {
	return SZ_Activity_Activity::hide_all_for_user( $user_id );
}
 ...
 *
 * As of SportsZone 2.3, this function is no longer in use.
 *
 * @since 1.2.0
 *
 ...
 *                             creation function (eg sz_blogs_record_activity()).
 * @return string $content The content with images stripped and replaced with a
 *                         single thumb.
 */
function sz_activity_thumbnail_content_images( $content, $link = false, $args = false ) {
	...
	if ( !empty( $matches ) && !empty( $matches[0] ) ) {
		...
		if ( ! empty( $src ) ) {
			...
			$image      = '<img src="' . esc_url( $src ) . '" width="' . absint( $new_width ) . '" height="' . absint( $new_height ) . '" alt="' . __( 'Thumbnail', 'sportszone' ) . '" class="align-left thumbnail" />';
	 ...
	 */
	return apply_filters( 'sz_activity_thumbnail_content_images', $content, $matches, $args );
}
 ...
 */
function sz_activity_get_excerpt_length() {
	/**
	 ...
	 */
	return (int) apply_filters( 'sz_activity_excerpt_length', 358 );
}
 ...
 *
 * @param string $content  The content of the activity item.
 * @param array  $activity The data passed to sz_activity_add() or the values
 *                         from an Activity obj.
 * @return string $summary
 */
function sz_activity_create_summary( $content, $activity ) {
	$args = array(
	 ...
	 * @param string $content   The content of the activity item.
	 * @param array  $activity  The data passed to sz_activity_add() or the values from an Activity obj.
	 */
	$extractor = apply_filters( 'sz_activity_create_summary_extractor_class', 'SZ_Media_Extractor', $content, $activity );
	 ...
	 * @param string             $content   The content of the activity item.
	 * @param array              $activity  The data passed to sz_activity_add() or the values from an Activity obj.
	 * @param SZ_Media_Extractor $extractor The media extractor object.
	 */
	$args = apply_filters( 'sz_activity_create_summary_extractor_args', $args, $content, $activity, $extractor );


	// Extract media information from the $content.
	$media = $extractor->extract( $content, SZ_Media_Extractor::ALL, $args );
	...
	if ( $use_media_type ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array  $extracted_media Extracted media item. See {@link SZ_Media_Extractor::extract()} for format.
		 * @param string $content         Content of the activity item.
		 * @param array  $activity        The data passed to sz_activity_add() or the values from an Activity obj.
		 * @param array  $media           All results from the media extraction.
		 *                                See {@link SZ_Media_Extractor::extract()} for format.
		...
		$extracted_media = apply_filters(
			'sz_activity_create_summary_extractor_result',
			$extracted_media,
			$content,
			$activity,
			$media,
	...
	$summary = sz_create_excerpt( html_entity_decode( $content ), 225, array(
		'html' => false,
	 ...
	 * @param string $content         Content of the activity item.
	 * @param array  $activity        The data passed to sz_activity_add() or the values from an Activity obj.
	 * @param array  $extracted_media Media item extracted. See {@link SZ_Media_Extractor::extract()} for format.
	 */
	return apply_filters( 'sz_activity_create_summary', $summary, $content, $activity, $extracted_media );
}
 ...
 */
function sz_activity_user_can_mark_spam() {
	 ...
	 * @since 1.6.0
	 *
	 * @param bool $moderate Whether or not the current user has sz_moderate capability.
	 */
	return apply_filters( 'sz_activity_user_can_mark_spam', sz_current_user_can( 'sz_moderate' ) );
}
 ...
 *
 * @param SZ_Activity_Activity $activity The activity item to be spammed.
 * @param string               $source   Optional. Default is "by_a_person" (ie, a person has
 *                                       manually marked the activity as spam). BP core also
 *                                       accepts 'by_akismet'.
 */
function sz_activity_mark_as_spam( &$activity, $source = 'by_a_person' ) {
	$sz = sportszone();
	...
	wp_cache_delete( $activity->id, 'sz_activity_comments' );

	// If Akismet is active, and this was a manual spam/ham request, stop Akismet checking the activity.
	if ( 'by_a_person' == $source && !empty( $sz->activity->akismet ) ) {
		remove_action( 'sz_activity_before_save', array( $sz->activity->akismet, 'check_activity' ), 4 );

		// Build data package for Akismet.
		$activity_data = SZ_Akismet::build_akismet_data_package( $activity );

		// Tell Akismet this is spam.
		$activity_data = $sz->activity->akismet->send_akismet_request( $activity_data, 'submit', 'spam' );

		// Update meta.
		add_action( 'sz_activity_after_save', array( $sz->activity->akismet, 'update_activity_spam_meta' ), 1, 1 );
	}
	 ...
	 * @since 1.6.0
	 *
	 * @param SZ_Activity_Activity $activity Activity item being marked as spam.
	 * @param string               $source   Source of determination of spam status. For example
	 *                                       "by_a_person" or "by_akismet".
	 */
	do_action( 'sz_activity_mark_as_spam', $activity, $source );
}
 ...
 * @since 1.6.0
 *
 * @param SZ_Activity_Activity $activity The activity item to be hammed. Passed by reference.
 * @param string               $source   Optional. Default is "by_a_person" (ie, a person has
 *                                       manually marked the activity as spam). BP core also accepts
 *                                       'by_akismet'.
 */
function sz_activity_mark_as_ham( &$activity, $source = 'by_a_person' ) {
	$sz = sportszone();
	...
	if ( 'by_a_person' == $source && !empty( $sz->activity->akismet ) ) {
		...
		// Tell Akismet this is spam.
		$activity_data = $sz->activity->akismet->send_akismet_request( $activity_data, 'submit', 'ham' );

		// Update meta.
		add_action( 'sz_activity_after_save', array( $sz->activity->akismet, 'update_activity_ham_meta' ), 1, 1 );
	}
	 ...
	 * @since 1.6.0
	 *
	 * @param SZ_Activity_Activity $activity Activity item being marked as ham.
	 * @param string               $source   Source of determination of ham status. For example
	 *                                       "by_a_person" or "by_akismet".
	 */
	do_action( 'sz_activity_mark_as_ham', $activity, $source );
}
 ...
 */
function sz_activity_at_message_notification( $activity_id, $receiver_user_id ) {
	$notifications = SZ_Core_Notification::get_all_for_user( $receiver_user_id, 'all' );
	...
	$group_name   = '';
	$message_link = sz_activity_get_permalink( $activity_id );
	$poster_name  = sz_core_get_user_displayname( $activity->user_id );

	remove_filter( 'sz_get_activity_content_body', 'convert_smilies' );
	remove_filter( 'sz_get_activity_content_body', 'wpautop' );
	...
	add_filter( 'sz_get_activity_content_body', 'convert_smilies' );
	add_filter( 'sz_get_activity_content_body', 'wpautop' );
	add_filter( 'sz_get_activity_content_body', 'sz_activity_truncate_entry', 5 );

	// Now email the user with the contents of the message (if they have enabled email notifications).
	if ( 'no' != sz_get_user_meta( $receiver_user_id, 'notification_activity_new_mention', true ) ) {
		...
		sz_send_email( $email_type, $receiver_user_id, $args );
	}
	 ...
	 * @param int                  $receiver_user_id The ID of the user who is receiving the update.
	 */
	do_action( 'sz_activity_sent_mention_email', $activity, '', '', '', $receiver_user_id );
}
 ...
 * @param array $params       {@link sz_activity_new_comment()}.
 */
function sz_activity_new_comment_notification( $comment_id = 0, $commenter_id = 0, $params = array() ) {
	$original_activity = new SZ_Activity_Activity( $params['activity_id'] );
	$poster_name       = sz_core_get_user_displayname( $commenter_id );
	$thread_link       = sz_activity_get_permalink( $params['activity_id'] );
	...
	/** This filter is documented in sz-activity/sz-activity-template.php */
	$content = apply_filters_ref_array( 'sz_get_activity_content_body', array( $params['content'], &$original_activity ) );
	...
	if ( $original_activity->user_id != $commenter_id ) {
		...
		if ( 'no' != sz_get_user_meta( $original_activity->user_id, 'notification_activity_new_reply', true ) ) {
			...
			sz_send_email( 'activity-comment', $original_activity->user_id, $args );
		}
		 ...
		 * @since 2.6.0
		 *
		 * @param SZ_Activity_Activity $original_activity The original activity.
		 * @param int                  $comment_id        ID for the newly received comment.
		 * @param int                  $commenter_id      ID of the user who made the comment.
		 * @param array                $params            Arguments used with the original activity comment.
		 */
		do_action( 'sz_activity_sent_reply_to_update_notification', $original_activity, $comment_id, $commenter_id, $params );
	}
	...
	$parent_comment = new SZ_Activity_Activity( $params['parent_id'] );

	if ( $parent_comment->user_id != $commenter_id && $original_activity->user_id != $parent_comment->user_id ) {

		// Send an email if the user hasn't opted-out.
		if ( 'no' != sz_get_user_meta( $parent_comment->user_id, 'notification_activity_new_reply', true ) ) {
			...
			sz_send_email( 'activity-comment-author', $parent_comment->user_id, $args );
		}
		 ...
		 * @since 2.6.0
		 *
		 * @param SZ_Activity_Activity $parent_comment The parent activity.
		 * @param int                  $comment_id     ID for the newly received comment.
		 * @param int                  $commenter_id   ID of the user who made the comment.
		 * @param array                $params         Arguments used with the original activity comment.
		 */
		do_action( 'sz_activity_sent_reply_to_reply_notification', $parent_comment, $comment_id, $commenter_id, $params );
	}
}
 ...
 */
function sz_activity_new_comment_notification_helper( $comment_id, $params ) {
	sz_activity_new_comment_notification( $comment_id, $params['user_id'], $params );
}
add_action( 'sz_activity_comment_posted', 'sz_activity_new_comment_notification_helper', 10, 2 );
 ...
 * For that, see {@link sz_activity_comment_embed()}.
 *
 * @since 1.5.0
 *
 * @see SZ_Embed
 * @see sz_embed_activity_cache()
 * @see sz_embed_activity_save_cache()
 *
 */
function sz_activity_embed() {
	add_filter( 'embed_post_id',         'sz_get_activity_id'                  );
	add_filter( 'oembed_dataparse',      'sz_activity_oembed_dataparse', 10, 2 );
	add_filter( 'sz_embed_get_cache',    'sz_embed_activity_cache',      10, 3 );
	add_action( 'sz_embed_update_cache', 'sz_embed_activity_save_cache', 10, 3 );
}
add_action( 'activity_loop_start', 'sz_activity_embed' );
 ...
 * @return string
 */
function sz_activity_oembed_dataparse( $retval, $data ) {
	sportszone()->activity->oembed_response = $data;
 ...
 * ({@link sz_activity_recurse_comments}), this function sets up the hooks
 ...
 *
 */
function sz_activity_comment_embed() {
	add_filter( 'embed_post_id',         'sz_get_activity_comment_id'          );
	add_filter( 'sz_embed_get_cache',    'sz_embed_activity_cache',      10, 3 );
	add_action( 'sz_embed_update_cache', 'sz_embed_activity_save_cache', 10, 3 );
}
add_action( 'sz_before_activity_comment', 'sz_activity_comment_embed' );
 ...
 */
function sz_dtheme_embed_read_more( $activity ) {
	sportszone()->activity->read_more_id = $activity->id;

	add_filter( 'embed_post_id',         function() { return sportszone()->activity->read_more_id; } );
	add_filter( 'sz_embed_get_cache',    'sz_embed_activity_cache',      10, 3 );
	add_action( 'sz_embed_update_cache', 'sz_embed_activity_save_cache', 10, 3 );
}
add_action( 'sz_dtheme_get_single_activity_content',       'sz_dtheme_embed_read_more' );
add_action( 'sz_legacy_theme_get_single_activity_content', 'sz_dtheme_embed_read_more' );
 ...
 * @since 1.5.0
 *
 * @see sz_activity_comment_embed()
 */
function sz_activity_comment_embed_after_recurse() {
	remove_filter( 'embed_post_id', 'sz_get_activity_comment_id' );
}
add_action( 'sz_after_activity_comment', 'sz_activity_comment_embed_after_recurse' );

/**
 * Fetch an activity item's cached embeds.
 *
 * Used during {@link SZ_Embed::parse_oembed()} via {@link sz_activity_embed()}.
 *
 * @since 1.5.0
 *
 * @see SZ_Embed::parse_oembed()
 *
 * @param string $cache    An empty string passed by SZ_Embed::parse_oembed() for
 *                         functions like this one to filter.
 * @param int    $id       The ID of the activity item.
 * @param string $cachekey The cache key generated in SZ_Embed::parse_oembed().
 * @return mixed The cached embeds for this activity item.
 */
function sz_embed_activity_cache( $cache, $id, $cachekey ) {
	return sz_activity_get_meta( $id, $cachekey );
}
 ...
 * @param int    $id       The ID of the activity item.
 */
function sz_embed_activity_save_cache( $cache, $cachekey, $id ) {
	sz_activity_update_meta( $id, $cachekey, $cache );

	// Cache full oEmbed response.
	if ( true === isset( sportszone()->activity->oembed_response ) ) {
		$cachekey = str_replace( '_oembed', '_oembed_response', $cachekey );
		sz_activity_update_meta( $id, $cachekey, sportszone()->activity->oembed_response );
	}
}
...
function sz_activity_do_heartbeat() {
	 ...
	 */
	return (bool) apply_filters( 'sz_activity_do_heartbeat', $retval );
}
 ...
 * @since 2.1.0
 */
function sz_ajax_get_suggestions() {
	if ( ! sz_is_user_active() || empty( $_GET['term'] ) || empty( $_GET['type'] ) ) {
...
add_action( 'wp_ajax_sz_get_suggestions', 'sz_ajax_get_suggestions' );
 ...
 */
function sz_activity_catch_transition_post_type_status( $new_status, $old_status, $post ) {
	if ( ! post_type_supports( $post->post_type, 'sportszone-activity' ) ) {
		return;
	}
	...
	if ( $new_status === $old_status ) {
		...
		if ( $new_status == 'publish' ) {
			$edit = sz_activity_post_type_update( $post );

			// Post was never recorded into activity stream, so record it now!
			if ( null === $edit ) {
				sz_activity_post_type_publish( $post->ID, $post );
			}
		...
		// Allow plugins to eventually deal with other post statuses.
			 ...
			 * @param string $old_status Old status for the post.
			 */
			do_action( 'sz_activity_post_type_edit_' . $post->post_type, $post, $new_status, $old_status );
		}
	...
	if ( 'publish' === $new_status ) {
		...
		if ( 'trash' == $old_status ) {
			 ...
			 * @param WP_Post $post Post data.
			 */
			do_action( 'sz_activity_post_type_untrash_' . $post->post_type, $post );
		} else {
			// Record the post.
			sz_activity_post_type_publish( $post->ID, $post );
		}
	...
	// Unpublishing a previously published post.
		...
		sz_activity_post_type_unpublish( $post->ID, $post );
	...
	// For any other cases, allow plugins to eventually deal with it.
		 ...
		 * @param string $old_status Old status for the post.
		 */
		do_action( 'sz_activity_post_type_transition_status_' . $post->post_type, $post, $new_status, $old_status );
	}
}
add_action( 'transition_post_status', 'sz_activity_catch_transition_post_type_status', 10, 3 );
 ...
 */
function sz_activity_transition_post_type_comment_status( $new_status, $old_status, $comment ) {
	$post_type = get_post_type( $comment->comment_post_ID );
	if ( ! $post_type ) {
		return;
	}
	...
	if ( in_array( $new_status, array( 'delete', 'hold' ) ) ) {
		return sz_activity_post_type_remove_comment( $comment->comment_ID, $activity_post_object );
	...
	if ( sz_disable_blogforum_comments() ) {
		...
		$activity_id = get_comment_meta( $comment->comment_ID, 'sz_activity_comment_id', true );
	}
	 ...
	 * @since  2.5.0
	 *
	 * @param bool        $value       True to override SportsZone management.
	 ...
	 * @param WP_Comment  $comment Comment data.
	 */
	if ( true === apply_filters( 'sz_activity_pre_transition_post_type_comment_status', false, $post_type, $activity_id, $new_status, $old_status, $comment ) ) {
		return false;
	}
	...
	if ( empty( $activity_id ) ) {
		...
		if ( 'approved' == $new_status ) {
			return sz_activity_post_type_comment( $comment->comment_ID, true, $activity_post_object );
		}
	...
	if ( 'spam_activity' === $action && ! $activity->is_spam ) {
		...
		sz_activity_mark_as_ham( $activity );
	}
	...
	add_filter( 'sz_akismet_get_activity_types', $comment_akismet_history );

	// Make sure the activity change won't edit the comment if sync is on
	remove_action( 'sz_activity_before_save', 'sz_blogs_sync_activity_edit_to_post_comment', 20 );

	// Save the updated activity
	$activity->save();

	// Restore the action
	add_action( 'sz_activity_before_save', 'sz_blogs_sync_activity_edit_to_post_comment', 20 );

	// Remove the "new_blog_comment" activity type whitelist so we don't break anything
	remove_filter( 'sz_akismet_get_activity_types', $comment_akismet_history );
}
add_action( 'transition_comment_status', 'sz_activity_transition_post_type_comment_status', 10, 3 );
