...
function hashbuddy_activity_hashtags_filter( $content ) {
	...
	$activity_url = trailingslashit( get_bloginfo('url') ) . SZ_ACTIVITY_SLUG;
...
add_filter( 'sz_ajax_querystring', 'hashbuddy_activity_hashtags_querystring', 11, 2 );
add_filter( 'sz_dtheme_ajax_querystring', 'hashbuddy_activity_hashtags_querystring', 11, 2 );


function hashbuddy_bbpress_hashtags_filter( $content ) {
	global $sz;

	$forum_slug = get_option('_bsz_root_slug');
	$search_slug = get_option('_bsz_search_slug');
	...
	if ( $hashtags ) {
		...
		foreach( (array)$hashtags as $hashtag ) {
			$pattern = "/(^|\s|\b)#". $hashtag ."($|\b)/";
			$content = preg_replace( $pattern, ' <a href="'. $site_url . $forum_slug . '/' . $search_slug . '/?bsz_search=%23' . htmlspecialchars( $hashtag ) . '" rel="nofollow" class="hashtag" id="' . htmlspecialchars( $hashtag ) . '">#'. htmlspecialchars( $hashtag ) .'</a>', $content );
		}
	}
