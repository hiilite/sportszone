<?php
/**
 * SportsZone Activity Template Functions.
 *
 * @package SportsZone
 ...
 *
 */
function sz_activity_slug() {
	echo sz_get_activity_slug();
}
	/**
	...
	function sz_get_activity_slug() {
		 ...
		 */
		return apply_filters( 'sz_get_activity_slug', sportszone()->activity->slug );
	}
 ...
 *
 */
function sz_activity_root_slug() {
	echo sz_get_activity_root_slug();
}
	/**
	 ...
	 */
	function sz_get_activity_root_slug() {
		 ...
		 */
		return apply_filters( 'sz_get_activity_root_slug', sportszone()->activity->root_slug );
	}
 ...
 *
 */
function sz_activity_directory_permalink() {
	echo esc_url( sz_get_activity_directory_permalink() );
}
	/**
	 ...
	 */
	function sz_get_activity_directory_permalink() {
		 ...
		 */
		return apply_filters( 'sz_get_activity_directory_permalink', trailingslashit( sz_get_root_domain() . '/' . sz_get_activity_root_slug() ) );
	}

/**
 * Initialize the activity loop.
 *
 * Based on the $args passed, sz_has_activities() populates the
 * $activities_template global, enabling the use of SportsZone templates and
 * template functions to display a list of activity items.
 *
 * @since 1.0.0
 * @since 2.4.0 Introduced the `$fields` parameter.
 *
 * @global object $activities_template {@link SZ_Activity_Template}
 *
 * @param array|string $args {
 *     Arguments for limiting the contents of the activity loop. Most arguments
 *     are in the same format as {@link SZ_Activity_Activity::get()}. However,
 *     because the format of the arguments accepted here differs in a number of
 *     ways, and because sz_has_activities() determines some default arguments in
 ...
 *     @type array             $filter_query     Advanced activity filtering.  See {@link SZ_Activity_Query::__construct()}.
 *     @type string            $search_terms     Limit results by a search term. Default: false.
 *     @type string            $scope            Use a SportsZone pre-built filter.
 ...
 *                                               component ID in the case of SportsZone components, or the plugin slug in
 ...
 */
function sz_has_activities( $args = '' ) {
	...
	$scope = array_key_exists( sz_current_action(), (array) $sz->loaded_components )
		? $sz->loaded_components[ sz_current_action() ]
		: sz_current_action();

	// Support for permalinks on single item pages: /groups/my-group/activity/124/.
	$include = sz_is_current_action( sz_get_activity_slug() )
		? sz_action_variable( 0 )
		: false;

	$search_terms_default = false;
	$search_query_arg = sz_core_get_component_search_query_arg( 'activity' );
	...
	$r = sz_parse_args( $args, array(
		...
		'page_arg'          => 'acpage',     // See https://sportszone.trac.wordpress.org/ticket/3679.
		...
		'filter_query'      => false,        // Advanced filtering.  See SZ_Activity_Query for format.
	...
	/**
	 * Filters whether SportsZone should enable afilter support.
	 *
	 * Support for basic filters in earlier BP versions is disabled by default.
	 * To enable, put add_filter( 'sz_activity_enable_afilter_support', '__return_true' );
	 * into sz-custom.php or your theme's functions.php.
	 *
	 * @since 1.6.0
	 *
	 * @param bool $value True if SportsZone should enable afilter support.
	 */
	if ( isset( $_GET['afilter'] ) && apply_filters( 'sz_activity_enable_afilter_support', false ) ) {
		$r['filter'] = array(
			'object' => $_GET['afilter']
		);
	...
	$activities_template = new SZ_Activity_Template( $r );
	 ...
	 * @param array  $r                   Array of arguments passed into the SZ_Activity_Template class.
	 */
	return apply_filters( 'sz_has_activities', $activities_template->has_activities(), $activities_template, $r );
}
 ...
 * @since 2.1.0
 */
function sz_activity_load_more_link() {
	echo esc_url( sz_get_activity_load_more_link() );
}
	/**
	 ...
	 * @return string $link
	 */
	function sz_get_activity_load_more_link() {
		global $activities_template;

		$url  = sz_get_requested_url();
		 ...
		 */
		return apply_filters( 'sz_get_activity_load_more_link', $link, $url, $activities_template );
	}
 ...
 * @global object $activities_template {@link SZ_Activity_Template}
 */
function sz_activity_pagination_count() {
	echo sz_get_activity_pagination_count();
}
	 ...
	 */
	function sz_get_activity_pagination_count() {
		global $activities_template;

		$start_num = intval( ( $activities_template->pag_page - 1 ) * $activities_template->pag_num ) + 1;
		$from_num  = sz_core_number_format( $start_num );
		$to_num    = sz_core_number_format( ( $start_num + ( $activities_template->pag_num - 1 ) > $activities_template->total_activity_count ) ? $activities_template->total_activity_count : $start_num + ( $activities_template->pag_num - 1 ) );
		$total     = sz_core_number_format( $activities_template->total_activity_count );

		if ( 1 == $activities_template->total_activity_count ) {
			$message = __( 'Viewing 1 item', 'sportszone' );
		} else {
			$message = sprintf( _n( 'Viewing %1$s - %2$s of %3$s item', 'Viewing %1$s - %2$s of %3$s items', $activities_template->total_activity_count, 'sportszone' ), $from_num, $to_num, $total );
		}
 ...
 *
 */
function sz_activity_pagination_links() {
	echo sz_get_activity_pagination_links();
}
	 ...
	 */
	function sz_get_activity_pagination_links() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_pagination_links', $activities_template->pag_links );
	}
 ...
 */
function sz_activity_has_more_items() {
	global $activities_template;
	 ...
	 */
	return apply_filters( 'sz_activity_has_more_items', $has_more_items );
}
 ...
 *
 */
function sz_activity_count() {
	echo sz_get_activity_count();
}
	 ...
	 */
	function sz_get_activity_count() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_count', (int) $activities_template->activity_count );
	}
 ...
 *
 */
function sz_activity_per_page() {
	echo sz_get_activity_per_page();
}
	 ...
	 */
	function sz_get_activity_per_page() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_per_page', (int) $activities_template->pag_num );
	}
 ...
 * @todo Deprecate.
 */
function sz_activities_title() {
	echo sz_get_activities_title();
}
	 ...
	 * @since 1.0.0
	 *
	 * @global string $sz_activity_title
	 * @todo Deprecate.
	 *
	 * @return string The activities title.
	 */
	function sz_get_activities_title() {
		global $sz_activity_title;
		 ...
		 * @since 1.0.0
		 *
		 * @param string $sz_activity_title The title to be displayed.
		 */
		return apply_filters( 'sz_get_activities_title', $sz_activity_title );
	}
 ...
 * @todo Deprecate.
 */
function sz_activities_no_activity() {
	echo sz_get_activities_no_activity();
}
	 ...
	 * @since 1.0.0
	 *
	 * @global string $sz_activity_no_activity
	 * @todo Deprecate.
	 *
	 * @return string
	 */
	function sz_get_activities_no_activity() {
		global $sz_activity_no_activity;
		 ...
		 * @since 1.0.0
		 *
		 * @param string $sz_activity_no_activity Text to display for no activity.
		 */
		return apply_filters( 'sz_get_activities_no_activity', $sz_activity_no_activity );
	}
 ...
 *
 */
function sz_activity_id() {
	echo sz_get_activity_id();
}
	...
	function sz_get_activity_id() {
		 ...
		 */
		return apply_filters( 'sz_get_activity_id', $activities_template->activity->id );
	}
 ...
 *
 */
function sz_activity_item_id() {
	echo sz_get_activity_item_id();
}
	 ...
	 */
	function sz_get_activity_item_id() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_item_id', $activities_template->activity->item_id );
	}
 ...
 *
 */
function sz_activity_secondary_item_id() {
	echo sz_get_activity_secondary_item_id();
}
	 ...
	 */
	function sz_get_activity_secondary_item_id() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_secondary_item_id', $activities_template->activity->secondary_item_id );
	}
 ...
 *
 */
function sz_activity_date_recorded() {
	echo sz_get_activity_date_recorded();
}
	 ...
	 */
	function sz_get_activity_date_recorded() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_date_recorded', $activities_template->activity->date_recorded );
	}
 ...
 *
 */
function sz_activity_member_display_name() {
	echo sz_get_activity_member_display_name();
}
	 ...
	 * @return string The date the activity was recorded.
	 */
	function sz_get_activity_member_display_name() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_member_display_name', $retval );
	}
 ...
 *
 */
function sz_activity_object_name() {
	echo sz_get_activity_object_name();
}
	 ...
	 */
	function sz_get_activity_object_name() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_object_name', $activities_template->activity->component );
	}
 ...
 *
 */
function sz_activity_type() {
	echo sz_get_activity_type();
}
	...
	function sz_get_activity_type() {
		 ...
		 */
		return apply_filters( 'sz_get_activity_type', $activities_template->activity->type );
	}

	/**
	 * Output the activity action name.
	 *
	 * Just a wrapper for sz_activity_type().
	 *
	 * @since 1.2.0
	 * @deprecated 1.5.0
	 *
	 * @todo Properly deprecate in favor of sz_activity_type() and
	 *       remove redundant echo
	 *
	 */
	function sz_activity_action_name() { echo sz_activity_type(); }

	/**
	 * Return the activity type.
	 *
	 * Just a wrapper for sz_get_activity_type().
	 *
	 * @since 1.2.0
	 * @deprecated 1.5.0
	 *
	 * @todo Properly deprecate in favor of sz_get_activity_type().
	 *
	 *
	 * @return string The activity type.
	 */
	function sz_get_activity_action_name() { return sz_get_activity_type(); }
 ...
 *
 */
function sz_activity_user_id() {
	echo sz_get_activity_user_id();
}
	 ...
	 */
	function sz_get_activity_user_id() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_user_id', $activities_template->activity->user_id );
	}
 ...
 *
 */
function sz_activity_user_link() {
	echo sz_get_activity_user_link();
}
	 ...
	 */
	function sz_get_activity_user_link() {
		global $activities_template;
		...
		if ( empty( $activities_template->activity->user_id ) || empty( $activities_template->activity->user_nicename ) || empty( $activities_template->activity->user_login ) ) {
			...
			$link = sz_core_get_user_domain( $activities_template->activity->user_id, $activities_template->activity->user_nicename, $activities_template->activity->user_login );
		}
		 ...
		 */
		return apply_filters( 'sz_get_activity_user_link', $link );
	}
 ...
 * @since 1.1.0
 *
 * @see sz_get_activity_avatar() for description of arguments.
 *
 * @param array|string $args See {@link sz_get_activity_avatar()} for description.
 */
function sz_activity_avatar( $args = '' ) {
	echo sz_get_activity_avatar( $args );
}
	/**
	 ...
	 * @since 1.1.0
	 *
	 * @see sz_core_fetch_avatar() For a description of the arguments.
	 * @global object $activities_template {@link SZ_Activity_Template}
	 *
	 * @param array|string $args  {
	 ...
	 */
	function sz_get_activity_avatar( $args = '' ) {
		global $activities_template;

		$sz = sportszone();

		// On activity permalink pages, default to the full-size avatar.
		$type_default = sz_is_single_activity() ? 'full' : 'thumb';
		...
		$alt_default = !empty( $dn_default ) ? sprintf( __( 'Profile picture of %s', 'sportszone' ), $dn_default ) : __( 'Profile picture', 'sportszone' );
		...
		if ( !isset( $height ) && !isset( $width ) ) {
			...
			// Backpat.
			if ( isset( $sz->avatar->full->height ) || isset( $sz->avatar->thumb->height ) ) {
				$height = ( 'full' == $type ) ? $sz->avatar->full->height : $sz->avatar->thumb->height;
			} else {
				$height = 20;
			}

			// Backpat.
			if ( isset( $sz->avatar->full->width ) || isset( $sz->avatar->thumb->width ) ) {
				$width = ( 'full' == $type ) ? $sz->avatar->full->width : $sz->avatar->thumb->width;
			} else {
				$width = 20;
			}
		}
		 ...
		 * Possible hooks are sz_get_activity_avatar_object_blog,
		 * sz_get_activity_avatar_object_group, and sz_get_activity_avatar_object_user.
		 *
		 * @since 1.1.0
		 *
		 * @param string $component Component being displayed.
		 */
		$object  = apply_filters( 'sz_get_activity_avatar_object_' . $current_activity_item->component, 'user' );
		 ...
		 */
		$item_id = apply_filters( 'sz_get_activity_avatar_item_id', $item_id );
		...
		/**
		 * Filters the value returned by sz_core_fetch_avatar.
		 *
		 * @since 1.1.3
		 *
		 * @param array $value HTML image element containing the activity avatar.
		 */
		return apply_filters( 'sz_get_activity_avatar', sz_core_fetch_avatar( array(
			'item_id' => $item_id,
			'object'  => $object,
			'type'    => $type,
			'alt'     => $alt,
 ...
 * @since 1.2.0
 *
 * @see sz_get_activity_secondary_avatar() for description of arguments.
 *
 * @param array|string $args See {@link sz_get_activity_secondary_avatar} for description.
 */
function sz_activity_secondary_avatar( $args = '' ) {
	echo sz_get_activity_secondary_avatar( $args );
}
	 ...
	 *
	 * @param array|string $args  {
	 *     For a complete description of arguments, see {@link sz_core_fetch_avatar()}.
	 ...
	 */
	function sz_get_activity_secondary_avatar( $args = '' ) {
		global $activities_template;
		...
		switch ( $activities_template->activity->component ) {
			case 'groups' :
				...
				if ( empty( $alt ) ) {
					$alt = __( 'Group logo', 'sportszone' );

					if ( ! empty( $name ) ) {
						$alt = sprintf( __( 'Group logo of %s', 'sportszone' ), $name );
					}
				}
			...
			case 'blogs' :
				...
				if ( empty( $alt ) ) {
					$alt = sprintf( __( 'Profile picture of the author of the site %s', 'sportszone' ), get_blog_option( $item_id, 'blogname' ) );
				}
			...
			case 'friends' :
				...
				$link    = sz_core_get_userlink( $item_id, false, true );

				if ( empty( $alt ) ) {
					$alt = sprintf( __( 'Profile picture of %s', 'sportszone' ), sz_core_get_user_displayname( $activities_template->activity->secondary_item_id ) );
				}
			...
			default :
				...
				if ( empty( $alt ) ) {
					$alt = sprintf( __( 'Profile picture of %s', 'sportszone' ), $activities_template->activity->display_name );
				}
		 ...
		 * sz_get_activity_secondary_avatar_object_blog, sz_get_activity_secondary_avatar_object_group,
		 * and sz_get_activity_secondary_avatar_object_user.
		 *
		 * @since 1.2.10
		 *
		 * @param string $object Component being displayed.
		 */
		$object  = apply_filters( 'sz_get_activity_secondary_avatar_object_' . $activities_template->activity->component, $object );
		 ...
		 */
		$item_id = apply_filters( 'sz_get_activity_secondary_avatar_item_id', $item_id );
		...
		if ( !empty( $linked ) ) {
			 ...
			 */
			$link = apply_filters( 'sz_get_activity_secondary_avatar_link', $link, $activities_template->activity->component );
			 ...
			 */
			$avatar = apply_filters( 'sz_get_activity_secondary_avatar', $avatar );
		...
		return apply_filters( 'sz_get_activity_secondary_avatar', $avatar );
	}
 ...
 * @since 1.2.0
 *
 * @param array $args See sz_get_activity_action().
 */
function sz_activity_action( $args = array() ) {
	echo sz_get_activity_action( $args );
}
	 ...
	 */
	function sz_get_activity_action( $args = array() ) {
		global $activities_template;
		 ...
		 * @param array $value Array containing the current action, the current activity, and the $args array passed into the function.
		 */
		return apply_filters_ref_array( 'sz_get_activity_action', array(
			$action,
			&$activities_template->activity,
			$r
		) );
 ...
 *
 */
function sz_activity_content_body() {
	echo sz_get_activity_content_body();
}
	 ...
	 */
	function sz_get_activity_content_body() {
		global $activities_template;
		...
		if ( empty( $activities_template->activity->action ) && ! empty( $activities_template->activity->content ) ) {
			$activities_template->activity->content = sz_insert_activity_meta( $activities_template->activity->content );
		}
		 ...
		 */
		return apply_filters_ref_array( 'sz_get_activity_content_body', array( $activities_template->activity->content, &$activities_template->activity ) );
	}
 ...
 */
function sz_activity_has_content() {
	global $activities_template;
 ...
 *
 */
function sz_activity_content() {
	echo sz_get_activity_content();
}
	 ...
	 */
	function sz_get_activity_content() {

		/**
		 * If you want to filter activity update content, please use
		 * the filter 'sz_get_activity_content_body'.
		 *
		 * This function is mainly for backwards compatibility.
		 */
		$content = sz_get_activity_action() . ' ' . sz_get_activity_content_body();
		return apply_filters( 'sz_get_activity_content', $content );
	}
 ...
 */
function sz_insert_activity_meta( $content = '' ) {
	global $activities_template;
	...
	$date_recorded  = sz_core_time_since( $activities_template->activity->date_recorded );

	// Set up 'time-since' <span>.
	$time_since = sprintf(
		'<span class="time-since" data-livestamp="%1$s">%2$s</span>',
		sz_core_get_iso8601_date( $activities_template->activity->date_recorded ),
		$date_recorded
	);
	 ...
	 */
	$time_since = apply_filters_ref_array( 'sz_activity_time_since', array(
		$time_since,
		&$activities_template->activity
	) );
	...
	if ( ! sz_is_single_activity() ) {
		...
		$activity_permalink = sz_activity_get_permalink( $activities_template->activity->id, $activities_template->activity );
		$activity_meta      = sprintf( '%1$s <a href="%2$s" class="view activity-time-since sz-tooltip" data-sz-tooltip="%3$s">%4$s</a>',
			$new_content,
			$activity_permalink,
			esc_attr__( 'View Discussion', 'sportszone' ),
			$time_since
		);
	 ...
	 */
	return apply_filters( 'sz_insert_activity_meta', $new_content, $content );
}
 ...
 * @global object $activities_template {@link SZ_Activity_Template}
 *
 * @param false|SZ_Activity_Activity $activity Optional. Falls back on the current item in the loop.
 * @return bool True if can delete, false otherwise.
 */
function sz_activity_user_can_delete( $activity = false ) {
	global $activities_template;
	...
	if ( is_user_logged_in() ) {
		...
		if ( isset( $activity->user_id ) && ( $activity->user_id === sz_loggedin_user_id() ) ) {
			$can_delete = true;
		}

		// Viewing a single item, and this user is an admin of that item.
		if ( sz_is_single_item() && sz_is_item_admin() ) {
			$can_delete = true;
		}
	}
	 ...
	 */
	return (bool) apply_filters( 'sz_activity_user_can_delete', $can_delete, $activity );
}
 ...
 * @since 1.2.0
 *
 * @see sz_get_activity_parent_content() for a description of arguments.
 *
 * @param array|string $args See {@link sz_get_activity_parent_content} for description.
 */
function sz_activity_parent_content( $args = '' ) {
	echo sz_get_activity_parent_content($args);
}
	 ...
	 */
	function sz_get_activity_parent_content( $args = '' ) {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_parent_content', $content );
	}
 ...
 * @since 1.7.0
 */
function sz_activity_parent_user_id() {
	echo sz_get_activity_parent_user_id();
}
	 ...
	 */
	function sz_get_activity_parent_user_id() {
		global $activities_template;
		 ...
		 */
		return (int) apply_filters( 'sz_get_activity_parent_user_id', $retval );
	}
 ...
 *
 */
function sz_activity_is_favorite() {
	echo sz_get_activity_is_favorite();
}
	 ...
	 */
	function sz_get_activity_is_favorite() {
		global $activities_template;
		 ...
		 */
		return (bool) apply_filters( 'sz_get_activity_is_favorite', in_array( $activities_template->activity->id, (array) $activities_template->my_favs ) );
	}
 ...
 *
 * @param array|string $args See {@link sz_activity_get_comments} for description.
 */
function sz_activity_comments( $args = '' ) {
	echo sz_activity_get_comments( $args );
}
	 ...
	 *
	 * @todo deprecate $args param
	 * @todo Given that checks for children already happen in sz_activity_recurse_comments(),
	 ...
	 * @return bool
	 */
	function sz_activity_get_comments( $args = '' ) {
		global $activities_template;
		...
		sz_activity_recurse_comments( $activities_template->activity );
	}
		 ...
		 * @return bool|string
		 */
		function sz_activity_recurse_comments( $comment ) {
			global $activities_template;
			...
			foreach ( (array) $comment->children as $comment_child ) {
				...
				$template = sz_locate_template( 'activity/comment.php', false, false );
 ...
 */
function sz_activity_current_comment() {
	global $activities_template;
	 ...
	 */
	return apply_filters( 'sz_activity_current_comment', $current_comment );
}
 ...
 *
 */
function sz_activity_comment_id() {
	echo sz_get_activity_comment_id();
}
	 ...
	 */
	function sz_get_activity_comment_id() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_activity_comment_id', $comment_id );
	}
 ...
 *
 */
function sz_activity_comment_user_id() {
	echo sz_get_activity_comment_user_id();
}
	 ...
	 */
	function sz_get_activity_comment_user_id() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_activity_comment_user_id', $user_id );
	}
 ...
 *
 */
function sz_activity_comment_user_link() {
	echo sz_get_activity_comment_user_link();
}
	 ...
	 */
	function sz_get_activity_comment_user_link() {
		$user_link = sz_core_get_user_domain( sz_get_activity_comment_user_id() );
		 ...
		 */
		return apply_filters( 'sz_activity_comment_user_link', $user_link );
	}
 ...
 *
 */
function sz_activity_comment_name() {
	echo sz_get_activity_comment_name();
}

	/**
	 * Return the author name for the activity comment currently being displayed.
	 *
	 * The use of the 'sz_acomment_name' filter is deprecated. Please use
	 * 'sz_activity_comment_name'.
	 *
	 * @since 1.5.0
	 *
	 * @global object $activities_template {@link SZ_Activity_Template}
	 ...
	 */
	function sz_get_activity_comment_name() {
		global $activities_template;

		if ( isset( $activities_template->activity->current_comment->user_fullname ) ) {

			$name = apply_filters( 'sz_acomment_name', $activities_template->activity->current_comment->user_fullname, $activities_template->activity->current_comment );  // Backward compatibility.
		} else {
		 ...
		 */
		return apply_filters( 'sz_activity_comment_name', $name );
	}
 ...
 *
 */
function sz_activity_comment_date_recorded() {
	echo sz_get_activity_comment_date_recorded();
}
	 ...
	 */
	function sz_get_activity_comment_date_recorded() {
		 ...
		 */
		return apply_filters( 'sz_activity_comment_date_recorded', sz_core_time_since( sz_get_activity_comment_date_recorded_raw() ) );
	}
 ...
 *
 */
function sz_activity_comment_date_recorded_raw() {
	echo sz_get_activity_comment_date_recorded_raw();
}
	 ...
	 *                                    in the form "%s ago". False on failure.
	 */
	function sz_get_activity_comment_date_recorded_raw() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_activity_comment_date_recorded', $activities_template->activity->current_comment->date_recorded );
	}
 ...
 *
 */
function sz_activity_comment_delete_link() {
	echo sz_get_activity_comment_delete_link();
}
	 ...
	 *                      activity comment.
	 */
	function sz_get_activity_comment_delete_link() {
		$link = wp_nonce_url( trailingslashit( sz_get_activity_directory_permalink() . 'delete/' . sz_get_activity_comment_id() ) . '?cid=' . sz_get_activity_comment_id(), 'sz_activity_delete_link' );
		 ...
		 */
		return apply_filters( 'sz_activity_comment_delete_link', $link );
	}
 ...
 *
 */
function sz_activity_comment_content() {
	echo sz_get_activity_comment_content();
}

	/**
	 * Return the content of the activity comment currently being displayed.
	 *
	 * The content is run through two filters. 'sz_get_activity_content'
	 * will apply all filters applied to activity items in general. Use
	 * 'sz_activity_comment_content' to modify the content of activity
	 * comments only.
	 *
	 * @since 1.5.0
	 *
	 ...
	 */
	function sz_get_activity_comment_content() {
		global $activities_template;

		/** This filter is documented in sz-activity/sz-activity-template.php */
		$content = apply_filters( 'sz_get_activity_content', $activities_template->activity->current_comment->content );
		 ...
		 */
		return apply_filters( 'sz_activity_comment_content', $content, 'get' );
	}
 ...
 *
 */
function sz_activity_comment_count() {
	echo sz_activity_get_comment_count();
}
	 ...
	 */
	function sz_activity_get_comment_count( $deprecated = null ) {
		global $activities_template;

		// Deprecated notice about $args.
		if ( ! empty( $deprecated ) ) {
			_deprecated_argument( __FUNCTION__, '1.2', sprintf( __( '%1$s no longer accepts arguments. See the inline documentation at %2$s for more details.', 'sportszone' ), __FUNCTION__, __FILE__ ) );
		}

		// Get the count using the purpose-built recursive function.
		$count = ! empty( $activities_template->activity->children )
			? sz_activity_recurse_comment_count( $activities_template->activity )
			: 0;
		 ...
		 */
		return apply_filters( 'sz_activity_get_comment_count', (int) $count );
	}
		 ...
		 * @return int $count The activity comment count.
		 */
		function sz_activity_recurse_comment_count( $comment, $count = 0 ) {
			...
			if ( ! empty( $comment->children ) ) {
				foreach ( (array) $comment->children as $comment ) {
					$new_count++;
					$new_count = sz_activity_recurse_comment_count( $comment, $new_count );
				}
			}
			 ...
			 */
			return apply_filters( 'sz_activity_recurse_comment_count', $new_count, $comment, $count );
		}
 ...
 */
function sz_activity_comment_depth( $comment = 0 ) {
	echo sz_activity_get_comment_depth( $comment );
}
	 ...
	 * @return int
	 */
	function sz_activity_get_comment_depth( $comment = 0 ) {
		$depth = 0;
		...
		// Get depth for activity comment manually.
			...
			if ( is_int( $comment ) ) {
				$comment = new SZ_Activity_Activity( $comment );
			}
			...
			if ( is_object( $comment ) && isset( $comment->type ) && 'activity_comment' === $comment->type ) {
				...
				$comments = SZ_Activity_Activity::get_activity_comments( $comment->item_id, 1, constant( 'PHP_INT_MAX' ) );
		 ...
		 */
		return apply_filters( 'sz_activity_get_comment_depth', $depth );
	}
 ...
 *
 */
function sz_activity_comment_link() {
	echo sz_get_activity_comment_link();
}
	 ...
	 */
	function sz_get_activity_comment_link() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_comment_link', '?ac=' . $activities_template->activity->id . '/#ac-form-' . $activities_template->activity->id );
	}
 ...
 *
 */
function sz_activity_comment_form_nojs_display() {
	echo sz_get_activity_comment_form_nojs_display();
}
	 ...
	 */
	function sz_get_activity_comment_form_nojs_display() {
		global $activities_template;
 ...
 *
 */
function sz_activity_comment_form_action() {
	echo sz_get_activity_comment_form_action();
}
	 ...
	 */
	function sz_get_activity_comment_form_action() {
		 ...
		 */
		return apply_filters( 'sz_get_activity_comment_form_action', home_url( sz_get_activity_root_slug() . '/reply/' ) );
	}
 ...
 *
 */
function sz_activity_permalink_id() {
	echo sz_get_activity_permalink_id();
}
	 ...
	 */
	function sz_get_activity_permalink_id() {
		 ...
		 */
		return apply_filters( 'sz_get_activity_permalink_id', sz_current_action() );
	}
 ...
 *
 */
function sz_activity_thread_permalink() {
	echo esc_url( sz_get_activity_thread_permalink() );
}
	 ...
	 */
	function sz_get_activity_thread_permalink() {
		global $activities_template;

		$link = sz_activity_get_permalink( $activities_template->activity->id, $activities_template->activity );
		 ...
		 */
		return apply_filters( 'sz_get_activity_thread_permalink', $link );
	}
 ...
 *
 */
function sz_activity_comment_permalink() {
	echo esc_url( sz_get_activity_comment_permalink() );
}
	/**
	 ...
	 */
	function sz_get_activity_comment_permalink() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_comment_permalink', $link, $comment_id );
	}
 ...
 *
 */
function sz_activity_favorite_link() {
	echo sz_get_activity_favorite_link();
}
	 ...
	 */
	function sz_get_activity_favorite_link() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_favorite_link', wp_nonce_url( home_url( sz_get_activity_root_slug() . '/favorite/' . $activities_template->activity->id . '/' ), 'mark_favorite' ) );
	}
 ...
 *
 */
function sz_activity_unfavorite_link() {
	echo sz_get_activity_unfavorite_link();
}
	 ...
	 */
	function sz_get_activity_unfavorite_link() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_unfavorite_link', wp_nonce_url( home_url( sz_get_activity_root_slug() . '/unfavorite/' . $activities_template->activity->id . '/' ), 'unmark_favorite' ) );
	}
 ...
 *
 */
function sz_activity_css_class() {
	echo sz_get_activity_css_class();
}
	 ...
	 */
	function sz_get_activity_css_class() {
		global $activities_template;
		 ...
		 */
		$mini_activity_actions = apply_filters( 'sz_activity_mini_activity_types', array(
			'friendship_accepted',
			'friendship_created',
			'new_blog',
			'joined_group',
		...
		if ( sz_activity_get_comment_count() && sz_activity_can_comment() ) {
			$class .= ' has-comments';
		}
		 ...
		 */
		return apply_filters( 'sz_get_activity_css_class', $activities_template->activity->component . ' ' . $activities_template->activity->type . $class );
	}
 ...
 *
 */
function sz_activity_delete_link() {
	echo sz_get_activity_delete_link();
}
	 ...
	 */
	function sz_get_activity_delete_link() {

		$url   = sz_get_activity_delete_url();
		$class = 'delete-activity';

		// Determine if we're on a single activity page, and customize accordingly.
		if ( sz_is_activity_component() && is_numeric( sz_current_action() ) ) {
			$class = 'delete-activity-single';
		}

		$link = '<a href="' . esc_url( $url ) . '" class="button item-button sz-secondary-action ' . $class . ' confirm" rel="nofollow">' . __( 'Delete', 'sportszone' ) . '</a>';
		 ...
		 */
		return apply_filters( 'sz_get_activity_delete_link', $link );
	}
 ...
 *
 */
function sz_activity_delete_url() {
	echo esc_url( sz_get_activity_delete_url() );
}
	/**
	 ...
	 *                      if on single activity page.
	 */
	function sz_get_activity_delete_url() {
		global $activities_template;

		$url = trailingslashit( sz_get_root_domain() . '/' . sz_get_activity_root_slug() . '/delete/' . $activities_template->activity->id );
		...
		$url = wp_nonce_url( $url, 'sz_activity_delete_link' );
		 ...
		 */
		return apply_filters( 'sz_get_activity_delete_url', $url );
	}
 ...
 * @since 1.2.0
 *
 * @see sz_get_activity_latest_update() for description of parameters.
 *
 * @param int $user_id See {@link sz_get_activity_latest_update()} for description.
 */
function sz_activity_latest_update( $user_id = 0 ) {
	echo sz_get_activity_latest_update( $user_id );
}
	 ...
	 *                                    False on failure.
	 */
	function sz_get_activity_latest_update( $user_id = 0 ) {
		...
		if ( !$update = sz_get_user_meta( $user_id, 'sz_latest_update', true ) ) {
			return false;
		}
		 ...
		 */
		$latest_update = apply_filters( 'sz_get_activity_latest_update_excerpt', trim( strip_tags( sz_create_excerpt( $update['content'], sz_activity_get_excerpt_length() ) ) ), $user_id );

		$latest_update = sprintf(
			'%s <a href="%s">%s</a>',
			$latest_update,
			esc_url_raw( sz_activity_get_permalink( $update['id'] ) ),
			esc_attr__( 'View', 'sportszone' )
		);
		 ...
		 * @param int    $user_id       ID of the queried user.
		 */
		return apply_filters( 'sz_get_activity_latest_update', $latest_update, $user_id );
	}
 ...
 * @since 1.1.0
 *
 * @see sz_get_activity_filter_links() for description of parameters.
 *
 * @param array|bool $args See {@link sz_get_activity_filter_links()} for description.
 */
function sz_activity_filter_links( $args = false ) {
	echo sz_get_activity_filter_links( $args );
}
	 ...
	 *         False on failure.
	 */
	function sz_get_activity_filter_links( $args = false ) {
		...
		$components = SZ_Activity_Activity::get_recorded_components();
		...
		foreach ( (array) $components as $component ) {
			 ...
			 */
			$link = apply_filters( 'sz_get_activity_filter_link_href', $link, $component );
		...
		if ( isset( $_GET['afilter'] ) ) {
			$component_links[] = '<' . $tag . ' id="afilter-clear"><a href="' . esc_url( $link ) . '">' . __( 'Clear Filter', 'sportszone' ) . '</a></' . $tag . '>';
		}
		 ...
		 */
		return apply_filters( 'sz_get_activity_filter_links', implode( "\n", $component_links ), $r );
	}
 ...
 */
function sz_activity_can_comment() {
	global $activities_template;
	$sz = sportszone();
	...
	$can_comment = sz_activity_type_supports( $activity_type, 'comment-reply' );
	 ...
	 */
	return apply_filters( 'sz_activity_can_comment', $can_comment, $activity_type );
}
 ...
 *                                  otherwise false.
 */
function sz_activity_can_comment_reply( $comment = false ) {
	...
	if ( empty( $comment ) ) {
		$comment = sz_activity_current_comment();
	}
	...
	if ( ! empty( $comment ) ) {
		...
		$comment_depth = isset( $comment->depth )
			...
			: sz_activity_get_comment_depth( $comment );
	 ...
	 */
	return (bool) apply_filters( 'sz_activity_can_comment_reply', $can_comment, $comment );
}
 ...
 */
function sz_activity_can_favorite() {
	 ...
	 */
	return apply_filters( 'sz_activity_can_favorite', true );
}
 ...
 * @since 1.2.0
 *
 * @see sz_get_total_favorite_count_for_user() for description of parameters.
 *
 * @param int $user_id See {@link sz_get_total_favorite_count_for_user()}.
 */
function sz_total_favorite_count_for_user( $user_id = 0 ) {
	echo sz_get_total_favorite_count_for_user( $user_id );
}
	 ...
	 */
	function sz_get_total_favorite_count_for_user( $user_id = 0 ) {
		$retval = false;
		...
		if ( sz_activity_can_favorite() ) {
			...
			if ( ! empty( $user_id ) ) {
				$retval = sz_activity_total_favorites_for_user( $user_id );
			}
		}
		 ...
		 * @param int      $user_id ID of the queried user.
		 */
		return apply_filters( 'sz_get_total_favorite_count_for_user', $retval, $user_id );
	}
 ...
 * @since 1.2.0
 *
 * @see sz_get_total_mention_count_for_user() for description of parameters.
 *
 * @param int $user_id See {@link sz_get_total_mention_count_for_user()}.
 */
function sz_total_mention_count_for_user( $user_id = 0 ) {
	echo sz_get_total_mention_count_for_user( $user_id );
}
	 ...
	 */
	function sz_get_total_mention_count_for_user( $user_id = 0 ) {
		...
		$retval = ! empty( $user_id )
			? sz_get_user_meta( $user_id, 'sz_new_mention_count', true )
			: false;
		 ...
		 * @param int      $user_id ID of the queried user.
		 */
		return apply_filters( 'sz_get_total_mention_count_for_user', $retval, $user_id );
	}
 ...
 *
 */
function sz_send_public_message_link() {
	echo esc_url( sz_get_send_public_message_link() );
}
	 ...
	 */
	function sz_get_send_public_message_link() {

		// No link if not logged in, not looking at someone else's profile.
		if ( ! is_user_logged_in() || ! sz_is_user() || sz_is_my_profile() ) {
			$retval = '';
		} else {
			$args   = array( 'r' => sz_get_displayed_user_mentionname() );
			$url    = add_query_arg( $args, sz_get_activity_directory_permalink() );
		 ...
		 */
		return apply_filters( 'sz_get_send_public_message_link', $retval );
	}
 ...
 * @since 2.0.0
 *
 * @param array $activity Array of activities generated from {@link sz_activity_get()}.
 * @param array $activity_ids Used for recursion purposes in this function.
 * @return array
 */
function sz_activity_recurse_comments_activity_ids( $activity = array(), $activity_ids = array() ) {
	...
	if ( ! empty( $activity->children ) ) {
		foreach ($activity->children as $child ) {
			...
			if( ! empty( $child->children ) ) {
				$activity_ids = sz_activity_recurse_comments_activity_ids( $child, $activity_ids );
			}
		}
	}
 ...
 * @since 1.2.0
 *
 * @see sz_get_mentioned_user_display_name() for description of parameters.
 *
 * @param int|string|bool $user_id_or_username See {@link sz_get_mentioned_user_display_name()}.
 */
function sz_mentioned_user_display_name( $user_id_or_username = false ) {
	echo sz_get_mentioned_user_display_name( $user_id_or_username );
}
	 ...
	 */
	function sz_get_mentioned_user_display_name( $user_id_or_username = false ) {

		// Get user display name.
		$name = sz_core_get_user_displayname( $user_id_or_username );

		// If user somehow has no name, return this really lame string.
		if ( empty( $name ) ) {
			$name = __( 'a user', 'sportszone' );
		}
		 ...
		 */
		return apply_filters( 'sz_get_mentioned_user_display_name', $name, $user_id_or_username );
	}
 ...
 * @since 1.2.0
 *
 * @see sz_get_send_public_message_button() for description of parameters.
 *
 * @param array|string $args See {@link sz_get_send_public_message_button()}.
 */
function sz_send_public_message_button( $args = '' ) {
	echo sz_get_send_public_message_button( $args );
}
	 ...
	 *
	 * @param array|string $args {
	 *     All arguments are optional. See {@link SZ_Button} for complete
	 *     descriptions.
	 ...
	 */
	function sz_get_send_public_message_button( $args = '' ) {
		...
		$r = sz_parse_args( $args, array(
			...
			'link_href'         => sz_get_send_public_message_link(),
			'link_text'         => __( 'Public Message', 'sportszone' ),
		 ...
		 */
		return sz_get_button( apply_filters( 'sz_get_send_public_message_button', $r ) );
	}
 ...
 *
 */
function sz_activity_post_form_action() {
	echo sz_get_activity_post_form_action();
}
	 ...
	 */
	function sz_get_activity_post_form_action() {
		 ...
		 */
		return apply_filters( 'sz_get_activity_post_form_action', home_url( sz_get_activity_root_slug() . '/post/' ) );
	}
 ...
 * @since 1.7.0
 *
 * @see sz_core_fetch_avatar() for a description of arguments.
 *
 * @param array $args See {@link sz_core_fetch_avatar()}.
 */
function sz_activity_comments_user_avatars( $args = array() ) {
	...
	$user_ids = sz_activity_get_comments_user_ids();
	$output   = array();
	$retval   = '';
	...
	if ( ! empty( $user_ids ) ) {
		foreach ( (array) $user_ids as $user_id ) {
			...
			$profile_link = sz_core_get_user_domain( $user_id );

			// Get avatar for this user.
			$image_html   = sz_core_fetch_avatar( array(
				'item_id' => $user_id,
				'height'  => $r['height'],
				'html'    => $r['html'],
				'type'    => $r['type'],
	 ...
	 */
	echo apply_filters( 'sz_activity_comments_user_avatars', $retval, $r, $output );
}
 ...
 */
function sz_activity_get_comments_user_ids() {
	global $activities_template;

	$user_ids = ! empty( $activities_template->activity->children )
		? (array) sz_activity_recurse_comments_user_ids( $activities_template->activity->children )
		: array();
	 ...
	 */
	return apply_filters( 'sz_activity_get_comments_user_ids', array_unique( $user_ids ) );
}
	 ...
	 * @since 1.7.0
	 *
	 * @param array $comments Array of {@link SZ_Activity_Activity} items.
	 * @return array Array of user IDs.
	 */
	function sz_activity_recurse_comments_user_ids( array $comments = array() ) {
		...
		if ( ! empty( $comments ) ) {
			foreach ( $comments as $comment ) {
				...
				if ( ! empty( $comment->children ) ) {
					$user_ids = array_merge( $user_ids, sz_activity_recurse_comments_user_ids( $comment->children ) );
				}
			}
		}
		 ...
		 */
		return apply_filters( 'sz_activity_recurse_comments_user_ids', $user_ids, $comments );
	}
 ...
 * @since 1.9.0
 */
function sz_displayed_user_mentionname() {
	echo sz_get_displayed_user_mentionname();
}
	/**
	 ...
	 */
	function sz_get_displayed_user_mentionname() {
		 ...
		 */
		return apply_filters( 'sz_get_displayed_user_mentionname', sz_activity_get_user_mentionname( sz_displayed_user_id() ) );
	}
 ...
 * }
 */
function sz_activity_types_list( $output = 'select', $args = '' ) {

	$args = sz_parse_args( $args, array(
		'checkbox_name' => 'sz_activity_types',
		'selected'      => array(),
	) );

	$activities = sz_activity_get_types();
	natsort( $activities );
	...
	foreach ( $activities as $type => $description ) {
		 ...
		 * Two default hooks are sz_activity_types_list_select and sz_activity_types_list_checkbox.
		 *
		 * @since 1.7.0
		 *
		 ...
		 */
		do_action( 'sz_activity_types_list_' . $output, $args, $type, $description );
	}
 ...
 *
 */
function sz_sitewide_activity_feed_link() {
	echo sz_get_sitewide_activity_feed_link();
}
	 ...
	 */
	function sz_get_sitewide_activity_feed_link() {
		 ...
		 */
		return apply_filters( 'sz_get_sitewide_activity_feed_link', sz_get_root_domain() . '/' . sz_get_activity_root_slug() . '/feed/' );
	}
 ...
 *
 */
function sz_member_activity_feed_link() {
	echo sz_get_member_activity_feed_link();
}
 ...
 * @deprecated 1.2.0
 *
 * @todo properly deprecate in favor of sz_member_activity_feed_link().
 *
 */
function sz_activities_member_rss_link() { echo sz_get_member_activity_feed_link(); }
	 ...
	 */
	function sz_get_member_activity_feed_link() {

		// Single member activity feed link.
		if ( sz_is_profile_component() || sz_is_current_action( 'just-me' ) ) {
			$link = sz_displayed_user_domain() . sz_get_activity_slug() . '/feed/';

		// Friend feed link.
		} elseif ( sz_is_active( 'friends' ) && sz_is_current_action( sz_get_friends_slug() ) ) {
			$link = sz_displayed_user_domain() . sz_get_activity_slug() . '/' . sz_get_friends_slug() . '/feed/';
		...
		} elseif ( ( 'mentions' === sz_current_action() ) && sz_activity_do_mentions() ) {
			$link = sz_displayed_user_domain() . sz_get_activity_slug() . '/mentions/feed/';
		 ...
		 */
		return apply_filters( 'sz_get_activities_member_rss_link', $link );
	}
	 ...
	 * @deprecated 1.2.0
	 *
	 * @todo properly deprecate in favor of sz_get_member_activity_feed_link().
	 *
	 *
	 * @return string The member activity feed link.
	 */
	function sz_get_activities_member_rss_link() { return sz_get_member_activity_feed_link(); }
 ...
 *
 */
function sz_activity_feed_item_guid() {
	echo sz_get_activity_feed_item_guid();
}
	 ...
	 */
	function sz_get_activity_feed_item_guid() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_feed_item_guid', md5( $activities_template->activity->date_recorded . '-' . $activities_template->activity->content ) );
	}
 ...
 *
 */
function sz_activity_feed_item_title() {
	echo sz_get_activity_feed_item_title();
}
	 ...
	 */
	function sz_get_activity_feed_item_title() {
		global $activities_template;
		...
		if ( 'activity_update' === $activities_template->activity->type ) {
			$title .= ': ' . strip_tags( ent2ncr( trim( convert_chars( sz_create_excerpt( $activities_template->activity->content, 70, array( 'ending' => " [&#133;]" ) ) ) ) ) );
		}
		 ...
		 */
		return apply_filters( 'sz_get_activity_feed_item_title', $title );
	}
 ...
 *
 */
function sz_activity_feed_item_link() {
	echo sz_get_activity_feed_item_link();
}
	 ...
	 */
	function sz_get_activity_feed_item_link() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_feed_item_link', $retval );
	}
 ...
 *
 */
function sz_activity_feed_item_date() {
	echo sz_get_activity_feed_item_date();
}
	 ...
	 */
	function sz_get_activity_feed_item_date() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_feed_item_date', $retval );
	}
 ...
 *
 */
function sz_activity_feed_item_description() {
	echo sz_get_activity_feed_item_description();
}
	 ...
	 */
	function sz_get_activity_feed_item_description() {
		global $activities_template;
		 ...
		 */
		return apply_filters( 'sz_get_activity_feed_item_description', $content );
	}
 ...
 *
 */
function sz_activity_sitewide_feed() {
?>

	<link rel="alternate" type="application/rss+xml" title="<?php bloginfo( 'name' ) ?> | <?php _e( 'Site Wide Activity RSS Feed', 'sportszone' ) ?>" href="<?php sz_sitewide_activity_feed_link() ?>" />

<?php
}
add_action( 'sz_head', 'sz_activity_sitewide_feed' );
 ...
 *                        'member_groups', 'group'.
 */
function sz_activity_show_filters( $context = '' ) {
	echo sz_get_activity_show_filters( $context );
}
	/**
	 ...
	 */
	function sz_get_activity_show_filters( $context = '' ) {
		$filters = array();
		$actions = sz_activity_get_actions_for_context( $context );
		 ...
		 */
		$filters = apply_filters( 'sz_get_activity_show_filters_options', $filters, $context );
		 ...
		 * @param string $context Context for the filter. 'activity', 'member', 'member_groups', 'group'.
		 */
		return apply_filters( 'sz_get_activity_show_filters', $output, $filters, $context );
	}
