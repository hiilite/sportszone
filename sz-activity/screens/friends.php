<?php
/**
 * Activity: User's "Activity > Friends" screen handler
 *
 * @package SportsZone
 * @subpackage ActivityScreens
 * @since 3.0.0
 */

/**
 * Load the 'My Friends' activity page.
 *
 * @since 1.0.0
 */
function sz_activity_screen_friends() {
	if ( !sz_is_active( 'friends' ) )
		return false;

	sz_update_is_item_admin( sz_current_user_can( 'sz_moderate' ), 'activity' );

	/**
	 * Fires right before the loading of the "My Friends" screen template file.
	 *
	 * @since 1.2.0
	 */
	do_action( 'sz_activity_screen_friends' );

	/**
	 * Filters the template to load for the "My Friends" screen.
	 *
	 * @since 1.0.0
	 *
	 * @param string $template Path to the activity template to load.
	 */
	sz_core_load_template( apply_filters( 'sz_activity_template_friends_activity', 'members/single/home' ) );
}