 ...
 * @since 2.6.0
 */
function sz_activity_setup_oembed() {
	sportszone()->activity->oembed = new SZ_Activity_oEmbed_Extension;
}
add_action( 'sz_loaded', 'sz_activity_setup_oembed' );
 ...
 * @return string
 */
function sz_activity_embed_excerpt_onclick_location_filter( $text ) {
	return preg_replace_callback( '/<a\s+[^>]*href=\"([^\"]*)\"/iU', 'sz_activity_embed_excerpt_onclick_location_filter_callback', $text );
}
	/**
	 ...
	 * @since 2.6.0
	 *
	 * @param  array $matches Items matched by sz_activity_embed_excerpt_onclick_location_filter().
	 * @return string
	 */
	function sz_activity_embed_excerpt_onclick_location_filter_callback( $matches ) {
 ...
 * @since  2.6.0
 */
function sz_activity_embed_add_inline_styles() {
	if ( false === sz_is_single_activity() ) {
		return;
	}
	...
	if ( is_rtl() ) {
		$css = sz_locate_template_asset( "css/embeds-activity-rtl{$min}.css" );
	} else {
		$css = sz_locate_template_asset( "css/embeds-activity{$min}.css" );
	}
...
add_action( 'embed_head', 'sz_activity_embed_add_inline_styles', 20 );

/**
 * Query for the activity item on the activity embed template.
 *
 * Basically a wrapper for {@link sz_has_activities()}, but allows us to
 ...
 * @return bool
 */
function sz_activity_embed_has_activity( $activity_id = 0 ) {
	...
	return sz_has_activities( array(
 ...
 * @since 2.6.0
 */
function sz_activity_embed_excerpt( $content = '' ) {
	echo sz_activity_get_embed_excerpt( $content );
}
	 ...
	 * @return string
	 */
	function sz_activity_get_embed_excerpt( $content = '' ) {
		if ( empty( $content ) && ! empty( $GLOBALS['activities_template']->in_the_loop ) ) {
			$content = $GLOBALS['activities_template']->activity->content;
		}

		/*
		 * sz_activity_truncate_entry() includes the 'Read More' link, which is why
		 * we're using this instead of sz_create_excerpt().
		 */
		$content = html_entity_decode( $content );
		$content = sz_activity_truncate_entry( $content, array(
			'html' => false,
		 ...
		 */
		return apply_filters( 'sz_activity_get_embed_excerpt', $content, $GLOBALS['activities_template']->activity->content );
	}
 ...
 * @since 2.6.0
 */
function sz_activity_embed_media() {
	...
	if ( isset( $_GET['hide_media'] ) && true == wp_validate_boolean( $_GET['hide_media'] ) ) {
		 ...
		 * @since 2.6.0
		 */
		do_action( 'sz_activity_embed_after_media' );
	 ...
	 */
	$allow_media = apply_filters( 'sz_activity_embed_display_media', true );

	// Find oEmbeds from only WP registered providers.
	sz_remove_all_filters( 'oembed_providers' );
	$media = sz_core_extract_media_from_content( $GLOBALS['activities_template']->activity->content, 'embeds' );
	sz_restore_all_filters( 'oembed_providers' );
	...
	if ( isset( $media['embeds'] ) && true === $allow_media ) {
		...
		$oembed = sz_activity_get_meta( sz_get_activity_id(), $cachekey );
		...
		if ( '' === $oembed ) {
			...
			sz_activity_update_meta( sz_get_activity_id(), $cachekey, $oembed );
		}
		 ...
		 * @param int $width.
		 */
		$width = (int) apply_filters( 'sz_activity_embed_display_media_width', 550 );
...
EOD;
			...
			if ( isset( $oembed->description ) ) {
				$caption .= sprintf( '<div class="caption-description">%s</div>', apply_filters( 'sz_activity_get_embed_excerpt', $oembed->description ) );
			}

			// Show author info.
			if ( isset( $oembed->provider_name ) && isset( $oembed->author_name ) ) {
				/* translators: By [oEmbed author] on [oEmbed provider]. eg. By SportsZone on YouTube. */
				$anchor_text = sprintf( __( 'By %1$s on %2$s', 'sportszone' ), $oembed->author_name, $oembed->provider_name );

			} elseif ( isset( $oembed->provider_name ) ) {
				$anchor_text = sprintf( __( 'View on %s', 'sportszone' ), $oembed->provider_name );
			}
		...
		// Print rich content.
		if ( '' !== $content ) {
			printf( '<div class="sz-activity-embed-display-media %s" style="max-width:%spx">%s</div>',
	...
	// Video / audio.
		...
		// Call SZ_Embed if it hasn't already loaded.
		sz_embed_init();

		// Run shortcode and embed routine.
		$content = sportszone()->embed->run_shortcode( $GLOBALS['activities_template']->activity->content );
		$content = sportszone()->embed->autoembed( $content );

		// Try to find inline video / audio.
		$media = sz_core_extract_media_from_content( $content, 96 );
		...
		if ( isset( $media['videos'] ) && 'shortcodes' === $media['videos'][0]['source'] ) {
			printf( '<video controls preload="metadata"><source src="%1$s"><p>%2$s</p></video>',
				...
				esc_html__( 'Your browser does not support HTML5 video', 'sportszone' )
			);
		...
		// No video? Try audio. HTML5-only.
			...
			printf( '<audio controls preload="metadata"><source src="%1$s"><p>%2$s</p></audio>',
				...
				esc_html__( 'Your browser does not support HTML5 audio', 'sportszone' )
			);
		}

	}

	/** This hook is documented in /sz-activity/sz-activity-embeds.php */
	do_action( 'sz_activity_embed_after_media' );
}
