 ...
 * @since 2.1.0
 */
function sz_activity_mentions_script() {
	...
	wp_enqueue_script( 'sz-mentions', sportszone()->plugin_url . "sz-activity/js/mentions{$min}.js", array( 'jquery', 'jquery-atwho' ), sz_get_version(), true );
	wp_enqueue_style( 'sz-mentions-css', sportszone()->plugin_url . "sz-activity/css/mentions{$min}.css", array(), sz_get_version() );

	wp_style_add_data( 'sz-mentions-css', 'rtl', true );
	if ( $min ) {
		wp_style_add_data( 'sz-mentions-css', 'suffix', $min );
	}

	// If the script has been enqueued, let's attach our mentions TinyMCE init callback.
	add_filter( 'tiny_mce_before_init', 'sz_add_mentions_on_tinymce_init', 10, 2 );
	 ...
	 * @since 2.1.0
	 */
	do_action( 'sz_activity_mentions_prime_results' );
}
add_action( 'sz_enqueue_scripts', 'sz_activity_mentions_script' );
add_action( 'sz_admin_enqueue_scripts', 'sz_activity_mentions_script' );
 ...
 */
function sz_add_mentions_on_tinymce_init( $settings, $editor_id ) {
