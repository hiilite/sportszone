...
class Activity extends BuddypressCommand {
	...
	public function create( $args, $assoc_args ) {
		$r = wp_parse_args( $assoc_args, array(
			...
			'item-id'           => '',
			'secondary-item-id' => '',
			'date-recorded'     => sz_core_current_time(),
			'hide-sitewide'     => 0,
			'is-spam'           => 0,
		) );
	 ...
	 *
	 * [--<field>=<value>]
	 * : One or more parameters to pass to \SZ_Activity_Activity::get()
	 *
	 * [--user-id=<user>]
	...
	public function get( $args, $assoc_args ) {
		...
		$activity = sz_activity_get_specific( array(
		...
		$activity_arr['url'] = sz_activity_get_permalink( $activity_id );
	...
	public function delete( $args, $assoc_args ) {
		...
		parent::_delete( array( $activity_id ), $assoc_args, function( $activity_id ) {
			...
			$retval = sz_activity_delete( array(
				'id' => $activity_id,
			) );
	...
	public function spam( $args, $assoc_args ) {
		$activity = new \SZ_Activity_Activity( $args[0] );
	...
	public function post_update( $args, $assoc_args ) {
		...
		$id = sz_activity_post_update( array(
	...
	public function comment( $args, $assoc_args ) {
		...
		$id = sz_activity_new_comment( array(
			'content'           => $assoc_args['content'],
			'user_id'           => $user->ID,
	...
	public function delete_comment( $args, $assoc_args ) {
		...
		if ( sz_activity_delete_comment( $activity_id, $assoc_args['comment-id'] ) ) {
	...
	protected function get_random_component() {
		$c = sportszone()->active_components;
		...
		switch ( $r['type'] ) {
			case 'activity_update':
				...
				if ( 'groups' === $r['component'] ) {
					...
					$r['action']  = sprintf( __( '%1$s posted an update in the group %2$s', 'sportszone'), sz_core_get_userlink( $r['user-id'] ), '<a href="' . sz_get_group_permalink( $group_obj ) . '">' . esc_attr( $group_obj->name ) . '</a>' );
				} else {
					// old way, for some other kind of update.
					$r['action'] = sprintf( __( '%s posted an update', 'sportszone' ), sz_core_get_userlink( $r['user-id'] ) );
				}
				if ( empty( $r['content'] ) ) {
					$r['content'] = $this->generate_random_text();
				}

				$r['primary-link'] = sz_core_get_userlink( $r['user-id'] );
			...
			case 'activity_comment':
				...
				$parent_item = $wpdb->get_row( "SELECT * FROM {$sz->activity->table_name} ORDER BY RAND() LIMIT 1" );
				...
				$r['action'] = sprintf( __( '%s posted a new activity comment', 'sportszone' ), sz_core_get_userlink( $r['user-id'] ) );
				$r['content'] = $this->generate_random_text();
				$r['primary-link'] = sz_core_get_userlink( $r['user-id'] );
			...
			case 'new_blog_comment':
				...
				if ( 'new_blog' === $r['type'] ) {
					...
					if ( ! $r['action'] ) {
						$r['action'] = sprintf( __( '%s created the site %s', 'sportszone'), sz_core_get_userlink( $r['user-id'] ), '<a href="' . get_home_url( $r['item-id'] ) . '">' . esc_attr( get_blog_option( $r['item-id'], 'blogname' ) ) . '</a>' );
					}
				...
				// new_blog_post.
					...
					if ( '' === $r['action'] ) {
						$r['action'] = sprintf( __( '%1$s wrote a new post, %2$s', 'sportszone' ), sz_core_get_userlink( (int) $post->post_author ), '<a href="' . $r['primary-link'] . '">' . $post->post_title . '</a>' );
					}
				...
				// new_blog_comment.
					...
					if ( '' === $r['action'] ) {
						$r['action'] = sprintf( __( '%1$s commented on the post, %2$s', 'sportszone' ), sz_core_get_userlink( $r['user-id'] ), '<a href="' . $post_permalink . '">' . apply_filters( 'the_title', $post->post_title ) . '</a>' );
					}
			...
			case 'friendship_created':
				...
				$r['action'] = sprintf( __( '%1$s and %2$s are now friends', 'sportszone' ), sz_core_get_userlink( $r['user-id'] ), sz_core_get_userlink( $r['item-id'] ) );

				break;

			case 'created_group':
				if ( empty( $r['item-id'] ) ) {
					$random_group = \SZ_Groups_Group::get_random( 1, 1 );
				...
				$group_permalink = sz_get_group_permalink( $group );

				if ( empty( $r['action'] ) ) {
					$r['action'] = sprintf( __( '%1$s created the group %2$s', 'sportszone'), sz_core_get_userlink( $r['user-id'] ), '<a href="' . $group_permalink . '">' . esc_attr( $group->name ) . '</a>' );
				}
			...
			case 'joined_group':
				...
				if ( empty( $r['action'] ) ) {
					$r['action'] = sprintf( __( '%1$s joined the group %2$s', 'sportszone' ), sz_core_get_userlink( $r['user-id'] ), '<a href="' . sz_get_group_permalink( $group ) . '">' . esc_attr( $group->name ) . '</a>' );
				}

				if ( empty( $r['primary-link'] ) ) {
					$r['primary-link'] = sz_get_group_permalink( $group );
				}
			...
			case 'updated_profile':
				...
				$userlink = sz_core_get_userlink( $r['user-id'] );

				// new_avatar.
				if ( 'new_avatar' === $r['type'] ) {
					$r['action'] = sprintf( __( '%s changed their profile picture', 'sportszone' ), $userlink );

				// new_member.
				} elseif ( 'new_member' === $r['type'] ) {
					$r['action'] = sprintf( __( '%s became a registered member', 'sportszone' ), $userlink );

				// updated_profile.
				} else {
					$r['action'] = sprintf( __( '%s updated their profile', 'sportszone' ), $userlink );
				}
