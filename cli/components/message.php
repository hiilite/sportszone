...
class Message extends BuddypressCommand {
	...
	public function create( $args, $assoc_args ) {
		$r = wp_parse_args( $assoc_args, array(
			'to'        => '',
			'thread-id' => false,
			'date-sent' => sz_core_current_time(),
		) );
	...
	public function get( $args, $assoc_args ) {
		$message     = new \SZ_Messages_Message( $args[0] );
	 ...
	 *
	 * [--<field>=<value>]
	 * : One or more parameters to pass. See \SZ_Messages_Box_Template()
	 *
	 * [--fields=<fields>]
	 * : Fields to display.
	 *
	...
	public function _list( $_, $assoc_args ) {
		...
		$inbox = new \SZ_Messages_Box_Template( array(
			'user_id'      => $user->ID,
			'box'          => $box,
			'type'         => $type,
			'max'          => $r['count'],
	...
	public function star( $args, $assoc_args ) {
		...
		if ( sz_messages_is_message_starred( $msg_id, $user_id ) ) {
		...
		if ( sz_messages_star_set_action( $star_args ) ) {
