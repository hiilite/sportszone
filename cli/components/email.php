...
/**
 * Manage SportsZone Email Post Types.
 *
 * @since 1.6.0
 */
class Email extends BuddypressCommand {
	 ...
	 *
	 * --subject=<subject>
	 * : Email subject line. Email tokens allowed. View https://codex.sportszone.org/emails/email-tokens/ for more info.
	 *
	 * [--content=<content>]
	 * : Email content. Email tokens allowed. View https://codex.sportszone.org/emails/email-tokens/ for more info.
	 *
	 * [--plain-text-content=<plain-text-content>]
	 * : Plain-text email content. Email tokens allowed. View https://codex.sportszone.org/emails/email-tokens/ for more info.
	 *
	 * [<file>]
	...
	public function create( $args, $assoc_args ) {
		...
		if ( false === sz_is_root_blog() ) {
			$switched = true;
			switch_to_blog( sz_get_root_blog_id() );
		}

		$term = term_exists( $assoc_args['type'], sz_get_email_tax_type() );
		...
		// Email post content.
		$post_id = wp_insert_post( sz_parse_args( $email, $defaults, 'install_email_' . $id ), true );
		...
		if ( ! is_wp_error( $post_id ) ) {
			...
			if ( ! is_wp_error( $tt_ids ) && ! empty( $assoc_args['type-description'] ) ) {
				$term = get_term_by( 'term_taxonomy_id', (int) $tt_ids[0], sz_get_email_tax_type() );
	...
	/**
	 * Reinstall SportsZone default emails.
	 *
	 * ## OPTIONS
	 *
	 * [--yes]
	...
	public function reinstall( $args, $assoc_args ) {
		WP_CLI::confirm( 'Are you sure you want to reinstall SportsZone emails?', $assoc_args );

		require_once sportszone()->plugin_dir . 'sz-core/admin/sz-core-admin-tools.php';

		$result = sz_admin_reinstall_emails();
