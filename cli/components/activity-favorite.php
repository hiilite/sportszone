...
/**
 * Manage SportsZone activity favorite.
 *
 * @since 1.5.0
 */
class Activity_Favorite extends BuddypressCommand {
	...
	public function create( $args, $assoc_args ) {
		...
		if ( sz_activity_add_user_favorite( $activity_id, $user->ID ) ) {
	...
	public function remove( $args, $assoc_args ) {
		...
		if ( sz_activity_remove_user_favorite( $activity_id, $user->ID ) ) {
	...
	public function _list( $args, $assoc_args ) {
		...
		$favorites = sz_activity_get_user_favorites( $user->ID );

		if ( ! $favorites ) {
			WP_CLI::error( 'No favorite found for this user.' );
		}

		$activities = sz_activity_get_specific( array(
