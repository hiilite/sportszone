 ...
 *     +--------+---------+--------+------------------------------------------+--------------------------+
 *     | 1      | core    | Active | Núcleo do SportsZone   | É o que torna <del>viajar no tempo</del> o |
 *     |        |         |        |                        | SportsZone possível!                       |
 *     | 2      | members | Active | Membros da Comunidade  | Tudo em uma comunidade SportsZone gira em  |
...
class Components extends BuddypressCommand {
	...
	public function activate( $args, $assoc_args ) {
		...
		$active_components =& sportszone()->active_components;

		// Set for the rest of the page load.
		$active_components[ $component ] = 1;

		// Save in the db.
		sz_update_option( 'sz-active-components', $active_components );
		...
		require_once( \SZ_PLUGIN_DIR . 'sz-core/admin/sz-core-admin-schema.php' );
		sz_core_install( $active_components );
		sz_core_add_page_mappings( $active_components );
	...
	public function deactivate( $args, $assoc_args ) {
		...
		if ( array_key_exists( $component, sz_core_get_components( 'required' ) ) ) {
	...
	protected function component_exists( $component ) {
		$keys = array_keys( sz_core_get_components() );
	...
	protected function verify_component_status( $id ) {
		...
		return ( sz_is_active( $id ) ) ? $active : 'Inactive';
	}
