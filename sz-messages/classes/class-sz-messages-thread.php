...
/**
 * SportsZone Message Thread class.
 *
 * @since 1.0.0
 */
class SZ_Messages_Thread {
	 ...
	 * @since 1.0.0
	 *
	 * @see SZ_Messages_Thread::populate() for full description of parameters.
	 *
	...
	public function populate( $thread_id = 0, $order = 'ASC', $args = array() ) {
		...
		if ( true === (bool) $r['update_meta_cache'] ) {
			sz_messages_update_meta_cache( wp_list_pluck( $this->messages, 'id' ) );
		}

		/**
		 * Fires after a SZ_Messages_Thread object has been populated.
		 *
		 * @since 2.2.0
		 *
		 * @param SZ_Messages_Thread $this Message thread object.
		 */
		do_action( 'sz_messages_thread_post_populate', $this );
	}
	 ...
	 * @since 1.0.0
	 *
	 * @see SZ_Messages_Thread::mark_as_read()
	 */
	public function mark_read() {
		SZ_Messages_Thread::mark_as_read( $this->thread_id );
	}
	 ...
	 * @since 1.0.0
	 *
	 * @see SZ_Messages_Thread::mark_as_unread()
	 */
	public function mark_unread() {
		SZ_Messages_Thread::mark_as_unread( $this->thread_id );
	}
	...
	public function get_recipients( $thread_id = 0 ) {
		...
		$recipients = wp_cache_get( 'thread_recipients_' . $thread_id, 'sz_messages' );
		if ( false === $recipients ) {
			$sz = sportszone();

			$recipients = array();
			$sql        = $wpdb->prepare( "SELECT * FROM {$sz->messages->table_name_recipients} WHERE thread_id = %d", $thread_id );
			...
			wp_cache_set( 'thread_recipients_' . $thread_id, $recipients, 'sz_messages' );
		}
		 ...
		 */
		return apply_filters( 'sz_messages_thread_get_recipients', $recipients, $thread_id );
	}
	...
	public static function get_messages( $thread_id = 0 ) {
		$thread_id = (int) $thread_id;
		$messages  = wp_cache_get( $thread_id, 'sz_messages_threads' );
		...
		if ( false === $messages ) {
			...
			$messages = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$sz->messages->table_name_messages} WHERE thread_id = %d ORDER BY date_sent ASC", $thread_id ) );

			wp_cache_set( $thread_id, (array) $messages, 'sz_messages_threads' );
		}
	...
	public static function delete( $thread_id = 0, $user_id = 0 ) {
		 ...
		 */
		do_action( 'sz_messages_thread_before_mark_delete', $thread_id, $user_id );

		$sz = sportszone();

		// Mark messages as deleted
		$wpdb->query( $wpdb->prepare( "UPDATE {$sz->messages->table_name_recipients} SET is_deleted = 1 WHERE thread_id = %d AND user_id = %d", $thread_id, $user_id ) );

		// Get the message ids in order to pass to the action.
		$message_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$sz->messages->table_name_messages} WHERE thread_id = %d", $thread_id ) );

		// Check to see if any more recipients remain for this message.
		$recipients = $wpdb->get_results( $wpdb->prepare( "SELECT id FROM {$sz->messages->table_name_recipients} WHERE thread_id = %d AND is_deleted = 0", $thread_id ) );
		...
		if ( empty( $recipients ) ) {
			 ...
			 */
			do_action( 'sz_messages_thread_before_delete', $thread_id, $message_ids );

			// Delete all the messages.
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->messages->table_name_messages} WHERE thread_id = %d", $thread_id ) );

			// Do something for each message ID.
			foreach ( $message_ids as $message_id ) {

				// Delete message meta.
				sz_messages_delete_meta( $message_id );
			...
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->messages->table_name_recipients} WHERE thread_id = %d", $thread_id ) );
		}
		 ...
		 */
		do_action( 'sz_messages_thread_after_delete', $thread_id, $message_ids, $user_id );
	...
	public static function get_current_threads_for_user( $args = array() ) {
		...
		if ( ! is_array( $args ) || func_num_args() > 1 ) {
			_deprecated_argument( __METHOD__, '2.2.0', sprintf( __( 'Arguments passed to %1$s should be in an associative array. See the inline documentation at %2$s for more details.', 'sportszone' ), __METHOD__, __FILE__ ) );
		...
		$sql['from']   = "FROM {$sz->messages->table_name_recipients} r INNER JOIN {$sz->messages->table_name_messages} m ON m.thread_id = r.thread_id {$meta_query_sql['join']}";
		...
		foreach ( (array) $sorted_threads as $thread_id => $date_sent ) {
			$threads[] = new SZ_Messages_Thread( $thread_id, 'ASC', array(
		 ...
		 * }
		 */
		return apply_filters( 'sz_messages_thread_current_threads', array(
			'threads' => &$threads,
	...
	/**
	 * Get the SQL for the 'meta_query' param in SZ_Messages_Thread::get_current_threads_for_user().
	 *
	...
	public static function get_meta_query_sql( $meta_query = array() ) {
		...
		if ( ! empty( $meta_query ) ) {
			...
			// $wpdb->messagemeta.
			$wpdb->messagemeta = sportszone()->messages->table_name_meta;
	...
	public static function mark_as_read( $thread_id = 0 ) {
		...
		$sz     = sportszone();
		$retval = $wpdb->query( $wpdb->prepare( "UPDATE {$sz->messages->table_name_recipients} SET unread_count = 0 WHERE user_id = %d AND thread_id = %d", $user_id, $thread_id ) );

		wp_cache_delete( 'thread_recipients_' . $thread_id, 'sz_messages' );
		wp_cache_delete( $user_id, 'sz_messages_unread_count' );
	...
	public static function mark_as_unread( $thread_id = 0 ) {
		...
		$sz     = sportszone();
		$retval = $wpdb->query( $wpdb->prepare( "UPDATE {$sz->messages->table_name_recipients} SET unread_count = 1 WHERE user_id = %d AND thread_id = %d", $user_id, $thread_id ) );
	...
	public static function get_total_threads_for_user( $user_id, $box = 'inbox', $type = 'all' ) {
		...
		return (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(thread_id) FROM {$sz->messages->table_name_recipients} WHERE user_id = %d AND is_deleted = 0 {$exclude_sender} {$type_sql}", $user_id ) );
	}
	...
	public static function user_is_sender( $thread_id ) {
		...
		$sender_ids = $wpdb->get_col( $wpdb->prepare( "SELECT sender_id FROM {$sz->messages->table_name_messages} WHERE thread_id = %d", $thread_id ) );

		if ( empty( $sender_ids ) ) {
			return false;
		}

		return in_array( sz_loggedin_user_id(), $sender_ids );
	}
	...
	public static function get_last_sender( $thread_id ) {
		...
		if ( ! $sender_id = $wpdb->get_var( $wpdb->prepare( "SELECT sender_id FROM {$sz->messages->table_name_messages} WHERE thread_id = %d GROUP BY sender_id ORDER BY date_sent LIMIT 1", $thread_id ) ) ) {
			return false;
		}

		return sz_core_get_userlink( $sender_id, true );
	}
	...
	public static function get_inbox_count( $user_id = 0 ) {
		...
		$unread_count = wp_cache_get( $user_id, 'sz_messages_unread_count' );

		if ( false === $unread_count ) {
			$sz = sportszone();

			$unread_count = (int) $wpdb->get_var( $wpdb->prepare( "SELECT SUM(unread_count) FROM {$sz->messages->table_name_recipients} WHERE user_id = %d AND is_deleted = 0 AND sender_only = 0", $user_id ) );

			wp_cache_set( $user_id, $unread_count, 'sz_messages_unread_count' );
		}
	...
	public static function get_recipient_links( $recipients ) {
		...
		if ( count( $recipients ) >= 5 ) {
			return sprintf( __( '%s Recipients', 'sportszone' ), number_format_i18n( count( $recipients ) ) );
		}
	...
	public static function update_tables() {
		...
		$errors    = false;
		$threads   = $wpdb->get_results( "SELECT * FROM {$sz_prefix}sz_messages_threads" );
		...
		foreach( (array) $threads as $thread ) {
			...
			if ( ! empty( $message_ids ) ) {
				...
				if ( ! $wpdb->query( $wpdb->prepare( "UPDATE {$sz->messages->table_name_messages} SET thread_id = %d WHERE id IN ({$message_ids})", $thread->id ) ) ) {
					$errors = true;
				}
			}
		}
