...
class SZ_Messages_Message {
	...
	public function __construct( $id = null ) {
		$this->date_sent = sz_core_current_time();
		$this->sender_id = sz_loggedin_user_id();
	...
	public function populate( $id ) {
		...
		if ( $message = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$sz->messages->table_name_messages} WHERE id = %d", $id ) ) ) {
	...
	public function send() {
		 ...
		 * @since 1.0.0
		 *
		 * @param SZ_Messages_Message $this Current instance of the message item being saved. Passed by reference.
		 */
		...
		if ( empty( $this->thread_id ) ) {
			$this->thread_id = (int) $wpdb->get_var( "SELECT MAX(thread_id) FROM {$sz->messages->table_name_messages}" ) + 1;
			$new_thread      = true;
		}

		// First insert the message into the messages table.
		if ( ! $wpdb->query( $wpdb->prepare( "INSERT INTO {$sz->messages->table_name_messages} ( thread_id, sender_id, subject, message, date_sent ) VALUES ( %d, %d, %s, %s, %s )", $this->thread_id, $this->sender_id, $this->subject, $this->message, $this->date_sent ) ) ) {
			return false;
		}
		...
		if ( $new_thread ) {
			...
			foreach ( (array) $this->recipients as $recipient ) {
				$wpdb->query( $wpdb->prepare( "INSERT INTO {$sz->messages->table_name_recipients} ( user_id, thread_id, unread_count ) VALUES ( %d, %d, 1 )", $recipient->user_id, $this->thread_id ) );
			...
			if ( ! in_array( $this->sender_id, $recipient_ids ) ) {
				$wpdb->query( $wpdb->prepare( "INSERT INTO {$sz->messages->table_name_recipients} ( user_id, thread_id, sender_only ) VALUES ( %d, %d, 1 )", $this->sender_id, $this->thread_id ) );
			}
		} else {
			// Update the unread count for all recipients.
			$wpdb->query( $wpdb->prepare( "UPDATE {$sz->messages->table_name_recipients} SET unread_count = unread_count + 1, sender_only = 0, is_deleted = 0 WHERE thread_id = %d AND user_id != %d", $this->thread_id, $this->sender_id ) );
		}
	...
	public function get_recipients() {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT user_id FROM {$sz->messages->table_name_recipients} WHERE thread_id = %d", $this->thread_id ) );
	}
	...
	public static function get_recipient_ids( $recipient_usernames ) {
		...
		if ( is_array( $recipient_usernames ) ) {
			...
			for ( $i = 0, $count = $rec_un_count; $i < $count; ++ $i ) {
				if ( $rid = sz_core_get_userid( trim( $recipient_usernames[ $i ] ) ) ) {
					$recipient_ids[] = $rid;
				}
			}
		}
	...
	public static function get_last_sent_for_user( $thread_id ) {
		...
		$query = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$sz->messages->table_name_messages} WHERE sender_id = %d AND thread_id = %d ORDER BY date_sent DESC LIMIT 1", sz_loggedin_user_id(), $thread_id ) );
	...
	public static function is_user_sender( $user_id, $message_id ) {
		...
		$query = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$sz->messages->table_name_messages} WHERE sender_id = %d AND id = %d", $user_id, $message_id ) );
	...
	public static function get_message_sender( $message_id ) {
		...
		$query = $wpdb->get_var( $wpdb->prepare( "SELECT sender_id FROM {$sz->messages->table_name_messages} WHERE id = %d", $message_id ) );
