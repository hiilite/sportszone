...
class SZ_Messages_Notice {
	...
	public function populate() {
		...
		$notice = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$sz->messages->table_name_notices} WHERE id = %d", $this->id ) );
	...
	public function save() {
		 ...
		 * @since 1.0.0
		 *
		 * @param SZ_Messages_Notice $this Current instance of the message notice item being saved. Passed by reference.
		 */
		do_action_ref_array( 'messages_notice_before_save', array( &$this ) );

		if ( empty( $this->id ) ) {
			$sql = $wpdb->prepare( "INSERT INTO {$sz->messages->table_name_notices} (subject, message, date_sent, is_active) VALUES (%s, %s, %s, %d)", $this->subject, $this->message, $this->date_sent, $this->is_active );
		} else {
			$sql = $wpdb->prepare( "UPDATE {$sz->messages->table_name_notices} SET subject = %s, message = %s, is_active = %d WHERE id = %d", $this->subject, $this->message, $this->is_active, $this->id );
		}
		...
		$wpdb->query( $wpdb->prepare( "UPDATE {$sz->messages->table_name_notices} SET is_active = 0 WHERE id != %d", $id ) );

		sz_update_user_last_activity( sz_loggedin_user_id(), sz_core_current_time() );
		 ...
		 * @since 1.0.0
		 *
		 * @param SZ_Messages_Notice $this Current instance of the message item being saved. Passed by reference.
		 */
	...
	public function delete() {
		 ...
		 * @since 1.0.0
		 *
		 * @param SZ_Messages_Notice $this Current instance of the message notice item being deleted.
		 */
		do_action( 'messages_notice_before_delete', $this );

		$sz  = sportszone();
		$sql = $wpdb->prepare( "DELETE FROM {$sz->messages->table_name_notices} WHERE id = %d", $this->id );
	...
	public static function get_notices( $args = array() ) {
		...
		$notices = $wpdb->get_results( "SELECT * FROM {$sz->messages->table_name_notices} ORDER BY date_sent DESC {$limit_sql}" );
	...
	public static function get_total_notice_count() {
		...
		$notice_count = $wpdb->get_var( "SELECT COUNT(id) FROM {$sz->messages->table_name_notices}" );
	 ...
	 * @since 1.0.0
	 *
	 * @return object The SZ_Messages_Notice object.
	 */
	public static function get_active() {
		$notice = wp_cache_get( 'active_notice', 'sz_messages' );
		...
		if ( false === $notice ) {
			...
			$notice_id = $wpdb->get_var( "SELECT id FROM {$sz->messages->table_name_notices} WHERE is_active = 1" );
			$notice    = new SZ_Messages_Notice( $notice_id );

			wp_cache_set( 'active_notice', $notice, 'sz_messages' );
		}
