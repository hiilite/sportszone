<?php
/**
 * SportsZone Messages Box Template Class.
 *
 * @package SportsZone
 ...
 */
class SZ_Messages_Box_Template {
	 ...
	 *
	 * @param array $args {
	 *     Array of arguments. See sz_has_message_threads() for full description.
	 * }
	 */
	public function __construct( $args = array() ) {
		...
		if ( 'notices' === $this->box ) {
			$this->threads = SZ_Messages_Notice::get_notices( array(
				'pag_num'  => $this->pag_num,
				'pag_page' => $this->pag_page
			) );
		} else {
			$threads = SZ_Messages_Thread::get_current_threads_for_user( array(
		...
		if ( !$this->threads ) {
			...
			$total_notice_count = SZ_Messages_Notice::get_total_notice_count();
		...
		if ( (int) $this->total_thread_count && (int) $this->pag_num ) {
			...
			$this->pag_links = paginate_links( array(
				...
				'prev_text' => _x( '&larr;', 'Message pagination previous text', 'sportszone' ),
				'next_text' => _x( '&rarr;', 'Message pagination next text', 'sportszone' ),
				'mid_size'  => 1,
				'add_args'  => $add_args,
			) );
		}
	 ...
	 *
	 * This method is used by {@link sz_message_threads()} as part of the
	 * while loop that controls iteration inside the threads loop, eg:
	 *     while ( sz_message_threads() ) { ...
	 *
	 * @see sz_message_threads()
	 *
			...
			if ( count($this->thread->messages) > 1 ) {
				if ( 'inbox' == $this->box ) {
					foreach ( (array) $this->thread->messages as $key => $message ) {
						if ( sz_loggedin_user_id() != $message->sender_id ) {
