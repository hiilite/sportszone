 ...
 * @return boolean
 */
function sz_messages_action_create_message() {

	// Bail if not posting to the compose message screen.
	if ( ! sz_is_post_request() || ! sz_is_messages_component() || ! sz_is_current_action( 'compose' ) ) {
		return false;
	}
	...
	if ( empty( $_POST['subject'] ) || empty( $_POST['content'] ) ) {
		...
		if ( empty( $_POST['subject'] ) ) {
			$feedback = __( 'Your message was not sent. Please enter a subject line.', 'sportszone' );
		} else {
	...
	// Subject and content present.
		...
		$member_messages = trailingslashit( sz_loggedin_user_domain() . sz_get_messages_slug() );
		...
		if ( isset( $_POST['send-notice'] ) ) {
			...
			if ( messages_send_notice( $_POST['subject'], $_POST['content'] ) ) {
				$success     = true;
				$feedback    = __( 'Notice successfully created.', 'sportszone' );
				$redirect_to = trailingslashit( $member_messages . 'notices' );

			// Notice could not be sent.
			} else {
				$success  = false;
				$feedback = __( 'Notice was not created. Please try again.', 'sportszone' );
			}
		...
		// Private conversation.
			 ...
			 */
			$recipients = apply_filters( 'sz_messages_recipients', $recipients );
...
add_action( 'sz_actions', 'sz_messages_action_create_message' );