...
function messages_new_message( $args = '' ) {
	...
	$r = sz_parse_args( $args, array(
		...
		'subject'    => false,
		'content'    => false,
		'date_sent'  => sz_core_current_time(),
		'error_type' => 'bool'
	...
	if ( empty( $r['sender_id'] ) || empty( $r['content'] ) ) {
		if ( 'wp_error' === $r['error_type'] ) {
			if ( empty( $r['sender_id'] ) ) {
				...
				$feedback   = __( 'Your message was not sent. Please use a valid sender.', 'sportszone' );
			} else {
				$error_code = 'messages_empty_content';
				$feedback   = __( 'Your message was not sent. Please enter some content.', 'sportszone' );
			}
	...
	$message            = new SZ_Messages_Message;
	...
	if ( ! empty( $r['thread_id'] ) ) {
		...
		if ( empty( $message->subject ) ) {
			$message->subject = sprintf( __( 'Re: %s', 'sportszone' ), $thread->messages[0]->subject );
		}
	...
	// ...otherwise use the recipients passed
		...
		if ( empty( $r['recipients'] ) ) {
			if ( 'wp_error' === $r['error_type'] ) {
				return new WP_Error( 'message_empty_recipients', __( 'Message could not be sent. Please enter a recipient.', 'sportszone' ) );
			} else {
				return false;
			}
		}
		...
		if ( empty( $message->subject ) ) {
			$message->subject = __( 'No Subject', 'sportszone' );
		}
		...
		foreach ( (array) $r['recipients'] as $recipient ) {
			...
			// @see http://sportszone.trac.wordpress.org/ticket/5151.
			if ( sz_is_username_compatibility_mode() ) {
				$recipient_id = sz_core_get_userid( urldecode( $recipient ) );
			} else {
				$recipient_id = sz_core_get_userid_from_nicename( $recipient );
			}

			// Check against user ID column if no match and if passed recipient is numeric.
			if ( empty( $recipient_id ) && is_numeric( $recipient ) ) {
				if ( sz_core_get_core_userdata( (int) $recipient ) ) {
		...
		if ( empty( $recipient_ids ) ) {
			if ( 'wp_error' === $r['error_type'] ) {
				return new WP_Error( 'message_invalid_recipients', __( 'Message could not be sent because you have entered an invalid username. Please try again.', 'sportszone' ) );
			} else {
				return false;
			}
		}
	...
	if ( false === is_int( $send ) ) {
		if ( 'wp_error' === $r['error_type'] ) {
			if ( is_wp_error( $send ) ) {
				...
				return new WP_Error( 'message_generic_error', __( 'Message was not sent. Please try again.', 'sportszone' ) );
			}
		}
	 ...
	 * @since 1.1.0
	 *
	 * @param SZ_Messages_Message $message Message object. Passed by reference.
	 */
...
function messages_send_notice( $subject, $message ) {
	if ( !sz_current_user_can( 'sz_moderate' ) || empty( $subject ) || empty( $message ) ) {
		return false;

	// Has access to send notices, lets do it.
	} else {
		$notice            = new SZ_Messages_Notice;
		$notice->subject   = $subject;
		$notice->message   = $message;
		$notice->date_sent = sz_core_current_time();
		$notice->is_active = 1;
		$notice->save(); // Send it.
...
function messages_delete_thread( $thread_ids, $user_id = 0 ) {
	...
	if ( is_array( $thread_ids ) ) {
		...
		for ( $i = 0, $count = count( $thread_ids ); $i < $count; ++$i ) {
			if ( ! SZ_Messages_Thread::delete( $thread_ids[$i], $user_id ) ) {
				$error = 1;
			}
		}
		...
		if ( ! SZ_Messages_Thread::delete( $thread_ids, $user_id ) ) {
			return false;
		}

		/** This action is documented in sz-messages/sz-messages-functions.php */
		do_action( 'messages_delete_thread', $thread_ids, $user_id );
...
function messages_check_thread_access( $thread_id, $user_id = 0 ) {
	return SZ_Messages_Thread::check_access( $thread_id, $user_id );
}

/**
 * Mark a thread as read.
 *
 * Wrapper for {@link SZ_Messages_Thread::mark_as_read()}.
 *
 * @param int $thread_id ID of the thread.
 *
...
function messages_mark_thread_read( $thread_id ) {
	return SZ_Messages_Thread::mark_as_read( $thread_id );
}

/**
 * Mark a thread as unread.
 *
 * Wrapper for {@link SZ_Messages_Thread::mark_as_unread()}.
 *
 * @param int $thread_id ID of the thread.
 *
...
function messages_mark_thread_unread( $thread_id ) {
	return SZ_Messages_Thread::mark_as_unread( $thread_id );
}

/**
 * Set messages-related cookies.
 *
 * Saves the 'sz_messages_send_to', 'sz_messages_subject', and
 * 'sz_messages_content' cookies, which are used when setting up the default
...
function messages_add_callback_values( $recipients, $subject, $content ) {
	@setcookie( 'sz_messages_send_to', $recipients, time() + 60 * 60 * 24, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
	@setcookie( 'sz_messages_subject', $subject,    time() + 60 * 60 * 24, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
	@setcookie( 'sz_messages_content', $content,    time() + 60 * 60 * 24, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
}
...
function messages_remove_callback_values() {
	@setcookie( 'sz_messages_send_to', false, time() - 1000, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
	@setcookie( 'sz_messages_subject', false, time() - 1000, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
	@setcookie( 'sz_messages_content', false, time() - 1000, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
}
...
function messages_get_unread_count( $user_id = 0 ) {
	return SZ_Messages_Thread::get_inbox_count( $user_id );
}
...
function messages_is_user_sender( $user_id, $message_id ) {
	return SZ_Messages_Message::is_user_sender( $user_id, $message_id );
}
...
function messages_get_message_sender( $message_id ) {
	return SZ_Messages_Message::get_message_sender( $message_id );
}
...
function messages_is_valid_thread( $thread_id ) {
	return SZ_Messages_Thread::is_valid( $thread_id );
}
...
function messages_get_message_thread_id( $message_id = 0 ) {
	...
	return (int) $wpdb->get_var( $wpdb->prepare( "SELECT thread_id FROM {$sz->messages->table_name_messages} WHERE id = %d", $message_id ) );
}
 ...
 */
function sz_messages_delete_meta( $message_id, $meta_key = false, $meta_value = false, $delete_all = false ) {
 ...
 * @return mixed
 */
function sz_messages_get_meta( $message_id, $meta_key = '', $single = true ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
 ...
 * @return mixed
 */
function sz_messages_update_meta( $message_id, $meta_key, $meta_value, $prev_value = '' ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
 ...
 * @return mixed
 */
function sz_messages_add_meta( $message_id, $meta_key, $meta_value, $unique = false ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
 ...
 * @since 1.0.0
 *
 * @param array|SZ_Messages_Message $raw_args {
 *     Array of arguments. Also accepts a SZ_Messages_Message object.
...
function messages_notification_new_message( $raw_args = array() ) {
	...
	$sender_name = sz_core_get_user_displayname( $sender_id );
	...
	foreach ( $recipients as $recipient ) {
		if ( $sender_id == $recipient->user_id || 'no' == sz_get_user_meta( $recipient->user_id, 'notification_messages_new_message', true ) ) {
			continue;
		}
		...
		sz_send_email( 'messages-unread', $ud, array(
			'tokens' => array(
				'usermessage' => wp_strip_all_tags( stripslashes( $message ) ),
				'message.url' => esc_url( sz_core_get_user_domain( $recipient->user_id ) . sz_get_messages_slug() . '/view/' . $thread_id . '/' ),
	 ...
	 */
	do_action( 'sz_messages_sent_notification_email', $recipients, '', '', $args );
}
