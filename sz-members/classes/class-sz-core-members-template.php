<?php
/**
 * SportsZone Member Template loop class.
 *
 * @package SportsZone
 * @subpackage Members
 */
 ...
 * @since 1.0.0
 */
class SZ_Core_Members_Template {
	 ...
	 * @since 1.5.0
	 *
	 * @see SZ_User_Query for an in-depth description of parameters.
	 *
	...
	function __construct( $type, $page_number, $per_page, $max, $user_id, $search_terms, $include, $populate_extras, $exclude, $meta_key, $meta_value, $page_arg = 'upage', $member_type = '', $member_type__in = '', $member_type__not_in = '' ) {
		...
		$this->pag_page = sz_sanitize_pagination_arg( $this->pag_arg, $page_number );
		$this->pag_num  = sz_sanitize_pagination_arg( 'num',          $per_page    );
		$this->type     = $type;

		if ( !empty( $_REQUEST['letter'] ) )
			$this->members = SZ_Core_User::get_users_by_letter( $_REQUEST['letter'], $this->pag_num, $this->pag_page, $populate_extras, $exclude );
		else
			$this->members = sz_core_get_users( array( 'type' => $this->type, 'per_page' => $this->pag_num, 'page' => $this->pag_page, 'user_id' => $user_id, 'include' => $include, 'search_terms' => $search_terms, 'populate_extras' => $populate_extras, 'exclude' => $exclude, 'meta_key' => $meta_key, 'meta_value' => $meta_value, 'member_type' => $member_type, 'member_type__in' => $member_type__in, 'member_type__not_in' => $member_type__not_in ) );
		...
		if ( (int) $this->total_member_count && (int) $this->pag_num ) {
			...
			$this->pag_links = paginate_links( array(
				...
				'prev_text' => _x( '&larr;', 'Member pagination previous text', 'sportszone' ),
				'next_text' => _x( '&rarr;', 'Member pagination next text', 'sportszone' ),
				'mid_size'  => 1,
				'add_args'  => $add_args,
			) );
		}
	 ...
	 *
	 * This method is used by {@link sz_members()} as part of the while loop
	 ...
	 *
	 * Used by {@link sz_the_member()} to set up the current member data
