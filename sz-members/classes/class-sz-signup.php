...
class SZ_Signup {
	...
	public function populate() {
		...
		$signups_table = sportszone()->members->table_name_signups;
	 ...
	 *                                       Default: 'all' (return SZ_Signup objects).
	 * }
	 * @return array {
	...
	public static function get( $args = array() ) {
		...
		if ( empty( $r['include'] ) ) {
			...
			if ( ! empty( $r['usersearch'] ) ) {
				$search_terms_like = '%' . sz_esc_like( $r['usersearch'] ) . '%';
			...
			$sql['order']	= sz_esc_sql_order( $r['order'] );
		 ...
		 */
		$paged_signups = $wpdb->get_results( apply_filters( 'sz_members_signups_paged_query', join( ' ', $sql ), $sql, $args, $r ) );
		 ...
		 * @param array  $r     Array of parsed arguments for get() method.
		 */
		$total_signups = $wpdb->get_var( apply_filters( 'sz_members_signups_count_query', join( ' ', $sql ), $sql, $args, $r ) );
	...
	public static function add( $args = array() ) {
		...
		$r = sz_parse_args( $args,
			...
			'sz_core_signups_add_args'
		);

		$r['meta'] = maybe_serialize( $r['meta'] );

		$inserted = $wpdb->insert(
			sportszone()->members->table_name_signups,
			$r,
		 ...
		 */
		return apply_filters( 'sz_core_signups_add', $retval );
	}
	...
	public static function add_backcompat( $user_login = '', $user_password = '', $user_email = '', $usermeta = array() ) {
		 ...
		 */
		return apply_filters( 'sz_core_signups_add_backcompat', $user_id );
	}
	...
	public static function check_user_status( $user_id = 0 ) {
		 ...
		 */
		return apply_filters( 'sz_core_signups_check_user_status', intval( $user_status ) );
	}
	...
	public static function validate( $key = '' ) {
		 ...
		 */
		return apply_filters( 'sz_core_signups_validate', $activated );
	}
	...
	public static function count_signups() {
		 ...
		 */
		return apply_filters( 'sz_core_signups_count', (int) $count_signups );
	}
	...
	public static function update( $args = array() ) {
		 ...
		 */
		return apply_filters( 'sz_core_signups_update', $r['signup_id'] );
	}
	...
	public static function resend( $signup_ids = array() ) {
		 ...
		 */
		do_action( 'sz_core_signup_before_resend', $signup_ids );
		...
		foreach ( $signups as $signup ) {
			...
			if ( is_multisite() ) {
				...
				if ( ! empty( $user_id ) && 2 != self::check_user_status( $user_id ) ) {
					...
					$result['errors'][ $signup->signup_id ] = array( $signup->user_login, esc_html__( 'the sign-up has already been activated.', 'sportszone' ) );
				...
				// Send the validation email.
					...
					sz_core_signup_send_validation_email( false, $signup->user_email, $signup->activation_key, $signup->user_login );
				}
			}
		 ...
		 */
		do_action( 'sz_core_signup_after_resend', $signup_ids, $result );
		 ...
		 * @param array $result Updated metadata related to activation emails.
		 */
		return apply_filters( 'sz_core_signup_resend', $result );
	}
	...
	public static function activate( $signup_ids = array() ) {
		 ...
		 */
		do_action( 'sz_core_signup_before_activate', $signup_ids );

		foreach ( $signups as $signup ) {

			$user = sz_core_activate_signup( $signup->activation_key );
		 ...
		 */
		do_action( 'sz_core_signup_after_activate', $signup_ids, $result );
		 ...
		 */
		return apply_filters( 'sz_core_signup_activate', $result );
	}
	...
	public static function delete( $signup_ids = array() ) {
		 ...
		 */
		do_action( 'sz_core_signup_before_delete', $signup_ids );

		foreach ( $signups as $signup ) {
			$user_id = username_exists( $signup->user_login );

			if ( ! empty( $user_id ) && $signup->activation_key === sz_get_user_meta( $user_id, 'activation_key', true ) ) {
				...
				// We have a user id, account is not active, let's delete it.
					...
					sz_core_delete_account( $user_id );
				}
			}
		 ...
		 */
		do_action( 'sz_core_signup_after_delete', $signup_ids, $result );
		 ...
		 */
		return apply_filters( 'sz_core_signup_delete', $result );
	}
}
