...
if ( !class_exists( 'SZ_Members_Admin' ) ) :
...
class SZ_Members_Admin {
	...
	public static function register_members_admin() {
		...
		if ( empty( $sz->members->admin ) ) {
			$sz->members->admin = new self;
		}

		return $sz->members->admin;
	}
	...
	private function setup_globals() {
		...
		// Paths and URLs
		$this->admin_dir = trailingslashit( $sz->plugin_dir  . 'sz-members/admin' ); // Admin path.
		$this->admin_url = trailingslashit( $sz->plugin_url  . 'sz-members/admin' ); // Admin URL.
		$this->css_url   = trailingslashit( $this->admin_url . 'css' ); // Admin CSS URL.
		$this->js_url    = trailingslashit( $this->admin_url . 'js'  ); // Admin CSS URL.

		// Capability depends on config.
		$this->capability = sz_core_do_network_admin() ? 'manage_network_users' : 'edit_users';
		...
		// SportsZone edit user's profile args.
		$this->edit_profile_args = array( 'page' => 'sz-profile-edit' );
		...
		$this->users_page   = '';
		$this->signups_page = '';
		$this->users_url    = sz_get_admin_url( 'users.php' );
		$this->users_screen = sz_core_do_network_admin() ? 'users-network' : 'users';

		// Specific config: SportsZone is not network activated.
		$this->subsite_activated = (bool) is_multisite() && ! sz_is_network_activated();

		// When SportsZone is not network activated, only Super Admin can moderate signups.
	...
	private function setup_actions() {
		...
		add_action( 'sz_members_admin_load', array( $this, 'process_member_type_update' ) );
		...
		if ( is_admin() ) {
			...
			if ( current_user_can( $this->capability ) ) {
				 ...
				 */
				if ( sz_is_network_activated() && sz_is_multiblog_mode() && false === strpos( $user_screen, '-network' ) ) {
					$user_screen .= '-network';
				}
		...
		if ( is_admin() && sz_get_member_types() ) {
	...
	public function multisite_registration_on( $option_name, $value ) {
		if ( 'user' === $value || 'all' === $value ) {
			sz_core_add_page_mappings( array(
				'register' => 1,
				'activate' => 1
			) );
		}
	 ...
	 *
	 * Self profile editing / or sz_moderate check.
	...
	private function member_can_edit( $user_id = 0 ) {
		...
		// Trust the 'sz_moderate' capability.
		} else {
			$retval = sz_current_user_can( 'sz_moderate' );
		}
	...
	private function get_user_notice() {
		...
		if ( ! empty( $_REQUEST['updated'] ) ) {
			...
			case 'avatar':
				$notice = array(
					'class'   => 'updated',
					'message' => __( 'Profile photo was deleted.', 'sportszone' )
				);
				break;
			case 'ham' :
				$notice = array(
					'class'   => 'updated',
					'message' => __( 'User removed as spammer.', 'sportszone' )
				);
				break;
			case 'spam' :
				$notice = array(
					'class'   => 'updated',
					'message' => __( 'User marked as spammer. Spam users are visible only to site admins.', 'sportszone' )
				);
				break;
			case 1 :
				$notice = array(
					'class'   => 'updated',
					'message' => __( 'Profile updated.', 'sportszone' )
				);
				break;
			}
		}
		...
		if ( ! empty( $_REQUEST['error'] ) ) {
			...
			case 'avatar':
				$notice = array(
					'class'   => 'error',
					'message' => __( 'There was a problem deleting that profile photo. Please try again.', 'sportszone' )
				);
				break;
			case 'ham' :
				$notice = array(
					'class'   => 'error',
					'message' => __( 'User could not be removed as spammer.', 'sportszone' )
				);
				break;
			case 'spam' :
				$notice = array(
					'class'   => 'error',
					'message' => __( 'User could not be marked as spammer.', 'sportszone' )
				);
				break;
			case 1 :
				$notice = array(
					'class'   => 'error',
					'message' => __( 'An error occurred while trying to update the profile.', 'sportszone' )
				);
				break;
			case 2:
				$notice = array(
					'class'   => 'error',
					'message' => __( 'Your changes have not been saved. Please fill in all required fields, and save your changes again.', 'sportszone' )
				);
				break;
			case 3:
				$notice = array(
					'class'   => 'error',
					'message' => __( 'There was a problem updating some of your profile information. Please try again.', 'sportszone' )
				);
				break;
			}
		}
	...
	public function admin_menus() {
		...
		if ( ( ! sz_is_network_activated() && ! is_network_admin() ) || ( is_network_admin() && sz_is_network_activated() ) ) {

			// Manage signups.
			$hooks['signups'] = $this->signups_page = add_users_page(
				__( 'Manage Signups',  'sportszone' ),
				__( 'Manage Signups',  'sportszone' ),
				$this->capability,
				'sz-signups',
	...
	public function profile_admin_head() {
		...
		} elseif ( isset( $_GET['user_id' ] ) || ( isset( $_GET['page'] ) && ( 'sz-profile-edit' === $_GET['page'] ) ) ) {
	...
	public function admin_head() {
		remove_submenu_page( 'users.php',   'sz-profile-edit' );
		remove_submenu_page( 'profile.php', 'sz-profile-edit' );
	}
	...
	public function enqueue_scripts() {
		 ...
		 */
		$css = apply_filters( 'sz_members_admin_css', $css );

		wp_enqueue_style( 'sz-members-css', $css, array(), sz_get_version() );

		wp_style_add_data( 'sz-members-css', 'rtl', true );
		if ( $min ) {
			wp_style_add_data( 'sz-members-css', 'suffix', $min );
		}

		// Only load JavaScript for SportsZone profile.
		if ( get_current_screen()->id == $this->user_page ) {
			 ...
			 */
			$js = apply_filters( 'sz_members_admin_js', $js );
			wp_enqueue_script( 'sz-members-js', $js, array( 'jquery' ), sz_get_version(), true );
		}
		 ...
		 */
		do_action( 'sz_members_admin_enqueue_scripts', get_current_screen()->id, $this->screen_id );
	}
	...
	public function profile_nav( $user = null, $active = 'WordPress' ) {
		...
		if ( 'SportsZone' === $active ) {
			$sz_active = ' nav-tab-active';
			$wp_active = false;
		} ?>
		...
		<h2 id="profile-nav" class="nav-tab-wrapper">
			<?php
			/**
			 * In configs where SportsZone is not network activated, as regular
			 * admins do not have the capacity to edit other users, we must add
			 * this check.
			 */
			if ( current_user_can( 'edit_user', $user->ID ) ) : ?>

				<a class="nav-tab<?php echo esc_attr( $wp_active ); ?>" href="<?php echo esc_url( $wordpress_url );?>"><?php _e( 'Profile', 'sportszone' ); ?></a>

			<?php endif; ?>

			<a class="nav-tab<?php echo esc_attr( $sz_active ); ?>" href="<?php echo esc_url( $community_url );?>"><?php _e( 'Extended Profile', 'sportszone' ); ?></a>
		</h2>
	...
	public function user_admin_load() {
		...
		if ( ! $this->member_can_edit( $user_id ) ) {
			wp_die( __( 'You cannot edit the requested user.', 'sportszone' ) );
		}
		...
		if ( ! empty( $_REQUEST['user_status'] ) ) {
			...
			if ( $spam !== sz_is_user_spammer( $user_id ) ) {
		 ...
		 */
		do_action_ref_array( 'sz_members_admin_load', array( $doaction, $_REQUEST ) );
		 ...
		 */
		$allowed_actions = apply_filters( 'sz_members_admin_allowed_actions', array( 'update', 'delete_avatar', 'spam', 'ham' ) );
		...
		if ( ! in_array( $doaction, $allowed_actions ) ) {
			...
			get_current_screen()->add_help_tab( array(
				'id'      => 'sz-profile-edit-overview',
				'title'   => __( 'Overview', 'sportszone' ),
				'content' =>
				'<p>' . __( 'This is the admin view of a user&#39;s profile.', 'sportszone' ) . '</p>' .
				'<p>' . __( 'In the main column, you can edit the fields of the user&#39;s extended profile.', 'sportszone' ) . '</p>' .
				'<p>' . __( 'In the right-hand column, you can update the user&#39;s status, delete the user&#39;s avatar, and view recent statistics.', 'sportszone' ) . '</p>'
			) );

			// Help panel - sidebar links.
			get_current_screen()->set_help_sidebar(
				'<p><strong>' . __( 'For more information:', 'sportszone' ) . '</strong></p>' .
				'<p>' . __( '<a href="https://codex.sportszone.org/administrator-guide/extended-profiles/">Managing Profiles</a>', 'sportszone' ) . '</p>' .
			...
			add_meta_box(
				'submitdiv',
				_x( 'Status', 'members user-admin edit screen', 'sportszone' ),
			 ...
			 *
			 * Plugins should not use this hook, please use 'sz_members_admin_user_metaboxes' instead.
			 *
			 * @since 2.0.0
			 *
			 ...
			 */
			do_action_ref_array( 'sz_members_admin_xprofile_metabox', array( $user_id, get_current_screen()->id, $this->stats_metabox ) );
			...
			if ( 'normal' === $this->stats_metabox->context ) {
				...
				$display_name = __( 'Member', 'sportszone' );
			}

			// User Stat metabox.
			add_meta_box(
				'sz_members_admin_user_stats',
				sprintf( _x( "%s's Stats", 'members user-admin edit screen', 'sportszone' ), $display_name ),
			...
			$member_types = sz_get_member_types();
			if ( ! empty( $member_types ) ) {
				add_meta_box(
					'sz_members_admin_member_type',
					_x( 'Member Type', 'members user-admin edit screen', 'sportszone' ),
			 ...
			 *
			 * Plugins can restrict metabox to "sz_moderate" admins by checking if
			 ...
			 */
			do_action( 'sz_members_admin_user_metaboxes', $this->is_self_profile, $user_id );
		...
		// Spam or Ham user.
			...
			if ( sz_core_process_spammer_status( $user_id, $doaction ) ) {
		...
		// Update other stuff once above ones are done.
			 ...
			 */
			do_action_ref_array( 'sz_members_admin_update_user', array( $doaction, $user_id, $_REQUEST, $this->redirect ) );

			sz_core_redirect( $this->redirect );
		}
	}
	...
	public function user_admin() {
		...
		if ( ! sz_current_user_can( 'sz_moderate' ) && empty( $this->is_self_profile ) ) {
			die( '-1' );
		}
		...
		if ( ! empty( $notice ) ) : ?>
			...
			<div <?php if ( 'updated' === $notice['class'] ) : ?>id="message" <?php endif; ?>class="<?php echo esc_attr( $notice['class'] ); ?>">
				...
				<?php if ( !empty( $wp_http_referer ) && ( 'updated' === $notice['class'] ) ) : ?>
					...
					<p><a href="<?php echo esc_url( $wp_http_referer ); ?>"><?php esc_html_e( '&larr; Back to Users', 'sportszone' ); ?></a></p>
		...
		<div class="wrap" id="community-profile-page">
			<?php if ( version_compare( $GLOBALS['wp_version'], '4.8', '>=' ) ) : ?>
				...
				<?php if ( empty( $this->is_self_profile ) ) : ?>
					...
					<?php if ( current_user_can( 'create_users' ) ) : ?>
						...
						<a href="user-new.php" class="page-title-action"><?php echo esc_html_x( 'Add New', 'user', 'sportszone' ); ?></a>

					<?php elseif ( is_multisite() && current_user_can( 'promote_users' ) ) : ?>

						<a href="user-new.php" class="page-title-action"><?php echo esc_html_x( 'Add Existing', 'user', 'sportszone' ); ?></a>
				...
				<h1><?php echo esc_html( $title ); ?>
					...
					<?php if ( empty( $this->is_self_profile ) ) : ?>
						...
						<?php if ( current_user_can( 'create_users' ) ) : ?>
							...
							<a href="user-new.php" class="add-new-h2"><?php echo esc_html_x( 'Add New', 'user', 'sportszone' ); ?></a>

						<?php elseif ( is_multisite() && current_user_can( 'promote_users' ) ) : ?>

							<a href="user-new.php" class="add-new-h2"><?php echo esc_html_x( 'Add Existing', 'user', 'sportszone' ); ?></a>
			...
			<?php if ( ! empty( $user ) ) :
				...
				$this->profile_nav( $user, 'SportsZone' ); ?>
				...
				<form action="<?php echo esc_url( $form_action_url ); ?>" id="your-profile" method="post">
					...
					<?php wp_nonce_field( 'edit-sz-profile_' . $user->ID ); ?>
			...
			<?php else : ?>
				...
				<p><?php
					printf(
						...
						__( 'No user found with this ID.', 'sportszone' ),
						esc_url( sz_get_admin_url( 'users.php' ) ),
	...
	public function user_admin_status_metabox( $user = null ) {
		...
		if ( isset( $user->user_status ) && ( 2 == $user->user_status ) ) : ?>
			...
			<p class="not-activated"><?php esc_html_e( 'User account has not yet been activated', 'sportszone' ); ?></p><br/>
		...
		<div class="submitbox" id="submitcomment">
			<div id="minor-publishing">
				<div id="misc-publishing-actions">
					...
					$is_spammer = sz_is_user_spammer( $user->ID );

					/**
					 * In configs where SportsZone is not network activated,
					...
					if ( ( empty( $this->is_self_profile ) && ( ! in_array( $user->user_login, get_super_admins() ) ) && empty( $this->subsite_activated ) ) || ( ! empty( $this->subsite_activated ) && current_user_can( 'manage_network_users' ) ) ) : ?>
						...
						<div class="misc-pub-section" id="comment-status-radio">
							<label class="approved"><input type="radio" name="user_status" value="ham" <?php checked( $is_spammer, false ); ?>><?php esc_html_e( 'Active', 'sportszone' ); ?></label><br />
							<label class="spam"><input type="radio" name="user_status" value="spam" <?php checked( $is_spammer, true ); ?>><?php esc_html_e( 'Spammer', 'sportszone' ); ?></label>
						</div>
					...
					<div class="misc-pub-section curtime misc-pub-section-last">
						...
						$datef = __( 'M j, Y @ G:i', 'sportszone' );
						$date  = date_i18n( $datef, strtotime( $user->user_registered ) );
						?>
						<span id="timestamp"><?php printf( __( 'Registered on: %s', 'sportszone' ), '<strong>' . $date . '</strong>' ); ?></span>
					</div>
			...
			<div id="major-publishing-actions">
				...
				<div id="publishing-action">
					<a class="button sz-view-profile" href="<?php echo esc_url( sz_core_get_user_domain( $user->ID ) ); ?>" target="_blank"><?php esc_html_e( 'View Profile', 'sportszone' ); ?></a>
					<?php submit_button( esc_html__( 'Update Profile', 'sportszone' ), 'primary', 'save', false ); ?>
				</div>
				<div class="clear"></div>
	...
	public function user_admin_stats_metabox( $user = null ) {
		...
		// Account is activated, getting user's last activity.
			...
			$last_active = sz_get_user_last_activity( $user->ID );
		}
		...
		<ul>
			<li class="sz-members-profile-stats"><?php printf( __( 'Last active: %1$s', 'sportszone' ), '<strong>' . $date . '</strong>' ); ?></li>
			...
			if ( empty( $user->user_status ) ) {
				 ...
				 */
				do_action( 'sz_members_admin_user_stats', array( 'user_id' => $user->ID ), $user );
			}
			?>
		</ul>
	...
	public function user_admin_member_type_metabox( $user = null ) {
		...
		$current_type = sz_get_member_type( $user->ID );
		?>

		<label for="sz-members-profile-member-type" class="screen-reader-text"><?php
			/* translators: accessibility text */
			esc_html_e( 'Select member type', 'sportszone' );
		?></label>
		<select name="sz-members-profile-member-type" id="sz-members-profile-member-type">
			<option value="" <?php selected( '', $current_type ); ?>><?php
				/* translators: no option picked in select box */
				esc_attr_e( '----', 'sportszone' );
			?></option>
		...
		wp_nonce_field( 'sz-member-type-change-' . $user->ID, 'sz-member-type-nonce' );
	}
	...
	public function process_member_type_update() {
		if ( ! isset( $_POST['sz-member-type-nonce'] ) || ! isset( $_POST['sz-members-profile-member-type'] ) ) {
			return;
		}

		$user_id = $this->get_user_id();

		check_admin_referer( 'sz-member-type-change-' . $user_id, 'sz-member-type-nonce' );

		// Permission check.
		if ( ! sz_current_user_can( 'sz_moderate' ) && $user_id != sz_loggedin_user_id() ) {
			return;
		}

		// Member type string must either reference a valid member type, or be empty.
		$member_type = stripslashes( $_POST['sz-members-profile-member-type'] );
		 ...
		 */
		if ( sz_set_member_type( $user_id, $member_type ) ) {
	...
	public function row_actions( $actions = '', $user = null ) {
		...
		if ( current_user_can( 'edit_user', $user->ID ) || sz_current_user_can( 'sz_moderate' ) ) {

			// Add query args and setup the Extended link.
			$edit_profile      = add_query_arg( $args, $this->edit_profile_url );
			$edit_profile_link = sprintf( '<a href="%1$s">%2$s</a>',  esc_url( $edit_profile ), esc_html__( 'Extended', 'sportszone' ) );
	...
	public function add_edit_profile_url_filter() {
		add_filter( 'sz_members_edit_profile_url', array( $this, 'filter_adminbar_profile_link' ), 10, 3 );
	}
	...
	public function remove_edit_profile_url_filter() {
		remove_filter( 'sz_members_edit_profile_url', array( $this, 'filter_adminbar_profile_link' ), 10 );
	}
	...
	public function signup_screen_options( $value = 0, $option = '', $new_value = 0 ) {
		if ( 'users_page_sz_signups_network_per_page' != $option && 'users_page_sz_signups_per_page' != $option ) {
			return $value;
		}
	...
	/**
	 * Filter the WP Users List Table views to include 'sz-signups'.
	 *
	 * @since 2.0.0
	 *
	...
	public function signup_filter_view( $views = array() ) {
		...
		$signups = SZ_Signup::count_signups();

		if ( is_network_admin() ) {
			$base_url = network_admin_url( 'users.php' );
		} else {
			$base_url = sz_get_admin_url( 'users.php' );
		}
	 ...
	 * @since 2.0.0
	 *
	 * @global $sz_members_signup_list_table
	 */
	public function signups_admin_load() {
		 ...
		 * @param array  $_REQUEST Current $_REQUEST global.
		 */
		do_action( 'sz_signups_admin_load', $doaction, $_REQUEST );
		 ...
		 * @param array $value Array of allowed actions to use.
		 */
		$allowed_actions = apply_filters( 'sz_signups_admin_allowed_actions', array( 'do_delete', 'do_activate', 'do_resend' ) );

		// Prepare the display of the Community Profile screen.
		if ( ! in_array( $doaction, $allowed_actions ) || ( -1 == $doaction ) ) {

			if ( is_network_admin() ) {
				$sz_members_signup_list_table = self::get_list_table_class( 'SZ_Members_MS_List_Table', 'ms-users' );
			} else {
				$sz_members_signup_list_table = self::get_list_table_class( 'SZ_Members_List_Table', 'users' );
			}

			// The per_page screen option.
			add_screen_option( 'per_page', array( 'label' => _x( 'Pending Accounts', 'Pending Accounts per page (screen options)', 'sportszone' ) ) );

			get_current_screen()->add_help_tab( array(
				'id'      => 'sz-signups-overview',
				'title'   => __( 'Overview', 'sportszone' ),
				'content' =>
				'<p>' . __( 'This is the administration screen for pending accounts on your site.', 'sportszone' ) . '</p>' .
				'<p>' . __( 'From the screen options, you can customize the displayed columns and the pagination of this screen.', 'sportszone' ) . '</p>' .
				'<p>' . __( 'You can reorder the list of your pending accounts by clicking on the Username, Email or Registered column headers.', 'sportszone' ) . '</p>' .
				'<p>' . __( 'Using the search form, you can find pending accounts more easily. The Username and Email fields will be included in the search.', 'sportszone' ) . '</p>'
			) );

			get_current_screen()->add_help_tab( array(
				'id'      => 'sz-signups-actions',
				'title'   => __( 'Actions', 'sportszone' ),
				'content' =>
				'<p>' . __( 'Hovering over a row in the pending accounts list will display action links that allow you to manage pending accounts. You can perform the following actions:', 'sportszone' ) . '</p>' .
				'<ul><li>' . __( '"Email" takes you to the confirmation screen before being able to send the activation link to the desired pending account. You can only send the activation email once per day.', 'sportszone' ) . '</li>' .
				'<li>' . __( '"Delete" allows you to delete a pending account from your site. You will be asked to confirm this deletion.', 'sportszone' ) . '</li></ul>' .
				'<p>' . __( 'By clicking on a Username you will be able to activate a pending account from the confirmation screen.', 'sportszone' ) . '</p>' .
				'<p>' . __( 'Bulk actions allow you to perform these 3 actions for the selected rows.', 'sportszone' ) . '</p>'
			) );
			...
			get_current_screen()->set_screen_reader_content( array(
				/* translators: accessibility text */
				'heading_views'      => __( 'Filter users list', 'sportszone' ),
				/* translators: accessibility text */
				'heading_pagination' => __( 'Pending users list navigation', 'sportszone' ),
				/* translators: accessibility text */
				'heading_list'       => __( 'Pending users list', 'sportszone' ),
			) );
			...
			if ( 'do_resend' == $doaction ) {
				...
				$resent = SZ_Signup::resend( $signups );
				...
				if ( empty( $resent ) ) {
					...
					if ( ! empty( $resent['errors'] ) ) {
						...
						set_transient( '_sz_admin_signups_errors', $resent['errors'], 30 );
					}
			...
			// Handle activated accounts.
				...
				$activated = SZ_Signup::activate( $signups );
				...
				if ( empty( $activated ) ) {
					...
					if ( ! empty( $activated['errors'] ) ) {
						...
						set_transient( '_sz_admin_signups_errors', $activated['errors'], 30 );
					}
			...
			// Handle sign-ups delete.
				...
				$deleted = SZ_Signup::delete( $signups );
				...
				if ( empty( $deleted ) ) {
					...
					if ( ! empty( $deleted['errors'] ) ) {
						...
						set_transient( '_sz_admin_signups_errors', $deleted['errors'], 30 );
					}
			...
			// Plugins can update other stuff from here.
				 ...
				 * @param string $redirect Determined redirect url to send user to.
				 */
				do_action( 'sz_members_admin_update_signups', $doaction, $_REQUEST, $this->redirect );
	...
	public function signups_display_errors() {
		...
		$errors = get_transient( '_sz_admin_signups_errors' );
		...
		delete_transient( '_sz_admin_signups_errors' );
	}
		...
		if ( ! empty( $_REQUEST['error'] ) ) {
			switch ( $_REQUEST['error'] ) {
				case 'do_resend':
					$notice = array(
						'class'   => 'error',
						'message' => esc_html__( 'There was a problem sending the activation emails. Please try again.', 'sportszone' ),
					);
					break;
				...
				case 'do_activate':
					$notice = array(
						'class'   => 'error',
						'message' => esc_html__( 'There was a problem activating accounts. Please try again.', 'sportszone' ),
					);
					break;
				...
				case 'do_delete':
					$notice = array(
						'class'   => 'error',
						'message' => esc_html__( 'There was a problem deleting sign-ups. Please try again.', 'sportszone' ),
					);
					break;
			}
		}
	...
	public function signups_admin_index() {
		global $plugin_page, $sz_members_signup_list_table;

		$usersearch = ! empty( $_REQUEST['s'] ) ? stripslashes( $_REQUEST['s'] ) : '';

		// Prepare the group items for display.
		$sz_members_signup_list_table->prepare_items();

		if ( is_network_admin() ) {
			$form_url = network_admin_url( 'users.php' );
		} else {
			$form_url = sz_get_admin_url( 'users.php' );
		}
		...
		<div class="wrap">
			<?php if ( version_compare( $GLOBALS['wp_version'], '4.8', '>=' ) ) : ?>
				...
				<h1 class="wp-heading-inline"><?php _e( 'Users', 'sportszone' ); ?></h1>
				...
				if ( $usersearch ) {
					printf( '<span class="subtitle">' . __( 'Search results for &#8220;%s&#8221;', 'sportszone' ) . '</span>', esc_html( $usersearch ) );
				}
				?>
			...
			<?php $sz_members_signup_list_table->views(); ?>

			<form id="sz-signups-search-form" action="<?php echo esc_url( $search_form_url ) ;?>">
				<input type="hidden" name="page" value="<?php echo esc_attr( $plugin_page ); ?>" />
				<?php $sz_members_signup_list_table->search_box( __( 'Search Pending Users', 'sportszone' ), 'sz-signups' ); ?>
			</form>

			<form id="sz-signups-form" action="<?php echo esc_url( $form_url );?>" method="post">
				<?php $sz_members_signup_list_table->display(); ?>
			</form>
		</div>
	<?php
	}
	...
	public function signups_admin_manage( $action = '' ) {
		...
		$signups_query = SZ_Signup::get( array(
			'include' => $ids,
		) );
		...
		switch ( $action ) {
			case 'delete' :
				$header_text = __( 'Delete Pending Accounts', 'sportszone' );
				if ( 1 == count( $signup_ids ) ) {
					$helper_text = __( 'You are about to delete the following account:', 'sportszone' );
				} else {
					$helper_text = __( 'You are about to delete the following accounts:', 'sportszone' );
				}
				break;

			case 'activate' :
				$header_text = __( 'Activate Pending Accounts', 'sportszone' );
				if ( 1 == count( $signup_ids ) ) {
					$helper_text = __( 'You are about to activate the following account:', 'sportszone' );
				} else {
					$helper_text = __( 'You are about to activate the following accounts:', 'sportszone' );
				}
				break;

			case 'resend' :
				$header_text = __( 'Resend Activation Emails', 'sportszone' );
				if ( 1 == count( $signup_ids ) ) {
					$helper_text = __( 'You are about to resend an activation email to the following account:', 'sportszone' );
				} else {
					$helper_text = __( 'You are about to resend an activation email to the following accounts:', 'sportszone' );
				}
				break;
		}

		// These arguments are added to all URLs.
		$url_args = array( 'page' => 'sz-signups' );
		...
		$fdata = array();
		if ( 'activate' === $action && sz_is_active( 'xprofile' ) ) {
			$field_groups = sz_xprofile_get_groups( array(
				'exclude_fields'    => 1,
		...
		<div class="wrap">
				...
				<li>
					...
					<?php if ( 'activate' == $action ) : ?>
						<table class="wp-list-table widefat fixed striped">
							<tbody>
								<tr>
									<td class="column-fields"><?php esc_html_e( 'Display Name', 'sportszone' ); ?></td>
									<td><?php echo esc_html( $signup->user_name ); ?></td>
								</tr>

								<tr>
									<td class="column-fields"><?php esc_html_e( 'Email', 'sportszone' ); ?></td>
									<td><?php echo sanitize_email( $signup->user_email ); ?></td>
								</tr>

								<?php if ( sz_is_active( 'xprofile' ) && ! empty( $profile_field_ids ) ) : ?>
					...
					<?php if ( 'resend' == $action ) : ?>
						...
						<p class="description">
							<?php printf( esc_html__( 'Last notified: %s', 'sportszone'), $last_notified ) ;?>

							<?php if ( ! empty( $signup->recently_sent ) ) : ?>

								<span class="attention wp-ui-text-notification"> <?php esc_html_e( '(less than 24 hours ago)', 'sportszone' ); ?></span>
			...
			<?php if ( 'delete' === $action ) : ?>
				...
				<p><strong><?php esc_html_e( 'This action cannot be undone.', 'sportszone' ) ?></strong></p>

			<?php endif ; ?>

			<a class="button-primary" href="<?php echo esc_url( $action_url ); ?>"><?php esc_html_e( 'Confirm', 'sportszone' ); ?></a>
			<a class="button" href="<?php echo esc_url( $cancel_url ); ?>"><?php esc_html_e( 'Cancel', 'sportszone' ) ?></a>
		</div>
	...
	public function users_table_output_type_change_select( $which = 'top' ) {
		...
		if ( ! sz_current_user_can( 'promote_users' ) ) {
			return;
		}
		...
		<label class="screen-reader-text" for="<?php echo $id_name; ?>"><?php _e( 'Change member type to&hellip;', 'sportszone' ) ?></label>
		<select name="<?php echo $id_name; ?>" id="<?php echo $id_name; ?>" style="display:inline-block;float:none;">
			<option value=""><?php _e( 'Change member type to&hellip;', 'sportszone' ) ?></option>
			...
			<option value="remove_member_type"><?php _e( 'No Member Type', 'sportszone' ) ?></option>

		</select>
		<?php
		wp_nonce_field( 'sz-bulk-users-change-type-' . sz_loggedin_user_id(), 'sz-bulk-users-change-type-nonce' );
		submit_button( __( 'Change', 'sportszone' ), 'button', 'sz_change_member_type', false );
	}
	...
	public function users_table_process_bulk_type_change() {
		...
		// Bail if no users are specified or if this isn't a SportsZone action.
		if ( empty( $_REQUEST['users'] )
			|| ( empty( $_REQUEST['sz_change_type'] ) && empty( $_REQUEST['sz_change_type2'] ) )
			|| empty( $_REQUEST['sz_change_member_type'] )
		) {
			return;
		}

		// Bail if nonce check fails.
		check_admin_referer( 'sz-bulk-users-change-type-' . sz_loggedin_user_id(), 'sz-bulk-users-change-type-nonce' );
		...
		if ( 'remove_member_type' != $new_type && null === sz_get_member_type_object( $new_type ) ) {
			$error = true;
		} else {
			...
			foreach ( (array) $_REQUEST['users'] as $user_id ) {
				...
				$member_type = sz_get_member_type( $user_id );

				if ( 'remove_member_type' === $new_type ) {
					// Remove the current member type, if there's one to remove.
					if ( $member_type ) {
						$removed = sz_remove_member_type( $user_id, $member_type );
					...
					if ( $new_type !== $member_type ) {
						$set = sz_set_member_type( $user_id, $new_type );
	...
	public function users_type_change_notice() {
		...
		if ( $updated && in_array( $updated, array( 'member-type-change-error', 'member-type-change-success' ), true ) ) {
			...
			if ( 'member-type-change-error' === $updated ) {
				$notice = __( 'There was an error while changing member type. Please try again.', 'sportszone' );
				$type   = 'error';
			} else {
				$notice = __( 'Member type was changed successfully.', 'sportszone' );
				$type   = 'updated';
			}
	...
	public function users_table_add_type_column( $columns = array() ) {
		$columns[ sz_get_member_type_tax_name() ] = _x( 'Member Type', 'Label for the WP users table member type column', 'sportszone' );
	...
	public function users_table_populate_type_cell( $retval = '', $column_name = '', $user_id = 0 ) {
		...
		if ( sz_get_member_type_tax_name() !== $column_name ) {
			return $retval;
		}

		// Get the member type.
		$type = sz_get_member_type( $user_id );

		// Output the
		if ( $type_obj = sz_get_member_type_object( $type ) ) {
			$url = add_query_arg( array( 'sz-member-type' => urlencode( $type ) ) );
	...
	public function users_table_filter_by_type( $query ) {
		...
		if ( is_admin() && 'users.php' === $pagenow && ! empty( $_REQUEST['sz-member-type'] ) ) {
			$type_slug = sanitize_text_field( $_REQUEST['sz-member-type'] );

			// Check that the type is registered.
			if ( null == sz_get_member_type_object( $type_slug ) ) {
				return;
			}

			// Get the list of users that are assigned to this member type.
			$type = sz_get_term_by( 'slug', $type_slug, sz_get_member_type_tax_name() );

			if ( empty( $type->term_id ) ) {
				return;
			}

			$user_ids = sz_get_objects_in_term( $type->term_id, sz_get_member_type_tax_name() );
