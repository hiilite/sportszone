<?php
/**
 * SportsZone Member Theme Compat.
 *
 * @package SportsZone
...
/**
 * The main theme compat class for SportsZone Members.
 *
...
class SZ_Members_Theme_Compat {
	...
	public function __construct() {
		add_action( 'sz_setup_theme_compat', array( $this, 'is_members' ) );
	}
	...
	public function is_members() {
		...
		if ( sz_is_user() ) {
			...
			if ( sz_is_active( 'activity' ) && sz_is_single_activity() ) {
				return;
			}
			 ...
			 * @since 1.5.0
			 */
			do_action( 'sz_members_screen_display_profile' );
		...
		// Members Directory.
		} elseif ( ! sz_current_action() && ! sz_current_item() ) {
	 ...
	 * @since 1.8.0
	 *
	 * @param array $templates The templates from sz_get_theme_compat_templates().
	...
	public function directory_template_hierarchy( $templates = array() ) {
		...
		if ( '' !== sz_get_current_member_type() ) {
			$new_templates[] = 'members/index-directory-type-' . sanitize_file_name( sz_get_current_member_type() ) . '.php';
		}
		 ...
		 */
		$new_templates = apply_filters( 'sz_template_hierarchy_members_directory', $new_templates );
	...
	public function directory_dummy_post() {
		sz_theme_compat_reset_post( array(
			'ID'             => 0,
			'post_title'     => sz_get_directory_title( 'members' ),
			'post_author'    => 0,
			'post_date'      => 0,
			'post_content'   => '',
			'post_type'      => 'page',
	...
	public function directory_content() {
		return sz_buffer_template_part( 'members/index', null, false );
	}
	 ...
	 *                          sz_get_theme_compat_templates().
	 * @return array $templates Array of custom templates to look for.
	 */
	public function single_template_hierarchy( $templates ) {
		// Setup some variables we're going to reference in our custom templates.
		$user_nicename = sportszone()->displayed_user->userdata->user_nicename;
		 ...
		 * @param array $value Array of template paths to add to hierarchy.
		 */
		$new_templates = apply_filters( 'sz_template_hierarchy_members_single_item', array(
			'members/single/index-id-'        . sanitize_file_name( sz_displayed_user_id() ) . '.php',
			'members/single/index-nicename-'  . sanitize_file_name( $user_nicename )         . '.php',
			'members/single/index-action-'    . sanitize_file_name( sz_current_action() )    . '.php',
			'members/single/index-component-' . sanitize_file_name( sz_current_component() ) . '.php',
	...
	public function single_dummy_post() {
		sz_theme_compat_reset_post( array(
			'ID'             => 0,
			'post_title'     => sz_get_displayed_user_fullname(),
			'post_author'    => 0,
			'post_date'      => 0,
			'post_content'   => '',
			'post_type'      => 'page',
	...
	public function single_dummy_content() {
		return sz_buffer_template_part( 'members/single/home', null, false );
	}
}
