<?php
/**
 * SportsZone Members List Table class.
 *
 * @package SportsZone
 ...
 * @since 2.0.0
 */
class SZ_Members_List_Table extends WP_Users_List_Table {
	...
	public function get_columns() {
		 ...
		 */
		return apply_filters( 'sz_members_signup_columns', array(
			'cb'         => '<input type="checkbox" />',
			'username'   => __( 'Username',    'sportszone' ),
			'name'       => __( 'Name',        'sportszone' ),
			'email'      => __( 'Email',       'sportszone' ),
			'registered' => __( 'Registered',  'sportszone' ),
			'date_sent'  => __( 'Last Sent',   'sportszone' ),
			'count_sent' => __( 'Emails Sent', 'sportszone' )
		) );
	}
	...
	public function get_bulk_actions() {
		$actions = array(
			'activate' => _x( 'Activate', 'Pending signup action', 'sportszone' ),
			'resend'   => _x( 'Email',    'Pending signup action', 'sportszone' ),
		);

		if ( current_user_can( 'delete_users' ) ) {
			$actions['delete'] = __( 'Delete', 'sportszone' );
		}
	...
	public function no_items() {
		...
		if ( sz_get_signup_allowed() ) {
			esc_html_e( 'No pending accounts found.', 'sportszone' );
		} else {
			$link = false;

			// Specific case when SportsZone is not network activated.
			if ( is_multisite() && current_user_can( 'manage_network_users') ) {
				$link = sprintf( '<a href="%1$s">%2$s</a>', esc_url( network_admin_url( 'settings.php'       ) ), esc_html__( 'Edit settings', 'sportszone' ) );
			} elseif ( current_user_can( 'manage_options' ) ) {
				$link = sprintf( '<a href="%1$s">%2$s</a>', esc_url( sz_get_admin_url( 'options-general.php' ) ), esc_html__( 'Edit settings', 'sportszone' ) );
			}

			printf( __( 'Registration is disabled. %s', 'sportszone' ), $link );
		}
	...
	?>
		<label class="screen-reader-text" for="signup_<?php echo intval( $signup_object->id ); ?>"><?php
			...
			printf( esc_html__( 'Select user: %s', 'sportszone' ), $signup_object->user_login );
		?></label>
	...
	public function column_username( $signup_object = null ) {
		...
		$actions['activate'] = sprintf( '<a href="%1$s">%2$s</a>', esc_url( $activate_link ), __( 'Activate', 'sportszone' ) );
		$actions['resend']   = sprintf( '<a href="%1$s">%2$s</a>', esc_url( $email_link ), __( 'Email', 'sportszone' ) );

		if ( current_user_can( 'delete_users' ) ) {
			$actions['delete'] = sprintf( '<a href="%1$s" class="delete">%2$s</a>', esc_url( $delete_link ), __( 'Delete', 'sportszone' ) );
		}
		 ...
		 */
		$actions = apply_filters( 'sz_members_ms_signup_row_actions', $actions, $signup_object );
	...
	function column_default( $signup_object = null, $column_name = '' ) {
		 ...
		 * @param object $signup_object The signup data object.
		 */
		return apply_filters( 'sz_members_signup_custom_column', '', $column_name, $signup_object );
	}
}
