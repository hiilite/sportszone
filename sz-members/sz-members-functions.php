 ...
 */
function sz_members_has_directory() {
	$sz = sportszone();

	return (bool) !empty( $sz->pages->members->id );
}
 ...
 * SZ_MEMBERS_SLUG, SZ_REGISTER_SLUG, and SZ_ACTIVATION_SLUG. If these
 * constants are not overridden in wp-config.php or sz-custom.php, they are
 ...
 * @since 1.5.0
 */
function sz_core_define_slugs() {
	$sz = sportszone();

	// No custom members slug.
	if ( !defined( 'SZ_MEMBERS_SLUG' ) ) {
		if ( !empty( $sz->pages->members ) ) {
			define( 'SZ_MEMBERS_SLUG', $sz->pages->members->slug );
		} else {
			define( 'SZ_MEMBERS_SLUG', 'members' );
		}
	}

	// No custom registration slug.
	if ( !defined( 'SZ_REGISTER_SLUG' ) ) {
		if ( !empty( $sz->pages->register ) ) {
			define( 'SZ_REGISTER_SLUG', $sz->pages->register->slug );
		} else {
			define( 'SZ_REGISTER_SLUG', 'register' );
		}
	}

	// No custom activation slug.
	if ( !defined( 'SZ_ACTIVATION_SLUG' ) ) {
		if ( !empty( $sz->pages->activate ) ) {
			define( 'SZ_ACTIVATION_SLUG', $sz->pages->activate->slug );
		} else {
			define( 'SZ_ACTIVATION_SLUG', 'activate' );
		}
	}
}
add_action( 'sz_setup_globals', 'sz_core_define_slugs', 11 );

/**
 * Fetch an array of users based on the parameters passed.
 *
 * Since SportsZone 1.7, sz_core_get_users() uses SZ_User_Query. If you
 * need backward compatibility with SZ_Core_User::get_users(), filter the
 * sz_use_legacy_user_query value, returning true.
 *
 * @since 1.2.0
 *
 * @param array|string $args {
 *     Array of arguments. All are optional. See {@link SZ_User_Query} for
 ...
 * @return array
 */
function sz_core_get_users( $args = '' ) {
	...
	// For legacy users. Use of SZ_Core_User::get_users() is deprecated.
	if ( apply_filters( 'sz_use_legacy_user_query', false, __FUNCTION__, $r ) ) {
		$retval = SZ_Core_User::get_users(
			$r['type'],
			$r['per_page'],
			$r['page'],
			$r['user_id'],
	...
	// Default behavior as of SportsZone 1.7.0.
	} else {

		// Get users like we were asked to do...
		$users = new SZ_User_Query( $r );

		// ...but reformat the results to match sz_core_get_users() behavior.
		$retval = array(
	 ...
	 */
	return apply_filters( 'sz_core_get_users', $retval, $r );
}
 ...
 * @return string
 */
function sz_core_get_user_domain( $user_id = 0, $user_nicename = false, $user_login = false ) {

	if ( empty( $user_id ) ) {
		return;
	}

	$username = sz_core_get_username( $user_id, $user_nicename, $user_login );

	if ( sz_is_username_compatibility_mode() ) {
		$username = rawurlencode( $username );
	}

	$after_domain = sz_core_enable_root_profiles() ? $username : sz_get_members_root_slug() . '/' . $username;
	$domain       = trailingslashit( sz_get_root_domain() . '/' . $after_domain );

	// Don't use this filter.  Subject to removal in a future release.
	// Use the 'sz_core_get_user_domain' filter instead.
	$domain       = apply_filters( 'sz_core_get_user_domain_pre_cache', $domain, $user_id, $user_nicename, $user_login );
	 ...
	 */
	return apply_filters( 'sz_core_get_user_domain', $domain, $user_id, $user_nicename, $user_login );
}
 ...
 */
function sz_core_get_core_userdata( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
		return false;
	}

	// Get core user data
	$userdata = SZ_Core_User::get_core_userdata( $user_id );
	 ...
	 */
	return apply_filters( 'sz_core_get_core_userdata', $userdata );
}
 ...
 * @return int
 */
function sz_core_get_displayed_userid( $user_login ) {
	return apply_filters( 'sz_core_get_displayed_userid', sz_core_get_userid( $user_login ) );
}
 ...
 */
function sz_core_get_userid( $username = '' ) {
	if ( empty( $username ) ) {
		return false;
	}
	 ...
	 */
	return apply_filters( 'sz_core_get_userid', ! empty( $user->ID ) ? $user->ID : NULL, $username );
}
 ...
 * @return int|null The ID of the matched user on success, null on failure.
 */
function sz_core_get_userid_from_nicename( $user_nicename = '' ) {
	 ...
	 */
	return apply_filters( 'sz_core_get_userid_from_nicename', ! empty( $user->ID ) ? $user->ID : NULL, $user_nicename );
}

/**
 * Return the username for a user based on their user id.
 *
 * This function is sensitive to the SZ_ENABLE_USERNAME_COMPATIBILITY_MODE,
 ...
 */
function sz_core_get_username( $user_id = 0, $user_nicename = false, $user_login = false ) {
	...
	if ( ! $user_nicename && ! $user_login ) {
		...
		$username = sz_is_username_compatibility_mode() ? $user_login : $user_nicename;
	}
	 ...
	 */
	return apply_filters( 'sz_core_get_username', $username );
}
 ...
 * @return string The username of the matched user or an empty string if no user is found.
 */
function sz_members_get_user_nicename( $user_id ) {
	/**
	 ...
	 */
	return apply_filters( 'sz_members_get_user_nicename', get_the_author_meta( 'nicename', $user_id ) );
}
 ...
 *                matches the $user_id.
 */
function sz_core_get_user_email( $user_id ) {
	/**
	 ...
	 */
	return apply_filters( 'sz_core_get_user_email', get_the_author_meta( 'email', $user_id ) );
}
 ...
 *                     no match.
 */
function sz_core_get_userlink( $user_id, $no_anchor = false, $just_link = false ) {
	$display_name = sz_core_get_user_displayname( $user_id );
	...
	if ( !$url = sz_core_get_user_domain( $user_id ) ) {
		return false;
	}
	 ...
	 */
	return apply_filters( 'sz_core_get_userlink', '<a href="' . $url . '">' . $display_name . '</a>', $user_id );
}
 ...
 */
function sz_core_get_user_displaynames( $user_ids ) {
	...
	foreach ( $user_ids as $user_id ) {
		$retval[ $user_id ] = sz_core_get_user_displayname( $user_id );
	}
 ...
 *                     user not found.
 */
function sz_core_get_user_displayname( $user_id_or_username ) {
	if ( empty( $user_id_or_username ) ) {
		return false;
	}

	if ( ! is_numeric( $user_id_or_username ) ) {
		$user_id = sz_core_get_userid( $user_id_or_username );
	} else {
	 ...
	 * @param int    $user_id  ID of the user to check.
	 */
	return apply_filters( 'sz_core_get_user_displayname', get_the_author_meta( 'display_name', $user_id ), $user_id );
}
add_filter( 'sz_core_get_user_displayname', 'strip_tags', 1 );
add_filter( 'sz_core_get_user_displayname', 'trim'          );
add_filter( 'sz_core_get_user_displayname', 'stripslashes'  );
add_filter( 'sz_core_get_user_displayname', 'esc_html'      );
 ...
 */
function sz_core_get_userlink_by_email( $email ) {
	 ...
	 */
	return apply_filters( 'sz_core_get_userlink_by_email', sz_core_get_userlink( $user->ID, false, false, true ) );
}
 ...
 * @since 1.0.0
 *
 * @param string $username If SZ_ENABLE_USERNAME_COMPATIBILITY_MODE is set,
 *                         this should be user_login, otherwise it should
 *                         be user_nicename.
 * @return string|bool The link to the user's domain, false on no match.
 */
function sz_core_get_userlink_by_username( $username ) {
	if ( sz_is_username_compatibility_mode() ) {
		$user_id = sz_core_get_userid( $username );
	} else {
		$user_id = sz_core_get_userid_from_nicename( $username );
	}
	 ...
	 * @param string|bool $value URL for the user if found, otherwise false.
	 */
	return apply_filters( 'sz_core_get_userlink_by_username', sz_core_get_userlink( $user_id, false, false, true ) );
}
 ...
 * {@link sz_core_get_active_member_count()}.
 *
 * @since 1.2.0
 *
 * @return int The total number of members.
 */
function sz_core_get_total_member_count() {
	global $wpdb;

	$count = wp_cache_get( 'sz_total_member_count', 'sz' );

	if ( false === $count ) {
		$status_sql = sz_core_get_status_sql();
		$count = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->users} WHERE {$status_sql}" );
		wp_cache_set( 'sz_total_member_count', $count, 'sz' );
	}
	 ...
	 */
	return apply_filters( 'sz_core_get_total_member_count', $count );
}
 ...
 */
function sz_core_get_active_member_count() {
	global $wpdb;

	$count = get_transient( 'sz_active_member_count' );
	if ( false === $count ) {
		$sz = sportszone();
		...
		$count             = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(user_id) FROM {$sz->members->table_name_last_activity} WHERE component = %s AND type = 'last_activity' {$exclude_users_sql}", $sz->members->id ) );

		set_transient( 'sz_active_member_count', $count );
	}
	 ...
	 */
	return apply_filters( 'sz_core_get_active_member_count', $count );
}
 ...
 *
 * - in sz_settings_action_capabilities() (from the front-end)
 * - by sz_core_mark_user_spam_admin()    (from wp-admin)
 * - sz_core_mark_user_ham_admin()        (from wp-admin)
 *
 * @since 1.6.0
 *
 ...
 */
function sz_core_process_spammer_status( $user_id, $status, $do_wp_cleanup = true ) {
	global $wpdb;
	...
	remove_action( 'make_spam_user', 'sz_core_mark_user_spam_admin' );
	remove_action( 'make_ham_user',  'sz_core_mark_user_ham_admin'  );
	...
	if ( $do_wp_cleanup ) {
		...
		foreach ( (array) array_values( $blogs ) as $details ) {
			...
			if ( 1 == $details->userblog_id || sz_get_root_blog_id() == $details->userblog_id ) {
				continue;
			}
	...
	if ( ( true === $is_spam ) && sz_is_active( 'activity' ) ) {
		sz_activity_hide_user_activity( $user_id );
	}
	...
	if ( true === $is_spam ) {
		 ...
		 */
		do_action( 'sz_make_spam_user', $user_id );
	} else {
		 ...
		 * @param int $value Displayed user ID.
		 */
		do_action( 'sz_make_ham_user', $user_id );
	}
	 ...
	 */
	do_action( 'sz_core_process_spammer_status', $user_id, $is_spam );

	// Put things back how we found them.
	add_action( 'make_spam_user', 'sz_core_mark_user_spam_admin' );
	add_action( 'make_ham_user',  'sz_core_mark_user_ham_admin'  );
 ...
 */
function sz_core_mark_user_spam_admin( $user_id ) {
	sz_core_process_spammer_status( $user_id, 'spam', false );
}
add_action( 'make_spam_user', 'sz_core_mark_user_spam_admin' );
 ...
 */
function sz_core_mark_user_ham_admin( $user_id ) {
	sz_core_process_spammer_status( $user_id, 'ham', false );
}
add_action( 'make_ham_user', 'sz_core_mark_user_ham_admin' );
 ...
 */
function sz_is_user_spammer( $user_id = 0 ) {
	...
	switch ( $user_id ) {
		case sz_loggedin_user_id() :
			$user = ! empty( $sz->loggedin_user->userdata ) ? $sz->loggedin_user->userdata : false;
			break;

		case sz_displayed_user_id() :
			$user = ! empty( $sz->displayed_user->userdata ) ? $sz->displayed_user->userdata : false;
			break;

		case sz_get_member_user_id() :
	 ...
	 */
	return apply_filters( 'sz_is_user_spammer', (bool) $is_spammer );
}
 ...
 */
function sz_is_user_deleted( $user_id = 0 ) {
	 ...
	 */
	return apply_filters( 'sz_is_user_deleted', (bool) $is_deleted );
}
 ...
 */
function sz_is_user_active( $user_id = 0 ) {
 ...
 */
function sz_is_user_inactive( $user_id = 0 ) {
	...
	return !sz_is_user_active( $user_id );
}
 ...
 * @return bool True on success, false on failure.
 */
function sz_update_user_last_activity( $user_id = 0, $time = '' ) {
	...
	// As of SportsZone 2.0, last_activity is no longer stored in usermeta.
	// However, we mirror it there for backward compatibility. Do not use!
	// Remove our warning and re-add.
	remove_filter( 'update_user_metadata', '_sz_update_user_meta_last_activity_warning', 10 );
	remove_filter( 'get_user_metadata', '_sz_get_user_meta_last_activity_warning', 10 );
	sz_update_user_meta( $user_id, 'last_activity', $time );
	add_filter( 'update_user_metadata', '_sz_update_user_meta_last_activity_warning', 10, 4 );
	add_filter( 'get_user_metadata', '_sz_get_user_meta_last_activity_warning', 10, 4 );

	return SZ_Core_User::update_last_activity( $user_id, $time );
}

/**
 * Backward compatibility for 'last_activity' usermeta fetching.
 *
 * In SportsZone 2.0, user last_activity data was moved out of usermeta. For
 ...
 * @return string|null
 */
function _sz_get_user_meta_last_activity_warning( $retval, $object_id, $meta_key, $single ) {
	static $warned = false;

	if ( 'last_activity' === $meta_key ) {
		// Don't send the warning more than once per pageload.
		if ( false === $warned ) {
			_doing_it_wrong( 'get_user_meta( $user_id, \'last_activity\' )', __( 'User last_activity data is no longer stored in usermeta. Use sz_get_user_last_activity() instead.', 'sportszone' ), '2.0.0' );
			$warned = true;
		}

		$user_last_activity = sz_get_user_last_activity( $object_id );
		if ( $single ) {
 ...
 */
function _sz_update_user_meta_last_activity_warning( $meta_id, $object_id, $meta_key, $meta_value ) {
	if ( 'last_activity' === $meta_key ) {
		_doing_it_wrong( 'update_user_meta( $user_id, \'last_activity\' )', __( 'User last_activity data is no longer stored in usermeta. Use sz_update_user_last_activity() instead.', 'sportszone' ), '2.0.0' );
		sz_update_user_last_activity( $object_id, $meta_value );
	}
}
add_filter( 'update_user_metadata', '_sz_update_user_meta_last_activity_warning', 10, 4 );
 ...
 */
function sz_get_user_last_activity( $user_id = 0 ) {
	$activity = '';

	$last_activity = SZ_Core_User::get_last_activity( $user_id );
	 ...
	 */
	return apply_filters( 'sz_get_user_last_activity', $activity, $user_id );
}
 ...
 * be called directly from the SportsZone Tools panel.
 *
 * @since 2.0.0
 *
 * @return bool
 */
function sz_last_activity_migrate() {
	global $wpdb;
	...
	// table.
	$wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->members->table_name_last_activity} WHERE component = %s AND type = 'last_activity'", $sz->members->id ) );

	$sql = "INSERT INTO {$sz->members->table_name_last_activity} (`user_id`, `component`, `type`, `action`, `content`, `primary_link`, `item_id`, `date_recorded` ) (
		  SELECT user_id, '{$sz->members->id}' as component, 'last_activity' as type, '' as action, '' as content, '' as primary_link, 0 as item_id, meta_value AS date_recorded
		  FROM {$wpdb->usermeta}
		  WHERE
 ...
 */
function sz_core_get_all_posts_for_user( $user_id = 0 ) {
	global $wpdb;
	...
	return apply_filters( 'sz_core_get_all_posts_for_user', $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_author = %d AND post_status = 'publish' AND post_type = 'post'", $user_id ) ) );
}
 ...
 * @return bool True on success, false on failure.
 */
function sz_core_delete_account( $user_id = 0 ) {
	...
	if ( sz_loggedin_user_id() !== absint( $user_id ) ) {

		// Bail if current user cannot delete any users.
		if ( ! sz_current_user_can( 'delete_users' ) ) {
			return false;
		}

		// Bail if current user cannot delete this user.
		if ( ! current_user_can_for_blog( sz_get_root_blog_id(), 'delete_user', $user_id ) ) {
			return false;
		}
	}
	 ...
	 */
	do_action( 'sz_core_pre_delete_account', $user_id );
	 ...
	 */
	do_action( 'sz_core_deleted_account', $user_id );
 ...
 * @return bool True on success, false on failure.
 */
function sz_core_delete_avatar_on_user_delete( $user_id ) {
	return sz_core_delete_existing_avatar( array(
		'item_id' => $user_id,
		'object'  => 'user',
	) );
}
 ...
 */
function sz_core_boot_spammer( $user ) {
	...
	if ( is_a( $user, 'WP_User' ) && ( ( is_multisite() && (int) $user->spam ) || 1 == $user->user_status ) ) {
		return new WP_Error( 'invalid_username', __( '<strong>ERROR</strong>: Your account has been marked as a spammer.', 'sportszone' ) );
	}

	// User is good to go!
	return $user;
}
add_filter( 'authenticate', 'sz_core_boot_spammer', 30 );
 ...
 */
function sz_core_remove_data( $user_id ) {

	// Remove last_activity data.
	SZ_Core_User::delete_last_activity( $user_id );

	// Flush the cache to remove the user from all cached objects.
	wp_cache_flush();
}
add_action( 'wpmu_delete_user',  'sz_core_remove_data' );
add_action( 'delete_user',       'sz_core_remove_data' );
add_action( 'sz_make_spam_user', 'sz_core_remove_data' );
 ...
 */
function sz_core_can_edit_settings() {
	$status = false;

	if ( sz_is_my_profile() ) {
		$status = true;
	} elseif ( is_super_admin( sz_displayed_user_id() ) && ! is_super_admin() ) {
		$status = false;
	} elseif ( sz_current_user_can( 'sz_moderate' ) || current_user_can( 'edit_users' ) ) {
		$status = true;
	}
	 ...
	 */
	return apply_filters( 'sz_core_can_edit_settings', $status );
}
 ...
 * @since 1.2.5
 */
function sz_core_flush_illegal_names() {
	$illegal_names = get_site_option( 'illegal_names' );
	update_site_option( 'illegal_names', $illegal_names );
}

/**
 * Add SportsZone-specific items to the illegal_names array.
 *
 * @since 1.2.7
 *
 ...
 */
function sz_core_get_illegal_names( $value = '', $oldvalue = '' ) {
	...
	$slug_constants = array(
		...
		'SZ_SEARCH_SLUG',
		'SZ_SETTINGS_SLUG',
		'SZ_NOTIFICATIONS_SLUG',
		'SZ_REGISTER_SLUG',
		'SZ_ACTIVATION_SLUG',
	);
	foreach( $slug_constants as $constant ) {
		if ( defined( $constant ) ) {
			$sz_component_slugs[] = constant( $constant );
		}
	}
	 ...
	 */
	$filtered_illegal_names = apply_filters( 'sz_core_illegal_usernames', array_merge( array( 'www', 'web', 'root', 'admin', 'main', 'invite', 'administrator' ), $sz_component_slugs ) );
	...
	// First merge SportsZone illegal names.
	$sz_merged_names           = array_merge( (array) $filtered_illegal_names, (array) $db_illegal_names );

	// Then merge WordPress and SportsZone illegal names.
	$merged_names              = array_merge( (array) $wp_filtered_illegal_names, (array) $sz_merged_names );
	 ...
	 */
	return apply_filters( 'sz_core_illegal_names', $illegal_names );
}
add_filter( 'pre_update_site_option_illegal_names', 'sz_core_get_illegal_names', 10, 2 );
 ...
 *                    of error codes.
 */
function sz_core_validate_email_address( $user_email ) {
	$errors = array();
 ...
 *
 * Functions like sz_core_validate_email_address() return a structured array
 * of error codes. sz_core_add_validation_error_messages() takes this array and
 * parses, adding the appropriate error messages to the WP_Error object.
 *
 * @since 1.7.0
 *
 * @see sz_core_validate_email_address()
 *
 * @param WP_Error $errors             WP_Error object.
 * @param array    $validation_results The return value of a validation function
 *                                     like sz_core_validate_email_address().
 */
function sz_core_add_validation_error_messages( WP_Error $errors, $validation_results ) {
	if ( ! empty( $validation_results['invalid'] ) ) {
		$errors->add( 'user_email', __( 'Please check your email address.', 'sportszone' ) );
	}

	if ( ! empty( $validation_results['domain_banned'] ) ) {
		$errors->add( 'user_email',  __( 'Sorry, that email address is not allowed!', 'sportszone' ) );
	}
	...
	if ( ! empty( $validation_results['in_use'] ) ) {
		$errors->add( 'user_email', __( 'Sorry, that email address is already used!', 'sportszone' ) );
	}
}
 ...
 */
function sz_core_validate_user_signup( $user_name, $user_email ) {

	// Make sure illegal names include SportsZone slugs and values.
	sz_core_flush_illegal_names();
	...
	// minus the multisite-specific restrictions on user_login.
		...
		if ( empty( $user_name ) ) {
			$errors->add( 'user_name', __( 'Please enter a username', 'sportszone' ) );
		}

		// User name can't be on the blacklist.
		$illegal_names = get_site_option( 'illegal_names' );
		if ( in_array( $user_name, (array) $illegal_names ) ) {
			$errors->add( 'user_name', __( 'That username is not allowed', 'sportszone' ) );
		}

		// User name must pass WP's validity check.
		if ( ! validate_username( $user_name ) ) {
			$errors->add( 'user_name', __( 'Usernames can contain only letters, numbers, ., -, and @', 'sportszone' ) );
		}

		// Minimum of 4 characters.
		if ( strlen( $user_name ) < 4 ) {
			$errors->add( 'user_name',  __( 'Username must be at least 4 characters', 'sportszone' ) );
		}

		// No underscores. @todo Why not?
		if ( false !== strpos( ' ' . $user_name, '_' ) ) {
			$errors->add( 'user_name', __( 'Sorry, usernames may not contain the character "_"!', 'sportszone' ) );
		}
		...
		if ( $match[0] == $user_name ) {
			$errors->add( 'user_name', __( 'Sorry, usernames must have letters too!', 'sportszone' ) );
		}

		// Check into signups.
		$signups = SZ_Signup::get( array(
		...
		if ( username_exists( $user_name ) || ! empty( $signup ) ) {
			$errors->add( 'user_name', __( 'Sorry, that username already exists!', 'sportszone' ) );
		}

		// Validate the email address and process the validation results into
		// error messages.
		$validate_email = sz_core_validate_email_address( $user_email );
		sz_core_add_validation_error_messages( $errors, $validate_email );
	 ...
	 */
	return apply_filters( 'sz_core_validate_user_signup', $result );
}
 ...
 * @return array
 */
function sz_core_validate_blog_signup( $blog_url, $blog_title ) {
	 ...
	 */
	return apply_filters( 'sz_core_validate_blog_signup', wpmu_validate_blog_signup( $blog_url, $blog_title ) );
}
 ...
 */
function sz_core_signup_user( $user_login, $user_password, $user_email, $usermeta ) {
	$sz = sportszone();
	...
	if ( is_multisite() ) {
		 ...
		 * immediately at registration time. SportsZone uses a system
		 ...
		 * clutter by defining setting the SZ_SIGNUPS_SKIP_USER_CREATION
		 * to true in your wp-config.php file.
		 */
		if ( ! defined( 'SZ_SIGNUPS_SKIP_USER_CREATION' ) || ! SZ_SIGNUPS_SKIP_USER_CREATION ) {
			$user_id = SZ_Signup::add_backcompat( $user_login, $user_password, $user_email, $usermeta );

			if ( is_wp_error( $user_id ) ) {
				return $user_id;
			}

			sz_update_user_meta( $user_id, 'activation_key', $activation_key );
		}
		...
		/**
		 * Filters if SportsZone should send an activation key for a new signup.
		 *
		 * @since 1.2.3
		 *
		 ...
		 *                               signup data, xprofile data, etc).
		 */
		if ( apply_filters( 'sz_core_signup_send_activation_key', true, $user_id, $user_email, $activation_key, $usermeta ) ) {
			sz_core_signup_send_validation_email( $user_id, $user_email, $activation_key, $user_login );
		}
	}

	$sz->signup->username = $user_login;
	 ...
	 *                                       signup data, xprofile data, etc).
	 */
	do_action( 'sz_core_signup_user', $user_id, $user_login, $user_password, $user_email, $usermeta );
 ...
 * @return bool
 */
function sz_core_signup_blog( $blog_domain, $blog_path, $blog_title, $user_name, $user_email, $usermeta ) {
	 ...
	 * @param void $value
	 */
	return apply_filters( 'sz_core_signup_blog', wpmu_signup_blog( $blog_domain, $blog_path, $blog_title, $user_name, $user_email, $usermeta ) );
}
 ...
 */
function sz_core_activate_signup( $key ) {
	global $wpdb;
	...
	if ( is_multisite() ) {
		...
		if ( empty( $signups['signups'] ) ) {
			return new WP_Error( 'invalid_key', __( 'Invalid activation key.', 'sportszone' ) );
		}
		...
		if ( $signup->active ) {
			if ( empty( $signup->domain ) ) {
				return new WP_Error( 'already_active', __( 'The user is already active.', 'sportszone' ), $signup );
			} else {
				return new WP_Error( 'already_active', __( 'The site is already active.', 'sportszone' ), $signup );
			}
		}
		...
		// Create the user. This should only be necessary if SZ_SIGNUPS_SKIP_USER_CREATION is true.
		if ( ! $user_id ) {
			$user_id = wp_create_user( $signup->user_login, $password, $signup->user_email );

		// Otherwise, update the existing user's status.
		} elseif ( $key === sz_get_user_meta( $user_id, 'activation_key', true ) || $key === wp_hash( $user_id ) ) {
			...
			sz_delete_user_meta( $user_id, 'activation_key' );
		...
		if ( ! $user_id ) {
			return new WP_Error( 'create_user', __( 'Could not create user', 'sportszone' ), $signup );
		}
		...
		// Activate the signup.
		SZ_Signup::validate( $key );

		if ( isset( $user_already_exists ) ) {
			return new WP_Error( 'user_already_exists', __( 'That username is already activated.', 'sportszone' ), $signup );
		}
		 ...
		 */
		if ( apply_filters( 'sz_core_send_user_registration_admin_notification', true ) ) {
		...
		if ( isset( $user_already_created ) ) {
			 ...
			 */
			do_action( 'sz_core_activated_user', $user_id, $key, $user );
			return $user_id;
		}
	}
 ...
 */
function sz_members_add_role_after_activation( $user_id ) {
	// Get default role to add.
	$role = sz_get_option( 'default_role' );

	// Multisite.
	if ( is_multisite() && ! is_user_member_of_blog( $user_id, sz_get_root_blog_id() ) ) {
		add_user_to_blog( sz_get_root_blog_id(), $user_id, $role );
...
add_action( 'sz_core_activated_user', 'sz_members_add_role_after_activation', 1 );
 ...
 * @since 2.0.1
 */
function sz_members_migrate_signups() {
	global $wpdb;
 ...
 * @return bool
 */
function sz_core_map_user_registration( $user_id ) {
	...
	if ( sz_is_active( 'xprofile' ) ) {
		$firstname = sz_get_user_meta( $user_id, 'first_name', true );
		$lastname = ' ' . sz_get_user_meta( $user_id, 'last_name', true );
		$name = $firstname . $lastname;

		if ( empty( $name ) || ' ' == $name ) {
			$name = sz_get_user_meta( $user_id, 'nickname', true );
		}

		xprofile_set_field_data( 1, $user_id, $name );
	}
}
add_action( 'user_register', 'sz_core_map_user_registration' );
 ...
 */
function sz_core_signup_avatar_upload_dir() {
	$sz = sportszone();

	if ( empty( $sz->signup->avatar_dir ) ) {
		return false;
	}

	$directory = 'avatars/signups';
	$path      = sz_core_avatar_upload_path() . '/' . $directory . '/' . $sz->signup->avatar_dir;
	$newbdir   = $path;
	$newurl    = sz_core_avatar_url() . '/' . $directory . '/' . $sz->signup->avatar_dir;
	$newburl   = $newurl;
	$newsubdir = '/' . $directory . '/' . $sz->signup->avatar_dir;
	 ...
	 */
	return apply_filters( 'sz_core_signup_avatar_upload_dir', array(
		'path'    => $path,
		'url'     => $newurl,
		'subdir'  => $newsubdir,
		'basedir' => $newbdir,
 ...
 *
 * @param int|bool $user_id    ID of the new user, false if SZ_SIGNUPS_SKIP_USER_CREATION is true.
 * @param string   $user_email Email address of the new user.
 * @param string   $key        Activation key.
 * @param string   $user_login Optional. The user login name.
 */
function sz_core_signup_send_validation_email( $user_id, $user_email, $key, $user_login = '' ) {
	$args = array(
		'tokens' => array(
	...
	sz_send_email( 'core-user-registration', $to, $args );
}
 ...
 */
function sz_core_signup_disable_inactive( $user = null, $username = '', $password ='' ) {
	// Login form not used.
	...
	// {@link sz_core_signup_user()} for more details.
	...
	$signup = SZ_Signup::get( array( 'user_login' => sanitize_user( $user_login ) ) );
	...
	$resend_url_params = array(
		'action' => 'sz-resend-activation',
		'id'     => $signup_id,
	);

	$resend_url = wp_nonce_url(
		add_query_arg( $resend_url_params, wp_login_url() ),
		'sz-resend-activation'
	);

	$resend_string = '<br /><br />' . sprintf( __( 'If you have not received an email yet, <a href="%s">click here to resend it</a>.', 'sportszone' ), esc_url( $resend_url ) );

	return new WP_Error( 'sz_account_not_activated', __( '<strong>ERROR</strong>: Your account has not been activated. Check your email for the activation link.', 'sportszone' ) . $resend_string );
}
add_filter( 'authenticate', 'sz_core_signup_disable_inactive', 30, 3 );
 ...
 * @since 2.0.0
 *
 * @see sz_core_signup_disable_inactive()
 */
function sz_members_login_resend_activation_email() {
	global $error;
	...
	// Verify nonce.
	if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'sz-resend-activation' ) ) {
		die( 'Security check' );
	}
	...
	$resend = SZ_Signup::resend( array( $signup_id ) );

	// Add feedback message.
	if ( ! empty( $resend['errors'] ) ) {
		$error = __( '<strong>ERROR</strong>: Your account has already been activated.', 'sportszone' );
	} else {
		$error = __( 'Activation email resent! Please check your inbox or spam folder.', 'sportszone' );
	}
}
add_action( 'login_form_sz-resend-activation', 'sz_members_login_resend_activation_email' );
 ...
 * @since 1.1.0
 */
function sz_core_wpsignup_redirect() {

	// Bail in admin or if custom signup page is broken.
	if ( is_admin() || ! sz_has_custom_signup_page() ) {
		return;
	}
	...
	sz_core_redirect( sz_get_signup_page() );
}
add_action( 'sz_init', 'sz_core_wpsignup_redirect' );
 ...
 * reauthenticate.
 *
 * Note: A spammer cannot log back in - {@see sz_core_boot_spammer()}.
 *
 * Runs on 'sz_init' at priority 5 so the members component globals are setup
 * before we do our spammer checks.
 *
 * This is important as the $sz->loggedin_user object is setup at priority 4.
 *
 * @since 1.8.0
 */
function sz_stop_live_spammer() {
	...
	if ( sz_is_user_spammer( sz_loggedin_user_id() ) ) {
		// Setup login args.
		$args = array(
		 ...
		 */
		$login_url = apply_filters( 'sz_live_spammer_redirect', add_query_arg( $args, wp_login_url() ) );
...
add_action( 'sz_init', 'sz_stop_live_spammer', 5 );
 ...
 * @since 1.8.0
 */
function sz_live_spammer_login_error() {
	global $error;

	$error = __( '<strong>ERROR</strong>: Your account has been marked as a spammer.', 'sportszone' );

	// Shake shake shake!
	add_action( 'login_head', 'wp_shake_js', 12 );
}
add_action( 'login_form_sz-spam', 'sz_live_spammer_login_error' );
 ...
 */
function sz_get_displayed_user() {
	$sz = sportszone();

	$displayed_user = null;
	if ( ! empty( $sz->displayed_user->id ) ) {
		$displayed_user = $sz->displayed_user;
	}
	 ...
	 */
	return apply_filters( 'sz_get_displayed_user', $displayed_user );
}
 ...
 * @since 2.7.0
 */
function sz_member_type_tax_name() {
	echo sz_get_member_type_tax_name();
}
	 ...
	 */
	function sz_get_member_type_tax_name() {
		/**
		 ...
		 */
		return apply_filters( 'sz_get_member_type_tax_name', 'sz_member_type' );
	}
 ...
 */
function sz_register_member_type( $member_type, $args = array() ) {
	$sz = sportszone();

	if ( isset( $sz->members->types[ $member_type ] ) ) {
		return new WP_Error( 'sz_member_type_exists', __( 'Member type already exists.', 'sportszone' ), $member_type );
	}
	 ...
	 */
	$illegal_names = apply_filters( 'sz_member_type_illegal_names', array( 'any', 'null', '_none' ) );
	if ( in_array( $member_type, $illegal_names, true ) ) {
		return new WP_Error( 'sz_member_type_illegal_name', __( 'You may not register a member type with this name.', 'sportszone' ), $member_type );
	}
	...
	$sz->members->types[ $member_type ] = $type = (object) $r;
	 ...
	 */
	do_action( 'sz_registered_member_type', $member_type, $type );
 ...
 */
function sz_get_member_type_object( $member_type ) {
	$types = sz_get_member_types( array(), 'objects' );
 ...
 * @since 2.2.0
 *
 * @see sz_register_member_type() for accepted arguments.
 *
 ...
 */
function sz_get_member_types( $args = array(), $output = 'names', $operator = 'and' ) {
	$types = sportszone()->members->types;
	 ...
	 */
	$types = apply_filters( 'sz_get_member_types', $types, $args, $operator );
 ...
 * @return false|array $retval See {@see sz_set_object_terms()}.
 */
function sz_set_member_type( $user_id, $member_type, $append = false ) {
	// Pass an empty $member_type to remove a user's type.
	if ( ! empty( $member_type ) && ! sz_get_member_type_object( $member_type ) ) {
		return false;
	}

	$retval = sz_set_object_terms( $user_id, $member_type, sz_get_member_type_tax_name(), $append );
	...
	if ( ! is_wp_error( $retval ) ) {
		 ...
		 */
		do_action( 'sz_set_member_type', $user_id, $member_type, $append );
	}
 ...
 */
function sz_remove_member_type( $user_id, $member_type ) {
	// Bail if no valid member type was passed.
	if ( empty( $member_type ) || ! sz_get_member_type_object( $member_type ) ) {
		return false;
	}

	$deleted = sz_remove_object_terms( $user_id, $member_type, sz_get_member_type_tax_name() );
	...
	if ( ! is_wp_error( $deleted ) ) {
		 ...
		 * @param string $member_type Member type.
		 */
		do_action( 'sz_remove_member_type', $user_id, $member_type );
	}
 ...
 */
function sz_get_member_type( $user_id, $single = true ) {
	$types = wp_cache_get( $user_id, 'sz_member_member_type' );

	if ( false === $types ) {
		$raw_types = sz_get_object_terms( $user_id, sz_get_member_type_tax_name() );
		...
		if ( ! is_wp_error( $raw_types ) ) {
			...
			foreach ( $raw_types as $mtype ) {
				if ( sz_get_member_type_object( $mtype->name ) ) {
					$types[] = $mtype->name;
				}
			}

			wp_cache_set( $user_id, $types, 'sz_member_member_type' );
		}
	}
	 ...
	 */
	return apply_filters( 'sz_get_member_type', $type, $user_id, $single );
}
 ...
 */
function sz_has_member_type( $user_id, $member_type ) {
	// Bail if no valid member type was passed.
	if ( empty( $member_type ) || ! sz_get_member_type_object( $member_type ) ) {
		return false;
	}
	...
	$types = sz_get_member_type( $user_id, false );
 ...
 *
 * @param int $user_id ID of the user.
 * @return false|array $value See {@see sz_set_member_type()}.
 */
function sz_remove_member_type_on_user_delete( $user_id ) {
	return sz_set_member_type( $user_id, '' );
}
add_action( 'wpmu_delete_user', 'sz_remove_member_type_on_user_delete' );
add_action( 'delete_user', 'sz_remove_member_type_on_user_delete' );
 ...
 * @return string
 */
function sz_get_current_member_type() {
