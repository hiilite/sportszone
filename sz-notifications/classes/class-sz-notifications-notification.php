<?php
/**
 * SportsZone Notifications Classes.
 *
...
/**
 * SportsZone Notification items.
 *
 * Use this class to create, access, edit, or delete SportsZone Notifications.
 *
 * @since 1.9.0
 */
class SZ_Notifications_Notification {
	...
	public function save() {
		 ...
		 * @since 2.0.0
		 *
		 * @param SZ_Notifications_Notification $value Current instance of the notification item being saved. Passed by reference.
		 */
		do_action_ref_array( 'sz_notification_before_save', array( &$this ) );
		 ...
		 * @since 2.0.0
		 *
		 * @param SZ_Notifications_Notification $value Current instance of the notification item being saved.
		 *                                             Passed by reference.
		 */
		do_action_ref_array( 'sz_notification_after_save', array( &$this ) );
	...
	public function populate() {
		...
		$notification = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$sz->notifications->table_name} WHERE id = %d", $this->id ) );
	...
	protected static function _insert( $data = array(), $data_format = array() ) {
		global $wpdb;
		return $wpdb->insert( sportszone()->notifications->table_name, $data, $data_format );
	}
	 ...
	 *                            SZ_Notification_Notification object.
	...
	protected static function _update( $data = array(), $where = array(), $data_format = array(), $where_format = array() ) {
		global $wpdb;
		return $wpdb->update( sportszone()->notifications->table_name, $data, $where, $data_format, $where_format );
	}
	...
	protected static function _delete( $where = array(), $where_format = array() ) {
		global $wpdb;
		return $wpdb->delete( sportszone()->notifications->table_name, $where, $where_format );
	}

	/**
	 * Assemble the WHERE clause of a get() SQL statement.
	 *
	 * Used by SZ_Notifications_Notification::get() to create its WHERE
	 * clause.
	 *
	 * @since 1.9.0
	 *
	 * @param array  $args           See {@link SZ_Notifications_Notification::get()}
	 *                               for more details.
	...
	protected static function get_where_sql( $args = array(), $select_sql = '', $from_sql = '', $join_sql = '', $meta_query_sql = '' ) {
		...
		if ( ! empty( $args['search_terms'] ) ) {
			$search_terms_like = '%' . sz_esc_like( $args['search_terms'] ) . '%';
		 ...
		 */
		$where_conditions = apply_filters( 'sz_notifications_get_where_conditions', $where_conditions, $args, $select_sql, $from_sql, $join_sql, $meta_query_sql );
	 ...
	 *
	 * Used by SZ_Notifications_Notification::get() to create its ORDER BY
	 * clause.
	 *
	 * @since 1.9.0
	 *
	 ...
	 *
	 * Used by SZ_Notifications_Notification::get() to create its LIMIT clause.
	 *
	 * @since 1.9.0
	 *
	 * @param array $args See {@link SZ_Notifications_Notification::get()}
	 ...
	 *                    See {@SZ_Notifications_Notification::get()}
	 *                    for a breakdown.
	...
	public static function check_access( $user_id, $notification_id ) {
		...
		return $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(id) FROM {$sz->core->table_name_notifications} WHERE id = %d AND user_id = %d", $notification_id, $user_id ) );
	}
	...
	public static function get( $args = array() ) {
		...
		// FROM.
		$from_sql   = "FROM {$sz->notifications->table_name} n ";
		...
		if ( true === $r['update_meta_cache'] ) {
			sz_notifications_update_meta_cache( wp_list_pluck( $results, 'id' ) );
		}
	 ...
	 * @since 1.9.0
	 *
	 * @see SZ_Notifications_Notification::get() for a description of arguments.
	 *
	 * @param array $args See {@link SZ_Notifications_Notification::get()}.
	...
	/**
	 * Get the SQL for the 'meta_query' param in SZ_Notifications_Notification::get().
	 *
	 * We use WP_Meta_Query to do the heavy lifting of parsing the
	 * meta_query array and creating the necessary SQL clauses. However,
	 * since SZ_Notifications_Notification::get() builds its SQL differently than
	...
	public static function get_meta_query_sql( $meta_query = array() ) {
		...
		$GLOBALS['wpdb']->notificationmeta = sportszone()->notifications->table_name_meta;
	...
	/**
	 * Get the SQL for the 'date_query' param in SZ_Notifications_Notification::get().
	 *
	 ...
	 * However, since SZ_Notifications_Notification::get() builds its SQL
	...
	public static function get_date_query_sql( $date_query = array() ) {
		...
		// Date query.
		$date_query = new SZ_Date_Query( $date_query, 'date_notified' );
	 ...
	 * @since 1.9.0
	 *
	 * @see SZ_Notifications_Notification::get() for a description of
	...
	public static function update( $update_args = array(), $where_args = array() ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param array $update_args See SZ_Notifications_Notification::update().
		 * @param array $where_args  See SZ_Notifications_Notification::update().
		 */
		do_action( 'sz_notification_before_update', $update_args, $where_args );
	...
	public static function delete( $args = array() ) {
		 ...
		 *                    array( 'item_id' => 7, 'component_action' => 'members' ).
		 */
		do_action( 'sz_notification_before_delete', $args );
	 ...
	 * @since 1.9.0
	 *
	 * @see SZ_Notifications_Notification::delete() for explanation of
	 *      return value.
	 *
	 ...
	 * friendship requests", etc. See sz_notifications_get_notifications_for_user().
	 * For a full-fidelity list of user notifications, use
	 * sz_notifications_get_all_notifications_for_user().
	 *
	 * @since 3.0.0
	 *
