<?php
/**
 * SportsZone Notifications Template Loop Class.
 *
 * @package SportsZone
 ...
 * @since 1.9.0
 */
class SZ_Notifications_Template {
	 ...
	 * Constructor method.
	 *
	 * @see sz_has_notifications() For information on the array format.
	 *
	 * @since 1.9.0
	 *
	 * @param array $args {
	 *     An array of arguments. See {@link sz_has_notifications()}
	 *     for more details.
	 * }
	 */
	public function __construct( $args = array() ) {
		...
		$this->notifications            = SZ_Notifications_Notification::get( $this->query_vars );
		$this->total_notification_count = SZ_Notifications_Notification::get_total_count( $this->query_vars );
		...
		if ( (int) $this->total_notification_count && (int) $this->pag_num ) {
			...
			$this->pag_links = paginate_links( array(
				...
				'prev_text' => _x( '&larr;', 'Notifications pagination previous text', 'sportszone' ),
				'next_text' => _x( '&rarr;', 'Notifications pagination next text',     'sportszone' ),
				'mid_size'  => 1,
				'add_args'  => $add_args,
			) );
		}
	 ...
	 *
	 * This method is used by {@link sz_notifications()} as part of the
	 * while loop that controls iteration inside the notifications loop, eg:
	 *     while ( sz_notifications() ) { ...
	 *
	 * @since 1.9.0
	 *
	 * @see sz_notifications()
	 *
	 ...
	 *
	 * Used by {@link sz_the_notification()} to set up the current
