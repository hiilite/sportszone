<?php
/**
 * SportsZone Member Notifications Functions.
 *
 ...
 *                  on failure.
 */
function sz_notifications_add_notification( $args = array() ) {
	...
	if ( ! $r['allow_duplicate'] ) {
		...
		// duplicate status.
		$existing = SZ_Notifications_Notification::get( array(
			'user_id'           => $r['user_id'],
			'item_id'           => $r['item_id'],
	...
	$notification                    = new SZ_Notifications_Notification;
 ...
 * @return SZ_Notifications_Notification Notification object for ID specified.
 */
function sz_notifications_get_notification( $id ) {
	return new SZ_Notifications_Notification( $id );
}
 ...
 */
function sz_notifications_delete_notification( $id ) {
	if ( ! sz_notifications_check_notification_access( sz_displayed_user_id(), $id ) ) {
		return false;
	}

	return SZ_Notifications_Notification::delete( array( 'id' => $id ) );
}
 ...
 * @return false|int True on success, false on failure.
 */
function sz_notifications_mark_notification( $id, $is_new = false ) {
	if ( ! sz_notifications_check_notification_access( sz_displayed_user_id(), $id ) ) {
		return false;
	}

	return SZ_Notifications_Notification::update(
		array( 'is_new' => $is_new ),
		array( 'id'     => $id     )
	);
}
 ...
 */
function sz_notifications_get_all_notifications_for_user( $user_id = 0 ) {
	...
	$notifications = wp_cache_get( 'all_for_user_' . $user_id, 'sz_notifications' );
	if ( false === $notifications ) {
		$notifications = SZ_Notifications_Notification::get( array(
			'user_id' => $user_id
		) );
		wp_cache_set( 'all_for_user_' . $user_id, $notifications, 'sz_notifications' );
	}
	 ...
	 */
	return apply_filters( 'sz_notifications_get_all_notifications_for_user', $notifications, $user_id );
}
 ...
 * See SZ_Notifications_Notification::get_grouped_notifications_for_user() for
 * more details.
 *
 * @since 3.0.0
 *
 ...
 */
function sz_notifications_get_grouped_notifications_for_user( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
		$user_id = ( sz_displayed_user_id() ) ? sz_displayed_user_id() : sz_loggedin_user_id();
	}

	$notifications = wp_cache_get( $user_id, 'sz_notifications_grouped_notifications' );
	if ( false === $notifications ) {
		$notifications = SZ_Notifications_Notification::get_grouped_notifications_for_user( $user_id );
		wp_cache_set( $user_id, $notifications, 'sz_notifications_grouped_notifications' );
	}
 ...
 */
function sz_notifications_get_notifications_for_user( $user_id, $format = 'string' ) {
	$sz = sportszone();

	$notifications = sz_notifications_get_grouped_notifications_for_user( $user_id );
	...
	foreach ( $notifications as $notification_item ) {
		...
		// object in the $sz object is keyed as 'profile', which is where we need
		...
		if ( isset( $sz->{$component_name}->notification_callback ) && is_callable( $sz->{$component_name}->notification_callback ) ) {

			// Function should return an object.
			if ( 'object' === $format ) {

				// Retrieve the content of the notification using the callback.
				$content = call_user_func( $sz->{$component_name}->notification_callback, $notification_item->component_action, $notification_item->item_id, $notification_item->secondary_item_id, $notification_item->total_count, 'array', $notification_item->id );
				...
				if ( is_string( $content ) ) {
					...
					$notification_object->href    = sz_loggedin_user_domain();
				} else {
				...
				$content      = call_user_func( $sz->{$component_name}->notification_callback, $notification_item->component_action, $notification_item->item_id, $notification_item->secondary_item_id, $notification_item->total_count, 'string', $notification_item->id );
				$renderable[] = $content;
			}

			// @deprecated format_notification_function - 1.5
		} elseif ( isset( $sz->{$component_name}->format_notification_function ) && function_exists( $sz->{$component_name}->format_notification_function ) ) {
			$renderable[] = call_user_func( $sz->{$component_name}->notification_callback, $notification_item->component_action, $notification_item->item_id, $notification_item->secondary_item_id, $notification_item->total_count );
			...
			if ( 'object' === $format ) {
				 ...
				 * If your plugin extends the {@link SZ_Component} class, you should use the
				 * 'notification_callback' parameter in your extended
				 * {@link SZ_Component::setup_globals()} method instead.
				 *
				 * @since 1.9.0
				 ...
				 */
				$content = apply_filters_ref_array( 'sz_notifications_get_notifications_for_user', $ref_array );
				...
				/** This filters is documented in sz-notifications/sz-notifications-functions.php */
				$renderable[] = apply_filters_ref_array( 'sz_notifications_get_notifications_for_user', $ref_array );
			}
		}
	}
	 ...
	 */
	return apply_filters( 'sz_core_get_notifications_for_user', $renderable, $user_id, $format );
}
 ...
 */
function sz_notifications_delete_notifications_by_type( $user_id, $component_name, $component_action ) {
	return SZ_Notifications_Notification::delete( array(
		'user_id'          => $user_id,
 ...
 * @return int|false True on success, false on failure.
 */
function sz_notifications_delete_notifications_by_item_id( $user_id, $item_id, $component_name, $component_action, $secondary_item_id = false ) {
	return SZ_Notifications_Notification::delete( array(
		'user_id'           => $user_id,
		'item_id'           => $item_id,
 ...
 * @return int|false True on success, false on failure.
 */
function sz_notifications_delete_all_notifications_by_type( $item_id, $component_name, $component_action = false, $secondary_item_id = false ) {
	return SZ_Notifications_Notification::delete( array(
		'item_id'           => $item_id,
		'secondary_item_id' => $secondary_item_id,
		'component_name'    => $component_name,
 ...
 * @return int|false True on success, false on failure.
 */
function sz_notifications_delete_notifications_from_user( $user_id, $component_name, $component_action ) {
	return SZ_Notifications_Notification::delete( array(
		'item_id'           => $user_id,
		'component_name'    => $component_name,
		'component_action'  => $component_action,
 ...
 */
function sz_notifications_delete_notifications_on_user_delete( $user_id ) {
	return SZ_Notifications_Notification::delete( array(
		'user_id'           => $user_id,
		'item_id'           => false,
...
add_action( 'wpmu_delete_user', 'sz_notifications_delete_notifications_on_user_delete' );
add_action( 'delete_user', 'sz_notifications_delete_notifications_on_user_delete' );
 ...
 * @return int|false True on success, false on failure.
 */
function sz_notifications_mark_notifications_by_type( $user_id, $component_name, $component_action, $is_new = false ) {
	return SZ_Notifications_Notification::update(
		array(
			'is_new' => $is_new
		),
 ...
 * @return int|false True on success, false on failure.
 */
function sz_notifications_mark_notifications_by_item_id( $user_id, $item_id, $component_name, $component_action, $secondary_item_id = false, $is_new = false ) {
	return SZ_Notifications_Notification::update(
		array(
			'is_new' => $is_new
		),
 ...
 * @return int|false True on success, false on failure.
 */
function sz_notifications_mark_all_notifications_by_type( $item_id, $component_name, $component_action = false, $secondary_item_id = false, $is_new = false ) {
	return SZ_Notifications_Notification::update(
		array(
			'is_new' => $is_new
		),
 ...
 * @return int|false True on success, false on failure.
 */
function sz_notifications_mark_notifications_from_user( $user_id, $component_name, $component_action, $is_new = false ) {
	return SZ_Notifications_Notification::update(
		array(
			'is_new' => $is_new
		),
 ...
 */
function sz_notifications_check_notification_access( $user_id, $notification_id ) {
	return (bool) SZ_Notifications_Notification::check_access( $user_id, $notification_id );
}
 ...
 */
function sz_notifications_get_unread_notification_count( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
		$user_id = ( sz_displayed_user_id() ) ? sz_displayed_user_id() : sz_loggedin_user_id();
	}

	$count = wp_cache_get( $user_id, 'sz_notifications_unread_count' );
	if ( false === $count ) {
		$count = SZ_Notifications_Notification::get_total_count( array(
			'user_id' => $user_id,
			'is_new'  => true,
		) );
		wp_cache_set( $user_id, $count, 'sz_notifications_unread_count' );
	}
	 ...
	 */
	return apply_filters( 'sz_notifications_get_total_notification_count', (int) $count, $user_id );
}
 ...
 * @since 1.9.1
 *
 * @see http://sportszone.trac.wordpress.org/ticket/5300
 *
 * @return array $component_names Array of registered components.
 */
function sz_notifications_get_registered_components() {
	...
	$active_components = array_keys( $sz->active_components );

	// Loop through components, look for callbacks, add to return value.
	foreach ( $active_components as $component ) {
		if ( !empty( $sz->$component->notification_callback ) ) {
			$component_names[] = $component;
		}
		// The extended profile component is identified in the active_components array as 'xprofile'.
		// However, the extended profile child object has the key 'profile' in the $sz object.
		if ( 'xprofile' == $component && ! empty( $sz->profile->notification_callback ) ) {
			$component_names[] = $component;
		}
	}
	 ...
	 */
	return apply_filters( 'sz_notifications_get_registered_components', $component_names, $active_components );
}
 ...
 * @since 1.9.0
 */
function sz_notifications_screen_settings() {}
 ...
 */
function sz_notifications_delete_meta( $notification_id, $meta_key = '', $meta_value = '', $delete_all = false ) {

	// Legacy - if no meta_key is passed, delete all for the item.
	if ( empty( $meta_key ) ) {
		$all_meta = sz_notifications_get_meta( $notification_id );
		$keys     = ! empty( $all_meta )
			? array_keys( $all_meta )
			: array();
 ...
 */
function sz_notifications_get_meta( $notification_id = 0, $meta_key = '', $single = true ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
	 ...
	 */
	return apply_filters( 'sz_notifications_get_meta', $retval, $notification_id, $meta_key, $single );
}
 ...
 */
function sz_notifications_update_meta( $notification_id, $meta_key, $meta_value, $prev_value = '' ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
 ...
 */
function sz_notifications_add_meta( $notification_id, $meta_key, $meta_value, $unique = false ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
