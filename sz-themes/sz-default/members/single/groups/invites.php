<?php do_action( 'sz_before_group_invites_content' ); ?>

<?php if ( sz_has_groups( 'type=invites&user_id=' . sz_loggedin_user_id() ) ) : ?>

	<ul id="group-list" class="invites item-list" role="main">

		<?php while ( sz_groups() ) : sz_the_group(); ?>

			<li>
				<div class="item-avatar">
					<a href="<?php sz_group_permalink(); ?>"><?php sz_group_avatar( 'type=thumb&width=50&height=50' ); ?></a>
				</div>

				<h4><a href="<?php sz_group_permalink(); ?>"><?php sz_group_name(); ?></a><span class="small"> - <?php printf( _n( '1 member', '%d members', sz_get_group_total_members( false ), 'sportszone' )  ); ?></span></h4>

				<p class="desc">
					<?php sz_group_description_excerpt(); ?>
				</p>

				<?php do_action( 'sz_group_invites_item' ); ?>

				<div class="action">
					<a class="button accept" href="<?php sz_group_accept_invite_link(); ?>"><?php _e( 'Accept', 'sportszone' ); ?></a> &nbsp;
					<a class="button reject confirm" href="<?php sz_group_reject_invite_link(); ?>"><?php _e( 'Reject', 'sportszone' ); ?></a>

					<?php do_action( 'sz_group_invites_item_action' ); ?>

				</div>
			</li>

		<?php endwhile; ?>
	</ul>

<?php else: ?>

	<div id="message" class="info" role="main">
		<p><?php _e( 'You have no outstanding group invites.', 'sportszone' ); ?></p>
	</div>

<?php endif;?>

<?php do_action( 'sz_after_group_invites_content' ); ?>