 ...
 */
function sz_blogs_has_directory() {
	$sz = sportszone();

	return (bool) !empty( $sz->pages->blogs->id );
}

/**
 * Retrieve a set of blogs.
 *
 * @see SZ_Blogs_Blog::get() for a description of arguments and return value.
 *
 * @param array|string $args {
 *     Arguments are listed here with their default values. For more
 *     information about the arguments, see {@link SZ_Blogs_Blog::get()}.
 ...
 * }
 * @return array See {@link SZ_Blogs_Blog::get()}.
 */
function sz_blogs_get_blogs( $args = '' ) {
	 ...
	 */
	return apply_filters( 'sz_blogs_get_blogs', $blogs, $r );
}
 ...
 * @return bool
 */
function sz_blogs_record_existing_blogs( $args = array() ) {
	global $wpdb;

	// Query for all sites in network.
	$r = sz_parse_args( $args, array(
		'offset'   => (int) sz_get_option( '_sz_record_blogs_offset' ),
		'limit'    => 50,
		'blog_ids' => array(),
		'site_id'  => $wpdb->siteid
	...
	if ( empty( $r['offset'] ) && empty( $r['blog_ids'] ) ) {
		...
		$truncate = $wpdb->query( "TRUNCATE {$sz->blogs->table_name}" );
		if ( is_wp_error( $truncate ) ) {
			return false;
		}

		// Truncate user blogmeta table
		$truncate = $wpdb->query( "TRUNCATE {$sz->blogs->table_name_blogmeta}" );
		if ( is_wp_error( $truncate ) ) {
			return false;
		}
	}
	...
	if ( is_multisite() ) {
		...
		if ( wp_is_large_network( 'users' ) ) {
			$sql['omit_root_blog'] = $wpdb->prepare( "AND blog_id != %d", sz_get_root_blog_id() );
		}
	...
	// Record a single site.
		...
		$record = sz_blogs_record_blog( $wpdb->blogid, get_current_user_id(), true );
	 ...
	 if ( empty( $blogs ) ) {
		...
		if ( is_multisite() ) {
			sz_delete_option( '_sz_record_blogs_offset' );
		}
	...
	foreach ( (array) $blogs as $blog ) {
		...
		foreach ( (array) $users as $user_id ) {
			sz_blogs_add_user_to_blog( $user_id, false, $blog->blog_id );

			// Clear cache
			sz_blogs_clear_blog_object_cache( $blog->blog_id, $user_id );
		}

		// Update blog last activity timestamp
		if ( ! empty( $blog->last_updated ) && false !== strtotime( $blog->last_updated ) ) {
			sz_blogs_update_blogmeta( $blog->blog_id, 'last_activity', $blog->last_updated );
		}
	}
	...
	if ( is_multisite() && empty( $r['blog_ids'] ) ) {
		...
		if ( ! empty( $blog_ids  ) ) {
			sz_update_option( '_sz_record_blogs_offset', $r['limit'] + $r['offset'] );
			sz_blogs_record_existing_blogs( array(
	 ...
	 * @since 2.4.0
	 */
	do_action( 'sz_blogs_recorded_existing_blogs' );
 ...
 */
function sz_blogs_is_blog_recordable( $blog_id, $user_id = 0 ) {
	 ...
	 */
	$recordable_globally = apply_filters( 'sz_blogs_is_blog_recordable', true, $blog_id );
	...
	if ( !empty( $user_id ) ) {
		 ...
		 */
		$recordable_for_user = apply_filters( 'sz_blogs_is_blog_recordable_for_user', $recordable_globally, $blog_id, $user_id );
	} else {
 ...
 */
function sz_blogs_is_blog_trackable( $blog_id, $user_id = 0 ) {
	 ...
	 * @param int  $blog_id Current blog ID.
	 */
	$trackable_globally = apply_filters( 'sz_blogs_is_blog_trackable', sz_blogs_is_blog_recordable( $blog_id, $user_id ), $blog_id );
	...
	if ( !empty( $user_id ) ) {
		 ...
		 * @param int  $user_id Current user ID.
		 */
		$trackable_for_user = apply_filters( 'sz_blogs_is_blog_trackable_for_user', $trackable_globally, $blog_id, $user_id );
	} else {
...
/**
 * Make SportsZone aware of a new site so that it can track its activity.
 *
 * @since 1.0.0
 *
 ...
 */
function sz_blogs_record_blog( $blog_id, $user_id, $no_activity = false ) {

	if ( empty( $user_id ) )
		$user_id = sz_loggedin_user_id();

	// If blog is not recordable, do not record the activity.
	if ( !sz_blogs_is_blog_recordable( $blog_id, $user_id ) )
		return false;
	...
	$recorded_blog          = new SZ_Blogs_Blog;
	...
	sz_blogs_update_blogmeta( $recorded_blog->blog_id, 'url', $url );
	sz_blogs_update_blogmeta( $recorded_blog->blog_id, 'name', $name );
	sz_blogs_update_blogmeta( $recorded_blog->blog_id, 'description', $description );
	sz_blogs_update_blogmeta( $recorded_blog->blog_id, 'last_activity', sz_core_current_time() );
	sz_blogs_update_blogmeta( $recorded_blog->blog_id, 'close_comments_for_old_posts', $close_old_posts );
	sz_blogs_update_blogmeta( $recorded_blog->blog_id, 'close_comments_days_old', $close_days_old );
	sz_blogs_update_blogmeta( $recorded_blog->blog_id, 'thread_comments_depth', $thread_depth );
	sz_blogs_update_blogmeta( $recorded_blog->blog_id, 'comment_moderation', $moderation );
	 ...
	 */
	$is_private = !apply_filters( 'sz_is_new_blog_public', !$is_private );

	/**
	 * Fires after SportsZone has been made aware of a new site for activity tracking.
	 *
	 * @since 1.0.0
	 * @since 2.6.0 Added $no_activity as a parameter.
	 *
	 * @param SZ_Blogs_Blog $recorded_blog Current blog being recorded. Passed by reference.
	 ...
	 */
	do_action_ref_array( 'sz_blogs_new_blog', array( &$recorded_blog, $is_private, $is_recorded, $no_activity ) );
}
add_action( 'wpmu_new_blog', 'sz_blogs_record_blog', 10, 2 );

/**
 * Update blog name in SportsZone blogmeta table.
 *
 ...
 */
function sz_blogs_update_option_blogname( $oldvalue, $newvalue ) {
	global $wpdb;

	sz_blogs_update_blogmeta( $wpdb->blogid, 'name', $newvalue );
}
add_action( 'update_option_blogname', 'sz_blogs_update_option_blogname', 10, 2 );

/**
 * Update blog description in SportsZone blogmeta table.
 *
 * @global object $wpdb DB Layer.
 *
 * @param string $oldvalue Value before save. Passed by do_action() but
 ...
 * @param string $newvalue Value to change meta to.
 */
function sz_blogs_update_option_blogdescription( $oldvalue, $newvalue ) {
	global $wpdb;

	sz_blogs_update_blogmeta( $wpdb->blogid, 'description', $newvalue );
}
add_action( 'update_option_blogdescription', 'sz_blogs_update_option_blogdescription', 10, 2 );

/**
 * Update "Close comments for old posts" option in SportsZone blogmeta table.
 *
 * @since 2.0.0
 *
 * @global object $wpdb DB Layer.
 ...
 * @param string $newvalue Value to change meta to.
 */
function sz_blogs_update_option_close_comments_for_old_posts( $oldvalue, $newvalue ) {
	global $wpdb;

	sz_blogs_update_blogmeta( $wpdb->blogid, 'close_comments_for_old_posts', $newvalue );
}
add_action( 'update_option_close_comments_for_old_posts', 'sz_blogs_update_option_close_comments_for_old_posts', 10, 2 );

/**
 * Update "Close comments after days old" option in SportsZone blogmeta table.
 *
 * @since 2.0.0
 *
 * @global object $wpdb DB Layer.
 ...
 * @param string $newvalue Value to change meta to.
 */
function sz_blogs_update_option_close_comments_days_old( $oldvalue, $newvalue ) {
	global $wpdb;

	sz_blogs_update_blogmeta( $wpdb->blogid, 'close_comments_days_old', $newvalue );
}
add_action( 'update_option_close_comments_days_old', 'sz_blogs_update_option_close_comments_days_old', 10, 2 );
 ...
 * @param string $newvalue Value to change meta to.
 */
function sz_blogs_update_option_thread_comments( $oldvalue, $newvalue ) {
	global $wpdb;
	...
	sz_blogs_update_blogmeta( $wpdb->blogid, 'thread_comments_depth', $thread_depth );
}
add_action( 'update_option_thread_comments', 'sz_blogs_update_option_thread_comments', 10, 2 );
 ...
 * @param string $newvalue Value to change meta to.
 */
function sz_blogs_update_option_thread_comments_depth( $oldvalue, $newvalue ) {
	global $wpdb;

	$comments_enabled = get_option( 'thread_comments' );

	if (  $comments_enabled ) {
		sz_blogs_update_blogmeta( $wpdb->blogid, 'thread_comments_depth', $newvalue );
	}
}
add_action( 'update_option_thread_comments_depth', 'sz_blogs_update_option_thread_comments_depth', 10, 2 );
 ...
 * @param string $newvalue Value to change meta to.
 */
function sz_blogs_update_option_comment_moderation( $oldvalue, $newvalue ) {
	sz_blogs_update_blogmeta( $GLOBALS['wpdb']->blogid, 'comment_moderation', $newvalue );
}
add_action( 'update_option_comment_moderation', 'sz_blogs_update_option_comment_moderation', 10, 2 );
 ...
 */
function sz_blogs_update_option_site_icon( $old_value, $new_value ) {
	if ( 0 === $new_value ) {
		sz_blogs_update_blogmeta( get_current_blog_id(), 'site_icon_url_thumb', 0 );
		sz_blogs_update_blogmeta( get_current_blog_id(), 'site_icon_url_full',  0 );
	} else {
		// Save site icon URL as blogmeta.
		sz_blogs_update_blogmeta( get_current_blog_id(), 'site_icon_url_thumb', get_site_icon_url( sz_core_avatar_thumb_width() ) );
		sz_blogs_update_blogmeta( get_current_blog_id(), 'site_icon_url_full',  get_site_icon_url( sz_core_avatar_full_width()  ) );
	}
}
add_action( 'update_option_site_icon', 'sz_blogs_update_option_site_icon', 10, 2 );
 ...
 */
function sz_blogs_delete_url_blogmeta( $site_id = 0 ) {
	sz_blogs_delete_blogmeta( (int) $site_id, 'url' );
}

if ( function_exists( 'wp_switch_roles_and_user' ) ) {
	add_action( 'clean_site_cache', 'sz_blogs_delete_url_blogmeta' );
} else {
	add_action( 'refresh_blog_details', 'sz_blogs_delete_url_blogmeta' );
}
 ...
 */
function sz_blogs_publish_post_activity_meta( $activity_id, $post, $args ) {
	if ( empty( $activity_id ) || 'post' != $post->post_type ) {
		return;
	}

	sz_activity_update_meta( $activity_id, 'post_title', $post->post_title );
	...
	sz_activity_update_meta( $activity_id, 'post_url',   $post_permalink );

	// Update the blog's last activity.
	sz_blogs_update_blogmeta( $args['item_id'], 'last_activity', sz_core_current_time() );

	/**
	 * Fires after SportsZone has recorded metadata about a published blog post.
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	do_action( 'sz_blogs_new_blog_post', $post->ID, $post, $args['user_id'] );
}
add_action( 'sz_activity_post_type_published', 'sz_blogs_publish_post_activity_meta', 10, 3 );
 ...
 */
function sz_blogs_update_post_activity_meta( $post, $activity, $activity_post_object ) {
	if ( empty( $activity->id ) || empty( $activity_post_object->action_id ) ) {
		return;
	}

	// Update post title in activity meta.
	$existing_title = sz_activity_get_meta( $activity->id, 'post_title' );
	if ( $post->post_title !== $existing_title ) {
		sz_activity_update_meta( $activity->id, 'post_title', $post->post_title );

		if ( ! empty( $activity_post_object->comments_tracking->action_id ) ) {
			// Now update activity meta for post comments... sigh.
			add_filter( 'comments_clauses', 'sz_blogs_comments_clauses_select_by_id' );
			$comments = get_comments( array( 'post_id' => $post->ID ) );
			remove_filter( 'comments_clauses', 'sz_blogs_comments_clauses_select_by_id' );
			...
			if ( ! empty( $comments ) ) {
				...
				$activities = sz_activity_get( $args );
				...
				$args['meta_query'] = array( array(
					'key'     => 'sz_blogs_' . $post->post_type . '_comment_id',
					'value'   => $comment_ids,
					'compare' => 'IN',
				) );
				...
				if ( ! empty( $activity_ids ) ) {
					foreach ( $activity_ids as $aid ) {
						sz_activity_update_meta( $aid, 'post_title', $post->post_title );
					}
				}
	...
	if( 'closed' == $post->comment_status ) {
		sz_activity_update_meta( $activity->id, 'post_comment_status', $post->comment_status );
	} else {
		sz_activity_delete_meta( $activity->id, 'post_comment_status' );
	}
}
add_action( 'sz_activity_post_type_updated', 'sz_blogs_update_post_activity_meta', 10, 3 );
 ...
 */
function sz_blogs_comment_sync_activity_comment( &$activity_id, $comment = null, $activity_args = array(), $activity_post_object = null ) {
	...
	if ( ! empty( $activity_id ) && ! empty( $activity_args['item_id'] ) && 'new_blog_comment' === $activity_post_object->comment_action_id ) {
		...
		sz_activity_update_meta( $activity_id, 'post_title', $comment->post->post_title );
		sz_activity_update_meta( $activity_id, 'post_url',   esc_url_raw( add_query_arg( 'p', $comment->post->ID, home_url( '/' ) ) ) );
	}
	...
	if ( ! sz_disable_blogforum_comments() ) {
		...
		if ( ! empty( $_REQUEST['action'] ) ) {
			$existing_activity_id = get_comment_meta( $comment->comment_ID, 'sz_activity_comment_id', true );
		...
		if ( empty( $activity_post_object ) ) {
			$activity_post_object = sz_activity_get_post_type_tracking_args( $comment->post->post_type );
		}
		...
		if ( isset( $activity_post_object->action_id ) && isset( $activity_post_object->component_id ) ) {
			...
			$parent_activity_id = sz_activity_get_activity_id( array(
			...
			if ( empty( $parent_activity_id ) ) {
				$parent_activity_id = sz_activity_post_type_publish( $comment->post->ID, $comment->post );
			}
		}
		...
		if ( ! empty( $parent_activity_id ) ) {
			...
			if ( ! empty( $comment->comment_parent ) ) {
				$comment_parent = get_comment_meta( $comment->comment_parent, 'sz_activity_comment_id', true );
			}
		...
		if ( ! empty( $activity_args ) ) {
			$activity_id = sz_activity_new_comment( $activity_args );

			if ( empty( $activity_args['id'] ) ) {
				// The activity metadata to inform about the corresponding comment ID
				sz_activity_update_meta( $activity_id, "sz_blogs_{$comment->post->post_type}_comment_id", $comment->comment_ID );

				// The comment metadata to inform about the corresponding activity ID
				add_comment_meta( $comment->comment_ID, 'sz_activity_comment_id', $activity_id );
			 ...
			 * @param object     $activity_post_object The post type tracking args object.
			 */
			do_action( 'sz_blogs_comment_sync_activity_comment', $activity_id, $comment, $activity_args, $activity_post_object );
		}
	}

	// Update the blogs last active date
	sz_blogs_update_blogmeta( $blog_id, 'last_activity', sz_core_current_time() );

	if ( 'new_blog_comment' === $activity_post_object->comment_action_id ) {
		/**
		 * Fires after SportsZone has recorded metadata about a published blog post comment.
		 *
		 * @since 2.5.0
		 *
		 ...
		 */
		do_action( 'sz_blogs_new_blog_comment', $comment->comment_ID, $comment, sz_loggedin_user_id() );
	}
 ...
 *
 * SportsZone does not track blogs for users with the 'subscriber' role by
 * default, though as of 2.1.0 you can filter 'sz_blogs_get_allowed_roles' to
 ...
 */
function sz_blogs_add_user_to_blog( $user_id, $role = false, $blog_id = 0 ) {
	global $wpdb;

	// If no blog ID was passed, use the root blog ID.
	if ( empty( $blog_id ) ) {
		$blog_id = isset( $wpdb->blogid ) ? $wpdb->blogid : sz_get_root_blog_id();
	}
	...
	if ( empty( $role ) ) {
		...
		$user_roles = array_keys( (array) sz_get_user_meta( $user_id, $key, true ) );

		// User has roles so lets.
		if ( ! empty( $user_roles ) ) {

			// Get blog roles.
			$blog_roles      = array_keys( sz_get_current_blog_roles() );
	...
	if ( empty( $role ) || ! in_array( $role, sz_blogs_get_allowed_roles() ) ) {
		return false;
	}

	// Record the blog activity for this user being added to this blog.
	sz_blogs_record_blog( $blog_id, $user_id, true );
}
add_action( 'add_user_to_blog', 'sz_blogs_add_user_to_blog', 10, 3 );
add_action( 'profile_update',   'sz_blogs_add_user_to_blog'        );
add_action( 'user_register',    'sz_blogs_add_user_to_blog'        );

/**
 * The allowed blog roles a member must have to be recorded into the
 * `sz_user_blogs` pointer table.
 *
 * This added and was made filterable in SportsZone 2.1.0 to make it easier
 ...
 * @return string
 */
function sz_blogs_get_allowed_roles() {

	/**
	 * Filters the allowed roles a member must have to be recorded into sz_user_blogs pointer table.
	 *
	 * @since 2.1.0
	 *
	 * @param array $value Array of potential roles user needs.
	 */
	return apply_filters( 'sz_blogs_get_allowed_roles', array( 'contributor', 'author', 'editor', 'administrator' ) );
}
 ...
 */
function sz_blogs_remove_user_from_blog( $user_id, $blog_id = 0 ) {
	global $wpdb;
	...
	sz_blogs_remove_blog_for_user( $user_id, $blog_id );
}
add_action( 'remove_user_from_blog', 'sz_blogs_remove_user_from_blog', 10, 2 );
 ...
 * first. See https://sportszone.trac.wordpress.org/ticket/3916.
 *
 * @since 1.6.0
 */
function sz_blogs_maybe_add_user_to_blog() {
	if ( ! is_multisite() )
		return;
...
add_action( 'init', 'sz_blogs_maybe_add_user_to_blog', 1 );
 ...
 */
function sz_blogs_remove_blog( $blog_id ) {
	 ...
	 */
	do_action( 'sz_blogs_before_remove_blog', $blog_id );

	SZ_Blogs_Blog::delete_blog_for_all( $blog_id );
	 ...
	 */
	do_action( 'sz_blogs_remove_blog', $blog_id );
}
add_action( 'delete_blog', 'sz_blogs_remove_blog' );
 ...
 * @param int $blog_id ID of the blog being removed.
 */
function sz_blogs_remove_blog_for_user( $user_id, $blog_id ) {
	 ...
	 */
	do_action( 'sz_blogs_before_remove_blog_for_user', $blog_id, $user_id );

	SZ_Blogs_Blog::delete_blog_for_user( $blog_id, $user_id );
	 ...
	 * @param int $user_id ID of the user having the blog removed for.
	 */
	do_action( 'sz_blogs_remove_blog_for_user', $blog_id, $user_id );
}
add_action( 'remove_user_from_blog', 'sz_blogs_remove_blog_for_user', 10, 2 );
 ...
 */
function sz_blogs_post_type_remove_comment( $deleted, $comment_id, $activity_post_object, $activity_type = '' ) {
	// Remove synced activity comments, if needed.
	if ( ! sz_disable_blogforum_comments() ) {
		// Get associated activity ID from comment meta
		$activity_id = get_comment_meta( $comment_id, 'sz_activity_comment_id', true );
		...
		if ( ! empty( $activity_id ) ) {
			...
			if ( ! empty( $activity['activities'] ) ) {
				...
				sz_blogs_remove_associated_blog_comments( $activity_ids );

				// rebuild activity comment tree
				SZ_Activity_Activity::rebuild_activity_comment_tree( $activity['activities'][0]->item_id );
	...
	if ( 'new_blog_comment' === $activity_type ) {
		 ...
		 */
		do_action( 'sz_blogs_remove_comment', get_current_blog_id(), $comment_id, sz_loggedin_user_id() );
	}

	return $deleted;
}
add_action( 'sz_activity_post_type_remove_comment', 'sz_blogs_post_type_remove_comment', 10, 4 );
 ...
 * @since 2.0.0
 *
 * @see sz_blogs_remove_synced_comment()
 * @see sz_blogs_sync_delete_from_activity_comment()
 *
 ...
 */
function sz_blogs_remove_associated_blog_comments( $activity_ids = array(), $force_delete = true ) {
	// Query args.
	$query_args = array(
		'meta_query' => array(
			array(
				'key'     => 'sz_activity_comment_id',
	...
	foreach ( $comments as $comment ) {
		...
		if ( empty( $force_delete ) ) {
			delete_comment_meta( $comment->comment_ID, 'sz_activity_comment_id' );
		}
	}
}

/**
 * Get the total number of blogs being tracked by SportsZone.
 *
 * @return int $count Total blog count.
 */
function sz_blogs_total_blogs() {
	$count = wp_cache_get( 'sz_total_blogs', 'sz' );

	if ( false === $count ) {
		$blogs = SZ_Blogs_Blog::get_all();
		$count = $blogs['total'];
		wp_cache_set( 'sz_total_blogs', $count, 'sz' );
	}
	return $count;
}
 ...
 */
function sz_blogs_total_blogs_for_user( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
	...
	$count = wp_cache_get( 'sz_total_blogs_for_user_' . $user_id, 'sz' );
	if ( false === $count ) {
		$count = SZ_Blogs_Blog::total_blog_count_for_user( $user_id );
		wp_cache_set( 'sz_total_blogs_for_user_' . $user_id, $count, 'sz' );
	}
 ...
 */
function sz_blogs_remove_data_for_blog( $blog_id ) {
	 ...
	 */
	do_action( 'sz_blogs_before_remove_data_for_blog', $blog_id );
	 ...
	 * @param int $blog_id ID of the blog whose data is being removed.
	 */
	do_action( 'sz_blogs_remove_data_for_blog', $blog_id );
}
add_action( 'delete_blog', 'sz_blogs_remove_data_for_blog', 1 );

/**
 * Get all of a user's blogs, as tracked by SportsZone.
 *
 * @see SZ_Blogs_Blog::get_blogs_for_user() for a description of parameters
 *      and return values.
 *
 * @param int  $user_id     See {@SZ_Blogs_Blog::get_blogs_for_user()}.
 * @param bool $show_hidden See {@SZ_Blogs_Blog::get_blogs_for_user()}.
 * @return array See {@SZ_Blogs_Blog::get_blogs_for_user()}.
 */
function sz_blogs_get_blogs_for_user( $user_id, $show_hidden = false ) {
	return SZ_Blogs_Blog::get_blogs_for_user( $user_id, $show_hidden );
}

/**
 * Retrieve a list of all blogs.
 *
 * @see SZ_Blogs_Blog::get_all() for a description of parameters and return values.
 *
 * @param int|null $limit See {@SZ_Blogs_Blog::get_all()}.
 * @param int|null $page  See {@SZ_Blogs_Blog::get_all()}.
 * @return array See {@SZ_Blogs_Blog::get_all()}.
 */
function sz_blogs_get_all_blogs( $limit = null, $page = null ) {
	return SZ_Blogs_Blog::get_all( $limit, $page );
}

/**
 * Retrieve a random list of blogs.
 *
 * @see SZ_Blogs_Blog::get() for a description of parameters and return values.
 *
 * @param int|null $limit See {@SZ_Blogs_Blog::get()}.
 * @param int|null $page  See {@SZ_Blogs_Blog::get()}.
 * @return array See {@SZ_Blogs_Blog::get()}.
 */
function sz_blogs_get_random_blogs( $limit = null, $page = null ) {
	return SZ_Blogs_Blog::get( 'random', $limit, $page );
}

/**
 * Check whether a given blog is hidden.
 *
 * @see SZ_Blogs_Blog::is_hidden() for a description of parameters and return values.
 *
 * @param int $blog_id See {@SZ_Blogs_Blog::is_hidden()}.
 * @return bool See {@SZ_Blogs_Blog::is_hidden()}.
 */
function sz_blogs_is_blog_hidden( $blog_id ) {
	return SZ_Blogs_Blog::is_hidden( $blog_id );
}
 ...
 */
function sz_blogs_delete_blogmeta( $blog_id, $meta_key = false, $meta_value = false, $delete_all = false ) {
	global $wpdb;
 ...
 */
function sz_blogs_get_blogmeta( $blog_id, $meta_key = '', $single = true ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
 ...
 */
function sz_blogs_update_blogmeta( $blog_id, $meta_key, $meta_value, $prev_value = '' ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
 ...
 */
function sz_blogs_add_blogmeta( $blog_id, $meta_key, $meta_value, $unique = false ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
 ...
 */
function sz_blogs_remove_data( $user_id ) {
	if ( !is_multisite() )
		return false;
	 ...
	 */
	do_action( 'sz_blogs_before_remove_data', $user_id );

	// If this is regular blog, delete all data for that blog.
	SZ_Blogs_Blog::delete_blogs_for_user( $user_id );
	 ...
	 */
	do_action( 'sz_blogs_remove_data', $user_id );
}
add_action( 'wpmu_delete_user',  'sz_blogs_remove_data' );
add_action( 'delete_user',       'sz_blogs_remove_data' );
add_action( 'sz_make_spam_user', 'sz_blogs_remove_data' );
 ...
 */
function sz_blogs_restore_data( $user_id = 0 ) {
	if ( ! is_multisite() ) {
		return;
	}
	...
	foreach ( $blogs as $blog_id ) {
		sz_blogs_add_user_to_blog( $user_id, false, $blog_id );
	}
}
add_action( 'sz_make_ham_user', 'sz_blogs_restore_data', 10, 1 );
