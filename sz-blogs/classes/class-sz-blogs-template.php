<?php
/**
 * SportsZone Blogs Template Class.
 *
 * @package SportsZone
...
class SZ_Blogs_Template {
	 ...
	 * Constructor method.
	 *
	 * @see SZ_Blogs_Blog::get() for a description of parameters.
	 *
	 * @param string     $type              See {@link SZ_Blogs_Blog::get()}.
	 * @param string     $page              See {@link SZ_Blogs_Blog::get()}.
	 * @param string     $per_page          See {@link SZ_Blogs_Blog::get()}.
	 * @param string     $max               See {@link SZ_Blogs_Blog::get()}.
	 * @param string     $user_id           See {@link SZ_Blogs_Blog::get()}.
	 * @param string     $search_terms      See {@link SZ_Blogs_Blog::get()}.
	...
	public function __construct( $type, $page, $per_page, $max, $user_id, $search_terms, $page_arg = 'bpage', $update_meta_cache = true, $include_blog_ids = false ) {
		...
		$this->pag_page = sz_sanitize_pagination_arg( $this->pag_arg, $page     );
		$this->pag_num  = sz_sanitize_pagination_arg( 'num',          $per_page );

		// Backwards compatibility support for blogs by first letter.
		if ( ! empty( $_REQUEST['letter'] ) ) {
			$this->blogs = SZ_Blogs_Blog::get_by_letter( $_REQUEST['letter'], $this->pag_num, $this->pag_page );

		// Typical blogs query.
		} else {
			$this->blogs = sz_blogs_get_blogs( array(
				'type'              => $type,
		...
		if ( ! empty( $this->total_blog_count ) && ! empty( $this->pag_num ) ) {
			$this->pag_links = paginate_links( array(
				...
				'prev_text' => _x( '&larr;', 'Blog pagination previous text', 'sportszone' ),
				'next_text' => _x( '&rarr;', 'Blog pagination next text',     'sportszone' ),
				'mid_size'  => 1,
				'add_args'  => array(),
			) );
		}
	 ...
	 *
	 * This method is used by {@link sz_blogs()} as part of the while loop
	 ...
	 *
	 * Used by {@link sz_the_blog()} to set up the current blog data while
