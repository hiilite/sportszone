...
/**
 * The main SportsZone blog class.
 *
 * A SZ_Blogs_Object represents a link between a specific WordPress blog on a
...
class SZ_Blogs_Blog {
	...
	public function populate() {
		...
		$blog = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$sz->blogs->table_name} WHERE id = %d", $this->id ) );
	...
	public function save() {
		 ...
		 */
		$this->user_id = apply_filters( 'sz_blogs_blog_user_id_before_save', $this->user_id, $this->id );
		 ...
		 * @param int $value Site ID.
		 */
		$this->blog_id = apply_filters( 'sz_blogs_blog_id_before_save', $this->blog_id, $this->id );
		 ...
		 * @since 1.0.0
		 *
		 * @param SZ_Blogs_Blog $this Current instance of the blog item being saved. Passed by reference.
		 */
		do_action_ref_array( 'sz_blogs_blog_before_save', array( &$this ) );
		...
		if ( $this->id ) {
			// Update.
			$sql = $wpdb->prepare( "UPDATE {$sz->blogs->table_name} SET user_id = %d, blog_id = %d WHERE id = %d", $this->user_id, $this->blog_id, $this->id );
		} else {
			// Save.
			$sql = $wpdb->prepare( "INSERT INTO {$sz->blogs->table_name} ( user_id, blog_id ) VALUES ( %d, %d )", $this->user_id, $this->blog_id );
		}
		 ...
		 * @param SZ_Blogs_Blog $this Current instance of the blog item being saved. Passed by reference.
		 */
		do_action_ref_array( 'sz_blogs_blog_after_save', array( &$this ) );
	...
	public function exists() {
		...
		return $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(id) FROM {$sz->blogs->table_name} WHERE user_id = %d AND blog_id = %d", $this->user_id, $this->blog_id ) );
	}
	...
	public static function get( $type, $limit = false, $page = false, $user_id = 0, $search_terms = false, $update_meta_cache = true, $include_blog_ids = false ) {
		...
		if ( !is_user_logged_in() || ( !sz_current_user_can( 'sz_moderate' ) && ( $user_id != sz_loggedin_user_id() ) ) )
		...
		$paged_blogs = $wpdb->get_results( "
			...
			FROM
			  {$sz->blogs->table_name} b
			  LEFT JOIN {$sz->blogs->table_name_blogmeta} bm ON (b.blog_id = bm.blog_id)
			  LEFT JOIN {$sz->blogs->table_name_blogmeta} bm_name ON (b.blog_id = bm_name.blog_id)
			  LEFT JOIN {$sz->blogs->table_name_blogmeta} bm_description ON (b.blog_id = bm_description.blog_id)
		...
		$paged_blogs = SZ_Blogs_Blog::get_blog_extras( $paged_blogs, $blog_ids, $type );
	...
	public static function delete_blog_for_all( $blog_id ) {
		...
		sz_blogs_delete_blogmeta( $blog_id );

		$sz = sportszone();

		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->blogs->table_name} WHERE blog_id = %d", $blog_id ) );
	}
	...
	public static function delete_blog_for_user( $blog_id, $user_id = null ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->blogs->table_name} WHERE user_id = %d AND blog_id = %d", $user_id, $blog_id ) );
	}
	...
	public static function delete_blogs_for_user( $user_id = null ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->blogs->table_name} WHERE user_id = %d", $user_id ) );
	}
	 ...
	 * blogs that have been recorded by SportsZone, while the WP function
	...
	public static function get_blogs_for_user( $user_id = 0, $show_hidden = false ) {
		...
		if ( !sz_is_my_profile() && !$show_hidden )
			$blogs = $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT b.blog_id, b.id, bm1.meta_value as name, wb.domain, wb.path FROM {$sz->blogs->table_name} b, {$wpdb->base_prefix}blogs wb, {$sz->blogs->table_name_blogmeta} bm1 WHERE b.blog_id = wb.blog_id AND b.blog_id = bm1.blog_id AND bm1.meta_key = 'name' AND wb.public = 1 AND wb.deleted = 0 AND wb.spam = 0 AND wb.mature = 0 AND wb.archived = '0' AND b.user_id = %d ORDER BY b.blog_id", $user_id ) );
		else
			$blogs = $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT b.blog_id, b.id, bm1.meta_value as name, wb.domain, wb.path FROM {$sz->blogs->table_name} b, {$wpdb->base_prefix}blogs wb, {$sz->blogs->table_name_blogmeta} bm1 WHERE b.blog_id = wb.blog_id AND b.blog_id = bm1.blog_id AND bm1.meta_key = 'name' AND wb.deleted = 0 AND wb.spam = 0 AND wb.mature = 0 AND wb.archived = '0' AND b.user_id = %d ORDER BY b.blog_id", $user_id ) );

		$total_blog_count = SZ_Blogs_Blog::total_blog_count_for_user( $user_id );
	...
	/**
	 * Get IDs of all of a user's blogs, as tracked by SportsZone.
	 *
	...
	public static function get_blog_ids_for_user( $user_id = 0 ) {
		...
		return array_map( 'intval', $wpdb->get_col( $wpdb->prepare( "SELECT blog_id FROM {$sz->blogs->table_name} WHERE user_id = %d", $user_id ) ) );
	}

	/**
	 * Check whether a blog has been recorded by SportsZone.
	 *
	...
	public static function is_recorded( $blog_id ) {
		...
		$query = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$sz->blogs->table_name} WHERE blog_id = %d", $blog_id ) );
	 ...
	 * $user_id parameter, or when the logged-in user has the sz_moderate
	 * cap.
	 *
	...
	public static function total_blog_count_for_user( $user_id = null ) {
		...
		if ( ( is_user_logged_in() && $user_id == sz_loggedin_user_id() ) || sz_current_user_can( 'sz_moderate' ) ) {
			return $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(DISTINCT b.blog_id) FROM {$sz->blogs->table_name} b LEFT JOIN {$wpdb->base_prefix}blogs wb ON b.blog_id = wb.blog_id WHERE wb.deleted = 0 AND wb.spam = 0 AND wb.mature = 0 AND wb.archived = '0' AND user_id = %d", $user_id ) );
		} else {
			return $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(DISTINCT b.blog_id) FROM {$sz->blogs->table_name} b LEFT JOIN {$wpdb->base_prefix}blogs wb ON b.blog_id = wb.blog_id WHERE wb.public = 1 AND wb.deleted = 0 AND wb.spam = 0 AND wb.mature = 0 AND wb.archived = '0' AND user_id = %d", $user_id ) );
		}
	}
	...
	public static function search_blogs( $filter, $limit = null, $page = null ) {
		...
		$paged_blogs = $wpdb->get_results( "SELECT DISTINCT bm.blog_id FROM {$sz->blogs->table_name_blogmeta} bm LEFT JOIN {$wpdb->base_prefix}blogs wb ON bm.blog_id = wb.blog_id WHERE ( ( bm.meta_key = 'name' OR bm.meta_key = 'description' ) AND {$search_terms_sql} ) {$hidden_sql} AND wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 ORDER BY meta_value ASC{$pag_sql}" );
		$total_blogs = $wpdb->get_var( "SELECT COUNT(DISTINCT bm.blog_id) FROM {$sz->blogs->table_name_blogmeta} bm LEFT JOIN {$wpdb->base_prefix}blogs wb ON bm.blog_id = wb.blog_id WHERE ( ( bm.meta_key = 'name' OR bm.meta_key = 'description' ) AND {$search_terms_sql} ) {$hidden_sql} AND wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 ORDER BY meta_value ASC" );
	...
	public static function get_all( $limit = null, $page = null ) {
		...
		$hidden_sql = !sz_current_user_can( 'sz_moderate' ) ? "AND wb.public = 1" : '';
		$pag_sql    = ( $limit && $page ) ? $wpdb->prepare( " LIMIT %d, %d", intval( ( $page - 1 ) * $limit), intval( $limit ) ) : '';

		$paged_blogs = $wpdb->get_results( "SELECT DISTINCT b.blog_id FROM {$sz->blogs->table_name} b LEFT JOIN {$wpdb->base_prefix}blogs wb ON b.blog_id = wb.blog_id WHERE wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 {$hidden_sql} {$pag_sql}" );
		$total_blogs = $wpdb->get_var( "SELECT COUNT(DISTINCT b.blog_id) FROM {$sz->blogs->table_name} b LEFT JOIN {$wpdb->base_prefix}blogs wb ON b.blog_id = wb.blog_id WHERE wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 {$hidden_sql}" );
	...
	public static function get_by_letter( $letter, $limit = null, $page = null ) {
		...
		$paged_blogs = $wpdb->get_results( "SELECT DISTINCT bm.blog_id FROM {$sz->blogs->table_name_blogmeta} bm LEFT JOIN {$wpdb->base_prefix}blogs wb ON bm.blog_id = wb.blog_id WHERE bm.meta_key = 'name' AND {$letter_sql} {$hidden_sql} AND wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 ORDER BY bm.meta_value ASC{$pag_sql}" );
		$total_blogs = $wpdb->get_var( "SELECT COUNT(DISTINCT bm.blog_id) FROM {$sz->blogs->table_name_blogmeta} bm LEFT JOIN {$wpdb->base_prefix}blogs wb ON bm.blog_id = wb.blog_id WHERE bm.meta_key = 'name' AND {$letter_sql} {$hidden_sql} AND wb.mature = 0 AND wb.spam = 0 AND wb.archived = '0' AND wb.deleted = 0 ORDER BY bm.meta_value ASC" );
	...
	public static function get_blog_extras( &$paged_blogs, &$blog_ids, $type = false ) {
		...
		$blog_descs = $wpdb->get_results( "SELECT blog_id, meta_value as description FROM {$sz->blogs->table_name_blogmeta} WHERE meta_key = 'description' AND blog_id IN ( {$blog_ids} )" );
	...
	public static function get_user_blog( $user_id, $blog_id ) {
		...
		$user_blog = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$sz->blogs->table_name} WHERE user_id = %d AND blog_id = %d", $user_id, $blog_id ) );
