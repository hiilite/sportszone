 ...
 */
function sz_blogs_register_activity_actions() {
	if ( is_multisite() ) {
		sz_activity_set_action(
			sportszone()->blogs->id,
			'new_blog',
			__( 'New site created', 'sportszone' ),
			'sz_blogs_format_activity_action_new_blog',
			__( 'New Sites', 'sportszone' ),
	 ...
	 * @since 1.1.0
	 */
	do_action( 'sz_blogs_register_activity_actions' );
}
add_action( 'sz_register_activity_actions', 'sz_blogs_register_activity_actions' );

/**
 * Set up the tracking arguments for the 'post' post type.
 *
 * @since 2.5.0 This was moved out of the SZ_Blogs_Component class.
 *
 * @see sz_activity_get_post_type_tracking_args() for information on parameters.
 *
 * @param object|null $params    Tracking arguments.
 * @param string|int  $post_type Post type to track.
 * @return object|null
 */
function sz_blogs_register_post_tracking_args( $params = null, $post_type = 0 ) {
	 ...
	 * @deprecated 2.3.0
	 *
	 * Make sure plugins still using 'sz_blogs_record_post_post_types'
	 * to track their post types will generate new_blog_post activities
	 * See https://sportszone.trac.wordpress.org/ticket/6306
	 *
	...
	$params->component_id    = sportszone()->blogs->id;
	$params->action_id       = 'new_blog_post';
	$params->admin_filter    = __( 'New post published', 'sportszone' );
	$params->format_callback = 'sz_blogs_format_activity_action_new_blog_post';
	$params->front_filter    = __( 'Posts', 'sportszone' );
	...
	if ( post_type_supports( $post_type, 'comments' ) ) {
		 ...
		 * @deprecated 2.5.0
		 *
		 * Make sure plugins still using 'sz_blogs_record_comment_post_types'
		 * to track comment about their post types will generate new_blog_comment activities
		 * See https://sportszone.trac.wordpress.org/ticket/6306
		 *
		 * @param array $value Array of post types to track.
		 */
		$comment_post_types = apply_filters( 'sz_blogs_record_comment_post_types', array( 'post' ) );
		...
		if ( isset( $comment_post_types_array[ $post_type ] ) ) {
			...
			$params->comments_tracking->component_id    = sportszone()->blogs->id;
			$params->comments_tracking->action_id       = 'new_blog_comment';
			$params->comments_tracking->admin_filter    = __( 'New post comment posted', 'sportszone' );
			$params->comments_tracking->format_callback = 'sz_blogs_format_activity_action_new_blog_comment';
			$params->comments_tracking->front_filter    = __( 'Comments', 'sportszone' );
...
add_filter( 'sz_activity_get_post_type_tracking_args', 'sz_blogs_register_post_tracking_args', 10, 2 );
 ...
 * @return string
 */
function sz_blogs_format_activity_action_new_blog( $action, $activity ) {
	$blog_url  = sz_blogs_get_blogmeta( $activity->item_id, 'url' );
	$blog_name = sz_blogs_get_blogmeta( $activity->item_id, 'name' );

	$action = sprintf( __( '%s created the site %s', 'sportszone' ), sz_core_get_userlink( $activity->user_id ), '<a href="' . esc_url( $blog_url ) . '">' . esc_html( $blog_name ) . '</a>' );

	// Legacy filter - requires the SZ_Blogs_Blog object.
	if ( has_filter( 'sz_blogs_activity_created_blog_action' ) ) {
		$user_blog = SZ_Blogs_Blog::get_user_blog( $activity->user_id, $activity->item_id );
		if ( $user_blog ) {
			$recorded_blog = new SZ_Blogs_Blog( $user_blog );
		}

		if ( isset( $recorded_blog ) ) {
			$action = apply_filters( 'sz_blogs_activity_created_blog_action', $action, $recorded_blog, $blog_name, sz_blogs_get_blogmeta( $activity->item_id, 'description' ) );
		}
	}
	 ...
	 * @param object $activity Activity data object.
	 */
	return apply_filters( 'sz_blogs_format_activity_action_new_blog', $action, $activity );
}
 ...
 */
function sz_blogs_format_activity_action_new_blog_post( $action, $activity ) {
	$blog_url  = sz_blogs_get_blogmeta( $activity->item_id, 'url' );
	$blog_name = sz_blogs_get_blogmeta( $activity->item_id, 'name' );
	...
	if ( empty( $blog_url ) || empty( $blog_name ) ) {
		...
		sz_blogs_update_blogmeta( $activity->item_id, 'url', $blog_url );
		sz_blogs_update_blogmeta( $activity->item_id, 'name', $blog_name );
	}
	...
	// If activity already exists try to get the post title from activity meta.
		...
		$post_title = sz_activity_get_meta( $activity->id, 'post_title' );
	}
	...
	if ( empty( $post_title ) ) {
		...
		$post_title = esc_html__( '(no title)', 'sportszone' );
		...
		if ( is_a( $post, 'WP_Post' ) ) {
			...
			if ( ! empty( $activity->id ) ) {
				sz_activity_update_meta( $activity->id, 'post_title', $post_title );
			}
		}
	...
	if ( is_multisite() ) {
		$action  = sprintf( __( '%1$s wrote a new post, %2$s, on the site %3$s', 'sportszone' ), $user_link, $post_link, '<a href="' . esc_url( $blog_url ) . '">' . esc_html( $blog_name ) . '</a>' );
	} else {
		$action  = sprintf( __( '%1$s wrote a new post, %2$s', 'sportszone' ), $user_link, $post_link );
	}

	// Legacy filter - requires the post object.
	if ( has_filter( 'sz_blogs_activity_new_post_action' ) ) {
		switch_to_blog( $activity->item_id );
		$post = get_post( $activity->secondary_item_id );
		restore_current_blog();

		if ( ! empty( $post ) && ! is_wp_error( $post ) ) {
			$action = apply_filters( 'sz_blogs_activity_new_post_action', $action, $post, $post_url );
		}
	}
	 ...
	 * @param object $activity Activity data object.
	 */
	return apply_filters( 'sz_blogs_format_activity_action_new_blog_post', $action, $activity );
}
 ...
 * @return string Constructed activity action.
 */
function sz_blogs_format_activity_action_new_blog_comment( $action, $activity ) {
	/**
	 ...
	 */
		...
		$post_url = sz_activity_get_meta( $activity->id, 'post_url' );
	}
	...
	if ( empty( $post_url ) || empty( $post_title ) ) {
		...
		if ( ! empty( $comment->comment_post_ID ) ) {
			...
			sz_activity_update_meta( $activity->id, 'post_url', $post_url );
	...
	if ( is_multisite() ) {
		$action  = sprintf( __( '%1$s commented on the post, %2$s, on the site %3$s', 'sportszone' ), $user_link, $post_link, '<a href="' . esc_url( $blog_url ) . '">' . esc_html( $blog_name ) . '</a>' );
	} else {
		$action  = sprintf( __( '%1$s commented on the post, %2$s', 'sportszone' ), $user_link, $post_link );
	}

	// Legacy filter - requires the comment object.
	if ( has_filter( 'sz_blogs_activity_new_comment_action' ) ) {
		switch_to_blog( $activity->item_id );
		$comment = get_comment( $activity->secondary_item_id );
		restore_current_blog();

		if ( ! empty( $comment ) && ! is_wp_error( $comment ) ) {
			$action = apply_filters( 'sz_blogs_activity_new_comment_action', $action, $comment, $post_url . '#' . $activity->secondary_item_id );
		}
	}
	 ...
	 * @param object $activity Activity data object.
	 */
	return apply_filters( 'sz_blogs_format_activity_action_new_blog_comment', $action, $activity );
}
 ...
 * @return array
 */
function sz_blogs_prefetch_activity_object_data( $activities ) {
	if ( empty( $activities ) ) {
		return $activities;
	}
	...
	foreach ( $activities as $activity ) {
		if ( sportszone()->blogs->id !== $activity->component ) {
			continue;
		}
	...
	if ( ! empty( $blog_ids ) ) {
		sz_blogs_update_meta_cache( $blog_ids );
	}

	return $activities;
}
add_filter( 'sz_activity_prefetch_object_data', 'sz_blogs_prefetch_activity_object_data' );
 ...
 * @since 1.0.0
 *
 * @see sz_activity_add() for description of parameters.
 *
 * @param array|string $args {
 *     See {@link sz_activity_add()} for complete description of arguments.
 ...
 */
function sz_blogs_record_activity( $args = '' ) {
	$defaults = array(
		'user_id'           => sz_loggedin_user_id(),
		'action'            => '',
		'content'           => '',
		'primary_link'      => '',
		'component'         => sportszone()->blogs->id,
		'type'              => false,
		'item_id'           => false,
	...
	if ( ! empty( $r['action'] ) ) {
		 ...
		 */
		$r['action'] = apply_filters( 'sz_blogs_record_activity_action', $r['action'] );
	}
	...
	if ( ! empty( $r['content'] ) ) {
		 ...
		 */
		$r['content'] = apply_filters( 'sz_blogs_record_activity_content', sz_activity_create_summary( $r['content'], $r ), $r['content'], $r );
	}

	// Check for an existing entry and update if one exists.
	$id = sz_activity_get_activity_id( array(
		'user_id'           => $r['user_id'],
	...
	return sz_activity_add( array( 'id' => $id, 'user_id' => $r['user_id'], 'action' => $r['action'], 'content' => $r['content'], 'primary_link' => $r['primary_link'], 'component' => $r['component'], 'type' => $r['type'], 'item_id' => $r['item_id'], 'secondary_item_id' => $r['secondary_item_id'], 'recorded_time' => $r['recorded_time'], 'hide_sitewide' => $r['hide_sitewide'] ) );
}
 ...
 * @since 1.0.0
 *
 * @see sz_activity_delete() for description of parameters.
 *
 * @param array|string $args {
 *     See {@link sz_activity_delete()} for complete description of arguments.
 *     The arguments listed here have different default values from
 *     sz_activity_add().
 *     @type string $component Default: 'blogs'.
 * }
 ...
 */
function sz_blogs_delete_activity( $args = '' ) {
	...
	sz_activity_delete_by_item_id( $r );
}
 ...
 * functions, but for use with the SportsZone activity stream to be as
 ...
 * @since 2.0.0
 *
 * @param object $activity The SZ_Activity_Activity object.
 * @return bool
 */
function sz_blogs_comments_open( $activity ) {
	$open = true;

	$blog_id = $activity->item_id;

	// See if we've mirrored the close comments option before.
	$days_old = sz_blogs_get_blogmeta( $blog_id, 'close_comments_days_old' );
	...
	if ( '' === $days_old ) {
		...
		// Use comments_open().
		remove_filter( 'comments_open', 'sz_comments_open', 10 );
		...
		sz_blogs_update_blogmeta( $blog_id, 'close_comments_for_old_posts', get_option( 'close_comments_for_old_posts' ) );
		sz_blogs_update_blogmeta( $blog_id, 'close_comments_days_old',      get_option( 'close_comments_days_old' ) );
		sz_blogs_update_blogmeta( $blog_id, 'thread_comments_depth',        $thread_depth );
	...
	// Basically a copy of _close_comments_for_old_post().
		...
		// Comments are closed.
		if ( 'closed' == sz_activity_get_meta( $activity->id, 'post_comment_status' ) ) {
			return false;
		}

		if ( ! sz_blogs_get_blogmeta( $blog_id, 'close_comments_for_old_posts' ) ) {
			return $open;
		}
 ...
 *
 * Hooked to the 'sz_blogs_new_blog' action.
 *
 * @since 2.6.0
 *
 * @param SZ_Blogs_Blog $recorded_blog Current site being recorded. Passed by reference.
 ...
 */
function sz_blogs_record_activity_on_site_creation( $recorded_blog, $is_private, $is_recorded, $no_activity ) {
	// Only record this activity if the blog is public.
	if ( ! $is_private && ! $no_activity && sz_blogs_is_blog_trackable( $recorded_blog->blog_id, $recorded_blog->user_id ) ) {
		sz_blogs_record_activity( array(
			 ...
			 */
			'primary_link' => apply_filters( 'sz_blogs_activity_created_blog_primary_link', sz_blogs_get_blogmeta( $recorded_blog->blog_id, 'url' ), $recorded_blog->blog_id ),
			'type'         => 'new_blog',
			'item_id'      => $recorded_blog->blog_id
		) );
	}
}
add_action( 'sz_blogs_new_blog', 'sz_blogs_record_activity_on_site_creation', 10, 4 );
 ...
 */
function sz_blogs_delete_new_blog_activity_for_site( $blog_id, $user_id = 0 ) {
	$args = array(
		'item_id'   => $blog_id,
		'component' => sportszone()->blogs->id,
		'type'      => 'new_blog'
	...
	sz_blogs_delete_activity( $args );
}
add_action( 'sz_blogs_remove_blog',          'sz_blogs_delete_new_blog_activity_for_site', 10, 1 );
add_action( 'sz_blogs_remove_blog_for_user', 'sz_blogs_delete_new_blog_activity_for_site', 10, 2 );
 ...
 * @param int $blog_id Site ID.
 */
function sz_blogs_delete_activity_for_site( $blog_id ) {
	sz_blogs_delete_activity( array(
		'item_id'   => $blog_id,
		'component' => sportszone()->blogs->id,
		'type'      => false
	) );
}
add_action( 'sz_blogs_remove_data_for_blog', 'sz_blogs_delete_activity_for_site' );
 ...
 * @return bool
 */
function sz_blogs_remove_post( $post_id, $blog_id = 0, $user_id = 0 ) {
	global $wpdb;
	 ...
	 */
	do_action( 'sz_blogs_before_remove_post', $blog_id, $post_id, $user_id );
	 ...
	 * @param int $user_id ID of the user having the blog removed for.
	 */
	do_action( 'sz_blogs_remove_post', $blog_id, $post_id, $user_id );
}
add_action( 'delete_post', 'sz_blogs_remove_post' );
 ...
 *
 * For blog post -> activity comment, see {@link sz_activity_post_type_comment()}.
 *
 * @since 2.0.0
 ...
 */
function sz_blogs_sync_add_from_activity_comment( $comment_id, $params, $parent_activity ) {
	// if parent activity isn't a post type having the sportszone-activity support, stop now!
	if ( ! sz_activity_type_supports( $parent_activity->type, 'post-type-comment-tracking' ) ) {
		return;
	}
	...
	$activity = new SZ_Activity_Activity( $comment_id );
	if ( $activity->is_spam ) {
		return;
	}

	// Get userdata.
	if ( $params['user_id'] == sz_loggedin_user_id() ) {
		$user = sportszone()->loggedin_user->userdata;
	} else {
		$user = sz_core_get_core_userdata( $params['user_id'] );
	}

	// Get associated post type and set default comment parent
	$post_type      = sz_activity_post_type_get_tracking_arg( $parent_activity->type, 'post_type' );
	$comment_parent = 0;

	// See if a parent WP comment ID exists.
	if ( ! empty( $params['parent_id'] ) && ! empty( $post_type ) ) {
		$comment_parent = sz_activity_get_meta( $params['parent_id'], "sz_blogs_{$post_type}_comment_id" );
	}

	// Comment args.
	$args = array(
		'comment_post_ID'      => $parent_activity->secondary_item_id,
		'comment_author'       => sz_core_get_user_displayname( $params['user_id'] ),
		'comment_author_email' => $user->user_email,
		'comment_author_url'   => sz_core_get_user_domain( $params['user_id'], $user->user_nicename, $user->user_login ),
		'comment_content'      => $params['content'],
		'comment_type'         => '', // Could be interesting to add 'sportszone' here...
	...
	remove_action( 'comment_post', 'sz_activity_post_type_comment', 10 );
	...
	// Add meta to comment.
	add_comment_meta( $post_comment_id, 'sz_activity_comment_id', $comment_id );

	// Add meta to activity comment.
	if ( ! empty( $post_type ) ) {
		sz_activity_update_meta( $comment_id, "sz_blogs_{$post_type}_comment_id", $post_comment_id );
	}

	// Resave activity comment with WP comment permalink.
	//
	// in sz_blogs_activity_comment_permalink(), we change activity comment
	...
	$resave_activity = new SZ_Activity_Activity( $comment_id );
	 ...
	 */
	do_action( 'sz_blogs_sync_add_from_activity_comment', $comment_id, $args, $parent_activity, $user );
}
add_action( 'sz_activity_comment_posted', 'sz_blogs_sync_add_from_activity_comment', 10, 3 );

/**
 * Deletes the blog comment when the associated activity comment is deleted.
 *
 * Note: This is hooked on the 'sz_activity_delete_comment_pre' filter instead
 * of the 'sz_activity_delete_comment' action because we need to fetch the
 * activity comment children before they are deleted.
 *
 * @since 2.0.0
 * @since 2.5.0 Add the $delected parameter
 *
 * @param bool $retval             Whether SportsZone should continue or not.
 ...
 * @return bool
 */
function sz_blogs_sync_delete_from_activity_comment( $retval, $parent_activity_id, $activity_id, &$deleted ) {
	// Check if parent activity is a blog post.
	$parent_activity = new SZ_Activity_Activity( $parent_activity_id );
	...
	$activity = sz_activity_get( array(
		'in'               => $activity_id,
	...
	$activity_ids   = sz_activity_recurse_comments_activity_ids( $activity );
	...
	sz_blogs_remove_associated_blog_comments( $activity_ids, current_user_can( 'moderate_comments' ) );

	// Multisite again!
	restore_current_blog();

	// Rebuild activity comment tree
	// emulate sz_activity_delete_comment().
	SZ_Activity_Activity::rebuild_activity_comment_tree( $parent_activity_id );

	// Avoid the error message although the comments were successfully deleted
	$deleted = true;

	// We're overriding the default sz_activity_delete_comment() functionality
	// so we need to return false.
	return false;
}
add_filter( 'sz_activity_delete_comment_pre', 'sz_blogs_sync_delete_from_activity_comment', 10, 4 );
 ...
 * @since 2.0.0
 *
 * @param SZ_Activity_Activity $activity The activity object.
 */
function sz_blogs_sync_activity_edit_to_post_comment( SZ_Activity_Activity $activity ) {
	// This is a new entry, so stop!
	// We only want edits!
	if ( empty( $activity->id ) || sz_disable_blogforum_comments() ) {
		return;
	}

	// fetch parent activity item
	$parent_activity = new SZ_Activity_Activity( $activity->item_id );

	// if parent activity isn't a post type having the sportszone-activity support for comments, stop now!
	if ( ! sz_activity_type_supports( $parent_activity->type, 'post-type-comment-tracking' ) ) {
		return;
	}
	...
	$post_comment_id = sz_activity_get_meta( $activity->id, "sz_blogs_{$post_type}_comment_id" );
	...
	remove_action( 'transition_comment_status', 'sz_activity_transition_post_type_comment_status', 10 );
	remove_action( 'sz_activity_post_type_comment', 'sz_blogs_comment_sync_activity_comment', 10 );
	...
	add_action( 'sz_activity_post_type_comment', 'sz_blogs_comment_sync_activity_comment',          10, 4 );
 ...
 */
function sz_blogs_remove_activity_meta_for_trashed_comments( $post_id = 0, $comments = array() ) {
	if ( ! empty( $comments ) ) {
		foreach ( array_keys( $comments ) as $comment_id ) {
			delete_comment_meta( $comment_id, 'sz_activity_comment_id' );
		}
	}
}
add_action( 'trashed_post_comments', 'sz_blogs_remove_activity_meta_for_trashed_comments', 10, 2 );

/**
 * Filter 'new_blog_comment' sz_has_activities() loop to include new- and old-style blog activity comment items.
 *
 * In SportsZone 2.0, the schema for storing activity items related to blog
 ...
 * ensures that the 'new_blog_comment' filter in sz_has_activities() (which
 ...
 *
 * @param array $args Arguments passed from sz_parse_args() in sz_has_activities().
 * @return array $args
 */
function sz_blogs_new_blog_comment_query_backpat( $args ) {
	global $wpdb;
	$sz = sportszone();
	...
	$post_type = sz_activity_post_type_get_tracking_arg( $args['action'], 'post_type' );
	...
	if ( sz_activity_post_type_get_tracking_arg( $args['action'], 'comment_action_id' ) ) {
		return $args;
	}

	// Comment synced ?
	$activity_ids = $wpdb->get_col( $wpdb->prepare( "SELECT activity_id FROM {$sz->activity->table_name_meta} WHERE meta_key = %s", "sz_blogs_{$post_type}_comment_id" ) );
...
add_filter( 'sz_after_has_activities_parse_args',                'sz_blogs_new_blog_comment_query_backpat' );
add_filter( 'sz_activity_list_table_filter_activity_type_items', 'sz_blogs_new_blog_comment_query_backpat' );
 ...
 * @since 2.0.0
 *
 * @see sz_blogs_disable_activity_commenting()
 * @see sz_blogs_setup_comment_loop_globals_on_ajax()
 *
 * @param object $activity The SZ_Activity_Activity object.
 */
function sz_blogs_setup_activity_loop_globals( $activity ) {
	if ( ! is_object( $activity ) ) {
		return;
	}

	// The activity type does not support comments or replies ? stop now!
	if ( ! sz_activity_type_supports( $activity->type, 'post-type-comment-reply' ) ) {
		return;
	}
	...
	if ( isset( sportszone()->blogs->allow_comments[ $activity->id ] ) ) {
		return;
	}

	$allow_comments = sz_blogs_comments_open( $activity );
	$thread_depth   = sz_blogs_get_blogmeta( $activity->item_id, 'thread_comments_depth' );
	$moderation     = sz_blogs_get_blogmeta( $activity->item_id, 'comment_moderation' );

	// Initialize a local object so we won't have to query this again in the
	// comment loop.
	if ( empty( sportszone()->blogs->allow_comments ) ) {
		sportszone()->blogs->allow_comments = array();
	}
	if ( empty( sportszone()->blogs->thread_depth ) ) {
		sportszone()->blogs->thread_depth   = array();
	}
	if ( empty( sportszone()->blogs->comment_moderation ) ) {
		sportszone()->blogs->comment_moderation = array();
	}

	/*
	 * Cache comment settings in the sportszone() singleton for later reference.
	 *
	 * See sz_blogs_disable_activity_commenting() / sz_blogs_can_comment_reply().
	 *
	 ...
	 */
	sportszone()->blogs->allow_comments[ $activity->id ]     = $allow_comments;
	sportszone()->blogs->thread_depth[ $activity->id ]       = $thread_depth;
	sportszone()->blogs->comment_moderation[ $activity->id ] = $moderation;
}
 ...
 * @since 2.0.0
 *
 * @see sz_blogs_setup_activity_loop_globals()
 */
function sz_blogs_setup_comment_loop_globals_on_ajax() {
	// Not AJAX? stop now!
	...
	$parent_activity = new SZ_Activity_Activity( $comment->item_id );

	// Setup the globals.
	sz_blogs_setup_activity_loop_globals( $parent_activity );
}
add_action( 'sz_before_activity_comment', 'sz_blogs_setup_comment_loop_globals_on_ajax' );
 ...
 * @return bool
 */
function sz_blogs_disable_activity_commenting( $retval ) {
	global $activities_template;

	// If activity commenting is disabled, return current value.
	if ( sz_disable_blogforum_comments() || ! isset( $activities_template->in_the_loop ) ) {
		return $retval;
	}

	$type = sz_get_activity_type();

	// It's a post type supporting comment tracking.
	if ( sz_activity_type_supports( $type, 'post-type-comment-tracking' ) ) {
		// The activity type is supporting comments or replies
		if ( sz_activity_type_supports( $type, 'post-type-comment-reply' ) ) {
			// Setup some globals we'll need to reference later.
			sz_blogs_setup_activity_loop_globals( $activities_template->activity );

			// If comments are closed for the WP blog post, we should disable
			// activity comments for this activity entry.
			if ( empty( sportszone()->blogs->allow_comments[ sz_get_activity_id() ] ) ) {
				$retval = false;
			}

			// If comments need moderation, disable activity commenting.
			if ( ! empty( sportszone()->blogs->comment_moderation[ sz_get_activity_id() ] ) ) {
				$retval = false;
			}
...
add_filter( 'sz_activity_can_comment', 'sz_blogs_disable_activity_commenting' );
 ...
 */
function sz_blogs_post_type_comments_avoid_duplicates( $retval ) {
	/**
	...
	if ( 'activity_comment' !== sz_get_activity_type() ) {
		return $retval;
	}

	// Check the parent activity
	$parent_activity = new SZ_Activity_Activity( sz_get_activity_item_id() );

	if ( isset( $parent_activity->type ) && sz_activity_post_type_get_tracking_arg( $parent_activity->type, 'post_type' ) ) {
		$retval = 0;
	}

	return $retval;
}
add_filter( 'sz_activity_get_comment_count', 'sz_blogs_post_type_comments_avoid_duplicates' );
 ...
 *
 * This check uses a locally-cached value set in {@link sz_blogs_disable_activity_commenting()}
 * via {@link sz_blogs_setup_activity_loop_globals()}.
 *
 * @since 2.0.0
 *
 ...
 * @return bool
 */
function sz_blogs_can_comment_reply( $retval, $comment ) {
	if ( is_array( $comment ) ) {
		$comment = (object) $comment;
	}

	// Check comment depth and disable if depth is too large.
	if ( isset( sportszone()->blogs->thread_depth[$comment->item_id] ) ){
		if ( sz_activity_get_comment_depth( $comment ) >= sportszone()->blogs->thread_depth[$comment->item_id] ) {
			$retval = false;
		}
	}

	// Check if we should disable activity replies based on the parent activity.
	if ( isset( sportszone()->blogs->allow_comments[$comment->item_id] ) ){
		...
		if ( empty( sportszone()->blogs->allow_comments[$comment->item_id] ) ) {
			$retval = false;
		}
	}

	// If comments need moderation, disable activity commenting.
	if ( ! empty( sportszone()->blogs->comment_moderation[$comment->item_id] ) ) {
		$retval = false;
	}

	return $retval;
}
add_filter( 'sz_activity_can_comment_reply', 'sz_blogs_can_comment_reply', 10, 2 );
 ...
 * @return string
 */
function sz_blogs_activity_comment_permalink( $retval = '' ) {
	global $activities_template;
	...
	if ( ( false !== $item_id ) && isset( sportszone()->blogs->allow_comments[ $item_id ] ) ) {
		$retval = $activities_template->activity->current_comment->primary_link;
	}

	return $retval;
}
add_filter( 'sz_get_activity_comment_permalink', 'sz_blogs_activity_comment_permalink' );
 ...
 * @return string
 */
function sz_blogs_activity_comment_single_permalink( $retval, $activity ) {
...
add_filter( 'sz_activity_get_permalink', 'sz_blogs_activity_comment_single_permalink', 10, 2 );
 ...
 * @return string
 */
function sz_blogs_activity_comment_single_action( $retval, $activity ) {
	if ( 'activity_comment' !== $activity->type ) {
		return $retval;
	}
	...
	$blog_comment_id = sz_activity_get_meta( $activity->id, "sz_blogs_{$post_type}_comment_id" );

	if ( ! empty( $blog_comment_id ) ) {
		$sz = sportszone();

		// Check if a comment action id is set for the parent activity
		$comment_action_id = sz_activity_post_type_get_tracking_arg( $parent_activity->type, 'comment_action_id' );
		...
		if ( ! empty( $comment_action_id ) ) {
			...
			$format_callback = sz_activity_post_type_get_tracking_arg( $comment_action_id, 'format_callback' );
...
add_filter( 'sz_get_activity_action_pre_meta', 'sz_blogs_activity_comment_single_action', 10, 2 );
