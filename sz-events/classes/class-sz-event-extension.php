...
class SZ_Event_Extension {
	...
	public function _register() {
		...
		// Hook the extension into SportsZone.
	...
	protected function get_default_screens() {
		...
		$screens = array(
			...
			'edit'   => array(
				'submit_text' => __( 'Save Changes', 'sportszone' ),
			),
			'admin'  => array(
	...
	protected function setup_display_hooks() {
		...
		if ( $user_can_visit ) {
			...
			if ( sz_is_current_action( $this->slug ) ) {
				...
				add_action( 'sz_template_title', function() use ( $extension_name ) {
					echo esc_attr( $extension_name );
				} );
			}
		}
	...
	public function _display_hook() {
		add_action( 'sz_template_content', array( &$this, 'call_display' ) );
		 ...
		 */
		sz_core_load_template( apply_filters( 'sz_core_template_plugin', $this->template_file ) );
	}
	...
	public function user_can_see_nav_item( $user_can_see_nav_item = false ) {
		...
		if ( ( 'noone' !== $this->params['show_tab'] ) && sz_current_user_can( 'sz_moderate' ) ) {
			return true;
		}
	...
	public function user_can_visit( $user_can_visit = false ) {
		...
		if ( ( 'noone' !== $this->params['access'] ) && sz_current_user_can( 'sz_moderate' ) ) {
			return true;
		}
	...
	/**
	 * Hook this extension's Create step into SportsZone, if necessary.
	 *
	 * @since 1.8.0
	 */
	...
	/**
	 * Hook this extension's Edit panel into SportsZone, if necessary.
	 *
	 * @since 1.8.0
	 */
	protected function setup_edit_hooks() {
		...
		if ( ! $this->is_screen_enabled( 'edit' ) || ! sz_is_item_admin() ) {
			return;
		}
		...
		if ( sz_is_events_component() && sz_is_current_action( 'admin' ) && sz_is_action_variable( $screen['slug'], 0 ) ) {
			...
			add_action( 'sz_screens', array( $this, 'call_edit_screen_template_loader' ) );
		}
	}
	...
	/**
	 * Hook this extension's Admin metabox into SportsZone, if necessary.
	 *
	 * @since 1.8.0
	 */
	...
	/*
	 * In SportsZone 1.8, the recommended technique for configuring
