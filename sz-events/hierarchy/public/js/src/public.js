(function ( $ ) {
	...
	$( document ).ready( function() {
		...
		$( "#hgsz-enable-tree-view-container input" ).prop( "disabled", $( "#groups-personal" ).hasClass( "selected" ) );
		 ...
		 */
		$( "#sportszone" ).on( "click", ".toggle-child-groups", function( e ) {
			e.preventDefault();
	...
	function send_filter_request( input ) {
		...
		$.cookie( "sz-groups-use-tree-view", checked, { path: "/" } );

		if ( $(".dir-search input").length ) {
			search_terms = $(".dir-search input").val();
		}

		sz_filter_request( "groups", filter, "filter", "div.groups", search_terms, 1, $.cookie( "sz-group-extras" ) );
