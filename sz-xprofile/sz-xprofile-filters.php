...
add_filter( 'sz_get_the_profile_group_name',            'wp_filter_kses',       1 );
add_filter( 'sz_get_the_profile_group_description',     'wp_filter_kses',       1 );
add_filter( 'sz_get_the_profile_field_value',           'xprofile_filter_kses', 1 );
add_filter( 'sz_get_the_profile_field_name',            'wp_filter_kses',       1 );
add_filter( 'sz_get_the_profile_field_edit_value',      'wp_filter_kses',       1 );
add_filter( 'sz_get_the_profile_field_description',     'wp_filter_kses',       1 );

add_filter( 'sz_get_the_profile_field_value',           'wptexturize'        );
add_filter( 'sz_get_the_profile_field_value',           'convert_chars'      );
add_filter( 'sz_get_the_profile_field_value',           'wpautop'            );
add_filter( 'sz_get_the_profile_field_value',           'force_balance_tags' );
add_filter( 'sz_get_the_profile_field_value',           'make_clickable'     );
add_filter( 'sz_get_the_profile_field_value',           'sz_xprofile_escape_field_data', 8, 3 );
add_filter( 'sz_get_the_profile_field_value',           'convert_smilies', 9 );
add_filter( 'sz_get_the_profile_field_value',           'xprofile_filter_format_field_value',         1, 2 );
add_filter( 'sz_get_the_profile_field_value',           'xprofile_filter_format_field_value_by_type', 8, 3 );
add_filter( 'sz_get_the_profile_field_value',           'xprofile_filter_link_profile_data',          9, 3 );

add_filter( 'sz_get_the_profile_field_edit_value',      'force_balance_tags' );
add_filter( 'sz_get_the_profile_field_edit_value',      'sz_xprofile_escape_field_data', 10, 3 );

add_filter( 'sz_get_the_profile_group_name',            'stripslashes' );
add_filter( 'sz_get_the_profile_group_description',     'stripslashes' );
add_filter( 'sz_get_the_profile_field_value',           'stripslashes' );
add_filter( 'sz_get_the_profile_field_edit_value',      'stripslashes' );
add_filter( 'sz_get_the_profile_field_name',            'stripslashes' );
add_filter( 'sz_get_the_profile_field_description',     'stripslashes' );
...
add_filter( 'sz_xprofile_set_field_data_pre_validate',  'xprofile_filter_pre_validate_value_by_field_type', 10, 3 );
...
// Save field options.
add_filter( 'xprofile_field_options_before_save', 'sz_xprofile_sanitize_field_options' );
add_filter( 'xprofile_field_default_before_save', 'sz_xprofile_sanitize_field_default' );
 ...
 * @return mixed
 */
function sz_xprofile_sanitize_field_options( $field_options = '' ) {
 ...
 * @return array|int
 */
function sz_xprofile_sanitize_field_default( $field_default = '' ) {
 ...
 * @param object|null $data_obj The SZ_XProfile_ProfileData object.
...
function xprofile_filter_kses( $content, $data_obj = null ) {
	...
	if ( $data_obj instanceof SZ_XProfile_ProfileData && sz_xprofile_is_richtext_enabled_for_field( $data_obj->field_id ) ) {
		$richtext_tags = array(
	 ...
	 * @param SZ_XProfile_ProfileData $data_obj             The SZ_XProfile_ProfileData object.
	 */
...
function xprofile_sanitize_data_value_before_save( $field_value, $field_id = 0, $reserialize = true, $data_obj = null ) {
	...
	// Sanitize multiple individual option values.
		...
		foreach ( (array) $field_value as $value ) {
			...
			/** This filter is documented in sz-xprofile/sz-xprofile-filters.php */
...
/**
 * Apply display_filter() filters as defined by SZ_XProfile_Field_Type classes, when inside a sz_has_profile() loop.
 *
 * @since 2.1.0
...
function xprofile_filter_format_field_value_by_type( $field_value, $field_type = '', $field_id = '' ) {
	foreach ( sz_xprofile_get_field_types() as $type => $class ) {
		if ( $type !== $field_type ) {
			continue;
		}
...
/**
 * Apply display_filter() filters as defined by the SZ_XProfile_Field_Type classes, when fetched
...
/**
 * Apply pre_validate_filter() filters as defined by the SZ_XProfile_Field_Type classes before validating.
 *
 * @since 2.1.0
 *
 * @param mixed                  $value          Value passed to the sz_xprofile_set_field_data_pre_validate filter.
 ...
 * @return string
 */
function sz_xprofile_escape_field_data( $value, $field_type, $field_id ) {
	if ( sz_xprofile_is_richtext_enabled_for_field( $field_id ) ) {
		// The xprofile_filter_kses() expects a SZ_XProfile_ProfileData object.
		$data_obj = null;
		if ( sz_is_user() ) {
			$data_obj = new SZ_XProfile_ProfileData( $field_id, sz_displayed_user_id() );
		}
 ...
 *
 * To disable globally:
 *     remove_filter( 'sz_get_the_profile_field_value', 'xprofile_filter_link_profile_data', 9, 3 );
 *
...
function xprofile_filter_link_profile_data( $field_value, $field_type = 'textbox' ) {
	...
	if ( strpos( $field_value, ',' ) !== false ) {
		 ...
		 *
		 * sz_xprofile_escape_field_data() runs before this function, which often runs esc_html().
		...
		array_walk( $values, function( &$value, $key ) use ( $field_type, $field ) {
			$value = sz_xprofile_escape_field_data( $value, $field_type, $field->id );
		} );
	}
		...
		foreach ( (array) $values as $value ) {
			...
			// Is not clickable.
				...
				// Less than 5 spaces.
					...
					$search_url   = add_query_arg( array( $query_arg => urlencode( $value ) ), sz_get_members_directory_permalink() );
...
function xprofile_filter_comments( $comments, $post_id = 0 ) {
	...
	if ( $fullnames = sz_core_get_user_displaynames( $user_ids ) ) {
...
/**
 * Filter SZ_User_Query::populate_extras to override each queries users fullname.
 *
 * @since 1.7.0
 *
 * @param SZ_User_Query $user_query   User query to filter.
 * @param string        $user_ids_sql SQL statement to use.
 */
function sz_xprofile_filter_user_query_populate_extras( SZ_User_Query $user_query, $user_ids_sql = '' ) {

	if ( ! sz_is_active( 'xprofile' ) ) {
		return;
	}

	$user_id_names = sz_core_get_user_displaynames( $user_query->user_ids );
...
add_filter( 'sz_user_query_populate_extras', 'sz_xprofile_filter_user_query_populate_extras', 2, 2 );

/**
 * Parse 'xprofile_query' argument passed to SZ_User_Query.
 *
 * @since 2.2.0
 *
 * @param SZ_User_Query $q User query object.
 */
function sz_xprofile_add_xprofile_query_to_user_query( SZ_User_Query $q ) {
	...
	$xprofile_query = new SZ_XProfile_Query( $q->query_vars['xprofile_query'] );
...
add_action( 'sz_pre_user_query', 'sz_xprofile_add_xprofile_query_to_user_query' );
 ...
 * @return string
 */
function sz_xprofile_filter_meta_query( $q ) {
	global $wpdb;
