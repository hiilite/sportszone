 ...
 */
class Field_Type_Multi_Select_Post_Type extends \SZ_XProfile_Field_Type implements Field_Type_Selectable, Field_Type_Multi_Valued {

	public function __construct() {

		parent::__construct();

		$this->name     = _x( 'Custom Post Type Multiselector', 'xprofile field type', 'sz-xprofile-custom-field-types' );
		...
		do_action( 'sz_xprofile_field_type_multiselect_custom_post_type', $this );
	}
	...
	public function edit_field_options_html( array $args = array() ) {
		...
		$posts_selected = maybe_unserialize( \SZ_XProfile_ProfileData::get_value_byid( $this->field_obj->id, $args['user_id'] ) );
		...
		echo apply_filters( 'sz_get_the_profile_field_multiselect_custom_post_type', $html, $args['type'], $post_type_selected, $this->field_obj->id );
	}
			...
			<?php else : ?>
                ...
                <div class="inside">
                    <p>
                        ...
                        <select name="szxcftr_multi_selected_post_type" id="szxcftr_multi_selected_post_type">
	...
	public static function display_filter( $field_value, $field_id = 0 ) {
		...
		if ( $list ) {
			return "<ul class='szxcftr-multi-post-type-posts-list'>{$list}</ul>";
		}
