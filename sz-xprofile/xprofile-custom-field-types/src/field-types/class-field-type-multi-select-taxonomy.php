 ...
 */
class Field_Type_Multi_Select_Taxonomy extends \SZ_XProfile_Field_Type implements Field_Type_Selectable, Field_Type_Multi_Valued {

	public function __construct() {
		parent::__construct();

		$this->name     = _x( 'Custom Taxonomy Multiselector', 'xprofile field type', 'sz-xprofile-custom-field-types' );
		...
		do_action( 'sz_xprofile_field_type_multiselect_custom_taxonomy', $this );
	}
	...
	public function edit_field_options_html( array $args = array() ) {
		...
		$terms_selected    = maybe_unserialize( \SZ_XProfile_ProfileData::get_value_byid( $this->field_obj->id, $args['user_id'] ) );
		...
		echo apply_filters( 'sz_get_the_profile_field_multiselect_custom_taxonomy', $html, $args['type'], $terms_selected, $this->field_obj->id );
	}
	...
	public function admin_new_field_html( \SZ_XProfile_Field $current_field, $control_type = '' ) {
        ...
        <div id="<?php echo esc_attr( $type ); ?>" class="postbox sz-options-box" style="<?php echo esc_attr( $class ); ?> margin-top: 15px;">
			<?php if ( ! $taxonomies ) : ?>
                <h3><?php _e( 'There is no custom taxonomy. You need to create at least one to use this field.', 'sz-xprofile-custom-field-types' ); ?></h3>
			<?php else : ?>
                <h3><?php esc_html_e( 'Select a custom taxonomy:', 'sz-xprofile-custom-field-types' ); ?></h3>
                <div class="inside">
                    <p>
						<?php _e( 'Select a custom taxonomy:', 'sz-xprofile-custom-field-types' ); ?>
                        <select name="szxcftr_multi_selected_taxonomy" id="szxcftr_multi_selected_taxonomy">
                    ...
                    <p>
                        <label>
                            <input type="checkbox" name="szxcftr_multi_tax_allow_new_terms" id="szxcftr_multi_tax_allow_new_terms" value="1" <?php checked(true,  self::allow_new_terms( $current_field->id ) );?> />
		                    <?php _e( 'Allow users to add new terms:', 'sz-xprofile-custom-field-types' ); ?>
                        </label>
	...
	public function is_valid( $values ) {
		...
		$terms = get_terms( array( 'include' => $values, 'hide_empty'=> false, 'fields' => 'id=>name' ) );
	...
	public static function display_filter( $field_value, $field_id = '' ) {
		...
		if ( $list ) {
			return "<ul class='szxcftr-multi-taxonomy-terms-list'>{$list}</ul>";
		}
	...
	public static function get_selected_taxonomy( $field_id ) {
		...
		return sz_xprofile_get_meta( $field_id, 'field', 'selected_taxonomy', true );
	}
	...
	public static function allow_new_terms( $field_id ) {
		...
		return (bool)sz_xprofile_get_meta( $field_id, 'field', 'allow_new_terms', true );
	}
}
