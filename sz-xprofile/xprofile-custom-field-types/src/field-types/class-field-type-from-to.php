 ...
 * 'fromto' field type.
 */
class Field_Type_From_To extends \SZ_XProfile_Field_Type {

	public function __construct() {

		parent::__construct();

		$this->name     = __( 'From/To values', 'sz-xprofile-custom-field-types' );
		$this->category = _x( 'Custom Fields', 'xprofile field type category', 'sz-xprofile-custom-field-types' );

		$this->accepts_null_value = true;
		$this->supports_options   = false;

		do_action( 'sz_xprofile_field_type_from_to_number', $this );
	}
	...
	public function edit_field_html( array $raw_properties = array() ) {
		...
		$field_name = sz_get_the_profile_field_input_name();
		...
		// Input.
		?>
		<div class="szxcftr-from-to-edit-field szxcftr-from-to-edit-field-<?php echo esc_attr( $type );?>">
			<input <?php echo $from_atts; ?> /> <span class="szxcftr-fromto-edit-separator">-</span>
	...
	public function admin_field_html( array $raw_properties = array() ) {
		...
		?>
		<div class="szxcftr-from-to-edit-field">
			<input <?php echo $from_atts; ?> /> <span class="szxcftr-fromto-edit-separator">-</span>
			<input <?php echo $to_atts; ?> />
		</div>
	...
	public function admin_new_field_html( \SZ_XProfile_Field $current_field, $control_type = '' ) {
		...
		<div id="<?php echo esc_attr( $type ); ?>" class="postbox sz-options-box" style="<?php echo esc_attr( $class ); ?> margin-top: 15px;">
			...
			<div class="inside">
				<p>
					<label for="szxcftr_fromto_value_type"> <?php _e( 'Value Type', 'sz-xprofile-custom-field-types' );?></label>
					<select name="szxcftr_fromto_value_type">
						<option value="" <?php selected( $value_type, '', true);?>><?php _e( 'No restrictions', 'sz-xprofile-custom-field-types');?></option>
						<option value="integer" <?php selected( $value_type, 'integer', true );?>><?php _e( 'Integers', 'sz-xprofile-custom-field-types');?></option>
						<option value="numeric" <?php selected( $value_type, 'numeric', true );?>><?php _e( 'Numeric', 'sz-xprofile-custom-field-types');?></option>
						<option value="string" <?php selected( $value_type, 'string', true );?>><?php _e( 'String', 'sz-xprofile-custom-field-types');?></option>
					</select>
				</p>

				<div class="int-numeric-constraints">
					<label for="szxcftr_numeric_type_constraints"><?php _e('Integer/Numeric Type Constraints', 'sz-xprofile-custom-field-types');?></label>

					<h4><?php _e( 'Default Values', 'sz-xprofile-custom-field-types');?></h4>
					<p>
						<label>
							<?php _e( 'From', 'sz-xprofile-custom-field-types');?>
							<input type="text" name="szxcftr_fromto_from_value" value="<?php echo esc_attr( $from_value );?>" />
						</label>

						<label>
							<?php _e( 'To', 'sz-xprofile-custom-field-types');?>
							<input type="text" name="szxcftr_fromto_to_value" value="<?php echo esc_attr( $to_value );?>" />
						</label>
	...
	public static function display_filter( $field_value, $field_id = 0 ) {
		...
		return sprintf( '<span class="szxcftr-fromto-from-value">%s</span>-<span class="szxcftr-fromto-to-value">%s</span>', $field_value[0], $field_value[1] );
	}
	...
	private static function get_value_type( $field_id ) {
		return sz_xprofile_get_meta( $field_id, 'field', 'value_type',  true );
	}
	...
	private static function get_from_value( $field_id ) {
		return sz_xprofile_get_meta( $field_id, 'field', 'from_value',  true );
	}
	...
	private static function get_to_value( $field_id ) {
		return sz_xprofile_get_meta( $field_id, 'field', 'to_value',  true );
	}
}
