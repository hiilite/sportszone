 ...
 * Image Type
 */
class Field_Type_Image extends \SZ_XProfile_Field_Type {

    public function __construct() {
		parent::__construct();

	    $this->name     = _x( 'Image', 'xprofile field type', 'sz-xprofile-custom-field-types' );
	    $this->category = _x( 'Custom Fields', 'xprofile field type category', 'sz-xprofile-custom-field-types' );

		$this->set_format( '/^.+$/', 'replace' );
		do_action( 'sz_xprofile_field_type_image', $this );
	}
	...
	public static function display_filter( $field_value, $field_id = 0 ) {
		...
		$new_field_value = sprintf( '<img src="%s" alt="" class="szxcftr-image" />', esc_url( $new_field_value ) );

		return apply_filters( 'szxcftr_image_display_data', $new_field_value, $field_id );
	}
}
