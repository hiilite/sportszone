...
class Field_Upload_Helper {
	 ...
	 * @var string
	 */
	private $dir_base = "szxcftr-profile-uploads";
	 ...
	 *
	 * @param \SZ_XProfile_ProfileData $data data object.
	 */
	public function on_field_data_save( $data ) {
		...
		if ( empty( $_FILES[ 'field_' . $field_id ] ) || empty( $_FILES[ 'field_' . $field_id ]['tmp_name'] ) ) {
			...
			if ( $data->value == '-' || empty( $data->value ) ) {
				...
				$values = maybe_unserialize( \SZ_XProfile_ProfileData::get_value_byid( $field_id, $data->user_id ) );
				$data->value = $values;
			}
			return;
		}
		...
		if ( is_wp_error( $value ) ) {
			sz_core_add_message( $value->get_error_message(), 'error' );
	...
	private function handle_upload( $field, $data  ) {
		...
		if ( ! $is_upload ) {
			return new \WP_Error( 'file_err', __( 'Invalid file.', 'sz-xprofile-custom-field-types' ) );
		}

		if ( $file_size <= 0 ) {
			return new \WP_Error( 'file_size_err', __( 'Invalid file size.', 'sz-xprofile-custom-field-types' ) );
		}

		$ext          = strtolower( substr( $_FILES[ 'field_' . $field_id ]['name'], strrpos( $_FILES[ 'field_' . $field_id ]['name'], '.' ) + 1 ) );
		$ext_allowed  = szxcftr_get_allowed_file_extensions( $field->type );
		$allowed_size = szxcftr_get_allowed_file_size( $field->type );

		if ( ! in_array( $ext, $ext_allowed ) ) {
			return new \WP_Error( 'invalid_file_type', sprintf( __( 'File type not allowed: (%s).', 'sz-xprofile-custom-field-types' ), implode( ',', $ext_allowed ) ) );
		} elseif ( $file_size > $allowed_size ) {
			return new \WP_Error( sprintf( __( 'Max image upload size: %s MB.', 'sz-xprofile-custom-field-types' ), $allowed_size ) );
		}
	...
	public function upload_dir( $upload_dir ) {
		if ( empty( $this->user_id ) ) {
			$this->user_id = sz_displayed_user_id();
		}
		...
		// uploads/szxcftr-profile-uploads/1/file
		// uploads/szxcftr-profile-uploads/1/image
		return $upload_dir;
	}
}