 ...
 * @since 2.0.0
 */
class SZ_XProfile_Field_Type_Datebox extends SZ_XProfile_Field_Type {
	...
	public function __construct() {
		...
		$this->name     = _x( 'Date Selector', 'xprofile field type', 'sportszone' );
		...
		/**
		 * Fires inside __construct() method for SZ_XProfile_Field_Type_Datebox class.
		 *
		 * @since 2.0.0
		 *
		 * @param SZ_XProfile_Field_Type_Datebox $this Current instance of
		 *                                             the field type datebox.
		 */
		do_action( 'sz_xprofile_field_type_datebox', $this );
	}
	...
	public function edit_field_html( array $raw_properties = array() ) {
		...
		$day_r = sz_parse_args( $raw_properties, array(
			'id'   => sz_get_the_profile_field_input_name() . '_day',
			'name' => sz_get_the_profile_field_input_name() . '_day'
		) );

		$month_r = sz_parse_args( $raw_properties, array(
			'id'   => sz_get_the_profile_field_input_name() . '_month',
			'name' => sz_get_the_profile_field_input_name() . '_month'
		) );

		$year_r = sz_parse_args( $raw_properties, array(
			'id'   => sz_get_the_profile_field_input_name() . '_year',
			'name' => sz_get_the_profile_field_input_name() . '_year'
		) ); ?>
			...
			<div class="input-options datebox-selects">
				 ...
				 * field ID. The hooks will be similar to `sz_field_12_errors`
				...
				<label for="<?php sz_the_profile_field_input_name(); ?>_day" class="xprofile-field-label"><?php
					esc_html_e( 'Day', 'sportszone' );
				?></label>
				<select <?php echo $this->get_edit_field_html_elements( $day_r ); ?>>
					<?php sz_the_profile_field_options( array(
						'type'    => 'day',
						'user_id' => $user_id
					) ); ?>
				</select>

				<label for="<?php sz_the_profile_field_input_name(); ?>_month" class="xprofile-field-label"><?php
					esc_html_e( 'Month', 'sportszone' );
				?></label>
				...
				<label for="<?php sz_the_profile_field_input_name(); ?>_year" class="xprofile-field-label"><?php
					esc_html_e( 'Year', 'sportszone' );
				?></label>
	...
	public function edit_field_options_html( array $args = array() ) {
		...
		$date       = SZ_XProfile_ProfileData::get_value_byid( $this->field_obj->id, $args['user_id'] );
		$day        = 0;
		$month      = 0;
		$year       = 0;
		$html       = '';
		...
		switch ( $args['type'] ) {
			case 'day':
				$html = sprintf( '<option value="" %1$s>%2$s</option>', selected( $day, 0, false ), /* translators: no option picked in select box */ __( '----', 'sportszone' ) );
			...
			case 'month':
				$months = array(
					__( 'January',   'sportszone' ),
					__( 'February',  'sportszone' ),
					__( 'March',     'sportszone' ),
					__( 'April',     'sportszone' ),
					__( 'May',       'sportszone' ),
					__( 'June',      'sportszone' ),
					__( 'July',      'sportszone' ),
					__( 'August',    'sportszone' ),
					__( 'September', 'sportszone' ),
					__( 'October',   'sportszone' ),
					__( 'November',  'sportszone' ),
					__( 'December',  'sportszone' )
				);

				$html = sprintf( '<option value="" %1$s>%2$s</option>', selected( $month, 0, false ), /* translators: no option picked in select box */ __( '----', 'sportszone' ) );
			...
			case 'year':
				$html = sprintf( '<option value="" %1$s>%2$s</option>', selected( $year, 0, false ), /* translators: no option picked in select box */ __( '----', 'sportszone' ) );
		 ...
		 */
		echo apply_filters( 'sz_get_the_profile_field_datebox', $html, $args['type'], $day, $month, $year, $this->field_obj->id, $date );
	}
	...
	public function admin_field_html( array $raw_properties = array() ) {
		...
		?></label>
		<select <?php echo $this->get_edit_field_html_elements( $day_r ); ?>>
			<?php sz_the_profile_field_options( array( 'type' => 'day' ) ); ?>
		</select>
		...
		?></label>
		<select <?php echo $this->get_edit_field_html_elements( $month_r ); ?>>
			<?php sz_the_profile_field_options( array( 'type' => 'month' ) ); ?>
		</select>
		...
		?></label>
		<select <?php echo $this->get_edit_field_html_elements( $year_r ); ?>>
			<?php sz_the_profile_field_options( array( 'type' => 'year' ) ); ?>
		</select>
	...
	public static function get_field_settings( $field_id ) {
		...
		foreach ( $defaults as $key => $value ) {
			$saved = sz_xprofile_get_meta( $field_id, 'field', $key, true );
	...
	public function admin_save_settings( $field_id, $settings ) {
		...
		foreach ( $saved_settings as $setting_key => $setting_value ) {
			sz_xprofile_update_meta( $field_id, 'field', $setting_key, $setting_value );
		}
...
<div id="<?php echo esc_attr( $type ); ?>" class="postbox sz-options-box" style="<?php echo esc_attr( $class ); ?> margin-top: 15px;">
	<table class="form-table sz-date-options">
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Date format', 'sportszone' ); ?>
			</th>
			...
			<td>
				<fieldset>
					...
					<?php foreach ( $this->get_date_formats() as $format ): ?>
						<div class="sz-date-format-option">
					...
					<div class="sz-date-format-option">
						<label for="date-format-elapsed">
							...
							<span class="date-format-label" id="date-format-elapsed-setting"><?php esc_html_e( 'Time elapsed', 'sportszone' ); ?></span> <?php _e( '<code>4 years ago</code>, <code>4 years from now</code>', 'sportszone' ); ?>
						</label>
					</div>
					...
					<div class="sz-date-format-option">
						<label for="date-format-custom">
							...
							<span class="date-format-label"><?php esc_html_e( 'Custom:', 'sportszone' ); ?></span>
						</label>
						<label for="date-format-custom-value" class="screen-reader-text"><?php esc_html_e( 'Enter custom time format', 'sportszone' ); ?></label>
						<input type="text" name="field-settings[date_format_custom]" id="date-format-custom-value" class="date-format-custom-value" value="<?php echo esc_attr( $settings['date_format_custom'] ); ?>" aria-describedby="date-format-custom-example" /> <span class="screen-reader-text"><?php esc_html_e( 'Example:', 'sportszone' ); ?></span><span class="date-format-custom-example" id="date-format-custom-sample"><?php if ( $settings['date_format_custom'] ) : ?><?php echo esc_html( date_i18n( $settings['date_format_custom'] ) ); endif; ?></span><span class="spinner" id="date-format-custom-spinner" aria-hidden="true"></span>

						<p><a href="https://codex.wordpress.org/Formatting_Date_and_Time"><?php esc_html_e( 'Documentation on date and time formatting', 'sportszone' ); ?></a></p>
					</div>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Range', 'sportszone' ); ?>
			</th>

			<td>
				<fieldset class="sz-range-types">
					<legend class="screen-reader-text">
						<?php esc_html_e( 'Range', 'sportszone' ); ?>
					</legend>

					<div class="sz-date-format-option">
						<div class="sz-date-range-type-label">
							<label for="range_type_absolute">
								...
								<?php esc_html_e( 'Absolute', 'sportszone' ); ?>
							</label>
						</div>

						<div class="sz-date-range-type-values">
							<label for="field-settings[range_absolute_start]" aria-label="Year"><?php esc_html_e( 'Start:', 'sportszone' ); ?></label>
							...
							<label for="field-settings[range_absolute_end]" aria-label="Year"><?php esc_html_e( 'End:', 'sportszone' ); ?></label>
					...
					<div class="sz-date-format-option">
						<div class="sz-date-range-type-label">
							<label for="range_type_relative">
								...
								<?php esc_html_e( 'Relative', 'sportszone' ); ?>
							</label>
						</div>

						<div class="sz-date-range-type-values">
							<label for="field-settings[range_relative_start]"><?php esc_html_e( 'Start:', 'sportszone' ); ?></label>
							...
							<label class="screen-reader-text" for="field-settings[range_relative_start_type]"><?php esc_html_e( 'Select range', 'sportszone' ); ?></label>
							<?php printf( '<select name="field-settings[range_relative_start_type]" id="field-settings[range_relative_start_type]"><option value="past" %s>%s</option><option value="future" %s>%s</option></select>',
								...
								esc_attr__( 'years ago', 'sportszone' ),
								selected( true, $settings['range_relative_start'] > 0, false ),
								esc_attr__( 'years from now', 'sportszone' )
								);
							?>

							<label for="field-settings[range_relative_end]"><?php esc_html_e( 'End:', 'sportszone' ); ?></label>
							...
							?>
							<label class="screen-reader-text" for="field-settings[range_relative_end_type]"><?php esc_html_e( 'Select range', 'sportszone' ); ?></label>
	 ...
	 * @return string Date formatted by sz_format_time().
	 */
	public static function display_filter( $field_value, $field_id = '' ) {
		...
		switch ( $settings['date_format'] ) {
			case 'elapsed' :
				$formatted = sz_core_time_since( $field_value );
			break;
	...
	public function get_date_formats() {
		$date_formats = array_unique( apply_filters( 'date_formats', array( __( 'F j, Y', 'sportszone' ), 'Y-m-d', 'm/d/Y', 'd/m/Y' ) ) );
		 ...
		 */
		return apply_filters( 'sz_xprofile_date_field_date_formats', $date_formats );
	}
}
