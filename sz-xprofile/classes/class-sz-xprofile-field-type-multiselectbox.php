 ...
 * @since 2.0.0
 */
class SZ_XProfile_Field_Type_Multiselectbox extends SZ_XProfile_Field_Type {
	...
	public function __construct() {
		...
		$this->name     = _x( 'Multi Select Box', 'xprofile field type', 'sportszone' );
		...
		/**
		 * Fires inside __construct() method for SZ_XProfile_Field_Type_Multiselectbox class.
		 *
		 * @since 2.0.0
		 *
		 * @param SZ_XProfile_Field_Type_Multiselectbox $this Current instance of
		 *                                                    the field type multiple select box.
		 */
		do_action( 'sz_xprofile_field_type_multiselectbox', $this );
	}
	...
	public function edit_field_html( array $raw_properties = array() ) {
		...
		<select <?php echo $this->get_edit_field_html_elements( $r ); ?> aria-labelledby="<?php sz_the_profile_field_input_name(); ?>-1" aria-describedby="<?php sz_the_profile_field_input_name(); ?>-3">
	...
	public function edit_field_options_html( array $args = array() ) {
		...
		for ( $k = 0, $count = count( $options ); $k < $count; ++$k ) {
			 ...
			 */
			$html .= apply_filters( 'sz_get_the_profile_field_options_multiselect', '<option' . $selected . ' value="' . esc_attr( stripslashes( $options[$k]->name ) ) . '">' . esc_html( stripslashes( $options[$k]->name ) ) . '</option>', $options[$k], $this->field_obj->id, $selected, $k );
		}
