 ...
 * @since 2.0.0
 */
abstract class SZ_XProfile_Field_Type {
	...
	/**
	 * If object is created by an SZ_XProfile_Field object.
	 *
	 * @since 2.0.0
	 * @var SZ_XProfile_Field If this object is created by instantiating a {@link SZ_XProfile_Field},
	...
	public function __construct() {
		...
		/**
		 * Fires inside __construct() method for SZ_XProfile_Field_Type class.
		 *
		 * @since 2.0.0
		 *
		 * @param SZ_XProfile_Field_Type $this Current instance of
		 *                                     the field type class.
		 */
		do_action( 'sz_xprofile_field_type', $this );
	}
	...
	public function set_format( $format, $replace_format = 'add' ) {
		 ...
		 * @param SZ_XProfile_Field_Type $this           Current instance of the SZ_XProfile_Field_Type class.
		 */
		$format = apply_filters( 'sz_xprofile_field_type_set_format', $format, $replace_format, $this );
	...
	public function set_whitelist_values( $values ) {
		foreach ( (array) $values as $value ) {
			 ...
			 * @param SZ_XProfile_Field_Type $this   Current instance of the SZ_XProfile_Field_Type class.
			 */
			$this->validation_whitelist[] = apply_filters( 'sz_xprofile_field_type_set_whitelist_values', $value, $values, $this );
		}
	...
	public function is_valid( $values ) {
		 ...
		 * @param SZ_XProfile_Field_Type $this      Current instance of the SZ_XProfile_Field_Type class.
		 */
		return (bool) apply_filters( 'sz_xprofile_field_type_is_valid', $validated, $values, $this );
	}
	...
	public function admin_new_field_html( SZ_XProfile_Field $current_field, $control_type = '' ) {
		...
		$current_type_obj = sz_xprofile_create_field_type( $type );
		?>

		<div id="<?php echo esc_attr( $type ); ?>" class="postbox sz-options-box" style="<?php echo esc_attr( $class ); ?> margin-top: 15px;">
			<h3><?php esc_html_e( 'Please enter options for this Field:', 'sportszone' ); ?></h3>
			<div class="inside" aria-live="polite" aria-atomic="true" aria-relevant="all">
				<p>
					<label for="sort_order_<?php echo esc_attr( $type ); ?>"><?php esc_html_e( 'Sort Order:', 'sportszone' ); ?></label>
					<select name="sort_order_<?php echo esc_attr( $type ); ?>" id="sort_order_<?php echo esc_attr( $type ); ?>" >
						<option value="custom" <?php selected( 'custom', $current_field->order_by ); ?>><?php esc_html_e( 'Custom',     'sportszone' ); ?></option>
						<option value="asc"    <?php selected( 'asc',    $current_field->order_by ); ?>><?php esc_html_e( 'Ascending',  'sportszone' ); ?></option>
						<option value="desc"   <?php selected( 'desc',   $current_field->order_by ); ?>><?php esc_html_e( 'Descending', 'sportszone' ); ?></option>
					</select>
				</p>
				...
				if ( ! empty( $options ) ) {
					...
					for ( $i = 0, $count = count( $options ); $i < $count; ++$i ) :
						...
						<div id="<?php echo esc_attr( "{$type}_div{$j}" ); ?>" class="sz-option sortable">
							<span class="sz-option-icon grabber"></span>
							<label for="<?php echo esc_attr( "{$type}_option{$j}" ); ?>" class="screen-reader-text"><?php
								/* translators: accessibility text */
								esc_html_e( 'Add an option', 'sportszone' );
							?></label>
							...
							<label for="<?php echo esc_attr( "{$type}_option{$default_name}" ); ?>">
								...
								<?php _e( 'Default Value', 'sportszone' ); ?>
							</label>

							<?php if ( 1 !== $j ) : ?>
								<div class ="delete-button">
									<a href='javascript:hide("<?php echo esc_attr( "{$type}_div{$j}" ); ?>")' class="delete"><?php esc_html_e( 'Delete', 'sportszone' ); ?></a>
								</div>
							<?php endif; ?>
				...
				<p><a href="javascript:add_option('<?php echo esc_js( $type ); ?>')"><?php esc_html_e( 'Add Another Option', 'sportszone' ); ?></a></p>
				 ...
				 * @since 2.3.0
				 *
				 * @param SZ_XProfile_Field $current_field Current field being rendered.
				 */
				do_action( 'sz_xprofile_admin_new_field_additional_settings', $current_field ) ?>
			</div>
		</div>
	 ...
	 * without this scheme, SZ_XProfile_Field_Type_URL prepends 'http://'
	...
	protected function get_edit_field_html_elements( array $properties = array() ) {
		...
		$r = sz_parse_args( $properties, array(
			'id'   => sz_get_the_profile_field_input_name(),
		 ...
		 */
		$r = (array) apply_filters( 'sz_xprofile_field_edit_html_elements', $r, get_class( $this ) );

		return sz_get_form_field_attributes( sanitize_key( sz_get_the_profile_field_name() ), $r );
	}
}
