...
class SZ_XProfile_Group {
	...
	public function save() {
		 ...
		 * @since 1.0.0
		 *
		 * @param SZ_XProfile_Group $this Current instance of the group being saved. Passed by reference.
		 */
		...
		// Update or insert.
		if ( ! empty( $this->id ) ) {
			$sql = $wpdb->prepare( "UPDATE {$sz->profile->table_name_groups} SET name = %s, description = %s WHERE id = %d", $this->name, $this->description, $this->id );
		} else {
			$sql = $wpdb->prepare( "INSERT INTO {$sz->profile->table_name_groups} (name, description, can_delete) VALUES (%s, %s, 1)", $this->name, $this->description );
		}
	...
	public function delete() {
		 ...
		 * @since 2.0.0
		 *
		 * @param SZ_XProfile_Group $this Current instance of the group being deleted. Passed by reference.
		 */
		do_action_ref_array( 'xprofile_group_before_delete', array( &$this ) );

		$sz      = sportszone();
		$sql     = $wpdb->prepare( "DELETE FROM {$sz->profile->table_name_groups} WHERE id = %d", $this->id );
		...
		if ( SZ_XProfile_Field::delete_for_group( $this->id ) ) {

			// Remove profile data for the groups fields.
			for ( $i = 0, $count = count( $this->fields ); $i < $count; ++$i ) {
				SZ_XProfile_ProfileData::delete_for_field( $this->fields[$i]->id );
			}
		}
	...
	/**
	 * Populates the SZ_XProfile_Group object with profile field groups, fields,
	 * and field data.
	 *
	 * @since 1.2.0
	 *
	...
	public static function get( $args = array() ) {
		...
		if ( ! empty( $r['hide_empty_groups'] ) ) {
			$group_ids = $wpdb->get_col( "SELECT DISTINCT g.id FROM {$sz->profile->table_name_groups} g INNER JOIN {$sz->profile->table_name_fields} f ON g.id = f.group_id {$where_sql} ORDER BY g.group_order ASC" );
		} else {
			$group_ids = $wpdb->get_col( "SELECT DISTINCT g.id FROM {$sz->profile->table_name_groups} g {$where_sql} ORDER BY g.group_order ASC" );
		}
		...
		// exclude_fields parameter. See sz_xprofile_get_hidden_fields_for_user().
		$hidden_user_fields = sz_xprofile_get_hidden_fields_for_user( $r['user_id'] );
		...
		if ( sz_get_member_types() ) {
			if ( $r['user_id'] || false !== $r['member_type'] ) {
				...
				if ( $r['user_id'] ) {
					$member_types = sz_get_member_type( $r['user_id'], false );
					if ( empty( $member_types ) ) {
						$member_types = array( 'null' );
					}
				}

				$member_types_fields = SZ_XProfile_Field::get_fields_for_member_type( $member_types );
		...
		// Fetch the fields.
		$field_ids = $wpdb->get_col( "SELECT id FROM {$sz->profile->table_name_fields} WHERE group_id IN ( {$group_ids_in} ) AND parent_id = 0 {$exclude_fields_sql} {$in_sql} ORDER BY field_order" );
		...
		$uncached_field_ids = sz_get_non_cached_ids( $field_ids, 'sz_xprofile_fields' );
		if ( ! empty( $uncached_field_ids ) ) {
			$_uncached_field_ids = implode( ',', array_map( 'intval', $uncached_field_ids ) );
			$uncached_fields = $wpdb->get_results( "SELECT * FROM {$sz->profile->table_name_fields} WHERE id IN ({$_uncached_field_ids})" );
			foreach ( $uncached_fields as $uncached_field ) {
				$fid = intval( $uncached_field->id );
				wp_cache_set( $fid, $uncached_field, 'sz_xprofile_fields' );
			}
		}
		...
		if ( ! empty( $r['fetch_field_data'] ) ) {
			...
			if ( ! empty( $field_ids ) && ! empty( $r['user_id'] ) ) {
				$field_data = SZ_XProfile_ProfileData::get_data_for_user( $r['user_id'], $field_ids );
			}
		...
		if ( ! empty( $r['update_meta_cache'] ) ) {
			sz_xprofile_update_meta_cache( $object_ids );
		}
	...
	protected static function get_group_data( $group_ids ) {
		...
		foreach ( $group_ids as $group_id ) {
			...
			$group_data = wp_cache_get( $group_id, 'sz_xprofile_groups' );
			if ( false !== $group_data ) {
		...
		if ( ! empty( $uncached_gids ) ) {
			...
			$table_name = sportszone()->profile->table_name_groups;
			...
			if ( ! empty( $queried_gdata ) && ! is_wp_error( $queried_gdata ) ) {
				...
				foreach ( (array) $queried_gdata as $gdata ) {
					...
					wp_cache_set( $gdata->id, $gdata, 'sz_xprofile_groups' );
				}
			}
		}
	...
	public static function admin_validate() {
		...
		if ( empty( $_POST['group_name'] ) ) {
			$message = __( 'Please make sure you give the group a name.', 'sportszone' );
			return false;
		} else {
			return true;
		}
	...
	public static function update_position( $field_group_id, $position ) {
		...
		wp_cache_delete( 'all', 'sz_xprofile_groups' );

		$sz = sportszone();

		return $wpdb->query( $wpdb->prepare( "UPDATE {$sz->profile->table_name_groups} SET group_order = %d WHERE id = %d", $position, $field_group_id ) );
	}
	...
	public static function fetch_visibility_level( $user_id = 0, $fields = array() ) {
		...
		$visibility_levels = sz_get_user_meta( $user_id, 'sz_xprofile_visibility_levels', true );

		foreach( (array) $fields as $key => $field ) {

			// Does the admin allow this field to be customized?
			$visibility   = sz_xprofile_get_meta( $field->id, 'field', 'allow_custom_visibility' );
			...
			// If no admin-set default is saved, fall back on a global default.
				...
				$fallback_visibility = sz_xprofile_get_meta( $field->id, 'field', 'default_visibility' );
				...
				$field_visibility = ! empty( $fallback_visibility )
					? $fallback_visibility
					: apply_filters( 'sz_xprofile_default_visibility_level', 'public' );
			}
	...
	public static function fetch_default_visibility_levels() {
		...
		$default_visibility_levels = wp_cache_get( 'default_visibility_levels', 'sz_xprofile' );

		if ( false === $default_visibility_levels ) {
			$sz = sportszone();

			$levels = $wpdb->get_results( "SELECT object_id, meta_key, meta_value FROM {$sz->profile->table_name_meta} WHERE object_type = 'field' AND ( meta_key = 'default_visibility' OR meta_key = 'allow_custom_visibility' )" );
			...
			wp_cache_set( 'default_visibility_levels', $default_visibility_levels, 'sz_xprofile' );
		}
	...
	public function render_admin_form() {
		...
		// New field group.
		if ( empty( $this->id ) ) {
			$title	= __( 'Add New Field Group', 'sportszone' );
		...
		// Existing field group.
			...
			$title  = __( 'Edit Field Group', 'sportszone' );
		...
		<div class="wrap">
			...
			<form id="sz-xprofile-add-field-group" action="<?php echo esc_url( $action ); ?>" method="post">
				<div id="poststuff">
					<div id="post-body" class="metabox-holder columns-<?php echo ( 1 == get_current_screen()->get_columns() ) ? '1' : '2'; ?>">
						<div id="post-body-content">
							<div id="titlediv">
								<div class="titlewrap">
									<label id="title-prompt-text" for="title"><?php esc_html_e( 'Field Group Name (required)', 'sportszone') ?></label>
									<input type="text" name="group_name" id="title" value="<?php echo esc_attr( $this->name ); ?>" autocomplete="off" />
								</div>
							</div>
							<div class="postbox">
								<h2><?php esc_html_e( 'Field Group Description', 'sportszone' ); ?></h2>
								<div class="inside">
							 ...
							 * @since 2.6.0
							 *
							 * @param SZ_XProfile_Group $this Current XProfile group.
							 */
						...
						<div id="postbox-container-1" class="postbox-container">
							...
							<div id="submitdiv" class="postbox">
								<h2><?php _e( 'Submit', 'sportszone' ); ?></h2>
								<div class="inside">
									<div id="submitcomment" class="submitbox">
										<div id="major-publishing-actions">
											...
											// Nonce fields
											wp_nonce_field( 'sz_xprofile_admin_group', 'sz_xprofile_admin_group' );
											...
											<div id="delete-action">
												<a href="<?php echo esc_url( $cancel_url ); ?>" class="deletion"><?php _e( 'Cancel', 'sportszone' ); ?></a>
											</div>
											<div class="clear"></div>
										</div>
									</div>
