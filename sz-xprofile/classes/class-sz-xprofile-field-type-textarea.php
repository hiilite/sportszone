 ...
 * @since 2.0.0
 */
class SZ_XProfile_Field_Type_Textarea extends SZ_XProfile_Field_Type {
	...
	public function __construct() {
		...
		$this->name     = _x( 'Multi-line Text Area', 'xprofile field type', 'sportszone' );
		$this->supports_richtext = true;

		$this->set_format( '/^.*$/m', 'replace' );

		/**
		 * Fires inside __construct() method for SZ_XProfile_Field_Type_Textarea class.
		 *
		 * @since 2.0.0
		 *
		 * @param SZ_XProfile_Field_Type_Textarea $this Current instance of
		 *                                              the field type textarea.
		 */
		do_action( 'sz_xprofile_field_type_textarea', $this );
	}
	...
	public function edit_field_html( array $raw_properties = array() ) {
		...
		$richtext_enabled = sz_xprofile_is_richtext_enabled_for_field(); ?>
		...
		if ( ! $richtext_enabled ) {
			...
			<textarea <?php echo $this->get_edit_field_html_elements( $r ); ?> aria-labelledby="<?php sz_the_profile_field_input_name(); ?>-1" aria-describedby="<?php sz_the_profile_field_input_name(); ?>-3"><?php sz_the_profile_field_edit_value(); ?></textarea>
			 ...
			 *                        Dashboard panel.
			 */
			$editor_args = apply_filters( 'sz_xprofile_field_type_textarea_editor_args', array(
				'teeny'         => true,
				'media_buttons' => false,
				'quicktags'     => true,
				'textarea_rows' => 10,
			...
			wp_editor(
				sz_get_the_profile_field_edit_value(),
				sz_get_the_profile_field_input_name(),
				$editor_args
			);
		}

		if ( sz_get_the_profile_field_description() ) : ?>
	...
	public function admin_field_html( array $raw_properties = array() ) {
		...
		if ( ! $richtext_enabled ) {
			...
			/** This filter is documented in sz-xprofile/classes/class-sz-xprofile-field-type-textarea.php */
			$editor_args = apply_filters( 'sz_xprofile_field_type_textarea_editor_args', array(
				'teeny'         => true,
				'media_buttons' => false,
				'quicktags'     => true,
			...
			wp_editor(
				'',
				'xprofile_textarea_' . sz_get_the_profile_field_id(),
				$editor_args
			);
		}
	}
