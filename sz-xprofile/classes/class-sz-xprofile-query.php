...
class SZ_XProfile_Query {
	 ...
	 *
	 * Called by the public {@see SZ_XProfile_Query::get_sql()}, this method is abstracted out to maintain parity
	...
	public function get_sql_for_clause( &$clause, $parent_query ) {
		...
		$data_table = sportszone()->profile->table_name_data;
		...
		if ( array_key_exists( 'value', $clause ) ) {
			...
			switch ( $field_compare ) {
				...
				case 'LIKE' :
				case 'NOT LIKE' :
					$field_value = '%' . sz_esc_like( $field_value ) . '%';
	 ...
	 * operator and relation between the clauses allows for a shared table join. In the case of SZ_XProfile_Query,
