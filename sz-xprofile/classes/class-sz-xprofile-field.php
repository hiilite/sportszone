...
class SZ_XProfile_Field {
	 ...
	 *
	 * @since 2.0.0
	 * @var SZ_XProfile_Field_Type Field type object used for validation.
	 */
	 ...
	 *
	 * @since 2.0.0
	 * @var SZ_XProfile_ProfileData Field data for user ID.
	 */
	public $data;
	...
	public function __construct( $id = null, $user_id = null, $get_data = true ) {
		...
		// Initialise the type obj to prevent fatals when creating new profile fields.
			...
			$this->type_obj            = sz_xprofile_create_field_type( 'textbox' );
	...
	public function populate( $id, $user_id = null, $get_data = true ) {
		...
		$field = wp_cache_get( $id, 'sz_xprofile_fields' );
		if ( false === $field ) {
			$sz = sportszone();

			$field = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$sz->profile->table_name_fields} WHERE id = %d", $id ) );

			if ( ! $field ) {
				return false;
			}

			wp_cache_add( $id, $field, 'sz_xprofile_fields' );
		}
	...
	/**
	 * Retrieve a `SZ_XProfile_Field` instance.
	 *
	 * @since 2.4.0
	 ...
	 * @return SZ_XProfile_Field|false Field object if found, otherwise false.
	 */
	 ...
	 * @since 2.4.0
	 *
	 * @param array|object $args Array or object representing the `SZ_XProfile_Field` properties.
	...
	public function fill_data( $args ) {
		...
		$this->type_obj            = sz_xprofile_create_field_type( $this->type );
		$this->type_obj->field_obj = $this;
	}
	...
	public function delete( $delete_data = false ) {
		 ...
		 * @since 3.0.0
		 *
		 * @param SZ_XProfile_Field $this        Current instance of the field being deleted. Passed by reference.
		...
		$sz  = sportszone();
		$sql = $wpdb->prepare( "DELETE FROM {$sz->profile->table_name_fields} WHERE id = %d OR parent_id = %d", $this->id, $this->id );
		...
		sz_xprofile_delete_meta( $this->id, 'field' );

		// Delete the data in the DB for this field.
		if ( true === $delete_data ) {
			SZ_XProfile_ProfileData::delete_for_field( $this->id );
		}
	...
	public function save() {
		 ...
		 * @since 1.0.0
		 *
		 * @param SZ_XProfile_Field $this Current instance of the field being saved.
		 */
		...
		if ( ! $is_new_field ) {
			$sql = $wpdb->prepare( "UPDATE {$sz->profile->table_name_fields} SET group_id = %d, parent_id = 0, type = %s, name = %s, description = %s, is_required = %d, order_by = %s, field_order = %d, option_order = %d, can_delete = %d, is_default_option = %d WHERE id = %d", $this->group_id, $this->type, $this->name, $this->description, $this->is_required, $this->order_by, $this->field_order, $this->option_order, $this->can_delete, $this->is_default_option, $this->id );
		} else {
			$sql = $wpdb->prepare( "INSERT INTO {$sz->profile->table_name_fields} (group_id, parent_id, type, name, description, is_required, order_by, field_order, option_order, can_delete, is_default_option ) VALUES ( %d, %d, %s, %s, %s, %d, %s, %d, %d, %d, %d )", $this->group_id, $this->parent_id, $this->type, $this->name, $this->description, $this->is_required, $this->order_by, $this->field_order, $this->option_order, $this->can_delete, $this->is_default_option );
		}
		...
		if ( $wpdb->query( $sql ) !== null ) {
			...
			if ( $this->type_obj->supports_options ) {
				 ...
				 * @param SZ_XProfile_Field $type        Current field type being saved for.
				 */
				...
				if ( !empty( $options ) ) {
					foreach ( (array) $options as $option_key => $option_value ) {
						...
						if ( '' != $option_value ) {
							$sql = $wpdb->prepare( "INSERT INTO {$sz->profile->table_name_fields} (group_id, parent_id, type, name, description, is_required, option_order, is_default_option) VALUES (%d, %d, 'option', %s, '', 0, %d, %d)", $this->group_id, $parent_id, $option_value, $counter, $is_default );
							if ( ! $wpdb->query( $sql ) ) {
								return false;
							}
						}
	 ...
	 * @return SZ_XProfile_ProfileData
	 */
	public function get_field_data( $user_id = 0 ) {
		return new SZ_XProfile_ProfileData( $this->id, $user_id );
	}
	...
	public function get_children( $for_editing = false ) {
		...
		$sz  = sportszone();
		$sql = $wpdb->prepare( "SELECT * FROM {$sz->profile->table_name_fields} WHERE parent_id = %d AND group_id = %d {$sort_sql}", $parent_id, $this->group_id );
		 ...
		 * @param bool              $for_editing Whether or not the field is for editing.
		 * @param SZ_XProfile_Field $this        Field object
		 */
		return apply_filters( 'sz_xprofile_field_get_children', $children, $for_editing, $this );
	}
	...
	public function delete_children() {
		...
		$sz  = sportszone();
		$sql = $wpdb->prepare( "DELETE FROM {$sz->profile->table_name_fields} WHERE parent_id = %d", $this->id );
	...
	public function get_member_types() {
		...
		$raw_types = sz_xprofile_get_meta( $this->id, 'field', 'member_type', false );
		...
		if ( ! in_array( '_none', $raw_types ) ) {
			$registered_types = sz_get_member_types();
		 ...
		 */
		$this->member_types = apply_filters( 'sz_xprofile_field_member_types', $types, $this );
	...
	public function set_member_types( $member_types, $append = false ) {
		...
		foreach ( $member_types as $member_type ) {
			// 'null' is a special case - it represents users without a type.
			if ( 'null' === $member_type || sz_get_member_type_object( $member_type ) ) {
				$types[] = $member_type;
			}
		}
		...
		if ( ! $append ) {
			sz_xprofile_delete_meta( $this->id, 'field', 'member_type' );
			 ...
			 */
			if ( empty( $types ) ) {
				sz_xprofile_add_meta( $this->id, 'field', 'member_type', '_none' );
			}
		}
		...
		if ( $types !== $rtypes ) {
			...
			foreach ( $types as $type ) {
				sz_xprofile_add_meta( $this->id, 'field', 'member_type', $type );
			}
		}
		 ...
		 * @param SZ_XProfile_Field $this Field object.
		 */
		do_action( 'sz_xprofile_field_set_member_type', $this );
		...
		if ( ! empty( $member_types ) ) {
			...
			foreach ( $member_types as $member_type ) {
				if ( 'null' === $member_type ) {
					...
					$mt_obj = sz_get_member_type_object( $member_type );
			...
			if ( $has_null ) {
				$member_type_labels[] = __( 'Users with no member type', 'sportszone' );
			}

			$label = sprintf( __( '(Member types: %s)', 'sportszone' ), implode( ', ', array_map( 'esc_html', $member_type_labels ) ) );
		} else {
			$label = '<span class="member-type-none-notice">' . __( '(Unavailable to all members)', 'sportszone' ) . '</span>';
		}
	...
	public function get_default_visibility() {
		if ( ! isset( $this->default_visibility ) ) {
			$this->default_visibility = sz_xprofile_get_meta( $this->id, 'field', 'default_visibility' );
	...
	public function get_allow_custom_visibility() {
		if ( ! isset( $this->allow_custom_visibility ) ) {
			$allow_custom_visibility = sz_xprofile_get_meta( $this->id, 'field', 'allow_custom_visibility' );
	...
	public function get_do_autolink() {
		if ( ! isset( $this->do_autolink ) ) {
			$do_autolink = sz_xprofile_get_meta( $this->id, 'field', 'do_autolink' );
	...
	public static function get_type( $field_id = 0 ) {
		...
		$sz   = sportszone();
		$sql  = $wpdb->prepare( "SELECT type FROM {$sz->profile->table_name_fields} WHERE id = %d", $field_id );
	...
	public static function delete_for_group( $group_id = 0 ) {
		...
		$sz      = sportszone();
		$sql     = $wpdb->prepare( "DELETE FROM {$sz->profile->table_name_fields} WHERE group_id = %d", $group_id );
	...
	public static function get_id_from_name( $field_name = '' ) {
		...
		if ( empty( $sz->profile->table_name_fields ) || empty( $field_name ) ) {
			return false;
		}

		$id = sz_core_get_incremented_cache( $field_name, 'sz_xprofile_fields_by_name' );
		if ( false === $id ) {
			$sql = $wpdb->prepare( "SELECT id FROM {$sz->profile->table_name_fields} WHERE name = %s AND parent_id = 0", $field_name );
			$id = $wpdb->get_var( $sql );
			sz_core_set_incremented_cache( $field_name, 'sz_xprofile_fields_by_name', $id );
		}
	...
	public static function update_position( $field_id, $position = null, $field_group_id = null ) {
		...
		$table_name = sportszone()->profile->table_name_fields;
		...
		if ( ! empty( $parent ) && ! is_wp_error( $parent ) ) {
			...
			wp_cache_delete( $field_id, 'sz_xprofile_fields' );
	...
	public static function get_fields_for_member_type( $member_types ) {
		...
		$mt_meta = wp_cache_get( 'field_member_types', 'sz_xprofile' );
		if ( false === $mt_meta ) {
			$mt_meta = $wpdb->get_results( "SELECT object_id, meta_value FROM {$sz->profile->table_name_meta} WHERE meta_key = 'member_type' AND object_type = 'field'" );
			wp_cache_set( 'field_member_types', $mt_meta, 'sz_xprofile' );
		}
		...
		if ( ! in_array( '_none', $member_types ) ) {
			if ( ! empty( $all_recorded_field_ids ) ) {
				...
				$unrestricted_field_ids = $wpdb->get_col( "SELECT id FROM {$sz->profile->table_name_fields} WHERE id NOT IN ({$all_recorded_field_ids_sql})" );
			} else {
				$unrestricted_field_ids = $wpdb->get_col( "SELECT id FROM {$sz->profile->table_name_fields}" );
			}

			// Append the 'null' pseudo-type.
			$all_member_types   = sz_get_member_types();
	...
	public static function admin_validate() {
		...
		if ( ! isset( $_POST['title'] ) || ( '' === $_POST['title'] ) ) {
			$message = esc_html__( 'Profile fields must have a name.', 'sportszone' );
			return false;
		}

		// Check field requirement.
		if ( ! isset( $_POST['required'] ) ) {
			$message = esc_html__( 'Profile field requirement is missing.', 'sportszone' );
			return false;
		}

		// Check field type.
		if ( empty( $_POST['fieldtype'] ) ) {
			$message = esc_html__( 'Profile field type is missing.', 'sportszone' );
			return false;
		}

		// Check that field is of valid type.
		if ( ! in_array( $_POST['fieldtype'], array_keys( sz_xprofile_get_field_types() ), true ) ) {
			$message = sprintf( esc_html__( 'The profile field type %s is not registered.', 'sportszone' ), '<code>' . esc_attr( $_POST['fieldtype'] ) . '</code>' );
			return false;
		}

		// Get field type so we can check for and lavidate any field options.
		$field_type = sz_xprofile_create_field_type( $_POST['fieldtype'] );
		...
		if ( true === $field_type->supports_options ) {
			...
			if ( empty( $_POST[ $option_name ] ) || ! is_array( $_POST[ $option_name ] ) ) {
				$message = esc_html__( 'These field options are invalid.', 'sportszone' );
				return false;
			}
			...
			// Check for missing or malformed options.
			if ( 0 === $field_count ) {
				$message = sprintf( esc_html__( '%s require at least one option.', 'sportszone' ), $field_type->name );
				return false;
			}
	...
	public function render_admin_form_children() {
		foreach ( array_keys( sz_xprofile_get_field_types() ) as $field_type ) {
			$type_obj = sz_xprofile_create_field_type( $field_type );
	...
	public function render_admin_form( $message = '' ) {
		...
		// Users Admin URL
		$users_url = sz_get_admin_url( 'users.php' );

		// Add New
		if ( empty( $this->id ) ) {
			$title  = __( 'Add New Field', 'sportszone' );
			$button	= __( 'Save',          'sportszone' );
			$action = add_query_arg( array(
				'page'     => 'sz-profile-setup',
				'mode'     => 'add_field',
		...
		// Edit
			...
			$title  = __( 'Edit Field', 'sportszone' );
			$button	= __( 'Update',     'sportszone' );
			$action = add_query_arg( array(
				'page'     => 'sz-profile-setup',
				'mode'     => 'edit_field',
				'group_id' => (int) $this->group_id,
		...
		<div class="wrap">
			...
			<form id="sz-xprofile-add-field" action="<?php echo esc_url( $action ); ?>" method="post">
				<div id="poststuff">
					<div id="post-body" class="metabox-holder columns-<?php echo ( 1 == get_current_screen()->get_columns() ) ? '1' : '2'; ?>">
						...
						<div id="postbox-container-1" class="postbox-container">
							 ...
							 * @since 2.2.0
							 *
							 * @param SZ_XProfile_Field $this Current XProfile field.
							 */
	...
	private function submit_metabox( $button_text = '' ) {
		...
		<div id="submitdiv" class="postbox">
			<h2><?php esc_html_e( 'Submit', 'sportszone' ); ?></h2>
			<div class="inside">
				<div id="submitcomment" class="submitbox">
					<div id="major-publishing-actions">
						...
						<div id="delete-action">
							<a href="<?php echo esc_url( $cancel_url ); ?>" class="deletion"><?php esc_html_e( 'Cancel', 'sportszone' ); ?></a>
						</div>
	...
	?>
		...
		<div id="titlediv">
			<div class="titlewrap">
				<label id="title-prompt-text" for="title"><?php echo esc_html_x( 'Name (required)', 'XProfile admin edit field', 'sportszone' ); ?></label>
				<input type="text" name="title" id="title" value="<?php echo esc_attr( $this->name ); ?>" autocomplete="off" />
			</div>
		</div>

		<div class="postbox">
			<h2><?php echo esc_html_x( 'Description', 'XProfile admin edit field', 'sportszone' ); ?></h2>
			<div class="inside">
				<label for="description" class="screen-reader-text"><?php
					/* translators: accessibility text */
					esc_html_e( 'Add description', 'sportszone' );
				?></label>
	...
	private function member_type_metabox() {
		...
		if ( ! $member_types = sz_get_member_types( array(), 'objects' ) ) {
			return;
		}
		...
		<div id="member-types-div" class="postbox">
			<h2><?php _e( 'Member Types', 'sportszone' ); ?></h2>
			<div class="inside">
				<p class="description"><?php _e( 'This field should be available to:', 'sportszone' ); ?></p>
				...
				<ul>
					...
					<li>
						<label for="member-type-none">
							...
							<?php _e( 'Users with no member type', 'sportszone' ); ?>
						</label>
					</li>

				</ul>
				<p class="description member-type-none-notice<?php if ( ! empty( $field_member_types ) ) : ?> hide<?php endif; ?>"><?php _e( 'Unavailable to all members.', 'sportszone' ) ?></p>
			</div>
	...
	private function visibility_metabox() {
		...
		<div class="postbox">
			<h2><label for="default-visibility"><?php esc_html_e( 'Visibility', 'sportszone' ); ?></label></h2>
			<div class="inside">
				<div>
				...
				<div>
					<ul>
						<li>
							...
							<label for="allow-custom-visibility-allowed"><?php esc_html_e( 'Allow members to override', 'sportszone' ); ?></label>
						</li>
						<li>
							...
							<label for="allow-custom-visibility-disabled"><?php esc_html_e( 'Enforce field visibility', 'sportszone' ); ?></label>
						</li>
					</ul>
				</div>
			</div>
	...
	private function required_metabox() {
		...
		<div class="postbox">
			<h2><label for="required"><?php esc_html_e( 'Requirement', 'sportszone' ); ?></label></h2>
			<div class="inside">
				<select name="required" id="required">
					<option value="0"<?php selected( $this->is_required, '0' ); ?>><?php esc_html_e( 'Not Required', 'sportszone' ); ?></option>
					<option value="1"<?php selected( $this->is_required, '1' ); ?>><?php esc_html_e( 'Required',     'sportszone' ); ?></option>
				</select>
			</div>
		</div>
	...
	private function autolink_metabox() {
		...
		<div class="postbox">
			<h2><?php esc_html_e( 'Autolink', 'sportszone' ); ?></h2>
			<div class="inside">
				<p class="description"><?php esc_html_e( 'On user profiles, link this field to a search of the Members directory, using the field value as a search term.', 'sportszone' ); ?></p>

				<p>
					<label for="do-autolink" class="screen-reader-text"><?php
						/* translators: accessibility text */
						esc_html_e( 'Autolink status for this field', 'sportszone' );
					?></label>
					<select name="do_autolink" id="do-autolink">
						<option value="on" <?php selected( $this->get_do_autolink() ); ?>><?php esc_html_e( 'Enabled', 'sportszone' ); ?></option>
						<option value="" <?php selected( $this->get_do_autolink(), false ); ?>><?php esc_html_e( 'Disabled', 'sportszone' ); ?></option>
					</select>
				</p>
			</div>
		</div>
	...
	private function type_metabox() {
		...
		<div class="postbox">
			<h2><label for="fieldtype"><?php esc_html_e( 'Type', 'sportszone'); ?></label></h2>
			<div class="inside" aria-live="polite" aria-atomic="true" aria-relevant="all">
				<select name="fieldtype" id="fieldtype" onchange="show_options(this.value)">
					...
					<?php sz_xprofile_admin_form_field_types( $this->type ); ?>

				</select>

				<?php

				// Deprecated filter, don't use. Go look at {@link SZ_XProfile_Field_Type::admin_new_field_html()}.
	...
	private function default_field_hidden_inputs() {
		...
		// Nonce
		wp_nonce_field( 'sz_xprofile_admin_field', 'sz_xprofile_admin_field' );
