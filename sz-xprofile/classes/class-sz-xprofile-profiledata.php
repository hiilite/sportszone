...
class SZ_XProfile_ProfileData {
	...
	/**
	 * SZ_XProfile_ProfileData constructor.
	 *
	 * @since 1.5.0
	 *
	...
	public function populate( $field_id, $user_id ) {
		...
		$profiledata = wp_cache_get( $cache_key, 'sz_xprofile_data' );

		if ( false === $profiledata ) {
			$sz = sportszone();

			$sql         = $wpdb->prepare( "SELECT * FROM {$sz->profile->table_name_data} WHERE field_id = %d AND user_id = %d", $field_id, $user_id );
			$profiledata = $wpdb->get_row( $sql );

			if ( $profiledata ) {
				wp_cache_set( $cache_key, $profiledata, 'sz_xprofile_data' );
			}
		}
	...
	public function exists() {
		...
		$cached    = wp_cache_get( $cache_key, 'sz_xprofile_data' );

		if ( $cached && ! empty( $cached->id ) ) {
			$retval = true;
		} else {
			$sz = sportszone();
			$retval = $wpdb->get_row( $wpdb->prepare( "SELECT id FROM {$sz->profile->table_name_data} WHERE user_id = %d AND field_id = %d", $this->user_id, $this->field_id ) );
		}
		 ...
		 * @param SZ_XProfile_ProfileData $this   Instance of the current SZ_XProfile_ProfileData class.
		 */
	...
	public function is_valid_field() {
		...
		$retval = $wpdb->get_row( $wpdb->prepare( "SELECT id FROM {$sz->profile->table_name_fields} WHERE id = %d", $this->field_id ) );
	...
	public function save() {
		 ...
		 * @param SZ_XProfile_ProfileData $this        Current instance of the profile data being saved.
		 */
		 ...
		 * @param int $data_id      The field data ID.
		 */
		$this->last_updated = apply_filters( 'xprofile_data_last_updated_before_save', sz_core_current_time(), $this->id );
		...
		if ( $this->is_valid_field() ) {
			if ( $this->exists() && strlen( trim( $this->value ) ) ) {
				$result   = $wpdb->query( $wpdb->prepare( "UPDATE {$sz->profile->table_name_data} SET value = %s, last_updated = %s WHERE user_id = %d AND field_id = %d", $this->value, $this->last_updated, $this->user_id, $this->field_id ) );
				...
				$result   = $wpdb->query( $wpdb->prepare("INSERT INTO {$sz->profile->table_name_data} (user_id, field_id, value, last_updated) VALUES (%d, %d, %s, %s)", $this->user_id, $this->field_id, $this->value, $this->last_updated ) );
	...
	public function delete() {
		 ...
		 * @since 1.9.0
		 *
		 * @param SZ_XProfile_ProfileData $this Current instance of the profile data being deleted.
		 */
		do_action_ref_array( 'xprofile_data_before_delete', array( $this ) );

		$deleted = $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->profile->table_name_data} WHERE field_id = %d AND user_id = %d", $this->field_id, $this->user_id ) );
		if ( empty( $deleted ) ) {
			return false;
		}
	...
	public static function get_data_for_user( $user_id, $field_ids ) {
		...
		$uncached_field_ids = sz_xprofile_get_non_cached_field_ids( $user_id, $field_ids );

		// Prime the cache.
		if ( ! empty( $uncached_field_ids ) ) {
			$sz = sportszone();
			$uncached_field_ids_sql = implode( ',', wp_parse_id_list( $uncached_field_ids ) );
			$uncached_data = $wpdb->get_results( $wpdb->prepare( "SELECT id, user_id, field_id, value, last_updated FROM {$sz->profile->table_name_data} WHERE field_id IN ({$uncached_field_ids_sql}) AND user_id = %d", $user_id ) );
			...
			foreach ( $uncached_field_ids as $field_id ) {
				...
				if ( isset( $queried_data[ $field_id ] ) ) {
					wp_cache_set( $cache_key, $queried_data[ $field_id ], 'sz_xprofile_data' );
				...
				// to avoid future cache misses.
					...
					wp_cache_set( $cache_key, $d, 'sz_xprofile_data' );
				}
			}
		}
		...
		foreach ( $field_ids as $field_id ) {
			$cache_key = "{$user_id}:{$field_id}";
			$data[]    = wp_cache_get( $cache_key, 'sz_xprofile_data' );
		}
	...
	public static function get_fielddataid_byid( $field_id, $user_id ) {
		...
		if ( empty( $field_id ) || empty( $user_id ) ) {
			...
			// Check cache first.
			$cache_key = "{$user_id}:{$field_id}";
			$fielddata = wp_cache_get( $cache_key, 'sz_xprofile_data' );
			if ( false === $fielddata || empty( $fielddata->id ) ) {
				$fielddata_id = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$sz->profile->table_name_data} WHERE field_id = %d AND user_id = %d", $field_id, $user_id ) );
			} else {
	...
	public static function get_value_byid( $field_id, $user_ids = null ) {
		...
		foreach ( $user_ids as $user_id ) {
			$cache_key = "{$user_id}:{$field_id}";
			if ( false === wp_cache_get( $cache_key, 'sz_xprofile_data' ) ) {
		...
		if ( ! empty( $uncached_ids ) ) {
			...
			$queried_data = $wpdb->get_results( $wpdb->prepare( "SELECT id, user_id, field_id, value, last_updated FROM {$sz->profile->table_name_data} WHERE field_id = %d AND user_id IN ({$uncached_ids_sql})", $field_id ) );
	...
	public static function get_value_byfieldname( $fields, $user_id = null ) {
		...
		$sql    = $wpdb->prepare( "SELECT d.value, f.name FROM {$sz->profile->table_name_data} d, {$sz->profile->table_name_fields} f WHERE d.field_id = f.id AND d.user_id = %d AND f.parent_id = 0 $field_sql", $user_id );
	...
	public static function delete_for_field( $field_id ) {
		...
		$sz      = sportszone();
		$deleted = $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->profile->table_name_data} WHERE field_id = %d", $field_id ) );
	...
	public static function get_last_updated( $user_id ) {
		...
		$last_updated = $wpdb->get_var( $wpdb->prepare( "SELECT last_updated FROM {$sz->profile->table_name_data} WHERE user_id = %d ORDER BY last_updated LIMIT 1", $user_id ) );
	...
	public static function delete_data_for_user( $user_id ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$sz->profile->table_name_data} WHERE user_id = %d", $user_id ) );
	}
	...
	public static function get_random( $user_id, $exclude_fullname ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT pf.type, pf.name, pd.value FROM {$sz->profile->table_name_data} pd INNER JOIN {$sz->profile->table_name_fields} pf ON pd.field_id = pf.id AND pd.user_id = %d {$exclude_sql} ORDER BY RAND() LIMIT 1", $user_id ) );
	}
	...
	public static function get_fullname( $user_id = 0 ) {
		...
		return xprofile_get_field_data( sz_xprofile_fullname_field_id(), $user_id );
	}
}
