 ...
 * @since 2.0.0
 */
class SZ_XProfile_Field_Type_Selectbox extends SZ_XProfile_Field_Type {
	...
	public function __construct() {
		...
		$this->name     = _x( 'Drop Down Select Box', 'xprofile field type', 'sportszone' );
		...
		/**
		 * Fires inside __construct() method for SZ_XProfile_Field_Type_Selectbox class.
		 *
		 * @since 2.0.0
		 *
		 * @param SZ_XProfile_Field_Type_Selectbox $this Current instance of
		 *                                               the field type select box.
		 */
		do_action( 'sz_xprofile_field_type_selectbox', $this );
	}
	...
	public function edit_field_html( array $raw_properties = array() ) {
		...
		<select <?php echo $this->get_edit_field_html_elements( $raw_properties ); ?> aria-labelledby="<?php sz_the_profile_field_input_name(); ?>-1" aria-describedby="<?php sz_the_profile_field_input_name(); ?>-3">
	...
	public function edit_field_options_html( array $args = array() ) {
		$original_option_values = maybe_unserialize( SZ_XProfile_ProfileData::get_value_byid( $this->field_obj->id, $args['user_id'] ) );

		$options = $this->field_obj->get_children();
		$html    = '<option value="">' . /* translators: no option picked in select box */ esc_html__( '----', 'sportszone' ) . '</option>';
		...
		for ( $k = 0, $count = count( $options ); $k < $count; ++$k ) {
			 ...
			 */
			$html .= apply_filters( 'sz_get_the_profile_field_options_select', '<option' . $selected . ' value="' . esc_attr( stripslashes( $options[$k]->name ) ) . '">' . esc_html( stripslashes( $options[$k]->name ) ) . '</option>', $options[$k], $this->field_obj->id, $selected, $k );
		}
