<?php
/**
 * SportsZone XProfile CSS and JS.
 *
 * @package SportsZone
...
function xprofile_add_admin_css() {
	if ( !empty( $_GET['page'] ) && strpos( $_GET['page'], 'sz-profile-setup' ) !== false ) {
		$min = sz_core_get_minified_asset_suffix();

		wp_enqueue_style( 'xprofile-admin-css', sportszone()->plugin_url . "sz-xprofile/admin/css/admin{$min}.css", array(), sz_get_version() );
...
add_action( 'sz_admin_enqueue_scripts', 'xprofile_add_admin_css' );
...
function xprofile_add_admin_js() {
	if ( !empty( $_GET['page'] ) && strpos( $_GET['page'], 'sz-profile-setup' ) !== false ) {
		...
		$min = sz_core_get_minified_asset_suffix();
		wp_enqueue_script( 'xprofile-admin-js', sportszone()->plugin_url . "sz-xprofile/admin/js/admin{$min}.js", array( 'jquery', 'jquery-ui-sortable' ), sz_get_version() );
		...
		foreach ( sz_xprofile_get_field_types() as $field_type => $field_type_class ) {
		...
		if ( ! empty( $_GET['field_id'] ) ) {
			...
			$strings['do_autolink'] = sz_xprofile_get_meta( $field_id, 'field', 'do_autolink' );
		}

		wp_localize_script( 'xprofile-admin-js', 'XProfileAdmin', $strings );
	}
}
add_action( 'sz_admin_enqueue_scripts', 'xprofile_add_admin_js', 1 );
