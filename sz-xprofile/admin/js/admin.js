...
function add_option(forWhat) {
	...
	newDiv.setAttribute('class', 'sz-option sortable');

	grabber.setAttribute( 'class', 'sz-option-icon grabber');
...
function show_options( forWhat ) {
	...
	jQuery( document ).trigger( 'sz-xprofile-show-options', forWhat );
}
...
function enableSortableFieldOptions() {
	jQuery( '.sz-options-box' ).sortable( {
		cursor: 'move',
		items: 'div.sortable',
		tolerance: 'intersect',
		axis: 'y'
...
function destroySortableFieldOptions() {
	jQuery( '.sz-options-box' ).sortable( 'destroy' );
...
jQuery( document ).ready( function() {
	...
	jQuery( '#sz-xprofile-add-field #title' ).focus();
