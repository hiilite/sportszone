...
function xprofile_screen_edit_profile() {
	...
	if ( ! sz_action_variable( 1 ) ) {
		sz_core_redirect( trailingslashit( sz_displayed_user_domain() . sz_get_profile_slug() . '/edit/group/1' ) );
	}

	// Check the field group exists.
	if ( ! sz_is_action_variable( 'group' ) || ! xprofile_get_field_group( sz_action_variable( 1 ) ) ) {
		sz_do_404();
		return;
	}
	...
	if ( isset( $_POST['field_ids'] ) ) {
		...
		// Check the nonce.
		check_admin_referer( 'sz_xprofile_edit' );

		// Check we have field ID's.
		if ( empty( $_POST['field_ids'] ) ) {
			sz_core_redirect( trailingslashit( sz_displayed_user_domain() . sz_get_profile_slug() . '/edit/group/' . sz_action_variable( 1 ) ) );
		}
		...
		// There are errors.
		if ( !empty( $errors ) ) {
			sz_core_add_message( __( 'Your changes have not been saved. Please fill in all required fields, and save your changes again.', 'sportszone' ), 'error' );
		...
		// No errors.
			...
			foreach ( (array) $posted_field_ids as $field_id ) {
				...
				$old_values[ $field_id ] = array(
					'value'      => xprofile_get_field_data( $field_id, sz_displayed_user_id() ),
					'visibility' => xprofile_get_field_visibility_level( $field_id, sz_displayed_user_id() ),
				);
				...
				$field_updated = xprofile_set_field_data( $field_id, sz_displayed_user_id(), $value, $is_required[ $field_id ] );
				$value         = xprofile_get_field_data( $field_id, sz_displayed_user_id() );
			 ...
			 */
			do_action( 'xprofile_updated_profile', sz_displayed_user_id(), $posted_field_ids, $errors, $old_values, $new_values );

			// Set the feedback messages.
			if ( !empty( $errors ) ) {
				sz_core_add_message( __( 'There was a problem updating some of your profile information. Please try again.', 'sportszone' ), 'error' );
			} else {
				sz_core_add_message( __( 'Changes saved.', 'sportszone' ) );
			}
	 ...
	 */
	sz_core_load_template( apply_filters( 'xprofile_template_edit_profile', 'members/single/home' ) );
}