...
function xprofile_add_admin_menu() {
	...
	add_users_page( _x( 'Profile Fields', 'xProfile admin page title', 'sportszone' ), _x( 'Profile Fields', 'Admin Users menu', 'sportszone' ), 'manage_options', 'sz-profile-setup', 'xprofile_admin' );
}
add_action( sz_core_admin_hook(), 'xprofile_add_admin_menu' );
...
function xprofile_admin_screen( $message = '', $type = 'error' ) {
	...
	$groups = sz_xprofile_get_groups( array(
		'fetch_fields' => true
	) ); ?>
	...
	<div class="wrap">
		<?php if ( version_compare( $GLOBALS['wp_version'], '4.8', '>=' ) ) : ?>
			...
			<h1 class="wp-heading-inline"><?php _ex( 'Profile Fields', 'Settings page header', 'sportszone'); ?></h1>

				<a id="add_group" class="page-title-action" href="<?php echo esc_url( $add_group_url ); ?>"><?php _e( 'Add New Field Group', 'sportszone' ); ?></a>
		...
		<?php else : ?>
			...
			<h1>
				<?php _ex( 'Profile Fields', 'Settings page header', 'sportszone'); ?>
				<a id="add_group" class="add-new-h2" href="<?php echo esc_url( $add_group_url ); ?>"><?php _e( 'Add New Field Group', 'sportszone' ); ?></a>
			</h1>
		...
		<form action="" id="profile-field-form" method="post">
			...
			wp_nonce_field( 'sz_reorder_fields', '_wpnonce_reorder_fields'        );
			wp_nonce_field( 'sz_reorder_groups', '_wpnonce_reorder_groups', false );
			...
			<div id="tabs" aria-live="polite" aria-atomic="true" aria-relevant="all">
					...
					<?php if ( !empty( $groups ) ) : foreach ( $groups as $group ) : ?>
						...
						<li id="group_<?php echo esc_attr( $group->id ); ?>">
							<a href="#tabs-<?php echo esc_attr( $group->id ); ?>" class="ui-tab">
								<?php
								/** This filter is documented in sz-xprofile/sz-xprofile-template.php */
								echo esc_html( apply_filters( 'sz_get_the_profile_group_name', $group->name ) );
								?>

								<?php if ( !$group->can_delete ) : ?>
									<?php _e( '(Primary)', 'sportszone'); ?>
								<?php endif; ?>
					...
					<div id="tabs-<?php echo esc_attr( $group->id ); ?>" class="tab-wrapper">
						<div class="tab-toolbar">
							<div class="tab-toolbar-left">
								<a class="button-primary" href="<?php echo esc_url( $add_field_url ); ?>"><?php _e( 'Add New Field', 'sportszone' ); ?></a>
								<a class="button edit" href="<?php echo esc_url( $edit_group_url ); ?>"><?php _ex( 'Edit Group', 'Edit Profile Fields Group', 'sportszone' ); ?></a>

								<?php if ( $group->can_delete ) : ?>

									<div class="delete-button">
										<a class="confirm submitdelete deletion ajax-option-delete" href="<?php echo esc_url( $delete_group_url ); ?>"><?php _ex( 'Delete Group', 'Delete Profile Fields Group', 'sportszone' ); ?></a>
									</div>
								 ...
								 * @since 2.2.0
								 *
								 * @param SZ_XProfile_Group $group SZ_XProfile_Group object
						...
						<?php if ( ! empty( $group->description ) ) : ?>
							...
							<p><?php
							/** This filter is documented in sz-xprofile/sz-xprofile-template.php */
							echo esc_html( apply_filters( 'sz_get_the_profile_group_description', $group->description ) );
							?></p>
						...
						<fieldset id="<?php echo esc_attr( $group->id ); ?>" class="connectedSortable field-group" aria-live="polite" aria-atomic="true" aria-relevant="all">
							...
							printf( esc_html__( 'Fields for "%s" Group', 'sportszone' ), apply_filters( 'sz_get_the_profile_group_name', $group->name ) );
							?></legend>
							...
							if ( !empty( $group->fields ) ) :
								...
								<p class="nodrag nofields"><?php _e( 'There are no fields in this group.', 'sportszone' ); ?></p>
						...
						<?php if ( empty( $group->can_delete ) ) : ?>
							...
							<p><?php esc_html_e( '* Fields in this group appear on the signup page.', 'sportszone' ); ?></p>
				...
				<?php endforeach; else : ?>
					...
					<div id="message" class="error"><p><?php _ex( 'You have no groups.', 'You have no profile fields groups.', 'sportszone' ); ?></p></div>
					<p><a href="<?php echo esc_url( $add_group_url ); ?>"><?php _ex( 'Add New Group', 'Add New Profile Fields Group', 'sportszone' ); ?></a></p>
...
function xprofile_admin_manage_group( $group_id = null ) {
	...
	if ( isset( $_POST['save_group'] ) ) {
		...
		// Check nonce
		check_admin_referer( 'sz_xprofile_admin_group', 'sz_xprofile_admin_group' );

		// Validate $_POSTed data.
		if ( SZ_XProfile_Group::admin_validate() ) {
			...
			if ( false === $group->save() ) {
				$message = __( 'There was an error saving the group. Please try again.', 'sportszone' );
				$type    = 'error';

			// Save successful.
			} else {
				$message = __( 'The group was saved successfully.', 'sportszone' );
				$type    = 'success';

				// @todo remove these old options
				if ( 1 == $group_id ) {
					sz_update_option( 'sz-xprofile-base-group-name', $group->name );
				}
				 ...
				 * @since 1.0.0
				 *
				 * @param SZ_XProfile_Group $group Current SZ_XProfile_Group object.
				 */
...
function xprofile_admin_delete_group( $group_id ) {
	...
	check_admin_referer( 'sz_xprofile_delete_group' );

	$group = new SZ_XProfile_Group( $group_id );

	if ( !$group->delete() ) {
		$message = _x( 'There was an error deleting the group. Please try again.', 'Error when deleting profile fields group', 'sportszone' );
		$type    = 'error';
	} else {
		$message = _x( 'The group was deleted successfully.', 'Profile fields group was deleted successfully', 'sportszone' );
		$type    = 'success';
...
function xprofile_admin_manage_field( $group_id, $field_id = null ) {
	...
	if ( isset( $_POST['saveField'] ) ) {
		...
		// Check nonce
		check_admin_referer( 'sz_xprofile_admin_field', 'sz_xprofile_admin_field' );

		if ( SZ_XProfile_Field::admin_validate() ) {
			...
			$field->field_order = $wpdb->get_var( $wpdb->prepare( "SELECT field_order FROM {$sz->profile->table_name_fields} WHERE id = %d", $field_id ) );
			if ( ! is_numeric( $field->field_order ) || is_wp_error( $field->field_order ) ) {
				$field->field_order = (int) $wpdb->get_var( $wpdb->prepare( "SELECT max(field_order) FROM {$sz->profile->table_name_fields} WHERE group_id = %d", $group_id ) );
				$field->field_order++;
			}
			...
			if ( empty( $field_id ) ) {
				$message = __( 'There was an error saving the field. Please try again.', 'sportszone' );
				$type    = 'error';
			} else {
				$message = __( 'The field was saved successfully.', 'sportszone' );
				$type    = 'success';

				// @todo remove these old options
				if ( 1 == $field_id ) {
					sz_update_option( 'sz-xprofile-fullname-field-name', $field->name );
				}
				...
				if ( ! empty( $_POST['default-visibility'] ) && in_array( $_POST['default-visibility'], wp_list_pluck( sz_xprofile_get_visibility_levels(), 'id' ) ) ) {
					sz_xprofile_update_field_meta( $field_id, 'default_visibility', $_POST['default-visibility'] );
				}
				...
				if ( ! empty( $_POST['allow-custom-visibility'] ) && in_array( $_POST['allow-custom-visibility'], array( 'allowed', 'disabled' ) ) ) {
					sz_xprofile_update_field_meta( $field_id, 'allow_custom_visibility', $_POST['allow-custom-visibility'] );
				}

				// Validate signup.
				if ( ! empty( $_POST['signup-position'] ) ) {
					sz_xprofile_update_field_meta( $field_id, 'signup_position', (int) $_POST['signup-position'] );
				} else {
					sz_xprofile_delete_meta( $field_id, 'field', 'signup_position' );
				}

				// Save autolink settings.
				if ( isset( $_POST['do_autolink'] ) && 'on' === wp_unslash( $_POST['do_autolink'] ) ) {
					sz_xprofile_update_field_meta( $field_id, 'do_autolink', 'on' );
				} else {
					sz_xprofile_update_field_meta( $field_id, 'do_autolink', 'off' );
				}
				 ...
				 * @since 1.0.0
				 *
				 * @param SZ_XProfile_Field $field Current SZ_XProfile_Field object.
				 */
				do_action( 'xprofile_fields_saved_field', $field );

				$groups = sz_xprofile_get_groups();
			}
...
function xprofile_admin_delete_field( $field_id, $field_type = 'field', $delete_data = false ) {
	...
	$field_type  = ( 'field' == $field_type ) ? __( 'field', 'sportszone' ) : __( 'option', 'sportszone' );
	$field       = xprofile_get_field( $field_id );

	if ( !$field->delete( (bool) $delete_data ) ) {
		$message = sprintf( __( 'There was an error deleting the %s. Please try again.', 'sportszone' ), $field_type );
		$type    = 'error';
	} else {
		$message = sprintf( __( 'The %s was deleted successfully!', 'sportszone' ), $field_type );
		$type    = 'success';
...
function xprofile_ajax_reorder_fields() {
	...
	// Check the nonce.
	check_admin_referer( 'sz_reorder_fields', '_wpnonce_reorder_fields' );
...
function xprofile_ajax_reorder_field_groups() {
	...
	// Check the nonce.
	check_admin_referer( 'sz_reorder_groups', '_wpnonce_reorder_groups' );
 ...
 * @since 1.5.0
 *
 * @param SZ_XProfile_Field   $admin_field Admin field.
	...
	<fieldset id="draggable_field_<?php echo esc_attr( $field->id ); ?>" class="sortable<?php echo ' ' . $field->type; if ( !empty( $class ) ) echo ' ' . $class; ?>">
		<legend>
			<span>
				...
				<?php if ( empty( $field->can_delete )                                    ) : ?><?php esc_html_e( '(Primary)', 'sportszone' ); endif; ?>
				<?php sz_the_profile_field_required_label(); ?>
				<?php if ( sz_xprofile_get_meta( $field->id, 'field', 'signup_position' ) ) : ?><?php esc_html_e( '(Sign-up)', 'sportszone' ); endif; ?>
				<?php if ( sz_get_member_types() ) : echo $field->get_member_type_label(); endif; ?>
				 ...
				 * @since 2.2.0
				 *
				 * @param SZ_XProfile_Field $field Current SZ_XProfile_Field
		...
		<div class="field-wrapper">
			...
			<?php
			if ( in_array( $field->type, array_keys( sz_xprofile_get_field_types() ) ) ) {
				$field_type = sz_xprofile_create_field_type( $field->type );
			...
			<div class="actions">
				<a class="button edit" href="<?php echo esc_url( $field_edit_url ); ?>"><?php _ex( 'Edit', 'Edit field link', 'sportszone' ); ?></a>

				<?php if ( $field->can_delete ) : ?>

					<div class="delete-button">
						<a class="confirm submit-delete deletion" href="<?php echo esc_url( $field_delete_url ); ?>"><?php _ex( 'Delete', 'Delete field link', 'sportszone' ); ?></a>
					</div>
 ...
 */
function sz_xprofile_admin_form_field_types( $select_field_type ) {
	$categories = array();
	...
	foreach ( sz_xprofile_get_field_types() as $field_name => $field_class ) {
		$field_type_obj = new $field_class;
		$the_category   = $field_type_obj->category;

		// Fallback to a catch-all if category not set.
		if ( ! $the_category ) {
			$the_category = _x( 'Other', 'xprofile field type category', 'sportszone' );
		}
...
add_action( 'sz_init', array( 'SZ_XProfile_User_Admin', 'register_xprofile_user_admin' ), 11 );
