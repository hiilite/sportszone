<?php
/**
 * SportsZone XProfile Caching Functions.
 *
 ...
 * @return array
 */
function sz_xprofile_get_non_cached_field_ids( $user_id = 0, $field_ids = array() ) {
 ...
 *
 * We do not use sz_update_meta_cache() for the xprofile component. This is
 ...
 * in sz_update_meta_cache(), pre-fetching would take three separate database
 * queries. By grouping them together, we can reduce the required queries to
 * one.
 *
 * This function is called within a sz_has_profile() loop.
 *
 * @since 2.0.0
 *
 ...
 * @return bool
 */
function sz_xprofile_update_meta_cache( $object_ids = array() ) {
	global $wpdb;
	...
	foreach ( $uncached_object_ids as $object_type ) {
		...
		$uncached_object_ids[ $object_type ] = sz_get_non_cached_ids( $object_type_ids, $cache_groups[ $object_type ] );
	...
	$meta_list = $wpdb->get_results( "SELECT object_id, object_type, meta_key, meta_value FROM {$sz->profile->table_name_meta} WHERE {$where_sql}" );
...
function xprofile_clear_profile_groups_object_cache( $group_obj ) {
	...
	wp_cache_delete( $group_obj->id, 'sz_xprofile_groups' );
}
 ...
 * @since 2.0.0
 *
 * @param SZ_XProfile_Field $field_obj Field object cache to delete.
 */
function xprofile_clear_profile_field_object_cache( $field_obj ) {

	// Clear default visibility level cache.
	wp_cache_delete( 'default_visibility_levels', 'sz_xprofile' );
	...
	wp_cache_delete( 'all',                'sz_xprofile_groups' );
	wp_cache_delete( $field_obj->group_id, 'sz_xprofile_groups' );
}
 ...
 * @since 2.4.0
 */
function sz_xprofile_clear_member_type_cache() {
	wp_cache_delete( 'field_member_types', 'sz_xprofile' );
}
add_action( 'sz_xprofile_field_set_member_type', 'sz_xprofile_clear_member_type_cache' );
 ...
 * @since 2.0.0
 *
 * @param SZ_XProfile_ProfileData $data_obj Field data object to delete.
 */
function xprofile_clear_profiledata_object_cache( $data_obj ) {
	wp_cache_delete( "{$data_obj->user_id}:{$data_obj->field_id}", 'sz_xprofile_data' );
}
...
/**
 * Clear fullname_field_id cache when sz-xprofile-fullname-field-name is updated.
 *
 * Note for future developers: Dating from an early version of SportsZone where
 * the fullname field (field #1) did not have a title that was editable in the
 * normal Profile Fields admin interface, we have the sz-xprofile-fullname-field-name
 * option. In many places throughout SportsZone, the ID of the fullname field
...
function xprofile_clear_fullname_field_id_cache() {
	wp_cache_delete( 'fullname_field_id', 'sz_xprofile' );
}
add_action( 'update_option_sz-xprofile-fullname-field-name', 'xprofile_clear_fullname_field_id_cache' );
 ...
 * @since 2.4.0
 *
 * @param int|SZ_XProfile_Field $field A field ID or a field object.
 * @return bool False on failure.
 */
function sz_xprofile_clear_field_cache( $field ) {
	if ( is_numeric( $field ) ) {
		$field_id = (int) $field;
	} elseif ( $field instanceof SZ_XProfile_Field ) {
...
add_action( 'xprofile_field_after_save', 'sz_xprofile_clear_field_cache' );
 ...
 * @since 2.8.0
 */
function sz_xprofile_reset_fields_by_name_cache_incrementor() {
	sz_core_reset_incrementor( 'sz_xprofile_fields_by_name' );
}
add_action( 'xprofile_field_before_save', 'sz_xprofile_reset_fields_by_name_cache_incrementor' );

// List actions to clear super cached pages on, if super cache is installed.
add_action( 'xprofile_updated_profile', 'sz_core_clear_cache' );
