<?php
/**
 * SportsZone XProfile Activity & Notification Functions.
 *
...
function xprofile_register_activity_actions() {
	...
	sz_activity_set_action(
		...
		'profile',
		'new_avatar',
		__( 'Member changed profile picture', 'sportszone' ),
		'sz_xprofile_format_activity_action_new_avatar',
		__( 'Updated Profile Photos', 'sportszone' )
	);

	sz_activity_set_action(
		sportszone()->profile->id,
		'updated_profile',
		__( 'Updated Profile', 'sportszone' ),
		'sz_xprofile_format_activity_action_updated_profile',
		__( 'Profile Updates', 'sportszone' ),
		array( 'activity' )
	);
...
add_action( 'sz_register_activity_actions', 'xprofile_register_activity_actions' );
 ...
 * @return string
 */
function sz_xprofile_format_activity_action_new_avatar( $action, $activity ) {
	$userlink = sz_core_get_userlink( $activity->user_id );
	$action   = sprintf( __( '%s changed their profile picture', 'sportszone' ), $userlink );

	// Legacy filter - pass $user_id instead of $activity.
	if ( has_filter( 'sz_xprofile_new_avatar_action' ) ) {
		$action = apply_filters( 'sz_xprofile_new_avatar_action', $action, $activity->user_id );
	}
	 ...
	 * @param object $activity Activity object.
	 */
	return apply_filters( 'sz_xprofile_format_activity_action_new_avatar', $action, $activity );
}
 ...
 * @return string
 */
function sz_xprofile_format_activity_action_updated_profile( $action, $activity ) {
	...
	// natural translation.
	$profile_link = trailingslashit( sz_core_get_user_domain( $activity->user_id ) . sz_get_profile_slug() );
	$action	      = sprintf( __( "%s's profile was updated", 'sportszone' ), '<a href="' . $profile_link . '">' . sz_core_get_user_displayname( $activity->user_id ) . '</a>' );
	 ...
	 * @param object $activity Activity object.
	 */
	return apply_filters( 'sz_xprofile_format_activity_action_updated_profile', $action, $activity );
}
 ...
 * @since 1.0.0
 *
 * @param array|string $args String containing all variables used after sz_parse_args() call.
...
function xprofile_record_activity( $args = '' ) {
	...
	$r = sz_parse_args( $args, array(
		...
		'content'           => '',
		'primary_link'      => '',
		'component'         => sportszone()->profile->id,
		'type'              => false,
		'item_id'           => false,
 ...
 *
 * @param array|string $args Containing all variables used after sz_parse_args() call.
 * @return bool
 */
...
function xprofile_register_activity_action( $key, $value ) {
	...
	/**
	 * Filters the return value of sz_activity_set_action.
	 *
	 * @since 1.1.0
	 *
	 ...
	 */
	return apply_filters( 'xprofile_register_activity_action', sz_activity_set_action( sportszone()->profile->id, $key, $value ), $key, $value );
}
 ...
 * @return bool
 */
function sz_xprofile_new_avatar_activity( $user_id = 0 ) {
	 ...
	 */
	$user_id = apply_filters( 'sz_xprofile_new_avatar_user_id', $user_id );
...
add_action( 'xprofile_avatar_uploaded', 'sz_xprofile_new_avatar_activity' );
 ...
 */
function sz_xprofile_updated_profile_activity( $user_id, $field_ids = array(), $errors = false, $old_values = array(), $new_values = array() ) {
	...
	if ( ! empty( $existing['activities'] ) ) {
		 ...
		 */
		$throttle_period = apply_filters( 'sz_xprofile_updated_profile_activity_throttle_time', HOUR_IN_SECONDS * 2 );
	...
	$profile_link = trailingslashit( sz_core_get_user_domain( $user_id ) . sz_get_profile_slug() );
...
add_action( 'xprofile_updated_profile', 'sz_xprofile_updated_profile_activity', 10, 5 );
...
function xprofile_activity_filter_options() {
	...
	<option value="updated_profile"><?php _e( 'Profile Updates', 'sportszone' ) ?></option>
