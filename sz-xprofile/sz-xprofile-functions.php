 ...
 *
 * Procedural wrapper for SZ_XProfile_Group::get() method.
 *
 * @since 2.1.0
 *
 * @param array $args See {@link SZ_XProfile_Group::get()} for description of arguments.
 * @return array $groups
 */
function sz_xprofile_get_groups( $args = array() ) {

	$groups = SZ_XProfile_Group::get( $args );
	 ...
	 */
	return apply_filters( 'sz_xprofile_get_groups', $groups, $args );
}
...
function xprofile_insert_field_group( $args = '' ) {
	...
	$field_group              = new SZ_XProfile_Group( $r['field_group_id'] );
...
function xprofile_update_field_group_position( $field_group_id = 0, $position = 0 ) {
	return SZ_XProfile_Group::update_position( $field_group_id, $position );
}
 ...
 */
function sz_xprofile_get_field_types() {
	$fields = array(
		'checkbox'       => 'SZ_XProfile_Field_Type_Checkbox',
		'datebox'        => 'SZ_XProfile_Field_Type_Datebox',
		'multiselectbox' => 'SZ_XProfile_Field_Type_Multiselectbox',
		'number'         => 'SZ_XProfile_Field_Type_Number',
		'url'            => 'SZ_XProfile_Field_Type_URL',
		'radio'          => 'SZ_XProfile_Field_Type_Radiobutton',
		'selectbox'      => 'SZ_XProfile_Field_Type_Selectbox',
		'textarea'       => 'SZ_XProfile_Field_Type_Textarea',
		'textbox'        => 'SZ_XProfile_Field_Type_Textbox',
		'telephone'      => 'SZ_XProfile_Field_Type_Telephone',
	);
	 ...
	 */
	return apply_filters( 'sz_xprofile_get_field_types', $fields );
}
 ...
 * @since 2.0.0
 *
 * @param string $type Type of profile field to create. See {@link sz_xprofile_get_field_types()} for default core values.
 * @return object $value If field type unknown, returns SZ_XProfile_Field_Type_Textarea.
 *                       Otherwise returns an instance of the relevant child class of SZ_XProfile_Field_Type.
 */
function sz_xprofile_create_field_type( $type ) {

	$field = sz_xprofile_get_field_types();
	...
	if ( $class && class_exists( $class ) ) {
		...
		return new SZ_XProfile_Field_Type_Placeholder;
	}
}
...
function xprofile_insert_field( $args = '' ) {
	...
	if ( ! in_array( $r['type'], (array) sportszone()->profile->field_types ) ) {
		return false;
	}
 ...
 * @return SZ_XProfile_Field|null Field object if found, otherwise null.
 */
function xprofile_get_field( $field, $user_id = null, $get_data = true ) {
	if ( $field instanceof SZ_XProfile_Field ) {
		$_field = $field;
	} elseif ( is_object( $field ) ) {
		$_field = new SZ_XProfile_Field();
		$_field->fill_data( $field );
	} else {
		$_field = SZ_XProfile_Field::get_instance( $field, $user_id, $get_data );
	}
...
function xprofile_get_field_data( $field, $user_id = 0, $multi_format = 'array' ) {
	...
	$values = maybe_unserialize( SZ_XProfile_ProfileData::get_value_byid( $field_id, $user_id ) );
	...
	if ( is_array( $values ) ) {
		...
		/** This filter is documented in sz-xprofile/sz-xprofile-functions.php */
...
function xprofile_set_field_data( $field, $user_id, $value, $is_required = false ) {
	...
	$field_type     = SZ_XProfile_Field::get_type( $field_id );
	$field_type_obj = sz_xprofile_create_field_type( $field_type );
	 ...
	 * @param SZ_XProfile_Field_Type $field_type_obj Field type object.
	 */
	$value = apply_filters( 'sz_xprofile_set_field_data_pre_validate', $value, $field, $field_type_obj );
	...
	$field           = new SZ_XProfile_ProfileData();
...
function xprofile_set_field_visibility_level( $field_id = 0, $user_id = 0, $visibility_level = '' ) {
	...
	$allowed_values = sz_xprofile_get_visibility_levels();
	if ( !array_key_exists( $visibility_level, $allowed_values ) ) {
		return false;
	}

	// Stored in an array in usermeta.
	$current_visibility_levels = sz_get_user_meta( $user_id, 'sz_xprofile_visibility_levels', true );
	...
	return sz_update_user_meta( $user_id, 'sz_xprofile_visibility_levels', $current_visibility_levels );
}
...
function xprofile_get_field_visibility_level( $field_id = 0, $user_id = 0 ) {
	...
	$current_levels = sz_get_user_meta( $user_id, 'sz_xprofile_visibility_levels', true );
...
function xprofile_delete_field_data( $field = '', $user_id = 0 ) {
	...
	$field = new SZ_XProfile_ProfileData( $field_id, $user_id );
...
function xprofile_get_field_id_from_name( $field_name ) {
	return SZ_XProfile_Field::get_id_from_name( $field_name );
}
...
function xprofile_get_random_profile_data( $user_id, $exclude_fullname = true ) {
	$field_data = SZ_XProfile_ProfileData::get_random( $user_id, $exclude_fullname );
...
function xprofile_format_profile_field( $field_type, $field_value ) {
	...
	$field_value = sz_unserialize_profile_field( $field_value );
...
function xprofile_update_field_position( $field_id, $position, $field_group_id ) {
	return SZ_XProfile_Field::update_position( $field_id, $position, $field_group_id );
}
 ...
 * value of sportszone()->loggedin_user->fullname. However, in cases where
 ...
 *
 * Runs at sz_setup_globals:100 to ensure that all components have loaded their
...
function xprofile_override_user_fullnames() {
	...
	if ( ! sz_disable_profile_sync() ) {
		return;
	}

	if ( sz_loggedin_user_id() ) {
		sportszone()->loggedin_user->fullname = sz_core_get_user_displayname( sz_loggedin_user_id() );
	}

	if ( sz_displayed_user_id() ) {
		sportszone()->displayed_user->fullname = sz_core_get_user_displayname( sz_displayed_user_id() );
	}
}
add_action( 'sz_setup_globals', 'xprofile_override_user_fullnames', 100 );
...
function xprofile_avatar_upload_dir( $directory = 'avatars', $user_id = 0 ) {
	...
	$path      = sz_core_avatar_upload_path() . '/' . $directory. '/' . $user_id;
	$newbdir   = $path;
	$newurl    = sz_core_avatar_url() . '/' . $directory. '/' . $user_id;
	$newburl   = $newurl;
...
/**
 * When search_terms are passed to SZ_User_Query, search against xprofile fields.
 *
 * @since 2.0.0
 *
 * @param array         $sql   Clauses in the user_id SQL query.
 * @param SZ_User_Query $query User query object.
 * @return array
 */
function sz_xprofile_sz_user_query_search( $sql, SZ_User_Query $query ) {
	global $wpdb;
	...
	$search_terms_clean = sz_esc_like( wp_kses_normalize_entities( $query->query_vars['search_terms'] ) );
	...
	$matched_user_ids = $wpdb->get_col( $wpdb->prepare(
		"SELECT user_id FROM {$sz->profile->table_name_data} WHERE value LIKE %s OR value LIKE %s",
...
add_action( 'sz_user_query_uid_clauses', 'sz_xprofile_sz_user_query_search', 10, 2 );
...
function xprofile_sync_wp_profile( $user_id = 0 ) {
	...
	$fullname = xprofile_get_field_data( sz_xprofile_fullname_field_id(), $user_id );
	...
	sz_update_user_meta( $user_id, 'nickname',   $fullname  );
	sz_update_user_meta( $user_id, 'first_name', $firstname );
	sz_update_user_meta( $user_id, 'last_name',  $lastname  );

	wp_update_user( array( 'ID' => $user_id, 'display_name' => $fullname ) );
}
add_action( 'sz_core_signup_user',      'xprofile_sync_wp_profile' );
add_action( 'sz_core_activated_user',   'xprofile_sync_wp_profile' );
 ...
 */
function xprofile_sync_sz_profile( &$errors, $update, &$user ) {

	// Bail if profile syncing is disabled.
	if ( sz_disable_profile_sync() || ! $update || $errors->get_error_codes() ) {
		return;
	}

	xprofile_set_field_data( sz_xprofile_fullname_field_id(), $user->ID, $user->display_name );
}
add_action( 'user_profile_update_errors', 'xprofile_sync_sz_profile', 10, 3 );
 ...
 * @since 3.0.0
 *
 * @param SZ_XProfile_ProfileData $data Current instance of the profile data being saved.
 */
function xprofile_sync_wp_profile_on_single_field_set( $data ) {

	if ( sz_xprofile_fullname_field_id() !== $data->field_id ) {
		return;
	}
...
function xprofile_remove_data( $user_id ) {
	SZ_XProfile_ProfileData::delete_data_for_user( $user_id );
}
add_action( 'wpmu_delete_user',  'xprofile_remove_data' );
add_action( 'delete_user',       'xprofile_remove_data' );
add_action( 'sz_make_spam_user', 'xprofile_remove_data' );
 ...
 * @return bool True on success, false on failure.
 */
function sz_xprofile_delete_meta( $object_id, $object_type, $meta_key = false, $meta_value = false, $delete_all = false ) {
	global $wpdb;
	...
	add_filter( 'query', 'sz_xprofile_filter_meta_query' );
	...
	remove_filter( 'query', 'sz_xprofile_filter_meta_query' );
 ...
 */
function sz_xprofile_get_meta( $object_id, $object_type, $meta_key = '', $single = true ) {
	// Sanitize object type.
	if ( ! in_array( $object_type, array( 'group', 'field', 'data' ) ) ) {
		return false;
	}
 ...
 */
function sz_xprofile_update_meta( $object_id, $object_type, $meta_key, $meta_value, $prev_value = '' ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
	add_filter( 'query', 'sz_xprofile_filter_meta_query' );
 ...
 */
function sz_xprofile_add_meta( $object_id, $object_type, $meta_key, $meta_value, $unique = false ) {
	add_filter( 'query', 'sz_filter_metaid_column_name' );
	add_filter( 'query', 'sz_xprofile_filter_meta_query' );
 ...
 * @return bool|int
 */
function sz_xprofile_update_fieldgroup_meta( $field_group_id, $meta_key, $meta_value ) {
	return sz_xprofile_update_meta( $field_group_id, 'group', $meta_key, $meta_value );
}
 ...
 * @return bool|int
 */
function sz_xprofile_update_field_meta( $field_id, $meta_key, $meta_value ) {
	return sz_xprofile_update_meta( $field_id, 'field', $meta_key, $meta_value );
}
 ...
 * @return bool|int
 */
function sz_xprofile_update_fielddata_meta( $field_data_id, $meta_key, $meta_value ) {
	return sz_xprofile_update_meta( $field_data_id, 'data', $meta_key, $meta_value );
}
 ...
 * @return int Field ID.
 */
function sz_xprofile_fullname_field_id() {
	$id = wp_cache_get( 'fullname_field_id', 'sz_xprofile' );

	if ( false === $id ) {
		global $wpdb;

		$sz = sportszone();
		$id = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$sz->profile->table_name_fields} WHERE name = %s", addslashes( sz_xprofile_fullname_field_name() ) ) );

		wp_cache_set( 'fullname_field_id', $id, 'sz_xprofile' );
	}
 ...
 */
function sz_xprofile_fullname_field_name() {
	 ...
	 * @since 1.5.0
	 *
	 * @param string $value SZ_XPROFILE_FULLNAME_FIELD_NAME Full name field constant.
	 */
	return apply_filters( 'sz_xprofile_fullname_field_name', SZ_XPROFILE_FULLNAME_FIELD_NAME );
}
 ...
 * @return bool
 */
function sz_xprofile_is_richtext_enabled_for_field( $field_id = null ) {
	if ( ! $field_id ) {
	 ...
	 */
	return apply_filters( 'sz_xprofile_is_richtext_enabled_for_field', $enabled, $field_id );
}

/**
 * Get visibility levels out of the $sz global.
 *
 * @since 1.6.0
 *
 * @return array
 */
function sz_xprofile_get_visibility_levels() {

	/**
	 * Filters the visibility levels out of the $sz global.
	 *
	 * @since 1.6.0
	 *
	 * @param array $visibility_levels Array of visibility levels.
	 */
	return apply_filters( 'sz_xprofile_get_visibility_levels', sportszone()->profile->visibility_levels );
}
 ...
 */
function sz_xprofile_get_hidden_fields_for_user( $displayed_user_id = 0, $current_user_id = 0 ) {
	...
	if ( !$current_user_id ) {
		$current_user_id = sz_loggedin_user_id();
	}

	// @todo - This is where you'd swap out for current_user_can() checks
	$hidden_levels = sz_xprofile_get_hidden_field_types_for_user( $displayed_user_id, $current_user_id );
	$hidden_fields = sz_xprofile_get_fields_by_visibility_levels( $displayed_user_id, $hidden_levels );
	 ...
	 */
	return apply_filters( 'sz_xprofile_get_hidden_fields_for_user', $hidden_fields, $displayed_user_id, $current_user_id );
}
 ...
 * current field. (See sz_xprofile_get_hidden_fields_for_user().) This
 ...
 * @since 1.8.2
 *
 * @see sz_xprofile_get_hidden_fields_for_user()
 *
 * @param int $displayed_user_id The id of the user the profile fields belong to.
 * @param int $current_user_id   The id of the user viewing the profile.
 * @return array An array of visibility levels hidden to the current user.
 */
function sz_xprofile_get_hidden_field_types_for_user( $displayed_user_id = 0, $current_user_id = 0 ) {
	...
	if ( ! empty( $current_user_id ) ) {
		...
		if ( $displayed_user_id == $current_user_id || sz_current_user_can( 'sz_moderate' ) ) {
			$hidden_levels = array();

		// If the current user and displayed user are friends, show all.
		} elseif ( sz_is_active( 'friends' ) && friends_check_friendship( $displayed_user_id, $current_user_id ) ) {
	 ...
	 * @param int   $current_user_id   ID of the current user.
	 */
	return apply_filters( 'sz_xprofile_get_hidden_field_types_for_user', $hidden_levels, $displayed_user_id, $current_user_id );
}
 ...
 */
function sz_xprofile_get_fields_by_visibility_levels( $user_id, $levels = array() ) {
	if ( !is_array( $levels ) ) {
		$levels = (array)$levels;
	}

	$user_visibility_levels = sz_get_user_meta( $user_id, 'sz_xprofile_visibility_levels', true );

	// Parse the user-provided visibility levels with the default levels, which may take
	// precedence.
	$default_visibility_levels = SZ_XProfile_Group::fetch_default_visibility_levels();
 ...
 *              the datebox data.
 */
function sz_xprofile_maybe_format_datebox_post_data( $field_id ) {
